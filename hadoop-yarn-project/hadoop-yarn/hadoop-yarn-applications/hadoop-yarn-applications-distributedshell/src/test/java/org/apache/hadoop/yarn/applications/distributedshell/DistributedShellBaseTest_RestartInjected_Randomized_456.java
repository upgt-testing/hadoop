 ...
 */
public abstract class DistributedShellBaseTest_RestartInjected_Randomized_456 {
    ...
    protected static final int TEST_TIME_WINDOW_EXPIRE = (TEST_TIME_OUT * 90) / 100;

    private static final Logger LOG = LoggerFactory.getLogger(DistributedShellBaseTest_RestartInjected.class);

    private static final String APP_MASTER_JAR = JarFinder.getJar(ApplicationMaster.class);

    private static final int NUM_NMS = 1;

    // set the timeout of the yarnClient to be 95% of the globalTimeout.
    private static final String YARN_CLIENT_TIMEOUT = String.valueOf(TEST_TIME_WINDOW_EXPIRE);

    private static final String[] COMMON_ARGS = { "--jar", APP_MASTER_JAR, "--timeout", YARN_CLIENT_TIMEOUT, "--appname", "" };
    ...
    protected static String[] createArguments(Supplier<String> testNameProvider, String... args) {
    ...
    protected void waitForContainersLaunch(YarnClient client, int nContainers, AtomicReference<ApplicationAttemptReport> appAttemptReportRef, AtomicReference<List<ContainerReport>> containersListRef, AtomicReference<ApplicationAttemptId> appAttemptIdRef, AtomicReference<Throwable> thrownErrorRef) throws Exception {
    ...
    protected abstract void customizeConfiguration(YarnConfiguration config) throws Exception;

    protected String[] appendFlowArgsForTestDSShell(String[] args, boolean defaultFlow) {
        return args;
    }

    protected String[] appendDomainArgsForTestDSShell(String[] args, boolean haveDomain) {
        String[] result = args;
        if (haveDomain) {
    ...
    protected Client setAndGetDSClient(String appMasterMainClass, Configuration config) throws Exception {
        dsClient = new Client(appMasterMainClass, config);
        return dsClient;
    }

    protected void baseTestDSShell(boolean haveDomain, boolean defaultFlow) throws Exception {
    ...
    protected void checkTimeline(ApplicationId appId, boolean defaultFlow, boolean haveDomain, ApplicationReport appReport) throws Exception {
    ...
    protected void setUpYarnCluster(int numNodeManagers, YarnConfiguration yarnConfig) throws Exception {
        if (yarnCluster != null) {
            return;
        }
    ...
    protected void setupInternal(int numNodeManagers, YarnConfiguration yarnConfig) throws Exception {
        ...
        RestartFramework.at("after_cluster_setup").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        conf = yarnConfig;
