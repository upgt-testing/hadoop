...
import org.apache.hadoop.yarn.api.records.LogAggregationContextJVMInterface;
...
public class TestDistributedShell {
    ...
    private void setupInternal(int numNodeManager, float timelineVersion) throws Exception {
        ...
        if (yarnCluster == null) {
            yarnCluster = new MiniYARNClusterInJVM(TestDistributedShell.class.getSimpleName(), 1, numNodeManager, 1, 1);
            yarnCluster.init(conf);
            yarnCluster.start();
            conf.set(YarnConfiguration.TIMELINE_SERVICE_WEBAPP_ADDRESS, MiniYARNClusterInJVM.getHostname() + ":" + yarnCluster.getApplicationHistoryServer().getPort());
    ...
    private void checkTimelineV1(boolean haveDomain) throws Exception {
        ...
        org.apache.hadoop.yarn.api.records.timeline.TimelineEntity entity = yarnCluster.getApplicationHistoryServer().getTimelineStore().getEntity(entityId, ApplicationMaster.DSEntity.DS_CONTAINER.toString(), null);
    ...
    private void checkTimelineV2(boolean haveDomain, ApplicationId appId, boolean defaultFlow, ApplicationReport appReport) throws Exception {
        ...
        try {
            ...
            // on each publish.
            verifyEntityForTimelineV2(dsAppAttemptEntityFile, DSEvent.DS_APP_ATTEMPT_START.toString(), 1, 1, 0, true);
            // to avoid race condition of testcase, atleast check 40 times with sleep
            // of 50ms
            verifyEntityForTimelineV2(dsAppAttemptEntityFile, DSEvent.DS_APP_ATTEMPT_END.toString(), 1, 40, 50, true);
            ...
            // Check if required events are published and same idprefix is sent for
            // on each publish.
            verifyEntityForTimelineV2(dsContainerEntityFile, DSEvent.DS_CONTAINER_START.toString(), 1, 1, 0, true);
            // to avoid race condition of testcase, atleast check 40 times with sleep
            // of 50ms
            verifyEntityForTimelineV2(dsContainerEntityFile, DSEvent.DS_CONTAINER_END.toString(), 1, 40, 50, true);
            ...
            verifyEntityForTimelineV2(containerEntityFile, ContainerMetricsConstants.CREATED_EVENT_TYPE, 1, 1, 0, true);
            // to avoid race condition of testcase, atleast check 40 times with sleep
            // of 50ms
            verifyEntityForTimelineV2(containerEntityFile, ContainerMetricsConstants.FINISHED_EVENT_TYPE, 1, 40, 50, true);
            ...
            verifyEntityForTimelineV2(appEntityFile, ApplicationMetricsConstants.CREATED_EVENT_TYPE, 1, 1, 0, false);
            // to avoid race condition of testcase, atleast check 40 times with sleep
            // of 50ms
            verifyEntityForTimelineV2(appEntityFile, ApplicationMetricsConstants.FINISHED_EVENT_TYPE, 1, 40, 50, false);
            ...
            verifyEntityForTimelineV2(appAttemptEntityFile, AppAttemptMetricsConstants.REGISTERED_EVENT_TYPE, 1, 1, 0, true);
            verifyEntityForTimelineV2(appAttemptEntityFile, AppAttemptMetricsConstants.FINISHED_EVENT_TYPE, 1, 1, 0, true);
        } finally {
    ...
    private void verifyEntityForTimelineV2(File entityFile, String expectedEvent, long numOfExpectedEvent, int checkTimes, long sleepTime, boolean checkIdPrefix) throws IOException, InterruptedException {
        ...
        Assert.assertEquals("Unexpected number of " + expectedEvent + " event published.", numOfExpectedEvent, actualCount);
    }
   ...
   */
    private boolean checkHostname(String appHostname) throws Exception {
        ...
        return (compareFQDNs(appHostnameParts[0], hostnameParts[0]) && checkIPs(hostnameParts[0], hostnameParts[1], appHostnameParts[1]));
    }
    ...
    public void testDSRestartWithPreviousRunningContainers() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--keep_containers_across_application_attempts" };
        LOG.info("Initializing DS Client");
   ...
   */
    ...
    public void testDSAttemptFailuresValidityIntervalSucess() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "2500" };
        LOG.info("Initializing DS Client");
   ...
   */
    ...
    public void testDSAttemptFailuresValidityIntervalFailed() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "15000" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
        Client client = new Client(TestDSSleepingAppMaster.class.getName(), new Configuration(conf));
    ...
    public void testSpecifyingLogAggregationContext() throws Exception {
        ...
        String[] args = { "--jar", APPMASTER_JAR, "--shell_command", "echo", "--rolling_log_pattern", regex };
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
    ...
    public void testDSShellWithCommands() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", "\"echo output_ignored;echo output_expected\"", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1" };
        LOG.info("Initializing DS Client");
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = client.init(args);
        Assert.assertTrue(initSuccess);
    ...
    public void testDSShellWithInvalidArgs() throws Exception {
        ...
        try {
            String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1", "--shell_command", "date", "--container_type", "UNSUPPORTED_TYPE" };
            client.init(args);
            Assert.fail("Exception is expected");
        } catch (IllegalArgumentException e) {
    ...
    public void testDSShellWithEnforceExecutionType() throws Exception {
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        try {
            String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1", "--shell_command", "date", "--container_type", "OPPORTUNISTIC", "--enforce_execution_type" };
            client.init(args);
            final AtomicBoolean result = new AtomicBoolean(false);
            Thread t = new Thread() {
    ...
    private void waitForContainersLaunch(YarnClient client, int nContainers) throws Exception {
    ...
    public void doTestDistributedShellWithResources(boolean largeContainers) throws Exception {
        Resource clusterResource = yarnCluster.getResourceManager().getResourceScheduler().getClusterResource();
        String masterMemoryString = "1 Gi";
        String containerMemoryString = "512 Mi";
        long masterMemory = 1024;
        long containerMemory = 512;
        Assume.assumeTrue("The cluster doesn't have enough memory for this test", clusterResource.getMemorySize() >= masterMemory + containerMemory);
        ...
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "memory=" + masterMemoryString + ",vcores=1", "--container_resources", "memory=" + containerMemoryString + ",vcores=1" };
        LOG.info("Initializing DS Client");
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        Assert.assertTrue(client.init(args));
        LOG.info("Running DS Client");
        ...
        while (true) {
            ...
            List<ContainerReport> containers = yarnClient.getContainers(appAttemptReport.getApplicationAttemptId());
            ...
            for (ContainerReport container : containers) {
                if (!container.getContainerId().equals(amContainerId)) {
                    ...
                    Assert.assertEquals(containerMemory, containerResource.getMemorySize());
    ...
    public void testDistributedShellAMResourcesWithIllegalArguments() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "memory-mb=invalid" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        client.init(args);
    }
    ...
    public void testDistributedShellAMResourcesWithMissingArgumentValue() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        client.init(args);
    }
    ...
    public void testDistributedShellAMResourcesWithUnknownResource() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "unknown-resource=5" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        client.init(args);
        client.run();
    }
    ...
    public void testDistributedShellNonExistentQueue() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--queue", "non-existent-queue" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        client.init(args);
        client.run();
    }
    ...
    @Test
    public void testDSRestartWithPreviousRunningContainers_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--keep_containers_across_application_attempts" };
        LOG.info("Initializing DS Client");
    ...
    @Test
    public void testDSRestartWithPreviousRunningContainers_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--keep_containers_across_application_attempts" };
        LOG.info("Initializing DS Client");
        Client client = new Client(TestDSFailedAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
    ...
    @Test
    public void testDSRestartWithPreviousRunningContainers_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--keep_containers_across_application_attempts" };
        LOG.info("Initializing DS Client");
        Client client = new Client(TestDSFailedAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
        client.init(args);
    ...
    @Test
    public void testDSRestartWithPreviousRunningContainers_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--keep_containers_across_application_attempts" };
        LOG.info("Initializing DS Client");
        Client client = new Client(TestDSFailedAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
        client.init(args);
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalSucess_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "2500" };
        LOG.info("Initializing DS Client");
        yarnCluster.upgradeAllNodes();
        Configuration conf = yarnCluster.getConfig();
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalSucess_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "2500" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalSucess_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "2500" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalSucess_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "2500" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalFailed_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "15000" };
        LOG.info("Initializing DS Client");
        yarnCluster.upgradeAllNodes();
        Configuration conf = yarnCluster.getConfig();
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalFailed_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "15000" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalFailed_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "15000" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
   ...
   */
    @Test
    public void testDSAttemptFailuresValidityIntervalFailed_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", getSleepCommand(8), "--master_memory", "512", "--container_memory", "128", "--attempt_failures_validity_interval", "15000" };
        LOG.info("Initializing DS Client");
        Configuration conf = yarnCluster.getConfig();
        conf.setInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS, 2);
    ...
    @Test
    public void testDSShellWithCustomLogPropertyFile_withUpgrade20() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customLogProperty = new File(tmpDir, "custom_log4j.properties");
    ...
    @Test
    public void testDSShellWithCustomLogPropertyFile_withUpgrade40() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customLogProperty = new File(tmpDir, "custom_log4j.properties");
    ...
    @Test
    public void testDSShellWithCustomLogPropertyFile_withUpgrade60() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customLogProperty = new File(tmpDir, "custom_log4j.properties");
    ...
    @Test
    public void testDSShellWithCustomLogPropertyFile_withUpgrade80() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customLogProperty = new File(tmpDir, "custom_log4j.properties");
    ...
    @Test
    public void testSpecifyingLogAggregationContext_withUpgrade20() throws Exception {
        String regex = ".*(foo|bar)\\d";
        String[] args = { "--jar", APPMASTER_JAR, "--shell_command", "echo", "--rolling_log_pattern", regex };
        yarnCluster.upgradeAllNodes();
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
    ...
    @Test
    public void testSpecifyingLogAggregationContext_withUpgrade40() throws Exception {
        String regex = ".*(foo|bar)\\d";
        String[] args = { "--jar", APPMASTER_JAR, "--shell_command", "echo", "--rolling_log_pattern", regex };
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
    ...
    @Test
    public void testSpecifyingLogAggregationContext_withUpgrade60() throws Exception {
        String regex = ".*(foo|bar)\\d";
        String[] args = { "--jar", APPMASTER_JAR, "--shell_command", "echo", "--rolling_log_pattern", regex };
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        Assert.assertTrue(client.init(args));
    ...
    @Test
    public void testSpecifyingLogAggregationContext_withUpgrade80() throws Exception {
        String regex = ".*(foo|bar)\\d";
        String[] args = { "--jar", APPMASTER_JAR, "--shell_command", "echo", "--rolling_log_pattern", regex };
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        Assert.assertTrue(client.init(args));
    ...
    @Test
    public void testDSShellWithMultipleArgs_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "4", "--shell_command", "echo", "--shell_args", "HADOOP YARN MAPREDUCE HDFS", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1" };
        LOG.info("Initializing DS Client");
        yarnCluster.upgradeAllNodes();
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
    ...
    @Test
    public void testDSShellWithMultipleArgs_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "4", "--shell_command", "echo", "--shell_args", "HADOOP YARN MAPREDUCE HDFS", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1" };
        LOG.info("Initializing DS Client");
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = client.init(args);
    ...
    @Test
    public void testDSShellWithMultipleArgs_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "4", "--shell_command", "echo", "--shell_args", "HADOOP YARN MAPREDUCE HDFS", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1" };
        LOG.info("Initializing DS Client");
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = client.init(args);
    ...
    @Test
    public void testDSShellWithMultipleArgs_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "4", "--shell_command", "echo", "--shell_args", "HADOOP YARN MAPREDUCE HDFS", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1" };
        LOG.info("Initializing DS Client");
        final Client client = new Client(new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = client.init(args);
    ...
    @Test
    public void testDSShellWithShellScript_withUpgrade20() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customShellScript = new File(tmpDir, "custom_script.sh");
    ...
    @Test
    public void testDSShellWithShellScript_withUpgrade40() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customShellScript = new File(tmpDir, "custom_script.sh");
    ...
    @Test
    public void testDSShellWithShellScript_withUpgrade60() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customShellScript = new File(tmpDir, "custom_script.sh");
    ...
    @Test
    public void testDSShellWithShellScript_withUpgrade80() throws Exception {
        final File basedir = new File("target", TestDistributedShell.class.getName());
        final File tmpDir = new File(basedir, "tmpDir");
        tmpDir.mkdirs();
        final File customShellScript = new File(tmpDir, "custom_script.sh");
    ...
    @Test
    public void testDSShellWithInvalidArgs_withUpgrade20() throws Exception {
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        LOG.info("Initializing DS Client with no args");
        try {
            client.init(new String[] {});
    ...
    @Test
    public void testDSShellWithInvalidArgs_withUpgrade40() throws Exception {
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        LOG.info("Initializing DS Client with no args");
        try {
            client.init(new String[] {});
    ...
    @Test
    public void testDSShellWithInvalidArgs_withUpgrade60() throws Exception {
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        LOG.info("Initializing DS Client with no args");
        try {
            client.init(new String[] {});
    ...
    @Test
    public void testDSShellWithInvalidArgs_withUpgrade80() throws Exception {
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        LOG.info("Initializing DS Client with no args");
        try {
            client.init(new String[] {});
    ...
    @Test
    public void testContainerLaunchFailureHandling_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--container_memory", "128" };
        LOG.info("Initializing DS Client");
        yarnCluster.upgradeAllNodes();
        Client client = new Client(ContainerLaunchFailAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
    ...
    @Test
    public void testContainerLaunchFailureHandling_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--container_memory", "128" };
        LOG.info("Initializing DS Client");
        Client client = new Client(ContainerLaunchFailAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
    ...
    @Test
    public void testContainerLaunchFailureHandling_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--container_memory", "128" };
        LOG.info("Initializing DS Client");
        Client client = new Client(ContainerLaunchFailAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = client.init(args);
    ...
    @Test
    public void testContainerLaunchFailureHandling_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--container_memory", "128" };
        LOG.info("Initializing DS Client");
        Client client = new Client(ContainerLaunchFailAppMaster.class.getName(), new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = client.init(args);
    ...
    @Test
    public void testDebugFlag_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1", "--debug" };
        LOG.info("Initializing DS Client");
        yarnCluster.upgradeAllNodes();
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
    ...
    @Test
    public void testDebugFlag_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1", "--debug" };
        LOG.info("Initializing DS Client");
        yarnCluster.upgradeAllNodes();
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
    ...
    @Test
    public void testDebugFlag_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1", "--debug" };
        LOG.info("Initializing DS Client");
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
    ...
    @Test
    public void testDebugFlag_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "2", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_memory", "512", "--master_vcores", "2", "--container_memory", "128", "--container_vcores", "1", "--debug" };
        LOG.info("Initializing DS Client");
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        Assert.assertTrue(client.init(args));
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellAMResourcesWithIllegalArguments_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "memory-mb=invalid" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellAMResourcesWithIllegalArguments_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "memory-mb=invalid" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellAMResourcesWithIllegalArguments_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "memory-mb=invalid" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellAMResourcesWithIllegalArguments_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "memory-mb=invalid" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = MissingArgumentException.class)
    public void testDistributedShellAMResourcesWithMissingArgumentValue_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = MissingArgumentException.class)
    public void testDistributedShellAMResourcesWithMissingArgumentValue_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = MissingArgumentException.class)
    public void testDistributedShellAMResourcesWithMissingArgumentValue_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = MissingArgumentException.class)
    public void testDistributedShellAMResourcesWithMissingArgumentValue_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = ResourceNotFoundException.class)
    public void testDistributedShellAMResourcesWithUnknownResource_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "unknown-resource=5" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = ResourceNotFoundException.class)
    public void testDistributedShellAMResourcesWithUnknownResource_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "unknown-resource=5" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = ResourceNotFoundException.class)
    public void testDistributedShellAMResourcesWithUnknownResource_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "unknown-resource=5" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = ResourceNotFoundException.class)
    public void testDistributedShellAMResourcesWithUnknownResource_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--master_resources", "unknown-resource=5" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        client.init(args);
        yarnCluster.upgradeAllNodes();
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellNonExistentQueue_withUpgrade20() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--queue", "non-existent-queue" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellNonExistentQueue_withUpgrade40() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--queue", "non-existent-queue" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellNonExistentQueue_withUpgrade60() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--queue", "non-existent-queue" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        yarnCluster.upgradeAllNodes();
        client.init(args);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testDistributedShellNonExistentQueue_withUpgrade80() throws Exception {
        String[] args = { "--jar", APPMASTER_JAR, "--num_containers", "1", "--shell_command", Shell.WINDOWS ? "dir" : "ls", "--queue", "non-existent-queue" };
        Client client = new Client(new Configuration(yarnCluster.getConfig()));
        client.init(args);
        yarnCluster.upgradeAllNodes();
