 ...
 * exceeded.
 */
public class TestCleanupAfterKill_RestartInjected_Randomized_456 extends ServiceTestUtils {

    private static final Logger LOG = LoggerFactory.getLogger(TestCleanupAfterKill_RestartInjected.class);
    ...
    public void testRegistryCleanedOnLifetimeExceeded() throws Exception {
        ...
        RestartFramework.at("after_app_create").on(getYarnCluster()).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cluster_setup").on(getYarnCluster()).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
