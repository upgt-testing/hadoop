...
public class TestUnmanagedAMLauncher_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestUnmanagedAMLauncher_RestartInjected.class);
    ...
    public static void setup() throws InterruptedException, IOException {
        ...
        if (yarnCluster == null) {
            yarnCluster = new MiniYARNCluster(TestUnmanagedAMLauncher_RestartInjected.class.getSimpleName(), 1, 1, 1);
    ...
    public void testUMALauncher() throws Exception {
        RestartFramework.at("after_launcher_run").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        String[] args = { "--classpath", classpath, "--queue", "default", "--cmd", javaHome + "/bin/java -Xmx512m " + TestUnmanagedAMLauncher_RestartInjected.class.getCanonicalName() + " success" };
        LOG.info("Initializing Launcher");
        RestartFramework.at("after_launcher_init").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testUMALauncherError() throws Exception {
        ...
        String[] args = { "--classpath", classpath, "--queue", "default", "--cmd", javaHome + "/bin/java -Xmx512m " + TestUnmanagedAMLauncher_RestartInjected.class.getCanonicalName() + " failure" };
        LOG.info("Initializing Launcher");
        UnmanagedAMLauncher launcher = new UnmanagedAMLauncher(new Configuration(yarnCluster.getConfig()));
        boolean initSuccess = launcher.init(args);
        Assert.assertTrue(initSuccess);
