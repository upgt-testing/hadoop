 ...
 */
public class TestDiskFailures_RestartInjected {
  ...
  private static final File testDir = new File("target",
      TestDiskFailures_RestartInjected.class.getName()).getAbsoluteFile();
  private static final File localFSDirBase = new File(testDir,
      TestDiskFailures_RestartInjected.class.getName() + "-localDir");
  ...
  private void testDirsFailures(boolean localORLogDirs) throws IOException {
    ...
    yarnCluster = new MiniYARNCluster(TestDiskFailures_RestartInjected.class.getName(),
    ...
    RestartFramework.at("after_initial_health_check")
        .on(yarnCluster)
        .restart("nodemanager")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    expectedDirs = dirs[0] + "," + dirs[1] + ","
                                 + dirs[3];
    verifyDisksHealth(localORLogDirs, expectedDirs, true);
    RestartFramework.at("after_first_disk_failure")
        .on(yarnCluster)
        .restart("nodemanager")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_node_unhealthy")
        .on(yarnCluster)
        .restart("resourcemanager")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
