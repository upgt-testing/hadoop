 ...
 */
public class TestDiskFailures_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestDiskFailures_RestartInjected.class);
   ...
   */
    ...
    private static final File testDir = new File("target", TestDiskFailures_RestartInjected.class.getName()).getAbsoluteFile();

    private static final File localFSDirBase = new File(testDir, TestDiskFailures_RestartInjected.class.getName() + "-localDir");
    ...
    private void testDirsFailures(boolean localORLogDirs) throws IOException {
        ...
        RestartFramework.at("after_first_disk_failure").on(yarnCluster).restart("nodemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        LOG.info("Starting up YARN cluster");
        RestartFramework.at("after_initial_health_check").on(yarnCluster).restart("nodemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        yarnCluster = new MiniYARNCluster(TestDiskFailures_RestartInjected.class.getName(), 1, numLocalDirs, numLogDirs);
        ...
        RestartFramework.at("after_cluster_start").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_node_unhealthy").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
