...
public class TestMiniYARNClusterForHA_RestartInjected_Randomized_123 {
    ...
    public void testClusterWorks() throws YarnException, InterruptedException {
        ...
        RestartFramework.at("before_nm_connect").on(cluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_nm_connect").on(cluster).restart("nodemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
}
