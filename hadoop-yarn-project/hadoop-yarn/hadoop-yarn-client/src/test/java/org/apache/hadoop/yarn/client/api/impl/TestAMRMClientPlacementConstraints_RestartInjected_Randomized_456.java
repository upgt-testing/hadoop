 ...
 */
public class TestAMRMClientPlacementConstraints_RestartInjected_Randomized_456 extends BaseAMRMClientTest {
    ...
    public void testAMRMClientWithPlacementConstraintsByPlacementProcessor() throws Exception {
        ...
        asyncClient.addSchedulingRequests(Arrays.asList(// 4 reqs with numAlloc = 1
        schedulingRequest(1, 1, 1, 1, 512, "foo"), schedulingRequest(1, 1, 2, 1, 512, "foo"), schedulingRequest(1, 1, 3, 1, 512, "foo"), schedulingRequest(1, 1, 4, 1, 512, "foo"), // 1 req with numAlloc = 4
        schedulingRequest(4, 1, 5, 1, 512, "bar")));
        RestartFramework.at("after_scheduling_request").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // kick the scheduler
    ...
    public void testAMRMClientWithPlacementConstraintsByScheduler() throws Exception {
        ...
        asyncClient.addSchedulingRequests(Arrays.asList(// 4 reqs with numAlloc = 1
        schedulingRequest(1, 1, 1, 1, 512, "foo"), schedulingRequest(1, 1, 2, 1, 512, "foo"), schedulingRequest(1, 1, 3, 1, 512, "foo"), schedulingRequest(1, 1, 4, 1, 512, "foo"), // 1 req with numAlloc = 4
        schedulingRequest(4, 1, 5, 1, 512, "bar"), // 1 empty tag
   ...
   * 3. Set tag as null
   */
    void testEmptyKeyOfOutstandingSchedRequests() {
        AMRMClient<AMRMClient.ContainerRequest> amClient = AMRMClient.<AMRMClient.ContainerRequest>createAMRMClient();
    ...
    private static SchedulingRequest schedulingRequest(int numAllocations, int priority, long allocReqId, int cores, int mem, Set<String> tags) {
        return schedulingRequest(numAllocations, priority, allocReqId, cores, mem, ExecutionType.GUARANTEED, tags);
    }

    private static SchedulingRequest schedulingRequest(int numAllocations, int priority, long allocReqId, int cores, int mem, ExecutionType execType, Set<String> tags) {
    ...
    private static SchedulingRequest schedulingRequest(int numAllocations, int priority, long allocReqId, int cores, int mem, ExecutionType execType) {
