...
public class TestYarnClient extends ParameterizedSchedulerTestBase {
    ...
    private static class MockYarnClient extends YarnClientImpl {
        ...
        protected List<ContainerReport> getContainersOnAHSFail(ApplicationAttemptId appAttemptId) throws YarnException, IOException {
            when(mockContainersResponse.getContainerList()).thenReturn(getContainersReport(appAttemptId));
            when(historyClient.getContainers(any(ApplicationAttemptId.class))).thenThrow(new ApplicationNotFoundException(appAttemptId.getApplicationId() + " does not exist in the timeline store"));
            return super.getContainers(appAttemptId);
        }
