...
public class TestResourceTrackerOnHA_RestartInjected_Randomized_123 extends ProtocolHATestBase {
    ...
    public void testResourceTrackerOnHA() throws Exception {
        ...
        RestartFramework.at("after_nm_connect").on(cluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_heartbeat").on(cluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    private boolean waitForNodeManagerToConnect(final int maxTime, final NodeId nodeId) throws Exception {
