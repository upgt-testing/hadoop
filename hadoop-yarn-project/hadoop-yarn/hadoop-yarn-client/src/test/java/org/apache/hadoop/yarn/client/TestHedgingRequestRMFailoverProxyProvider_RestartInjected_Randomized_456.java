...
public class TestHedgingRequestRMFailoverProxyProvider_RestartInjected_Randomized_456 {
    ...
    public void testHedgingRequestProxyProvider() throws Exception {
        ...
        try (MiniYARNCluster cluster = new MiniYARNCluster("testHedgingRequestProxyProvider", 5, 0, 1, 1)) {
            ...
            RestartFramework.at("after_first_active").on(cluster).restart("resourcemanager").withIndex(4).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_failover").on(cluster).restart("resourcemanager").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            validateActiveRM(client);
        }
    }
