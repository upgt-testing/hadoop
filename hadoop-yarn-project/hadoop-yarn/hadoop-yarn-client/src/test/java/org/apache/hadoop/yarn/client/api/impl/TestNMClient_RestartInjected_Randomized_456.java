...
public class TestNMClient_RestartInjected_Randomized_456 {
    ...
    public void testNMClientNoCleanupOnStop() throws YarnException, IOException {
        RestartFramework.at("after_container_allocate").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_am_register").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        stopNmClient(false);
        assertFalse(nmClient.startedContainers.isEmpty());
        RestartFramework.at("before_am_finish").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNMClient() throws YarnException, IOException {
        ...
        rmClient.unregisterApplicationMaster(FinalApplicationStatus.SUCCEEDED, null, null);
        assertFalse(nmClient.startedContainers.isEmpty());
        RestartFramework.at("after_container_management").on(yarnCluster).restart("nodemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_am_finish").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_am_register").on(yarnCluster).restart("resourcemanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testContainerManagement(NMClientImpl client, Set<Container> containers) throws YarnException, IOException {
    ...
    private void testContainer(NMClientImpl client, int i, Container container, ContainerLaunchContext clc, List<Integer> exitCode) throws YarnException, IOException {
     ...
     */
    private void waitForContainerTransitionCount(Container container, org.apache.hadoop.yarn.server.nodemanager.containermanager.container.ContainerState state, long transitions) throws YarnException {
    ...
    private void testReInitializeContainer(ContainerId containerId, ContainerLaunchContext clc, boolean autoCommit) throws YarnException, IOException {
        try {
