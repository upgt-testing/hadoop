 ...
 */
public class TestRouterRpcStoragePolicySatisfier_RestartInjected {
  ...
  public void testStoragePolicySatisfier() throws Exception {
    ...
    RestartFramework.at("after_wait_disk_storage")
        .on(cluster.getCluster())
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_get_storage_policy")
        .on(cluster.getCluster())
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    DFSTestUtil.waitExpectedStorageType(file, StorageType.ARCHIVE, 1, 20000,
        (DistributedFileSystem) routerFS);
    RestartFramework.at("after_wait_archive_storage")
        .on(cluster.getCluster())
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
