 ...
 * Store.
 */
public class TestRouterNamenodeHeartbeat_RestartInjected_Randomized_123 {
    ...
    public void testNamenodeHeartbeatService() throws IOException {
        ...
        RestartFramework.at("after_heartbeat_service_init").on(testCluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_heartbeat_service_start").on(testCluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testHearbeat() throws InterruptedException, IOException {
        ...
        RestartFramework.at("after_initial_ha_setup").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_failover").on(cluster.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(NAMENODES[1], standby.getNamenodeId());
    }
}
