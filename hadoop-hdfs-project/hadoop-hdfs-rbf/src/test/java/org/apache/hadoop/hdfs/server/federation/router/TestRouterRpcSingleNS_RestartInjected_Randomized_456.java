 ...
 * single nameService.
 */
public class TestRouterRpcSingleNS_RestartInjected_Randomized_456 {
    ...
    public void testGetCurrentTXIDandRollEdits() throws IOException {
        RestartFramework.at("after_roll_edits").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Long rollEdits = routerProtocol.rollEdits();
        Long currentTXID = routerProtocol.getCurrentEditLogTxid();
        RestartFramework.at("after_get_txid").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(rollEdits, currentTXID);
    }

    @Test
    public void testSaveNamespace() throws IOException {
        RestartFramework.at("after_save_namespace").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.getCluster().getFileSystem().setSafeMode(HdfsConstants.SafeModeAction.SAFEMODE_ENTER);
        RestartFramework.at("after_enter_safemode").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
