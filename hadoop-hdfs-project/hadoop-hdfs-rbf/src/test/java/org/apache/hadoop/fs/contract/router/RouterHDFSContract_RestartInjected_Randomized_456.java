 ...
 */
public class RouterHDFSContract_RestartInjected_Randomized_456 extends HDFSContract {

    public static final int BLOCK_SIZE = AbstractFSContractTestBase.TEST_FILE_LEN;

    private static MiniRouterDFSCluster cluster;

    public RouterHDFSContract_RestartInjected_Randomized_456(Configuration conf) {
        super(conf);
    }
    ...
    public static void createCluster(boolean ha, int numNameServices, boolean security) throws IOException {
        try {
            ...
            RestartFramework.at("after_cluster_start").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Start routers with only an RPC service
            cluster.startRouters();
            RestartFramework.at("after_routers_start").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_active_namespaces").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } catch (Exception e) {
            destroyCluster();
