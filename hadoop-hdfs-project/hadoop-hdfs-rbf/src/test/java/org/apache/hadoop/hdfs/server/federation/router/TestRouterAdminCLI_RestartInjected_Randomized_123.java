 ...
 */
public class TestRouterAdminCLI_RestartInjected_Randomized_123 {
    ...
    public void testAddMountTable() throws Exception {
        ...
        RestartFramework.at("after_add_mount_table_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_update_mount_table_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRemoveMountTable() throws Exception {
        ...
        assertEquals(0, ToolRunner.run(admin, argv));
        RestartFramework.at("after_remove_mount_table_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSetAndClearQuota() throws Exception {
        ...
        RestartFramework.at("after_clear_quota_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // test clrQuota command
        argv = new String[] { "-clrQuota", src };
        assertEquals(0, ToolRunner.run(admin, argv));
        stateStore.loadCache(MountTableStoreImpl.class, true);
        RestartFramework.at("after_set_quota_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        getResponse = client.getMountTableManager().getMountTableEntries(getRequest);
        mountTable = getResponse.getEntries().get(0);
        quotaUsage = mountTable.getQuota();
    ...
    public void testManageSafeMode() throws Exception {
        ...
        RestartFramework.at("after_leave_safemode_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.reset();
        RestartFramework.at("after_enter_safemode_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNameserviceManager() throws Exception {
        ...
        stateStore.loadCache(DisabledNameserviceStoreImpl.class, true);
        assertEquals(0, ToolRunner.run(admin, new String[] { "-getDisabledNameservices" }));
        RestartFramework.at("after_disable_nameservice_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse("ns0 should not be reported: " + out, out.toString().contains("ns0"));
        RestartFramework.at("after_enable_nameservice_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Wrong commands
        System.setErr(new PrintStream(err));
    ...
    public void testRefreshMountTableCache() throws Exception {
        ...
        RestartFramework.at("after_refresh_cache_cli").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
