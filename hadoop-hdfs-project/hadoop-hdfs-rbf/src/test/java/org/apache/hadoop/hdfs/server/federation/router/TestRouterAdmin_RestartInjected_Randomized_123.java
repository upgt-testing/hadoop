 ...
 */
public class TestRouterAdmin_RestartInjected_Randomized_123 {
    ...
    public void testAddDuplicateMountTable() throws IOException {
        MountTable newEntry = MountTable.newInstance("/testpath", Collections.singletonMap("ns0", "/testdir"), Time.now(), Time.now());
        RestartFramework.at("after_add_duplicate_check").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RouterClient client = routerContext.getAdminClient();
        MountTableManager mountTable = client.getMountTableManager();
        // Existing mount table size
    ...
    public void testAddReadOnlyMountTable() throws IOException {
        ...
        RouterClient client = routerContext.getAdminClient();
        MountTableManager mountTable = client.getMountTableManager();
        RestartFramework.at("after_add_readonly").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Existing mount table size
        List<MountTable> records = getMountTableEntries(mountTable);
        assertEquals(records.size(), mockMountTable.size());
        // Add
        ...
        RestartFramework.at("after_remove_readonly").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testAddOrderMountTable(final DestinationOrder order) throws IOException {
        ...
        assertTrue(addResponse.getStatus());
        RestartFramework.at("after_add_order_" + order).on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Check that we have the read only entry
    ...
    public void testRemoveMountTable() throws IOException {
        ...
        // Existing mount table size
        List<MountTable> entries1 = getMountTableEntries(mountTable);
        RestartFramework.at("after_remove_mount_table").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(entries1.size(), mockMountTable.size());
        // Remove an entry
    ...
    public void testEditMountTable() throws IOException {
        ...
        RestartFramework.at("after_update_mount_table").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetMountTable() throws IOException {
        ...
        RestartFramework.at("before_get_mount_table").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetSingleMountTableEntry() throws IOException {
        RestartFramework.at("before_get_single_entry").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNameserviceManager() throws IOException {
        ...
        RestartFramework.at("after_enable_ns").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disable_ns").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
