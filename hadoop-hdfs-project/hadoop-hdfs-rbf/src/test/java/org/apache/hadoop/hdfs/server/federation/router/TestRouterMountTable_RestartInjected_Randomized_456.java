 ...
 */
public class TestRouterMountTable_RestartInjected_Randomized_456 {
    ...
    public void testReadOnly() throws Exception {
        ...
        RestartFramework.at("after_mkdir").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FileStatus dirStatusNn = nnFs0.getFileStatus(new Path("/testdir/newdir"));
        assertTrue(dirStatusNn.isDirectory());
        RestartFramework.at("after_mount_entries").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListFilesTime() throws Exception {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_mount_entries_setup").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Create test dir in NN
            assertTrue(nnFs0.mkdirs(new Path("/newdir")));
            RestartFramework.at("after_nn_mkdir").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetMountPointStatusWithIOException() throws IOException, InterruptedException {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_mount_entries_with_permissions").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(nnFs0.mkdirs(new Path("/test1")));
            nnFs0.setPermission(new Path("/test1"), FsPermission.createImmutable((short) 0700));
            RestartFramework.at("after_permission_setup").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetMountPointStatus() throws IOException {
        ...
        RestartFramework.at("after_nested_mount_entry").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListWhenDisableDefaultMountTable() throws IOException {
        RestartFramework.at("after_mount_entries_disable_default").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        mountTable.setDefaultNSEnable(false);
        /**
    ...
    public void testMountTablePermissionsNoDest() throws IOException {
        ...
        assertTrue(addMountTable(addEntry));
        RestartFramework.at("after_mount_entry_no_dest").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMountTablePermissionsWithDest() throws IOException {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_mount_entry_with_dest").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_dest_permissions_set").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            FileStatus[] list = routerFs.listStatus(new Path("/"));
    ...
    public void testMountTablePermissionsMultiDest() throws IOException {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_multi_dest_mount_entry").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/tmp/testdir"));
            nnFs0.setOwner(new Path("/tmp/testdir"), "Aowner", "Agroup");
            nnFs0.setPermission(new Path("/tmp/testdir"), FsPermission.createImmutable((short) 775));
            RestartFramework.at("after_ns0_dest_setup").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_ns1_dest_setup").on(cluster.getCluster()).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            FileStatus[] list = routerFs.listStatus(new Path("/"));
            assertEquals("Agroup", list[0].getGroup());
            assertEquals("Aowner", list[0].getOwner());
            assertEquals((short) 775, list[0].getPermission().toShort());
    ...
    public void testMountTablePermissionsMultiDestDifferentPerm() throws IOException {
        try {
            ...
            MountTable addEntry = MountTable.newInstance("/testdir", destMap);
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_multi_dest_diff_perm_mount").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/tmp/testdir"));
            nnFs0.setOwner(new Path("/tmp/testdir"), "Aowner", "Agroup");
            nnFs0.setPermission(new Path("/tmp/testdir"), FsPermission.createImmutable((short) 775));
            RestartFramework.at("after_ns0_dest_diff_perm_setup").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs1.mkdirs(new Path("/tmp/testdir01"));
            ...
            RestartFramework.at("after_ns1_dest_diff_perm_setup").on(cluster.getCluster()).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            FileStatus[] list = routerFs.listStatus(new Path("/"));
    ...
    public void testMountPointResolved() throws IOException {
        ...
        RestartFramework.at("after_mount_entry_with_owner_group").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMountPointChildren() throws IOException {
        try {
            MountTable addEntry = MountTable.newInstance("/testdir", Collections.singletonMap("ns0", "/tmp/testdir"));
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_mount_entry_for_children").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/tmp/testdir"));
            nnFs0.mkdirs(new Path("/tmp/testdir/1"));
            nnFs0.mkdirs(new Path("/tmp/testdir/2"));
            RestartFramework.at("after_children_creation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            FileStatus[] finfo1 = routerFs.listStatus(new Path("/"));
    ...
    public void testMountPointChildrenMultiDest() throws IOException {
        try {
            ...
            MountTable addEntry = MountTable.newInstance("/testdir", destMap);
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_multi_dest_children_mount").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/tmp/testdir"));
            nnFs0.mkdirs(new Path("/tmp/testdir"));
            nnFs1.mkdirs(new Path("/tmp/testdir01"));
            nnFs0.mkdirs(new Path("/tmp/testdir/1"));
            nnFs1.mkdirs(new Path("/tmp/testdir01/1"));
            RestartFramework.at("after_multi_dest_children_creation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            FileStatus[] finfo1 = routerFs.listStatus(new Path("/"));
            assertEquals(2, ((HdfsFileStatus) finfo1[0]).getChildrenNum());
        } finally {
            nnFs0.delete(new Path("/tmp"), true);
    ...
    public void testPathInException() throws Exception {
        ...
        RestartFramework.at("after_hash_all_mount_entry").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(addMountTable(addEntry));
    ...
    public void testGetListingWithTrailingSlash() throws IOException {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_trailing_slash_mount_entries").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/testlist/tmp0"));
            nnFs1.mkdirs(new Path("/testlist/tmp1"));
            RestartFramework.at("after_trailing_slash_dirs_creation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Fetch listing
    ...
    public void testDeleteMountPoint() throws Exception {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_delete_mount_entry").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/testdelete/subdir"));
            RestartFramework.at("after_delete_dir_creation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRenameMountPoint() throws Exception {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_rename_mount_entries").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/testrename1/sub/sub"));
            nnFs0.mkdirs(new Path("/testrename2"));
            RestartFramework.at("after_rename_dirs_creation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListStatusMountPoint() throws Exception {
        try {
            ...
            assertTrue(addMountTable(addEntry));
            RestartFramework.at("after_ec_mount_entry").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nnFs0.mkdirs(new Path("/testLsMountEntryDest"));
            RestartFramework.at("after_ec_dest_creation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_ec_policy_set").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
