...
public class TestRouterNetworkTopologyServlet_RestartInjected_Randomized_123 {
    ...
    public void testPrintTopologyTextFormat() throws Exception {
        ...
        RestartFramework.at("after_first_topology_query_text").on(clusterWithDatanodes.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPrintTopologyJsonFormat() throws Exception {
        ...
        RestartFramework.at("after_first_topology_query_json").on(clusterWithDatanodes.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // parse json
    ...
    public void testPrintTopologyNoDatanodesTextFormat() throws Exception {
        ...
        conn.setConnectTimeout(20000);
        conn.connect();
        RestartFramework.at("after_topology_query_no_datanodes_text").on(clusterNoDatanodes.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        IOUtils.copyBytes(conn.getInputStream(), out, 4096, true);
        StringBuilder sb = new StringBuilder("-- Network Topology -- \n");
        sb.append(out);
    ...
    public void testPrintTopologyNoDatanodesJsonFormat() throws Exception {
        ...
        sb.append("\n-- Network Topology -- ");
        String topology = sb.toString();
        RestartFramework.at("after_topology_query_no_datanodes_json").on(clusterNoDatanodes.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // assert node number
        assertTrue(topology.contains("No DataNodes"));
    }
}
