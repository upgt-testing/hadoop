 ...
 */
public class TestRouterWebHDFSContractCreate_RestartInjected_Randomized_123 extends AbstractContractCreateTest {
    ...
    public void testCreateNewFile() throws Throwable {
        RestartFramework.at("after_file_write").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreatedFileIsVisibleOnFlush() throws Throwable {
        ...
        out.write('a');
        RestartFramework.at("after_flush").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.flush();
        if (!fs.exists(path)) {
        ...
        RestartFramework.at("after_file_create_before_write").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.close();
    }
}
