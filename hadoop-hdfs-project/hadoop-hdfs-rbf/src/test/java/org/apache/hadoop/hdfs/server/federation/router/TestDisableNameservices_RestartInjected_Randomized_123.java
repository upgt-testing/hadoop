 ...
 */
public class TestDisableNameservices_RestartInjected_Randomized_123 {
    ...
    public void testWithoutDisabling() throws IOException {
        ...
        RestartFramework.at("before_list_status").on(cluster.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_renew_lease").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDisabling() throws Exception {
        ...
        long t0 = monotonicNow();
        routerProtocol.renewLease("client0");
        RestartFramework.at("after_renew_lease_disabled").on(cluster.getCluster()).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_list_status_disabled").on(cluster.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        long t = monotonicNow() - t0;
        assertTrue("It took too long: " + t + "ms", t < TimeUnit.SECONDS.toMillis(1));
        // We should not report anything from ns0
        FileSystem routerFs = routerContext.getFileSystem();
        RestartFramework.at("after_disable_nameservice").on(cluster.getCluster()).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        FileStatus[] filesStatus = routerFs.listStatus(new Path("/"));
    ...
    public void testMetrics() throws Exception {
        ...
        RestartFramework.at("after_disable_nameservice_metrics").on(cluster.getCluster()).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_get_metrics").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
