...
/**
 * Test append operations on the Router-based WebHDFS FS.
 */
public class TestRouterWebHDFSContractAppend_RestartInjected_Randomized_456 extends AbstractContractAppendTest {
    ...
    public void testAppendToEmptyFile() throws Throwable {
        RestartFramework.at("after_append_open").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        touch(getFileSystem(), target);
        byte[] dataset = dataset(256, 'a', 'z');
        FSDataOutputStream outputStream = getFileSystem().append(target);
        RestartFramework.at("after_create_empty_file").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        outputStream.write(dataset);
        outputStream.close();
        RestartFramework.at("before_append_close").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAppendToExistingFile() throws Throwable {
        ...
        RestartFramework.at("before_close_appended").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FSDataOutputStream out = getFileSystem().append(target);
        RestartFramework.at("after_append_open_existing").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.write(appended);
        out.close();
    ...
    public void testRenameFileBeingAppended() throws Throwable {
        RestartFramework.at("after_append_before_write").on(RouterWebHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        touch(getFileSystem(), target);
        ...
        outputStream.write(dataset);
        RestartFramework.at("before_rename_during_append").on(RouterWebHDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
