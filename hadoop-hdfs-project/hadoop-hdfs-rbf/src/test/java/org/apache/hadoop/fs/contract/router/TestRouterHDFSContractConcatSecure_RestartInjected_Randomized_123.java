 ...
 */
public class TestRouterHDFSContractConcatSecure_RestartInjected_Randomized_123 extends AbstractContractConcatTest {
    ...
    public void testConcatFileOnFile() throws Throwable {
        ...
        RestartFramework.at("before_concat_operation").on(RouterHDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        createFile(getFileSystem(), target, false, block);
        getFileSystem().concat(target, new Path[] { srcFile });
        RestartFramework.at("after_concat_operation").on(RouterHDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFileHasLength(getFileSystem(), target, TEST_FILE_LEN * 2);
        RestartFramework.at("after_target_file_creation").on(RouterHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
