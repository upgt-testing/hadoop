 ...
 */
public class TestRouterHDFSContractCreateSecure_RestartInjected_Randomized_456 extends AbstractContractCreateTest {
    ...
    public void testCreateNewFile() throws Throwable {
        ...
        RestartFramework.at("after_file_write").on(RouterHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_file_creation").on(RouterHDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreatedFileIsVisibleOnFlush() throws Throwable {
        ...
        RestartFramework.at("after_flush").on(RouterHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.write('a');
        RestartFramework.at("after_file_create_before_write").on(RouterHDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.flush();
        if (!fs.exists(path)) {
