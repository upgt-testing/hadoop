 ...
 */
public class TestRouterRPCMultipleDestinationMountTableResolver_RestartInjected_Randomized_123 {
    ...
    public void testInvocationHashAllOrder() throws Exception {
        ...
        RestartFramework.at("after_setup").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_invocation").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        boolean isDirAll = rpcServer.isPathAll("/mount/dir");
        assertTrue(isDirAll);
        testInvocation(isDirAll);
    }
     ...
     */
    private void testDirectoryAndFileLevelInvocation(boolean dirAll, Path mountDir, Path nameSpaceFile, Path mountFile, Path nameSpaceDir, final String name, final byte[] value) throws IOException {
     ...
     * @throws IOException
     */
    private void verifyFileLevelInvocations(Path nameSpaceFile, DistributedFileSystem nnFs, Path mountFile, final String name, final byte[] value) throws IOException {
     ...
     * @throws IOException
     */
    private boolean verifyDirectoryLevelInvocations(boolean dirAll, Path nameSpaceDir, DistributedFileSystem nnFs, final String name, final byte[] value) throws IOException {
        boolean checked = false;
        if (dirAll || nnFs.exists(nameSpaceDir)) {
            checked = true;
    ...
    public void testECMultipleDestinations() throws Exception {
        RestartFramework.at("after_set_ec_policy").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        setupOrderMountPath(DestinationOrder.HASH_ALL);
        ...
        RestartFramework.at("after_check_ec").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testACLMultipleDestinations() throws Exception {
        ...
        RestartFramework.at("after_remove_acl_entries").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_remove_all_acl").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        routerFs.removeAclEntries(mountPath, aclSpec);
        RestartFramework.at("after_modify_acl").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(6, nnFs0.getAclStatus(nsPath).getEntries().size());
        assertEquals(6, nnFs1.getAclStatus(nsPath).getEntries().size());
        RestartFramework.at("after_remove_default_acl").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        routerFs.modifyAclEntries(mountPath, aclSpec);
        routerFs.removeDefaultAcl(mountPath);
        RestartFramework.at("after_set_acl").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRenameMultipleDestDirectories() throws Exception {
        ...
        resetTestEnvironment();
        RestartFramework.at("after_rename_random").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyRenameOnMultiDestDirectories(DestinationOrder.RANDOM, false);
        resetTestEnvironment();
        RestartFramework.at("after_rename_space").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyRenameOnMultiDestDirectories(DestinationOrder.SPACE, false);
        RestartFramework.at("after_rename2_space").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        resetTestEnvironment();
        RestartFramework.at("after_rename_hash_all").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_rename2_random").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Test renaming directories using rename2 API.
        verifyRenameOnMultiDestDirectories(DestinationOrder.HASH_ALL, true);
        resetTestEnvironment();
        RestartFramework.at("after_rename2_hash_all").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testClearQuota() throws Exception {
        long nsQuota = 5;
        long ssQuota = 100;
        RestartFramework.at("after_set_quota").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_mount_table_add").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        //clear quota and validate the quota
        argv = new String[] { "-clrQuota", path.toString() };
        RestartFramework.at("after_clear_quota").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(0, ToolRunner.run(admin, argv));
        updateService.periodicInvoke();
        resolver.loadCache(true);
    ...
    public void testContentSummaryWithMultipleDest() throws Exception {
        ...
        RestartFramework.at("after_content_summary_check").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
