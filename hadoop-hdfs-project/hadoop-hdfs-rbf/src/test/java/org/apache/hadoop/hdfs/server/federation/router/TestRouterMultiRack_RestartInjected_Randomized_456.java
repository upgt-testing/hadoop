 ...
 */
public class TestRouterMultiRack_RestartInjected_Randomized_456 {
    ...
    public void testGetECTopologyResultForPolicies() throws IOException {
        ...
        RestartFramework.at("after_reverse_ec_policy_update").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertTrue(result.isSupported());
        RestartFramework.at("after_enable_large_ec_policy").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Specify multiple policies with one policy requiring more datanodes than
        // present in the actual cluster
        result = routerFs.getECTopologyResultForPolicies("RS-10-4-1024k", "RS-3-2-1024k");
        RestartFramework.at("after_cross_namespace_ec_policy_update").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(result.isSupported());
        ...
        result = routerFs.getECTopologyResultForPolicies();
        RestartFramework.at("after_enable_ec_policy").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(result.isSupported());
