 ...
 */
public class TestRouterRpcStoragePolicySatisfier_RestartInjected_Randomized_123 {
    ...
    public void testStoragePolicySatisfier() throws Exception {
        RestartFramework.at("after_wait_disk_storage").on(cluster.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final String file = "/testStoragePolicySatisfierCommand";
        short repl = 1;
        int size = 32;
        RestartFramework.at("after_file_creation").on(cluster.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_wait_archive_storage").on(cluster.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_storage_policy").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_satisfy_storage_policy").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_set_storage_policy").on(cluster.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
