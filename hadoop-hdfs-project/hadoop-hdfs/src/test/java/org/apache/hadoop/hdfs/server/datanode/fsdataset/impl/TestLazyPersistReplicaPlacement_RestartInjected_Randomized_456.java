...
public class TestLazyPersistReplicaPlacement_RestartInjected_Randomized_456 extends LazyPersistTestCase {
    ...
    public void testPlacementOnRamDisk() throws IOException, TimeoutException, InterruptedException {
        ...
        RestartFramework.at("after_ramdisk_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFallbackToDisk() throws IOException, TimeoutException, InterruptedException {
        ...
        final String METHOD_NAME = GenericTestUtils.getMethodName();
        Path path = new Path("/" + METHOD_NAME + ".dat");
        RestartFramework.at("after_fallback_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        makeTestFile(path, BLOCK_SIZE, true);
    ...
    public void testFallbackToDiskFull() throws Exception {
        ...
        final String METHOD_NAME = GenericTestUtils.getMethodName();
        RestartFramework.at("after_full_ramdisk_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path path = new Path("/" + METHOD_NAME + ".dat");
        makeTestFile(path, BLOCK_SIZE, true);
        ensureFileReplicasOnStorageType(path, DEFAULT);
        RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFallbackToDiskPartial() throws IOException, InterruptedException, TimeoutException {
        RestartFramework.at("after_multiblock_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        getClusterBuilder().setMaxLockedMemory(2 * BLOCK_SIZE).build();
        RestartFramework.at("after_lazy_writer_wait").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final String METHOD_NAME = GenericTestUtils.getMethodName();
        Path path = new Path("/" + METHOD_NAME + ".dat");
