   ...
   */
    ...
    public void testPendingReplicationRetry() throws IOException {
        ...
        try {
            ...
            /* Start the MiniDockerDFSCluster with more datanodes since once a writeBlock
       ...
       */
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDataNodes * 2).format(false).build();
            cluster.waitActive();
            dfsClient = new DFSClient(new InetSocketAddress("localhost", cluster.getNameNodePort()), conf);
            waitForBlockReplication(testFile, dfsClient.getNamenode(), numDataNodes, -1);
        } finally {
    ...
    public void testReplicateLenMismatchedBlock() throws Exception {
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(new HdfsConfiguration()).numDataNodes(2).build();
        try {
            cluster.waitActive();
    ...
    private void changeBlockLen(MiniDockerDFSCluster cluster, int lenDelta) throws IOException, InterruptedException, TimeoutException {
    ...
    public void testReplicationWhenBlockCorruption() throws Exception {
        ...
        try {
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storagesPerDatanode(1).build();
            ...
            for (final DataNodeInterface dn : cluster.getDataNodes()) {
    ...
    public void testNoExtraReplicationWhenBlockReceivedIsLate() throws Exception {
        ...
        final BlockManagerInterface bm = cluster.getNameNode().getNamesystem().getBlockManager();
        try {
            cluster.waitActive();
            ...
            Mockito.doAnswer(delayer).when(spy).blockReceivedAndDeleted(any(), Mockito.anyString(), any());
            FileSystem fs = cluster.getFileSystem();
    ...
    public void testReplicationWhileUnderConstruction() throws Exception {
        ...
        try {
            ...
            // make sure blocks are persisted, etc
            stm.hflush();
    ...
    private long pendingReplicationCount(BlockManagerInterface bm) {
        BlockManagerTestUtil.updateState(bm);
        return bm.getPendingReconstructionBlocksCount();
    }

    private void assertNoReplicationWasPerformed(MiniDockerDFSCluster cluster) {
