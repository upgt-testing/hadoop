...
public class TestCheckPointForSecurityTokens {
    ...
    public void testSaveNamespace() throws IOException {
        ...
        try {
            ...
            TokenInterface<DelegationTokenIdentifier> token1 = namesystem.getDelegationToken(new Text(renewer));
            TokenInterface<DelegationTokenIdentifier> token2 = namesystem.getDelegationToken(new Text(renewer));
            // Saving image without safe mode should fail
            DFSAdmin admin = new DFSAdmin(conf);
            String[] args = new String[] { "-saveNamespace" };
            // verify that the edits file is NOT empty
            NameNodeInterface nn = cluster.getNameNode();
            for (StorageDirectoryInterface sd : nn.getFSImage().getStorage().dirIterable(null)) {
            ...
            TokenInterface<DelegationTokenIdentifier> token3 = namesystem.getDelegationToken(new Text(renewer));
            TokenInterface<DelegationTokenIdentifier> token4 = namesystem.getDelegationToken(new Text(renewer));
            ...
            cluster.waitActive();
            namesystem = cluster.getNamesystem();
            TokenInterface<DelegationTokenIdentifier> token5 = namesystem.getDelegationToken(new Text(renewer));
            try {
                renewToken(token1);
                renewToken(token2);
                renewToken(token3);
