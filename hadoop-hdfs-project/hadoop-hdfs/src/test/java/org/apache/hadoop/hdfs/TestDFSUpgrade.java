...
public class TestDFSUpgrade {
    ...
    void checkDataNode(String[] baseDirs, String bpid) throws IOException {
        for (int i = 0; i < baseDirs.length; i++) {
            ...
            File currentFinalized = MiniDockerDFSCluster.getFinalizedDir(new File(baseDirs[i]), bpid);
    ...
    void startNameNodeShouldFail(StartupOption operation, Class<? extends Exception> exceptionClass, Pattern messagePattern) {
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).startupOption(operation).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).// should fail
            build();
     ...
     */
    private MiniDockerDFSCluster createCluster() throws IOException {
        return new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).startupOption(StartupOption.UPGRADE).build();
    }
