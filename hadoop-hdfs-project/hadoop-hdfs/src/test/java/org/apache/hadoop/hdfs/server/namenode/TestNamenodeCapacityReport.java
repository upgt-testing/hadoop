...
public class TestNamenodeCapacityReport {
    ...
    public void testVolumeSize() throws Exception {
        ...
        try {
            ...
            for (final DatanodeDescriptorInterface datanode : live) {
            ...
            // in the MiniDockerDFSCluster. This results in each data directory having
    ...
    public void testXceiverCountInternal(int minMaintenanceR) throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(nodes).build();
            cluster.waitActive();
            final FSNamesystemInterface namesystem = cluster.getNamesystem();
            ...
            for (int i = 0; i < nodes / 2; i++) {
                ...
                DatanodeDescriptorInterface dnd = dnm.getDatanode(dn.getDatanodeId());
                dn.shutdown();
            ...
            for (int i = 0; i < fileRepl; i++) {
                expectedInServiceNodes--;
                DatanodeDescriptorInterface dnd = dnm.getDatanode(datanodes.get(i).getDatanodeId());
            ...
            for (int i = 0; i < nodes; i++) {
                ...
                DatanodeDescriptorInterface dnDesc = dnm.getDatanode(dn.getDatanodeId());
