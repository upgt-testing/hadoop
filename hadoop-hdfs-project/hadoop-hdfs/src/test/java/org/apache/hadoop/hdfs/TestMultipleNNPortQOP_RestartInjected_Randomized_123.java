 ...
 * configured QOP.
 */
public class TestMultipleNNPortQOP_RestartInjected_Randomized_123 extends SaslDataTransferTestCase {
    ...
    public void setup() throws Exception {
        ...
        clusterConf.set(DFS_NAMENODE_RPC_ADDRESS_AUXILIARY_KEY, "12000,12100,12200");
        ...
        clusterConf.set(CommonConfigurationKeys.HADOOP_SECURITY_SASL_PROPS_RESOLVER_CLASS, "org.apache.hadoop.security.IngressPortBasedResolver");
    ...
    public void testAuxiliaryPortSendingQOP() throws Exception {
        ...
        try {
            cluster = new MiniDFSCluster.Builder(clusterConf).numDataNodes(3).build();
            cluster.waitActive();
            ...
            clientConf.unset(CommonConfigurationKeys.HADOOP_SECURITY_SASL_PROPS_RESOLVER_CLASS);
            URI currentURI = cluster.getURI();
            URI uriAuthPort = new URI(currentURI.getScheme() + "://" + currentURI.getHost() + ":12000");
            URI uriIntegrityPort = new URI(currentURI.getScheme() + "://" + currentURI.getHost() + ":12100");
            URI uriPrivacyPort = new URI(currentURI.getScheme() + "://" + currentURI.getHost() + ":12200");
            // If connecting to primary port, block token should not include
            // handshake secret
            byte[] secretOnPrimary = getHandshakeSecret(currentURI, clientConf, new Path(pathPrefix + "Primary"));
            assertTrue(secretOnPrimary == null || secretOnPrimary.length == 0);
            RestartFramework.at("after_primary_port_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // If connecting to auxiliary port, block token should include
            // handshake secret
            clientConf.set(HADOOP_RPC_PROTECTION, "privacy");
            byte[] secretPrivacy = getHandshakeSecret(uriPrivacyPort, clientConf, new Path(pathPrefix + "Privacy"));
            assertTrue(secretPrivacy.length > 0);
            RestartFramework.at("after_privacy_port_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            clientConf.set(HADOOP_RPC_PROTECTION, "integrity");
            byte[] secretIntegrity = getHandshakeSecret(uriIntegrityPort, clientConf, new Path(pathPrefix + "Integrity"));
            assertTrue(secretIntegrity.length > 0);
            RestartFramework.at("after_integrity_port_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            clientConf.set(HADOOP_RPC_PROTECTION, "authentication");
            byte[] secretAuthentication = getHandshakeSecret(uriAuthPort, clientConf, new Path(pathPrefix + "Authentication"));
            assertTrue(secretAuthentication.length > 0);
            RestartFramework.at("after_authentication_port_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    private byte[] getHandshakeSecret(URI uri, HdfsConfiguration conf, Path path) throws Exception {
        ...
        FSDataOutputStream out = fs.create(path, false, 4096, (short) 1, BLOCK_SIZE);
        try {
            out.write(0);
            out.hflush();
    ...
    public void testMultipleNNPort() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_privacy_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            clientConf.set(HADOOP_RPC_PROTECTION, "integrity");
            ...
            RestartFramework.at("after_integrity_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_authentication_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    public void testMultipleNNPortOverwriteDownStream() throws Exception {
        ...
        clusterConf.setBoolean(DFS_ENCRYPT_DATA_OVERWRITE_DOWNSTREAM_DERIVED_QOP_KEY, true);
        MiniDFSCluster cluster = null;
        try {
            cluster = new MiniDFSCluster.Builder(clusterConf).numDataNodes(3).build();
            cluster.waitActive();
