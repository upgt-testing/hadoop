...
public class TestDFSClientSocketSize {
    ...
    private int getSendBufferSize_withUpgrade20(Configuration conf) throws IOException {
        final MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        try {
            cluster.waitActive();
    ...
    private int getSendBufferSize_withUpgrade40(Configuration conf) throws IOException {
        final MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        try {
            cluster.waitActive();
            LOG.info("MiniDFSClusterInJVM started.");
    ...
    private int getSendBufferSize_withUpgrade80(Configuration conf) throws IOException {
        final MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        try {
            cluster.waitActive();
            LOG.info("MiniDFSClusterInJVM started.");
    ...
    @Test
    public void testDefaultSendBufferSize_withUpgrade20() throws IOException {
        final int sendBufferSize = getSendBufferSize_withUpgrade20(new Configuration());
        LOG.info("If not specified, the auto tuned send buffer size is: {}", sendBufferSize);
        assertTrue("Send buffer size should be non-negative value which is " + "determined by system (kernel).", sendBufferSize > 0);
    }

    @Test
    public void testDefaultSendBufferSize_withUpgrade40() throws IOException {
        final int sendBufferSize = getSendBufferSize_withUpgrade40(new Configuration());
        LOG.info("If not specified, the auto tuned send buffer size is: {}", sendBufferSize);
        assertTrue("Send buffer size should be non-negative value which is " + "determined by system (kernel).", sendBufferSize > 0);
    }

    @Test
    public void testDefaultSendBufferSize_withUpgrade80() throws IOException {
        final int sendBufferSize = getSendBufferSize_withUpgrade80(new Configuration());
        LOG.info("If not specified, the auto tuned send buffer size is: {}", sendBufferSize);
        assertTrue("Send buffer size should be non-negative value which is " + "determined by system (kernel).", sendBufferSize > 0);
    }

    @Test
    public void testSpecifiedSendBufferSize_withUpgrade20() throws IOException {
        final Configuration conf1 = new Configuration();
        // 256 KB
        conf1.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 256 * 1024);
        final int sendBufferSize1 = getSendBufferSize_withUpgrade20(conf1);
        final Configuration conf2 = new Configuration();
        // 1 KB
        conf2.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 1024);
        final int sendBufferSize2 = getSendBufferSize_withUpgrade20(conf2);
        LOG.info("Large buf size is {}, small is {}", sendBufferSize1, sendBufferSize2);
        assertTrue("Larger specified send buffer should have effect", sendBufferSize1 > sendBufferSize2);
    }

    @Test
    public void testSpecifiedSendBufferSize_withUpgrade40() throws IOException {
        final Configuration conf1 = new Configuration();
        // 256 KB
        conf1.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 256 * 1024);
        final int sendBufferSize1 = getSendBufferSize_withUpgrade40(conf1);
        final Configuration conf2 = new Configuration();
        // 1 KB
        conf2.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 1024);
        final int sendBufferSize2 = getSendBufferSize_withUpgrade40(conf2);
        LOG.info("Large buf size is {}, small is {}", sendBufferSize1, sendBufferSize2);
        assertTrue("Larger specified send buffer should have effect", sendBufferSize1 > sendBufferSize2);
    }

    @Test
    public void testSpecifiedSendBufferSize_withUpgrade80() throws IOException {
        final Configuration conf1 = new Configuration();
        // 256 KB
        conf1.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 256 * 1024);
        final int sendBufferSize1 = getSendBufferSize_withUpgrade80(conf1);
        final Configuration conf2 = new Configuration();
        // 1 KB
        conf2.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 1024);
        final int sendBufferSize2 = getSendBufferSize_withUpgrade80(conf2);
        LOG.info("Large buf size is {}, small is {}", sendBufferSize1, sendBufferSize2);
        assertTrue("Larger specified send buffer should have effect", sendBufferSize1 > sendBufferSize2);
    }

    @Test
    public void testAutoTuningSendBufferSize_withUpgrade20() throws IOException {
        final Configuration conf = new Configuration();
        conf.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 0);
        final int sendBufferSize = getSendBufferSize_withUpgrade20(conf);
        LOG.info("The auto tuned send buffer size is: {}", sendBufferSize);
        assertTrue("Send buffer size should be non-negative value which is " + "determined by system (kernel).", sendBufferSize > 0);
    }

    @Test
    public void testAutoTuningSendBufferSize_withUpgrade40() throws IOException {
        final Configuration conf = new Configuration();
        conf.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 0);
        final int sendBufferSize = getSendBufferSize_withUpgrade40(conf);
        LOG.info("The auto tuned send buffer size is: {}", sendBufferSize);
        assertTrue("Send buffer size should be non-negative value which is " + "determined by system (kernel).", sendBufferSize > 0);
    }

    @Test
    public void testAutoTuningSendBufferSize_withUpgrade80() throws IOException {
        final Configuration conf = new Configuration();
        conf.setInt(DFS_CLIENT_SOCKET_SEND_BUFFER_SIZE_KEY, 0);
        final int sendBufferSize = getSendBufferSize_withUpgrade80(conf);
        LOG.info("The auto tuned send buffer size is: {}", sendBufferSize);
        assertTrue("Send buffer size should be non-negative value which is " + "determined by system (kernel).", sendBufferSize > 0);
    }
}
