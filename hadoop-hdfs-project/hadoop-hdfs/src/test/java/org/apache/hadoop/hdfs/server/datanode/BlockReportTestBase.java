...
public abstract class BlockReportTestBase {
    ...
    public void blockReport_01() throws IOException {
        ...
        DataNodeInterface dn = cluster.getDataNodes().get(DN_N0);
        String poolId = cluster.getNamesystem().getBlockPoolId();
        DatanodeRegistrationInterface dnR = dn.getDNRegistrationForBP(poolId);
    ...
    public void blockReport_02() throws IOException {
        ...
        // all blocks belong to the same file, hence same BP
        String poolId = cluster.getNamesystem().getBlockPoolId();
        DatanodeRegistrationInterface dnR = dn0.getDNRegistrationForBP(poolId);
    ...
    public void blockReport_06() throws Exception {
        ...
        // all blocks belong to the same file, hence same BP
        DataNodeInterface dn = cluster.getDataNodes().get(DN_N1);
        String poolId = cluster.getNamesystem().getBlockPoolId();
        DatanodeRegistrationInterface dnR = dn.getDNRegistrationForBP(poolId);
        StorageBlockReport[] reports = getBlockReports(dn, poolId, false, false);
        sendBlockReports(dnR, poolId, reports);
    ...
    public void testInterleavedBlockReports() throws IOException, ExecutionException, InterruptedException {
        ...
        final DatanodeRegistrationInterface dnR = dn.getDNRegistrationForBP(poolId);
    ...
    private void waitForTempReplica(Block bl, int DN_N1) throws IOException {
        ...
        final DataNodeInterface dn1 = cluster.getDataNodes().get(DN_N1);
