...
public class TestDataNodeVolumeMetrics {
    ...
    public void testVolumeMetrics() throws Exception {
        MiniDockerDFSCluster cluster = setupClusterForVolumeMetrics();
        try {
    ...
    private MiniDockerDFSCluster setupClusterForVolumeMetrics() throws IOException {
        Configuration conf = new HdfsConfiguration();
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, 100);
        SimulatedFSDataset.setFactory(conf);
        return new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).storageTypes(new StorageType[] { StorageType.RAM_DISK, StorageType.DISK }).storagesPerDatanode(2).build();
    }
    ...
    public void testWriteIoVolumeMetrics() throws IOException {
        Configuration conf = new HdfsConfiguration();
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, 100);
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).storageTypes(new StorageType[] { StorageType.RAM_DISK, StorageType.DISK }).storagesPerDatanode(2).build();
        try {
            FileSystem fs = cluster.getFileSystem();
            final Path fileName = new Path("/test.dat");
            final long fileLen = Integer.MAX_VALUE + 1L;
