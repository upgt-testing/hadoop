...
public class TestReconstructStripedBlocks_RestartInjected_Randomized_456 {
    ...
    private void doTestMissingStripedBlock(int numOfMissed, int numOfBusy) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_nodes_busy").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_datanodes_removed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            bm = cluster.getNamesystem().getBlockManager();
            BlockManagerTestUtil.getComputedDatanodeWork(bm);
            RestartFramework.at("after_reconstruction_triggered").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void test2RecoveryTasksForSameBlockGroup() throws Exception {
        ...
        try {
            ...
            BlockManagerTestUtil.updateState(bm);
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_ec_file_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_first_datanode_stopped").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertEquals(1, bm.getPendingReconstructionBlocksCount());
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_second_datanode_stopped").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            bm = cluster.getNamesystem().getBlockManager();
        } finally {
            cluster.shutdown();
        }
    ...
    public void testCountLiveReplicas() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_ec_dir_and_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // stop a dn
            ...
            RestartFramework.at("after_first_reconstruction").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // bring the dn back: 10 internal blocks now
            cluster.restartDataNode(dnProp);
            cluster.waitActive();
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_first_datanode_restarted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.setDataNodeDead(dnToStop);
            RestartFramework.at("after_second_datanode_stopped").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            // currently namenode is able to track the missing block. but restart NN
            cluster.restartNameNode(true);
            for (DataNode dn : cluster.getDataNodes()) {
                DataNodeTestUtils.triggerBlockReport(dn);
            }
            RestartFramework.at("after_namenode_restart_and_block_reports").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReconstructionWork() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_ec_striped_file_creation").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_datanode_removed").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            BlockManagerTestUtil.getComputedDatanodeWork(bm);
            BlockManagerTestUtil.updateState(bm);
            RestartFramework.at("after_reconstruction_work_triggered").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
