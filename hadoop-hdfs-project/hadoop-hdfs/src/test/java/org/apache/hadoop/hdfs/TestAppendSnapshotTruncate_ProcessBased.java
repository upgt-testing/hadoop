...
/**
 * ProcessBasedMiniDFSCluster version of {@link TestAppendSnapshotTruncate}.
 *
 ...
 * the correctness.
 *
 * NOTE: Due to the multi-threaded nature of this test, upgrade checkpoints
 * are limited to pause points where all workers are idle.
 *
 * @see TestAppendSnapshotTruncate Original test using MiniDFSCluster
 */
@RunWith(Parameterized.class)
public class TestAppendSnapshotTruncate_ProcessBased extends ProcessBasedUpgradeTestBase {
  static {
    GenericTestUtils.setLogLevel(NameNode.stateChangeLog, Level.ALL);
  }
  private static final Logger LOG =
      LoggerFactory.getLogger(TestAppendSnapshotTruncate_ProcessBased.class);
  ...
  // Test-local DFS reference (instead of static)
  private DistributedFileSystem testDfs;
  ...
  public void testAST() throws Exception {
    ...
    testConf.setLong(DFSConfigKeys.DFS_NAMENODE_MIN_BLOCK_SIZE_KEY, BLOCK_SIZE);
    testConf.setInt(DFSConfigKeys.DFS_BYTES_PER_CHECKSUM_KEY, BLOCK_SIZE);
    testConf.setInt(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY, SHORT_HEARTBEAT);
    testConf.setLong(
        DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_KEY, 1);
    testConf.setBoolean(ReplaceDatanodeOnFailure.BEST_EFFORT_KEY, true);

    cluster = new ProcessBasedMiniDFSCluster.Builder(testConf)
        .format(true)
        .numDataNodes(DATANODE_NUM)
        .build();
    testDfs = cluster.getFileSystem();
    ...
    final DirWorker w = new DirWorker(dir, localDir, FILE_WORKER_NUM, testDfs);
    w.startAllFiles();
    w.start();

    // Run for half the test time before potential upgrade
    Worker.sleep((TEST_TIME_SECOND * 1000) / 2);

    // Mid-test checkpoint - pause all workers, upgrade, resume
    if (shouldUpgrade("MID_TEST")) {
      LOG.info("Pausing all workers for mid-test upgrade checkpoint");
      w.pauseAllFiles();
      checkpoint("MID_TEST");
      LOG.info("Resuming all workers after upgrade");
      w.startAllFiles();
    }

    // Run for remaining test time
    Worker.sleep((TEST_TIME_SECOND * 1000) / 2);
  ...
  static class DirWorker extends Worker {
    ...
    DirWorker(Path dir, File localDir, int nFiles, DistributedFileSystem dfs) throws IOException {
      super(dir.getName());
      this.dir = dir;
      ...
      for(int i = 0; i < files.length; i++) {
        files[i] = new FileWorker(dir, localDir, String.format("file%02d", i), dfs);
      }
    }
    ...
    String checkSnapshot(String snapshot) throws IOException {
      ...
      for(int i = 0; i < statuses.length; i++) {
        FileWorker.checkFullFile(statuses[i].getPath(), localFiles[i], dfs);
      }
      return b.toString();
    }
  ...
  static class FileWorker extends Worker {
    ...
    FileWorker(Path dir, File localDir, String filename, DistributedFileSystem dfs) throws IOException {
      super(filename);
    ...
    int checkLength() throws IOException {
      return checkLength(file, localFile, dfs);
    }

    static int checkLength(Path file, File localFile, DistributedFileSystem dfs) throws IOException {
    ...
    String checkFullFile() throws IOException {
      return checkFullFile(file, localFile, dfs);
    }

    static String checkFullFile(Path file, File localFile, DistributedFileSystem dfs) throws IOException {
      final StringBuilder b = new StringBuilder("checkFullFile: ")
          .append(file.getName()).append(" vs ").append(localFile);
      final byte[] bytes = new byte[checkLength(file, localFile, dfs)];
