...
public class TestFavoredNodesEndToEnd_RestartInjected_Randomized_123 {
    ...
    public void testFavoredNodesEndToEnd() throws Exception {
        ...
        for (int i = 0; i < NUM_FILES; i++) {
            if (i == 5) {
                RestartFramework.at("during_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
    ...
    public void testWhenFavoredNodesNotPresent() throws Exception {
        ...
        RestartFramework.at("after_create_with_arbitrary_nodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 3; i++) {
        ...
        out.write(SOME_BYTES);
        out.close();
        RestartFramework.at("before_create_with_arbitrary_nodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        getBlockLocations(p);
    }

    @Test
    public void testWhenSomeNodesAreNotGood() throws Exception {
        RestartFramework.at("after_decommission_node").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // 4 favored nodes
        ...
        RestartFramework.at("after_reset_decommission").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_with_decommissioned_node").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFavoredNodesEndToEndForAppend() throws Exception {
        RestartFramework.at("after_append_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_append_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // create 10 files with random preferred nodes
        for (int i = 0; i < NUM_FILES; i++) {
            if (i == 5) {
                RestartFramework.at("during_append_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            Random rand = new Random(System.currentTimeMillis() + i);
            // pass a new created rand so as to get a uniform distribution each time
            // without too much collisions (look at the do-while loop in getDatanodes)
    ...
    public void testCreateStreamBuilderFavoredNodesEndToEnd() throws Exception {
        RestartFramework.at("after_builder_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_builder_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        //create 10 files with random preferred nodes
        for (int i = 0; i < NUM_FILES; i++) {
            if (i == 5) {
                RestartFramework.at("during_builder_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            Random rand = new Random(System.currentTimeMillis() + i);
            //pass a new created rand so as to get a uniform distribution each time
            //without too much collisions (look at the do-while loop in getDatanodes)
