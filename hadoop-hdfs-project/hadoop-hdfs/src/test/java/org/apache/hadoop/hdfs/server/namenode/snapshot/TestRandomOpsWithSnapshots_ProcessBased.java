...
/**
 * ProcessBasedMiniDFSCluster version of {@link TestRandomOpsWithSnapshots}.
 *
 ...
 *
 * @see TestRandomOpsWithSnapshots Original test using MiniDFSCluster
 */
@RunWith(Parameterized.class)
public class TestRandomOpsWithSnapshots_ProcessBased extends ProcessBasedUpgradeTestBase {

  private static final Logger LOG =
      LoggerFactory.getLogger(TestRandomOpsWithSnapshots_ProcessBased.class);
  ...
  private List<Path> snapshottableDirectories = new ArrayList<Path>();
  private Map<Path, ArrayList<String>> pathToSnapshotsMap =
  ...
  private enum Operations {
    ...
    public static Operations getRandomOperation(OperationType type, Random generator) {
      int randomNum = 0;
  ...
  public void testRandomOperationsWithSnapshots()
    ...
    checkpoint("AFTER_INITIAL_FILES_CREATED");
    ...
    for (int i = 0; i < numberOfIterations; i++) {
      ...
      for (int j = 0; j < numberFileSystemOperations; j++) {
        Operations fsOperation =
            Operations.getRandomOperation(OperationType.FileSystem, generator);
      ...
      // Insert checkpoint after first iteration
      if (i == 0) {
        checkpoint("AFTER_FIRST_ITERATION");
      }

      // random number of Snapshot operations
      for (int k = 0; k < numberSnapshotOperations; k++) {
        Operations snapshotOperation =
            Operations.getRandomOperation(OperationType.Snapshot, generator);
      ...
      checkpoint("BEFORE_HEALTH_CHECK");
  ...
  private void checkClusterHealth() throws Exception {
    ...
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      @Override
      public Boolean get() {
        // Use client API instead of cluster.getNameNode().isInSafeMode()
        try {
          return !hdfs.setSafeMode(HdfsConstants.SafeModeAction.SAFEMODE_GET);
        } catch (IOException e) {
          return false;
        }
      }
    ...
    // ProcessBasedMiniDFSCluster cluster health checks
    assertTrue("DataNodes are running", cluster.getNumDataNodes() > 0);
