...
public class TestAddOverReplicatedStripedBlocks_RestartInjected_Randomized_456 {
    ...
    public void testProcessOverReplicatedStripedBlock() throws Exception {
        ...
        RestartFramework.at("after_over_replication_injection").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testProcessOverReplicatedAndCorruptStripedBlock() throws Exception {
        ...
        RestartFramework.at("after_mark_corrupt").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.triggerBlockReports();
    ...
    public void testProcessOverReplicatedAndMissingStripedBlock() throws Exception {
        ...
        assertEquals(groupSize + 1, bg.getLocations().length);
        BitSet set = new BitSet(groupSize);
        RestartFramework.at("after_missing_block_injection").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (byte index : bg.getBlockIndices()) {
            set.set(index);
        }
