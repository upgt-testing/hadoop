...
public class TestDFSClientRetries {
    ...
    public void testNotYetReplicatedErrors() throws IOException {
        ...
        when(mockNN.addBlock(anyString(), anyString(), any(), any(), anyLong(), any(), any())).thenAnswer(answer);
        ...
        Mockito.doReturn(new HdfsFileStatus.Builder().replication(1).blocksize(1024).perm(new FsPermission((short) 777)).owner("owner").group("group").symlink(new byte[0]).fileId(1010).build()).when(mockNN).create(anyString(), any(), anyString(), any(), anyBoolean(), anyShort(), anyLong(), any(), any(), any());
    ...
    private boolean busyTest(int xcievers, int threads, int fileLen, int timeWin, int retries) throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(replicationFactor).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    class DFSClientReader implements Runnable {
        ...
        DFSClientReader(Path file, MiniDockerDFSCluster cluster, byte[] hash_sha, int fileLen, Counter cnt) {
            filePath = file;
    ...
    public static void namenodeRestartTest(final Configuration conf, final boolean isWebHDFS) throws Exception {
        ...
        conf.setInt(MiniDockerDFSCluster.DFS_NAMENODE_SAFEMODE_EXTENSION_TESTING_KEY, 5000);
    ...
    public void testDFSClientConfigurationLocateFollowingBlock() throws Exception {
        ...
        try {
            ...
            assertEquals(defaultInitialDelay, client.getConf().getBlockWriteLocateFollowingInitialDelayMs());
            assertEquals(defaultMaxDelay, client.getConf().getBlockWriteLocateFollowingMaxDelayMs());
            assertEquals(defultRetry, client.getConf().getNumBlockWriteLocateFollowingRetry());
            conf.setInt(HdfsClientConfigKeys.BlockWrite.LOCATEFOLLOWINGBLOCK_INITIAL_DELAY_MS_KEY, initialDelayTestValue);
            conf.setInt(HdfsClientConfigKeys.BlockWrite.LOCATEFOLLOWINGBLOCK_MAX_DELAY_MS_KEY, maxDelayTestValue);
            conf.setInt(HdfsClientConfigKeys.BlockWrite.LOCATEFOLLOWINGBLOCK_RETRIES_KEY, retryTestValue);
            client = new DFSClient(null, nn, conf, null);
            assertEquals(initialDelayTestValue, client.getConf().getBlockWriteLocateFollowingInitialDelayMs());
            assertEquals(maxDelayTestValue, client.getConf().getBlockWriteLocateFollowingMaxDelayMs());
            assertEquals(retryTestValue, client.getConf().getNumBlockWriteLocateFollowingRetry());
        } finally {
            cluster.shutdown();
        }
    }
