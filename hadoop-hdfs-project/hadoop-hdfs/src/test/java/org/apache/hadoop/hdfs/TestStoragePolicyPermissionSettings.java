...
public class TestStoragePolicyPermissionSettings {
    ...
    private void setStoragePolicyPermissions(boolean isStoragePolicyEnabled, boolean isStoragePolicySuperuserOnly) throws NoSuchFieldException, IllegalAccessException {
        ...
        setFSNameSystemFinalField("isStoragePolicySuperuserOnly", isStoragePolicySuperuserOnly);
    }
    ...
    public void testStoragePolicyPermissionDefault() throws Exception {
        ...
        final FileSystem fileSystemNonAdmin = DFSTestUtil.getFileSystemAs(nonAdmin, conf);
        LambdaTestUtils.intercept(AccessControlException.class, "Permission denied: user=user1", "Only super user can set storage policy.", () -> fileSystemNonAdmin.setStoragePolicy(foo, cold.getName()));
        // widen privilege
    ...
    public void testStoragePolicyPermissionAdmins() throws Exception {
        ...
        final FileSystem fileSystemNonAdmin = DFSTestUtil.getFileSystemAs(nonAdmin, conf);
        LambdaTestUtils.intercept(AccessControlException.class, "Access denied for user user1. Superuser privilege is required", "Only super user can set storage policy.", () -> fileSystemNonAdmin.setStoragePolicy(foo, cold.getName()));
        final FileSystem fileSystemAdmin = DFSTestUtil.getFileSystemAs(admin, conf);
        assertNotEquals(fs.getStoragePolicy(foo), cold);
    ...
    public void testStoragePolicyPermissionDisabled() throws Exception {
        ...
        final FileSystem fileSystemAdmin = DFSTestUtil.getFileSystemAs(admin, conf);
        LambdaTestUtils.intercept(IOException.class, "Failed to set storage policy " + "since dfs.storage.policy.enabled is set to false.", "Storage policy settings are disabled.", () -> fileSystemAdmin.setStoragePolicy(foo, cold.getName()));
        assertEquals(suite.getDefaultPolicy(), fs.getStoragePolicy(foo));
    }

    @Test
    public void testStoragePolicyPermissionDefault_withUpgrade20() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        setStoragePolicyPermissions(true, false);
        // Test default user fails
    ...
    @Test
    public void testStoragePolicyPermissionDefault_withUpgrade60() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        setStoragePolicyPermissions(true, false);
        // Test default user fails
    ...
    @Test
    public void testStoragePolicyPermissionAdmins_withUpgrade20() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        // Test only super can set storage policies
    ...
    @Test
    public void testStoragePolicyPermissionAdmins_withUpgrade40() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        // Test only super can set storage policies
    ...
    @Test
    public void testStoragePolicyPermissionAdmins_withUpgrade60() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        // Test only super can set storage policies
    ...
    @Test
    public void testStoragePolicyPermissionAdmins_withUpgrade80() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        // Test only super can set storage policies
    ...
    @Test
    public void testStoragePolicyPermissionDisabled_withUpgrade20() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testStoragePolicyPermissionDisabled_withUpgrade40() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        setStoragePolicyPermissions(false, false);
    ...
    @Test
    public void testStoragePolicyPermissionDisabled_withUpgrade60() throws Exception {
        Path foo = new Path("/foo");
        DFSTestUtil.createFile(fs, foo, SIZE, REPL, 0);
        fs.setPermission(foo, new FsPermission("777"));
        setStoragePolicyPermissions(false, false);
