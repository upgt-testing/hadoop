...
public class TestDeadDatanode {
    ...
    public void testDeadDatanode() throws Exception {
        ...
        DatanodeRegistrationInterface reg = InternalDataNodeTestUtils.getDNRegistrationForBP(cluster.getDataNodes().get(0), poolId);
    ...
    public void testDeadNodeAsBlockTarget() throws Exception {
        ...
        NodeInterface clientNode = dm.getDatanode(reg);
        DFSTestUtil.waitForDatanodeState(cluster, reg.getDatanodeUuid(), true, 20000);
        // Shutdown and wait for datanode to be marked dead
        dn.shutdown();
        DFSTestUtil.waitForDatanodeState(cluster, reg.getDatanodeUuid(), false, 20000);
        ...
        for (DatanodeStorageInfoInterface datanodeStorageInfo : results) {
    ...
    public void testNonDFSUsedONDeadNodeReReg() throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).simulatedCapacities(capacities).build();
            long initialCapacity = cluster.getNamesystem(0).getCapacityTotal();
            assertTrue(initialCapacity > 0);
            DataNodeInterface dn1 = cluster.getDataNodes().get(0);
            DataNodeInterface dn2 = cluster.getDataNodes().get(1);
            final DatanodeDescriptorInterface dn2Desc = cluster.getNamesystem(0).getBlockManager().getDatanodeManager().getDatanode(dn2.getDatanodeId());
            ...
            final DatanodeDescriptorInterface dn1Desc = cluster.getNamesystem(0).getBlockManager().getDatanodeManager().getDatanode(dn1.getDatanodeId());
