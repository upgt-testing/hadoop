...
public class TestDatanodeConfig {
    ...
    private static final File BASE_DIR = new File(MiniDFSClusterInJVM.getBaseDirectory());
    ...
    @Test
    public void testDataDirectories_withUpgrade20() throws IOException {
        File dataDir = new File(BASE_DIR, "data").getCanonicalFile();
        Configuration conf = cluster.getConfiguration(0);
        // 1. Test unsupported ecPolicy. Only "file:" is supported.
        String dnDir = makeURI("shv", null, fileAsURI(dataDir).getPath());
    ...
    @Test
    public void testDataDirectories_withUpgrade40() throws IOException {
        File dataDir = new File(BASE_DIR, "data").getCanonicalFile();
        Configuration conf = cluster.getConfiguration(0);
        // 1. Test unsupported ecPolicy. Only "file:" is supported.
        String dnDir = makeURI("shv", null, fileAsURI(dataDir).getPath());
    ...
    @Test
    public void testDataDirectories_withUpgrade60() throws IOException {
        File dataDir = new File(BASE_DIR, "data").getCanonicalFile();
        Configuration conf = cluster.getConfiguration(0);
        // 1. Test unsupported ecPolicy. Only "file:" is supported.
        String dnDir = makeURI("shv", null, fileAsURI(dataDir).getPath());
    ...
    @Test
    public void testDataDirectories_withUpgrade80() throws IOException {
        File dataDir = new File(BASE_DIR, "data").getCanonicalFile();
        Configuration conf = cluster.getConfiguration(0);
        // 1. Test unsupported ecPolicy. Only "file:" is supported.
        String dnDir = makeURI("shv", null, fileAsURI(dataDir).getPath());
    ...
    @Test(timeout = 60000)
    public void testMemlockLimit_withUpgrade20() throws Exception {
        assumeTrue(NativeIO.isAvailable());
        final long memlockLimit = NativeIO.POSIX.getCacheManipulator().getMemlockLimit();
        // Can't increase the memlock limit past the maximum.
        assumeTrue(memlockLimit != Long.MAX_VALUE);
    ...
    @Test(timeout = 60000)
    public void testMemlockLimit_withUpgrade40() throws Exception {
        assumeTrue(NativeIO.isAvailable());
        final long memlockLimit = NativeIO.POSIX.getCacheManipulator().getMemlockLimit();
        // Can't increase the memlock limit past the maximum.
        assumeTrue(memlockLimit != Long.MAX_VALUE);
    ...
    @Test(timeout = 60000)
    public void testMemlockLimit_withUpgrade60() throws Exception {
        assumeTrue(NativeIO.isAvailable());
        final long memlockLimit = NativeIO.POSIX.getCacheManipulator().getMemlockLimit();
        // Can't increase the memlock limit past the maximum.
        assumeTrue(memlockLimit != Long.MAX_VALUE);
    ...
    @Test(timeout = 60000)
    public void testMemlockLimit_withUpgrade80() throws Exception {
        assumeTrue(NativeIO.isAvailable());
        final long memlockLimit = NativeIO.POSIX.getCacheManipulator().getMemlockLimit();
        // Can't increase the memlock limit past the maximum.
        assumeTrue(memlockLimit != Long.MAX_VALUE);
