...
public class TestNamenodeStorageDirectives {
     ...
     */
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(nnTopology).numDataNodes(numDataNodes).storagesPerDatanode(storagePerDataNode).storageTypes(storageTypes).build();
        cluster.waitActive();
    }
    ...
    private boolean verifyFileReplicasOnStorageType(Path path, int numBlocks, StorageType storageType) throws IOException {
        MiniDockerDFSCluster.NameNodeInfo info = cluster.getNameNodeInfos()[0];
    ...
    private DatanodeStorageInfo getDatanodeStorageInfo(int dnIndex) throws UnregisteredNodeException {
        ...
        DatanodeIDInterface dnId = cluster.getDataNodes().get(dnIndex).getDatanodeId();
        DatanodeManagerInterface dnManager = cluster.getNamesystem().getBlockManager().getDatanodeManager();
    ...
    public void testStorageIDBlockPlacementSpecific() throws ReconfigurationException, InterruptedException, TimeoutException, IOException {
        ...
        final int numBlocks = 10;
        DatanodeStorageInfoInterface dnInfoToUse = getDatanodeStorageInfo(0);
