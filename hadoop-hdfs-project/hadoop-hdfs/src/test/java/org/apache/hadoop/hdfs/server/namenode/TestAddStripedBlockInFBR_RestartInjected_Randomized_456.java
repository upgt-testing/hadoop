...
public class TestAddStripedBlockInFBR_RestartInjected_Randomized_456 {
    ...
    public void testAddBlockInFullBlockReport() throws Exception {
        ...
        dfs.mkdirs(ecDir);
        dfs.mkdirs(repDir);
        RestartFramework.at("after_create_ec_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_directories").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dn = cluster.getDataNodes().get(0);
        RestartFramework.at("after_create_non_ec_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
