 ...
 * the new cluster.
 */
public class TestMiniDFSCluster_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestMiniDFSCluster_RestartInjected.class);
    ...
    public void testClusterSetStorageCapacity() throws Throwable {
        ...
        try {
            ...
            RestartFramework.at("after_first_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /* restart all data nodes */
            cluster.restartDataNodes();
            cluster.waitActive();
            verifyStorageCapacity(cluster, capacities);
            RestartFramework.at("after_datanode_restart_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /* restart all name nodes */
            cluster.restartNameNodes();
            cluster.waitActive();
            verifyStorageCapacity(cluster, capacities);
            RestartFramework.at("after_namenode_restart_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /* restart all name nodes firstly and data nodes then */
            cluster.restartNameNodes();
            cluster.restartDataNodes();
            cluster.waitActive();
            verifyStorageCapacity(cluster, capacities);
            RestartFramework.at("after_namenode_datanode_restart_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /* restart all data nodes firstly and name nodes then */
            cluster.restartDataNodes();
            cluster.restartNameNodes();
            cluster.waitActive();
            verifyStorageCapacity(cluster, capacities);
            RestartFramework.at("after_datanode_namenode_restart_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    private MiniDFSCluster newCluster(final Configuration conf, final int numDatanodes, final long[] storageCapacities, final int defaultBlockSize, final int fileLen) throws IOException, InterruptedException, TimeoutException {
    ...
    public void testIsClusterUpAfterShutdown() throws Throwable {
        ...
        try {
            RestartFramework.at("after_cluster_creation").on(cluster4).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DistributedFileSystem dfs = cluster4.getFileSystem();
            dfs.setSafeMode(HdfsConstants.SafeModeAction.SAFEMODE_ENTER);
            RestartFramework.at("after_safemode_entry").on(cluster4).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster4.shutdown();
        } finally {
    ...
    public void testClusterSetDatanodeHostname() throws Throwable {
        ...
        try (MiniDFSCluster cluster5 = new MiniDFSCluster.Builder(conf, testDataCluster5).numDataNodes(1).checkDataNodeHostConfig(true).build()) {
            RestartFramework.at("after_cluster_creation").on(cluster5).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testClusterSetDatanodeDifferentStorageType() throws IOException {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(3).storageTypes(storageType).build()) {
            ...
            RestartFramework.at("after_storage_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testSetUpFederatedCluster() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHAFederatedTopology(2)).numDataNodes(2).build()) {
            ...
            RestartFramework.at("after_state_transitions").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_configuration_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Shutdown namenodes individually.
            cluster.shutdownNameNode(0);
            RestartFramework.at("after_first_namenode_shutdown").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.shutdownNameNode(1);
            RestartFramework.at("after_second_namenode_shutdown").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Restart namenodes individually with wait active, both should be successful.
            cluster.restartNameNode(0);
            RestartFramework.at("after_first_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNode(1);
            RestartFramework.at("after_second_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testStartStopWithPorts() throws Exception {
        ...
        try (MiniDFSCluster miniDfsCluster = new MiniDFSCluster.Builder(conf).setDnHttpPorts(httpPorts).setDnIpcPorts(ipcPorts).numDataNodes(3).build()) {
            ...
            RestartFramework.at("after_cluster_creation").on(miniDfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_port_verification").on(miniDfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
