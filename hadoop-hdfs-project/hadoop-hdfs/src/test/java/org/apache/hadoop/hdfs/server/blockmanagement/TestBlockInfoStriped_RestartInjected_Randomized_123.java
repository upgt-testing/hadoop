...
public class TestBlockInfoStriped_RestartInjected_Randomized_123 {
    ...
    public TestBlockInfoStriped_RestartInjected_Randomized_123(ErasureCodingPolicy policy) {
        testECPolicy = policy;
    ...
    public void testGetBlockInfo() throws IllegalArgumentException, Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf, builderBaseDir).numDataNodes(totalSize).build()) {
            ...
            RestartFramework.at("after_set_ec_policy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_remove_storage").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ByteArrayOutputStream bStream = new ByteArrayOutputStream();
            PrintStream out = new PrintStream(bStream, true);
            assertEquals(0, ToolRunner.run(new DFSck(conf, out), new String[] { new Path("/ecDir/ecFile").toString(), "-blockId", id }));
