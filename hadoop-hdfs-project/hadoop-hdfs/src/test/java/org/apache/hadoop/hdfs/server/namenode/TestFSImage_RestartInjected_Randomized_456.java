...
public class TestFSImage_RestartInjected_Randomized_456 {
    ...
    private void testPersistHelper(Configuration conf) throws IOException {
        ...
        try {
            ...
            RestartFramework.at("before_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testHasNonEcBlockUsingStripedIDForLoadFile() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_first_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNodes();
            cluster.waitActive();
            ...
            fns.enterSafeMode(false);
            fns.saveNamespace(0, 0);
            RestartFramework.at("after_second_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_second_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNodes();
            cluster.waitActive();
            fns = cluster.getNamesystem();
    ...
    public void testHasNonEcBlockUsingStripedIDForLoadSnapshot() throws IOException {
        ...
        try {
            ...
            fs.truncate(p, 0);
            RestartFramework.at("after_truncate").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fns.enterSafeMode(false);
            fns.saveNamespace(0, 0);
            RestartFramework.at("after_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSaveAndLoadErasureCodingPolicies() throws IOException {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(10).build()) {
            ...
            fs.saveNamespace();
            fs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
            RestartFramework.at("after_initial_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_first_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNodes();
            cluster.waitActive();
            ...
            RestartFramework.at("after_add_ec_policy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Save namespace and restart NameNode
            fs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
            fs.saveNamespace();
            fs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
    ...
    private void testChangeErasureCodingPolicyState(MiniDFSCluster cluster, int blockSize, ErasureCodingPolicy targetPolicy, boolean isDefault) throws IOException {
        ...
        RestartFramework.at("after_disable_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_disable_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_enable_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        fs.saveNamespace();
        fs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
        RestartFramework.at("before_remove_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.restartNameNodes();
        cluster.waitActive();
        ...
        RestartFramework.at("after_enable_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        if (isDefault) {
            assertEquals("The erasure coding policy should be of " + "enabled state", ErasureCodingPolicyState.ENABLED, ecPolicyState);
        } else {
            assertEquals("The erasure coding policy should be of " + "disabled state", ErasureCodingPolicyState.DISABLED, ecPolicyState);
        }
        assertFalse("Policy should be in disabled state in FSImage!", isPolicyEnabledInFsImage(targetPolicy));
        RestartFramework.at("after_remove_policy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Read file regardless of the erasure coding policy state
        DFSTestUtil.readFileAsBytes(fs, filePath);
        RestartFramework.at("after_remove_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // 3. Remove an erasure coding policy
        try {
            fs.removeErasureCodingPolicy(ecPolicy.getName());
        } catch (RemoteException e) {
            // built-in policy cannot been removed
            assertTrue("Built-in policy cannot be removed", ecPolicy.isSystemPolicy());
        ...
        ecPolicy = ErasureCodingPolicyManager.getInstance().getByID(targetPolicy.getId());
        assertEquals("The erasure coding policy saved into and loaded from " + "fsImage is bad", targetPolicy, ecPolicy);
        assertEquals("The erasure coding policy should be of removed state", ErasureCodingPolicyState.REMOVED, DFSTestUtil.getECPolicyState(ecPolicy));
        // Read file regardless of the erasure coding policy state
        DFSTestUtil.readFileAsBytes(fs, filePath);
    ...
    private boolean isPolicyEnabledInFsImage(ErasureCodingPolicy testPolicy) {
        ErasureCodingPolicyInfo[] persistedPolicies = ErasureCodingPolicyManager.getInstance().getPersistedPolicies();
    ...
    private ArrayList<Section> getSubSectionsOfName(ArrayList<Section> sections, FSImageFormatProtobuf.SectionName name) {
    ...
    public void testParallelSaveAndLoad() throws IOException {
        ...
        try {
            ...
            ArrayList<Section> sections = Lists.newArrayList(summary.getSectionsList());
            ArrayList<Section> inodeSubSections = getSubSectionsOfName(sections, SectionName.INODE_SUB);
            ArrayList<Section> dirSubSections = getSubSectionsOfName(sections, SectionName.INODE_DIR_SUB);
            Section inodeSection = getSubSectionsOfName(sections, SectionName.INODE).get(0);
            Section dirSection = getSubSectionsOfName(sections, SectionName.INODE_DIR).get(0);
    ...
    private void ensureSubSectionsAlignWithParent(ArrayList<Section> subSec, Section parent) {
        ...
        assertEquals(parent.getLength() + parent.getOffset(), lastSubSection.getLength() + lastSubSection.getOffset());
    ...
    public void testUpdateBlocksMapAndNameCacheAsync() throws IOException {
        ...
        // checkpoint
        fs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
        RestartFramework.at("after_snapshot_operations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.saveNamespace();
        fs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
        cluster.restartNameNode();
        cluster.waitActive();
