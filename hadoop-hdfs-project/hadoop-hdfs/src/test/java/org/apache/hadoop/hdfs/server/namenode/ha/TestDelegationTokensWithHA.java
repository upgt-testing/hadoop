...
public class TestDelegationTokensWithHA {
    ...
    public void testObserverReadProxyProviderWithDT() throws Exception {
        ...
        final Token<DelegationTokenIdentifier> token = getDelegationToken(dfs, ugi.getShortUserName());
        ugi.addToken(token);
   ...
   */
    ...
    @Test(timeout = 300000)
    public void testObserverReadProxyProviderWithDT_withUpgrade20() throws Exception {
        // instead of just using and succeeding on the active
        conf.setInt(OBSERVER_PROBE_RETRY_PERIOD_KEY, 0);
        cluster.transitionToStandby(0);
        cluster.transitionToActive(1);
    ...
    @Test(timeout = 300000)
    public void testObserverReadProxyProviderWithDT_withUpgrade60() throws Exception {
        // instead of just using and succeeding on the active
        conf.setInt(OBSERVER_PROBE_RETRY_PERIOD_KEY, 0);
        cluster.transitionToStandby(0);
        cluster.transitionToActive(1);
    ...
    @Test(timeout = 300000)
    public void testObserverReadProxyProviderWithDT_withUpgrade80() throws Exception {
        // instead of just using and succeeding on the active
        conf.setInt(OBSERVER_PROBE_RETRY_PERIOD_KEY, 0);
        cluster.transitionToStandby(0);
        cluster.transitionToActive(1);
