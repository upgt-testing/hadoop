...
public class TestNNHealthCheck_RestartInjected_Randomized_123 {
    ...
    public void testNNHealthCheck() throws IOException {
        RestartFramework.at("after_cluster_build").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = new MiniDFSCluster.Builder(conf).numDataNodes(0).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        RestartFramework.at("after_health_check_test").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        doNNHealthCheckTest();
    }
    ...
    public void testNNHealthCheckWithLifelineAddress() throws IOException {
        ...
        RestartFramework.at("after_health_check_test_lifeline").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        cluster = new MiniDFSCluster.Builder(conf).numDataNodes(0).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        RestartFramework.at("after_cluster_build_with_lifeline").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        doNNHealthCheckTest();
    }
    ...
    public void testNNHealthCheckWithSafemodeAsUnhealthy() throws Exception {
        ...
        cluster = new MiniDFSCluster.Builder(conf).numDataNodes(0).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        cluster.waitActive();
        RestartFramework.at("after_health_monitor_proxy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_safemode_enter").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
