...
public class TestDebugAdmin_RestartInjected_Randomized_123 {

    static private final String TEST_ROOT_DIR = new File(System.getProperty("test.build.data", "/tmp"), TestDebugAdmin_RestartInjected.class.getSimpleName()).getAbsolutePath();
    ...
    public void testRecoverLease() throws Exception {
        ...
        cluster.waitActive();
        assertEquals("ret: 1, You must supply a -path argument to recoverLease.", runCmd(new String[] { "recoverLease", "-retries", "1" }));
        ...
        assertEquals("ret: 0, recoverLease SUCCEEDED on /foo", runCmd(new String[] { "recoverLease", "-path", "/foo" }));
        RestartFramework.at("after_recover_lease").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testVerifyMetaCommand() throws Exception {
        ...
        cluster.waitActive();
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_block_retrieval").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DistributedFileSystem fs = cluster.getFileSystem();
        RestartFramework.at("after_verify_meta").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertEquals("ret: 0, Checksum type: " + "DataChecksum(type=CRC32C, chunkSize=512)", runCmd(new String[] { "verifyMeta", "-meta", metaFile.getAbsolutePath() }));
        assertEquals("ret: 0, Checksum type: " + "DataChecksum(type=CRC32C, chunkSize=512)" + "Checksum verification succeeded on block file " + blockFile.getAbsolutePath(), runCmd(new String[] { "verifyMeta", "-meta", metaFile.getAbsolutePath(), "-block", blockFile.getAbsolutePath() }));
        datanode = cluster.getDataNodes().get(0);
    }
    ...
    public void testComputeMetaCommand() throws Exception {
        ...
        RestartFramework.at("after_block_retrieval").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        datanode = cluster.getDataNodes().get(0);
        RestartFramework.at("after_compute_meta").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        File blockFile = getBlockFile(fsd, block.getBlockPoolId(), block.getLocalBlock());
        RestartFramework.at("after_cluster_active").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRecoverLeaseforFileNotFound() throws Exception {
        cluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).build();
        RestartFramework.at("after_command_execution").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cluster_active").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.waitActive();
        assertTrue(runCmd(new String[] { "recoverLease", "-path", "/foo", "-retries", "2" }).contains("Giving up on recoverLease for /foo after 1 try"));
    }
    ...
    public void testVerifyECCommand() throws Exception {
        ...
        RestartFramework.at("after_regular_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.waitActive();
        DistributedFileSystem fs = cluster.getFileSystem();
        assertEquals("ret: 1, verifyEC -file <file>  Verify HDFS erasure coding on " + "all block groups of the file.", runCmd(new String[] { "verifyEC" }));
        ...
        RestartFramework.at("after_ec_policy_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_ec_file_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final short repl = 1;
        final long k = 1024;
        final long m = k * k;
        ...
        RestartFramework.at("after_block_corruption").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_small_ec_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_multiblock_ec_file").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_large_ec_files").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
