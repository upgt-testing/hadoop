 ...
 */
public class TestRBWBlockInvalidation_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestRBWBlockInvalidation_RestartInjected.class);
    ...
    public void testBlockInvalidationWhenRBWReplicaMissedInDN() throws IOException, InterruptedException {
        ...
        try {
            ...
            out.hsync();
            RestartFramework.at("after_initial_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.startDataNodes(conf, 1, true, null, null, null);
            RestartFramework.at("after_start_additional_datanode").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_corruption_detected").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_rereplication_complete").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            while (true) {
                Thread.sleep(100);
    ...
    public void testRWRInvalidation() throws Exception {
        ...
        try {
            ...
            try {
                ...
                RestartFramework.at("after_initial_write_flush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                for (Path path : testPaths) {
                    DFSTestUtil.waitReplication(cluster.getFileSystem(), path, (short) 2);
                }
                RestartFramework.at("after_initial_replication").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_new_genstamp_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                for (Path path : testPaths) {
                    DFSTestUtil.waitReplication(cluster.getFileSystem(), path, (short) 1);
                }
                RestartFramework.at("after_replication_to_one").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                cluster.waitActive();
                RestartFramework.at("after_datanodes_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_invalidation_computation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRWRShouldNotAddedOnDNRestart() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(2).build()) {
            ...
            out.writeBytes("old gs data\n");
            out.hflush();
            RestartFramework.at("after_initial_hflush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // stop one datanode
            ...
            out.writeBytes("old gs data\n");
            out.hflush();
            RestartFramework.at("after_write_post_dn_stop").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartDataNode(dnProp, true);
            // wait till the block report comes
            Thread.sleep(3000);
            RestartFramework.at("after_dn_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
