...
public class TestDFSAdminWithHA_RestartInjected_Randomized_456 {
    ...
    private void assertOutputMatches(String string) {
        ...
        if (!errOutput.matches(string) && !output.matches(string)) {
            fail("Expected output to match '" + string + "' but err_output was:\n" + errOutput + "\n and output was: \n" + output);
        }
        out.reset();
        err.reset();
    }
    ...
    private void setHAConf(Configuration conf, String nn1Addr, String nn2Addr) {
        conf.set(CommonConfigurationKeysPublic.FS_DEFAULT_NAME_KEY, "hdfs://" + NSID);
        ...
        conf.set(DFSUtil.addKeySuffixes(DFSConfigKeys.DFS_NAMENODE_RPC_ADDRESS_KEY, NSID, "nn1"), nn1Addr);
        conf.set(DFSUtil.addKeySuffixes(DFSConfigKeys.DFS_NAMENODE_RPC_ADDRESS_KEY, NSID, "nn2"), nn2Addr);
    }

    private void setUpHaCluster(boolean security) throws Exception {
        conf = new Configuration();
        conf.setBoolean(CommonConfigurationKeys.HADOOP_SECURITY_AUTHORIZATION, security);
        String baseDir = GenericTestUtils.getRandomizedTempPath();
        cluster = new MiniQJMHACluster.Builder(conf).baseDir(baseDir).build();
        setHAConf(conf, cluster.getDfsCluster().getNameNode(0).getHostAndPort(), cluster.getDfsCluster().getNameNode(1).getHostAndPort());
        ...
        conf.setInt(CommonConfigurationKeysPublic.IPC_CLIENT_CONNECT_MAX_RETRIES_KEY, 3);
        conf.setInt(CommonConfigurationKeysPublic.IPC_CLIENT_CONNECT_RETRY_INTERVAL_KEY, 500);
    ...
    public void testSetSafeMode() throws Exception {
        ...
        RestartFramework.at("after_ha_cluster_setup").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertOutputMatches(message + newLine + message + newLine);
        // Get safemode
        exitCode = admin.run(new String[] { "-safemode", "get" });
        RestartFramework.at("after_safemode_leave").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(err.toString().trim(), 0, exitCode);
        RestartFramework.at("after_safemode_enter").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSaveNamespace() throws Exception {
        ...
        assertEquals(err.toString().trim(), 0, exitCode);
        String message = "Safe mode is ON in.*";
        RestartFramework.at("after_save_namespace").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertOutputMatches(message + newLine + message + newLine);
        RestartFramework.at("after_safemode_enter").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRestoreFailedStorage() throws Exception {
        ...
        assertEquals(err.toString().trim(), 0, exitCode);
        RestartFramework.at("after_restore_failed_storage").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRefreshNodes() throws Exception {
        setUpHaCluster(false);
        RestartFramework.at("after_refresh_nodes").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSetBalancerBandwidth() throws Exception {
        RestartFramework.at("after_transition_to_active").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_set_balancer_bandwidth").on(cluster.getDfsCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        setUpHaCluster(false);
    ...
    public void testMetaSave() throws Exception {
        RestartFramework.at("after_transition_to_active").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_metasave").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        setUpHaCluster(false);
        cluster.getDfsCluster().transitionToActive(0);
        ...
        assertOutputMatches(messageFromActiveNN + newLine + messageFromStandbyNN + newLine);
    }
    ...
    public void testRefreshServiceAcl() throws Exception {
        ...
        RestartFramework.at("after_refresh_service_acl").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertOutputMatches(message + newLine + message + newLine);
    }
    ...
    public void testRefreshUserToGroupsMappings() throws Exception {
        ...
        assertEquals(err.toString().trim(), 0, exitCode);
        RestartFramework.at("after_refresh_user_to_groups_mappings").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRefreshSuperUserGroupsConfiguration() throws Exception {
        ...
        RestartFramework.at("after_refresh_super_user_groups_configuration").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(err.toString().trim(), 0, exitCode);
    ...
    public void testRefreshCallQueue() throws Exception {
        ...
        assertEquals(err.toString().trim(), 0, exitCode);
        RestartFramework.at("after_refresh_call_queue").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFinalizeUpgrade() throws Exception {
        ...
        assertOutputMatches(message + newLine);
        RestartFramework.at("after_finalize_upgrade").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.getDfsCluster().transitionToActive(0);
    ...
    public void testUpgradeCommand() throws Exception {
        RestartFramework.at("after_upgrade_finalize").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final String finalizedMsg = "Upgrade finalized for.*";
        final String notFinalizedMsg = "Upgrade not finalized for.*";
        RestartFramework.at("after_nn1_upgrade_start").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_upgrade_query_before_upgrade").on(cluster.getDfsCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_standby_bootstrap").on(cluster.getDfsCluster()).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
