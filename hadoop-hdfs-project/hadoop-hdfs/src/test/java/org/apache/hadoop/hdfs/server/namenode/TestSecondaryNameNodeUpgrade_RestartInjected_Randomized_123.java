 ...
 */
public class TestSecondaryNameNodeUpgrade_RestartInjected_Randomized_123 {
    ...
    private void doIt(Map<String, String> paramsToCorrupt) throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_secondary_namenode_started").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs = cluster.getFileSystem();
            fs.mkdirs(new Path("/test/foo"));
            RestartFramework.at("after_first_directory_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            snn.doCheckpoint();
            ...
            RestartFramework.at("after_version_files_corrupted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            snn = new SecondaryNameNode(conf);
            RestartFramework.at("after_secondary_namenode_restarted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.mkdirs(new Path("/test/bar"));
            RestartFramework.at("after_second_directory_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            snn.doCheckpoint();
