...
public class TestFailureToReadEdits {
        ...
        if (clusterType == TestType.SHARED_DIR_HA) {
            ...
            while (true) {
                try {
                    ...
                    cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(topology).numDataNodes(0).checkExitOnShutdown(false).build();
                    break;
    ...
    private LimitedEditLogAnswer causeFailureOnEditLogRead() throws IOException {
        FSEditLogInterface spyEditLog = NameNodeAdapter.spyOnEditLog(nn1);
    ...
    private static class LimitedEditLogAnswer implements Answer<Collection<EditLogInputStream>> {
        ...
        public Collection<EditLogInputStream> answer(InvocationOnMock invocation) throws Throwable {
            ...
            if (!throwExceptionOnRead) {
                ...
                for (EditLogInputStreamInterface stream : streams) {
                    EditLogInputStreamInterface spyStream = spy(stream);
                    doAnswer(new Answer<FSEditLogOp>() {

                        @Override
                        public FSEditLogOp answer(InvocationOnMock invocation) throws Throwable {
                            FSEditLogOpInterface op = (FSEditLogOp) invocation.callRealMethod();
