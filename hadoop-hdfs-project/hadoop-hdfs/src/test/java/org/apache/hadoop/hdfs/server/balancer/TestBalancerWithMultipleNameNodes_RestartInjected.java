...
public class TestBalancerWithMultipleNameNodes_RestartInjected {
  ...
  private void unevenDistribution(final int nNameNodes,
    ...
    final ExtendedBlock[][] blocks;
    {
      ...
      try {
        cluster.waitActive();
        RestartFramework.at("after_first_cluster_setup")
            .on(cluster)
            .restart("datanode")
            .withIndex(0)
        ...
        RestartFramework.at("after_generate_blocks")
            .on(cluster)
            .restart("datanode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
    ...
    }
    {
      ...
      try {
        cluster.waitActive();
        RestartFramework.at("after_second_cluster_setup")
            .on(cluster)
            .restart("datanode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        RestartFramework.at("after_inject_blocks")
            .on(cluster)
            .restart("datanode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        LOG.info("UNEVEN 14");
        RestartFramework.at("before_balancer_run")
            .on(cluster)
            .restart("namenode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        RestartFramework.at("after_balancer_complete")
            .on(cluster)
            .restart("datanode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  private void runTest(final int nNameNodes, String[] racks,
    ...
    try {
      ...
      cluster.startDataNodes(conf, newCapacity.length, true, null,
          newRack, newCapacity);
      RestartFramework.at("after_add_datanodes")
          .on(cluster)
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
