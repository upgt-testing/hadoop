...
public class TestBlockStoragePolicy {
    ...
    static final EnumSet<StorageType> disk_archive = EnumSet.of(StorageType.DISK, StorageType.ARCHIVE);

    static final EnumSet<StorageType> all = EnumSet.of(StorageType.SSD, StorageType.DISK, StorageType.ARCHIVE, StorageType.NVDIMM);
    ...
    public void testDefaultPolicies() {
        ...
        expectedPolicyStrings.put(ALLNVDIMM, "BlockStoragePolicy{ALL_NVDIMM:" + ALLNVDIMM + ", storageTypes=[NVDIMM], " + "creationFallbacks=[DISK], " + "replicationFallbacks=[DISK]}");
        for (byte i = 1; i < 16; i++) {
        ...
        assertCreationFallback(warm, StorageType.DISK, StorageType.DISK, StorageType.ARCHIVE, StorageType.DISK, null, StorageType.DISK);
        assertReplicationFallback(warm, StorageType.DISK, StorageType.DISK, StorageType.ARCHIVE, StorageType.DISK, StorageType.DISK);
        // check Hot policy
        ...
        assertReplicationFallback(hot, StorageType.ARCHIVE, null, StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE);
        ...
        assertCreationFallback(onessd, StorageType.SSD, StorageType.SSD, StorageType.SSD, StorageType.DISK, StorageType.SSD, StorageType.SSD);
        assertReplicationFallback(onessd, StorageType.SSD, StorageType.SSD, StorageType.SSD, StorageType.DISK, StorageType.SSD);
        ...
        assertCreationFallback(allssd, StorageType.DISK, StorageType.DISK, null, StorageType.DISK, null, StorageType.DISK);
        assertReplicationFallback(allssd, StorageType.DISK, StorageType.DISK, null, StorageType.DISK, StorageType.DISK);
        ...
        assertCreationFallback(lazyPersist, StorageType.DISK, StorageType.DISK, null, StorageType.DISK, null, StorageType.DISK);
        assertReplicationFallback(lazyPersist, StorageType.DISK, StorageType.DISK, null, StorageType.DISK, StorageType.DISK);
        // check ALL_NVDIMM policy
        final BlockStoragePolicy allnvdimm = POLICY_SUITE.getPolicy(ALLNVDIMM);
        for (short replication = 1; replication < 6; replication++) {
            final List<StorageType> computed = allnvdimm.chooseStorageTypes(replication);
            assertStorageType(computed, replication, StorageType.NVDIMM);
        }
        assertCreationFallback(allnvdimm, StorageType.DISK, StorageType.DISK, null, StorageType.DISK, null, StorageType.DISK);
        assertReplicationFallback(allnvdimm, StorageType.DISK, StorageType.DISK, null, StorageType.DISK, StorageType.DISK);
    }
    ...
    static void assertCreationFallback(BlockStoragePolicy policy, StorageType noneExpected, StorageType archiveExpected, StorageType diskExpected, StorageType ssdExpected, StorageType disk_archiveExpected, StorageType nvdimmExpected) {
        ...
        Assert.assertEquals(disk_archiveExpected, policy.getCreationFallback(disk_archive));
        Assert.assertEquals(null, policy.getCreationFallback(all));
    }

    static void assertReplicationFallback(BlockStoragePolicy policy, StorageType noneExpected, StorageType archiveExpected, StorageType diskExpected, StorageType ssdExpected, StorageType nvdimmExpected) {
    ...
    private static void run(CheckChooseStorageTypes method) {
        ...
        final short replication = 3;
        {
            ...
            method.checkChooseStorageTypes(allnvdimm, replication, chosen, StorageType.NVDIMM, StorageType.NVDIMM, StorageType.NVDIMM);
        }
        {
    ...
    private static void runWithArchiveUnavailable(CheckChooseStorageTypes method) {
        ...
        final short replication = 3;
        {
            final List<StorageType> chosen = Lists.newArrayList();
            method.checkChooseStorageTypes(hot, replication, chosen, StorageType.DISK, StorageType.DISK, StorageType.DISK);
            method.checkChooseStorageTypes(warm, replication, chosen, StorageType.DISK, StorageType.DISK, StorageType.DISK);
            method.checkChooseStorageTypes(cold, replication, chosen);
        }
        {
            final List<StorageType> chosen = Arrays.asList(StorageType.DISK);
            method.checkChooseStorageTypes(hot, replication, chosen, StorageType.DISK, StorageType.DISK);
            method.checkChooseStorageTypes(warm, replication, chosen, StorageType.DISK, StorageType.DISK);
            method.checkChooseStorageTypes(cold, replication, chosen);
        }
        {
            final List<StorageType> chosen = Arrays.asList(StorageType.ARCHIVE);
    ...
    static void checkChooseStorageTypes(BlockStoragePolicy p, short replication, List<StorageType> chosen, EnumSet<StorageType> unavailables, boolean isNewBlock, StorageType... expected) {
        final List<StorageType> types = p.chooseStorageTypes(replication, chosen, unavailables, isNewBlock);
        assertStorageTypes(types, expected);
    }
    ...
    public void testSetStoragePolicy() throws Exception {
        ...
        try {
            ...
            checkDirectoryListing(dirList, BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, BLOCK_STORAGE_POLICY_ID_UNSPECIFIED);
            checkDirectoryListing(barList, BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, BLOCK_STORAGE_POLICY_ID_UNSPECIFIED);
            ...
            Assert.assertEquals("File storage policy should be COLD", HdfsConstants.COLD_STORAGE_POLICY_NAME, fs.getStoragePolicy(fooFile).getName());
            Assert.assertEquals("File storage policy should be WARM", HdfsConstants.WARM_STORAGE_POLICY_NAME, fs.getStoragePolicy(barDir).getName());
            Assert.assertEquals("File storage policy should be HOT", HdfsConstants.HOT_STORAGE_POLICY_NAME, fs.getStoragePolicy(barFile2).getName());
    ...
    public void testSetStoragePolicyWithSnapshot() throws Exception {
        ...
        try {
            ...
            checkDirectoryListing(fs.getClient().listPaths(s1f1.toString(), HdfsFileStatus.EMPTY_NAME).getPartialListing(), COLD);
            ...
            // follow the latest
            checkDirectoryListing(fs.getClient().listPaths(s1foo.toString(), HdfsFileStatus.EMPTY_NAME).getPartialListing(), COLD, HOT);
            // delete foo
    ...
    private void testChangeFileRep(String policyName, byte policyId, StorageType[] before, StorageType[] after) throws Exception {
        ...
        final MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(numDataNodes).storageTypes(types).build();
        cluster.waitActive();
        final DistributedFileSystem fs = cluster.getFileSystem();
        try {
    ...
    public void testChangeWarmRep() throws Exception {
        testChangeFileRep(HdfsConstants.WARM_STORAGE_POLICY_NAME, WARM, new StorageType[] { StorageType.DISK, StorageType.ARCHIVE, StorageType.ARCHIVE }, new StorageType[] { StorageType.DISK, StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE });
    }
    ...
    public void testChangeColdRep() throws Exception {
        testChangeFileRep(HdfsConstants.COLD_STORAGE_POLICY_NAME, COLD, new StorageType[] { StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE }, new StorageType[] { StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE, StorageType.ARCHIVE });
    }
    ...
    public void testAddDatanode2ExistingPipelineInSsd() throws Exception {
        ...
        final StorageType[] disks = { StorageType.DISK, StorageType.DISK, StorageType.DISK };
        final DatanodeStorageInfo[] diskStorages = DFSTestUtil.createDatanodeStorageInfos(7, racks, hosts, disks);
        final DatanodeDescriptor[] dataNodes = DFSTestUtil.toDatanodeDescriptor(diskStorages);
        for (int i = 0; i < dataNodes.length; i++) {
            BlockManagerTestUtil.updateStorage(dataNodes[i], new DatanodeStorage("ssd" + i + 1, DatanodeStorage.State.NORMAL, StorageType.SSD));
        }
        FileSystem.setDefaultUri(conf, "hdfs://localhost:0");
        conf.set(DFSConfigKeys.DFS_NAMENODE_HTTP_ADDRESS_KEY, "0.0.0.0:0");
        File baseDir = PathUtils.getTestDir(TestReplicationPolicy.class);
        ...
        try {
            ...
            DatanodeStorageInfo[] targets = replicator.chooseTarget("/foo", 1, null, chsenDs, true, new HashSet<Node>(), 0, policy, null);
            Assert.assertEquals(3, targets.length);
        } finally {
            if (namenode != null) {
                namenode.stop();
    ...
    public void testGetFileStoragePolicyAfterRestartNN() throws Exception {
        //HDFS8219
        final MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(REPLICATION).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).build();
        cluster.waitActive();
        final DistributedFileSystem fs = cluster.getFileSystem();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    public void testStorageTypeCheckAccess() {
        ...
        testStorageTypeCheckAccessResult(StorageType.EMPTY_ARRAY, StorageType.EMPTY_ARRAY, false);
        testStorageTypeCheckAccessResult(new StorageType[] { StorageType.DISK }, StorageType.EMPTY_ARRAY, false);
        testStorageTypeCheckAccessResult(StorageType.EMPTY_ARRAY, new StorageType[] { StorageType.RAM_DISK }, true);
        ...
        testStorageTypeCheckAccessResult(new StorageType[] { StorageType.RAM_DISK, StorageType.SSD, StorageType.ARCHIVE }, new StorageType[] { StorageType.DISK }, false);
        testStorageTypeCheckAccessResult(new StorageType[] { StorageType.RAM_DISK, StorageType.SSD, StorageType.ARCHIVE }, new StorageType[] { StorageType.DISK }, false);
        testStorageTypeCheckAccessResult(new StorageType[] { StorageType.DISK, StorageType.NVDIMM }, new StorageType[] { StorageType.NVDIMM }, true);
        testStorageTypeCheckAccessResult(new StorageType[] { StorageType.RAM_DISK, StorageType.NVDIMM, StorageType.ARCHIVE }, new StorageType[] { StorageType.SSD }, false);
    }

    private void testStorageTypeCheckAccessResult(StorageType[] requested, StorageType[] allowed, boolean expAccess) {
        try {
            BlockTokenSecretManager.checkAccess(requested, allowed, "StorageTypes");
            if (!expAccess) {
                fail("No expected access with allowed StorageTypes " + Arrays.toString(allowed) + " and requested StorageTypes " + Arrays.toString(requested));
            }
        } catch (SecretManager.InvalidToken e) {
            if (expAccess) {
                fail("Expected access with allowed StorageTypes " + Arrays.toString(allowed) + " and requested StorageTypes " + Arrays.toString(requested));
            }
        }
    }
    ...
    private void testStorageIDCheckAccessResult(String[] requested, String[] allowed, boolean expAccess) {
        try {
            BlockTokenSecretManager.checkAccess(requested, allowed, "StorageIDs");
            if (!expAccess) {
                fail("No expected access with allowed StorageIDs" + Arrays.toString(allowed) + " and requested StorageIDs" + Arrays.toString(requested));
            }
        } catch (SecretManager.InvalidToken e) {
            if (expAccess) {
                fail("Expected access with allowed StorageIDs " + Arrays.toString(allowed) + " and requested StorageIDs" + Arrays.toString(requested));
            }
        }
    }
    ...
    public void testCreateDefaultPoliciesFromConf() {
        BlockStoragePolicySuite suite = BlockStoragePolicySuite.createDefaultSuite();
        Assert.assertEquals(HdfsConstants.StoragePolicy.HOT.value(), suite.getDefaultPolicy().getId());
        Configuration newConf = new Configuration();
        newConf.setEnum(DFSConfigKeys.DFS_STORAGE_DEFAULT_POLICY, HdfsConstants.StoragePolicy.ONE_SSD);
        BlockStoragePolicySuite suiteConf = BlockStoragePolicySuite.createDefaultSuite(newConf);
        Assert.assertEquals(HdfsConstants.StoragePolicy.ONE_SSD.value(), suiteConf.getDefaultPolicy().getId());
    }

    @Test
    public void testCreateFileWithConfiguredDefaultPolicies() throws IOException {
        Configuration newConf = new HdfsConfiguration();
        newConf.set(DFSConfigKeys.DFS_STORAGE_DEFAULT_POLICY, HdfsConstants.StoragePolicy.WARM.name());
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(newConf).numDataNodes(0).build();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
        try {
            cluster.waitActive();
