 ...
 */
public class TestQuotaWithStripedBlocks_RestartInjected_Randomized_456 {
    ...
    public void testUpdatingQuotaCount() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_block_added").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dir = cluster.getNamesystem().getFSDirectory();
            dfs = cluster.getFileSystem();
            ...
            RestartFramework.at("after_file_completed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dir = cluster.getNamesystem().getFSDirectory();
            dfs = cluster.getFileSystem();
