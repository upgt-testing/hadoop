 ...
 */
public class TestCorruptionWithFailover_RestartInjected_Randomized_456 {
    ...
    public void testCorruptReplicaAfterFailover() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(3).build()) {
            ...
            RestartFramework.at("after_file_create").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Write some data and flush.
            for (int i = 0; i < 1024 * 1024; i++) {
                out.write(i);
            }
            out.hsync();
            RestartFramework.at("after_hsync").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_mark_datanodes_stale").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Restart the datanode
            cluster.restartDataNode(dn);
            RestartFramework.at("after_restart_datanode").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_wait_corrupt_blocks").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Perform failover to other namenode
            cluster.transitionToStandby(0);
            cluster.transitionToActive(1);
            cluster.waitActive(1);
            RestartFramework.at("after_failover").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
