...
public class TestSnapshottableDirListing_RestartInjected_Randomized_123 {
    ...
    public void testListSnapshottableDir() throws Exception {
        ...
        RestartFramework.at("after_delete_dir1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disallow_snapshot_sub1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        hdfs.allowSnapshot(dir2);
        RestartFramework.at("after_allow_snapshot_dir2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertEquals(dir2, dirs[1].getFullPath());
        RestartFramework.at("after_allow_snapshot_dir2_again").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(2, dirs[1].getSnapshotNumber());
        // Create sub-dirs under dir1
        Path sub1 = new Path(dir1, "sub1");
        Path file1 = new Path(sub1, "file1");
        RestartFramework.at("after_rename_dir3_to_dir2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        dirs = hdfs.getSnapshottableDirListing();
        RestartFramework.at("after_allow_snapshot_dir1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(4, dirs.length);
        assertEquals(dir1, dirs[0].getFullPath());
        RestartFramework.at("after_allow_snapshot_sub1_sub2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(dir2, dirs[1].getFullPath());
    ...
    public void testListWithDifferentUser() throws Exception {
        ...
        RestartFramework.at("after_list_user1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path dir2_user1 = new Path("/dir2_user1");
        fs1.mkdirs(dir1_user1);
        RestartFramework.at("after_allow_snapshot_user1_dirs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_list_superuser").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_allow_snapshot_dir1_dir2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        UserGroupInformation superUgi = UserGroupInformation.createUserForTesting("superuser", new String[] { supergroup });
        RestartFramework.at("after_allow_snapshot_user2_dirs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
