...
public class TestLocatedBlocksRefresher_RestartInjected_Randomized_456 {
    ...
    public void testRefreshOnDeadNodes() throws Exception {
        ...
        try (DFSInputStream fin = client.open(fileName)) {
            RestartFramework.at("after_stream_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_stream_tracking").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_node_stop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_refresh").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_node_stop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            readBuffer = new byte[chunkReadSize];
            fin.read(0, readBuffer, 0, readBuffer.length);
            RestartFramework.at("after_second_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertRefreshes(refresher, 1);
            RestartFramework.at("after_second_refresh").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            synchronized (fin.infoLock) {
                assertNotSame(locatedBlocks, fin.locatedBlocks);
                assertTrue(fin.getLocalDeadNodes().isEmpty());
            }
    ...
    private void assertRefreshes(LocatedBlocksRefresher refresher, int expectedRefreshes) throws InterruptedException {
        ...
        LOG.info("Waiting for at least {} runs, from current {}. Expecting {} refreshes, from current {}", runCount + expectedRuns, runCount, refreshCount + expectedRefreshes, refreshCount);
