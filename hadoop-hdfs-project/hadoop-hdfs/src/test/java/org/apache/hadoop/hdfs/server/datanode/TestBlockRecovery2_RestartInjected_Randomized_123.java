 ...
 */
public class TestBlockRecovery2_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestBlockRecovery2_RestartInjected.class);
    ...
    private void waitForActiveNN() {
        try {
            GenericTestUtils.waitFor(() -> dn.getAllBpOs().get(0).getActiveNN() != null, 1000, 15 * 1000);
    ...
    public void testRaceBetweenReplicaRecoveryAndFinalizeBlock() throws Exception {
        ...
        configuration.setLong(DFSConfigKeys.DFS_DATANODE_XCEIVER_STOP_TIMEOUT_MILLIS_KEY, 5000L);
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(configuration).numDataNodes(1).build();
        try {
            ...
            out.writeBytes("data");
            out.hsync();
            RestartFramework.at("after_file_creation_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<LocatedBlock> blocks = DFSTestUtil.getAllBlocks(fs.open(path));
            final LocatedBlock block = blocks.get(0);
            final DataNode dataNode = cluster.getDataNodes().get(0);
            RestartFramework.at("after_get_blocks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final AtomicBoolean recoveryInitResult = new AtomicBoolean(true);
            Thread recoveryThread = new Thread(() -> {
                try {
                    DatanodeInfo[] locations = block.getLocations();
                    final BlockRecoveryCommand.RecoveringBlock recoveringBlock = new BlockRecoveryCommand.RecoveringBlock(block.getBlock(), locations, block.getBlock().getGenerationStamp() + 1);
            ...
            RestartFramework.at("after_recovery_thread_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
                out.close();
            } catch (IOException e) {
            ...
            RestartFramework.at("after_close_attempt").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_update_replica").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (null != cluster) {
                cluster.shutdown();
            }
    ...
    public void testEcRecoverBlocks() throws Throwable {
        ...
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(8).build();
        RestartFramework.at("after_cluster_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            cluster.waitActive();
            ...
            doAnswer(delayer).when(spyNN).complete(anyString(), anyString(), any(), anyLong());
            String topDir = "/myDir";
            ...
            t.start();
            RestartFramework.at("after_close_thread_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_recover_lease").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            delayer.proceed();
        } finally {
            cluster.shutdown();
        }
    ...
    public void testRecoveryWillIgnoreMinReplication() throws Exception {
        ...
        try {
            ...
            out.hsync();
            RestartFramework.at("after_file_creation_hsync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_get_block_locations").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_stop_datanodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            GenericTestUtils.waitFor(() -> fsn.getNumDeadDataNodes() == 2, 300, 300000);
            // Make sure hard lease expires to trigger replica recovery
            cluster.setLeasePeriod(100L, 100L);
            RestartFramework.at("after_set_lease_period").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_file_closed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
