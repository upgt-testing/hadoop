...
import static org.apache.hadoop.hdfs.MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR;
...
public class TestFsck {
    ...
    public void testFsck() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(4).build();
    ...
    public void testFsckPermission() throws Exception {
        ...
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(4).build();
        final MiniDockerDFSCluster c2 = cluster;
    ...
    public void testFsckMove() throws Exception {
        ...
        FileSystem fs = null;
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(numDatanodes).build();
    ...
    public void testFsckOpenECFiles() throws Exception {
        ...
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(numAllUnits + 1).build();
        String topDir = "/myDir";
        cluster.waitActive();
    ...
    public void testCorruptBlock() throws Exception {
        ...
        short factor = 1;
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(1).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
    ...
    public void testUnderMinReplicatedBlock() throws Exception {
        ...
        short factor = 1;
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(2).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
    ...
    public void testFsckReplicaDetails() throws Exception {
        ...
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(numDn).hosts(hosts).racks(racks).build();
        cluster.waitClusterUp();
        ...
        final DatanodeManagerInterface dnm = bm.getDatanodeManager();
        DatanodeDescriptorInterface dnDesc0 = dnm.getDatanode(cluster.getDataNodes().get(0).getDatanodeId());
        ...
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            ...
            @Override
            public Boolean get() {
                DatanodeInfoInterface datanodeInfo = null;
                try {
        ...
        assertFalse(fsckOut.contains("(ENTERING MAINTENANCE)"));
        assertFalse(fsckOut.contains("(IN MAINTENANCE)"));
        DatanodeDescriptorInterface dnDesc1 = dnm.getDatanode(cluster.getDataNodes().get(1).getDatanodeId());
        ...
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            ...
            @Override
            public Boolean get() {
                DatanodeInfoInterface dnInfo = null;
                try {
                    for (DatanodeInfoInterface info : dfs.getDataNodeStats()) {
    ...
    public void testFsckError() throws Exception {
        ...
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).build();
        ...
        INodeFileInterface node = (INodeFile) cluster.getNamesystem().dir.getINode(fileName, DirOp.READ);
    ...
    public void testFsckListCorruptFilesBlocks() throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j <= 1; j++) {
                ...
                List<File> metadataFiles = MiniDockerDFSCluster.getAllBlockMetadataFiles(dataDir);
    ...
    public void testFsckMissingReplicas() throws IOException {
        ...
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(numReplicas).build();
        ...
        NameNodeInterface namenode = cluster.getNameNode();
        NetworkTopologyInterface nettop = cluster.getNamesystem().getBlockManager().getDatanodeManager().getNetworkTopology();
    ...
    public void testFsckFileNotFound() throws Exception {
        // Number of replicas to actually start
        final short numReplicas = 1;
        NameNodeInterface namenode = mock(NameNode.class);
        NetworkTopologyInterface nettop = mock(NetworkTopology.class);
        Map<String, String[]> pmap = new HashMap<>();
        Writer result = new StringWriter();
        PrintWriter out = new PrintWriter(result, true);
        InetAddress remoteAddress = InetAddress.getLocalHost();
        FSNamesystemInterface fsName = mock(FSNamesystem.class);
    ...
    public void testBlockIdCKDecommission() throws Exception {
        ...
        DatanodeDescriptorInterface dn = bc.getBlocks()[0].getDatanode(0);
    ...
    public void testBlockIdCKMaintenance() throws Exception {
        ...
        FSNamesystemInterface fsn = cluster.getNameNode().getNamesystem();
        BlockManagerInterface bm = fsn.getBlockManager();
        DatanodeManagerInterface dnm = bm.getDatanodeManager();
        DatanodeDescriptorInterface dn = dnm.getDatanode(cluster.getDataNodes().get(0).getDatanodeId());
    ...
    public void testStoragePoliciesCK() throws Exception {
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(3).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).build();
        cluster.waitActive();
    ...
    public void testECFsck() throws Exception {
        ...
        File builderBaseDir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, builderBaseDir).numDataNodes(totalSize).build();
        fs = cluster.getFileSystem();
    ...
    private void waitForCorruptionBlocks(int corruptBlocks, String path) throws Exception {
        GenericTestUtils.waitFor(() -> {
            try {
                ...
                CorruptFileBlocksInterface corruptFileBlocks = namenode.listCorruptFileBlocks(path, null);
    ...
    private void testUpgradeDomain(boolean defineUpgradeDomain, boolean displayUpgradeDomain) throws Exception {
        ...
        DistributedFileSystem dfs;
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDN).hosts(hosts).racks(racks).build();
        cluster.waitClusterUp();
        dfs = cluster.getFileSystem();
        // Configure the upgrade domain on the datanode
        if (defineUpgradeDomain) {
            DatanodeAdminProperties dnProp = new DatanodeAdminProperties();
            DatanodeIDInterface datanodeID = cluster.getDataNodes().get(0).getDatanodeId();
    ...
    public void testFsckCorruptECFile() throws Exception {
        ...
        for (int i = 0; i < parityBlocks + 1; i++) {
            ...
            File blkFile = MiniDockerDFSCluster.getBlockFile(storageDir, blks[i].getBlock());
    ...
    public void testFsckCorruptWhenOneReplicaIsCorrupt() throws Exception {
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf, new File(GenericTestUtils.getRandomizedTempPath())).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(2).build()) {
            cluster.waitActive();
