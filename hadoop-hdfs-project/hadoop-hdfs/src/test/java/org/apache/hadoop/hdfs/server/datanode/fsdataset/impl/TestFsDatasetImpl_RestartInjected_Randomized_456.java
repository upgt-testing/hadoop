...
public class TestFsDatasetImpl_RestartInjected_Randomized_456 {

    Logger LOG = LoggerFactory.getLogger(TestFsDatasetImpl_RestartInjected.class);
    ...
    @Test
    public void testReadLockEnabledByDefault() throws Exception {
    ...
    @Test
    public void testReadLockCanBeDisabledByConfig() throws Exception {
        HdfsConfiguration conf = new HdfsConfiguration();
        conf.setBoolean(DFSConfigKeys.DFS_DATANODE_LOCK_READ_WRITE_ENABLED_KEY, false);
        ...
        try {
            ...
            assertEquals("The sequence of checkpoints does not correspond to shared lock", syncList, Arrays.asList(0, 1, 2));
        } finally {
            cluster.shutdown();
        }
    }
    ...
    public void testAddVolumeWithSameStorageUuid() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("before_datanode_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDeletingBlocks() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_replica_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dn = cluster.getDataNodes().get(0);
            ds.invalidate(BLOCKPOOL, blockList.toArray(new Block[0]));
            RestartFramework.at("after_block_invalidation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dn = cluster.getDataNodes().get(0);
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
    ...
    public void testReportBadBlocks() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("before_report_bad_blocks").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dataNode = cluster.getDataNodes().get(0);
    ...
    public void testMoveBlockFailure() {
        ...
        try {
            ...
            out.write(100);
            out.hflush();
            RestartFramework.at("after_append_hflush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dataNode = cluster.getDataNodes().get(0);
    ...
    public void testMoveBlockSuccess() {
        ...
        try {
            ...
            FsDatasetImpl fsDataSetImpl = (FsDatasetImpl) dataNode.getFSDataset();
            ReplicaInfo newReplicaInfo = createNewReplicaObj(block, fsDataSetImpl);
            RestartFramework.at("before_finalize_new_replica").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dataNode = cluster.getDataNodes().get(0);
    ...
    public void testBlockReadOpWhileMovingBlock() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_partial_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dataNode = cluster.getDataNodes().get(0);
            ...
            RestartFramework.at("after_block_move").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dataNode = cluster.getDataNodes().get(0);
    ...
    public void testReleaseVolumeRefIfExceptionThrown() throws IOException {
        ...
        try {
            ...
            blockList.add(info);
            RestartFramework.at("after_replica_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTransferAndNativeCopyMetrics() throws IOException {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(config).numDataNodes(1).storageTypes(new StorageType[] { StorageType.DISK, StorageType.DISK }).storagesPerDatanode(2).build()) {
            RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            FileSystem fs = cluster.getFileSystem();
            DataNode dataNode = cluster.getDataNodes().get(0);
            ...
            FsDatasetImpl fsDataSetImpl = (FsDatasetImpl) dataNode.getFSDataset();
            ReplicaInfo newReplicaInfo = createNewReplicaObj(block, fsDataSetImpl);
            RestartFramework.at("before_finalize_replica").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dataNode = cluster.getDataNodes().get(0);
            fsDataSetImpl.finalizeNewReplica(newReplicaInfo, block);
