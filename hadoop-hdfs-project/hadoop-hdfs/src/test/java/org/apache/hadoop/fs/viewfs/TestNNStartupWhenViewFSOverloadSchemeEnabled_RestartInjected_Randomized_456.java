 ...
 */
public class TestNNStartupWhenViewFSOverloadSchemeEnabled_RestartInjected_Randomized_456 {
    ...
    public void testHANameNodeAndDataNodeStartup() throws Exception {
        ...
        cluster.waitActive();
        RestartFramework.at("after_wait_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_ha_cluster_build").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNameNodeAndDataNodeStartup() throws Exception {
        cluster = new MiniDFSCluster.Builder(CONF).numDataNodes(1).waitSafeMode(false).build();
