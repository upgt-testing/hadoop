 ...
 */
public class TestBlockHasMultipleReplicasOnSameDN_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(TestBlockHasMultipleReplicasOnSameDN_RestartInjected.class);
    ...
    public void testBlockHasMultipleReplicasOnSameDN() throws IOException {
        ...
        RestartFramework.at("after_generate_fake_report").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_final_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StorageBlockReport[] reports = new StorageBlockReport[cluster.getStoragesPerDatanode()];
        RestartFramework.at("after_send_block_report").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_get_initial_block_locations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dn = cluster.getDataNodes().get(0);
