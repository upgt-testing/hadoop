 ...
 */
public class TestNameNodeRespectsBindHostKeys_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(TestNameNodeRespectsBindHostKeys_RestartInjected.class);
    ...
    public void testRpcBindHostKey() throws IOException {
        ...
        try {
            ...
            cluster.waitActive();
            RestartFramework.at("after_first_cluster_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            cluster = new MiniDFSCluster.Builder(conf).numDataNodes(0).build();
            cluster.waitActive();
            RestartFramework.at("after_wildcard_bind_cluster_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            String address = getRpcServerAddress(cluster);
    ...
    private static final String BASEDIR = GenericTestUtils.getTempPath(TestNameNodeRespectsBindHostKeys_RestartInjected.class.getSimpleName());
    ...
    private static void setupSsl() throws Exception {
        ...
        sslConfDir = KeyStoreTestUtil.getClasspathDir(TestNameNodeRespectsBindHostKeys_RestartInjected.class);
