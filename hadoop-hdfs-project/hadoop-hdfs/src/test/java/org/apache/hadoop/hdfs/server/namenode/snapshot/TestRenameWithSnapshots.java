...
public class TestRenameWithSnapshots {
    ...
    private static FSNamesystemInterface fsn;

    private static FSDirectoryInterface fsdir;
    ...
    public void setUp() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(REPL).format(true).build();
        cluster.waitActive();
    ...
    private static boolean existsInDiffReport(List<DiffReportEntry> entries, DiffType type, String sourcePath, String targetPath) {
        for (DiffReportEntry entry : entries) {
            if (entry.equals(new DiffReportEntry(type, DFSUtil.string2Bytes(sourcePath), targetPath == null ? null : DFSUtil.string2Bytes(targetPath)))) {
                return true;
            }
        }
        return false;
    ...
    public void testRenameFileAcrossSnapshottableDirs() throws Exception {
        ...
        INodeFileInterface sfoo = fsdir.getINode(newfoo.toString()).asFile();
    ...
    public void testRenameFileAndDeleteSnapshot() throws Exception {
        ...
        assertEquals(REPL, status.getReplication());
        INodeFileInterface snode = fsdir.getINode(newfoo.toString()).asFile();
    ...
    public void testRenameMoreThanOnceAcrossSnapDirs() throws Exception {
        ...
        INodeFileInterface bar1 = fsdir.getINode4Write(bar1_dir1.toString()).asFile();
    ...
    public void testRenameMoreThanOnceAcrossSnapDirs_2() throws Exception {
        ...
        SnapshotInterface s333 = sdir3Node.getSnapshot(DFSUtil.string2Bytes("s333"));
        SnapshotInterface s22 = sdir2Node.getSnapshot(DFSUtil.string2Bytes("s22"));
        SnapshotInterface s1 = sdir1Node.getSnapshot(DFSUtil.string2Bytes("s1"));
    ...
    public void testRenameFromNonSDir2SDir() throws Exception {
        ...
        hdfs.rename(foo, newfoo);
        INodeInterface fooNode = fsdir.getINode4Write(newfoo.toString());
    ...
    public void testRenameAndUpdateSnapshottableDirs() throws Exception {
        ...
        INodeDirectoryInterface fooNode = fsdir.getINode4Write(foo.toString()).asDirectory();
    ...
    public void testRenameWithNestedSnapshottableDirs() throws Exception {
        ...
        try {
            hdfs.rename(foo, bar, Rename.OVERWRITE);
            fail("Except exception since " + "Unable to rename because " + foo.toString() + " has snapshottable descendant directories and " + sdir2.toString() + " is a descent of a snapshottable directory, " + "and HDFS does not support nested snapshottable directory.");
        } catch (IOException e) {
            GenericTestUtils.assertExceptionContains("Unable to rename because " + foo.toString() + " has snapshottable descendant directories and " + sdir2.toString() + " is a descent of a snapshottable directory, " + "and HDFS does not support nested snapshottable directory.", e);
        }
    ...
    public void testRenameDirAndDeleteSnapshot_2() throws Exception {
        ...
        QuotaCountsInterface q = fsdir.getRoot().getDirectoryWithQuotaFeature().getSpaceConsumed();
    ...
    public void testRenameAndAppend() throws Exception {
        ...
        INodeInterface fooRef = fsdir.getINode4Write(foo2.toString());
        ...
        try {
            ...
            assertTrue(fooRef instanceof INodeReference.DstReference);
            INodeFileInterface fooNode = fooRef.asFile();
    ...
    public void testRenameUndo_1() throws Exception {
        ...
        SnapshotTestHelper.createSnapshot(hdfs, sdir1, "s1");
        INodeDirectoryInterface dir2 = fsdir.getINode4Write(sdir2.toString()).asDirectory();
        INodeDirectoryInterface mockDir2 = spy(dir2);
        doReturn(false).when(mockDir2).addChild(any(), anyBoolean(), anyInt());
        INodeDirectoryInterface root = fsdir.getINode4Write("/").asDirectory();
        ...
        // should be empty
        ChildrenDiffInterface childrenDiff = dir1Diffs.get(0).getChildrenDiff();
        ...
        INodeDirectoryInterface dir2Node = fsdir.getINode4Write(sdir2.toString()).asDirectory();
    ...
    public void testRenameUndo_3() throws Exception {
        ...
        SnapshotTestHelper.createSnapshot(hdfs, sdir1, "s1");
        SnapshotTestHelper.createSnapshot(hdfs, sdir2, "s2");
        INodeDirectoryInterface dir3 = fsdir.getINode4Write(sdir3.toString()).asDirectory();
        INodeDirectoryInterface mockDir3 = spy(dir3);
        doReturn(false).when(mockDir3).addChild(any(), anyBoolean(), anyInt());
        INodeDirectoryInterface root = fsdir.getINode4Write("/").asDirectory();
        ...
        SnapshotInterface s1 = dir1Node.getSnapshot(DFSUtil.string2Bytes("s1"));
        INodeDirectoryInterface dir2Node = fsdir.getINode4Write(sdir2.toString()).asDirectory();
        SnapshotInterface s2 = dir2Node.getSnapshot(DFSUtil.string2Bytes("s2"));
        ReadOnlyList<INode> dir2Children = dir2Node.getChildrenList(Snapshot.CURRENT_STATE_ID);
        assertEquals(1, dir2Children.size());
        ...
        SnapshotInterface s3 = dir2Node.getSnapshot(DFSUtil.string2Bytes("s3"));
    ...
    public void testRenameUndo_4() throws Exception {
        ...
        INodeInterface foo3Node = fsdir.getINode4Write(foo3.toString());
        assertTrue(foo3Node.isReference());
        INodeDirectoryInterface dir3 = fsdir.getINode4Write(sdir3.toString()).asDirectory();
        INodeDirectoryInterface mockDir3 = spy(dir3);
        // fail the rename but succeed in undo
        doReturn(false).when(mockDir3).addChild(Mockito.isNull(), anyBoolean(), anyInt());
        Mockito.when(mockDir3.addChild(Mockito.isNotNull(), anyBoolean(), anyInt())).thenReturn(false).thenCallRealMethod();
        INodeDirectoryInterface root = fsdir.getINode4Write("/").asDirectory();
        root.replaceChild(dir3, mockDir3, fsdir.getINodeMap());
        ...
        final INodeInterface foo3Node_undo = fsdir.getINode4Write(foo3.toString());
        assertSame(foo3Node, foo3Node_undo);
        INodeReference.WithCountInterface foo3_wc = (WithCount) foo3Node.asReference().getReferredINode();
    ...
    public void testRenameUndo_5() throws Exception {
        ...
        FSDirectoryInterface fsdir2 = Mockito.spy(fsdir);
        Mockito.doThrow(new NSQuotaExceededException("fake exception")).when(fsdir2).addLastINode(any(), any(), any(), anyBoolean());
        ...
        INodeDirectoryInterface dir1Node = fsdir2.getINode4Write(dir1.toString()).asDirectory();
        List<INode> childrenList = ReadOnlyList.Util.asList(dir1Node.getChildrenList(Snapshot.CURRENT_STATE_ID));
        assertEquals(1, childrenList.size());
        INodeInterface fooNode = childrenList.get(0);
        assertTrue(fooNode.asDirectory().isWithSnapshot());
        INodeInterface barNode = fsdir2.getINode4Write(bar.toString());
        assertTrue(barNode.getClass() == INodeFile.class);
        assertSame(fooNode, barNode.getParent());
        DiffList<DirectoryDiff> diffList = dir1Node.getDiffs().asList();
        assertEquals(1, diffList.size());
        DirectoryDiffInterface diff = diffList.get(0);
        assertSizes(0, 0, diff.getChildrenDiff());
        // check dir2
        INodeDirectoryInterface dir2Node = fsdir2.getINode4Write(dir2.toString()).asDirectory();
        assertTrue(dir2Node.isSnapshottable());
        QuotaCountsInterface counts = dir2Node.computeQuotaUsage(fsdir.getBlockStoragePolicySuite());
        ...
        assertEquals(1, childrenList.size());
        INodeInterface subdir2Node = childrenList.get(0);
    ...
    public void testRenameUndo_6() throws Exception {
        ...
        hdfs.setQuota(dir2, 4, Long.MAX_VALUE - 1);
        FSDirectoryInterface fsdir2 = Mockito.spy(fsdir);
        Mockito.doThrow(new RuntimeException("fake exception")).when(fsdir2).removeLastINode(any());
        Whitebox.setInternalState(fsn, "dir", fsdir2);
        ...
        INodeInterface subsubdir2Node = fsdir2.getINode4Write(subsub_dir2.toString());
    ...
    public void testRenameUndo_7() throws Exception {
        ...
        // check
        INodeDirectoryInterface rootNode = fsdir.getINode4Write(root.toString()).asDirectory();
        INodeDirectoryInterface fooNode = fsdir.getINode4Write(foo.toString()).asDirectory();
        ...
        SnapshotInterface s1 = rootNode.getSnapshot(DFSUtil.string2Bytes(snap1));
        ...
        INodeFileInterface barNode = fsdir.getINode4Write(bar.toString()).asFile();
        assertSame(barNode, children.get(0));
        assertSame(fooNode, barNode.getParent());
        DiffList<FileDiff> barDiffList = barNode.getDiffs().asList();
        assertEquals(1, barDiffList.size());
        FileDiffInterface barDiff = barDiffList.get(0);
    ...
    public void testRenameExceedQuota() throws Exception {
        ...
        // check dir2
        INodeInterface dir2Node = fsdir.getINode4Write(dir2.toString());
    ...
    public void testRenameDirAndDeleteSnapshot_3() throws Exception {
        ...
        hdfs.deleteSnapshot(sdir2, "s3");
        // check
        final INodeDirectoryInterface dir1Node = fsdir.getINode4Write(sdir1.toString()).asDirectory();
        QuotaCountsInterface q1 = dir1Node.getDirectoryWithQuotaFeature().getSpaceConsumed();
        ...
        INodeDirectoryInterface fooNode = wc.getReferredINode().asDirectory();
        ReadOnlyList<INode> children = fooNode.getChildrenList(Snapshot.CURRENT_STATE_ID);
        assertEquals(1, children.size());
        ...
        ChildrenDiffInterface diff = diffList.get(0).getChildrenDiff();
    ...
    public void testRenameDirAndDeleteSnapshot_4() throws Exception {
        ...
        final INodeInterface fooRef2 = fsdir.getINode4Write(foo.toString());
        assertTrue(fooRef2 instanceof INodeReference.DstReference);
        INodeReference.WithCountInterface wc2 = (WithCount) fooRef2.asReference().getReferredINode();
        assertSame(wc, wc2);
    ...
    public void testRenameDirAndDeleteSnapshot_5() throws Exception {
        ...
        INodeFileInterface barNode = (INodeFile) fsdir.getINode4Write(bar3.toString());
    ...
    public void testRenameDirAndDeleteSnapshot_6() throws Exception {
        ...
        INodeDirectoryInterface dir2Node = fsdir.getINode4Write(dir2.toString()).asDirectory();
        assertTrue("the diff list of " + dir2 + " should be empty after deleting s0", !dir2Node.isWithSnapshot());
        assertTrue(hdfs.exists(newfoo));
        INodeInterface fooRefNode = fsdir.getINode4Write(newfoo.toString());
    ...
    public void testRenameDirAndDeleteSnapshot_7() throws Exception {
        ...
        final ChildrenDiffInterface childrenDiff = dir1DiffList.get(0).getChildrenDiff();
        assertSizes(1, 0, childrenDiff);
        INodeInterface cNode = childrenDiff.getCreatedUnmodifiable().get(0);
        INodeInterface fooNode = fsdir.getINode4Write(newfoo.toString());
        assertSame(cNode, fooNode);
        // check foo and its subtree
        final Path newbar = new Path(newfoo, bar.getName());
        INodeDirectoryInterface barNode = fsdir.getINode4Write(newbar.toString()).asDirectory();
        ...
        assertEquals(1, barDiffList.size());
        DirectoryDiffInterface diff = barDiffList.get(0);
        INodeDirectoryInterface testNode = fsdir.getINode4Write(test.toString()).asDirectory();
        SnapshotInterface s0 = testNode.getSnapshot(DFSUtil.string2Bytes("s0"));
    ...
    public void testRenameWithOverWrite() throws Exception {
        ...
        SnapshotDiffReportInterface report = hdfs.getSnapshotDiffReport(root, "s0", "s1");
        LOG.info("DiffList is \n\"" + report.toString() + "\"");
    ...
    String printTree(String label) throws Exception {
        ...
        final String b = fsn.getFSDirectory().getINode("/").dumpTreeRecursively().toString();
        output.println(b);
        return b;
    }
    ...
    public void testQuotaForRenameFileInSnapshot() throws Exception {
        ...
        assertEquals(hdfs.getContentSummary(new Path("/")).getSpaceConsumed(), hdfs.getQuotaUsage(new Path("/")).getSpaceConsumed());
        assertEquals(hdfs.getContentSummary(new Path("/")).getFileAndDirectoryCount(), hdfs.getQuotaUsage(new Path("/")).getFileAndDirectoryCount());
    }
}
