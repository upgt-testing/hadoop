...
public class TestDatanodeReport {
    ...
    public void testDatanodeReport() throws Exception {
        ...
        500);
        conf.setLong(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY, 1L);
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_OF_DATANODES).build();
        try {
            //wait until the cluster is up
            cluster.waitActive();
            ...
            final DataNodeInterface last = datanodes.get(datanodes.size() - 1);
    ...
    static void assertReports(int numDatanodes, DatanodeReportType type, DFSClient client, List<DataNodeInterface> datanodes, String bpid) throws IOException {
        ...
        for (int i = 0; i < infos.length; i++) {
            ...
            final DataNodeInterface d = findDatanode(infos[i].getDatanodeUuid(), datanodes);
            if (bpid != null) {
                //check storage
    ...
    static DataNodeInterface findDatanode(String id, List<DataNodeInterface> datanodes) {
        for (DataNodeInterface d : datanodes) {
