 ...
 */
public class TestSeveralNameNodes_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestSeveralNameNodes.class);
    ...
    public void testCircularLinkedListWrites() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_transition_active").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_setup_writers").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            harness.startThreads();
            RestartFramework.at("after_start_threads").on(cluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_writers_complete").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            harness.stopThreads();
            RestartFramework.at("after_stop_threads").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            System.err.println("===========================\n\n\n\n");
            harness.shutdown();
        }
