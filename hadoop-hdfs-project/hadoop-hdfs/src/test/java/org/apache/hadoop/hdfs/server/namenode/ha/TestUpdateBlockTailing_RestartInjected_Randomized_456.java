 ...
 */
public class TestUpdateBlockTailing_RestartInjected_Randomized_456 {
    ...
    public void testStandbyAddBlockIBRRace() throws Exception {
        RestartFramework.at("after_ibr_processing").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_editlog_tail").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_initial_genstamp_check").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        StorageReceivedDeletedBlocks[] report = DFSTestUtil.makeReportForReceivedBlock(newBlock, ReceivedDeletedBlockInfo.BlockStatus.RECEIVED_BLOCK, dn0.getFSDataset().getStorage(targets[0].getStorageID()));
        ...
        fsn0.getEditLog().logSync();
        fsn1.getEditLogTailer().doTailEdits();
        RestartFramework.at("after_update_block_tail").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertEquals("Global Generation stamps on NN0 and " + "impending on NN1 should be equal", NameNodeAdapter.getGenerationStamp(fsn0), NameNodeAdapter.getImpendingGenerationStamp(fsn1));
        ...
        assertEquals("Global Generation stamps on new active should be " + "the same as on the old one", gs0, NameNodeAdapter.getGenerationStamp(fsn1));
        rpc1.delete(testFile, false);
    }

    @Test
    public void testStandbyAppendBlock() throws Exception {
        RestartFramework.at("after_editlog_tail_file_creation").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_creation").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        fsn0.getEditLog().logSync();
        fsn1.getEditLogTailer().doTailEdits();
        RestartFramework.at("after_editlog_tail_append").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_append_operation").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals("Global Generation stamps on NN0 and " + "NN1 should be equal", NameNodeAdapter.getGenerationStamp(fsn0), NameNodeAdapter.getGenerationStamp(fsn1));
    ...
    public void testStandbyAppendNewBlock() throws Exception {
        ...
        fsn0.getEditLog().logSync();
        fsn1.getEditLogTailer().doTailEdits();
        RestartFramework.at("after_append_new_block").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_editlog_tail_file_creation").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertEquals("Global Generation stamps on NN0 and " + "NN1 should be equal", NameNodeAdapter.getGenerationStamp(fsn0), NameNodeAdapter.getGenerationStamp(fsn1));
        // Append block with newBlock flag
        try (FSDataOutputStream out = dfs.append(new Path(testFile), EnumSet.of(CreateFlag.APPEND, CreateFlag.NEW_BLOCK), 4096, null)) {
            final byte[] data = new byte[1 << 16];
            ThreadLocalRandom.current().nextBytes(data);
            out.write(data);
        }
        ...
        fsn0.getEditLog().logSync();
        fsn1.getEditLogTailer().doTailEdits();
        RestartFramework.at("after_editlog_tail_append_new_block").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertEquals("Global Generation stamps on NN0 and " + "NN1 should be equal", NameNodeAdapter.getGenerationStamp(fsn0), NameNodeAdapter.getGenerationStamp(fsn1));
        RestartFramework.at("after_file_creation").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Remove the testFile
        final ClientProtocol rpc0 = dfsCluster.getNameNode(0).getRpcServer();
    ...
    public void testStandbyTruncateBlock() throws Exception {
        RestartFramework.at("after_truncate_operation").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final String testFile = TEST_DIR + "/testStandbyTruncateBlock";
        RestartFramework.at("after_editlog_tail_truncate").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        final long fileLen = 1 << 16;
        // Create a file
        DFSTestUtil.createFile(dfs, new Path(testFile), fileLen, (short) 1, 0);
        // NN1 tails OP_SET_GENSTAMP_V2 and OP_ADD_BLOCK
