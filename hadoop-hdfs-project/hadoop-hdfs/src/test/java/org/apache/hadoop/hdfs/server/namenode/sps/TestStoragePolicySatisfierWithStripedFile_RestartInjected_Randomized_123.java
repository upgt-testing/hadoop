 ...
 * storage policy.
 */
public class TestStoragePolicySatisfierWithStripedFile_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestStoragePolicySatisfierWithStripedFile_RestartInjected.class);
    ...
    public void testMoverWithFullStripe() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_new_datanodes_added").on(cluster).restart("datanode").withIndex(11).withMode(RestartMode.GRACEFUL).execute();
            // move file to ARCHIVE
            client.setStoragePolicy(barDir, "COLD");
            RestartFramework.at("after_set_cold_policy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            hdfsAdmin.satisfyStoragePolicy(new Path(fooFile));
            LOG.info("Sets storage policy to COLD and invoked satisfyStoragePolicy");
            cluster.triggerHeartbeats();
            RestartFramework.at("after_satisfy_storage_policy").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // verify storage types and locations
    ...
    public void testWhenOnlyFewTargetNodesAreAvailableToSatisfyStoragePolicy() throws Exception {
        ...
        try {
            ...
            cluster.startDataNodes(conf, 2, new StorageType[][] { { StorageType.ARCHIVE, StorageType.ARCHIVE }, { StorageType.ARCHIVE, StorageType.ARCHIVE } }, true, null, null, null, capacities, null, false, false, false, null, null, null);
            cluster.triggerHeartbeats();
            RestartFramework.at("after_new_datanodes_added").on(cluster).restart("datanode").withIndex(11).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSPSWhenFileHasLowRedundancyBlocks() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_satisfy_storage_policy_partial_cluster").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.waitActive();
            RestartFramework.at("after_all_datanodes_restarted").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // verify storage types and locations.
