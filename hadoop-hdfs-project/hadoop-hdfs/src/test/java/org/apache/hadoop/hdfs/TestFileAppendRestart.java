...
public class TestFileAppendRestart {
    ...
    public void testLoadLogsFromBuggyEarlierVersions() throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).numDataNodes(0).waitSafeMode(false).startupOption(StartupOption.UPGRADE).build();
        try {
            FileSystem fs = cluster.getFileSystem();
    ...
    public void testAppendWithPipelineRecovery() throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).manageDataDfsDirs(true).manageNameDfsDirs(true).numDataNodes(4).racks(new String[] { "/rack1", "/rack1", "/rack2", "/rack2" }).build();
            cluster.waitActive();
