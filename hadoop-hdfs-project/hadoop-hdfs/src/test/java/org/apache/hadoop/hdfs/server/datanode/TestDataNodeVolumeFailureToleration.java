...
public class TestDataNodeVolumeFailureToleration {
    ...
    @Test
    public void testValidVolumesAtStartup_withUpgrade20() throws Exception {
        assumeNotWindows();
        // Make sure no DNs are running.
        cluster.shutdownDataNodes();
        // Bring up a datanode with two default data dirs, but with one bad one.
    ...
    @Test
    public void testValidVolumesAtStartup_withUpgrade40() throws Exception {
        assumeNotWindows();
        // Make sure no DNs are running.
        cluster.shutdownDataNodes();
        // Bring up a datanode with two default data dirs, but with one bad one.
    ...
    @Test
    public void testValidVolumesAtStartup_withUpgrade60() throws Exception {
        assumeNotWindows();
        // Make sure no DNs are running.
        cluster.shutdownDataNodes();
        // Bring up a datanode with two default data dirs, but with one bad one.
    ...
    @Test
    public void testValidVolumesAtStartup_withUpgrade80() throws Exception {
        assumeNotWindows();
        // Make sure no DNs are running.
        cluster.shutdownDataNodes();
        // Bring up a datanode with two default data dirs, but with one bad one.
    ...
    @Test
    public void testConfigureMinValidVolumes_withUpgrade20() throws Exception {
        assumeNotWindows();
        // functioning in order to stay up.
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FAILED_VOLUMES_TOLERATED_KEY, 0);
        conf.setTimeDuration(DFSConfigKeys.DFS_DATANODE_DISK_CHECK_MIN_GAP_KEY, 0, TimeUnit.MILLISECONDS);
    ...
    @Test
    public void testConfigureMinValidVolumes_withUpgrade40() throws Exception {
        assumeNotWindows();
        // functioning in order to stay up.
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FAILED_VOLUMES_TOLERATED_KEY, 0);
        conf.setTimeDuration(DFSConfigKeys.DFS_DATANODE_DISK_CHECK_MIN_GAP_KEY, 0, TimeUnit.MILLISECONDS);
    ...
    @Test
    public void testConfigureMinValidVolumes_withUpgrade60() throws Exception {
        assumeNotWindows();
        // functioning in order to stay up.
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FAILED_VOLUMES_TOLERATED_KEY, 0);
        conf.setTimeDuration(DFSConfigKeys.DFS_DATANODE_DISK_CHECK_MIN_GAP_KEY, 0, TimeUnit.MILLISECONDS);
    ...
    @Test
    public void testConfigureMinValidVolumes_withUpgrade80() throws Exception {
        assumeNotWindows();
        // functioning in order to stay up.
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FAILED_VOLUMES_TOLERATED_KEY, 0);
        conf.setTimeDuration(DFSConfigKeys.DFS_DATANODE_DISK_CHECK_MIN_GAP_KEY, 0, TimeUnit.MILLISECONDS);
    ...
    @Test
    public void testFailedVolumeOnStartupIsCounted_withUpgrade20() throws Exception {
        assumeNotWindows();
        final DatanodeManagerJVMInterface dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        //long origCapacity = DFSTestUtil.getLiveDatanodeCapacity(dm);
        File dir = new File(cluster.getInstanceStorageDir(0, 0), "current");
    ...
    @Test
    public void testFailedVolumeOnStartupIsCounted_withUpgrade40() throws Exception {
        assumeNotWindows();
        final DatanodeManagerJVMInterface dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        //long origCapacity = DFSTestUtil.getLiveDatanodeCapacity(dm);
        File dir = new File(cluster.getInstanceStorageDir(0, 0), "current");
    ...
    @Test
    public void testFailedVolumeOnStartupIsCounted_withUpgrade60() throws Exception {
        assumeNotWindows();
        final DatanodeManagerJVMInterface dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        //long origCapacity = DFSTestUtil.getLiveDatanodeCapacity(dm);
        File dir = new File(cluster.getInstanceStorageDir(0, 0), "current");
    ...
    @Test
    public void testFailedVolumeOnStartupIsCounted_withUpgrade80() throws Exception {
        assumeNotWindows();
        final DatanodeManagerJVMInterface dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        //long origCapacity = DFSTestUtil.getLiveDatanodeCapacity(dm);
        File dir = new File(cluster.getInstanceStorageDir(0, 0), "current");
