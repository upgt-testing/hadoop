...
public class TestSecureNameNode_RestartInjected_Randomized_456 extends SaslDataTransferTestCase {
    ...
    public void testName() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_superuser_login").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsForSuperUser.mkdirs(new Path("/tmp"));
            fsForSuperUser.setPermission(new Path("/tmp"), new FsPermission((short) 511));
            RestartFramework.at("after_tmp_directory_created_with_permissions").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_regular_user_login").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path p = new Path("/mydir");
            exception.expect(IOException.class);
            fs.mkdirs(p);
            Path tmp = new Path("/tmp/alpha");
            fs.mkdirs(tmp);
            RestartFramework.at("after_directory_created_in_tmp").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNameNodeStatusMXBeanSecurityEnabled() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(simpleConf).build()) {
            ...
            RestartFramework.at("after_simple_cluster_started").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_simple_security_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(secureConf).build()) {
            cluster.waitActive();
            NameNode namenode = cluster.getNameNode();
            RestartFramework.at("after_secure_cluster_started").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Assert.assertEquals(namenode.isSecurityEnabled(), securityEnabled);
            RestartFramework.at("after_secure_security_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            namenode = cluster.getNameNode();
        }
    }
}
