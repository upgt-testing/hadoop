...
import org.apache.hadoop.hdfs.MiniDockerDFSClusterWithNodeGroup;
...
public class TestBalancerWithNodeGroup {
    ...
    public void testBalancerWithRackLocality() throws Exception {
        ...
        MiniDockerDFSCluster.Builder builder = new MiniDockerDFSCluster.Builder(conf).numDataNodes(capacities.length).racks(racks).simulatedCapacities(capacities);
        MiniDockerDFSClusterWithNodeGroup.setNodeGroups(nodeGroups);
        cluster = new MiniDockerDFSClusterWithNodeGroup(builder);
        try {
            cluster.waitActive();
    ...
    public void testBPPNodeGroup() throws Exception {
        ...
        MiniDockerDFSCluster.Builder builder = new MiniDockerDFSCluster.Builder(conf).numDataNodes(capacities.length).racks(racks).simulatedCapacities(capacities);
        MiniDockerDFSClusterWithNodeGroup.setNodeGroups(nodeGroups);
        LambdaTestUtils.intercept(IllegalArgumentException.class, "Configured cluster topology should be " + "org.apache.hadoop.net.NetworkTopologyWithNodeGroup", () -> new MiniDockerDFSClusterWithNodeGroup(builder));
    }
}
