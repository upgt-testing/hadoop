...
public class TestJournal {
    ...
    private static final File TEST_LOG_DIR = new File(new File(MiniDockerDFSCluster.getBaseDirectory()), "TestJournal");
    ...
    public void testEmptyEditsInProgressMovedAside() throws Exception {
        ...
        journal.startLogSegment(makeRI(4), 6, NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION - 1);
    ...
    public void testReadFromCache() throws Exception {
        ...
        assertJournaledEditsTxnCountAndContents(1, 7, 7, NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION);
    ...
    private void assertJournaledEditsTxnCountAndContents(int startTxn, int requestedMaxTxns, int expectedEndTxn, int layoutVersion) throws Exception {
        GetJournaledEditsResponseProto result = journal.getJournaledEdits(startTxn, requestedMaxTxns);
        int expectedTxnCount = expectedEndTxn - startTxn + 1;
        ByteArrayOutputStream headerBytes = new ByteArrayOutputStream();
        EditLogFileOutputStream.writeHeader(layoutVersion, new DataOutputStream(headerBytes));
        assertEquals(expectedTxnCount, result.getTxnCount());
        assertArrayEquals(Bytes.concat(headerBytes.toByteArray(), QJMTestUtil.createTxnData(startTxn, expectedTxnCount)), result.getEditLog().toByteArray());
    }
