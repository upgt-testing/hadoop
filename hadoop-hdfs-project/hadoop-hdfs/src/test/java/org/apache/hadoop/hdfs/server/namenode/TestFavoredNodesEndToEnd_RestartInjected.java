...
public class TestFavoredNodesEndToEnd_RestartInjected {
  ...
  public void testFavoredNodesEndToEnd() throws Exception {
    ...
    for (int i = 0; i < NUM_FILES; i++) {
      if (i == 5) {
        RestartFramework.at("during_file_creation")
            .on(cluster)
            .restart("namenode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  public void testWhenFavoredNodesNotPresent() throws Exception {
    ...
    out.write(SOME_BYTES);
    out.close();
    RestartFramework.at("after_create_with_arbitrary_nodes")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testWhenSomeNodesAreNotGood() throws Exception {
    ...
    d.setDecommissioned();
    RestartFramework.at("after_decommission_node")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    out.write(SOME_BYTES);
    out.close();
    RestartFramework.at("after_create_with_decommissioned_node")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    //reset the state
    d.stopDecommission();
    RestartFramework.at("after_reset_decommission")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testFavoredNodesEndToEndForAppend() throws Exception {
    ...
    for (int i = 0; i < NUM_FILES; i++) {
      if (i == 5) {
        RestartFramework.at("during_append_loop")
            .on(cluster)
            .restart("namenode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
  ...
  public void testCreateStreamBuilderFavoredNodesEndToEnd() throws Exception {
    ...
    for (int i = 0; i < NUM_FILES; i++) {
      if (i == 5) {
        RestartFramework.at("during_builder_loop")
            .on(cluster)
            .restart("namenode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_builder_test")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
