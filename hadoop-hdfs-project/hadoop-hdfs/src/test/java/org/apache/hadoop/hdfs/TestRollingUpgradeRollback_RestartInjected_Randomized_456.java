 ...
 */
public class // TODO: rollback could not succeed in all JN
TestRollingUpgradeRollback_RestartInjected_Randomized_456 {
    ...
    public void testRollbackCommand() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_storage_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    public void testRollbackWithQJM() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_namenode_restart_rollback").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // make sure /foo is still there, but /bar is not
            dfs = cluster.getFileSystem();
            Assert.assertTrue(dfs.exists(foo));
            Assert.assertFalse(dfs.exists(bar));
            RestartFramework.at("after_rollback_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRollbackWithHAQJM() throws Exception {
        ...
        try {
            ...
            Assert.assertTrue(dfs.exists(foo));
            Assert.assertFalse(dfs.exists(bar));
            RestartFramework.at("after_rollback_verification").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // check the details of NNStorage
            ...
            RestartFramework.at("after_second_namenode_restart").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // start the rolling upgrade again to make sure we do not load upgrade
            // status after the rollback
            dfsCluster.transitionToActive(0);
            RestartFramework.at("after_second_active_transition").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
