...
public class TestEditLogRace_RestartInjected_Randomized_123 {
    ...
    public TestEditLogRace_RestartInjected_Randomized_123(boolean useAsyncEditLog) {
        TestEditLogRace_RestartInjected.useAsyncEditLog = useAsyncEditLog;
    }
    ...
    public void testEditLogRolling() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_start_workers").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            for (int i = 0; i < NUM_ROLLS && caughtErr.get() == null; i++) {
                ...
                if (i == 15) {
                    RestartFramework.at("during_rolling").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
            }
        } finally {
    ...
    public void testSaveNamespace() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < NUM_SAVE_IMAGE && caughtErr.get() == null; i++) {
                ...
                if (i == 15) {
                    RestartFramework.at("during_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    namesystem = cluster.getNamesystem();
                    fsimage = cluster.getNameNode().getFSImage();
                    editLog = cluster.getNamesystem().getEditLog();
                }
