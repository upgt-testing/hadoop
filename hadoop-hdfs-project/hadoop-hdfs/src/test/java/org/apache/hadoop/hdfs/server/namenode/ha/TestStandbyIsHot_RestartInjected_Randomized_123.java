 ...
 */
public class TestStandbyIsHot_RestartInjected_Randomized_123 {

    protected static final Logger LOG = LoggerFactory.getLogger(TestStandbyIsHot_RestartInjected.class);
    ...
    public void testStandbyIsHot() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_block_locations_on_standby").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn1 = cluster.getNameNode(0);
            nn2 = cluster.getNameNode(1);
            ...
            RestartFramework.at("after_heartbeats_and_block_reports").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Change replication
            ...
            nn1.getRpcServer().rollEditLog();
            RestartFramework.at("after_replication_change_to_1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn1 = cluster.getNameNode(0);
            nn2 = cluster.getNameNode(1);
            LOG.info("Waiting for lowered replication to show up on standby");
            waitForBlockLocations(cluster, nn2, TEST_FILE, 1);
            RestartFramework.at("after_lowered_replication_on_standby").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn1 = cluster.getNameNode(0);
            nn2 = cluster.getNameNode(1);
            // Change back to 3
            ...
            BlockManagerTestUtil.computeAllPendingWork(nn1.getNamesystem().getBlockManager());
            nn1.getRpcServer().rollEditLog();
            RestartFramework.at("after_replication_change_to_3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn1 = cluster.getNameNode(0);
            nn2 = cluster.getNameNode(1);
            LOG.info("Waiting for higher replication to show up on standby");
            waitForBlockLocations(cluster, nn2, TEST_FILE, 3);
            RestartFramework.at("after_higher_replication_on_standby").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn1 = cluster.getNameNode(0);
            nn2 = cluster.getNameNode(1);
        } finally {
            cluster.shutdown();
    ...
    public void testDatanodeRestarts() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_file_creation_and_standby_catchup").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn0 = cluster.getNameNode(0);
            nn1 = cluster.getNameNode(1);
            // Stop the DN.
            ...
            RestartFramework.at("after_dn_stop_and_death_notice").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn0 = cluster.getNameNode(0);
            nn1 = cluster.getNameNode(1);
            ...
            RestartFramework.at("after_dn_restart_and_block_report").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_block_location_verification").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn0 = cluster.getNameNode(0);
            nn1 = cluster.getNameNode(1);
        } finally {
            cluster.shutdown();
