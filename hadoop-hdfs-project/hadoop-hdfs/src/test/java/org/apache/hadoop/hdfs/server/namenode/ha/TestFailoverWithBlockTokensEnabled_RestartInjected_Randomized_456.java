...
public class TestFailoverWithBlockTokensEnabled_RestartInjected_Randomized_456 {
    ...
    public void ensureSerialNumbersNeverOverlap() {
        ...
        RestartFramework.at("after_get_secret_managers").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void ensureInvalidBlockTokensAreRejected() throws IOException, URISyntaxException {
        ...
        RestartFramework.at("after_write_file").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_setup_spy_client").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            assertEquals(TEST_DATA, DFSTestUtil.readFile(fs, TEST_PATH));
    ...
    public void testFailoverAfterRegistration() throws IOException, URISyntaxException {
        ...
        RestartFramework.at("after_write_using_both_namenodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void TestFailoverAfterAccessKeyUpdate() throws IOException, URISyntaxException, InterruptedException {
        RestartFramework.at("after_lower_key_update_interval").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        lowerKeyUpdateIntervalAndClearKeys(cluster);
        RestartFramework.at("after_sleep").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Sleep 10s to guarantee DNs heartbeat and get new keys.
        Thread.sleep(10 * 1000);
        RestartFramework.at("after_write_using_both_namenodes").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        writeUsingBothNameNodes();
    }
