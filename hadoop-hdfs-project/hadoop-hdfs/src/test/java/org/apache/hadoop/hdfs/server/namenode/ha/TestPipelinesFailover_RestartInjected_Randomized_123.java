 ...
 */
public class TestPipelinesFailover_RestartInjected_Randomized_123 {

    static {
        GenericTestUtils.setLogLevel(LoggerFactory.getLogger(RetryInvocationHandler.class), org.slf4j.event.Level.DEBUG);
    ...
    static {
        ...
        LOG.info("Using random seed: " + FAILOVER_SEED + " for selecting active target NN during failover");
    }
    ...
    private void doWriteOverFailoverTest(TestScenario scenario, MethodToTestIdempotence methodToTest) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_first_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_failover").on(cluster).restart("namenode").withIndex(activeIndex).withMode(RestartMode.GRACEFUL).execute();
            ...
            if (methodToTest == MethodToTestIdempotence.ALLOCATE_BLOCK) {
                ...
                sizeWritten += BLOCK_AND_A_HALF;
                RestartFramework.at("after_second_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            RestartFramework.at("before_file_close").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stm.close();
            stm = null;
    ...
    private void doTestWriteOverFailoverWithDnFail(TestScenario scenario) throws Exception {
        ...
        try {
            ...
            cluster.transitionToActive(0);
            Thread.sleep(500);
            RestartFramework.at("after_cluster_setup_dnfail").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Starting with NN 0 active");
            FileSystem fs = HATestUtil.configureFailoverFs(cluster, conf);
            stm = fs.create(TEST_PATH);
            RestartFramework.at("after_file_creation_dnfail").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // write a block and a half
            AppendTestUtil.write(stm, 0, BLOCK_AND_A_HALF);
            RestartFramework.at("after_first_write_dnfail").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Make sure all the blocks are written before failover
            stm.hflush();
            RestartFramework.at("after_first_hflush_dnfail").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int nextActive = failover(cluster, scenario);
            RestartFramework.at("after_failover_dnfail").on(cluster).restart("namenode").withIndex(nextActive).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(fs.exists(TEST_PATH));
            cluster.stopDataNode(0);
            RestartFramework.at("after_dn_stop_dnfail").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // write another block and a half
            AppendTestUtil.write(stm, BLOCK_AND_A_HALF, BLOCK_AND_A_HALF);
            stm.hflush();
            RestartFramework.at("after_second_hflush_dnfail").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Failing back from NN " + nextActive + " to NN 0");
            cluster.transitionToStandby(nextActive);
            cluster.transitionToActive(0);
            RestartFramework.at("after_failback_dnfail").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.stopDataNode(1);
            RestartFramework.at("after_second_dn_stop_dnfail").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            AppendTestUtil.write(stm, BLOCK_AND_A_HALF * 2, BLOCK_AND_A_HALF);
            stm.hflush();
            RestartFramework.at("after_third_hflush_dnfail").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_close_dnfail").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            stm.close();
            stm = null;
    ...
    public void testLeaseRecoveryAfterFailover() throws Exception {
        ...
        try {
            ...
            Thread.sleep(500);
            RestartFramework.at("after_cluster_setup_lease").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Starting with NN 0 active");
            FileSystem fs = HATestUtil.configureFailoverFs(cluster, conf);
            stm = fs.create(TEST_PATH);
            RestartFramework.at("after_file_creation_lease").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // write a block and a half
            AppendTestUtil.write(stm, 0, BLOCK_AND_A_HALF);
            RestartFramework.at("after_write_lease").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stm.hflush();
            RestartFramework.at("after_hflush_lease").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Failing over to NN 1");
            cluster.transitionToStandby(0);
            cluster.transitionToActive(1);
            RestartFramework.at("after_failover_lease").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(fs.exists(TEST_PATH));
            FileSystem fsOtherUser = createFsAsOtherUser(cluster, conf);
            loopRecoverLease(fsOtherUser, TEST_PATH);
            RestartFramework.at("after_lease_recovery").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            AppendTestUtil.check(fs, TEST_PATH, BLOCK_AND_A_HALF);
            // Fail back to ensure that the block locations weren't lost on the
            // original node.
            cluster.transitionToStandby(1);
            cluster.transitionToActive(0);
            RestartFramework.at("after_failback_lease").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            AppendTestUtil.check(fs, TEST_PATH, BLOCK_AND_A_HALF);
        } finally {
            IOUtils.closeStream(stm);
            cluster.shutdown();
    ...
    public void testFailoverRightBeforeCommitSynchronization() throws Exception {
        ...
        try {
            ...
            cluster.transitionToActive(0);
            Thread.sleep(500);
            RestartFramework.at("after_cluster_setup_commit_sync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("Starting with NN 0 active");
            FileSystem fs = HATestUtil.configureFailoverFs(cluster, conf);
            stm = fs.create(TEST_PATH);
            RestartFramework.at("after_file_creation_commit_sync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // write a half block
            AppendTestUtil.write(stm, 0, BLOCK_SIZE / 2);
            RestartFramework.at("after_write_commit_sync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stm.hflush();
            RestartFramework.at("after_hflush_commit_sync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Mockito.any());
            RestartFramework.at("after_spy_setup_commit_sync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.transitionToStandby(0);
            cluster.transitionToActive(1);
            RestartFramework.at("after_failover_commit_sync").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            // active.
            loopRecoverLease(fsOtherUser, TEST_PATH);
            RestartFramework.at("after_lease_recovery_commit_sync").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            nn0 = cluster.getNameNode(0);
    ...
    private MiniDFSCluster newMiniCluster(Configuration conf, int dnCount) throws IOException {
        return new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology(NN_COUNT)).numDataNodes(dnCount).build();
    }
    ...
    public void testPipelineRecoveryStress() throws Exception {
        ...
        try {
            cluster.waitActive();
            cluster.transitionToActive(0);
            RestartFramework.at("after_cluster_setup_stress").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            FileSystem fs = harness.getFailoverFs();
            DistributedFileSystem fsAsOtherUser = createFsAsOtherUser(cluster, harness.conf);
            RestartFramework.at("after_fs_creation_stress").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_thread_setup_stress").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_threads_started_stress").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     * @return the index of the new active NN
     */
    private int failover(MiniDFSCluster cluster, TestScenario scenario, int activeIndex) throws IOException {
    ...
    private DistributedFileSystem createFsAsOtherUser(final MiniDFSCluster cluster, final Configuration conf) throws IOException, InterruptedException {
        return (DistributedFileSystem) UserGroupInformation.createUserForTesting("otheruser", new String[] { "othergroup" }).doAs(new PrivilegedExceptionAction<FileSystem>() {
