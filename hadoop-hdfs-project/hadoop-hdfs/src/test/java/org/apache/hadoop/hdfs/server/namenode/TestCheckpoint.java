...
public class TestCheckpoint {
    ...
    public void testNameDirError() throws IOException {
        ...
        for (URI nameDirUri : nameDirs) {
            ...
            try {
                ...
                cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).build();
   ...
   */
    ...
    public void testTooManyEditReplayFailures() throws IOException {
        ...
        FileSystem fs = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDatanodes).checkExitOnShutdown(false).build();
            cluster.waitActive();
            fs = cluster.getFileSystem();
            fos = fs.create(new Path("tmpfile0"));
            fos.write(new byte[] { 0, 1, 2, 3 });
   ...
   */
    ...
    public void testSecondaryNamenodeError1() throws IOException {
        ...
        //
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDatanodes).format(false).build();
            cluster.waitActive();
            fileSys = cluster.getFileSystem();
            checkFile(fileSys, file1, replication);
   ...
   */
    ...
    private void doSecondaryFailsToReturnImage() throws IOException {
        ...
        try {
            ...
            assertTrue(!fileSys.exists(file1));
            StorageDirectoryInterface sd = image.getStorage().getStorageDir(0);
            ...
            for (StorageDirectoryInterface sd2 : image.getStorage().dirIterable(NameNodeDirType.IMAGE)) {
    ...
    public void testNameDirLocking() throws IOException {
        ...
        // directories
        StorageDirectoryInterface savedSd = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).build();
            NNStorageInterface storage = cluster.getNameNode().getFSImage().getStorage();
    ...
    public void testSeparateEditsDirLocking() throws IOException {
        ...
        File editsDir = new File(MiniDockerDFSCluster.getBaseDirectory(), "testSeparateEditsDirLocking");
        ...
        StorageDirectoryInterface savedSd = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).manageNameDfsDirs(false).numDataNodes(0).build();
            NNStorageInterface storage = cluster.getNameNode().getFSImage().getStorage();
            for (StorageDirectoryInterface sd : storage.dirIterable(NameNodeDirType.EDITS)) {
    ...
    public void testSecondaryNameNodeLocking() throws Exception {
        ...
        try {
            ...
            // dirs got locked.
            secondary = startSecondaryNameNode(conf);
            NNStorageInterface storage = secondary.getFSImage().getStorage();
            for (StorageDirectoryInterface sd : storage.dirIterable(null)) {
                assertLockFails(sd);
                savedSd = sd;
            }
    ...
    private static void assertClusterStartFailsWhenDirLocked(Configuration conf, StorageDirectory sdToLock) throws IOException {
        ...
        MiniDockerDFSCluster cluster = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).format(false).manageNameDfsDirs(false).numDataNodes(0).build();
    ...
    public void testImportCheckpoint() throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).startupOption(StartupOption.IMPORT).build();
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).format(false).numDataNodes(0).startupOption(StartupOption.IMPORT).build();
    ...
    SecondaryNameNode startSecondaryNameNode(Configuration conf, int index) throws IOException {
        ...
        snnConf.set(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_DIR_KEY, MiniDockerDFSCluster.getBaseDirectory() + "/2nn-" + index);
    ...
    public void testCheckpoint() throws IOException {
        ...
        try {
            ...
            secondary = startSecondaryNameNode(conf);
            secondary.doCheckpoint();
            FSDirectoryInterface secondaryFsDir = secondary.getFSNamesystem().dir;
            INodeInterface rootInMap = secondaryFsDir.getInode(secondaryFsDir.rootDir.getId());
    ...
    public void testSaveNamespace() throws IOException {
        ...
        try {
            Configuration conf = new HdfsConfiguration();
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDatanodes).format(true).build();
            cluster.waitActive();
            fs = (cluster.getFileSystem());
    ...
    public void testCheckpointSignature() throws IOException {
        ...
        try {
            ...
            secondary.doCheckpoint();
            CheckpointSignatureInterface sig = nnRpc.rollEditLog();
    ...
    public void testMultipleSecondaryNamenodes() throws IOException {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleFederatedTopology(conf.get(DFSConfigKeys.DFS_NAMESERVICES))).build();
    ...
    public void testSecondaryImageDownload() throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDatanodes).format(true).build();
        cluster.waitActive();
        ...
        try {
            ...
            //
            secondary = startSecondaryNameNode(conf);
            File secondaryDir = MiniDockerDFSCluster.getCheckpointDirectory(MiniDockerDFSCluster.getBaseDirectory(), 0, 0)[0];
            ...
            for (StorageDirectoryInterface sd : image.getStorage().dirIterable(NameNodeDirType.IMAGE)) {
    ...
    public void testEditFailureBeforeRename() throws IOException {
        ...
        try {
            ...
            for (StorageDirectoryInterface sd : secondary.getFSImage().getStorage().dirIterable(NameNodeDirType.EDITS)) {
    ...
    public void testEditFailureOnFirstCheckpoint() throws IOException {
        ...
        try {
            ...
            FSNamesystemInterface fsns = cluster.getNamesystem();
    ...
    public void testMultipleSecondaryNNsAgainstSameNN() throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(true).build();
            // Start 2NNs
    ...
    public void testReformatNNBetweenCheckpoints() throws IOException {
        ...
        try {
            ...
            Configuration snnConf = new Configuration(conf);
            File checkpointDir = new File(MiniDockerDFSCluster.getBaseDirectory(), "namesecondary");
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).nameNodePort(origPort).nameNodeHttpPort(origHttpPort).format(true).build();
            try {
                secondary.doCheckpoint();
    ...
    public void testNamespaceVerifiedOnFileTransfer() throws IOException {
        ...
        try {
            ...
            nn.rollEditLog();
            RemoteEditLogManifestInterface manifest = nn.getEditLogManifest(1);
            RemoteEditLog log = manifest.getLogs().get(0);
            NNStorageInterface dstImage = Mockito.mock(NNStorage.class);
            Mockito.doReturn(Lists.newArrayList(new File("/wont-be-written"))).when(dstImage).getFiles(Mockito.<NameNodeDirType>any(), Mockito.anyString());
    ...
    public void testCheckpointWithFailedStorageDir() throws Exception {
        ...
        try {
            ...
            NamenodeProtocols nn = cluster.getNameNodeRpc();
            NNStorageInterface storage = cluster.getNameNode().getFSImage().getStorage();
            StorageDirectoryInterface sd0 = storage.getStorageDir(0);
            StorageDirectoryInterface sd1 = storage.getStorageDir(1);
    ...
    public void testCheckpointWithSeparateDirsAfterNameFails() throws Exception {
        ...
        File currentDir = null;
        Configuration conf = new HdfsConfiguration();
        File base_dir = new File(MiniDockerDFSCluster.getBaseDirectory());
        conf.setBoolean(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_RESTORE_KEY, true);
        conf.set(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_KEY, MiniDockerDFSCluster.getBaseDirectory() + "/name-only");
        conf.set(DFSConfigKeys.DFS_NAMENODE_EDITS_DIR_KEY, MiniDockerDFSCluster.getBaseDirectory() + "/edits-only");
        conf.set(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_DIR_KEY, fileAsURI(new File(base_dir, "namesecondary1")).toString());
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(true).manageNameDfsDirs(false).build();
            secondary = startSecondaryNameNode(conf);
            // Checkpoint once
            secondary.doCheckpoint();
    ...
    public void testCheckpointTriggerOnTxnCount() throws Exception {
        ...
        try {
            ...
            final NNStorageInterface storage = secondary.getFSImage().getStorage();
    ...
    public void testActiveRejectSmallerTxidDeltaImage() throws Exception {
        ...
        try {
            ...
            NameNodeInterface active = cluster.getNameNode();
    ...
    public void testActiveImageWithTimeDeltaRelaxation() throws Exception {
        ...
        SecondaryNameNode secondary = null;
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(true).build()) {
            // enable small delta rejection
            NameNodeInterface active = cluster.getNameNode();
            active.httpServer.getHttpServer().setAttribute(RECENT_IMAGE_CHECK_ENABLED, true);
            secondary = startSecondaryNameNode(conf);
    ...
    private static void cleanup(MiniDockerDFSCluster cluster) {
        if (cluster != null) {
            try {
                cluster.shutdown();
            } catch (Exception e) {
                LOG.warn("Could not shutdown MiniDockerDFSCluster ", e);
            }
        }
    }
     ...
     */
    private void assertParallelFilesInvariant(MiniDockerDFSCluster cluster, ImmutableList<SecondaryNameNode> secondaries) throws Exception {
