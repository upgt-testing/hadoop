...
public class TestHDFSFileContextMainOperations_RestartInjected_Randomized_123 extends FileContextMainOperationsBaseTest {
    ...
    protected FileContextTestHelper createFileContextHelper() {
        return new FileContextTestHelper("/tmp/TestHDFSFileContextMainOperations_RestartInjected");
    }
    ...
    public void testTruncate() throws Exception {
        ...
        RestartFramework.at("before_test_truncate").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FileStatus fileStatus = fc.getFileStatus(file);
        Assert.assertEquals(fileStatus.getLen(), newLength);
        AppendTestUtil.checkFullFile(fs, file, newLength, data, file.toString());
        RestartFramework.at("after_test_truncate").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testOldRenameWithQuota() throws Exception {
        ...
        RestartFramework.at("before_test_old_rename_with_quota").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
        ...
        RestartFramework.at("after_test_old_rename_with_quota").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testRenameWithQuota() throws Exception {
        ...
        fc.mkdir(dst1.getParent(), FileContext.DEFAULT_PERM, true);
        fs.setQuota(dst1.getParent(), 2, HdfsConstants.QUOTA_DONT_SET);
        RestartFramework.at("after_test_rename_with_quota").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /* 
     * Test1: src does not exceed quota and dst has no quota check and hence 
     * accommodates rename
     */
     ...
     */
        ...
        createFile(src1);
        fs.setQuota(src1.getParent(), 1, HdfsConstants.QUOTA_DONT_SET);
        RestartFramework.at("before_test_rename_with_quota").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        rename(dst1, src1, false, true, Rename.OVERWRITE);
    }

    @Test(expected = RemoteException.class)
    public void testRenameRoot() throws Exception {
        RestartFramework.at("after_test_rename_root").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_test_rename_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRenameToRoot() throws Exception {
        RestartFramework.at("before_test_rename_to_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path src = getTestRootPath(fc, "test/testRenameRoot/srcdir/src1");
        Path dst = new Path("/");
        createFile(src);
        rename(src, dst, true, true, Rename.OVERWRITE);
        RestartFramework.at("after_test_rename_to_root").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testEditsLogOldRename() throws Exception {
        ...
        createFile(dst1);
        RestartFramework.at("before_test_edits_log_old_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Set quota so that dst1 parent cannot allow under it new files/directories
        fs.setQuota(dst1.getParent(), 2, HdfsConstants.QUOTA_DONT_SET);
        // Free up quota for a subsequent rename
        fs.delete(dst1, true);
        RestartFramework.at("after_test_edits_log_old_rename").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        oldRename(src1, dst1, true, false);
    ...
    public void testEditsLogRename() throws Exception {
        ...
        RestartFramework.at("after_test_edits_log_rename").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Restart the cluster and ensure the above operations can be
        // loaded from the edits log
        restartCluster();
        fs = cluster.getFileSystem();
        src1 = getTestRootPath(fc, "testEditsLogRename/srcdir/src1");
        RestartFramework.at("before_test_edits_log_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testIsValidNameInvalidNames() {
        RestartFramework.at("after_test_is_valid_name_invalid_names").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_test_is_valid_name_invalid_names").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCrossFileSystemRename() throws IOException {
        ...
        RestartFramework.at("before_test_cross_file_system_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_test_cross_file_system_rename").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
}
