  ...
  public void testLazyTokenFetchForWebhdfs() throws Exception {
    ...
    try {
      ...
      uri = DFSUtil.createUri(
          "swebhdfs", cluster.getNameNode().getHttpsAddress().getHostName(), cluster.getNameNode().getHttpsAddress().getPort());
      validateLazyTokenFetch(ugi, clusterConf);

      //test with webhdfs
      uri = DFSUtil.createUri(
          "webhdfs", cluster.getNameNode().getHttpAddress().getHostName(), cluster.getNameNode().getHttpAddress().getPort());
      validateLazyTokenFetch(ugi, clusterConf);
