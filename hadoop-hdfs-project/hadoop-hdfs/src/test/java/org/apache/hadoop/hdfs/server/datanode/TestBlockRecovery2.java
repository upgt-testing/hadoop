...
public class TestBlockRecovery2 {
    ...
    private static final String DATA_DIR = MiniDFSClusterInJVM.getBaseDirectory() + "data";
    ...
    public void startUp() throws IOException {
        ...
        Mockito.doAnswer((Answer<DatanodeRegistration>) invocation -> (DatanodeRegistration) invocation.getArguments()[0]).when(namenode).registerDatanode(any(DatanodeRegistration.class));
        when(namenode.versionRequest()).thenReturn(new NamespaceInfo(1, CLUSTER_ID, POOL_ID, 1L));
        when(namenode.sendHeartbeat(any(), any(), anyLong(), anyLong(), Mockito.anyInt(), Mockito.anyInt(), Mockito.anyInt(), any(), Mockito.anyBoolean(), any(), any())).thenReturn(new HeartbeatResponse(new DatanodeCommand[0], new NNHAStatusHeartbeat(HAServiceProtocol.HAServiceState.ACTIVE, 1), null, ThreadLocalRandom.current().nextLong() | 1L));
   ...
   */
    ...
    public void testRecoveryWillIgnoreMinReplication() throws Exception {
        ...
        try {
            cluster = new MiniDFSClusterInJVM.Builder(configuration).numDataNodes(5).build();
            cluster.waitActive();
    ...
    @Test(timeout = 300000L)
    public void testRecoveryWillIgnoreMinReplication_withUpgrade20() throws Exception {
        // Stop the Mocked DN started in startup()
        tearDown();
        final int blockSize = 4096;
        final int numReplicas = 3;
    ...
    @Test(timeout = 300000L)
    public void testRecoveryWillIgnoreMinReplication_withUpgrade40() throws Exception {
        // Stop the Mocked DN started in startup()
        tearDown();
        final int blockSize = 4096;
        final int numReplicas = 3;
    ...
    @Test(timeout = 300000L)
    public void testRecoveryWillIgnoreMinReplication_withUpgrade60() throws Exception {
        // Stop the Mocked DN started in startup()
        tearDown();
        final int blockSize = 4096;
        final int numReplicas = 3;
    ...
    @Test(timeout = 300000L)
    public void testRecoveryWillIgnoreMinReplication_withUpgrade80() throws Exception {
        // Stop the Mocked DN started in startup()
        tearDown();
        final int blockSize = 4096;
        final int numReplicas = 3;
