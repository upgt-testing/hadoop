 ...
 */
public class TestMultiObserverNode_RestartInjected_Randomized_123 {
    ...
    public void testObserverFailover() throws Exception {
        ...
        RestartFramework.at("after_all_transitions").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_transition_standby_nn2").on(dfsCluster).restart("namenode").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
        ...
        dfs.getFileStatus(testPath);
        RestartFramework.at("after_first_getfilestatus").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(2);
    ...
    public void testMultiObserver() throws Exception {
        ...
        dfsCluster.rollEditLogAndTail(0);
        RestartFramework.at("after_first_roll_editlog").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        // Now shutdown the second observer, request should go to the first one
        dfsCluster.shutdownNameNode(3);
        RestartFramework.at("before_manual_shutdown_nn2").on(dfsCluster).restart("namenode").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_both_observers_restored").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        dfsCluster.restartNameNode(2);
        RestartFramework.at("after_manual_restart_nn2").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.transitionToObserver(2);
    ...
    public void testObserverFallBehind() throws Exception {
        ...
        RestartFramework.at("after_verification").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_state_id_set").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        dfs.getFileStatus(testPath);
