...
public class TestDatanodeDeath {
    ...
    class Modify extends Thread {
        ...
        Modify(Configuration conf, MiniDockerDFSCluster cluster) {
            running = true;
    ...
    private void simpleTest(int datanodeToKill) throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(myMaxNodes).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        short repl = 3;
