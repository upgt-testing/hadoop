...
public class TestFSEditLogLoader_RestartInjected_Randomized_456 {
    ...
    public TestFSEditLogLoader_RestartInjected_Randomized_456(Boolean async) {
    ...
    public void testDisplayRecentEditLogOpCodes() throws IOException {
        ...
        RestartFramework.at("after_mkdir_operations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReplicationAdjusted() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_replication_adjustment").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    public void testHasNonEcBlockUsingStripedIDForUpdateBlocks() throws IOException {
        ...
        try {
            ...
            fns.getEditLog().logUpdateBlocks(testFilePath, file, true);
            TestINodeFile.toCompleteFile(file);
            RestartFramework.at("after_block_update").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNodes();
            cluster.waitActive();
            fns = cluster.getNamesystem();
    ...
    public void testErasureCodingPolicyOperations() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("before_third_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNodes();
            cluster.waitActive();
            ecPolicy = ErasureCodingPolicyManager.getInstance().getByID(newPolicy.getId());
            assertEquals(ErasureCodingPolicyState.DISABLED, DFSTestUtil.getECPolicyState(ecPolicy));
            ...
            RestartFramework.at("before_fourth_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNodes();
            cluster.waitActive();
            ecPolicy = ErasureCodingPolicyManager.getInstance().getByID(newPolicy.getId());
