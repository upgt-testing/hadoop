 ...
 */
public class MiniDFSClusterManager_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(MiniDFSClusterManager_RestartInjected.class);
    ...
    private Options makeOptions() {
        ...
        options.addOption("datanodes", true, "How many datanodes to start (default 1)").addOption("format", false, "Format the DFS (default false)").addOption("cmdport", true, "Which port to listen on for commands (default 0--we choose)").addOption("nnport", true, "NameNode port (default 0--we choose)").addOption("httpport", true, "NameNode http port (default 0--we choose)").addOption("namenode", true, "URL of the namenode (default " + "is either the DFS cluster or a temporary dir)").addOption(OptionBuilder.hasArgs().withArgName("property=value").withDescription("Options to pass into configuration object").create("D")).addOption(OptionBuilder.hasArg().withArgName("path").withDescription("Save configuration to this XML file.").create("writeConfig")).addOption(OptionBuilder.hasArg().withArgName("path").withDescription("Write basic information to this JSON file.").create("writeDetails")).addOption(OptionBuilder.withDescription("Prints option help.").create("help"));
        return options;
    }
    ...
    public void start() throws IOException, FileNotFoundException {
        ...
        dfs.waitActive();
        RestartFramework.at("after_cluster_start").on(dfs).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void updateConfiguration(Configuration conf2, String[] keyvalues) {
        ...
        LOG.info("Updated {} configuration settings from command line.", num_confs_updated);
    }
    ...
    private int intArgument(CommandLine cli, String argName, int defaultValue) {
        ...
        try {
            ...
            LOG.error("Couldn't parse value ({}) for option {}. " + "Using default: {}", o, argName, defaultValue);
        }
        return defaultValue;
    }
    ...
    public static void main(String[] args) throws IOException {
        new MiniDFSClusterManager_RestartInjected().run(args);
    }
}
