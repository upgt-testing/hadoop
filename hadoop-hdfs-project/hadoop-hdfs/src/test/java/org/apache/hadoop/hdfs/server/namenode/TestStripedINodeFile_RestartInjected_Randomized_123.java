 ...
 */
public class TestStripedINodeFile_RestartInjected_Randomized_123 {
    ...
    public void testStripedLayoutRedundancy() {
        ...
        try {
            new INodeFile(HdfsConstants.GRANDFATHER_INODE_ID, null, perm, 0L, 0L, null, null, /*replication*/
            ecPolicyID, 1024L, HdfsConstants.WARM_STORAGE_POLICY_ID, CONTIGUOUS);
            fail("INodeFile construction should fail when replication param is " + "provided for striped layout!");
        } catch (IllegalArgumentException iae) {
            LOG.info("Expected exception: ", iae);
        }
        inodeFile = new INodeFile(HdfsConstants.GRANDFATHER_INODE_ID, null, perm, 0L, 0L, null, null, /*replication*/
        ecPolicyID, 1024L, HdfsConstants.WARM_STORAGE_POLICY_ID, STRIPED);
    ...
    public void testDeleteOp() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_ec_policy_enabled").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
            dfs.mkdirs(ecDir);
            // set erasure coding policy
            dfs.setErasureCodingPolicy(ecDir, StripedFileTestUtil.getDefaultECPolicy().getName());
            RestartFramework.at("after_ec_policy_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
            DFSTestUtil.createFile(dfs, ecFile, len, (short) 1, 0xFEED);
            DFSTestUtil.createFile(dfs, contiguousFile, len, (short) 1, 0xFEED);
            RestartFramework.at("after_files_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
            ...
            RestartFramework.at("after_ec_dir_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
    ...
    public void testUnsuitableStoragePoliciesWithECStripedMode() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_dir_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            client.setStoragePolicy(fooDir, HdfsConstants.ONESSD_STORAGE_POLICY_NAME);
            RestartFramework.at("after_storage_policy_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_file_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
