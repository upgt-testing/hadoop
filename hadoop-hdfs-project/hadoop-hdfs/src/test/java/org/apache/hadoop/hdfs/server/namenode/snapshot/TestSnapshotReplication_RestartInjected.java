...
public class TestSnapshotReplication_RestartInjected {
  ...
  public void testReplicationWithoutSnapshot() throws Exception {
    ...
    RestartFramework.at("after_check_replication")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_change_replication")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testReplicationWithSnapshot() throws Exception {
    ...
    for (; fileRep < NUMDATANODE; ) {
      ...
      Path snapshotRoot = SnapshotTestHelper.createSnapshot(hdfs, sub1, "s"
          + fileRep);
      RestartFramework.at("after_create_snapshot_" + fileRep)
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_set_replication_" + fileRep)
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_final_replication_change")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
