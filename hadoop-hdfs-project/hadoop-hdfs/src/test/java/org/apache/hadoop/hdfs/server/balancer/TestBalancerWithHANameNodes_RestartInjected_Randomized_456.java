 ...
 */
public class TestBalancerWithHANameNodes_RestartInjected_Randomized_456 {
    ...
    public static void waitStoragesNoStale(MiniDFSCluster cluster, ClientProtocol client, int nnIndex) throws Exception {
        ...
        GenericTestUtils.waitFor(() -> {
            ...
            for (DatanodeInfo dn : dataNodes) {
                DatanodeStorageInfo[] storageInfos = bm.getDatanodeManager().getDatanode(dn.getDatanodeUuid()).getStorageInfos();
    ...
    public void testBalancerWithHANameNodes() throws Exception {
        ...
        cluster = new MiniDFSCluster.Builder(copiedConf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(TEST_CAPACITIES.length).racks(TEST_RACKS).simulatedCapacities(TEST_CAPACITIES).build();
        ...
        try {
            ...
            RestartFramework.at("before_do_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            doTest(conf, true);
        } finally {
            cluster.shutdown();
        }
    ...
    void doTest(Configuration conf, boolean withHA) throws Exception {
        ...
        if (withHA) {
            ...
            RestartFramework.at("after_storages_no_stale").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        ...
        RestartFramework.at("after_add_datanode").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TestBalancer.waitForHeartBeat(totalUsedSpace, totalCapacity, client, cluster);
        Collection<URI> namenodes = DFSUtil.getInternalNsRpcUris(conf);
        Collection<String> nsIds = DFSUtilClient.getNameServiceIds(conf);
        assertEquals(1, namenodes.size());
        RestartFramework.at("before_balancer_run").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_balancer_complete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testBalancerRequestSBNWithHA() throws Exception {
        ...
        try {
            ...
            Thread.sleep(500);
            client = NameNodeProxies.createProxy(conf, FileSystem.getDefaultUri(conf), ClientProtocol.class).getProxy();
            RestartFramework.at("before_do_test").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            doTest(conf);
            RestartFramework.at("after_balancer_complete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testBalancerWithObserver(boolean withObserverFailure) throws Exception {
        ...
        try {
            qjmhaCluster = HATestUtil.setUpObserverCluster(conf, 2, TEST_CAPACITIES.length, true, TEST_CAPACITIES, TEST_RACKS);
            ...
            for (int i = 0; i < cluster.getNumNameNodes(); i++) {
                namesystemSpies.add(NameNodeAdapter.spyOnNamesystem(cluster.getNameNode(i)));
            }
            if (withObserverFailure) {
                cluster.shutdownNameNode(2);
                RestartFramework.at("after_observer_shutdown").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            ...
            RestartFramework.at("before_do_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            doTest(conf);
            RestartFramework.at("after_balancer_complete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int i = 0; i < cluster.getNumNameNodes(); i++) {
                int expectedObserverIdx = withObserverFailure ? 3 : 2;
                int expectedCount = (i == expectedObserverIdx) ? 2 : 0;
                verify(namesystemSpies.get(i), times(expectedCount)).getBlocks(any(), anyLong(), anyLong());
            }
        } finally {
