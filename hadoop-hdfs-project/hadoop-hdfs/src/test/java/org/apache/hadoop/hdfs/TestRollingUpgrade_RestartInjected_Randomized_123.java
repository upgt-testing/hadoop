 ...
 */
public class TestRollingUpgrade_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestRollingUpgrade_RestartInjected.class);
    ...
    public Configuration getHdfsConfiguration() throws IOException {
        ...
        conf.set(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_KEY, folder.newFolder("dfs/name").toString());
        conf.set(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_DIR_KEY, folder.newFolder("dfs/namesecondary").toString());
        conf.set(DFSConfigKeys.DFS_DATANODE_DATA_DIR_KEY, folder.newFolder("dfs/data").toString());
        return conf;
    }
    ...
    public void testDFSAdminRollingUpgradeCommands() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(0).build()) {
            ...
            final Path baz = new Path("/baz");
            {
                ...
                dfs.mkdirs(foo);
                RestartFramework.at("after_foo_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_upgrade_prepare").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                //query rolling upgrade
                runCmd(dfsadmin, true, "-rollingUpgrade", "query");
                checkMxBean();
                dfs.mkdirs(bar);
                RestartFramework.at("after_bar_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                checkMxBeanIsNull();
                RestartFramework.at("after_upgrade_finalize").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                dfs.mkdirs(baz);
                RestartFramework.at("after_baz_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                runCmd(dfsadmin, true, "-rollingUpgrade");
    ...
    public void testRollingUpgradeWithQJM() throws Exception {
        ...
        try (MiniJournalCluster mjc = new MiniJournalCluster.Builder(conf).build()) {
            ...
            setConf(conf, nn1Dir, mjc);
            {
                ...
                RestartFramework.at("after_first_cluster_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            try {
                ...
                RestartFramework.at("after_second_cluster_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                final Path foo = new Path("/foo");
                final Path bar = new Path("/bar");
                final Path baz = new Path("/baz");
                ...
                RestartFramework.at("after_cluster2_creation").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                dfs2.mkdirs(baz);
                RestartFramework.at("after_baz_creation").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                LOG.info("RESTART cluster 2");
                cluster2.restartNameNode();
                RestartFramework.at("after_first_namenode_restart").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                assertEquals(info1, dfs2.rollingUpgrade(RollingUpgradeAction.QUERY));
                Assert.assertTrue(dfs2.exists(foo));
                Assert.assertTrue(dfs2.exists(bar));
                ...
                cluster2.restartNameNode();
                RestartFramework.at("after_second_namenode_restart").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                assertEquals(info1, dfs2.rollingUpgrade(RollingUpgradeAction.QUERY));
                Assert.assertTrue(dfs2.exists(foo));
                Assert.assertTrue(dfs2.exists(bar));
                Assert.assertTrue(dfs2.exists(baz));
                ...
                RestartFramework.at("after_upgrade_finalize").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                LOG.info("RESTART cluster 2 with regular startup option");
                cluster2.getNameNodeInfos()[0].setStartOpt(StartupOption.REGULAR);
                cluster2.restartNameNode();
                RestartFramework.at("after_final_namenode_restart").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                Assert.assertTrue(dfs2.exists(foo));
                Assert.assertTrue(dfs2.exists(bar));
                Assert.assertTrue(dfs2.exists(baz));
            } finally {
    ...
    public void testRollback() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).build()) {
            cluster.waitActive();
            RestartFramework.at("after_cluster_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final Path foo = new Path("/foo");
            final Path bar = new Path("/bar");
            ...
            cluster.getFileSystem().rollEdits();
            RestartFramework.at("after_first_upgrade_rollback_cycle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rollbackRollingUpgrade(foo, bar, file, data, cluster);
            checkMxBeanIsNull();
            startRollingUpgrade(foo, bar, file, data, cluster);
            cluster.getFileSystem().rollEdits();
            cluster.getFileSystem().rollEdits();
            RestartFramework.at("after_second_upgrade_rollback_cycle").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rollbackRollingUpgrade(foo, bar, file, data, cluster);
            startRollingUpgrade(foo, bar, file, data, cluster);
            cluster.restartNameNode();
            RestartFramework.at("after_third_upgrade_with_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rollbackRollingUpgrade(foo, bar, file, data, cluster);
            startRollingUpgrade(foo, bar, file, data, cluster);
            cluster.restartNameNode();
            RestartFramework.at("after_fourth_upgrade_with_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rollbackRollingUpgrade(foo, bar, file, data, cluster);
            startRollingUpgrade(foo, bar, file, data, cluster);
            RestartFramework.at("after_fifth_upgrade_rollback_cycle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rollbackRollingUpgrade(foo, bar, file, data, cluster);
            startRollingUpgrade(foo, bar, file, data, cluster);
            RestartFramework.at("after_sixth_upgrade_rollback_cycle").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rollbackRollingUpgrade(foo, bar, file, data, cluster);
        }
    }
    ...
    public void testDFSAdminDatanodeUpgradeControlCommands() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).build()) {
            ...
            RestartFramework.at("after_datanode_info_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testFinalize(int nnCount, boolean skipImageDeltaCheck) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_cluster_creation").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            dfsCluster.getNameNode(0).getHttpServer().setAttribute(RECENT_IMAGE_CHECK_ENABLED, skipImageDeltaCheck);
            DistributedFileSystem dfs = dfsCluster.getFileSystem(0);
            dfs.mkdirs(foo);
            RestartFramework.at("after_foo_creation").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_upgrade_prepare").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dfs.mkdirs(bar);
            RestartFramework.at("after_bar_creation").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Assert.assertTrue(dfs.exists(foo));
            RestartFramework.at("after_upgrade_finalize").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_final_namenode_restart").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    private void testQuery(int nnCount) throws Exception {
        ...
        try (MiniQJMHACluster cluster = new MiniQJMHACluster.Builder(conf).setNumNameNodes(nnCount).build()) {
            ...
            RestartFramework.at("after_other_namenodes_shutdown").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // start rolling upgrade
            RollingUpgradeInfo info = dfs.rollingUpgrade(RollingUpgradeAction.PREPARE);
            Assert.assertTrue(info.isStarted());
            RestartFramework.at("after_upgrade_prepare").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_other_namenodes_restart").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRollBackImage() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_directory_creation").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.getDfsCluster().getNameNodeRpc(0).rollEdits();
            RestartFramework.at("after_edit_roll").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckpointWithSNN() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_snn_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dfs = cluster.getFileSystem();
            dfs.mkdirs(new Path("/test/foo"));
            RestartFramework.at("after_test_directory_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            snn.doCheckpoint();
            RestartFramework.at("after_first_checkpoint").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //start rolling upgrade
            dfs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
            dfs.rollingUpgrade(RollingUpgradeAction.PREPARE);
            dfs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
            ...
            RestartFramework.at("after_bar_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // do checkpoint in SNN again
            snn.doCheckpoint();
            RestartFramework.at("after_second_checkpoint").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
