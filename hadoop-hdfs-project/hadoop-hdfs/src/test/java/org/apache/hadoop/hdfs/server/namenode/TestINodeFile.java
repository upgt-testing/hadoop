...
public class TestINodeFile {
    ...
    public void testStoragePolicyID() {
        for (byte i = 0; i < 16; i++) {
            final INodeFileInterface f = createINodeFile(i);
    ...
    public void testReplication() {
        ...
        INodeFileInterface inf = createINodeFile(replication, preferredBlockSize);
    ...
    public void testGetBlockType() {
        ...
        INodeFileInterface striped = createStripedINodeFile(preferredBlockSize);
    ...
    public void testGetFullPathNameAfterSetQuota() throws Exception {
        ...
        try {
            ...
            FSDirectoryInterface fsdir = fsn.getFSDirectory();
    ...
    public void testValueOf() throws IOException {
        ...
        {
            //cast from null
            final INodeInterface from = null;
        ...
        {
            //cast from INodeFile
            final INodeInterface from = createINodeFile(replication, preferredBlockSize);
            //cast to INodeFile, should success
            final INodeFileInterface f = INodeFile.valueOf(from, path);
            assertTrue(f == from);
            //cast to INodeDirectory, should fail
            try {
                INodeDirectory.valueOf(from, path);
        ...
        }
        {
            ...
            final INodeDirectoryInterface d = INodeDirectory.valueOf(from, path);
            assertTrue(d == from);
        }
    }
    ...
    private void ensureReservedFileNamesCannotBeLoaded(MiniDockerDFSCluster cluster) throws IOException {
    ...
    private void ensureClusterRestartFails(MiniDockerDFSCluster cluster) {
        try {
            cluster.restartNameNode();
    ...
    private void ensureClusterRestartSucceeds(MiniDockerDFSCluster cluster) throws IOException {
        cluster.restartNameNode();
        cluster.waitActive();
    ...
    public void testGetPathFromInode() throws QuotaExceededException {
        String path = "/a/b/c";
        INodeInterface inode = createTreeOfInodes(path);
    ...
    public void testInodePath() throws IOException {
        ...
        FSDirectoryInterface fsd = Mockito.mock(FSDirectory.class);
    ...
    public void testInodeReplacement() throws Exception {
        ...
        try {
            ...
            final FSDirectoryInterface fsdir = cluster.getNamesystem().getFSDirectory();
            final Path dir = new Path("/dir");
            hdfs.mkdirs(dir);
            INodeDirectoryInterface dirNode = getDir(fsdir, dir);
            INodeInterface dirNodeFromNode = fsdir.getInode(dirNode.getId());
    ...
    public void testLocationLimitInListingOps() throws Exception {
        ...
        try {
            ...
            for (int j = 0; j < numEntries; j++) {
                DirectoryListingInterface dl = cluster.getNameNodeRpc().getListing("/tmp1", start, true);
            ...
            for (int j = 0; j < numEntries / 3; j++) {
                DirectoryListingInterface dl = cluster.getNameNodeRpc().getListing("/tmp2", start, true);
    ...
    public void testFilesInGetListingOps() throws Exception {
        ...
        try {
            ...
            DirectoryListingInterface dl = cluster.getNameNodeRpc().getListing("/tmp", HdfsFileStatus.EMPTY_NAME, false);
            assertTrue(dl.getPartialListing().length == 3);
            String f2 = new String("f2");
            dl = cluster.getNameNodeRpc().getListing("/tmp", f2.getBytes(), false);
            assertTrue(dl.getPartialListing().length == 1);
            INodeInterface f2INode = fsdir.getINode("/tmp/f2");
    ...
    public void testFileUnderConstruction() {
        ...
        FileUnderConstructionFeatureInterface uc = file.getFileUnderConstructionFeature();
    ...
    public void testXAttrFeature() {
        ...
        XAttrInterface xAttr = new XAttr.Builder().setNameSpace(XAttr.NameSpace.USER).setName("a1").setValue(new byte[] { 0x31, 0x32, 0x33 }).build();
        builder.add(xAttr);
        XAttrFeature f = new XAttrFeature(builder.build());
        inf.addXAttrFeature(f);
        XAttrFeatureInterface f1 = inf.getXAttrFeature();
    ...
    public void testClearBlocks() {
        INodeFileInterface toBeCleared = createINodeFiles(1, "toBeCleared")[0];
    ...
    public void testConcat() throws IOException {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).build()) {
            ...
            ContentSummaryInterface cs = dfs.getContentSummary(new Path(dir));
            QuotaUsageInterface qu = dfs.getQuotaUsage(new Path(dir));
