...
public class TestBlockReportRateLimiting {
    ...
    public void testRateLimitingDuringDataNodeStartup() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).build();
        cluster.waitActive();
    ...
    public void testLeaseExpiration() throws Exception {
        ...
        final AtomicReference<MiniDockerDFSCluster> cluster = new AtomicReference<>();
        ...
        try {
            BlockManagerFaultInjector.instance = injector;
            cluster.set(new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).build());
