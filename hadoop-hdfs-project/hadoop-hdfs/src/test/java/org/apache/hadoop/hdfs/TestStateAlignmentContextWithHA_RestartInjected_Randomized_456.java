 ...
 */
public class TestStateAlignmentContextWithHA_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(TestStateAlignmentContextWithHA_RestartInjected.class.getName());
    ...
    public void before() throws IOException, URISyntaxException {
        dfs = HATestUtil.configureObserverReadFs(cluster, CONF, ORPPwithAlignmentContexts.class, true);
    }
    ...
    public void testStateTransferOnWrite() throws Exception {
        long preWriteState = cluster.getNamesystem(active).getLastWrittenTransactionId();
        DFSTestUtil.writeFile(dfs, new Path("/testFile1"), "abc");
        long clientState = getContext(0).getLastSeenStateId();
        long postWriteState = cluster.getNamesystem(active).getLastWrittenTransactionId();
        // Write(s) should have increased state. Check for greater than.
        assertTrue(clientState > preWriteState);
        RestartFramework.at("after_state_check").on(cluster).restart("namenode").withIndex(active).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStateTransferOnRead() throws Exception {
        RestartFramework.at("after_write_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_read_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.writeFile(dfs, new Path("/testFile2"), "123");
        long lastWrittenId = cluster.getNamesystem(active).getLastWrittenTransactionId();
    ...
    public void testStateTransferOnWriteWithFailover() throws Exception {
        ...
        // Client and server state should be equal.
        assertEquals(clientState, postWriteState);
        RestartFramework.at("after_failover").on(cluster).restart("namenode").withIndex(active).withMode(RestartMode.GRACEFUL).execute();
        // Failover NameNode.
        failOver();
        ...
        long writeStateFO = cluster.getNamesystem(active).getLastWrittenTransactionId();
        // Write(s) should have increased state. Check for greater than.
    ...
    public void testMultiClientStatesWithRandomFailovers() throws Exception {
        RestartFramework.at("after_second_run").on(cluster).restart("namenode").withIndex(active).withMode(RestartMode.GRACEFUL).execute();
        // First run, half the load, with one failover.
        runClientsWithFailover(1, NUMCLIENTS / 2, NUMFILES / 2);
        RestartFramework.at("after_first_run").on(cluster).restart("namenode").withIndex(active).withMode(RestartMode.GRACEFUL).execute();
            ...
            try {
                for (i = 0; i < filesToMake; i++) {
                    ...
                    if (postClientStateFO < 0 || postClientStateFO <= preClientStateFO) {
                        LOG.error("FAIL: Worker started with: {} , but finished with: {}", preClientStateFO, postClientStateFO);
                        return STATE.FAIL;
                    }
                    if (i % (NUMFILES / 10) == 0) {
