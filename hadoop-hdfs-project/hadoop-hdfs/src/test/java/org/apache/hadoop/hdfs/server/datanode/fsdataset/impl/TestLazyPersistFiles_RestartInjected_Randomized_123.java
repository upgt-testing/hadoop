...
public class TestLazyPersistFiles_RestartInjected_Randomized_123 extends LazyPersistTestCase {
    ...
    public void testConcurrentWrites() throws IOException, InterruptedException, TimeoutException {
        ...
        RestartFramework.at("after_concurrent_writes").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        triggerBlockReport();
    ...
    class WriterRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                bFail.set(true);
                LOG.error("Writer exception: writer id:{} testfile: {}", id, paths[i].toString(), e);
            } finally {
                latch.countDown();
            }
        }
    ...
    public void testReleaseVolumeRefIfExceptionThrown() throws IOException, InterruptedException {
        ...
        for (int i = 0; i < volumes.size(); ++i) {
            ...
            Assert.assertTrue(beforeCnts[i] == afterCnt || beforeCnts[i] == (afterCnt - 1));
        }
    }
}
