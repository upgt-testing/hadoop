...
public class TestLocatedBlocksRefresher {
    ...
    private void setupTest(long refreshInterval) throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).racks(RACKS).build();
        cluster.waitActive();
    }
        ...
        synchronized (fin.infoLock) {
            ...
            for (int i = 0; i < REPLICATION_FACTOR; i++) {
                ...
                if (dataNodeProperties != null) {
                    List<DataNodeInterface> datanodesPostStoppage = cluster.getDataNodes();
