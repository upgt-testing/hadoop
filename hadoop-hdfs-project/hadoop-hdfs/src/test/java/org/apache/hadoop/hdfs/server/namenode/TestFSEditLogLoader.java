...
public class TestFSEditLogLoader {
    ...
    public void testDisplayRecentEditLogOpCodes() throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).enableManagedDfsDirsRedundancy(false).build();
        cluster.waitActive();
        ...
        StorageDirectoryInterface sd = fsimage.getStorage().dirIterator(NameNodeDirType.EDITS).next();
        cluster.shutdown();
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).enableManagedDfsDirsRedundancy(false).format(false).build();
    ...
    static private File prepareUnfinalizedTestEditLog(File testDir, int numTx, SortedMap<Long, Long> offsetToTxId) throws IOException {
        ...
        FSEditLogInterface fsel = null, spyLog = null;
        try {
    ...
    public void testValidateEditLogWithCorruptBody() throws IOException {
        ...
        for (Map.EntryInterface<Long, Long> entry : offsetToTxId.entrySet()) {
    ...
    public void testAddNewStripedBlock() throws IOException {
        ...
        try {
            ...
            INodeFileInterface inodeLoaded = (INodeFile) fns.getFSDirectory().getINode(testFilePath);
    ...
    private EditLogInputStream getFakeEditLogInputStream(long startTx, long endTx) throws IOException {
        EditLogInputStreamInterface fakeStream = mock(EditLogInputStream.class);
