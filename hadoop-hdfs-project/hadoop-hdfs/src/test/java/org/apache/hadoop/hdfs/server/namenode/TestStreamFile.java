...
public class TestStreamFile {
    ...
    private final HttpServletResponse mockHttpServletResponse = Mockito.mock(HttpServletResponse.class);
    ...
    private void setUpForDoGetTest(MiniDockerDFSCluster cluster, Path testFile) {
        Mockito.doReturn(CONF).when(mockServletContext).getAttribute(JspHelper.CURRENT_CONF);
        Mockito.doReturn(NetUtils.getHostPortString(DFSUtilClient.getNNAddress(CONF))).when(mockHttpServletRequest).getParameter("nnaddr");
        Mockito.doReturn(testFile.toString()).when(mockHttpServletRequest).getPathInfo();
        Mockito.doReturn("/streamFile" + testFile.toString()).when(mockHttpServletRequest).getRequestURI();
    }
