 ...
 */
public class TestFsDatasetCacheRevocation_RestartInjected_Randomized_123 {
    ...
    public void testPinning() throws Exception {
        ...
        cluster.waitActive();
        RestartFramework.at("after_uncache_directive").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cache_directive_added").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Create and cache a file.
        final String testFile = "/test_file";
        DFSTestUtil.createFile(dfs, new Path(testFile), BLOCK_SIZE, (short) 1, 0xcafe);
        RestartFramework.at("after_file_mmap").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfs.addCachePool(new CachePoolInfo("pool"));
        long cacheDirectiveId = dfs.addCacheDirective(new CacheDirectiveInfo.Builder().setPool("pool").setPath(new Path(testFile)).setReplication((short) 1).build());
        ...
        RestartFramework.at("after_buffer_release").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
