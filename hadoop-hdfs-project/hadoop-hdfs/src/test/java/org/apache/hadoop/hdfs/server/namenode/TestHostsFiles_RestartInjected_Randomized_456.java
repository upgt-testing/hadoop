...
public class TestHostsFiles_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestHostsFiles_RestartInjected.class.getName());

    private Class hostFileMgrClass;

    public TestHostsFiles_RestartInjected_Randomized_456(Class hostFileMgrClass) {
   ...
   */
    ...
    public void testHostsExcludeInUI() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_first_replication_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_decommission_refresh").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            DFSTestUtil.waitForReplication(cluster, b, 2, REPLICATION_FACTOR, 0);
            RestartFramework.at("after_second_replication_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testHostsIncludeForDeadCount() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_dead_nodes_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
