 ...
 */
public class TestFSImageWithSnapshot_RestartInjected_Randomized_456 {
    ...
    public void testSaveLoadImage() throws Exception {
        ...
        hdfs.mkdirs(dir);
        RestartFramework.at("after_snapshot_s4").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_mkdir_sub11").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_modifications").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_snapshot_s5").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_sub2_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSaveLoadImageWithAppending() throws Exception {
        ...
        Path sub1file1 = new Path(sub1, "sub1file1");
        Path sub1file2 = new Path(sub1, "sub1file2");
        RestartFramework.at("after_third_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(hdfs, sub1file1, BLOCKSIZE, (short) 1, seed);
        RestartFramework.at("after_snapshot_s1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(hdfs, sub1file2, BLOCKSIZE, (short) 1, seed);
        ...
        RestartFramework.at("after_second_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.hsync(EnumSet.of(SyncFlag.UPDATE_LENGTH));
        RestartFramework.at("after_hsync_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        loadFSImageFromTempFile(imageFile);
        RestartFramework.at("after_save_fsimage").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // dump the fsdir tree again
    ...
    public void testLoadImageWithAppending() throws Exception {
        ...
        // save namespace and restart cluster
        hdfs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
        RestartFramework.at("after_hsync_with_open_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.saveNamespace();
        RestartFramework.at("after_create_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
        RestartFramework.at("after_snapshot_s0").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testLoadImageWithEmptyFile() throws Exception {
        ...
        // save namespace
        hdfs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
        RestartFramework.at("after_create_empty_file").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_save_namespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.saveNamespace();
        RestartFramework.at("before_close_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
        // append to the empty file
        out = hdfs.append(file);
        out.write(1);
        out.close();
        RestartFramework.at("after_append_close").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // restart cluster
        cluster.shutdown();
        cluster = new MiniDFSCluster.Builder(conf).format(false).numDataNodes(NUM_DATANODES).build();
        cluster.waitActive();
    ...
    public void testSaveLoadImageAfterSnapshotDeletion() throws Exception {
        ...
        RestartFramework.at("after_delete_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        hdfs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
        hdfs.saveNamespace();
        RestartFramework.at("after_create_newfile").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_delete_subsubdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
        cluster.shutdown();
        RestartFramework.at("after_snapshot_s1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster = new MiniDFSCluster.Builder(conf).format(false).numDataNodes(NUM_DATANODES).build();
    ...
    public void testDoubleRename() throws Exception {
        ...
        RestartFramework.at("after_delete_snapshot_s2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        createFile(sub1foo, "file1");
        createFile(sub1foo, "file2");
        RestartFramework.at("after_snapshots_s1_s2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_file0").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_snapshot_s0").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        printTree("created snapshots: s1, s2");
        appendFile(sub2foo, "file1");
        RestartFramework.at("after_delete_sub3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_delete_snapshot_s1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        createFile(sub2foo, "file3");
        final Path sub3 = new Path(parent, "sub3");
        hdfs.mkdirs(sub3);
        RestartFramework.at("after_append_and_create").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFSImageWithDoubleRename() throws Exception {
        ...
        RestartFramework.at("after_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.mkdirs(dira);
        hdfs.mkdirs(dirb);
        RestartFramework.at("after_delete_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.mkdirs(dirx);
        ...
        RestartFramework.at("after_create_file1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path rennamePath = new Path(dirx, "dirb");
        // mv /dir1/dira/dirb to /dir1/dirx/dirb
        hdfs.rename(dirb, rennamePath);
        RestartFramework.at("after_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_snapshot_s1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.createSnapshot(dir1, "s1");
        RestartFramework.at("after_second_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.appendFile(hdfs, new Path("/dir1/dirx/dirb/file1"), "more data");
