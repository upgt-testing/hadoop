...
public class TestSaslDataTransferExpiredBlockToken extends SaslDataTransferTestCase {
    ...
    @Test
    public void testBlockSeekToWithExpiredToken_withUpgrade20() throws Exception {
        // read using blockSeekTo(). Acquired tokens are cached in in
        try (FileSystem fs = newFileSystem();
            FSDataInputStream in = fs.open(PATH)) {
            waitBlockTokenExpired(in);
    ...
    @Test
    public void testBlockSeekToWithExpiredToken_withUpgrade80() throws Exception {
        // read using blockSeekTo(). Acquired tokens are cached in in
        try (FileSystem fs = newFileSystem();
            FSDataInputStream in = fs.open(PATH)) {
            waitBlockTokenExpired(in);
    ...
    @Test
    public void testFetchBlockByteRangeWithExpiredToken_withUpgrade20() throws Exception {
        // read using fetchBlockByteRange(). Acquired tokens are cached in in
        try (FileSystem fs = newFileSystem();
            FSDataInputStream in = fs.open(PATH)) {
            waitBlockTokenExpired(in);
    ...
    @Test
    public void testFetchBlockByteRangeWithExpiredToken_withUpgrade80() throws Exception {
        // read using fetchBlockByteRange(). Acquired tokens are cached in in
        try (FileSystem fs = newFileSystem();
            FSDataInputStream in = fs.open(PATH)) {
            waitBlockTokenExpired(in);
    ...
    @Test
    public void testHedgedFetchBlockByteRangeWithExpiredToken_withUpgrade20() throws Exception {
        // read using hedgedFetchBlockByteRange(). Acquired tokens are cached in in
        try (FileSystem fs = newFileSystemHedgedRead();
            FSDataInputStream in = fs.open(PATH)) {
            waitBlockTokenExpired(in);
    ...
    @Test
    public void testHedgedFetchBlockByteRangeWithExpiredToken_withUpgrade80() throws Exception {
        // read using hedgedFetchBlockByteRange(). Acquired tokens are cached in in
        try (FileSystem fs = newFileSystemHedgedRead();
            FSDataInputStream in = fs.open(PATH)) {
            waitBlockTokenExpired(in);
