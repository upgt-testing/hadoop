...
public class TestMiniDFSCluster {
    ...
    public void testClusterWithoutSystemProperties() throws Throwable {
        String oldPrp = System.getProperty(MiniDockerDFSCluster.PROP_TEST_BUILD_DATA);
        System.clearProperty(MiniDockerDFSCluster.PROP_TEST_BUILD_DATA);
        Configuration conf = new HdfsConfiguration();
        File testDataCluster1 = new File(testDataPath, CLUSTER_1);
        String c1Path = testDataCluster1.getAbsolutePath();
        conf.set(MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR, c1Path);
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).build()) {
            ...
            if (oldPrp != null) {
                System.setProperty(MiniDockerDFSCluster.PROP_TEST_BUILD_DATA, oldPrp);
            }
        }
    }
    ...
    public void testClusterSetStorageCapacity() throws Throwable {
        ...
        final MiniDockerDFSCluster cluster = newCluster(conf, numDatanodes, capacities, defaultBlockSize, fileLen);
        try {
    ...
    private void verifyStorageCapacity(final MiniDockerDFSCluster cluster, final long[] capacities) throws IOException {
        FsVolumeImpl source = null;
        FsVolumeImpl dest = null;
        /* verify capacity */
        for (int i = 0; i < cluster.getDataNodes().size(); i++) {
            final DataNodeInterface dnNode = cluster.getDataNodes().get(i);
    ...
    private MiniDockerDFSCluster newCluster(final Configuration conf, final int numDatanodes, final long[] storageCapacities, final int defaultBlockSize, final int fileLen) throws IOException, InterruptedException, TimeoutException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDatanodes).storageCapacities(storageCapacities).storageTypes(new StorageType[] { StorageType.DISK, StorageType.DISK }).storagesPerDatanode(2).build();
        cluster.waitActive();
    ...
    public void testIsClusterUpAfterShutdown() throws Throwable {
        ...
        MiniDockerDFSCluster cluster4 = new MiniDockerDFSCluster.Builder(conf, testDataCluster4).build();
        try {
    ...
    /**
     * MiniDockerDFSCluster should not clobber dfs.datanode.hostname if requested
     */
    @Test(timeout = 100000)
    public void testClusterSetDatanodeHostname() throws Throwable {
        ...
        try (MiniDockerDFSCluster cluster5 = new MiniDockerDFSCluster.Builder(conf, testDataCluster5).numDataNodes(1).checkDataNodeHostConfig(true).build()) {
    ...
    public void testClusterSetDatanodeDifferentStorageType() throws IOException {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storageTypes(storageType).build()) {
            cluster.waitActive();
    ...
    public void testSetUpFederatedCluster() throws Exception {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHAFederatedTopology(2)).numDataNodes(2).build()) {
            cluster.waitActive();
    ...
    public void testStartStopWithPorts() throws Exception {
        ...
        LambdaTestUtils.intercept(IllegalArgumentException.class, "Num of http ports (1) should match num of DataNodes (3)", "MiniJournalCluster port validation failed", () -> {
            new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).setDnHttpPorts(8481).build();
        });
        LambdaTestUtils.intercept(IllegalArgumentException.class, "Num of ipc ports (2) should match num of DataNodes (1)", "MiniJournalCluster port validation failed", () -> {
            new MiniDockerDFSCluster.Builder(conf).setDnIpcPorts(8481, 8482).build();
        });
        LambdaTestUtils.intercept(IllegalArgumentException.class, "Num of ipc ports (1) should match num of DataNodes (3)", "MiniJournalCluster port validation failed", () -> {
            new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).setDnHttpPorts(800, 9000, 10000).setDnIpcPorts(8481).build();
        });
        LambdaTestUtils.intercept(IllegalArgumentException.class, "Num of http ports (4) should match num of DataNodes (3)", "MiniJournalCluster port validation failed", () -> {
            new MiniDockerDFSCluster.Builder(conf).setDnHttpPorts(800, 9000, 1000, 2000).setDnIpcPorts(8481, 8482, 8483).numDataNodes(3).build();
        });
        ...
        try (MiniDockerDFSCluster miniDfsCluster = new MiniDockerDFSCluster.Builder(conf).setDnHttpPorts(httpPorts).setDnIpcPorts(ipcPorts).numDataNodes(3).build()) {
