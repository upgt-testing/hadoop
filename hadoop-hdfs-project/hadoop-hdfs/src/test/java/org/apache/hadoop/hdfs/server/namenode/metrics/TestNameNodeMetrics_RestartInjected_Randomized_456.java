 ...
 */
public class TestNameNodeMetrics_RestartInjected_Randomized_456 {
    ...
    public void testStaleNodes() throws Exception {
        ...
        RestartFramework.at("after_heartbeat_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_stale_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testVolumeFailures() throws Exception {
        ...
        RestartFramework.at("after_volume_failure").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_heartbeat").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDataNodeLivenessAndDecom() throws Exception {
        ...
        RestartFramework.at("after_dns_killed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_refresh_nodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_decommission").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bm.getDatanodeManager().refreshNodes(CONF);
        assertGauge("NumDecomLiveDataNodes", 0, getMetrics(NS_METRICS));
    ...
    public void testFileAdd() throws Exception {
        ...
        RestartFramework.at("after_ec_file_deletion").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_normal_file_deletion").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_normal_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        filesTotal--;
        rb = waitForDnMetricValue(NS_METRICS, "FilesTotal", filesTotal);
        RestartFramework.at("after_ec_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCorruptBlock() throws Exception {
        ...
        RestartFramework.at("after_block_corruption").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Verify aggregated blocks metrics
        ...
        RestartFramework.at("after_block_recovery").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Start block reconstruction work
        BlockManagerTestUtil.getComputedDatanodeWork(bm);
        BlockManagerTestUtil.updateState(bm);
        RestartFramework.at("after_file_replication").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStripedFileCorruptBlocks() throws Exception {
        ...
        cluster.getNamesystem().writeLock();
        RestartFramework.at("after_ec_corruption").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_ec_reconstruction").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
    ...
    public void testExcessBlocks() throws Exception {
        ...
        RestartFramework.at("after_replication_change").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetBlockLocationMetric() throws Exception {
        ...
        RestartFramework.at("after_read").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTransactionSinceLastCheckpointMetrics() throws Exception {
        ...
        while (retryCount < 5) {
            try {
                ...
                RestartFramework.at("after_ha_setup").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_checkpoint").on(cluster2).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
                nn0 = cluster2.getNameNode(0);
                nn1 = cluster2.getNameNode(1);
    ...
    public void testTransactionAndCheckpointMetrics() throws Exception {
        ...
        RestartFramework.at("after_editlog_roll").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSyncAndBlockReportMetric() throws Exception {
        ...
        RestartFramework.at("after_initial_block_report").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNumActiveClientsAndFilesUnderConstructionMetrics() throws Exception {
        ...
        RestartFramework.at("after_first_file_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            RestartFramework.at("after_multi_client_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            output2.close();
            output3.close();
            assertGauge("NumActiveClients", 1L, getMetrics(NS_METRICS));
            assertGauge("NumFilesUnderConstruction", 2L, getMetrics(NS_METRICS));
    ...
    public void testGenerateEDEKTime() throws IOException, NoSuchAlgorithmException {
        ...
        try (MiniDFSCluster clusterEDEK = new MiniDFSCluster.Builder(conf, basedir).numDataNodes(1).build()) {
            ...
            RestartFramework.at("after_encryption_zone_creation").on(clusterEDEK).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            MetricsRecordBuilder rb = getMetrics(NN_METRICS);
            for (int i = 0; i < 3; i++) {
                Path filePath = new Path("/zones/zone1/testfile-" + i);
                DFSTestUtil.createFile(fsEDEK, filePath, 1024, (short) 3, 1L);
                RestartFramework.at("after_encrypted_file_creation_" + i).on(clusterEDEK).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testResourceCheck() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_ha_setup").on(tmpCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            MetricsRecordBuilder rb = getMetrics(NN_METRICS);
            for (long i = 0; i < 10; i++) {
                rpc.monitorHealth();
    ...
    public void testEditLogTailing() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_ha_setup").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path testDir = new Path("/testdir");
            dfs.mkdir(testDir, FsPermission.getDefault());
            dfsCluster.getNameNodeRpc(0).rollEditLog();
            RestartFramework.at("after_editlog_roll").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(2 * 1000);
