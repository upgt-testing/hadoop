 ...
 */
public class TestReencryption_RestartInjected_Randomized_456 {
     ...
     */
        ...
        RestartFramework.at("after_files_created_in_zone").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // test re-encrypt without keyroll
        final Path encFile1 = new Path(zone, "0");
        RestartFramework.at("after_key_roll").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final FileEncryptionInfo fei0 = getFileEncryptionInfo(encFile1);
        dfsAdmin.reencryptEncryptionZone(zone, ReencryptAction.START);
        RestartFramework.at("after_first_reencryption").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_encryption_zone_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_second_reencryption").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
        ...
        RestartFramework.at("after_zone_created").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fsWrapper.mkdir(subdir, FsPermission.getDirDefault(), true);
        DFSTestUtil.createFile(fs, new Path(subdir, "f"), len, (short) 1, 0xFEED);
     ...
     */
        ...
        // newly created files use new edek, no need to re-encrypt
        expected -= 1;
        // resume re-encrypt thread which was paused after first checkpoint
        getEzManager().resumeReencryptForTesting();
        waitForZoneCompletes(zone.toString());
