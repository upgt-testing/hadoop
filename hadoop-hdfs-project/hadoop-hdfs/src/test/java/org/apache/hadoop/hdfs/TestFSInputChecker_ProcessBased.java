...
/**
 * ProcessBasedMiniDFSCluster version of {@link TestFSInputChecker}.
 *
 * Transformed from MiniDFSCluster to ProcessBasedMiniDFSCluster to enable
 * process-based testing and multi-version upgrade scenarios.
 *
 * Tests FSInputChecker correctness for read/seek/skip operations with checksum
 * verification on both HDFS and LocalFileSystem.
 *
 * @see TestFSInputChecker Original test using MiniDFSCluster
 */
@RunWith(Parameterized.class)
public class TestFSInputChecker_ProcessBased extends ProcessBasedUpgradeTestBase {
  ...
  public void testFSInputChecker() throws Exception {
    ...
    FileSystem dfsFileSys = cluster.getFileSystem();
    fs = (DistributedFileSystem) dfsFileSys;

    testChecker(dfsFileSys, true);
    checkpoint("AFTER_FIRST_TEST");

    testChecker(dfsFileSys, false);
    checkpoint("AFTER_SECOND_TEST");

    testSeekAndRead(dfsFileSys);
    checkpoint(UpgradeCheckpoints.BEFORE_VERIFICATION);

    // test Local FS
    FileSystem localFileSys = FileSystem.getLocal(conf);
    try {
      testChecker(localFileSys, true);
      testChecker(localFileSys, false);
      testFileCorruption((LocalFileSystem)localFileSys);
      testSeekAndRead(localFileSys);
    }finally {
      localFileSys.close();
    }
  }
