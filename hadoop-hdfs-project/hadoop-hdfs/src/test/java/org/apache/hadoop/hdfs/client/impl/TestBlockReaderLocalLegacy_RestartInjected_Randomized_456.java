...
public class TestBlockReaderLocalLegacy_RestartInjected_Randomized_456 {
    ...
    public void testStablePositionAfterCorruptRead() throws Exception {
        ...
        cluster.waitActive();
        RestartFramework.at("after_file_creation_corrupt_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cluster_start_corrupt_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBothOldAndNewShortCircuitConfigured() throws Exception {
        ...
        RestartFramework.at("after_file_creation_old_new_config").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fos.write(orig);
        fos.close();
        DFSTestUtil.waitReplication(fs, path, REPL_FACTOR);
        RestartFramework.at("after_cluster_start_old_new_config").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBlockReaderLocalLegacyWithAppend() throws Exception {
        ...
        cluster.waitActive();
        RestartFramework.at("after_file_creation_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cluster_start_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        {
            // append one byte
