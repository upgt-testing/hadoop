 ...
 */
public class TestHDFSContractConcat_RestartInjected_Randomized_456 extends AbstractContractConcatTest {
    ...
    public void testConcatFileOnFile() throws Throwable {
        ...
        RestartFramework.at("after_target_file_creation").on(HDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        getFileSystem().concat(target, new Path[] { srcFile });
        RestartFramework.at("after_concat_operation").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFileHasLength(getFileSystem(), target, TEST_FILE_LEN * 2);
        RestartFramework.at("before_concat_operation").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
