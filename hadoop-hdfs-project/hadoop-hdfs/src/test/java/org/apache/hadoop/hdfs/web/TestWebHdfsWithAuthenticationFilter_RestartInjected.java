...
public class TestWebHdfsWithAuthenticationFilter_RestartInjected {
    ...
    static public class Initializer extends FilterInitializer {
      ...
      public void initFilter(FilterContainer container, Configuration config) {
        container.addFilter("customFilter",
            TestWebHdfsWithAuthenticationFilter_RestartInjected.CustomizedFilter.class.
            getName(), null);
      }
    }
  }
  ...
  public void testWebHdfsAuthFilter() throws IOException {
    ...
    RestartFramework.at("after_unauthorized_attempt")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    authorized = true;
    RestartFramework.at("after_authorization_change")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    fs.getFileStatus(new Path("/"));
    RestartFramework.at("after_authorized_operation")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
