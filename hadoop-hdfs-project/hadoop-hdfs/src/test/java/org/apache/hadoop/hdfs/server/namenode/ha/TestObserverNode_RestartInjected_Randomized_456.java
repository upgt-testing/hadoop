 ...
 */
public class TestObserverNode_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(TestObserverNode_RestartInjected.class.getName());
    ...
    public static void startUpCluster() throws Exception {
        ...
        conf.setTimeDuration(OBSERVER_PROBE_RETRY_PERIOD_KEY, 0, TimeUnit.MILLISECONDS);
    ...
    public void cleanUp() throws IOException {
        ...
        assertEquals("NN[0] should be active", HAServiceState.ACTIVE, getServiceState(dfsCluster.getNameNode(0)));
        assertEquals("NN[1] should be standby", HAServiceState.STANDBY, getServiceState(dfsCluster.getNameNode(1)));
        assertEquals("NN[2] should be observer", HAServiceState.OBSERVER, getServiceState(dfsCluster.getNameNode(2)));
    }
    ...
    public void testNoActiveToObserver() throws Exception {
        ...
        RestartFramework.at("before_transition_attempt").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetGroups() throws Exception {
        RestartFramework.at("before_get_groups").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNoObserverToActive() throws Exception {
        ...
        RestartFramework.at("before_transition_observer_to_active").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSimpleRead() throws Exception {
        ...
        RestartFramework.at("after_mkdir_testpath").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(2);
        RestartFramework.at("after_observer_read").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFailover() throws Exception {
        Path testPath2 = new Path(testPath, "test2");
        RestartFramework.at("after_first_mkdir").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        setObserverRead(false);
        dfs.mkdir(testPath, FsPermission.getDefault());
        assertSentTo(0);
        dfs.getFileStatus(testPath);
        assertSentTo(0);
        RestartFramework.at("after_second_mkdir").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_failover_back").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.transitionToStandby(0);
        dfsCluster.transitionToActive(1);
        dfsCluster.waitActive(1);
        dfs.mkdir(testPath2, FsPermission.getDefault());
        RestartFramework.at("after_first_failover").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(1);
        dfs.getFileStatus(testPath);
        assertSentTo(1);
    ...
    public void testDoubleFailover() throws Exception {
        ...
        dfs.mkdir(testPath2, FsPermission.getDefault());
        assertSentTo(0);
        RestartFramework.at("after_delete").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_failover").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.transitionToStandby(0);
        RestartFramework.at("after_mkdir_testpath2").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.transitionToActive(1);
        dfsCluster.waitActive(1);
        RestartFramework.at("after_first_failover").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        dfsCluster.transitionToActive(0);
        dfsCluster.waitActive(0);
        RestartFramework.at("after_mkdir_testpath3").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.rollEditLogAndTail(0);
    ...
    public void testObserverShutdown() throws Exception {
        ...
        RestartFramework.at("after_observer_read").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_observer_shutdown").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfs.getFileStatus(testPath);
        assertSentTo(0);
        RestartFramework.at("after_observer_restart").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testObserverFailOverAndShutdown() throws Exception {
        dfs.mkdir(testPath, FsPermission.getDefault());
        dfsCluster.rollEditLogAndTail(0);
        RestartFramework.at("after_initial_read").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        dfs.getFileStatus(testPath);
        assertSentTo(2);
        dfsCluster.transitionToStandby(0);
        dfsCluster.transitionToActive(1);
        ...
        dfsCluster.restartNameNode(2);
        dfs.getFileStatus(testPath);
        RestartFramework.at("after_observer_shutdown").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_observer_restart").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(1);
        dfsCluster.transitionToObserver(2);
        dfs.getFileStatus(testPath);
        ...
        // the second will properly go to the observer
        dfs.getFileStatus(testPath);
        RestartFramework.at("after_failover").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(2);
        dfsCluster.transitionToStandby(1);
        dfsCluster.transitionToActive(0);
        dfsCluster.waitActive(0);
    ...
    public void testBootstrap() throws Exception {
        ...
        RestartFramework.at("after_bootstrap").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testObserverNodeSafeModeWithBlockLocations() throws Exception {
        ...
        assertSentTo(0);
        RestartFramework.at("after_failover_to_active").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.rollEditLogAndTail(0);
        RestartFramework.at("after_create_file").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfs.open(testPath).close();
        assertSentTo(2);
        RestartFramework.at("after_observer_open").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        }).when(bmSpy).createLocatedBlocks(any(), anyLong(), anyBoolean(), anyLong(), anyLong(), anyBoolean(), anyBoolean(), any(), any());
        RestartFramework.at("after_safemode_leave").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_safemode_enter").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testObserverNodeBlockMissingRetry() throws Exception {
        ...
        doAnswer((invocation) -> {
            ...
            fakeBlocks.add(fakeBlock);
            return new LocatedBlocks(0, false, fakeBlocks, null, true, null, null);
        }).when(bmSpy).createLocatedBlocks(Mockito.any(), anyLong(), anyBoolean(), anyLong(), anyLong(), anyBoolean(), anyBoolean(), Mockito.any(), Mockito.any());
        RestartFramework.at("after_block_manager_mock").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        dfs.open(testPath);
        assertSentTo(0);
        RestartFramework.at("after_block_missing_retry").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFsckWithObserver() throws Exception {
        ...
        RestartFramework.at("after_fsck").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_file").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testObserverRetryActiveException() throws Exception {
        ...
        try {
            ...
            dfsCluster.getNameNode(2).getRpcServer().mkdirs("/testActiveRetryException", FsPermission.createImmutable((short) 0755), true);
        } catch (ObserverRetryOnActiveException orae) {
            thrownRetryException = true;
        }
        assertTrue(thrownRetryException);
        RestartFramework.at("after_retry_exception").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testAccessTimeUpdateRedirectToActive() throws Exception {
        ...
        RestartFramework.at("after_set_times").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_access_time_redirect").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(0);
    ...
    public void testStickyActive() throws Exception {
        ...
        RestartFramework.at("after_observer_to_standby").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_standby_to_observer").on(dfsCluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_file").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertSentTo(0);
        dfsCluster.rollEditLogAndTail(0);
        // No Observers present, should still go to Active
        dfsCluster.transitionToStandby(2);
        assertEquals("NN[2] should be standby", HAServiceState.STANDBY, getServiceState(dfsCluster.getNameNode(2)));
        newFs.open(testFile).close();
        assertSentTo(0);
        // Restore Observer
        int newObserver = 1;
        dfsCluster.transitionToObserver(newObserver);
        assertEquals("NN[" + newObserver + "] should be observer", HAServiceState.OBSERVER, getServiceState(dfsCluster.getNameNode(newObserver)));
        ...
        try {
            ...
            dfsCluster.transitionToStandby(1);
            dfsCluster.transitionToObserver(2);
            RestartFramework.at("after_restore_topology").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testFsckDelete() throws Exception {
        ...
        RestartFramework.at("after_corrupt_replica").on(dfsCluster).restart("datanode").withIndex(dnToCorrupt).withMode(RestartMode.GRACEFUL).execute();
        ...
        LOG.info("result=" + result);
        RestartFramework.at("after_fsck_delete").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirsRaceWithObserverRead() throws Exception {
        ...
        FSEditLog spyEditLog = NameNodeAdapter.spyDelayMkDirTransaction(dfsCluster.getNameNode(0), 100);
        RestartFramework.at("after_verification").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_submit_threads").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        // wait until mkdir is logged
        Thread.sleep(150);
        long activStateId = dfsCluster.getNameNode(0).getFSImage().getLastAppliedOrWrittenTxId();
        dfsCluster.rollEditLogAndTail(0);
        boolean finished = true;
        RestartFramework.at("after_threads_finish").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        threadPool.shutdown();
        assertEquals("Active and Observer stateIds don't match", dfsCluster.getNameNode(0).getFSImage().getLastAppliedOrWrittenTxId(), dfsCluster.getNameNode(2).getFSImage().getLastAppliedOrWrittenTxId());
        RestartFramework.at("after_initial_mkdir").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < numThreads; i++) {
            assertTrue("Client #" + i + " lastSeenStateId=" + clientStates[i].lastSeenStateId + " activStateId=" + activStateId + "\n" + clientStates[i].fnfe, clientStates[i].lastSeenStateId >= activStateId && clientStates[i].fnfe == null);
        }
        // Restore edit log
    ...
    public void testGetListingForDeletedDir() throws Exception {
        ...
        RestartFramework.at("after_delete_dir").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(dfs.delete(new Path("/dir1/dir2"), true));
        RestartFramework.at("after_list_deleted_dir").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSimpleReadEmptyDirOrFile() throws IOException {
        ...
        assertSentTo(0);
        RestartFramework.at("after_read_emptyfile").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        dfs.getClient().listPaths("/", new byte[0], true);
        assertSentTo(2);
        dfs.getClient().getLocatedFileInfo("/emptyDir", true);
        assertSentTo(2);
        RestartFramework.at("after_create_emptyfile").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdirs_emptydir").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_read_emptydir").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        // read empty file
    ...
    private static void assertSentTo(DistributedFileSystem fs, int nnIdx) throws IOException {
        assertTrue("Request was not sent to the expected namenode " + nnIdx, HATestUtil.isSentToAnyOfNameNodes(fs, dfsCluster, nnIdx));
    }
