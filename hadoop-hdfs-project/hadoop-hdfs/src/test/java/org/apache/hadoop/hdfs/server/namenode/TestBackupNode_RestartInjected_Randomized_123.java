...
public class TestBackupNode_RestartInjected_Randomized_123 {

    public static final Logger LOG = LoggerFactory.getLogger(TestBackupNode_RestartInjected.class);
    ...
    BackupNode startBackupNode(Configuration conf, StartupOption startupOpt, int idx) throws IOException {
    ...
    public void testBackupNodeTailsEdits() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_backup_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            BackupImage bnImage = (BackupImage) backup.getFSImage();
            testBNInSync(cluster, backup, 1);
            RestartFramework.at("after_first_sync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Force a roll -- BN should roll with NN.
            NameNode nn = cluster.getNameNode();
            NamenodeProtocols nnRpc = nn.getRpcServer();
            nnRpc.rollEditLog();
            RestartFramework.at("after_edit_log_roll").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_sync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            long nnImageBefore = nn.getFSImage().getStorage().getMostRecentCheckpointTxId();
            // BN checkpoint
            backup.doCheckpoint();
            RestartFramework.at("after_bn_checkpoint").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            ...
            RestartFramework.at("after_third_sync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            // Stop BN
            ...
            backup.stop();
            backup = null;
            RestartFramework.at("after_bn_stop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            ...
            RestartFramework.at("after_mkdir_bn_down").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            // start a new backup node
            backup = startBackupNode(conf, StartupOption.BACKUP, 1);
            RestartFramework.at("after_backup_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            testBNInSync(cluster, backup, 4);
            RestartFramework.at("after_fourth_sync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            assertNotNull(backup.getNamesystem().getFileInfo("/edit-while-bn-down", false, false, false));
            ...
            backup.stop(false);
            RestartFramework.at("after_bn_unclean_stop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
            ...
            RestartFramework.at("after_mkdir_bn_down_2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            nnRpc = cluster.getNameNode().getRpcServer();
        } finally {
    ...
    void testCheckpoint(StartupOption op) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_mkdir_file1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //
            // Take a checkpoint
            //
        ...
        try {
            ...
            RestartFramework.at("after_file1_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // create new file file2
            fileSys.mkdirs(file2);
            RestartFramework.at("after_mkdir_file2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //
            // Take a checkpoint
            //
            long txid = cluster.getNameNodeRpc().getTransactionID();
            ...
            RestartFramework.at("after_mkdirs_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            backup.doCheckpoint();
            waitCheckpointDone(cluster, txid);
            RestartFramework.at("after_fourth_checkpoint").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_file3_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCanReadData() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_datanode_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
