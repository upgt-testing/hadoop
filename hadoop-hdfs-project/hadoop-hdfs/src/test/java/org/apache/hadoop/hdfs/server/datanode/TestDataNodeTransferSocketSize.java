...
import org.apache.hadoop.hdfs.DataNodeProxy;
    ...
    try {
      ...
      assertEquals("Receive buffer size should be 4K",
              //4 * 1024, datanode.getXferServer().getPeerServer().getReceiveBufferSize());
                4 * 1024, datanode.fetchXferServerGetPeerServerGetReceiveBufferSize());
    } finally {
      if (cluster != null) {
        cluster.shutdown();
      }
      ...
      assertTrue(
        "Receive buffer size should be a default value (determined by kernel)",
              //datanode.getXferServer().getPeerServer().getReceiveBufferSize() > 0);
                datanode.fetchXferServerGetPeerServerGetReceiveBufferSize() > 0);
    } finally {
      if (cluster != null) {
        cluster.shutdown();
      }
