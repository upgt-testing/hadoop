...
public class TestBlockReaderLocal {
    ...
    private void testStatistics(boolean isShortCircuit) throws Exception {
        ...
        FileSystem fs = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).hosts(new String[] { NetUtils.getLocalHostname() }).build();
            cluster.waitActive();
            fs = cluster.getFileSystem();
    ...
    public void testStatisticsForErasureCodingRead() throws IOException {
        ...
        final short repl = 1;
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDataNodes).build()) {
            cluster.waitActive();
