...
@NotThreadSafe
public class TestDataNodeMetrics_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestDataNodeMetrics_RestartInjected.class);
    ...
    public void testDataNodeMXBeanActiveThreadCount() throws Exception {
        ...
        try {
            ...
            //not easy to produce
            assertEquals(0, recoveryWorkerCount);
    ...
    public void testNNRpcMetricsWithHA() throws IOException {
        ...
        // only sends heartbeat once after startup
        conf.setTimeDuration(DFS_HEARTBEAT_INTERVAL_KEY, 1, TimeUnit.HOURS);
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        RestartFramework.at("after_cluster_active").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.waitActive();
        DataNode dn = cluster.getDataNodes().get(0);
    ...
    public void testNNRpcMetricsWithFederation() throws IOException {
        ...
        // only sends heartbeat once after startup
        conf.setTimeDuration(DFS_HEARTBEAT_INTERVAL_KEY, 1, TimeUnit.HOURS);
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleFederatedTopology("ns1,ns2")).build();
        cluster.waitActive();
        DataNode dn = cluster.getDataNodes().get(0);
        RestartFramework.at("after_cluster_active").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        MetricsRecordBuilder rb = getMetrics(dn.getMetrics().name());
    ...
    public void testNNRpcMetricsWithFederationAndHA() throws IOException {
        ...
        // only sends heartbeat once after startup
        conf.setTimeDuration(DFS_HEARTBEAT_INTERVAL_KEY, 1, TimeUnit.HOURS);
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHAFederatedTopology(2)).build();
        RestartFramework.at("after_cluster_active").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.waitActive();
        DataNode dn = cluster.getDataNodes().get(0);
        MetricsRecordBuilder rb = getMetrics(dn.getMetrics().name());
