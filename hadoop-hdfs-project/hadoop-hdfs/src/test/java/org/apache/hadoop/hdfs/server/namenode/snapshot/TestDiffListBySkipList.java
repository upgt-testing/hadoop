...
public class TestDiffListBySkipList {
    ...
    private static ChildrenDiff getCombined(DiffList<DirectoryDiff> list, int from, int to, INodeDirectory dir) {
        ...
        for (DirectoryDiffInterface d : minList) {
    ...
    static List<INode> getChildrenList(DiffList<DirectoryDiff> list, int from, int to, INodeDirectory dir) {
        final ChildrenDiffInterface combined = getCombined(list, from, to, dir);
    ...
    static void testAddLast(int n) throws Exception {
        ...
        INodeDirectoryInterface dir = addDiff(n, skipList, arrayList, root);
    ...
    static void testAddFirst(int n) throws Exception {
        ...
        for (int i = diffs.size() - 1; i >= 0; i--) {
            final DirectoryDiffInterface d = diffs.get(i);
            skipList.addFirst(d);
    ...
    static INodeDirectory addDiff(int n, DiffList skipList, DiffList arrayList, final Path root) throws Exception {
        ...
        INodeDirectoryInterface dir = fsdir.getINode(root.toString()).asDirectory();
        DiffList<DirectoryDiff> diffs = dir.getDiffs().asList();
        for (DirectoryDiffInterface d : diffs) {
            skipList.addLast(d);
            arrayList.addLast(d);
        }
    ...
    static void testRemove(String name, int n, ToIntBiFunction<DiffListBySkipList, Integer> indexFunction) throws Exception {
        ...
        final DiffListBySkipList skipList = newDiffListBySkipList();
        final DiffList<DirectoryDiff> arrayList = new DiffListByArrayList<>(0);
        final INodeDirectoryInterface dir = addDiff(n, skipList, arrayList, root);
        ...
        for (int i = 0; i < n; i++) {
            ...
            final DirectoryDiffInterface diff = remove(index, skipList, arrayList);
    ...
    static DirectoryDiff remove(int i, DiffListBySkipList skip, DiffList<DirectoryDiff> array) {
        final DirectoryDiffInterface expected = array.remove(i);
        DiffListBySkipList.LOG.info("remove " + i + ", snapshotId=" + expected.getSnapshotId());
        final DirectoryDiffInterface computed = skip.remove(i);
    ...
    static void assertSkipListNode(SkipListNode n) {
        for (int i = 1; i <= n.level(); i++) {
            ...
            final ChildrenDiffInterface diff = n.getChildrenDiff(i);
            if (target == null) {
                if (diff != null) {
