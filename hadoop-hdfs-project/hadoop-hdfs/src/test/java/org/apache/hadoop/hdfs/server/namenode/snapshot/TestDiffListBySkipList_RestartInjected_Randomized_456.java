 ...
 */
public class TestDiffListBySkipList_RestartInjected_Randomized_456 {
    ...
    static void verifyChildrenList(DiffListBySkipList skip, INodeDirectory dir) {
        ...
        for (int i = 0; i < skip.size(); i++) {
            final List<INode> expected = ReadOnlyList.Util.asList(dir.getChildrenList(dir.getDiffs().asList().get(i).getSnapshotId()));
    ...
    static void verifyChildrenList(DiffList<DirectoryDiff> array, DiffListBySkipList skip, INodeDirectory dir, List<INode> childrenList) {
        final int n = array.size();
        Assert.assertEquals(n, skip.size());
        for (int i = 0; i < n - 1; i++) {
            for (int j = i + 1; j < n - 1; j++) {
                final List<INode> expected = getCombined(array, i, j, dir).apply2Previous(childrenList);
                final List<INode> computed = getCombined(skip, i, j, dir).apply2Previous(childrenList);
                try {
                    assertList(expected, computed);
                } catch (AssertionError ae) {
    ...
    private static ChildrenDiff getCombined(DiffList<DirectoryDiff> list, int from, int to, INodeDirectory dir) {
    ...
    static List<INode> getChildrenList(DiffList<DirectoryDiff> list, int from, int to, INodeDirectory dir) {
        final ChildrenDiff combined = getCombined(list, from, to, dir);
        return combined.apply2Current(ReadOnlyList.Util.asList(dir.getChildrenList(Snapshot.CURRENT_STATE_ID)));
    }
    ...
    static void testAddFirst(int n) throws Exception {
        ...
        RestartFramework.at("after_creating_children_in_addFirst").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        INodeDirectory dir = fsdir.getINode(root.toString()).asDirectory();
        SnapshotTestHelper.createSnapshot(hdfs, root, "s0");
        RestartFramework.at("after_initial_snapshot_in_addFirst").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 1; i < n; i++) {
            final Path child = getChildPath(root, n - i);
            hdfs.delete(child, false);
            hdfs.createSnapshot(root, "s" + i);
        }
        RestartFramework.at("after_deletes_and_snapshots_in_addFirst").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DiffList<DirectoryDiff> diffs = dir.getDiffs().asList();
        List<INode> childrenList = ReadOnlyList.Util.asList(dir.getChildrenList(diffs.get(0).getSnapshotId()));
        final DiffListBySkipList skipList = newDiffListBySkipList();
        final DiffList<DirectoryDiff> arrayList = new DiffListByArrayList<>(0);
    ...
    static INodeDirectory addDiff(int n, DiffList skipList, DiffList arrayList, final Path root) throws Exception {
        RestartFramework.at("after_all_snapshots_in_addDiff").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.mkdirs(root);
        SnapshotTestHelper.createSnapshot(hdfs, root, "s0");
        for (int i = 1; i < n; i++) {
            final Path child = getChildPath(root, i);
        ...
        RestartFramework.at("after_initial_snapshot_in_addDiff").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        INodeDirectory dir = fsdir.getINode(root.toString()).asDirectory();
        DiffList<DirectoryDiff> diffs = dir.getDiffs().asList();
    ...
    static void testRemove(String name, int n, ToIntBiFunction<DiffListBySkipList, Integer> indexFunction) throws Exception {
        ...
        RestartFramework.at("after_all_deletions_in_testRemove").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void testRemoveFromLowerLevel() throws Exception {
        testRemove("FromLowerLevel", NUM_SNAPSHOTS, new ToIntBiFunction<DiffListBySkipList, Integer>() {
    ...
    public void testRemoveFromUpperLevel() throws Exception {
        testRemove("FromUpperLevel", NUM_SNAPSHOTS, new ToIntBiFunction<DiffListBySkipList, Integer>() {
    ...
    static DirectoryDiff remove(int i, DiffListBySkipList skip, DiffList<DirectoryDiff> array) {
    ...
    static void assertDirectoryDiff(DirectoryDiff expected, DirectoryDiff computed) {
    ...
    static void assertSkipListNode(SkipListNode n) {
        for (int i = 1; i <= n.level(); i++) {
            ...
            if (target == null) {
                if (diff != null) {
                    throw new AssertionError("Target is null but children diff is not at i=" + i + n.appendTo(new StringBuilder(": ")));
                }
            } else if (target == n.getSkipNode(i - 1)) {
                if (diff != n.getChildrenDiff(i - 1)) {
                    throw new AssertionError("Same target but different children diff at i=" + i + n.appendTo(new StringBuilder(": ")));
                }
            }
        }
    }
