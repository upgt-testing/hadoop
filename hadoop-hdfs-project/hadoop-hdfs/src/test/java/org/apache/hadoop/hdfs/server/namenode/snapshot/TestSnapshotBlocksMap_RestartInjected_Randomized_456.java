 ...
 */
public class TestSnapshotBlocksMap_RestartInjected_Randomized_456 {
   ...
   */
    ...
    public void testDeletionWithZeroSizeBlock() throws Exception {
        ...
        RestartFramework.at("after_second_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_append_and_addblock").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDeletionOfLaterBlocksWithZeroSizeFirstBlock() throws Exception {
        final Path foo = new Path("/foo");
        RestartFramework.at("after_file_extension").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path bar = new Path(foo, "bar");
        RestartFramework.at("after_snapshot_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_zero_length_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Delete the file.
        hdfs.delete(bar, true);
