...
public class TestReconstructStripedFileWithValidator extends TestReconstructStripedFile {
    ...
    public void testValidatorWithBadDecoding() throws Exception {
        MiniDockerDFSCluster cluster = getCluster();
        cluster.getDataNodes().stream().map(DataNodeInterface::getMetrics).map(DataNodeMetricsInterface::getECInvalidReconstructionTasks).forEach(n -> Assert.assertEquals(0, (long) n));
        ...
        int fileLen = (getEcPolicy().getNumDataUnits() + getEcPolicy().getNumParityUnits()) * getBlockSize() + getBlockSize() / 10;
        try {
            assertFileBlocksReconstruction("/testValidatorWithBadDecoding", fileLen, ReconstructionType.DataOnly, getEcPolicy().getNumParityUnits());
            long sum = cluster.getDataNodes().stream().map(DataNodeInterface::getMetrics).mapToLong(DataNodeMetricsInterface::getECInvalidReconstructionTasks).sum();
