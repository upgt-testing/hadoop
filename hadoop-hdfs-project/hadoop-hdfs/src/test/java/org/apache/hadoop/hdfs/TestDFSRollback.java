...
public class TestDFSRollback {
    ...
    void startNameNodeShouldFail(String searchString) {
        try {
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).// should fail
            build();
    ...
    public void testRollback() throws Exception {
        ...
        for (int numDirs = 1; numDirs <= 2; numDirs++) {
            ...
            UpgradeUtilities.createNameNodeStorageDirs(nameNodeDirs, "previous");
            NameNode.doRollback(conf, false);
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).dnStartupOption(StartupOption.ROLLBACK).build();
            ...
            UpgradeUtilities.createNameNodeStorageDirs(nameNodeDirs, "current");
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).startupOption(StartupOption.UPGRADE).build();
            UpgradeUtilities.createDataNodeStorageDirs(dataNodeDirs, "current");
            cluster.startDataNodes(conf, 1, false, StartupOption.ROLLBACK, null);
            cluster.shutdown();
            UpgradeUtilities.createEmptyDirs(nameNodeDirs);
