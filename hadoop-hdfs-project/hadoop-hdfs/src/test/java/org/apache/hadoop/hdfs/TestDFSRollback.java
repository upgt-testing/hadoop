...
public class TestDFSRollback {
    ...
    void startNameNodeShouldFail(String searchString) {
        try {
            ...
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).// should fail
            build();
    ...
    public void testRollback() throws Exception {
        ...
        for (int numDirs = 1; numDirs <= 2; numDirs++) {
            ...
            UpgradeUtilities.createNameNodeStorageDirs(nameNodeDirs, "previous");
            NameNode.doRollback(conf, false);
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).dnStartupOption(StartupOption.ROLLBACK).build();
            ...
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).dnStartupOption(StartupOption.ROLLBACK).build();
            UpgradeUtilities.createDataNodeStorageDirs(dataNodeDirs, "current");
            UpgradeUtilities.createBlockPoolStorageDirs(dataNodeDirs, "current", UpgradeUtilities.getCurrentBlockPoolIDJVM(cluster));
            // Create a previous snapshot for the blockpool
            UpgradeUtilities.createBlockPoolStorageDirs(dataNodeDirs, "previous", UpgradeUtilities.getCurrentBlockPoolIDJVM(cluster));
            // Put newer layout version in current.
            storageInfo = new StorageInfo(HdfsServerConstants.DATANODE_LAYOUT_VERSION - 1, UpgradeUtilities.getCurrentNamespaceIDJVM(cluster), UpgradeUtilities.getCurrentClusterIDJVM(cluster), UpgradeUtilities.getCurrentFsscTimeJVM(cluster), NodeType.DATA_NODE);
            ...
            UpgradeUtilities.createDataNodeVersionFile(dataCurrentDirs, storageInfo, UpgradeUtilities.getCurrentBlockPoolIDJVM(cluster), conf);
            cluster.startDataNodes(conf, 1, false, StartupOption.ROLLBACK, null);
            ...
            UpgradeUtilities.createNameNodeStorageDirs(nameNodeDirs, "current");
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(0).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).startupOption(StartupOption.UPGRADE).build();
            UpgradeUtilities.createDataNodeStorageDirs(dataNodeDirs, "current");
            cluster.startDataNodes(conf, 1, false, StartupOption.ROLLBACK, null);
            cluster.shutdown();
            UpgradeUtilities.createEmptyDirs(nameNodeDirs);
            ...
            storageInfo = new StorageInfo(Integer.MIN_VALUE, UpgradeUtilities.getCurrentNamespaceIDJVM(cluster), UpgradeUtilities.getCurrentClusterIDJVM(cluster), UpgradeUtilities.getCurrentFsscTimeJVM(cluster), NodeType.DATA_NODE);
            UpgradeUtilities.createDataNodeVersionFile(baseDirs, storageInfo, UpgradeUtilities.getCurrentBlockPoolIDJVM(cluster), conf);
            ...
            UpgradeUtilities.createDataNodeStorageDirs(dataNodeDirs, "current");
            baseDirs = UpgradeUtilities.createDataNodeStorageDirs(dataNodeDirs, "previous");
            storageInfo = new StorageInfo(HdfsServerConstants.DATANODE_LAYOUT_VERSION, UpgradeUtilities.getCurrentNamespaceIDJVM(cluster), UpgradeUtilities.getCurrentClusterIDJVM(cluster), Long.MAX_VALUE, NodeType.DATA_NODE);
            UpgradeUtilities.createDataNodeVersionFile(baseDirs, storageInfo, UpgradeUtilities.getCurrentBlockPoolIDJVM(cluster), conf);
            startBlockPoolShouldFail(StartupOption.ROLLBACK, cluster.getNamesystem().getBlockPoolId());
            cluster.shutdown();
            UpgradeUtilities.createEmptyDirs(nameNodeDirs);
            ...
            UpgradeUtilities.createNameNodeVersionFile(conf, baseDirs, storageInfo, UpgradeUtilities.getCurrentBlockPoolIDJVM(cluster));
