...
public class TestUnderReplicatedBlocks_RestartInjected_Randomized_456 {
    ...
    public void testSetRepIncWithUnderReplicatedBlocks() throws Exception {
        ...
        try {
            ...
            BlockManagerTestUtil.updateState(bm);
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_blocksmap_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            BlockManagerTestUtil.updateState(bm);
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_setrep").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            bm = cluster.getNamesystem().getBlockManager();
        } finally {
            cluster.shutdown();
        }
    ...
    public void testNumberOfBlocksToBeReplicated() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_datanode_added").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            BlockManager bm = cluster.getNamesystem().getBlockManager();
            ExtendedBlock b = DFSTestUtil.getFirstBlock(fs, FILE_PATH);
            ...
            DFSTestUtil.verifyClientStats(conf, cluster);
            RestartFramework.at("after_datanode_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            bm = cluster.getNamesystem().getBlockManager();
            ...
            RestartFramework.at("after_compute_replication_work").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            bm = cluster.getNamesystem().getBlockManager();
            BlockManagerTestUtil.updateState(bm);
