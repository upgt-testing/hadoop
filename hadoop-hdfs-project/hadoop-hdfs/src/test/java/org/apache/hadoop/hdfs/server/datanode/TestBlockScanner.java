...
public class TestBlockScanner {
    ...
    private static class TestContext implements Closeable {
        ...
        final BlockScannerInterface blockScanner;
        ...
        TestContext(Configuration conf, int numNameServices) throws Exception {
            ...
            long expectedVScannerTimeOut = volumeScannerTimeOutFromConf == -1 ? MiniDockerDFSCluster.DEFAULT_SCANNER_VOLUME_JOIN_TIMEOUT_MSEC : volumeScannerTimeOutFromConf;
            MiniDockerDFSCluster.Builder bld = new MiniDockerDFSCluster.Builder(conf, basedir).numDataNodes(1).storagesPerDatanode(1);
    ...
    private void testScanAllBlocksImpl(final boolean rescan) throws Exception {
        ...
        if (!rescan) {
            ...
            StatisticsInterface stats = ctx.blockScanner.getVolumeStats(ctx.volumes.get(0).getStorageID());
