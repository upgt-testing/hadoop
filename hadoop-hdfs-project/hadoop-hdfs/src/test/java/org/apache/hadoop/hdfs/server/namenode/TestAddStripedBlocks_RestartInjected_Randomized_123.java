...
public class TestAddStripedBlocks_RestartInjected_Randomized_123 {
    ...
    private void checkStripedBlockUC(BlockInfoStriped block, boolean checkReplica) {
        ...
        assertEquals(0, block.getBlockId() & HdfsServerConstants.BLOCK_GROUP_INDEX_MASK);
        assertEquals(HdfsServerConstants.BlockUCState.UNDER_CONSTRUCTION, block.getBlockUCState());
        if (checkReplica) {
            assertEquals(groupSize, block.getUnderConstructionFeature().getNumExpectedLocations());
    ...
    public void testAddUCReplica() throws Exception {
        ...
        for (i = 0; i < groupSize; i++) {
            assertEquals(storageIDs.get(i), locs[groupSize - 1 - i].getStorageID());
    ...
    public void testStripedFlagInBlockLocation() throws IOException {
        ...
        assertEquals("There should be exactly one Block present", 1, locations.length);
