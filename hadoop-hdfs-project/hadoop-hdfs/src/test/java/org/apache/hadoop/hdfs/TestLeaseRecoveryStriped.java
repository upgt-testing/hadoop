...
public class TestLeaseRecoveryStriped {
        ...
        for (int i = 0; i < groups; i++) {
            ...
            for (int j = 0; j < blockLengths.length; j++) {
                ...
                if (i == groups - 1 && j < dataBlocks) {
                    numDelta = random.nextInt(maxNumDelta - minNumDelta + 1) + minNumDelta;
                }
    ...
    public void testLeaseRecovery() throws Exception {
        LOG.info("blockLengthsSuite: " + Arrays.toString(blockLengthsSuite));
    ...
    public void testSafeLength() {
        ...
        checkSafeLength(0, 0);
        // Length of: 1 MiB
        checkSafeLength(1024 * 1024, 6291456L);
        // Length of: 64 MiB
        checkSafeLength(64 * 1024 * 1024, 402653184L);
        // Length of: 189729792
        checkSafeLength(189729792, 1132462080L);
        // Length of: 256 MiB
        checkSafeLength(256 * 1024 * 1024, 1610612736L);
        // Length of: 517399040
        checkSafeLength(517399040, 3101687808L);
        // Length of: 1 GiB
        checkSafeLength(1024 * 1024 * 1024, 6442450944L);
    }

    private void checkSafeLength(int blockLength, long expectedSafeLength) {
        int[] blockLengths = new int[] { blockLength, blockLength, blockLength, blockLength, blockLength, blockLength };
    ...
    private void runTest(int[] blockLengths, long safeLength) throws Exception {
        ...
        oldGS.add(1001L);
        StripedFileTestUtil.checkData(dfs, p, checkDataLength, new ArrayList<DatanodeInfo>(), oldGS, blockGroupSize);
    ...
    private void writePartialBlocks(int[] blockLengths) throws Exception {
        ...
        try {
            for (int pos = 0; pos < testFileLength; pos++) {
                ...
                if (pos == checkingPos) {
                    for (int index : getIndexToStop(posToKill, pos)) {
                        ...
                        LOG.info("Stopping block stream idx {} at file offset {} block " + "length {}", index, pos, blockLengths[index]);
            ...
            for (int i = 0; i < blockLengths.length; i++) {
                ...
                LOG.info("Waiting for block stream idx {} to reach length {}", i, blockLengths[i]);
                waitStreamerAllAcked(s);
            }
    ...
    private int[] getPosToKill(int[] blockLengths) {
        ...
        for (int i = 0; i < dataBlocks; i++) {
            ...
            posToKill[i] = numStripe * stripeSize + i * cellSize + blockLengths[i] % cellSize;
    ...
    private void stopBlockStream(StripedDataStreamer s) throws Exception {
        ...
        Whitebox.setInternalState(s, "blockStream", new DataOutputStream(nullOutputStream));
    }
    ...
    @Test
    public void testLeaseRecovery_withUpgrade20() throws Exception {
        LOG.info("blockLengthsSuite: " + Arrays.toString(blockLengthsSuite));
        for (int i = 0; i < blockLengthsSuite.length; i++) {
            BlockLengths blockLengths = blockLengthsSuite[i];
    ...
    @Test
    public void testLeaseRecovery_withUpgrade40() throws Exception {
        LOG.info("blockLengthsSuite: " + Arrays.toString(blockLengthsSuite));
        for (int i = 0; i < blockLengthsSuite.length; i++) {
            BlockLengths blockLengths = blockLengthsSuite[i];
            try {
    ...
    @Test
    public void testLeaseRecovery_withUpgrade80() throws Exception {
        LOG.info("blockLengthsSuite: " + Arrays.toString(blockLengthsSuite));
        for (int i = 0; i < blockLengthsSuite.length; i++) {
            BlockLengths blockLengths = blockLengthsSuite[i];
            try {
    ...
    @Test
    public void testSafeLength_withUpgrade20() {
        // Length of: 0
        checkSafeLength(0, 0);
        // Length of: 1 MiB
        checkSafeLength(1024 * 1024, 6291456L);
    ...
    @Test
    public void testSafeLength_withUpgrade40() {
        // Length of: 0
        checkSafeLength(0, 0);
        // Length of: 1 MiB
        checkSafeLength(1024 * 1024, 6291456L);
    ...
    @Test
    public void testSafeLength_withUpgrade60() {
        // Length of: 0
        checkSafeLength(0, 0);
        // Length of: 1 MiB
        checkSafeLength(1024 * 1024, 6291456L);
    ...
    @Test
    public void testSafeLength_withUpgrade80() {
        // Length of: 0
        checkSafeLength(0, 0);
        // Length of: 1 MiB
        checkSafeLength(1024 * 1024, 6291456L);
