...
public class TestFileTruncate_RestartInjected_Randomized_123 {
    ...
    public void testBasicTruncate() throws IOException {
        RestartFramework.at("after_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (int fileLength = startingFileSize; fileLength > 0; fileLength -= BLOCK_SIZE - 1) {
            if (fileLength == startingFileSize - BLOCK_SIZE + 1) {
                RestartFramework.at("during_truncate_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
    ...
    public void testMultipleTruncate() throws IOException {
        ...
        for (int n = data.length; n > 0; ) {
            if (loopCount == 5) {
                RestartFramework.at("during_multiple_truncate").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            loopCount++;
    ...
    public void testTruncateEditLogLoad() throws IOException {
        ...
        checkFullFile(p, newLength, contents);
        RestartFramework.at("before_namenode_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.delete(p, false);
    }
    ...
    static void assertBlockExists(Block blk) {
        assertNotNull("BlocksMap does not contain block: " + blk, cluster.getNamesystem().getStoredBlock(blk));
    }

    static void assertBlockNotPresent(Block blk) {
        assertNull("BlocksMap should not contain block: " + blk, cluster.getNamesystem().getStoredBlock(blk));
    }
    ...
    static void restartCluster(StartupOption o) throws IOException {
        ...
        cluster = new MiniDFSCluster.Builder(conf).numDataNodes(DATANODE_NUM).format(false).startupOption(o == StartupOption.ROLLBACK ? StartupOption.REGULAR : o).dnStartupOption(o != StartupOption.ROLLBACK ? StartupOption.REGULAR : o).build();
        fs = cluster.getFileSystem();
    }
