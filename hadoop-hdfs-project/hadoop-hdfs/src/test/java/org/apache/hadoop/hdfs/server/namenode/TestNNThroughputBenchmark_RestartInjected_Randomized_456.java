...
public class TestNNThroughputBenchmark_RestartInjected_Randomized_456 {
    ...
    public void testNNThroughputAgainstRemoteNN() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_benchmark").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    public void testNNThroughputForBlockReportOp() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(3).build()) {
            ...
            RestartFramework.at("after_block_report_benchmark").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
