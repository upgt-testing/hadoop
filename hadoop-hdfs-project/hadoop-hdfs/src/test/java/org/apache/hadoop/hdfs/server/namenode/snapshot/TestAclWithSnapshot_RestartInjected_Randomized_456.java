 ...
 */
public class TestAclWithSnapshot_RestartInjected_Randomized_456 {
    ...
    public void testOriginalAclEnforcedForSnapshotRootAfterChange() throws Exception {
        ...
        RestartFramework.at("after_acl_modification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        returned = s.getEntries().toArray(new AclEntry[0]);
        RestartFramework.at("after_snapshot_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertArrayEquals(new AclEntry[] { aclEntry(ACCESS, USER, "bruce", READ_EXECUTE), aclEntry(ACCESS, GROUP, NONE) }, returned);
        RestartFramework.at("after_set_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testOriginalAclEnforcedForSnapshotContentsAfterChange() throws Exception {
        RestartFramework.at("after_subdir_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_snapshot_creation_contents").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_acl_modification_contents").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        returned = s.getEntries().toArray(new AclEntry[0]);
        ...
        hdfs.setAcl(filePath, aclSpec);
        RestartFramework.at("after_set_acl_on_contents").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.setAcl(subdirPath, aclSpec);
        // Original has changed, but snapshot still has old ACL.
    ...
    public void testOriginalAclEnforcedForSnapshotRootAfterRemoval() throws Exception {
        ...
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, ALL), aclEntry(ACCESS, USER, "bruce", READ_EXECUTE), aclEntry(ACCESS, GROUP, NONE), aclEntry(ACCESS, OTHER, NONE));
        hdfs.setAcl(path, aclSpec);
        RestartFramework.at("after_set_acl_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertDirPermissionGranted(fsAsBruce, BRUCE, path);
        assertDirPermissionDenied(fsAsDiana, DIANA, path);
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        // Both original and snapshot still have same ACL.
        ...
        returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(new AclEntry[] { aclEntry(ACCESS, USER, "bruce", READ_EXECUTE), aclEntry(ACCESS, GROUP, NONE) }, returned);
        RestartFramework.at("after_snapshot_creation_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertPermission((short) 010750, snapshotPath);
        RestartFramework.at("after_remove_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertDirPermissionGranted(fsAsBruce, BRUCE, snapshotPath);
        assertDirPermissionDenied(fsAsDiana, DIANA, snapshotPath);
        hdfs.removeAcl(path);
        // Original has changed, but snapshot still has old ACL.
        ...
        restart(false);
        RestartFramework.at("after_mkdir_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        doSnapshotRootRemovalAssertions(path, snapshotPath);
        restart(true);
        doSnapshotRootRemovalAssertions(path, snapshotPath);
    }
    ...
    public void testOriginalAclEnforcedForSnapshotContentsAfterRemoval() throws Exception {
        RestartFramework.at("after_file_creation_removal").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_subdir_creation_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path filePath = new Path(path, "file1");
        Path subdirPath = new Path(path, "subdir1");
        Path fileSnapshotPath = new Path(snapshotPath, "file1");
        Path subdirSnapshotPath = new Path(snapshotPath, "subdir1");
        ...
        assertFilePermissionGranted(fsAsBruce, BRUCE, filePath);
        assertFilePermissionDenied(fsAsDiana, DIANA, filePath);
        RestartFramework.at("after_snapshot_contents_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertDirPermissionGranted(fsAsBruce, BRUCE, subdirPath);
        assertDirPermissionDenied(fsAsDiana, DIANA, subdirPath);
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        // Both original and snapshot still have same ACL.
        ...
        s = hdfs.getAclStatus(subdirPath);
        returned = s.getEntries().toArray(new AclEntry[0]);
        RestartFramework.at("after_remove_acl_contents").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertArrayEquals(expected, returned);
        assertPermission((short) 010550, subdirPath);
        RestartFramework.at("after_set_acl_contents_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        s = hdfs.getAclStatus(fileSnapshotPath);
        returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(expected, returned);
        assertPermission((short) 010550, fileSnapshotPath);
    ...
    public void testModifyReadsCurrentState() throws Exception {
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        RestartFramework.at("after_mkdir_modify_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        ...
        hdfs.modifyAclEntries(path, aclSpec);
        RestartFramework.at("after_snapshot_modify_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_modify_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        AclStatus s = hdfs.getAclStatus(path);
        AclEntry[] returned = s.getEntries().toArray(new AclEntry[0]);
        RestartFramework.at("after_first_modify_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertArrayEquals(expected, returned);
    ...
    public void testRemoveReadsCurrentState() throws Exception {
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        RestartFramework.at("after_snapshot_remove_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "bruce", ALL));
        hdfs.modifyAclEntries(path, aclSpec);
        RestartFramework.at("after_modify_acl_remove_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.removeAcl(path);
        AclEntry[] expected = new AclEntry[] {};
        RestartFramework.at("after_mkdir_remove_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        AclStatus s = hdfs.getAclStatus(path);
        AclEntry[] returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(expected, returned);
        assertPermission((short) 0700, path);
        RestartFramework.at("after_remove_acl_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertDirPermissionDenied(fsAsBruce, BRUCE, path);
        assertDirPermissionDenied(fsAsDiana, DIANA, path);
    }
    ...
    public void testDefaultAclNotCopiedToAccessAclOfNewSnapshot() throws Exception {
        ...
        hdfs.modifyAclEntries(path, aclSpec);
        RestartFramework.at("after_snapshot_default_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        AclStatus s = hdfs.getAclStatus(path);
        AclEntry[] returned = s.getEntries().toArray(new AclEntry[0]);
        ...
        RestartFramework.at("after_modify_default_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        s = hdfs.getAclStatus(snapshotPath);
        returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(new AclEntry[] { aclEntry(DEFAULT, USER, ALL), aclEntry(DEFAULT, USER, "bruce", READ_EXECUTE), aclEntry(DEFAULT, GROUP, NONE), aclEntry(DEFAULT, MASK, READ_EXECUTE), aclEntry(DEFAULT, OTHER, NONE) }, returned);
        assertPermission((short) 010700, snapshotPath);
        RestartFramework.at("after_mkdir_default_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testModifyAclEntriesSnapshotPath() throws Exception {
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        RestartFramework.at("after_snapshot_modify_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(DEFAULT, USER, "bruce", READ_EXECUTE));
        exception.expect(SnapshotAccessControlException.class);
        RestartFramework.at("after_mkdir_modify_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRemoveAclEntriesSnapshotPath() throws Exception {
        ...
        exception.expect(SnapshotAccessControlException.class);
        RestartFramework.at("after_mkdir_remove_entries_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_snapshot_remove_entries_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRemoveDefaultAclSnapshotPath() throws Exception {
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        RestartFramework.at("after_mkdir_remove_default_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        exception.expect(SnapshotAccessControlException.class);
        RestartFramework.at("after_snapshot_remove_default_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.removeDefaultAcl(snapshotPath);
    }

    @Test
    public void testRemoveAclSnapshotPath() throws Exception {
        RestartFramework.at("after_snapshot_remove_acl_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        RestartFramework.at("after_mkdir_remove_acl_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        exception.expect(SnapshotAccessControlException.class);
    ...
    public void testSetAclSnapshotPath() throws Exception {
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        RestartFramework.at("after_snapshot_set_acl_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_set_acl_snapshot_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(DEFAULT, USER, "bruce"));
        exception.expect(SnapshotAccessControlException.class);
    ...
    public void testChangeAclExceedsQuota() throws Exception {
        RestartFramework.at("after_modify_acl_quota").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_permission_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path filePath = new Path(path, "file1");
        Path fileSnapshotPath = new Path(snapshotPath, "file1");
        ...
        RestartFramework.at("after_quota_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_snapshot_quota").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        s = hdfs.getAclStatus(fileSnapshotPath);
        returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(new AclEntry[] { aclEntry(ACCESS, USER, "bruce", READ_WRITE), aclEntry(ACCESS, GROUP, NONE) }, returned);
        assertPermission((short) 010660, filePath);
    ...
    public void testRemoveAclExceedsQuota() throws Exception {
        ...
        FileSystem.create(hdfs, filePath, FsPermission.createImmutable((short) 0600)).close();
        hdfs.setPermission(filePath, FsPermission.createImmutable((short) 0600));
        RestartFramework.at("after_create_snapshot_quota_remove").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "bruce", READ_WRITE));
        hdfs.modifyAclEntries(filePath, aclSpec);
        hdfs.createSnapshot(path, snapshotName);
        AclStatus s = hdfs.getAclStatus(filePath);
        ...
        assertArrayEquals(new AclEntry[] { aclEntry(ACCESS, USER, "bruce", READ_WRITE), aclEntry(ACCESS, GROUP, NONE) }, returned);
        assertPermission((short) 010660, filePath);
        RestartFramework.at("after_modify_acl_quota_remove").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "bruce", READ));
        RestartFramework.at("after_quota_setup_remove").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_permission_setup_remove").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.removeAcl(filePath);
    }
    ...
    public void testGetAclStatusDotSnapshotPath() throws Exception {
        ...
        RestartFramework.at("after_mkdir_get_acl_status").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_snapshot_get_acl_status").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        AclEntry[] returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(new AclEntry[] {}, returned);
    }
    ...
    public void testDeDuplication() throws Exception {
        RestartFramework.at("after_subdir_creation_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        hdfs.mkdirs(path);
        hdfs.modifyAclEntries(path, aclSpec);
        RestartFramework.at("after_initial_acl_on_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        AclFeature aclFeature;
        {
            ...
            RestartFramework.at("after_first_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        }
        {
            ...
            RestartFramework.at("after_first_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        {
            hdfs.modifyAclEntries(subdir, aclSpec);
            RestartFramework.at("after_acl_on_subdir_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            aclFeature = FSAclBaseTest.getAclFeature(subdir, cluster);
            assertEquals("Reference count should be 1", 1, aclFeature.getRefCount());
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            RestartFramework.at("after_second_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        {
            hdfs.modifyAclEntries(file, aclSpec);
            RestartFramework.at("after_acl_on_file_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            aclFeature = FSAclBaseTest.getAclFeature(file, cluster);
            assertEquals("Reference count should be 1", 1, aclFeature.getRefCount());
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            RestartFramework.at("after_third_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_third_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        RestartFramework.at("after_file_creation_dedup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        {
            ...
            hdfs.modifyAclEntries(path, aclSpec);
            RestartFramework.at("after_second_acl_on_path_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            RestartFramework.at("after_fourth_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_modify_path_with_new_user_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            aclFeature = FSAclBaseTest.getAclFeature(path, cluster);
            ...
            deleteSnapshotWithAclAndVerify(aclFeature, path, startSize);
            RestartFramework.at("after_fourth_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        {
            ...
            hdfs.modifyAclEntries(subdir, aclSpec);
            RestartFramework.at("after_second_acl_on_subdir_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            RestartFramework.at("after_fifth_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path subdirInSnapshot = new Path(snapshotPath, "sub-dir");
            AclFeature snapshotAclFeature = FSAclBaseTest.getAclFeature(subdirInSnapshot, cluster);
            List<AclEntry> newAcl = Lists.newArrayList(aclEntry(ACCESS, USER, "testNewUser", ALL));
            hdfs.modifyAclEntries(subdir, newAcl);
            RestartFramework.at("after_modify_subdir_with_new_user_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            aclFeature = FSAclBaseTest.getAclFeature(subdir, cluster);
            ...
            deleteSnapshotWithAclAndVerify(aclFeature, subdir, startSize);
            RestartFramework.at("after_fifth_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        {
            // Modifying the ACLs of file inside the snapshot root should refer new
            // AclFeature. And old AclFeature should be referenced by snapshot
            hdfs.modifyAclEntries(file, aclSpec);
            RestartFramework.at("after_second_acl_on_file_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            RestartFramework.at("after_sixth_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path fileInSnapshot = new Path(snapshotPath, file.getName());
            AclFeature snapshotAclFeature = FSAclBaseTest.getAclFeature(fileInSnapshot, cluster);
            List<AclEntry> newAcl = Lists.newArrayList(aclEntry(ACCESS, USER, "testNewUser", ALL));
            hdfs.modifyAclEntries(file, newAcl);
            RestartFramework.at("after_modify_file_with_new_user_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            aclFeature = FSAclBaseTest.getAclFeature(file, cluster);
            assertNotSame(aclFeature, snapshotAclFeature);
            assertEquals("Reference count should remain same", 1, snapshotAclFeature.getRefCount());
            deleteSnapshotWithAclAndVerify(aclFeature, file, startSize);
            RestartFramework.at("after_sixth_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        {
            // deleting the original directory containing dirs and files with ACLs
            // with snapshot
            hdfs.delete(path, true);
            RestartFramework.at("after_delete_path_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path dir = new Path(subdir, "dir");
            hdfs.mkdirs(dir);
            RestartFramework.at("after_create_nested_dir_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            hdfs.modifyAclEntries(dir, aclSpec);
            RestartFramework.at("after_acl_on_nested_dir_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            file = new Path(subdir, "file");
            hdfs.create(file).close();
            RestartFramework.at("after_create_nested_file_dedup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            aclSpec.add(aclEntry(ACCESS, USER, "testNewUser", ALL));
            hdfs.modifyAclEntries(file, aclSpec);
            RestartFramework.at("after_acl_on_nested_file_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            AclFeature fileAcl = FSAclBaseTest.getAclFeature(file, cluster);
            AclFeature dirAcl = FSAclBaseTest.getAclFeature(dir, cluster);
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            RestartFramework.at("after_seventh_snapshot_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_delete_subdir_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_seventh_snapshot_deletion_dedup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
