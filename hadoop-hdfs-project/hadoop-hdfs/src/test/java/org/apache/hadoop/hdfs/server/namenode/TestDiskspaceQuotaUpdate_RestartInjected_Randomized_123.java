...
public class TestDiskspaceQuotaUpdate_RestartInjected_Randomized_123 {
    ...
    public void testUpdateQuotaForAppend() throws Exception {
        ...
        getDFS().setQuota(foo, Long.MAX_VALUE - 1, Long.MAX_VALUE - 1);
        RestartFramework.at("after_second_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_first_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_third_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        quota = fooNode.getDirectoryWithQuotaFeature().getSpaceConsumed();
        ns = quota.getNameSpace();
        ds = quota.getStorageSpace();
        // foo and bar
    ...
    public void testUpdateQuotaForFSync() throws Exception {
        ...
        RestartFramework.at("after_append_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ((DFSOutputStream) out.getWrappedStream()).hsync(EnumSet.of(HdfsDataOutputStream.SyncFlag.UPDATE_LENGTH));
        INodeDirectory fooNode = getFSDirectory().getINode4Write(foo.toString()).asDirectory();
        QuotaCounts quota = fooNode.getDirectoryWithQuotaFeature().getSpaceConsumed();
        RestartFramework.at("after_close").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        long ns = quota.getNameSpace();
        long ds = quota.getStorageSpace();
        // foo and bar
        assertEquals(2, ns);
        ...
        RestartFramework.at("after_final_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.write(new byte[BLOCKSIZE / 4]);
        out.close();
    ...
    public void testAppendOverStorageQuota() throws Exception {
        ...
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_failed_append").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAppendOverTypeQuota() throws Exception {
        ...
        final INodeDirectory dirNode = getFSDirectory().getINode4Write(dir.toString()).asDirectory();
        RestartFramework.at("after_set_quota_by_storage_type").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final long spaceUsed = dirNode.getDirectoryWithQuotaFeature().getSpaceConsumed().getStorageSpace();
        try {
            DFSTestUtil.appendFile(getDFS(), file, BLOCKSIZE);
            Assert.fail("append didn't fail");
    ...
    public void testTruncateOverQuota() throws Exception {
        RestartFramework.at("after_failed_truncate").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path dir = getParent(GenericTestUtils.getMethodName());
        final Path file = new Path(dir, "file");
        // create partial block file
        getDFS().mkdirs(dir);
    ...
    public void testQuotaInitialization() throws Exception {
        ...
        RestartFramework.at("after_creating_subdirectories").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_testdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_update_count_4").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_update_count_1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Directly access the name system to obtain the current cached usage.
        INodeDirectory root = getFSDirectory().getRoot();
        RestartFramework.at("after_set_quota_testdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_update_count_2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        scanDirsWithQuota(root, nsMap, dsMap, true);
    ...
    public void testQuotaIssuesWhileCommitting() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_stopping_datanodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_helper_call").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            testQuotaIssuesWhileCommittingHelper(nnSpy, (short) 4, (short) 1);
            RestartFramework.at("after_second_helper_call").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("before_restarting_datanodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
