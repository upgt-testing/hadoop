...
public class TestHSync_RestartInjected_Randomized_123 {
    ...
    private void testHSyncOperation(boolean testWithAppend) throws IOException {
        ...
        out.hflush();
        RestartFramework.at("after_first_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testHSyncBlockBoundary() throws Exception {
        ...
        out.write(1);
        RestartFramework.at("after_block_write_hflush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.hsync();
        checkSyncMetric(cluster, 2);
        RestartFramework.at("after_second_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSequenceFileSync() throws Exception {
        Configuration conf = new HdfsConfiguration();
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).build();
        RestartFramework.at("after_writer_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final FileSystem fs = cluster.getFileSystem();
        final Path p = new Path("/testSequenceFileSync/foo");
        RestartFramework.at("after_append_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final int len = 1 << 16;
        FSDataOutputStream out = fs.create(p, FsPermission.getDefault(), EnumSet.of(CreateFlag.CREATE, CreateFlag.OVERWRITE, CreateFlag.SYNC_BLOCK), 4096, (short) 1, len, null);
    ...
    public void testHSyncWithReplication() throws Exception {
        ...
        RestartFramework.at("after_write_hflush").on(cluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        checkSyncMetric(cluster, 2, 2);
        RestartFramework.at("after_first_hsync").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.shutdown();
    }
}
