...
public class TestHASafeMode {
    ...
    public void setupCluster() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(3).waitSafeMode(false).build();
        cluster.waitActive();
    ...
    public void testEnterSafeModeInANNShouldNotThrowNPE() throws Exception {
        ...
        FSNamesystemInterface namesystem = nn0.getNamesystem();
    ...
    public void testEnterSafeModeInSBNShouldNotThrowNPE() throws Exception {
        ...
        restartStandby();
        FSNamesystemInterface namesystem = nn1.getNamesystem();
        String status = namesystem.getSafemode();
        assertTrue("Bad safemode status: '" + status + "'", status.startsWith("Safe mode is ON."));
    ...
    private static void assertSafeMode(NameNodeInterface nn, int safe, int total, int numNodes, int nodeThresh) {
        String status = nn.getNamesystem().getSafemode();
        if (total == 0 && nodeThresh == 0) {
            assertTrue("Bad safemode status: '" + status + "'", status.isEmpty() || status.startsWith("Safe mode is ON. The reported blocks 0 " + "has reached the threshold 0.9990 of total blocks 0. The " + "minimum number of live datanodes is not required. In safe " + "mode extension. Safe mode will be turned off automatically " + "in 0 seconds."));
        } else if (safe == total) {
    ...
    public void testTransitionToActiveWhenSafeMode() throws Exception {
        ...
        try (MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(config, new File(GenericTestUtils.getRandomizedTempPath())).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).build()) {
            miniCluster.waitActive();
            miniCluster.transitionToStandby(0);
            miniCluster.transitionToStandby(1);
            NameNodeInterface namenode0 = miniCluster.getNameNode(0);
            NameNodeInterface namenode1 = miniCluster.getNameNode(1);
