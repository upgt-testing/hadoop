 ...
 */
public class TestAclConfigFlag_RestartInjected_Randomized_456 {
    ...
    public void testEditLog() throws Exception {
        ...
        RestartFramework.at("after_restart_acls_disabled").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.mkdirs(PATH);
        fs.setAcl(PATH, Lists.newArrayList(aclEntry(DEFAULT, USER, "foo", READ_WRITE)));
    ...
    public void testFsImage() throws Exception {
        ...
        restart(true, true);
        RestartFramework.at("after_checkpoint_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Restart with ACLs disabled.  Expect successful restart.
        restart(false, false);
    }
