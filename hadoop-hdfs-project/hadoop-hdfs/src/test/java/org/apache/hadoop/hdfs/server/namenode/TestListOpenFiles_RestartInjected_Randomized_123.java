 ...
 */
public class TestListOpenFiles_RestartInjected_Randomized_123 {
    ...
    public void testListOpenFilesViaNameNodeRPC() throws Exception {
        RestartFramework.at("after_open_files_batch2").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_open_files_batch1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue("Open files list blocking decommission should be empty!", openFilesBlockingDecomEntries.size() == 0);
        RestartFramework.at("after_closed_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        openFiles.putAll(DFSTestUtil.createOpenFiles(fs, "open-1", 1));
        RestartFramework.at("after_open_files_batch3").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyOpenFiles(openFiles);
        ...
        RestartFramework.at("after_partial_close").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyOpenFiles(openFiles);
    ...
    private void verifyOpenFiles(Map<Path, FSDataOutputStream> openFiles, EnumSet<OpenFilesType> openFilesTypes, String path) throws IOException {
        ...
        do {
            if (lastEntry == null) {
                ...
                batchedEntries = nnRpc.listOpenFiles(lastEntry.getId(), openFilesTypes, path);
            }
            assertTrue("Incorrect open files list size!", batchedEntries.size() <= BATCH_SIZE);
            for (int i = 0; i < batchedEntries.size(); i++) {
                ...
                assertTrue("Unexpected open file: " + filePath, remainingFiles.remove(new Path(filePath)));
            }
        } while (batchedEntries.hasMore());
        assertTrue(remainingFiles.size() + " open files not listed!", remainingFiles.size() == 0);
    }
    ...
    private void verifyOpenFiles(Map<Path, FSDataOutputStream> openFiles, String path) throws IOException {
        ...
        verifyOpenFiles(new HashMap<>(), EnumSet.of(OpenFilesType.BLOCKING_DECOMMISSION), path);
    }
    ...
    public void testListOpenFilesInHA() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_ha_cluster_start").on(haCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final byte[] data = new byte[1024];
            ThreadLocalRandom.current().nextBytes(data);
            DFSTestUtil.createOpenFiles(fileSystem, "ha-open-file", ((BATCH_SIZE * 4) + (BATCH_SIZE / 2)));
            RestartFramework.at("after_open_files_creation").on(haCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListOpenFilesWithFilterPath() throws IOException {
        ...
        BatchedEntries<OpenFileEntry> openFilesBlockingDecomEntries = nnRpc.listOpenFiles(0, EnumSet.of(OpenFilesType.BLOCKING_DECOMMISSION), OpenFilesIterator.FILTER_PATH_DEFAULT);
        assertTrue("Open files list blocking decommission should be empty!", openFilesBlockingDecomEntries.size() == 0);
        RestartFramework.at("after_second_batch_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        openFiles.putAll(DFSTestUtil.createOpenFiles(fs, new Path("/base"), "open-1", 1));
        RestartFramework.at("after_first_batch_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Map<Path, FSDataOutputStream> baseOpen = DFSTestUtil.createOpenFiles(fs, new Path("/base-open"), "open-1", 1);
        RestartFramework.at("after_cluster_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListOpenFilesWithInvalidPathServerSide() throws Exception {
        RestartFramework.at("after_cluster_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HashMap<Path, FSDataOutputStream> openFiles = new HashMap<>();
        RestartFramework.at("after_open_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        openFiles.putAll(DFSTestUtil.createOpenFiles(fs, new Path("/base"), "open-1", 1));
        verifyOpenFiles(openFiles, EnumSet.of(OpenFilesType.ALL_OPEN_FILES), "/base");
        intercept(AssertionError.class, "Absolute path required", "Expect InvalidPathException", () -> verifyOpenFiles(new HashMap<>(), EnumSet.of(OpenFilesType.ALL_OPEN_FILES), "hdfs://cluster/base"));
        while (openFiles.size() > 0) {
            DFSTestUtil.closeOpenFiles(openFiles, 1);
            verifyOpenFiles(openFiles);
        }
    ...
    public void testListOpenFilesWithInvalidPathClientSide() throws Exception {
        intercept(IllegalArgumentException.class, "Wrong FS", "Expect IllegalArgumentException", () -> fs.listOpenFiles(EnumSet.of(OpenFilesType.ALL_OPEN_FILES), "hdfs://non-cluster/"));
        RestartFramework.at("after_cluster_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListOpenFilesWithDeletedPath() throws Exception {
        RestartFramework.at("after_cluster_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HashMap<Path, FSDataOutputStream> openFiles = new HashMap<>();
        openFiles.putAll(DFSTestUtil.createOpenFiles(fs, new Path("/"), "open-1", 1));
        BatchedEntries<OpenFileEntry> openFileEntryBatchedEntries = nnRpc.listOpenFiles(0, EnumSet.of(OpenFilesType.ALL_OPEN_FILES), OpenFilesIterator.FILTER_PATH_DEFAULT);
