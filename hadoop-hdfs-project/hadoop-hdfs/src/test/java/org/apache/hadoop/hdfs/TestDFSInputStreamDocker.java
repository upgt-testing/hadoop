...
public class TestDFSInputStreamDocker {
    private void testSkipInner(DockerHDFSCluster cluster) throws IOException {  // <--- Just simply change MiniDFSCluster to DockerHDFSCluster
    ...
    public void testSkipWithRemoteBlockReader() throws IOException {
        ...
        DockerHDFSCluster cluster = new DockerHDFSCluster.Builder(conf).build();   // <--- Change MiniDFSCluster to DockerHDFSCluster
        try {
    ...
    public void testSeekToNewSource() throws IOException {
        ...
        DockerHDFSCluster cluster = new DockerHDFSCluster.Builder(conf).numDataNodes(3).build(); // <-- Change MiniDFSCluster to DockerHDFSCluster
    ...
    public void testNullCheckSumWhenDNRestarted()
        ...
        DockerHDFSCluster cluster = new DockerHDFSCluster.Builder(conf).numDataNodes(2).build(); // <-- Change MiniDFSCluster to DockerHDFSCluster
        //cluster.waitActive();
        try {
            DistributedFileSystem fs = cluster.getDistributedFileSystem();
            ...
            try (FSDataOutputStream fout = fs.create(file)) {
                ...
                cluster.upgradeDataNode("hadoop:3.3.5", 0);
            }
    ...
    public void testCreateBlockReaderWhenInvalidBlockTokenException() throws
        ...
        try (DockerHDFSCluster cluster = new DockerHDFSCluster.Builder(conf).numDataNodes(3).build()) {  // <-- Change MiniDFSCluster to DockerHDFSCluster
            //cluster.waitActive();
            DistributedFileSystem fs = cluster.getFileSystem();
