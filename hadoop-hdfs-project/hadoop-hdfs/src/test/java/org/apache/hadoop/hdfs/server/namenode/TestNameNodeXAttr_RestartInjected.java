 ...
 */
public class TestNameNodeXAttr_RestartInjected extends FSXAttrBaseTest {
  ...
  public void testXAttrSymlinks() throws Exception {
    ...
    RestartFramework.at("after_first_xattr_set")
        .on(dfsCluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_xattr_get")
        .on(dfsCluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_xattr_set_via_link")
        .on(dfsCluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    Assert.assertArrayEquals(value2, xattrs.get(name2));
    Assert.assertArrayEquals(new byte[0], xattrs.get(name3));
    RestartFramework.at("after_first_xattr_remove")
        .on(dfsCluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    Assert.assertArrayEquals(value2, xattrs.get(name2));
    RestartFramework.at("before_cleanup")
        .on(dfsCluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
