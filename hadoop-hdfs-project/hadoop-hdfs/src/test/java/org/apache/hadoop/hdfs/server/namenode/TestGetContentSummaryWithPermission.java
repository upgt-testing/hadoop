...
public class TestGetContentSummaryWithPermission {
    ...
    public void testGetContentSummaryNonSuperUser() throws Exception {
        ...
        ContentSummaryJVMInterface summary = userUgi.doAs((PrivilegedExceptionAction<ContentSummaryJVMInterface>) () -> cluster.getNameNodeRpc().getContentSummary(foo.toString()));
        verifySummary(summary, 2, 1, 10);
        // set empty access on root dir, should disallow content summary
        dfs.setPermission(foo, new FsPermission((short) 0));
        try {
            userUgi.doAs((PrivilegedExceptionAction<ContentSummaryJVMInterface>) () -> cluster.getNameNodeRpc().getContentSummary(foo.toString()));
        ...
        summary = userUgi.doAs((PrivilegedExceptionAction<ContentSummaryJVMInterface>) () -> cluster.getNameNodeRpc().getContentSummary(foo.toString()));
        verifySummary(summary, 2, 1, 10);
