...
public class TestUnderReplicatedBlocks {
    ...
    public void testSetRepIncWithUnderReplicatedBlocks() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(REPLICATION_FACTOR + 1).build();
        try {
    ...
    public void testNumberOfBlocksToBeReplicated() throws Exception {
        ...
        final String FILE_NAME = "/testFile";
        final Path FILE_PATH = new Path(FILE_NAME);
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(REP_FACTOR).build();
        try {
