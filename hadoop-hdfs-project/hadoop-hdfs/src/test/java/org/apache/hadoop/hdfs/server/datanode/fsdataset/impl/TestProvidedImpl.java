...
public class TestProvidedImpl {
    ...
    private static Storage.StorageDirectory createProvidedStorageDirectory(String confString, Configuration conf) throws SecurityException, IOException {
        Storage.StorageDirectory sd = new Storage.StorageDirectory(StorageLocation.parse(confString));
        DataStorage.createStorageID(sd, false, conf);
        return sd;
    }
    ...
    public void setUp() throws IOException {
        ...
        this.conf.setClass(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_CLASS, TestFileRegionBlockAliasMap.class, BlockAliasMap.class);
        blkToPathMap = new HashMap<Long, String>();
        providedVolumes = new LinkedList<FsVolumeImpl>();
        createStorageDirs(storage, conf, NUM_LOCAL_INIT_VOLUMES, NUM_PROVIDED_INIT_VOLUMES);
    ...
    public void testProvidedBlockRead() throws IOException {
        for (int id = 0; id < NUM_PROVIDED_BLKS; id++) {
            ExtendedBlock eb = new ExtendedBlock(BLOCK_POOL_IDS[CHOSEN_BP_ID], id, BLK_LEN, HdfsConstants.GRANDFATHER_GENERATION_STAMP);
            InputStream ins = dataset.getBlockInputStream(eb, 0);
            String filepath = blkToPathMap.get((long) id);
            TestProvidedReplicaImpl.verifyReplicaContents(new File(filepath), ins, 0, BLK_LEN);
        }
    }
    ...
    private int getBlocksInProvidedVolumes(String basePath, int numBlocks, int minBlockId) throws IOException {
        ...
        for (int i = 0; i < providedVolumes.size(); i++) {
            ...
            vol.setFileRegionProvider(BLOCK_POOL_IDS[CHOSEN_BP_ID], new TestFileRegionBlockAliasMap(fileRegionIterator, minBlockId, numBlocks));
            ReplicaMap volumeMap = new ReplicaMap(new AutoCloseableLock());
    ...
    public void testProvidedVolumeContents() throws IOException {
        ...
        int blocksFound = getBlocksInProvidedVolumes(providedBasePath + "/test1/", expectedBlocks, minId);
        assertEquals("Number of blocks in provided volumes should be " + expectedBlocks, expectedBlocks, blocksFound);
        blocksFound = getBlocksInProvidedVolumes("file:/" + providedBasePath + "/test1/", expectedBlocks, minId);
        assertEquals("Number of blocks in provided volumes should be " + expectedBlocks, expectedBlocks, blocksFound);
        ...
        assertEquals("Number of blocks in provided volumes should be 0", 0, blocksFound);
    }
    ...
    @Test
    public void testProvidedReplicaWithPathHandle_withUpgrade20() throws Exception {
        Configuration conf = new Configuration();
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
        cluster.waitActive();
        DistributedFileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testProvidedReplicaWithPathHandle_withUpgrade40() throws Exception {
        Configuration conf = new Configuration();
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
        cluster.waitActive();
        DistributedFileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testProvidedReplicaWithPathHandle_withUpgrade60() throws Exception {
        Configuration conf = new Configuration();
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
        cluster.waitActive();
        DistributedFileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testProvidedReplicaWithPathHandle_withUpgrade80() throws Exception {
        Configuration conf = new Configuration();
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
        cluster.waitActive();
        DistributedFileSystem fs = cluster.getFileSystem();
