...
public class TestMultiThreadedHflush {
    ...
    private final SampleQuantiles quantiles = new SampleQuantiles(new Quantile[] { new Quantile(0.50, 0.050), new Quantile(0.75, 0.025), new Quantile(0.90, 0.010), new Quantile(0.95, 0.005), new Quantile(0.99, 0.001) });
   ...
   */
    ...
    private void doTestMultipleHflushers(int repl) throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(repl).build();
        FileSystem fs = cluster.getFileSystem();
        Path p = new Path("/multiple-hflushers.dat");
        try {
            doMultithreadedWrites(cluster, conf, p, NUM_THREADS, WRITE_SIZE, NUM_WRITES_PER_THREAD, repl);
    ...
    public void doMultithreadedWrites(MiniDockerDFSCluster cluster, Configuration conf, Path p, int numThreads, int bufferSize, int numWrites, int replication) throws Exception {
    ...
    private static class CLIBenchmark extends Configured implements Tool {
        ...
        public int run(String[] args) throws Exception {
            ...
            StopWatch sw = new StopWatch().start();
            test.doMultithreadedWrites(null, conf, p, numThreads, writeSize, numWrites, replication);
            sw.stop();
