 ...
 */
public class TestParallelImageWrite_RestartInjected_Randomized_456 {
    ...
    public void testRestartDFS() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_cluster_restart_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
            ...
            RestartFramework.at("after_first_check_images").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
            // Modify the system and then perform saveNamespace
            files.cleanup(fs, dir);
            files.createFiles(fs, dir);
            RestartFramework.at("after_second_files_created").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
