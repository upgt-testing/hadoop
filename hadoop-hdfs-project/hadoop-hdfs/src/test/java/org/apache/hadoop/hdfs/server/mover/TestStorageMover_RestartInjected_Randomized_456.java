 ...
 */
public class TestStorageMover_RestartInjected_Randomized_456 {
    ...
    static {
        ...
        DEFAULT_CONF.setLong(DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_INTERVAL_SECONDS_KEY, 2L);
        DEFAULT_CONF.setLong(DFSConfigKeys.DFS_MOVER_MOVEDWINWIDTH_KEY, 2000L);
        DEFAULT_CONF.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_DEFAULT);
    ...
    static class ClusterScheme {
        ...
        ClusterScheme() {
            this(DEFAULT_CONF, NUM_DATANODES, REPL, genStorageTypes(NUM_DATANODES), null);
        }
    ...
    public void testMigrateFileToArchival() throws Exception {
        ...
        test.setupCluster();
        try {
            RestartFramework.at("after_cluster_setup").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.prepareNamespace();
            RestartFramework.at("after_namespace_prepare").on(test.cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.verify(true);
            test.setStoragePolicy();
            RestartFramework.at("after_set_storage_policy").on(test.cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.migrate(ExitStatus.SUCCESS);
            RestartFramework.at("after_migration").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.verify(true);
        } finally {
    ...
    public void testMoveSpecificPaths() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_mover_run").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(5000);
            test.verify(true);
        } finally {
            test.shutdownCluster();
    ...
    public void testMigrateOpenFileToArchival() throws Exception {
        ...
        RestartFramework.at("after_append_hsync").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.writeBytes("hello, ");
        RestartFramework.at("after_file_creation").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            ((DFSOutputStream) out.getWrappedStream()).hsync();
            RestartFramework.at("after_continue_write_hsync").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testHotWarmColdDirs() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_set_storage_policy").on(test.cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.migrate(ExitStatus.SUCCESS);
            RestartFramework.at("after_initial_migration").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.verify(true);
            pathPolicyMap.moveAround(test.dfs);
            RestartFramework.at("after_move_around").on(test.cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.migrate(ExitStatus.SUCCESS);
            RestartFramework.at("after_second_migration").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.verify(true);
        } finally {
            test.shutdownCluster();
        }
    ...
    public void testNoSpaceDisk() throws Exception {
        ...
        try {
            ...
            test.verify(true);
            test.setStoragePolicy();
            RestartFramework.at("after_initial_setup").on(test.cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test.migrate(ExitStatus.SUCCESS);
            test.verify(true);
            ...
            RestartFramework.at("after_hot_files_creation").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // set all the DISK volume to full
            for (DataNode dn : test.cluster.getDataNodes()) {
                setVolumeFull(dn, StorageType.DISK);
                DataNodeTestUtils.triggerHeartbeat(dn);
            }
            RestartFramework.at("after_disk_full").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_increase_replication").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cold_file_replication").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //test move a hot file to warm
            final Path file1 = new Path(pathPolicyMap.hot, "file1");
            test.dfs.rename(file1, pathPolicyMap.warm);
            RestartFramework.at("after_rename").on(test.cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNoSpaceArchive() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_cold_files_creation").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // set all the ARCHIVE volume to full
            for (DataNode dn : test.cluster.getDataNodes()) {
                setVolumeFull(dn, StorageType.ARCHIVE);
                DataNodeTestUtils.triggerHeartbeat(dn);
            }
            RestartFramework.at("after_archive_full").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            {
                ...
                RestartFramework.at("after_replication_attempt").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            {
                ...
                RestartFramework.at("after_hot_file_creation").on(test.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            {
