...
public class TestBlocksWithNotEnoughRacks {
   ...
   */
    ...
    public void testSufficientlyReplBlocksUsesNewRack() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(racks.length).racks(racks).build();
        try {
   ...
   */
    ...
    public void testMisReplicatedBlockUsesNewRack() throws Exception {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(racks.length).racks(racks).build()) {
            cluster.waitActive();
            String poolId = cluster.getNamesystem().getBlockPoolId();
            DatanodeRegistrationInterface reg = InternalDataNodeTestUtils.getDNRegistrationForBP(cluster.getDataNodes().get(3), poolId);
            ...
            try {
                ...
                fail("NameNode should not have fixed the mis-replicated blocks" + " automatically.");
            } catch (TimeoutException e) {
                //Expected.
            }
            String fsckOp = DFSTestUtil.runFsck(conf, 0, true, filePath.toString(), "-replicate");
            LOG.info("fsck response {}", fsckOp);
            assertTrue(fsckOp.contains("/testFile:  Replica placement policy is violated"));
            assertTrue(fsckOp.contains(" Block should be additionally replicated" + " on 1 more rack(s). Total number of racks in the cluster: 2"));
            assertTrue(fsckOp.contains(" Blocks queued for replication:\t1"));
            try {
                DFSTestUtil.waitForReplication(cluster, b, 2, replicationFactor, 0);
            } catch (TimeoutException e) {
                fail("NameNode should have fixed the mis-replicated blocks as a" + " result of fsck command.");
            }
        }
    }
   ...
   */
    ...
    public void testReplDueToNodeFailRespectsRackPolicy() throws Exception {
        ...
        try {
            ...
            DataNodeInterface dataNode = datanodes.get(idx);
            DatanodeIDInterface dnId = dataNode.getDatanodeId();
   ...
   */
    ...
    public void testMultipleReplicasScheduledForUpgradeDomain() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(6).build();
        ...
        try {
            ...
            BlockInfoInterface storedBlock = bm.getStoredBlock(b.getLocalBlock());
            // The block should be replicated OK - so Reconstruction Work will be null
            BlockReconstructionWorkInterface work = scheduleReconstruction(cluster.getNamesystem(), storedBlock, 2);
            assertNull(work);
            ...
            int udInd = 0;
            for (DatanodeDescriptorInterface d : dnDescriptors) {
            ...
            work = scheduleReconstruction(cluster.getNamesystem(), storedBlock, 2);
    ...
    public void testUnderReplicatedRespectsRacksAndUpgradeDomain() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(6).racks(racks).build();
        cluster.waitClusterUp();
        List<DatanodeDescriptor> dnDescriptors = getDnDescriptors(cluster);
    ...
    private List<DatanodeDescriptor> getDnDescriptors(MiniDockerDFSCluster cluster) throws IOException {
        ...
        for (DataNodeInterface dn : cluster.getDataNodes()) {
            DatanodeDescriptorInterface d = dnManager.getDatanode(dn.getDatanodeUuid());
            if (d == null) {
