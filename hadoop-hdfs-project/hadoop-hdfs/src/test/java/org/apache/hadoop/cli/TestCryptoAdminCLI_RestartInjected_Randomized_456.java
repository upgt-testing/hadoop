...
public class TestCryptoAdminCLI_RestartInjected_Randomized_456 extends CLITestHelperDFS {
    ...
    @Override
    public void testAll() {
        RestartFramework.at("before_crypto_operations").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        super.testAll();
        RestartFramework.at("after_crypto_operations").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
}
