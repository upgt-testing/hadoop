...
@NotThreadSafe
public class TestFsDatasetCache_RestartInjected_Randomized_123 {

    private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(TestFsDatasetCache_RestartInjected.class);
    ...
    private static void setHeartbeatResponse(DatanodeCommand[] cmds) throws Exception {
        ...
        try {
            ...
            doReturn(response).when(spyNN).sendHeartbeat((DatanodeRegistration) any(), (StorageReport[]) any(), anyLong(), anyLong(), anyInt(), anyInt(), anyInt(), (VolumeFailureSummary) any(), anyBoolean(), any(SlowPeerReports.class), any(SlowDiskReports.class));
        } finally {
    ...
    private static DatanodeCommand[] cacheBlocks(HdfsBlockLocation[] locs) {
        return new DatanodeCommand[] { getResponse(locs, DatanodeProtocol.DNA_CACHE) };
    }
    ...
    private static DatanodeCommand[] uncacheBlocks(HdfsBlockLocation[] locs) {
        return new DatanodeCommand[] { getResponse(locs, DatanodeProtocol.DNA_UNCACHE) };
    }
    ...
    private void testCacheAndUncacheBlock() throws Exception {
        ...
        RestartFramework.at("after_all_blocks_uncached").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.verifyExpectedCacheUsage(0, 0, fsd);
        ...
        RestartFramework.at("after_all_blocks_cached").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
