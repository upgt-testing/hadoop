 ...
 */
public class TestViewFileSystemLinkFallback_RestartInjected_Randomized_123 extends ViewFileSystemBaseTest {
    ...
    private static final File TEST_DIR = GenericTestUtils.getTestDir(TestViewFileSystemLinkFallback_RestartInjected.class.getSimpleName());

    private static final String TEST_BASE_PATH = "/tmp/TestViewFileSystemLinkFallback";

    private final static Logger LOG = LoggerFactory.getLogger(TestViewFileSystemLinkFallback_RestartInjected.class);
    ...
    void setupMountPoints() {
        ...
        ConfigUtil.addLinkFallback(conf, LINK_FALLBACK_CLUSTER_1_NAME, targetTestRoot.toUri());
    }
    ...
    public void testConfLinkFallback() throws Exception {
        ...
        RestartFramework.at("after_first_file_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_second_file_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testConfLinkFallbackWithRegularLinks() throws Exception {
        ...
        URI viewFsUri = new URI(FsConstants.VIEWFS_SCHEME, clusterName, "/", null, null);
        Configuration conf = new Configuration();
        ConfigUtil.addLink(conf, clusterName, "/internalDir/linkToDir2", new Path(targetTestRoot, "dir2").toUri());
        ConfigUtil.addLink(conf, clusterName, "/internalDir/internalDirB/linkToDir3", new Path(targetTestRoot, "dir3").toUri());
        ConfigUtil.addLink(conf, clusterName, "/danglingLink", new Path(targetTestRoot, "missingTarget").toUri());
        ConfigUtil.addLink(conf, clusterName, "/linkToAFile", new Path(targetTestRoot, "aFile").toUri());
    ...
    public void testListingWithFallbackLink() throws Exception {
        ...
        try (FileSystem vfs = FileSystem.get(viewFsUri, conf)) {
            ...
            assertTrue("Listing didn't include fallback link", afterFallback.size() == 1);
            ...
            assertEquals("Path did not match", expected, fallbackArray[0]);
            RestartFramework.at("after_listing_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Create a directory using the returned fallback path and verify
            Path childDir = new Path(fallbackArray[0], "child");
            vfs.mkdirs(childDir);
            RestartFramework.at("after_child_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListingWithFallbackLinkWithSameMountDirectories() throws Exception {
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            afterFallback.removeAll(beforeFallback);
            assertEquals("The same directory name in fallback link should be shaded", 1, afterFallback.size());
            Path[] fallbackArray = new Path[afterFallback.size()];
            // Only user1 should be listed as fallback link
            Path expected = new Path(viewFsDefaultClusterUri.toString(), "user1");
            assertEquals("Path did not match", expected, afterFallback.toArray(fallbackArray)[0]);
            RestartFramework.at("after_listing_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Create a directory using the returned fallback path and verify
            Path childDir = new Path(fallbackArray[0], "child");
            vfs.mkdirs(childDir);
    ...
    public void testListingWithFallbackLinkWithSameMountDirectoryTree() throws Exception {
        ...
        RestartFramework.at("after_permission_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HashSet<Path> beforeFallback = new HashSet<>();
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            for (FileStatus stat : vfs.listStatus(new Path(viewFsDefaultClusterUri.toString(), "/user1/hive/"))) {
                ...
                if (dir1.getName().equals(stat.getPath().getName())) {
                    ...
                    assertEquals(FsPermission.valueOf("-rwxr--r--"), stat.getPermission());
                }
            }
    ...
    public void testLSOnLinkParentWithFallbackLinkWithSameMountDirectoryTree() throws Exception {
        ...
        RestartFramework.at("after_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fsTarget.mkdirs(dir2);
        fsTarget.setPermission(new Path(targetTestRoot, "fallbackDir/user1/hive/warehouse/partition-0"), FsPermission.valueOf("-rwxr--r--"));
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            for (FileStatus stat : vfs.listStatus(new Path(viewFsDefaultClusterUri.toString(), "/user1/hive/warehouse/"))) {
                ...
                if (dir1.getName().equals(stat.getPath().getName())) {
                    // make sure fallback dir listed out with correct permissions, but not
                    // with link permissions.
                    assertEquals(FsPermission.valueOf("-rwxr--rw-"), stat.getPermission());
                }
            }
            afterFallback.removeAll(beforeFallback);
            assertEquals("Just to make sure paths are same.", 0, afterFallback.size());
        }
    }
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            for (FileStatus stat : vfs.listStatus(new Path(viewFsDefaultClusterUri.toString(), "/"))) {
                ...
                if (dir1.getName().equals(stat.getPath().getName())) {
                    ...
                    assertEquals("Path is: " + stat.getPath(), FsPermission.valueOf("-rwxr--r--"), stat.getPermission());
                }
            }
            afterFallback.removeAll(beforeFallback);
            assertEquals(1, afterFallback.size());
            assertEquals("/user2 dir from fallback should be listed.", "user2", afterFallback.iterator().next().getName());
        }
    }
    ...
    public void testLSOnLinkParentWhereMountLinkMatchesWithAFileUnderFallback() throws Exception {
        ...
        ConfigUtil.addLink(conf, "/user1/hive/warehouse/part-0", new Path(targetTestRoot.toString()).toUri());
        // Create a file path in fallback matching to the path of mount link.
        Path file1 = new Path(targetTestRoot, "fallbackDir/user1/hive/warehouse/part-0");
        fsTarget.createNewFile(file1);
        Path dir2 = new Path(targetTestRoot, "fallbackDir/user1/hive/warehouse1");
        fsTarget.mkdirs(dir2);
        RestartFramework.at("after_file_and_dir_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ConfigUtil.addLinkFallback(conf, new Path(targetTestRoot, "fallbackDir").toUri());
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            for (FileStatus stat : vfs.listStatus(new Path(viewFsDefaultClusterUri.toString(), "/user1/hive/warehouse/"))) {
                if (file1.getName().equals(stat.getPath().getName())) {
                    ...
                    RestartFramework.at("after_symlink_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    Path fileUnderDir = new Path(stat.getPath(), "check");
                    // Creating dir under target
                    assertTrue(vfs.mkdirs(fileUnderDir));
                    assertTrue(fsTarget.exists(new Path(targetTestRoot, fileUnderDir.getName())));
                }
            }
        }
    }
    ...
    public void testMkdirsOfLinkParentWithFallbackLinkWithSameMountDirectoryTree() throws Exception {
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            RestartFramework.at("after_test_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirsOfNewDirWithOutMatchingToMountOrFallbackDirTree() throws Exception {
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        RestartFramework.at("after_fallback_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ConfigUtil.addLink(conf, "/user1/hive/warehouse/partition-0", new Path(targetTestRoot.toString()).toUri());
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            assertFalse(fsTarget.exists(test));
            assertTrue(vfs.mkdirs(p));
            RestartFramework.at("after_user2_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(fsTarget.exists(test));
        }
    }
    ...
    public void testMkdirsOfDeepTreeWithFallbackLinkAndMountPathMatchingDirExist() throws Exception {
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            RestartFramework.at("after_deep_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(fsTarget.exists(test));
        }
    }
    ...
    public void testMkdirsShouldReturnFalseWhenFallbackFSNotAvailable() throws Exception {
        ...
        try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
            ...
            RestartFramework.at("before_shutdown").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_mkdir_success").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(fsTarget.exists(test));
        }
    }
