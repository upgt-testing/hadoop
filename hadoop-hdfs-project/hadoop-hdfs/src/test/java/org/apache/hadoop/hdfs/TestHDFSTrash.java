...
public class TestHDFSTrash {
    ...
    private Trash getPerUserTrash(UserGroupInformation ugi, FileSystem fileSystem, Configuration config) throws IOException {
        ...
        Mockito.when(spyUserFs.getTrashRoot(Mockito.any(Path.class))).thenReturn(userTrashRoot);
        return new Trash(spyUserFs, config);
    }

    @Test
    public void testTrash_withUpgrade20() throws IOException {
        TestTrash.trashShell(cluster.getFileSystem(), new Path("/"));
    ...
    @Test
    public void testNonDefaultFS_withUpgrade20() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void testNonDefaultFS_withUpgrade40() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        config.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, fileSystem.getUri().toString());
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testNonDefaultFS_withUpgrade80() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        config.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, fileSystem.getUri().toString());
        TestTrash.trashNonDefaultFS(config);
    ...
    @Test
    public void testHDFSTrashPermission_withUpgrade20() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void testHDFSTrashPermission_withUpgrade40() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        config.set(CommonConfigurationKeys.FS_TRASH_INTERVAL_KEY, "0.2");
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testHDFSTrashPermission_withUpgrade80() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        config.set(CommonConfigurationKeys.FS_TRASH_INTERVAL_KEY, "0.2");
        TestTrash.verifyTrashPermission(fileSystem, config);
    ...
    @Test
    public void testMoveEmptyDirToTrash_withUpgrade20() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void testMoveEmptyDirToTrash_withUpgrade40() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        config.set(CommonConfigurationKeys.FS_TRASH_INTERVAL_KEY, "1");
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testMoveEmptyDirToTrash_withUpgrade80() throws IOException {
        FileSystem fileSystem = cluster.getFileSystem();
        Configuration config = fileSystem.getConf();
        config.set(CommonConfigurationKeys.FS_TRASH_INTERVAL_KEY, "1");
        TestTrash.verifyMoveEmptyDirToTrash(fileSystem, config);
