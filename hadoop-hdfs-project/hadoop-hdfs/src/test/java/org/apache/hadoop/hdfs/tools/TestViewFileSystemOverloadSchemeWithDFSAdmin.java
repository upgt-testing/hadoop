...
public class TestViewFileSystemOverloadSchemeWithDFSAdmin {
    ...
    public void testSaveNameSpace() throws Exception {
        ...
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "enter" });
        assertEquals(0, ret);
        redirectStream();
        ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-saveNamespace" });
        assertEquals(0, ret);
        assertOutMsg("Save namespace successful", 0);
        ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "leave" });
        assertEquals(0, ret);
    }
    ...
    public void testSafeModeWithWrongFS() throws Exception {
        ...
        final Path hdfsTargetPath = new Path(wrongFsUri + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER }, new String[] { hdfsTargetPath.toUri().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", wrongFsUri, "-safemode", "enter" });
        assertEquals(-1, ret);
    ...
    public void testSafeModeShouldFailOnLocalTargetFS() throws Exception {
        addMountLinks(defaultFSURI.getHost(), new String[] { LOCAL_FOLDER }, new String[] { localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        // ViewFSOveloadScheme uri with localfs mount point
        String uri = defaultFSURI.toString() + LOCAL_FOLDER;
        redirectStream();
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", uri, "-safemode", "enter" });
        assertEquals(-1, ret);
    ...
    public void testGetSafemodeWithoutMountLinksConfigured() throws Exception {
        ...
        try {
            redirectStream();
            int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "get" });
    ...
    public void testAllowAndDisalllowSnapShot() throws Exception {
        ...
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-allowSnapshot", "/" });
        assertOutMsg("Allowing snapshot on / succeeded", 0);
        assertEquals(0, ret);
        ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-disallowSnapshot", "/" });
    ...
    public void testSetBalancerBandwidth() throws Exception {
        ...
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-setBalancerBandwidth", "1000" });
        assertOutMsg("Balancer bandwidth is set to 1000", 0);
        assertEquals(0, ret);
    }

    @Test
    public void testSaveNameSpace_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "enter" });
    ...
    @Test
    public void testSaveNameSpace_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "enter" });
    ...
    @Test
    public void testSaveNameSpace_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "enter" });
    ...
    @Test
    public void testSaveNameSpace_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "enter" });
    ...
    @Test
    public void testSaveNamespaceWithoutSpecifyingFS_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-safemode", "enter" });
    ...
    @Test
    public void testSaveNamespaceWithoutSpecifyingFS_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-safemode", "enter" });
    ...
    @Test
    public void testSaveNamespaceWithoutSpecifyingFS_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-safemode", "enter" });
    ...
    @Test
    public void testSaveNamespaceWithoutSpecifyingFS_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        int ret = ToolRunner.run(dfsAdmin, new String[] { "-safemode", "enter" });
    ...
    @Test
    public void testSafeModeWithWrongFS_withUpgrade20() throws Exception {
        String wrongFsUri = "hdfs://nonExistent";
        final Path hdfsTargetPath = new Path(wrongFsUri + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER }, new String[] { hdfsTargetPath.toUri().toString() }, conf);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testSafeModeWithWrongFS_withUpgrade40() throws Exception {
        String wrongFsUri = "hdfs://nonExistent";
        final Path hdfsTargetPath = new Path(wrongFsUri + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER }, new String[] { hdfsTargetPath.toUri().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
    ...
    @Test
    public void testSafeModeWithWrongFS_withUpgrade60() throws Exception {
        String wrongFsUri = "hdfs://nonExistent";
        final Path hdfsTargetPath = new Path(wrongFsUri + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER }, new String[] { hdfsTargetPath.toUri().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
    ...
    @Test
    public void testSafeModeWithWrongFS_withUpgrade80() throws Exception {
        String wrongFsUri = "hdfs://nonExistent";
        final Path hdfsTargetPath = new Path(wrongFsUri + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER }, new String[] { hdfsTargetPath.toUri().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
    ...
    @Test
    public void testSafeModeShouldFailOnLocalTargetFS_withUpgrade20() throws Exception {
        addMountLinks(defaultFSURI.getHost(), new String[] { LOCAL_FOLDER }, new String[] { localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        // ViewFSOveloadScheme uri with localfs mount point
        String uri = defaultFSURI.toString() + LOCAL_FOLDER;
    ...
    @Test
    public void testSafeModeShouldFailOnLocalTargetFS_withUpgrade40() throws Exception {
        addMountLinks(defaultFSURI.getHost(), new String[] { LOCAL_FOLDER }, new String[] { localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        // ViewFSOveloadScheme uri with localfs mount point
        String uri = defaultFSURI.toString() + LOCAL_FOLDER;
    ...
    @Test
    public void testSafeModeShouldFailOnLocalTargetFS_withUpgrade60() throws Exception {
        addMountLinks(defaultFSURI.getHost(), new String[] { LOCAL_FOLDER }, new String[] { localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        // ViewFSOveloadScheme uri with localfs mount point
        String uri = defaultFSURI.toString() + LOCAL_FOLDER;
    ...
    @Test
    public void testSafeModeShouldFailOnLocalTargetFS_withUpgrade80() throws Exception {
        addMountLinks(defaultFSURI.getHost(), new String[] { LOCAL_FOLDER }, new String[] { localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        // ViewFSOveloadScheme uri with localfs mount point
        String uri = defaultFSURI.toString() + LOCAL_FOLDER;
    ...
    @Test
    public void testGetSafemodeWithoutMountLinksConfigured_withUpgrade20() throws Exception {
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        try {
            redirectStream();
            cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testGetSafemodeWithoutMountLinksConfigured_withUpgrade40() throws Exception {
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        try {
            redirectStream();
            int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "get" });
    ...
    @Test
    public void testGetSafemodeWithoutMountLinksConfigured_withUpgrade60() throws Exception {
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        try {
            redirectStream();
            int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "get" });
    ...
    @Test
    public void testGetSafemodeWithoutMountLinksConfigured_withUpgrade80() throws Exception {
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        try {
            redirectStream();
            int ret = ToolRunner.run(dfsAdmin, new String[] { "-fs", defaultFSURI.toString(), "-safemode", "get" });
    ...
    @Test
    public void testAllowAndDisalllowSnapShot_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testAllowAndDisalllowSnapShot_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
    ...
    @Test
    public void testAllowAndDisalllowSnapShot_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
    ...
    @Test
    public void testAllowAndDisalllowSnapShot_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
    ...
    @Test
    public void testSetBalancerBandwidth_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testSetBalancerBandwidth_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
    ...
    @Test
    public void testSetBalancerBandwidth_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
    ...
    @Test
    public void testSetBalancerBandwidth_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        redirectStream();
