...
public class TestStripedFileAppend {
    ...
    public void testAppendToNewBlock() throws IOException {
        ...
        for (int split = 0; split < totalSplit; split++) {
            if (split == 0) {
                ...
                out = dfs.append(file, EnumSet.of(CreateFlag.APPEND, CreateFlag.NEW_BLOCK), 4096, null);
            }
    ...
    public void testAppendWithoutNewBlock() throws IOException {
        ...
        try {
            ...
            assertTrue(e.getMessage().contains("Append on EC file without new block is not supported"));
        }
        List<OpenFilesType> types = new ArrayList<>();
        types.add(OpenFilesType.ALL_OPEN_FILES);
        RemoteIterator<OpenFileEntry> listOpenFiles = dfs.listOpenFiles(EnumSet.copyOf(types), file.toString());
        assertFalse("No file should be open after append failure", listOpenFiles.hasNext());
    }

    @Test
    public void testAppendToNewBlock_withUpgrade20() throws IOException {
        int fileLength = 0;
        int totalSplit = 6;
        byte[] expected = StripedFileTestUtil.generateBytes(BLOCK_GROUP_SIZE * totalSplit);
        Path file = new Path(dir, "testAppendToNewBlock");
    ...
    @Test
    public void testAppendToNewBlock_withUpgrade40() throws IOException {
        int fileLength = 0;
        int totalSplit = 6;
        byte[] expected = StripedFileTestUtil.generateBytes(BLOCK_GROUP_SIZE * totalSplit);
        Path file = new Path(dir, "testAppendToNewBlock");
    ...
    @Test
    public void testAppendToNewBlock_withUpgrade60() throws IOException {
        int fileLength = 0;
        int totalSplit = 6;
        byte[] expected = StripedFileTestUtil.generateBytes(BLOCK_GROUP_SIZE * totalSplit);
        Path file = new Path(dir, "testAppendToNewBlock");
    ...
    @Test
    public void testAppendToNewBlock_withUpgrade80() throws IOException {
        int fileLength = 0;
        int totalSplit = 6;
        byte[] expected = StripedFileTestUtil.generateBytes(BLOCK_GROUP_SIZE * totalSplit);
        Path file = new Path(dir, "testAppendToNewBlock");
    ...
    @Test
    public void testAppendWithoutNewBlock_withUpgrade20() throws IOException {
        Path file = new Path(dir, "testAppendWithoutNewBlock");
        // Create file
        FSDataOutputStream out = dfs.create(file);
        out.write("testAppendWithoutNewBlock".getBytes());
    ...
    @Test
    public void testAppendWithoutNewBlock_withUpgrade40() throws IOException {
        Path file = new Path(dir, "testAppendWithoutNewBlock");
        // Create file
        FSDataOutputStream out = dfs.create(file);
        out.write("testAppendWithoutNewBlock".getBytes());
    ...
    @Test
    public void testAppendWithoutNewBlock_withUpgrade60() throws IOException {
        Path file = new Path(dir, "testAppendWithoutNewBlock");
        // Create file
        FSDataOutputStream out = dfs.create(file);
        out.write("testAppendWithoutNewBlock".getBytes());
    ...
    @Test
    public void testAppendWithoutNewBlock_withUpgrade80() throws IOException {
        Path file = new Path(dir, "testAppendWithoutNewBlock");
        // Create file
        FSDataOutputStream out = dfs.create(file);
        out.write("testAppendWithoutNewBlock".getBytes());
