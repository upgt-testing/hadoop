...
public class TestHdfsAdmin {
    ...
    private void verifyOpenFilesHelper(RemoteIterator<OpenFileEntry> openFilesRemoteItr, HashSet<Path> closedFiles, HashSet<Path> openFiles) throws IOException {
        while (openFilesRemoteItr.hasNext()) {
            ...
            assertFalse(filePath + " should not be listed under open files!", closedFiles.contains(new Path(filePath)));
    ...
    private void verifyOpenFilesOld(HashSet<Path> closedFiles, HashMap<Path, FSDataOutputStream> openFileMap) throws IOException {
        HdfsAdmin hdfsAdmin = new HdfsAdmin(FileSystem.getDefaultUri(conf), conf);
        HashSet<Path> openFiles = new HashSet<>(openFileMap.keySet());
        RemoteIterator<OpenFileEntry> openFilesRemoteItr = hdfsAdmin.listOpenFiles(EnumSet.of(OpenFilesType.ALL_OPEN_FILES));
        verifyOpenFilesHelper(openFilesRemoteItr, closedFiles, openFiles);
        assertTrue("Not all open files are listed!", openFiles.isEmpty());
    }

    @Test
    public void testGetKeyProvider_withUpgrade20() throws IOException {
        HdfsAdmin hdfsAdmin = new HdfsAdmin(FileSystem.getDefaultUri(conf), conf);
        Assert.assertNull("should return null for an non-encrypted cluster", hdfsAdmin.getKeyProvider());
        shutDownCluster();
        Configuration conf = new Configuration();
    ...
    @Test
    public void testGetKeyProvider_withUpgrade40() throws IOException {
        HdfsAdmin hdfsAdmin = new HdfsAdmin(FileSystem.getDefaultUri(conf), conf);
        Assert.assertNull("should return null for an non-encrypted cluster", hdfsAdmin.getKeyProvider());
        shutDownCluster();
        Configuration conf = new Configuration();
    ...
    @Test
    public void testGetKeyProvider_withUpgrade80() throws IOException {
        HdfsAdmin hdfsAdmin = new HdfsAdmin(FileSystem.getDefaultUri(conf), conf);
        Assert.assertNull("should return null for an non-encrypted cluster", hdfsAdmin.getKeyProvider());
        shutDownCluster();
        Configuration conf = new Configuration();
