...
public class TestDelegationTokenFetcher_RestartInjected {
  private static final Logger LOG = LoggerFactory.getLogger(
      TestDelegationTokenFetcher_RestartInjected.class);
  ...
  public void testDelegationTokenWithoutRenewerViaRPC() throws Exception {
    ...
    try {
      ...
      DelegationTokenFetcher.saveDelegationToken(conf, fs, null, p);
      RestartFramework.at("after_save_token")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_read_token")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_print_tokens")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      try {
        RestartFramework.at("before_renew_attempt")
            .on(cluster)
            .restart("namenode")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
