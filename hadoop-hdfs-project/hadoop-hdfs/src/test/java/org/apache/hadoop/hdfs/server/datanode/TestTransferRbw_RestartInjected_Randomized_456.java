 ...
 */
public class TestTransferRbw_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestTransferRbw_RestartInjected.class);
    ...
    public void testTransferRbw() throws Exception {
        ...
        try {
            ...
            final String bpid = cluster.getNamesystem().getBlockPoolId();
            {
                ...
                RestartFramework.at("after_get_old_rbw").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                oldnode = cluster.getDataNodes().get(0);
                //add a datanode
                conf.setLong(DFS_DATANODE_DATA_WRITE_BANDWIDTHPERSEC_KEY, 1024 * 1024 * 8);
                cluster.startDataNodes(conf, 1, true, null, null);
                newnode = cluster.getDataNodes().get(REPLICATION);
                RestartFramework.at("after_add_new_datanode").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                Assert.assertEquals(1024 * 1024 * 8, newnode.xserver.getWriteThrottler().getBandwidth());
                ...
                RestartFramework.at("after_transfer_rbw").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
                oldnode = cluster.getDataNodes().get(0);
            }
            //check new rbw
            ...
            RestartFramework.at("after_verify_new_rbw").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            LOG.info("DONE");
        } finally {
            cluster.shutdown();
        }
