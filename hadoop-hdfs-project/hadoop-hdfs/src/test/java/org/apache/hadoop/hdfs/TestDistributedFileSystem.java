...
public class TestDistributedFileSystem {
    ...
    HdfsConfiguration getTestConfiguration() {
        ...
        if (noXmlDefaults) {
            ...
            String namenodeDir = new File(MiniDockerDFSCluster.getBaseDirectory(), "name").getAbsolutePath();
    ...
    public void testECStatistics() throws IOException {
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(getTestConfiguration()).build()) {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
            Path dir = new Path("/test");
            dfs.mkdirs(dir);
    ...
    public void testConcurrentStatistics() throws IOException, InterruptedException {
        FileSystem.getStatistics(HdfsConstants.HDFS_URI_SCHEME, DistributedFileSystem.class).reset();
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(new Configuration()).build();
        cluster.waitActive();
        final FileSystem fs = cluster.getFileSystem();
    ...
    public void testInvalidScriptMappingFileReadStatistics() throws Exception {
        ...
        // MiniDockerDFSCluster's datanode is on the local host and thus the network
        // distance is 0.
    ...
    private void testReadFileSystemStatistics(int expectedDistance, boolean useScriptMapping, boolean invalidScriptMappingFile) throws IOException {
        ...
        // MiniDockerDFSCluster by default uses StaticMapping unless the test
        // overrides it.
        if (useScriptMapping) {
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).useConfiguredTopologyMappingClass(true).build();
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).racks(new String[] { "/rackClient" }).build();
        } else if (expectedDistance == 4) {
            cluster = new MiniDockerDFSCluster.Builder(conf).racks(new String[] { "/rackFoo" }).build();
        }
    ...
    public void testLocatedFileStatusStorageIdsTypes() throws Exception {
        ...
        try {
            ...
            for (DataNodeInterface d : cluster.getDataNodes()) {
    ...
    public void testCreateWithStoragePolicy() throws Throwable {
        Configuration conf = getTestConfiguration();
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE, StorageType.SSD }).storagesPerDatanode(3).build()) {
            DistributedFileSystem fs = cluster.getFileSystem();
    ...
    public void testListStatusOfSnapshotDirs() throws IOException {
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(getTestConfiguration()).build();
        try {
    ...
    public void testSuperUserPrivilege() throws Exception {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).build()) {
            ...
            dfs.mkdirs(dir);
            final KeyProviderInterface provider = cluster.getNameNode().getNamesystem().getProvider();
            final KeyProvider.OptionsInterface options = KeyProvider.options(conf);
            provider.createKey("key", options);
            provider.flush();
    ...
    public void testStorageFavouredNodes() throws IOException, InterruptedException, TimeoutException {
        Configuration conf = getTestConfiguration();
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).storageTypes(new StorageType[] { StorageType.SSD, StorageType.DISK }).numDataNodes(3).storagesPerDatanode(2).build()) {
            DistributedFileSystem fs = cluster.getFileSystem();
            Path file1 = new Path("/tmp/file1");
            fs.mkdirs(new Path("/tmp"));
    ...
    public void testGetECTopologyResultForPolicies() throws Exception {
        Configuration conf = getTestConfiguration();
        try (MiniDockerDFSCluster cluster = DFSTestUtil.setupCluster(conf, 9, 3, 0)) {
            DistributedFileSystem dfs = cluster.getFileSystem();
