...
public class TestDistributedFileSystem {
    ...
    public void testListStatusOfSnapshotDirs() throws IOException {
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(new HdfsConfiguration()).build();
        try {
    ...
    private void testBuilderSetters(DistributedFileSystem fs) {
        ...
        builder.append().overwrite(false).newBlock().lazyPersist().noLocalWrite().ecPolicyName("ec-policy");
    ...
    @Test
    public void testEnableAndDisableErasureCodingPolicy_withUpgrade80() throws Exception {
        Configuration conf = getTestConfiguration();
        MiniDFSClusterInJVM cluster = null;
        try {
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
