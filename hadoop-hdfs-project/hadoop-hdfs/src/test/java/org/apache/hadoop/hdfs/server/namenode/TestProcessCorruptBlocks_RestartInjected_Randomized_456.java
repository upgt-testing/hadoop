...
public class TestProcessCorruptBlocks_RestartInjected_Randomized_456 {
    ...
    public void testWhenDecreasingReplication() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_block_corrupted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            namesystem = cluster.getNamesystem();
            ...
            RestartFramework.at("after_replication_decreased").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            namesystem = cluster.getNamesystem();
    ...
    public void testWithAllCorruptReplicas() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_all_blocks_corrupted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            namesystem = cluster.getNamesystem();
            // wait for 3 seconds so that all block reports are processed.
            try {
                Thread.sleep(3000);
    ...
    private void corruptBlock(MiniDFSCluster cluster, FileSystem fs, final Path fileName, int dnIndex, ExtendedBlock block) throws IOException {
