...
public class TestAclWithSnapshot {
    ...
    public void testDeDuplication() throws Exception {
        ...
        AclFeatureJVMInterface aclFeature;
        {
            ...
            AclFeatureJVMInterface snapshotAclFeature = FSAclBaseTest.getAclFeature(snapshotPath, cluster);
        ...
        }
        {
            ...
            AclFeatureJVMInterface snapshotAcl = FSAclBaseTest.getAclFeature(subdirInSnapshot, cluster);
        ...
        }
        {
            ...
            AclFeatureJVMInterface snapshotAcl = FSAclBaseTest.getAclFeature(fileInSnapshot, cluster);
            assertSame(aclFeature, snapshotAcl);
            assertEquals("Reference count should remain same", 1, aclFeature.getRefCount());
        ...
        }
        {
            ...
            hdfs.modifyAclEntries(path, aclSpec);
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            AclFeatureJVMInterface snapshotAcl = FSAclBaseTest.getAclFeature(snapshotPath, cluster);
            aclFeature = FSAclBaseTest.getAclFeature(path, cluster);
            ...
            aclFeature = FSAclBaseTest.getAclFeature(path, cluster);
            AclFeatureJVMInterface snapshotAclPostModification = FSAclBaseTest.getAclFeature(snapshotPath, cluster);
        ...
        }
        {
            ...
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            Path subdirInSnapshot = new Path(snapshotPath, "sub-dir");
            AclFeatureJVMInterface snapshotAclFeature = FSAclBaseTest.getAclFeature(subdirInSnapshot, cluster);
            List<AclEntry> newAcl = Lists.newArrayList(aclEntry(ACCESS, USER, "testNewUser", ALL));
        ...
        }
        {
            ...
            Path snapshotPath = SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
            Path fileInSnapshot = new Path(snapshotPath, file.getName());
            AclFeatureJVMInterface snapshotAclFeature = FSAclBaseTest.getAclFeature(fileInSnapshot, cluster);
            List<AclEntry> newAcl = Lists.newArrayList(aclEntry(ACCESS, USER, "testNewUser", ALL));
        ...
        }
        {
            ...
            AclFeatureJVMInterface snapshotDirAclFeature = FSAclBaseTest.getAclFeature(dirInSnapshot, cluster);
            Path fileInSnapshot = new Path(snapshotPath, "sub-dir/file");
            AclFeatureJVMInterface snapshotFileAclFeature = FSAclBaseTest.getAclFeature(fileInSnapshot, cluster);
    ...
    private void deleteSnapshotWithAclAndVerify(AclFeatureJVMInterface aclFeature, Path pathToCheckAcl, int totalAclFeatures) throws IOException {
        hdfs.deleteSnapshot(path, snapshotName);
        AclFeatureJVMInterface afterDeleteAclFeature = FSAclBaseTest.getAclFeature(pathToCheckAcl, cluster);
    ...
    @Test
    public void testDeDuplication_withUpgrade20() throws Exception {
        int startSize = AclStorage.getUniqueAclFeatures().getUniqueElementsSize();
        // unique default AclEntries for this test
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "testdeduplicateuser", ALL), aclEntry(ACCESS, GROUP, "testdeduplicategroup", ALL));
        hdfs.mkdirs(path);
    ...
    @Test
    public void testDeDuplication_withUpgrade40() throws Exception {
        int startSize = AclStorage.getUniqueAclFeatures().getUniqueElementsSize();
        // unique default AclEntries for this test
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "testdeduplicateuser", ALL), aclEntry(ACCESS, GROUP, "testdeduplicategroup", ALL));
        hdfs.mkdirs(path);
    ...
    @Test
    public void testDeDuplication_withUpgrade60() throws Exception {
        int startSize = AclStorage.getUniqueAclFeatures().getUniqueElementsSize();
        // unique default AclEntries for this test
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "testdeduplicateuser", ALL), aclEntry(ACCESS, GROUP, "testdeduplicategroup", ALL));
        hdfs.mkdirs(path);
    ...
    @Test
    public void testDeDuplication_withUpgrade80() throws Exception {
        int startSize = AclStorage.getUniqueAclFeatures().getUniqueElementsSize();
        // unique default AclEntries for this test
        List<AclEntry> aclSpec = Lists.newArrayList(aclEntry(ACCESS, USER, "testdeduplicateuser", ALL), aclEntry(ACCESS, GROUP, "testdeduplicategroup", ALL));
        hdfs.mkdirs(path);
