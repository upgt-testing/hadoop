 ...
 */
public class TestViewFileSystemOverloadSchemeWithHdfsScheme_RestartInjected_Randomized_456 {
    ...
    public void testListStatusOnRootShouldListAllMountLinks() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            ...
            RestartFramework.at("after_list_status").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAccessViewFsPathWithoutAuthority() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            ...
            // /local/test
            fs.mkdirs(localDir);
            RestartFramework.at("after_mkdirs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWithLinkFallBack() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            ...
            RestartFramework.at("after_create_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testViewFsOverloadSchemeWhenInnerCacheDisabled() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            ...
            RestartFramework.at("after_exists_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testNflyRename() throws Exception {
        ...
        RestartFramework.at("after_second_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(nfly.rename(testDirTmp, testDir));
        RestartFramework.at("after_first_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNflyWriteRead() throws Exception {
        ...
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
        RestartFramework.at("after_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path nflyRoot = new Path("/nflyroot");
        final String nflyLinkKey = Constants.CONFIG_VIEWFS_LINK_NFLY + ".minReplication=2." + nflyRoot.toString();
        addMountLinks(defaultFSURI.getAuthority(), new String[] { nflyLinkKey }, new String[] { uri1.toString() + "," + uri2.toString() }, conf);
        RestartFramework.at("after_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final FileSystem nfly = FileSystem.get(defaultFSURI, conf);
    ...
    public void testNflyRepair() throws Exception {
        ...
        try (FileSystem nfly = FileSystem.get(defaultFSURI, conf)) {
            ...
            try (FileSystem fs1 = getter.getNewInstance(testUris[0], conf)) {
                ...
                RestartFramework.at("after_delete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                assertFalse(fs1.exists(new Path(testUris[0].toString() + testFile)));
                // Verify read success.
                readString(nfly, testFilePath, TEST_STRING, testUris[0]);
                RestartFramework.at("after_repair").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMountTableNameShouldIgnorePortFromURI() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(uriWithoutPort, conf)) {
            ...
            RestartFramework.at("after_mkdir_without_port").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.delete(testDirOnRoot, true);
            RestartFramework.at("after_delete_without_port").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        //Initialize with port
        try (FileSystem fs = FileSystem.get(defaultFSURI, conf)) {
            fs.mkdirs(testDirOnRoot);
            RestartFramework.at("after_mkdir_with_port").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.delete(testDirOnRoot, true);
            RestartFramework.at("after_delete_with_port").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
