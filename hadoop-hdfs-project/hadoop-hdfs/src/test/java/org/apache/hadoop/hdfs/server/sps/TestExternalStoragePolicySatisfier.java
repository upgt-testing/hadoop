...
public class TestExternalStoragePolicySatisfier {
    ...
    private MiniDockerDFSCluster hdfsCluster = null;
     ...
     * Sets hdfs cluster.
     */
    private void setCluster(MiniDockerDFSCluster cluster) {
     ...
     * @return hdfs cluster.
     */
    private MiniDockerDFSCluster getCluster() {
        return hdfsCluster;
    }
    ...
    private MiniDockerDFSCluster startCluster(final Configuration conf, StorageType[][] storageTypes, int numberOfDatanodes, int storagesPerDn, long nodeCapacity) throws IOException {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numberOfDatanodes).storagesPerDatanode(storagesPerDn).storageTypes(storageTypes).storageCapacities(capacities).build();
        cluster.waitActive();
    ...
    public void testSPSWhenFileLengthIsZero() throws Exception {
        try {
            ...
            FSEditLogInterface editlog = hdfsCluster.getNameNode().getNamesystem().getEditLog();
            ...
            INodeInterface inode = hdfsCluster.getNameNode().getNamesystem().getFSDirectory().getINode(filePath.toString());
    ...
    private String createFileAndSimulateFavoredNodes(int favoredNodesCount) throws IOException {
        ...
        for (DatanodeInfoInterface dnInfo : locations) {
            ...
            if (favoredNodesCount <= 0) {
                // marked favoredNodesCount number of pinned block location
                break;
            }
        }
        return file1;
    ...
    private void startAdditionalDNs(final Configuration conf, int newNodesRequired, int existingNodesNum, StorageType[][] newTypes, int storagesPerDn, long nodeCapacity, final MiniDockerDFSCluster cluster) throws IOException {
        long[][] capacities;
        ...
        cluster.startDataNodes(conf, newNodesRequired, newTypes, true, null, null, null, capacities, null, false, false, false, null, null, null);
