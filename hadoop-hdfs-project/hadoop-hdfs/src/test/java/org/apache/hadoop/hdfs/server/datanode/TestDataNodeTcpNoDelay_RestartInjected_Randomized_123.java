 ...
 */
public class TestDataNodeTcpNoDelay_RestartInjected_Randomized_123 {
    ...
    public void testTcpNoDelayEnabled() throws Exception {
        ...
        RestartFramework.at("after_cluster_start").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DistributedFileSystem dfs = dfsCluster.getFileSystem();
        try {
            createData(dfs);
            RestartFramework.at("after_create_data").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            transferBlock(dfs);
            RestartFramework.at("after_transfer_block").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
