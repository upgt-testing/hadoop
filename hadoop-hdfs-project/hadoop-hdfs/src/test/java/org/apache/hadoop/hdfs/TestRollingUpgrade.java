...
public class TestRollingUpgrade {
    ...
    public void testRollingUpgradeWithQJM() throws Exception {
        String nnDirPrefix = MiniDockerDFSCluster.getBaseDirectory() + "/nn/";
        ...
        try (MiniJournalCluster mjc = new MiniJournalCluster.Builder(conf).build()) {
            ...
            setConf(conf, nn1Dir, mjc);
            {
                ...
                final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).manageNameDfsDirs(false).checkExitOnShutdown(false).build();
            ...
            MiniDockerDFSCluster cluster2 = null;
            try {
                ...
                final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).format(false).manageNameDfsDirs(false).checkExitOnShutdown(false).build();
                final Path foo = new Path("/foo");
                final Path bar = new Path("/bar");
                final Path baz = new Path("/baz");
                ...
                cluster2 = new MiniDockerDFSCluster.Builder(conf2).numDataNodes(0).format(false).manageNameDfsDirs(false).build();
    ...
    private static void startRollingUpgrade(Path foo, Path bar, Path file, byte[] data, MiniDockerDFSCluster cluster) throws IOException {
        final DistributedFileSystem dfs = cluster.getFileSystem();
        //start rolling upgrade
        dfs.setSafeMode(SafeModeAction.SAFEMODE_ENTER);
    ...
    private static void rollbackRollingUpgrade(Path foo, Path bar, Path file, byte[] data, MiniDockerDFSCluster cluster) throws IOException {
    ...
    private void testFinalize(int nnCount, boolean skipImageDeltaCheck) throws Exception {
        ...
        try {
            ...
            MiniDockerDFSCluster dfsCluster = cluster.getDfsCluster();
            ...
            dfs.mkdirs(foo);
            FSImageInterface fsimage = dfsCluster.getNamesystem(0).getFSImage();
            // start rolling upgrade
            RollingUpgradeInfoInterface info = dfs.rollingUpgrade(RollingUpgradeAction.PREPARE);
     ...
     */
    private void verifyNNCheckpoint(MiniDockerDFSCluster dfsCluster, long txid, int nnIndex) throws InterruptedException {
        int retries = 0;
        while (++retries < 5) {
            NNStorageInterface storage = dfsCluster.getNamesystem(nnIndex).getFSImage().getStorage();
