 ...
 */
public class TestReplicaCachingGetSpaceUsed_RestartInjected_Randomized_123 {
    ...
    public void testReplicaCachingGetSpaceUsedByFINALIZEDReplica() throws Exception {
        ...
        RestartFramework.at("after_block_metadata_collection").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_space_refresh").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_finalized").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReplicaCachingGetSpaceUsedByRBWReplica() throws Exception {
        ...
        byte[] bytes = new byte[20480];
        InputStream is = new ByteArrayInputStream(bytes);
        RestartFramework.at("after_rbw_space_check").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        IOUtils.copyBytes(is, os, bytes.length);
        os.hsync();
        DFSInputStream dfsInputStream = fs.getClient().open("/testReplicaCachingGetSpaceUsedByRBWReplica");
        RestartFramework.at("after_rbw_metadata_collection").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        long blockLength = 0;
        long metaLength = 0;
        List<LocatedBlock> locatedBlocks = dfsInputStream.getAllBlocks();
        RestartFramework.at("after_finalized_space_check").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (LocatedBlock locatedBlock : locatedBlocks) {
            ExtendedBlock extendedBlock = locatedBlock.getBlock();
            blockLength += extendedBlock.getLocalBlock().getNumBytes();
            metaLength += dataNode.getFSDataset().getMetaDataInputStream(extendedBlock).getLength();
        ...
        // recalculated
        Thread.sleep(2000);
        RestartFramework.at("after_rbw_replica_created").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertEquals(blockLength + metaLength, dataNode.getFSDataset().getDfsUsed());
        RestartFramework.at("after_replica_finalized").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFsDatasetImplDeepCopyReplica() {
        ...
        modifyThread.start();
        RestartFramework.at("after_deep_copy_operations").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_modify_thread_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
