...
public class TestReconstructStripedBlocksWithRackAwareness {
    ...
    private MiniDFSClusterInJVM.DataNodeProperties stopDataNode(String hostname) throws IOException {
    ...
    public void testReconstructForNotEnoughRacks() throws Exception {
        ...
        MiniDFSClusterInJVM.DataNodeProperties lastHost = stopDataNode(hosts[hosts.length - 1]);
        ...
        final INodeFileJVMInterface fileNode = fsn.getFSDirectory().getINode4Write(file.toString()).asFile();
    ...
    public void testReconstructionWithDecommission() throws Exception {
        ...
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(rackNames).hosts(hostNames).numDataNodes(hostNames.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
        fs.setErasureCodingPolicy(new Path("/"), StripedFileTestUtil.getDefaultECPolicy().getName());
        ...
        MiniDFSClusterInJVM.DataNodeProperties h9 = stopDataNode(hostNames[hostNames.length - 3]);
        MiniDFSClusterInJVM.DataNodeProperties h10 = stopDataNode(hostNames[hostNames.length - 2]);
        final Path file = new Path("/foo");
        DFSTestUtil.createFile(fs, file, cellSize * dataBlocks * 2, (short) 1, 0L);
        final BlockInfoJVMInterface blockInfo = cluster.getNamesystem().getFSDirectory().getINode(file.toString()).asFile().getLastBlock();
        // bring h9 back
        cluster.restartDataNode(h9);
        cluster.waitActive();
        // stop h11 so that the reconstruction happens
        MiniDFSClusterInJVM.DataNodeProperties h11 = stopDataNode(hostNames[hostNames.length - 1]);
        /*
    ...
    @Test
    public void testReconstructForNotEnoughRacks_withUpgrade20() throws Exception {
        LOG.info("cluster hosts: {}, racks: {}", Arrays.asList(hosts), Arrays.asList(racks));
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
    ...
    @Test
    public void testReconstructForNotEnoughRacks_withUpgrade40() throws Exception {
        LOG.info("cluster hosts: {}, racks: {}", Arrays.asList(hosts), Arrays.asList(racks));
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
    ...
    @Test
    public void testReconstructForNotEnoughRacks_withUpgrade60() throws Exception {
        LOG.info("cluster hosts: {}, racks: {}", Arrays.asList(hosts), Arrays.asList(racks));
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
    ...
    @Test
    public void testReconstructForNotEnoughRacks_withUpgrade80() throws Exception {
        LOG.info("cluster hosts: {}, racks: {}", Arrays.asList(hosts), Arrays.asList(racks));
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
    ...
    @Test
    public void testChooseExcessReplicasToDelete_withUpgrade20() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
    ...
    @Test
    public void testChooseExcessReplicasToDelete_withUpgrade40() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
    ...
    @Test
    public void testChooseExcessReplicasToDelete_withUpgrade60() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
    ...
    @Test
    public void testChooseExcessReplicasToDelete_withUpgrade80() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
    ...
    @Test
    public void testReconstructionWithDecommission_withUpgrade20() throws Exception {
        final String[] rackNames = getRacks(dataBlocks + parityBlocks + 2, dataBlocks);
        final String[] hostNames = getHosts(dataBlocks + parityBlocks + 2);
        // we now have 11 hosts on 6 racks with distribution: 2-2-2-2-2-1
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(rackNames).hosts(hostNames).numDataNodes(hostNames.length).build();
    ...
    @Test
    public void testReconstructionWithDecommission_withUpgrade40() throws Exception {
        final String[] rackNames = getRacks(dataBlocks + parityBlocks + 2, dataBlocks);
        final String[] hostNames = getHosts(dataBlocks + parityBlocks + 2);
        // we now have 11 hosts on 6 racks with distribution: 2-2-2-2-2-1
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(rackNames).hosts(hostNames).numDataNodes(hostNames.length).build();
    ...
    @Test
    public void testReconstructionWithDecommission_withUpgrade60() throws Exception {
        final String[] rackNames = getRacks(dataBlocks + parityBlocks + 2, dataBlocks);
        final String[] hostNames = getHosts(dataBlocks + parityBlocks + 2);
        // we now have 11 hosts on 6 racks with distribution: 2-2-2-2-2-1
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(rackNames).hosts(hostNames).numDataNodes(hostNames.length).build();
    ...
    @Test
    public void testReconstructionWithDecommission_withUpgrade80() throws Exception {
        final String[] rackNames = getRacks(dataBlocks + parityBlocks + 2, dataBlocks);
        final String[] hostNames = getHosts(dataBlocks + parityBlocks + 2);
        // we now have 11 hosts on 6 racks with distribution: 2-2-2-2-2-1
        cluster = new MiniDFSClusterInJVM.Builder(conf).racks(rackNames).hosts(hostNames).numDataNodes(hostNames.length).build();
