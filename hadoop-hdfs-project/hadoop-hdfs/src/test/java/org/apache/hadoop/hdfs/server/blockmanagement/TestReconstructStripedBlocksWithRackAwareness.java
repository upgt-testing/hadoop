...
public class TestReconstructStripedBlocksWithRackAwareness {
    ...
    private MiniDockerDFSCluster.DataNodeProperties stopDataNode(String hostname) throws IOException {
        MiniDockerDFSCluster.DataNodeProperties dnProp = null;
    ...
    public void testReconstructForNotEnoughRacks() throws Exception {
        ...
        MiniDockerDFSCluster.DataNodeProperties lastHost = stopDataNode(hosts[hosts.length - 1]);
        ...
        NetworkTopologyInterface topology = bm.getDatanodeManager().getNetworkTopology();
    ...
    public void testChooseExcessReplicasToDelete() throws Exception {
        ...
        DFSTestUtil.createFile(fs, file, cellSize * dataBlocks * 2, (short) 1, 0L);
        // stop host1
        MiniDockerDFSCluster.DataNodeProperties host1 = stopDataNode("host1");
    ...
    public void testReconstructionWithDecommission() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).racks(rackNames).hosts(hostNames).numDataNodes(hostNames.length).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
        fs.setErasureCodingPolicy(new Path("/"), StripedFileTestUtil.getDefaultECPolicy().getName());
        final BlockManagerInterface bm = cluster.getNamesystem().getBlockManager();
        final DatanodeManagerInterface dm = bm.getDatanodeManager();
        // stop h9 and h10 and create a file with 6+3 internal blocks
        MiniDockerDFSCluster.DataNodeProperties h9 = stopDataNode(hostNames[hostNames.length - 3]);
        ...
        MiniDockerDFSCluster.DataNodeProperties h11 = stopDataNode(hostNames[hostNames.length - 1]);
        ...
        DataNodeInterface datanode9 = getDataNode(hostNames[hostNames.length - 3]);
        Assert.assertNotNull(datanode9);
        final DatanodeDescriptorInterface dn9 = dm.getDatanode(datanode9.getDatanodeId());
