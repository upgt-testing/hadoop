...
public class TestSnapshot {
    ...
    protected static FSDirectoryInterface fsdir;
    ...
    private void runTestSnapshot(int iteration) throws Exception {
        for (int i = 0; i < iteration; i++) {
            ...
            TestDirectoryTree.NodeInterface chmodDir = dirTree.getRandomDirNode(random, null);
            Modification chmod = new FileChangePermission(chmodDir.nodePath, hdfs, genRandomPermission());
            String[] userGroup = genRandomOwner();
            TestDirectoryTree.NodeInterface chownDir = dirTree.getRandomDirNode(random, Arrays.asList(chmodDir));
    ...
    public void testAllowAndDisallowSnapshot() throws Exception {
        ...
        DFSTestUtil.createFile(hdfs, file0, BLOCKSIZE, REPLICATION, seed);
        DFSTestUtil.createFile(hdfs, file1, BLOCKSIZE, REPLICATION, seed);
        INodeDirectoryInterface dirNode = fsdir.getINode4Write(dir.toString()).asDirectory();
    ...
    private Modification[] prepareModifications(TestDirectoryTree.Node[] nodes) throws Exception {
        ...
        for (TestDirectoryTree.NodeInterface node : nodes) {
            ...
            // dir rename
            NodeInterface dstParent = dirTree.getRandomDirNode(random, Arrays.asList(nodes));
    ...
    static class FileChangePermission extends FileStatusChange {
        ...
        private final FsPermissionInterface newPermission;
    ...
    class DirCreationOrDeletion extends Modification {
        ...
        private final TestDirectoryTree.NodeInterface node;
        ...
        void modify() throws Exception {
            if (isCreation) {
                ...
                // deletion
                TestDirectoryTree.NodeInterface childToDelete = node.nonSnapshotChildren.remove(node.nonSnapshotChildren.size() - 1);
    ...
    class DirRename extends Modification {
        ...
        private final TestDirectoryTree.NodeInterface srcParent;
