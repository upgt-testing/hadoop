 ...
 */
public class TestBlockStatsMXBean_RestartInjected_Randomized_123 {
    ...
    public void testStorageTypeStats() throws Exception {
        ...
        RestartFramework.at("after_cluster_ready").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(3, storageTypeStats.getNodesInService());
    }
    ...
    public void testStorageTypeStatsJMX() throws Exception {
        ...
        RestartFramework.at("after_cluster_ready").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStorageTypeStatsWhenStorageFailed() throws Exception {
        ...
        Map<StorageType, StorageTypeStats> storageTypeStatsMap = cluster.getNamesystem().getBlockManager().getStorageTypeStats();
        StorageTypeStats storageTypeStats = storageTypeStatsMap.get(StorageType.RAM_DISK);
        RestartFramework.at("after_storage_restore").on(cluster).restart("namenode").withMode(RestartMode.CRASH).execute();
        assertEquals(6, storageTypeStats.getNodesInService());
        storageTypeStats = storageTypeStatsMap.get(StorageType.DISK);
        assertEquals(3, storageTypeStats.getNodesInService());
        storageTypeStats = storageTypeStatsMap.get(StorageType.ARCHIVE);
        ...
        RestartFramework.at("after_failure_injection").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_create").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.CRASH).execute();
    ...
    public void testStorageTypeLoad() throws Exception {
        HeartbeatManager heartbeatManager = cluster.getNamesystem().getBlockManager().getDatanodeManager().getHeartbeatManager();
        Map<StorageType, StorageTypeStats> storageTypeStatsMap = heartbeatManager.getStorageTypeStats();
        ...
        RestartFramework.at("after_cold_file_hflush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.CRASH).execute();
        RestartFramework.at("after_cluster_ready").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_hot_file_hflush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.CRASH).execute();
        coldSpFileStream.hflush();
        RestartFramework.at("after_trigger_heartbeats").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
        ...
        GenericTestUtils.waitFor(() -> storageTypeStatsMap.get(StorageType.DISK).getNodesInServiceXceiverCount() == 6, 100, 5000);
        // The count for ARCHIVE should be independent of the value of DISK.
        GenericTestUtils.waitFor(() -> storageTypeStatsMap.get(StorageType.ARCHIVE).getNodesInServiceXceiverCount() == 6, 100, 5000);
        // The total count should stay unaffected, that is sum of load from all
        // datanodes.
        GenericTestUtils.waitFor(() -> heartbeatManager.getInServiceXceiverCount() == 12, 100, 5000);
