...
/**
 * ProcessBasedMiniDFSCluster version of {@link TestFileLimit}.
 *
 ...
 *
 * @see TestFileLimit Original test using MiniDFSCluster
 */
@RunWith(Parameterized.class)
public class TestFileLimit_ProcessBased extends ProcessBasedUpgradeTestBase {
  ...
  public static Collection<String> checkpoints() {
    return Arrays.asList(
      ...
      "AFTER_FIRST_FILE_DELETE",
      "AFTER_FILE_RECREATE",
      "AFTER_SECOND_FILE_DELETE",
      "AFTER_DIR_CREATE",
  ...
  /**
   * Wait for NameNode to process deletions.
   *
   * TRANSFORMATION NOTE: Original test used namesys.getBlocksTotal() +
   * namesys.dir.totalInodes() to poll internal NameNode counters. These
   * internal metrics are not accessible via client APIs. Instead, we use
   * a fixed delay to allow the NameNode time to process deletions and
   * update its internal state.
   */
  private void waitForDeletionProcessing() throws InterruptedException {
    // Wait for NameNode to process deletion and update counters
    // Original used polling of internal counters; we use fixed delay
    System.out.println("Waiting for NameNode to process deletions...");
    Thread.sleep(3000);
  }
  ...
  public void testFileLimit() throws Exception {
    ...
    checkpoint("AFTER_SECOND_FILE_DELETE");
    ...
    // Allow time for directory creation to be processed
    Thread.sleep(1000);
  ...
  public void testMaxBlocksPerFileLimit() throws Exception {
    ...
    checkpoint("AFTER_MAX_BLOCKS_WRITTEN");
