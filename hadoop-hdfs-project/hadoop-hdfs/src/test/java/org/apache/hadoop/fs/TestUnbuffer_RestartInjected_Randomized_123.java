...
public class TestUnbuffer_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestUnbuffer.class.getName());
    ...
    public void testUnbufferClosesSockets() throws Exception {
        ...
        try {
            ...
            // cache.
            stream.unbuffer();
            RestartFramework.at("after_unbuffer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stream.seek(2);
    ...
    public void testOpenManyFilesViaTcp() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < NUM_OPENS; i++) {
                ...
                streams[i].unbuffer();
                if (i == 100) {
                    RestartFramework.at("after_100_opens").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
                if (i == 250) {
                    RestartFramework.at("after_250_opens").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
            }
        } finally {
