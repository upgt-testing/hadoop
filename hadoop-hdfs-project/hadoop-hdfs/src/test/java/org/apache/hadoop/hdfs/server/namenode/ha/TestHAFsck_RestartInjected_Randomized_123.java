...
public class TestHAFsck_RestartInjected_Randomized_123 {
    ...
    public static Iterable<Object[]> data() {
        return Arrays.asList(new Object[][] { { ConfiguredFailoverProxyProvider.class.getName() }, { RequestHedgingProxyProvider.class.getName() } });
    }
    ...
    public void testHaFsck() throws Exception {
        ...
        try {
            ...
            runFsck(conf);
            RestartFramework.at("after_first_fsck").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            runFsck(conf);
            RestartFramework.at("after_second_fsck").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Stop one standby namenode, FSCK should still be successful, since there
            // is one Active namenode available
            cluster.getNameNode(0).stop();
            RestartFramework.at("after_stop_standby_namenode").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            runFsck(conf);
            RestartFramework.at("after_third_fsck").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (fs != null) {
                fs.close();
            }
