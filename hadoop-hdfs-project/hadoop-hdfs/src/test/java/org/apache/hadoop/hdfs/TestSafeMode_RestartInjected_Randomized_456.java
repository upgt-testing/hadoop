 ...
 */
public class TestSafeMode_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(TestSafeMode_RestartInjected.class);
    ...
    public void testNoExtensionIfNoBlocks() throws IOException {
        ...
        RestartFramework.at("after_config_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testInitializeReplQueuesEarly() throws Exception {
        ...
        assertEquals("Safe mode is ON. The reported blocks 0 needs additional " + "14 blocks to reach the threshold 0.9990 of total blocks 15." + NEWLINE + "The minimum number of live datanodes is not required. " + "Safe mode will be turned off automatically once the thresholds have " + "been reached.", status);
        ...
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            ...
            @Override
            public Boolean get() {
                return getLongCounter("StorageBlockReportNumOps", getMetrics(NN_METRICS)) == cluster.getStoragesPerDatanode();
            }
        }, 10, 10000);
    ...
    public void testRbwBlocksNotConsideredUnderReplicated() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_junk_blocks_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_hflush_operations").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.restartNameNode();
            RestartFramework.at("after_namenode_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testOperationsWhileInSafeMode() throws IOException, InterruptedException {
        ...
        try {
            ...
            GenericTestUtils.assertExceptionContains("Cannot add erasure coding policy", ioe);
            // expected
        }
        try {
            ...
            GenericTestUtils.assertExceptionContains("Cannot remove erasure coding policy", ioe);
            // expected
        }
   ...
   */
    ...
    public void testSafeModeUtils() throws IOException {
        ...
        RestartFramework.at("after_safemode_leave").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_safemode_enter").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSafeModeWhenZeroBlockLocations() throws IOException {
        try {
            ...
            dfs.setSafeMode(SafeModeAction.SAFEMODE_LEAVE);
            RestartFramework.at("after_final_safemode_leave").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            namenode = cluster.getNameNode();
            assertFalse("Should not be in safemode", namenode.isInSafeMode());
            checkGetBlockLocationsWorks(fs, file1);
        } finally {
