...
public class TestBlockManager {
    ...
    private BlockInfo addCorruptBlockOnNodes(long blockId, List<DatanodeDescriptor> nodes) throws IOException {
        ...
        Mockito.doReturn(bc).when(fsn).getBlockCollection(inodeId);
        bm.blocksMap.addBlockCollection(blockInfo, bc);
        bm.markBlockReplicasAsCorrupt(blockInfo, blockInfo.getGenerationStamp() + 1, blockInfo.getNumBytes(), new DatanodeStorageInfo[] { nodes.get(0).getStorageInfos()[0] });
        return blockInfo;
    }
    ...
    public void testFavorDecomUntilHardLimit() throws Exception {
        ...
        List<DatanodeDescriptor> cntNodes = new LinkedList<DatanodeDescriptor>();
        List<DatanodeStorageInfo> liveNodes = new LinkedList<DatanodeStorageInfo>();
        assertNotNull("Chooses decommissioning source node for a normal replication" + " if all available source nodes have reached their replication" + " limits below the hard limit.", bm.chooseSourceDatanode(aBlock, cntNodes, liveNodes, new NumberReplicas(), UnderReplicatedBlocks.QUEUE_UNDER_REPLICATED));
        // Increase the replication count to test replication count > hard limit
        DatanodeStorageInfo[] targets = { origNodes.get(1).getStorageInfos()[0] };
        origNodes.get(0).addBlockToBeReplicated(aBlock, targets);
        assertNull("Does not choose a source decommissioning node for a normal" + " replication when all available nodes exceed the hard limit.", bm.chooseSourceDatanode(aBlock, cntNodes, liveNodes, new NumberReplicas(), UnderReplicatedBlocks.QUEUE_UNDER_REPLICATED));
    }
    ...
    public void testSafeModeIBRBeforeFirstFullBR() throws Exception {
        ...
        assertEquals(0, ds.getBlockReportCount());
        bm.processReport(node, new DatanodeStorage(ds.getStorageID()), builder.build(), new BlockReportContext(1, 0, System.nanoTime(), 0));
        assertEquals(1, ds.getBlockReportCount());
    ...
    public void testUCBlockNotConsideredMissing() throws Exception {
        ...
        assertEquals("UC block was incorrectly added to needed Replications", 0, bm.neededReplications.size());
   ...
   */
    ...
    public void testIsReplicaCorruptCall() throws Exception {
        ...
        spyBM.createLocatedBlocks(new BlockInfo[] { blockInfo }, 3L, false, 0L, 3L, false, false, null);
    ...
    private BlockInfo makeBlockReplicasMissing(long blockId, List<DatanodeDescriptor> nodesList) throws IOException {
        ...
        Mockito.doReturn(bc).when(fsn).getBlockCollection(inodeId);
        bm.blocksMap.addBlockCollection(blockInfo, bc);
        bm.markBlockReplicasAsCorrupt(blockInfo, blockInfo.getGenerationStamp() + 1, blockInfo.getNumBytes(), new DatanodeStorageInfo[] {});
    ...
    @Test(timeout = 60000)
    public void testNeededReplicationWhileAppending_withUpgrade20() throws IOException {
        Configuration conf = new HdfsConfiguration();
        String src = "/test-file";
        Path file = new Path(src);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
    ...
    @Test(timeout = 60000)
    public void testNeededReplicationWhileAppending_withUpgrade40() throws IOException {
        Configuration conf = new HdfsConfiguration();
        String src = "/test-file";
        Path file = new Path(src);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
    ...
    @Test(timeout = 60000)
    public void testNeededReplicationWhileAppending_withUpgrade60() throws IOException {
        Configuration conf = new HdfsConfiguration();
        String src = "/test-file";
        Path file = new Path(src);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
    ...
    @Test(timeout = 60000)
    public void testNeededReplicationWhileAppending_withUpgrade80() throws IOException {
        Configuration conf = new HdfsConfiguration();
        String src = "/test-file";
        Path file = new Path(src);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build();
