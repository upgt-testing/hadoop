...
public class TestNameNodeRpcServerMethods_RestartInjected_Randomized_123 {
    ...
    public void testDeleteSnapshotWhenSnapshotNameIsEmpty() throws Exception {
        ...
        RestartFramework.at("before_second_validation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
    ...
    public void testGetDatanodeStorageReportWithNumBLocksNotZero() throws Exception {
        ...
        outputStream.close();
        RestartFramework.at("before_storage_report").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int numBlocks = 0;
