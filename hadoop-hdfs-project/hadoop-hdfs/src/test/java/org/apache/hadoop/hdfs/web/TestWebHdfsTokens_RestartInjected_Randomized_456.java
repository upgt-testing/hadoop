...
public class TestWebHdfsTokens_RestartInjected_Randomized_456 {
    ...
    private static void initEnv() {
        ...
        UserGroupInformation.setLoginUser(UserGroupInformation.createUserForTesting("LoginUser", new String[] { "supergroup" }));
    }

    private static void initSecureConf(Configuration secureConf) throws Exception {
        baseDir = GenericTestUtils.getTestDir(TestWebHdfsTokens_RestartInjected.class.getSimpleName());
        FileUtil.fullyDelete(baseDir);
        assertTrue(baseDir.mkdirs());
        Properties kdcConf = MiniKdc.createConf();
        kdc = new MiniKdc(kdcConf, baseDir);
        kdc.start();
        SecurityUtil.setAuthenticationMethod(UserGroupInformation.AuthenticationMethod.KERBEROS, secureConf);
        UserGroupInformation.setConfiguration(secureConf);
        KerberosName.resetDefaultRealm();
        assertTrue("Expected secureConfiguration to enable security", UserGroupInformation.isSecurityEnabled());
        ...
        secureConf.set(HttpServer2.FILTER_INITIALIZER_PROPERTY, AuthenticationFilterInitializer.class.getName());
        ...
        secureConf.set(DFS_WEB_AUTHENTICATION_KERBEROS_PRINCIPAL_KEY, spnegoPrincipal);
        ...
        secureConf.set(DFS_HTTP_POLICY_KEY, HttpConfig.Policy.HTTP_AND_HTTPS.name());
        ...
        KeyStoreTestUtil.setupSSLConfig(keystoresDir, sslConfDir, secureConf, false);
        secureConf.set(DFS_CLIENT_HTTPS_KEYSTORE_RESOURCE_KEY, KeyStoreTestUtil.getClientSSLConfigFileName());
        secureConf.set(DFS_SERVER_HTTPS_KEYSTORE_RESOURCE_KEY, KeyStoreTestUtil.getServerSSLConfigFileName());
    }
    ...
    public void testLazyTokenFetchForWebhdfs() throws Exception {
        ...
        try {
            ...
            ugi = UserGroupInformation.loginUserFromKeytabAndReturnUGI(principal, keytabFile.getAbsolutePath());
            //test with swebhdfs
            uri = DFSUtil.createUri("swebhdfs", cluster.getNameNode().getHttpsAddress());
            validateLazyTokenFetch(ugi, clusterConf);
            RestartFramework.at("after_swebhdfs_token_fetch").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //test with webhdfs
    ...
    public void testSetTokenServiceAndKind() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_get_delegation_token").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void validateLazyTokenFetch(UserGroupInformation ugi, final Configuration clusterConf) throws Exception {
        ...
        is.close();
        // first bad, then good
        verify(fs, times(3)).getDelegationToken();
