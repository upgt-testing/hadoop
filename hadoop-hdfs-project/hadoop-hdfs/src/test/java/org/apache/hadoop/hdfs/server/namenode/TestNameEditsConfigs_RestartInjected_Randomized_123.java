 ...
 */
public class TestNameEditsConfigs_RestartInjected_Randomized_123 {
    ...
    public void testNameEditsConfigs() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_file2_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNameEditsRequiredConfigs() throws IOException {
        ...
        try {
            ...
            conf.set(DFSConfigKeys.DFS_NAMENODE_EDITS_DIR_REQUIRED_KEY, nameAndEditsDir2.toURI().toString());
            cluster = new MiniDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).manageNameDfsDirs(false).build();
            RestartFramework.at("after_cluster_start_case2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
        ...
        try {
            ...
            conf.setStrings(DFSConfigKeys.DFS_NAMENODE_EDITS_DIR_KEY, nameAndEditsDir.toURI().toString(), nameAndEditsDir2.toURI().toString());
            cluster = new MiniDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).manageNameDfsDirs(false).build();
            RestartFramework.at("after_cluster_start_case3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
    ...
    public void testNameEditsConfigsFailure() throws IOException {
        ...
        try {
            ...
            assertTrue(!fileSys.exists(file1));
            DFSTestUtil.createFile(fileSys, file1, FILE_SIZE, FILE_SIZE, BLOCK_SIZE, replication, SEED);
            RestartFramework.at("after_file1_creation_failure_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            checkFile(fileSys, file1, replication);
        } finally {
            fileSys.close();
            cluster.shutdown();
        ...
        try {
            ...
            cleanupFile(fileSys, file1);
            DFSTestUtil.createFile(fileSys, file2, FILE_SIZE, FILE_SIZE, BLOCK_SIZE, replication, SEED);
            RestartFramework.at("after_file2_creation_failure_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            checkFile(fileSys, file2, replication);
        } finally {
            fileSys.close();
            cluster.shutdown();
        ...
        try {
            ...
            cleanupFile(fileSys, file2);
            DFSTestUtil.createFile(fileSys, file3, FILE_SIZE, FILE_SIZE, BLOCK_SIZE, replication, SEED);
            RestartFramework.at("after_file3_creation_failure_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            checkFile(fileSys, file3, replication);
        } finally {
            fileSys.close();
            cluster.shutdown();
        ...
        try {
            ...
            DFSTestUtil.createFile(fileSys, file3, FILE_SIZE, FILE_SIZE, BLOCK_SIZE, replication, SEED);
            RestartFramework.at("after_file3_recreation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            checkFile(fileSys, file3, replication);
        } finally {
            fileSys.close();
            cluster.shutdown();
