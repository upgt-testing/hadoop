 ...
 */
public class TestDataNodeReconfiguration_RestartInjected_Randomized_456 {
    ...
    public void testMaxConcurrentMoversReconfiguration() throws ReconfigurationException, IOException {
        ...
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            ...
            RestartFramework.at("after_reconfigure_max_movers").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            // verify change
            ...
            RestartFramework.at("after_revert_to_default").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            // verify default
    ...
    public void testFailedDecreaseConcurrentMoversReconfiguration() throws IOException, ReconfigurationException {
        ...
        try {
            ...
            dataNode.reconfigurePropertyImpl(DFS_DATANODE_BALANCE_MAX_NUM_CONCURRENT_MOVES_KEY, "1");
        } catch (ReconfigurationException e) {
            Assert.assertEquals(DFS_DATANODE_BALANCE_MAX_NUM_CONCURRENT_MOVES_KEY, e.getProperty());
    ...
    private void testAcquireOnMaxConcurrentMoversReconfiguration(DataNode dataNode, int maxConcurrentMovers) throws IOException, ReconfigurationException {
        final int defaultMaxThreads = dataNode.getConf().getInt(DFSConfigKeys.DFS_DATANODE_BALANCE_MAX_NUM_CONCURRENT_MOVES_KEY, DFSConfigKeys.DFS_DATANODE_BALANCE_MAX_NUM_CONCURRENT_MOVES_DEFAULT);
        /**
    ...
    public void testBlockReportIntervalReconfiguration() throws ReconfigurationException {
        ...
        String[] blockReportParameters = { DFS_BLOCKREPORT_INTERVAL_MSEC_KEY, DFS_BLOCKREPORT_SPLIT_THRESHOLD_KEY, DFS_BLOCKREPORT_INITIAL_DELAY_KEY };
        RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            DataNode dn = cluster.getDataNodes().get(i);
            ...
            for (BPOfferService bpos : blockPoolManager.getAllNamenodeThreads()) {
                if (bpos != null) {
                    for (BPServiceActor actor : bpos.getBPServiceActors()) {
                        assertEquals(String.format("%s has wrong value", DFS_BLOCKREPORT_INTERVAL_MSEC_KEY), blockReportInterval, actor.getScheduler().getBlockReportIntervalMs());
                    }
                }
            }
            ...
            RestartFramework.at("after_reconfigure_block_report").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            // Revert to default and verify default.
            dn.reconfigureProperty(DFS_BLOCKREPORT_INTERVAL_MSEC_KEY, null);
            for (BPOfferService bpos : blockPoolManager.getAllNamenodeThreads()) {
                if (bpos != null) {
                    for (BPServiceActor actor : bpos.getBPServiceActors()) {
                        assertEquals(String.format("%s has wrong value", DFS_BLOCKREPORT_INTERVAL_MSEC_KEY), DFS_BLOCKREPORT_INTERVAL_MSEC_DEFAULT, actor.getScheduler().getBlockReportIntervalMs());
                    }
                }
            }
            ...
            assertNull(String.format("expect %s is not configured", DFS_BLOCKREPORT_SPLIT_THRESHOLD_KEY), dn.getConf().get(DFS_BLOCKREPORT_SPLIT_THRESHOLD_KEY));
            dn.reconfigureProperty(DFS_BLOCKREPORT_INITIAL_DELAY_KEY, null);
            assertNull(String.format("expect %s is not configured", DFS_BLOCKREPORT_INITIAL_DELAY_KEY), dn.getConf().get(DFS_BLOCKREPORT_INITIAL_DELAY_KEY));
        }
    }
    ...
    public void testDataXceiverReconfiguration() throws ReconfigurationException {
        ...
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            ...
            RestartFramework.at("after_reconfigure_receiver_threads").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(String.format("%s has wrong value", DFS_DATANODE_MAX_RECEIVER_THREADS_KEY), 123, dn.getXferServer().getMaxXceiverCount());
            // Revert to default.
            dn.reconfigureProperty(DFS_DATANODE_MAX_RECEIVER_THREADS_KEY, null);
            assertEquals(String.format("%s has wrong value", DFS_DATANODE_MAX_RECEIVER_THREADS_KEY), DFS_DATANODE_MAX_RECEIVER_THREADS_DEFAULT, dn.getXferServer().getMaxXceiverCount());
            assertNull(String.format("expect %s is not configured", DFS_DATANODE_MAX_RECEIVER_THREADS_KEY), dn.getConf().get(DFS_DATANODE_MAX_RECEIVER_THREADS_KEY));
        }
    }
    ...
    public void testCacheReportReconfiguration() throws ReconfigurationException {
        ...
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            ...
            // Change properties.
            dn.reconfigureProperty(DFS_CACHEREPORT_INTERVAL_MSEC_KEY, String.valueOf(cacheReportInterval));
            RestartFramework.at("after_reconfigure_cache_report").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            // Verify change.
            assertEquals(String.format("%s has wrong value", DFS_CACHEREPORT_INTERVAL_MSEC_KEY), cacheReportInterval, dn.getDnConf().getCacheReportInterval());
            // Revert to default.
            dn.reconfigureProperty(DFS_CACHEREPORT_INTERVAL_MSEC_KEY, null);
            assertEquals(String.format("%s has wrong value", DFS_CACHEREPORT_INTERVAL_MSEC_KEY), DFS_CACHEREPORT_INTERVAL_MSEC_DEFAULT, dn.getDnConf().getCacheReportInterval());
            assertNull(String.format("expect %s is not configured", DFS_CACHEREPORT_INTERVAL_MSEC_KEY), dn.getConf().get(DFS_CACHEREPORT_INTERVAL_MSEC_KEY));
        }
    }

    @Test
    public void testSlowPeerParameters() throws Exception {
        String[] slowPeersParameters = { DFS_DATANODE_MIN_OUTLIER_DETECTION_NODES_KEY, DFS_DATANODE_SLOWPEER_LOW_THRESHOLD_MS_KEY, DFS_DATANODE_PEER_METRICS_MIN_OUTLIER_DETECTION_SAMPLES_KEY };
        RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            DataNode dn = cluster.getDataNodes().get(i);
            // Try invalid values.
            LambdaTestUtils.intercept(ReconfigurationException.class, "Could not change property dfs.datanode.peer.stats.enabled from 'true' to 'text'", () -> dn.reconfigureProperty(DFS_DATANODE_PEER_STATS_ENABLED_KEY, "text"));
            ...
            RestartFramework.at("after_reconfigure_slow_peer").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_PEER_STATS_ENABLED_KEY), null, dn.getConf().get(DFS_DATANODE_PEER_STATS_ENABLED_KEY));
            // Reset DFS_DATANODE_PEER_STATS_ENABLED_KEY to true.
            dn.reconfigureProperty(DFS_DATANODE_PEER_STATS_ENABLED_KEY, "true");
            for (String parameter : slowPeersParameters) {
                dn.reconfigureProperty(parameter, null);
            }
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_MIN_OUTLIER_DETECTION_NODES_KEY), null, dn.getConf().get(DFS_DATANODE_MIN_OUTLIER_DETECTION_NODES_KEY));
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_SLOWPEER_LOW_THRESHOLD_MS_KEY), null, dn.getConf().get(DFS_DATANODE_SLOWPEER_LOW_THRESHOLD_MS_KEY));
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_PEER_METRICS_MIN_OUTLIER_DETECTION_SAMPLES_KEY), null, dn.getConf().get(DFS_DATANODE_PEER_METRICS_MIN_OUTLIER_DETECTION_SAMPLES_KEY));
            assertEquals(dn.getPeerMetrics().getSlowNodeDetector().getMinOutlierDetectionNodes(), DFS_DATANODE_MIN_OUTLIER_DETECTION_NODES_DEFAULT);
            assertEquals(dn.getPeerMetrics().getSlowNodeDetector().getLowThresholdMs(), DFS_DATANODE_SLOWPEER_LOW_THRESHOLD_MS_DEFAULT);
        }
    }
    ...
    public void testSlowDiskParameters() throws ReconfigurationException, IOException {
        RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String[] slowDisksParameters1 = { DFS_DATANODE_MIN_OUTLIER_DETECTION_DISKS_KEY, DFS_DATANODE_SLOWDISK_LOW_THRESHOLD_MS_KEY };
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            DataNode dn = cluster.getDataNodes().get(i);
            // Try invalid values.
            try {
            ...
            for (BPOfferService bpos : blockPoolManager.getAllNamenodeThreads()) {
                if (bpos != null) {
                    for (BPServiceActor actor : bpos.getBPServiceActors()) {
                        assertEquals(String.format("%s has wrong value", DFS_DATANODE_OUTLIERS_REPORT_INTERVAL_KEY), 1, actor.getScheduler().getOutliersReportIntervalMs());
                    }
                }
            }
            String[] slowDisksParameters2 = { DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, DFS_DATANODE_MIN_OUTLIER_DETECTION_DISKS_KEY, DFS_DATANODE_SLOWDISK_LOW_THRESHOLD_MS_KEY };
            for (String parameter : slowDisksParameters2) {
                dn.reconfigureProperty(parameter, "99");
            }
            RestartFramework.at("after_reconfigure_slow_disk").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            // Assert diskMetrics.
            ...
            assertEquals((int) ((double) 99 / 100 * Integer.MAX_VALUE), dn.getFileIoProvider().getProfilingEventHook().getSampleRangeMax());
            ...
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_OUTLIERS_REPORT_INTERVAL_KEY), null, dn.getConf().get(DFS_DATANODE_OUTLIERS_REPORT_INTERVAL_KEY));
            dn.reconfigureProperty(DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, null);
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY), null, dn.getConf().get(DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY));
            ...
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_MIN_OUTLIER_DETECTION_DISKS_KEY), null, dn.getConf().get(DFS_DATANODE_MIN_OUTLIER_DETECTION_DISKS_KEY));
            assertEquals(String.format("expect %s is not configured", DFS_DATANODE_SLOWDISK_LOW_THRESHOLD_MS_KEY), null, dn.getConf().get(DFS_DATANODE_SLOWDISK_LOW_THRESHOLD_MS_KEY));
    ...
    public void testDfsUsageParameters() throws ReconfigurationException {
        String[] dfsUsageParameters = { FS_DU_INTERVAL_KEY, FS_GETSPACEUSED_JITTER_KEY };
        RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            DataNode dn = cluster.getDataNodes().get(i);
            // Try invalid values.
            ...
            RestartFramework.at("after_reconfigure_dfs_usage").on(cluster).restart("datanode").withIndex(i).withMode(RestartMode.GRACEFUL).execute();
            ...
            for (FsVolumeImpl fsVolume : volumeList) {
                ...
                for (Map.Entry<String, BlockPoolSlice> entry : blockPoolSlices.entrySet()) {
                    ...
                    if (dfsUsage instanceof CachingGetSpaceUsed) {
                        assertEquals(99, ((CachingGetSpaceUsed) entry.getValue().getDfsUsage()).getRefreshInterval());
            ...
            for (FsVolumeImpl fsVolume : volumeList) {
                ...
                for (Map.Entry<String, BlockPoolSlice> entry : blockPoolSlices.entrySet()) {
                    GetSpaceUsed dfsUsage = entry.getValue().getDfsUsage();
                    if (dfsUsage instanceof CachingGetSpaceUsed) {
                        assertEquals(String.format("expect %s is not configured", FS_DU_INTERVAL_KEY), FS_DU_INTERVAL_DEFAULT, ((CachingGetSpaceUsed) entry.getValue().getDfsUsage()).getRefreshInterval());
                        assertEquals(String.format("expect %s is not configured", FS_GETSPACEUSED_JITTER_KEY), FS_GETSPACEUSED_JITTER_DEFAULT, ((CachingGetSpaceUsed) entry.getValue().getDfsUsage()).getJitter());
                    }
                    assertEquals(String.format("expect %s is not configured", FS_DU_INTERVAL_KEY), null, dn.getConf().get(FS_DU_INTERVAL_KEY));
                    assertEquals(String.format("expect %s is not configured", FS_GETSPACEUSED_JITTER_KEY), null, dn.getConf().get(FS_GETSPACEUSED_JITTER_KEY));
                }
            }
        }
    }
    ...
    public void testDfsUsageKlass() throws ReconfigurationException, InterruptedException {
        ...
        RestartFramework.at("after_reconfigure_usage_class").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        lastCounter = counter;
        RestartFramework.at("after_cluster_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(5000);
