...
public class TestDataNodeReconfiguration {
    ...
    private void startDFSCluster(int numNameNodes, int numDataNodes) throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(nnTopology).numDataNodes(numDataNodes).build();
        cluster.waitActive();
    }
    ...
    public void testMaxConcurrentMoversReconfiguration() throws ReconfigurationException, IOException {
        ...
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            DataNodeInterface dn = cluster.getDataNodes().get(i);
            // try invalid values
            try {
    ...
    public void testFailedDecreaseConcurrentMovers() throws IOException, ReconfigurationException {
        final DataNode[] dns = createDNsForTest(1);
        final DataNodeInterface dataNode = dns[0];
        try {
    ...
    public void testBlockReportIntervalReconfiguration() throws ReconfigurationException {
        ...
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            DataNodeInterface dn = cluster.getDataNodes().get(i);
            BlockPoolManagerInterface blockPoolManager = dn.getBlockPoolManager();
            // Try invalid values.
            ...
            for (BPOfferServiceInterface bpos : blockPoolManager.getAllNamenodeThreads()) {
                if (bpos != null) {
                    for (BPServiceActorInterface actor : bpos.getBPServiceActors()) {
    ...
    public void testDfsUsageParameters() throws ReconfigurationException {
        ...
        for (int i = 0; i < NUM_DATA_NODE; i++) {
            ...
            for (FsVolumeImpl fsVolume : volumeList) {
                ...
                for (Map.EntryInterface<String, BlockPoolSlice> entry : blockPoolSlices.entrySet()) {
