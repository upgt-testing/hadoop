...
public class TestEditLogTailer {
    ...
    public void testTailer() throws IOException, InterruptedException, ServiceFailedException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
        cluster.waitActive();
    ...
    public void testTailerBackoff() throws Exception {
        ...
        conf.setTimeDuration(DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_KEY, 1, TimeUnit.MILLISECONDS);
        conf.setTimeDuration(DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_BACKOFF_MAX_KEY, 10, TimeUnit.MILLISECONDS);
        FSNamesystemInterface mockNamesystem = mock(FSNamesystem.class);
        FSImageInterface mockImage = mock(FSImage.class);
        NNStorageInterface mockStorage = mock(NNStorage.class);
        ...
        tailer.start();
        try {
            GenericTestUtils.waitFor(() -> sleepDurations.size() > zeroEditCount, 50, 10000);
        } finally {
            tailer.stop();
        }
    ...
    private static void testStandbyTriggersLogRolls(int activeIndex) throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            try {
                cluster = createMiniDockerDFSCluster(conf, 3);
                break;
   ...
   */
    ...
    private static void waitForLogRollInSharedDir(MiniDockerDFSCluster cluster, long startTxId) throws Exception {
    ...
    public void testRollEditTimeoutForActiveNN() throws IOException {
        ...
        cluster.transitionToActive(0);
        try {
            EditLogTailerInterface tailer = Mockito.spy(cluster.getNamesystem(1).getEditLogTailer());
    ...
    public void testStandbyTriggersLogRollsWhenTailInProgressEdits() throws Exception {
        ...
        MiniDockerDFSCluster cluster = createMiniDockerDFSCluster(conf, 2);
        if (cluster == null) {
            fail("failed to start mini cluster.");
        }
        try {
            ...
            cluster.transitionToActive(activeIndex);
            NameNodeInterface active = cluster.getNameNode(activeIndex);
            NameNodeInterface standby = cluster.getNameNode(standbyIndex);
            long origTxId = active.getNamesystem().getFSImage().getEditLog().getCurSegmentTxId();
            for (int i = 0; i < DIRS_TO_MAKE / 2; i++) {
                NameNodeAdapter.mkdirs(active, getDirPath(i), new PermissionStatus("test", "test", new FsPermission((short) 00755)), true);
            }
            long activeTxId = active.getNamesystem().getFSImage().getEditLog().getLastWrittenTxId();
            waitForStandbyToCatchUpWithInProgressEdits(standby, activeTxId, standbyCatchupWaitTime);
            for (int i = DIRS_TO_MAKE / 2; i < DIRS_TO_MAKE; i++) {
                NameNodeAdapter.mkdirs(active, getDirPath(i), new PermissionStatus("test", "test", new FsPermission((short) 00755)), true);
            }
    ...
    private static MiniDockerDFSCluster createMiniDockerDFSCluster(Configuration conf, int nnCount) throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(topology).numDataNodes(0).build();
        return cluster;
    }
}
