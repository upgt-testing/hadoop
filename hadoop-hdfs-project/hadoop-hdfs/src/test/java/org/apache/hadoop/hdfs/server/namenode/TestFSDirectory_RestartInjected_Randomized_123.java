 ...
 */
public class TestFSDirectory_RestartInjected_Randomized_123 {

    public static final Logger LOG = LoggerFactory.getLogger(TestFSDirectory_RestartInjected.class);
    ...
    public void testDumpTree() throws Exception {
        ...
        checkClassName(line);
        RestartFramework.at("after_tree_dump").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSkipQuotaCheck() throws Exception {
        try {
            ...
            RestartFramework.at("after_disable_quota").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DFSTestUtil.createFile(hdfs, file6, 1024, REPLICATION, seed);
            ...
            RestartFramework.at("after_enable_quota").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
                // this should fail
                DFSTestUtil.createFile(hdfs, file6, 1024, REPLICATION, seed);
                throw new IOException("The create should have failed.");
    ...
    public void testINodeXAttrsLimit() throws Exception {
        ...
        RestartFramework.at("after_xattr_initialization").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(xAttrs.size(), 4);
        RestartFramework.at("after_system_raw_xattr_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testXAttrMultiSetRemove() throws Exception {
        ...
        RestartFramework.at("after_all_xattrs_added").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_all_xattrs_removed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_xattr_operations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        while (numExpectedXAttrs < numGeneratedXAttrs) {
            ...
            if (numExpectedXAttrs == numGeneratedXAttrs / 2) {
                RestartFramework.at("during_xattr_addition").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
        ...
        while (numExpectedXAttrs > 0) {
            ...
            existingXAttrs = newXAttrs;
            if (numExpectedXAttrs == numGeneratedXAttrs / 2) {
                RestartFramework.at("during_xattr_removal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
    ...
    public void testXAttrMultiAddRemoveErrors() throws Exception {
        ...
        RestartFramework.at("after_error_tests").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        toAdd.add(generatedXAttrs.get(2));
        toAdd.add(generatedXAttrs.get(0));
        RestartFramework.at("after_create_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
        ...
        RestartFramework.at("before_error_tests").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
        ...
        RestartFramework.at("after_replace_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        existingXAttrs = newXAttrs;
    ...
    public void testVerifyParentDir() throws Exception {
        ...
        RestartFramework.at("after_path_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fsdir.verifyParentDir(iip);
