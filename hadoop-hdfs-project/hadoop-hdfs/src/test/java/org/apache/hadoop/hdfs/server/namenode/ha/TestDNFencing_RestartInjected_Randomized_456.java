...
public class TestDNFencing_RestartInjected_Randomized_456 {

    protected static final Logger LOG = LoggerFactory.getLogger(TestDNFencing_RestartInjected.class);
    ...
    public void testDnFencing() throws Exception {
        ...
        doMetasave(nn2);
        RestartFramework.at("after_failover_to_nn2").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNNClearsCommandsOnFailoverAfterStartup() throws Exception {
        ...
        doMetasave(nn2);
        banner("Triggering heartbeats and block reports so that fencing is completed");
        RestartFramework.at("after_create_file_failover").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.triggerHeartbeats();
        cluster.triggerBlockReports();
        banner("Metadata after nodes have all block-reported");
        doMetasave(nn2);
    ...
    public void testNNClearsCommandsOnFailoverWithReplChanges() throws Exception {
        ...
        RestartFramework.at("after_failover_repl_changes").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // Check that the standby picked up the replication change.
        assertEquals(1, nn2.getRpcServer().getFileInfo(TEST_FILE).getReplication());
        // Dump some info for debugging purposes.
        banner("Metadata immediately after failover");
    ...
    public void testBlockReportsWhileFileBeingWritten() throws Exception {
        ...
        RestartFramework.at("after_file_write").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.transitionToActive(1);
    ...
    public void testQueueingWithAppend() throws Exception {
        ...
        BlockManagerTestUtil.updateState(nn1.getNamesystem().getBlockManager());
        BlockManagerTestUtil.updateState(nn2.getNamesystem().getBlockManager());
        RestartFramework.at("after_append").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(0, nn1.getNamesystem().getCorruptReplicaBlocks());
        assertEquals(0, nn2.getNamesystem().getCorruptReplicaBlocks());
    ...
    public void testRBWReportArrivesAfterEdits() throws Exception {
        ...
        FSDataOutputStream out = fs.create(TEST_FILE_PATH);
        RestartFramework.at("after_transition_rbw").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        try {
            AppendTestUtil.write(out, 0, 10);
            out.hflush();
