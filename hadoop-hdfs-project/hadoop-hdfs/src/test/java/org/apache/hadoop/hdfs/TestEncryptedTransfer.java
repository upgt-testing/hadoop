...
public class TestEncryptedTransfer {
    ...
    private FileChecksum writeUnencryptedAndThenRestartEncryptedCluster() throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).manageDataDfsDirs(false).manageNameDfsDirs(false).format(false).startupOption(StartupOption.REGULAR).build();
        fs = getFileSystem(conf);
        return checksum;
    }
    ...
    public void testEncryptedAppendRequiringBlockTransfer() throws IOException {
        ...
        DataNodeInterface dn = cluster.getDataNode(locatedBlocks.get(0).getLocations()[0].getIpcPort());
        dn.shutdown();
