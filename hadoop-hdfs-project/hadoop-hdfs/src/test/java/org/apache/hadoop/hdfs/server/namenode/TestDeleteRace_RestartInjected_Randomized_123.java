 ...
 * whole duration.
 */
public class TestDeleteRace_RestartInjected_Randomized_123 {

    private static final int BLOCK_SIZE = 4096;

    private static final Logger LOG = LoggerFactory.getLogger(TestDeleteRace_RestartInjected.class);
    ...
    private void testDeleteAddBlockRace(boolean hasSnapshot) throws Exception {
        try {
            ...
            deleteThread.start();
            RestartFramework.at("after_delete_thread_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
    ...
    public void testRenameRace() throws Exception {
        try {
            ...
            renameThread.start();
            RestartFramework.at("after_rename_thread_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testDeleteAndCommitBlockSynchronizationRace(boolean hasSnapshot) throws Exception {
        ...
        try {
            ...
            for (AbstractMap.SimpleImmutableEntry<String, Boolean> stest : testList) {
                ...
                RestartFramework.at("after_file_creation_in_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                // write a half block
                AppendTestUtil.write(stm, 0, BLOCK_SIZE / 2);
                stm.hflush();
                RestartFramework.at("after_hflush_in_loop").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                if (hasSnapshot) {
                    SnapshotTestHelper.createSnapshot(fs, rootPath, "st" + String.valueOf(stId));
                    ++stId;
                    RestartFramework.at("after_snapshot_creation_in_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
                ...
                RestartFramework.at("after_lease_recovery_in_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                LOG.info("Waiting for commitBlockSynchronization call from primary");
                delayer.waitForCall();
                LOG.info("Deleting recursively " + grandestNonRootParent);
                fs.delete(grandestNonRootParent, true);
                RestartFramework.at("after_delete_in_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDeleteAndLeaseRecoveryHardLimitSnapshot() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_block_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testOpenRenameRace() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_file_creation").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            open.join();
            rename.join();
            RestartFramework.at("after_threads_finish").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            status = hdfs.getFileStatus(new Path(dst));
            assertNotEquals(accessTime, status.getAccessTime());
            RestartFramework.at("before_namenode_restart").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDeleteOnSnapshottableDir() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_directory_snapshot_allowed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_failed_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // create files and create snapshots
            DFSTestUtil.createFile(hdfs, file1, BLOCK_SIZE, (short) 1, 0);
            hdfs.createSnapshot(dir, "s1");
            RestartFramework.at("after_file1_and_snapshot_s1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DFSTestUtil.createFile(hdfs, file2, BLOCK_SIZE, (short) 1, 0);
            hdfs.createSnapshot(dir, "s2");
            RestartFramework.at("after_file2_and_snapshot_s2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // should able to get snapshot diff on ancestor dir
            Path dirDir1 = new Path(dir, "dir1");
            hdfs.mkdirs(dirDir1);
            RestartFramework.at("after_subdirectory_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
