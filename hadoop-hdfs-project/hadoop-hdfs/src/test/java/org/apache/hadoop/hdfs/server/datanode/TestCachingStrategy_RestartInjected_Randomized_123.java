...
public class TestCachingStrategy_RestartInjected_Randomized_123 {
    ...
    public void testFadviseSkippedForSmallReads() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // read file
            fis = fs.open(new Path(TEST_PATH));
            byte[] buf = new byte[17];
            fis.readFully(4096, buf, 0, buf.length);
            RestartFramework.at("after_small_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSeekAfterSetDropBehind() throws Exception {
        ...
        try {
            ...
            try (FSDataInputStream fis = fs.open(new Path(TEST_PATH))) {
                ...
                // seek
                fis.seek(2);
                RestartFramework.at("after_seek").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        } finally {
            if (cluster != null) {
                cluster.shutdown();
