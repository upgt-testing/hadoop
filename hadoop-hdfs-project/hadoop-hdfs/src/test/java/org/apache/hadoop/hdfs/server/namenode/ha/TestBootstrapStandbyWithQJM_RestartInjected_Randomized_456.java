 ...
 */
public class TestBootstrapStandbyWithQJM_RestartInjected_Randomized_456 {

    enum UpgradeState {

        NORMAL, RECOVER, FORMAT
    }
    ...
    public void testBootstrapStandbyWithStandbyNN() throws Exception {
        ...
        RestartFramework.at("after_bootstrap_standbys").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    private void testUpgrade(UpgradeState state) throws Exception {
        ...
        RestartFramework.at("after_set_upgrade_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        switch(state) {
            case RECOVER:
        ...
        if (state != UpgradeState.NORMAL) {
            RestartFramework.at("after_rename_directory").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        int rc = BootstrapStandby.run(new String[] { "-force" }, confNN1);
        assertEquals(0, rc);
        RestartFramework.at("after_bootstrap").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Should have copied over the namespace from the standby
        ...
        RestartFramework.at("after_restart_nn1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
