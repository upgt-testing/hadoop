...
public class TestClientProtocolForPipelineRecovery {
    ...
    public void testEvictWriter() throws Exception {
        ...
        MiniDockerDFSCluster cluster = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes((int) 3).build();
            cluster.waitActive();
            FileSystem fs = cluster.getFileSystem();
    ...
    public void testPipelineRecoveryOnDatanodeUpgrade() throws Exception {
        ...
        try {
            ...
            MiniDockerDFSCluster.DataNodeProperties dnProps = cluster.stopDataNodeForUpgrade(0);
            GenericTestUtils.waitForThreadTermination("Async datanode shutdown thread", 100, 10000);
    ...
    public void testPipelineRecoveryOnRemoteDatanodeUpgrade() throws Exception {
        ...
        try {
            ...
            for (DatanodeInfo node : pipeline) {
                ...
                final long oldGs = out.getBlock().getGenerationStamp();
                MiniDockerDFSCluster.DataNodeProperties dnProps = cluster.stopDataNodeForUpgrade(indexToShutdown);
                GenericTestUtils.waitForThreadTermination("Async datanode shutdown thread", 100, 10000);
                cluster.restartDataNode(dnProps, true);
                cluster.waitActive();
                // wait pipeline to be recovered
    ...
    public void testUpdatePipeLineAfterDNReg() throws Exception {
        ...
        try {
            ...
            DatanodeDescriptorInterface dn1Desc = cluster.getNamesystem(0).getBlockManager().getDatanodeManager().getDatanode(dn1.getDatanodeId());
