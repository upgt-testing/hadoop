    ...
    public void testAddingDatanodeDuringClosing() throws Exception {
        ...
        try {
            ...
            GenericTestUtils.waitFor(new Supplier<Boolean>() {
                ...
                @Override
                public Boolean get() {
                    if (((DFSOutputStream) out.getWrappedStream()).getStreamer().getNodes() != null) {
                        return true;
                    }
                    return false;
                }
            ...
            DatanodeInfo[] pipeline = ((DFSOutputStream) out.getWrappedStream()).getStreamer().getNodes();
            DataNodeJVMInterface[] dataNodes = new DataNode[3];
            int i = 0;
            ...
            new Thread(() -> {
                try {
                    GenericTestUtils.waitFor(new Supplier<Boolean>() {
                        ...
                        @Override
                        public Boolean get() {
                            if (((DFSOutputStream) out.getWrappedStream()).getStreamer().getStage() == BlockConstructionStage.PIPELINE_CLOSE) {
                                return true;
                            }
                            return false;
                        }
