...
public class TestNameEditsConfigs {
    ...
    public void testNameEditsConfigs() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).manageNameDfsDirs(false).build();
        cluster.waitActive();
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).format(false).manageNameDfsDirs(false).build();
        cluster.waitActive();
        secondary = startSecondaryNameNode(conf);
        fileSys = cluster.getFileSystem();
        try {
    ...
    public void testCheckPointDirsAreTrimmed() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).manageNameDfsDirs(false).numDataNodes(3).build();
        try {
            cluster.waitActive();
            secondary = startSecondaryNameNode(conf);
            secondary.doCheckpoint();
