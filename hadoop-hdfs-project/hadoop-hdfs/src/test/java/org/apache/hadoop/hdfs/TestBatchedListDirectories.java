...
public class TestBatchedListDirectories {
    ...
    private static void assertSubDirEquals(int i, int j, Path p) {
        ...
        assertEquals("Unexpected subdir name", expected.toString(), p.toUri().getPath());
    }

    private static void assertFileEquals(int i, int j, int k, Path p) {
        assertTrue(p.toString().startsWith("hdfs://"));
        Path expected = getFileName(i, j, k);
        assertEquals("Unexpected file name", expected.toString(), p.toUri().getPath());
    }
    ...
    private static List<PartialListing<FileStatus>> getListings(List<Path> paths) throws IOException {
        ...
        RemoteIterator<PartialListing<FileStatus>> it = dfs.batchedListStatusIterator(paths);
        while (it.hasNext()) {
    ...
    public void listDirRelative() throws Exception {
        ...
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals("Wrong number of items", SECOND_LEVEL_DIRS, statuses.size());
    ...
    public void listFilesRelative() throws Exception {
        ...
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals("Wrong number of items", FILES_PER_DIR, statuses.size());
    ...
    public void testDFSHasCapability() throws Throwable {
        assertTrue("FS does not declare PathCapability support", dfs.hasPathCapability(new Path("/"), CommonPathCapabilities.FS_EXPERIMENTAL_BATCH_LISTING));
    }
    ...
    public void listTooManyDirectories() throws Exception {
        ...
        thrown.expectMessage(StringContains.containsString("Too many source paths"));
    ...
    public void listDirsAndEmpty() throws Exception {
        ...
        assertEquals(FILE_PATHS.get(0).toString(), listings.get(1).get().get(0).getPath().toUri().getPath());
    ...
    public void listLocatedStatus() throws Exception {
        ...
        RemoteIterator<PartialListing<LocatedFileStatus>> it = dfs.batchedListLocatedStatusIterator(paths);
    ...
    private void listAsNormalUser(List<Path> paths) throws IOException, InterruptedException {
        final UserGroupInformation ugi = UserGroupInformation.createRemoteUser("tiffany");
        ugi.doAs(new PrivilegedExceptionAction<Void>() {

            @Override
            public Void run() throws Exception {
                // try renew with long name
                DistributedFileSystem fs = (DistributedFileSystem) FileSystem.get(cluster.getURI(), conf);
    ...
    @Test
    public void testEmptyPath_withUpgrade20() throws Exception {
        thrown.expect(FileNotFoundException.class);
        List<Path> paths = Lists.newArrayList();
    ...
    @Test
    public void testEmptyPath_withUpgrade60() throws Exception {
        thrown.expect(FileNotFoundException.class);
        List<Path> paths = Lists.newArrayList();
        getStatuses(paths);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testEmptyDir_withUpgrade20() throws Exception {
        List<Path> paths = Lists.newArrayList(EMPTY_DIR_PATH);
        List<PartialListing<FileStatus>> listings = getListings(paths);
        assertEquals(1, listings.size());
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void testEmptyDir_withUpgrade40() throws Exception {
        List<Path> paths = Lists.newArrayList(EMPTY_DIR_PATH);
        List<PartialListing<FileStatus>> listings = getListings(paths);
        assertEquals(1, listings.size());
        PartialListing<FileStatus> listing = listings.get(0);
    ...
    @Test
    public void testEmptyDir_withUpgrade60() throws Exception {
        List<Path> paths = Lists.newArrayList(EMPTY_DIR_PATH);
        List<PartialListing<FileStatus>> listings = getListings(paths);
        assertEquals(1, listings.size());
        PartialListing<FileStatus> listing = listings.get(0);
    ...
    @Test
    public void testEmptyDir_withUpgrade80() throws Exception {
        List<Path> paths = Lists.newArrayList(EMPTY_DIR_PATH);
        List<PartialListing<FileStatus>> listings = getListings(paths);
        assertEquals(1, listings.size());
        PartialListing<FileStatus> listing = listings.get(0);
    ...
    @Test
    public void listOneFile_withUpgrade20() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(FILE_PATHS.get(0));
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void listOneFile_withUpgrade40() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(FILE_PATHS.get(0));
        List<FileStatus> statuses = getStatuses(paths);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listOneFile_withUpgrade60() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(FILE_PATHS.get(0));
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals(1, statuses.size());
    ...
    @Test
    public void listOneFile_withUpgrade80() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(FILE_PATHS.get(0));
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals(1, statuses.size());
    ...
    @Test
    public void listDoesNotExist_withUpgrade20() throws Exception {
        thrown.expect(FileNotFoundException.class);
        List<Path> paths = Lists.newArrayList();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void listDoesNotExist_withUpgrade40() throws Exception {
        thrown.expect(FileNotFoundException.class);
        List<Path> paths = Lists.newArrayList();
        paths.add(new Path("/does/not/exist"));
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listDoesNotExist_withUpgrade80() throws Exception {
        thrown.expect(FileNotFoundException.class);
        List<Path> paths = Lists.newArrayList();
        paths.add(new Path("/does/not/exist"));
        getStatuses(paths);
    ...
    @Test
    public void listSomeDoNotExist_withUpgrade20() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(new Path("/does/not/exist"));
        paths.addAll(SUBDIR_PATHS.subList(0, FIRST_LEVEL_DIRS));
        paths.add(new Path("/does/not/exist"));
    ...
    @Test
    public void listSomeDoNotExist_withUpgrade40() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(new Path("/does/not/exist"));
        paths.addAll(SUBDIR_PATHS.subList(0, FIRST_LEVEL_DIRS));
        paths.add(new Path("/does/not/exist"));
    ...
    @Test
    public void listSomeDoNotExist_withUpgrade60() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(new Path("/does/not/exist"));
        paths.addAll(SUBDIR_PATHS.subList(0, FIRST_LEVEL_DIRS));
        paths.add(new Path("/does/not/exist"));
    ...
    @Test
    public void listSomeDoNotExist_withUpgrade80() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(new Path("/does/not/exist"));
        paths.addAll(SUBDIR_PATHS.subList(0, FIRST_LEVEL_DIRS));
        paths.add(new Path("/does/not/exist"));
    ...
    @Test
    public void listDirRelative_withUpgrade20() throws Exception {
        dfs.setWorkingDirectory(new Path("/dir0"));
        List<Path> paths = Lists.newArrayList(new Path("."));
        List<FileStatus> statuses = getStatuses(paths);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listDirRelative_withUpgrade40() throws Exception {
        dfs.setWorkingDirectory(new Path("/dir0"));
        List<Path> paths = Lists.newArrayList(new Path("."));
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals("Wrong number of items", SECOND_LEVEL_DIRS, statuses.size());
    ...
    @Test
    public void listDirRelative_withUpgrade60() throws Exception {
        dfs.setWorkingDirectory(new Path("/dir0"));
        List<Path> paths = Lists.newArrayList(new Path("."));
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals("Wrong number of items", SECOND_LEVEL_DIRS, statuses.size());
    ...
    @Test
    public void listFilesRelative_withUpgrade20() throws Exception {
        dfs.setWorkingDirectory(new Path("/dir0"));
        List<Path> paths = Lists.newArrayList(new Path("subdir0"));
        List<FileStatus> statuses = getStatuses(paths);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listFilesRelative_withUpgrade40() throws Exception {
        dfs.setWorkingDirectory(new Path("/dir0"));
        List<Path> paths = Lists.newArrayList(new Path("subdir0"));
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals("Wrong number of items", FILES_PER_DIR, statuses.size());
    ...
    @Test
    public void listFilesRelative_withUpgrade60() throws Exception {
        dfs.setWorkingDirectory(new Path("/dir0"));
        List<Path> paths = Lists.newArrayList(new Path("subdir0"));
        List<FileStatus> statuses = getStatuses(paths);
        assertEquals("Wrong number of items", FILES_PER_DIR, statuses.size());
    ...
    @Test
    public void testDFSHasCapability_withUpgrade20() throws Throwable {
        assertTrue("FS does not declare PathCapability support", dfs.hasPathCapability(new Path("/"), CommonPathCapabilities.FS_EXPERIMENTAL_BATCH_LISTING));
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listOneFiles_withUpgrade20() throws Exception {
        listFilesInternal(1);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listSomeFiles_withUpgrade20() throws Exception {
        listFilesInternal(FILE_PATHS.size() / 2);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listAllFiles_withUpgrade20() throws Exception {
        listFilesInternal(FILE_PATHS.size());
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listOneDirectory_withUpgrade20() throws Exception {
        listDirectoriesInternal(1);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listSomeDirectories_withUpgrade20() throws Exception {
        listDirectoriesInternal(SUBDIR_PATHS.size() / 2);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listAllDirectories_withUpgrade20() throws Exception {
        listDirectoriesInternal(SUBDIR_PATHS.size());
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void listTooManyDirectories_withUpgrade20() throws Exception {
        thrown.expect(RemoteException.class);
        thrown.expectMessage(StringContains.containsString("Too many source paths"));
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void listTooManyDirectories_withUpgrade40() throws Exception {
        thrown.expect(RemoteException.class);
        thrown.expectMessage(StringContains.containsString("Too many source paths"));
        List<Path> paths = Lists.newArrayList(FILE_PATHS);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listTooManyDirectories_withUpgrade60() throws Exception {
        thrown.expect(RemoteException.class);
        thrown.expectMessage(StringContains.containsString("Too many source paths"));
        List<Path> paths = Lists.newArrayList(FILE_PATHS);
        paths.add(SUBDIR_PATHS.get(0));
    ...
    @Test
    public void listTooManyDirectories_withUpgrade80() throws Exception {
        thrown.expect(RemoteException.class);
        thrown.expectMessage(StringContains.containsString("Too many source paths"));
        List<Path> paths = Lists.newArrayList(FILE_PATHS);
        paths.add(SUBDIR_PATHS.get(0));
    ...
    @Test
    public void listDirsAndEmpty_withUpgrade20() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(EMPTY_DIR_PATH);
        paths.add(FILE_PATHS.get(0));
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listDirsAndEmpty_withUpgrade40() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(EMPTY_DIR_PATH);
        paths.add(FILE_PATHS.get(0));
        paths.add(EMPTY_DIR_PATH);
    ...
    @Test
    public void listDirsAndEmpty_withUpgrade60() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(EMPTY_DIR_PATH);
        paths.add(FILE_PATHS.get(0));
        paths.add(EMPTY_DIR_PATH);
    ...
    @Test
    public void listDirsAndEmpty_withUpgrade80() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(EMPTY_DIR_PATH);
        paths.add(FILE_PATHS.get(0));
        paths.add(EMPTY_DIR_PATH);
    ...
    @Test
    public void listSamePaths_withUpgrade20() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(FILE_PATHS.get(0));
    ...
    @Test
    public void listSamePaths_withUpgrade40() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(FILE_PATHS.get(0));
    ...
    @Test
    public void listSamePaths_withUpgrade60() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(FILE_PATHS.get(0));
    ...
    @Test
    public void listSamePaths_withUpgrade80() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(SUBDIR_PATHS.get(0));
        paths.add(FILE_PATHS.get(0));
    ...
    @Test
    public void listLocatedStatus_withUpgrade20() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(DATA_FILE_PATH);
        RemoteIterator<PartialListing<LocatedFileStatus>> it = dfs.batchedListLocatedStatusIterator(paths);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listLocatedStatus_withUpgrade40() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(DATA_FILE_PATH);
        RemoteIterator<PartialListing<LocatedFileStatus>> it = dfs.batchedListLocatedStatusIterator(paths);
        PartialListing<LocatedFileStatus> listing = it.next();
    ...
    @Test
    public void listLocatedStatus_withUpgrade60() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(DATA_FILE_PATH);
        RemoteIterator<PartialListing<LocatedFileStatus>> it = dfs.batchedListLocatedStatusIterator(paths);
        PartialListing<LocatedFileStatus> listing = it.next();
    ...
    @Test
    public void listLocatedStatus_withUpgrade80() throws Exception {
        List<Path> paths = Lists.newArrayList();
        paths.add(DATA_FILE_PATH);
        RemoteIterator<PartialListing<LocatedFileStatus>> it = dfs.batchedListLocatedStatusIterator(paths);
        PartialListing<LocatedFileStatus> listing = it.next();
    ...
    @Test
    public void listInaccessibleDir_withUpgrade20() throws Exception {
        thrown.expect(AccessControlException.class);
        List<Path> paths = Lists.newArrayList(INACCESSIBLE_DIR_PATH);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void listInaccessibleDir_withUpgrade60() throws Exception {
        thrown.expect(AccessControlException.class);
        List<Path> paths = Lists.newArrayList(INACCESSIBLE_DIR_PATH);
        listAsNormalUser(paths);
        cluster.restartNodeForTesting(0);
    ...
    @Test
    public void listInaccessibleFile_withUpgrade20() throws Exception {
        thrown.expect(AccessControlException.class);
        List<Path> paths = Lists.newArrayList(INACCESSIBLE_FILE_PATH);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    ...
    @Test
    public void listInaccessibleFile_withUpgrade60() throws Exception {
        thrown.expect(AccessControlException.class);
        List<Path> paths = Lists.newArrayList(INACCESSIBLE_FILE_PATH);
        listAsNormalUser(paths);
        cluster.restartNodeForTesting(0);
