 ...
 */
public class TestMetaSave_RestartInjected_Randomized_456 {
    ...
    public void testMetaSaveOverwrite() throws Exception {
        RestartFramework.at("after_first_metasave").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // metaSave twice.
    ...
    public void testConcurrentMetaSave() throws Exception {
        ...
        RestartFramework.at("after_concurrent_metasave").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 10; i++) {
            threads.add(new MetaSaveThread(nnRpc, "metaSaveConcurrent.out.txt"));
        }
        RestartFramework.at("before_concurrent_metasave").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 10; i++) {
            threads.get(i).start();
        }
        for (int i = 0; i < 10; i++) {
