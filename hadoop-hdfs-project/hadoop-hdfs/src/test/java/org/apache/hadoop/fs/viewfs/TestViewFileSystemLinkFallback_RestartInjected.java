 ...
 */
public class TestViewFileSystemLinkFallback_RestartInjected extends ViewFileSystemBaseTest {
  ...
  private static final File TEST_DIR = GenericTestUtils.getTestDir(
      TestViewFileSystemLinkFallback_RestartInjected.class.getSimpleName());
  private static final String TEST_BASE_PATH =
      "/tmp/TestViewFileSystemLinkFallback";
  private final static Logger LOG = LoggerFactory.getLogger(
      TestViewFileSystemLinkFallback_RestartInjected.class);
  ...
  public void testConfLinkFallback() throws Exception {
    ...
    RestartFramework.at("after_first_file_append")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_second_file_append")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testListingWithFallbackLink() throws Exception {
    ...
    try (FileSystem vfs = FileSystem.get(viewFsUri, conf)) {
      ...
      RestartFramework.at("after_listing_verification")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_child_mkdir")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testListingWithFallbackLinkWithSameMountDirectoryTree()
    ...
    RestartFramework.at("after_permission_set")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
      for (FileStatus stat : vfs.listStatus(
        ...
        if (file1.getName().equals(stat.getPath().getName())) {
          ...
          RestartFramework.at("after_symlink_verification")
              .on(cluster)
              .restart("namenode")
              .withIndex(0)
              .withMode(RestartMode.GRACEFUL)
  ...
  public void testMkdirsOfLinkParentWithFallbackLinkWithSameMountDirectoryTree()
    ...
    try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
      ...
      RestartFramework.at("after_test_mkdir")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testMkdirsOfNewDirWithOutMatchingToMountOrFallbackDirTree()
    ...
    RestartFramework.at("after_fallback_mkdir")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
      ...
      RestartFramework.at("after_user2_mkdir")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testMkdirsOfDeepTreeWithFallbackLinkAndMountPathMatchingDirExist()
    ...
    try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
      ...
      RestartFramework.at("after_deep_mkdir")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testMkdirsShouldReturnFalseWhenFallbackFSNotAvailable()
    ...
    try (FileSystem vfs = FileSystem.get(viewFsDefaultClusterUri, conf)) {
      ...
      RestartFramework.at("after_mkdir_success")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
