 ...
 */
public class TestDFSUpgradeWithHA_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestDFSUpgradeWithHA_RestartInjected.class);
    ...
    public void testCannotFinalizeIfNoActive() throws IOException, URISyntaxException {
        ...
        try {
            ...
            RestartFramework.at("after_mkdir_foo1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Do the upgrade. Shut down NN1 and then restart NN0 with the upgrade
            // flag.
            cluster.shutdownNameNode(1);
            cluster.getNameNodeInfos()[0].setStartOpt(StartupOption.UPGRADE);
            cluster.restartNameNode(0, false);
            RestartFramework.at("after_upgrade_restart_finalize").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertEquals(0, rc);
            RestartFramework.at("after_bootstrap_standby").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Now restart NN1 and make sure that we can do ops against that as well.
            cluster.restartNameNode(1);
            cluster.transitionToStandby(0);
            cluster.transitionToActive(1);
            assertTrue(fs.mkdirs(new Path("/foo4")));
            RestartFramework.at("after_mkdir_foo4").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            assertCTimesEqual(cluster);
    ...
    public void testNfsUpgrade() throws IOException, URISyntaxException {
        ...
        try {
            cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
            RestartFramework.at("after_cluster_setup_nfs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            File sharedDir = new File(cluster.getSharedEditsDir(0, 1));
            // No upgrade is in progress at the moment.
            checkClusterPreviousDirExistence(cluster, false);
            assertCTimesEqual(cluster);
            ...
            fs = HATestUtil.configureFailoverFs(cluster, conf);
            assertTrue(fs.mkdirs(new Path("/foo1")));
            RestartFramework.at("after_mkdir_nfs_upgrade").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Do the upgrade. Shut down NN1 and then restart NN0 with the upgrade
            // flag.
            cluster.shutdownNameNode(1);
            cluster.getNameNodeInfos()[0].setStartOpt(StartupOption.UPGRADE);
            cluster.restartNameNode(0, false);
            RestartFramework.at("after_upgrade_restart_nfs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            checkNnPreviousDirExistence(cluster, 0, true);
            checkNnPreviousDirExistence(cluster, 1, false);
            checkPreviousDirExistence(sharedDir, true);
            // NN0 should come up in the active state when given the -upgrade option,
            ...
            int rc = BootstrapStandby.run(new String[] { "-force" }, cluster.getConfiguration(1));
            assertEquals(0, rc);
            RestartFramework.at("after_bootstrap_nfs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Now restart NN1 and make sure that we can do ops against that as well.
            cluster.restartNameNode(1);
            cluster.transitionToStandby(0);
            cluster.transitionToActive(1);
    ...
    public void testUpgradeWithJournalNodes() throws IOException, URISyntaxException {
        ...
        try {
            ...
            RestartFramework.at("after_qjm_cluster_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // No upgrade is in progress at the moment.
            ...
            fs = HATestUtil.configureFailoverFs(cluster, conf);
            assertTrue(fs.mkdirs(new Path("/foo1")));
            RestartFramework.at("after_mkdir_qjm").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.getNameNodeInfos()[0].setStartOpt(StartupOption.UPGRADE);
            cluster.restartNameNode(0, false);
            RestartFramework.at("after_upgrade_restart_qjm").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            checkNnPreviousDirExistence(cluster, 0, true);
            checkNnPreviousDirExistence(cluster, 1, false);
    ...
    public void testFinalizeWithJournalNodes() throws IOException, URISyntaxException {
        ...
        try {
            ...
            qjCluster = builder.build();
            MiniDFSCluster cluster = qjCluster.getDfsCluster();
            RestartFramework.at("after_cluster_setup_finalize").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // No upgrade is in progress at the moment.
            checkJnPreviousDirExistence(qjCluster, false);
            checkClusterPreviousDirExistence(cluster, false);
            assertCTimesEqual(cluster);
    ...
    public void testFinalizeFromSecondNameNodeWithJournalNodes() throws IOException, URISyntaxException {
        ...
        try {
            ...
            qjCluster = builder.build();
            MiniDFSCluster cluster = qjCluster.getDfsCluster();
            RestartFramework.at("after_cluster_setup_second_nn").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // No upgrade is in progress at the moment.
            checkJnPreviousDirExistence(qjCluster, false);
            checkClusterPreviousDirExistence(cluster, false);
            assertCTimesEqual(cluster);
    ...
    public void testStartingWithUpgradeInProgressSucceeds() throws Exception {
        ...
        try {
            cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
            RestartFramework.at("after_cluster_setup_upgrade_progress").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRollbackWithNfs() throws Exception {
        ...
        try {
            cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
            RestartFramework.at("after_cluster_setup_rollback_nfs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            File sharedDir = new File(cluster.getSharedEditsDir(0, 1));
            // No upgrade is in progress at the moment.
            checkClusterPreviousDirExistence(cluster, false);
            assertCTimesEqual(cluster);
    ...
    public void testRollbackWithJournalNodes() throws IOException, URISyntaxException {
        ...
        try {
            ...
            qjCluster = builder.build();
            MiniDFSCluster cluster = qjCluster.getDfsCluster();
            RestartFramework.at("after_cluster_setup_rollback_jn").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // No upgrade is in progress at the moment.
            checkClusterPreviousDirExistence(cluster, false);
            assertCTimesEqual(cluster);
            checkJnPreviousDirExistence(qjCluster, false);
    ...
    public void testCannotUpgradeSecondNameNode() throws IOException, URISyntaxException {
        ...
        try {
            cluster = new MiniDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
            RestartFramework.at("after_cluster_setup_second_upgrade").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            File sharedDir = new File(cluster.getSharedEditsDir(0, 1));
            // No upgrade is in progress at the moment.
            checkClusterPreviousDirExistence(cluster, false);
            assertCTimesEqual(cluster);
