...
public class TestReadStripedFileWithDecoding {
    ...
    public void testCorruptionECBlockInvalidate() throws Exception {
        ...
        DFSTestUtil.writeFile(dfs, file, bytes);
        int dnIndex = findFirstDataNode(cluster, dfs, file, CELL_SIZE * NUM_DATA_UNITS);
        int dnIndex2 = findDataNodeAtIndex(cluster, dfs, file, CELL_SIZE * NUM_DATA_UNITS, 2);
        Assert.assertNotEquals(-1, dnIndex);
        ...
        File blkFile2 = MiniDFSClusterInJVM.getBlockFile(storageDir2, blks[1].getBlock());
