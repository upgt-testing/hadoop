...
public class TestAppendDifferentChecksum {
    ...
    public void testChecksumErrorAppendWhileTransfer() throws Exception {
        ...
        LogCapturer logCapturer = GenericTestUtils.LogCapturer.captureLogs(DataNode.LOG);
        try {
            Path f = new Path("/f");
            FSDataOutputStream o = fs.create(f, false, 1024, (short) 1, 128 * 1024 * 1024);
            try {
            ...
            try {
                ...
                final ExtendedBlock b = cluster.getFileSystem().getClient().getLocatedBlocks(f.toString(), 0).get(0).getBlock();
                Thread.sleep(1000);
                /*
         ...
         */
            ...
            Assert.assertFalse("There should not be any checkum exception thrown", logs.contains("ChecksumException"));
        }
    }
    ...
    @Test(timeout = 60000)
    public void testChecksumErrorAppendWhileTransfer_withUpgrade20() throws Exception {
        DataNodeFaultInjector oldFi = DataNodeFaultInjector.get();
        LogCapturer logCapturer = GenericTestUtils.LogCapturer.captureLogs(DataNode.LOG);
        try {
            Path f = new Path("/f");
    ...
    @Test(timeout = 60000)
    public void testChecksumErrorAppendWhileTransfer_withUpgrade40() throws Exception {
        DataNodeFaultInjector oldFi = DataNodeFaultInjector.get();
        LogCapturer logCapturer = GenericTestUtils.LogCapturer.captureLogs(DataNode.LOG);
        try {
            Path f = new Path("/f");
    ...
    @Test(timeout = 60000)
    public void testChecksumErrorAppendWhileTransfer_withUpgrade60() throws Exception {
        DataNodeFaultInjector oldFi = DataNodeFaultInjector.get();
        LogCapturer logCapturer = GenericTestUtils.LogCapturer.captureLogs(DataNode.LOG);
        try {
            Path f = new Path("/f");
    ...
    @Test(timeout = 60000)
    public void testChecksumErrorAppendWhileTransfer_withUpgrade80() throws Exception {
        DataNodeFaultInjector oldFi = DataNodeFaultInjector.get();
        LogCapturer logCapturer = GenericTestUtils.LogCapturer.captureLogs(DataNode.LOG);
        try {
            Path f = new Path("/f");
