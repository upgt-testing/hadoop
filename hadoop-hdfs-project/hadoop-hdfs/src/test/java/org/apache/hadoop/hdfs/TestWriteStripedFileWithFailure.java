...
public class TestWriteStripedFileWithFailure {
    ...
    @Ignore
    @Test(timeout = 300000)
    public void testWriteStripedFileWithDNFailure_withUpgrade20() throws IOException {
        for (int fileLength : fileLengths) {
            for (int dataDelNum = 1; dataDelNum <= parityBlocks; dataDelNum++) {
                for (int parityDelNum = 0; (dataDelNum + parityDelNum) <= parityBlocks; parityDelNum++) {
                    try {
    ...
    @Ignore
    @Test(timeout = 300000)
    public void testWriteStripedFileWithDNFailure_withUpgrade40() throws IOException {
        for (int fileLength : fileLengths) {
            for (int dataDelNum = 1; dataDelNum <= parityBlocks; dataDelNum++) {
                for (int parityDelNum = 0; (dataDelNum + parityDelNum) <= parityBlocks; parityDelNum++) {
                    try {
    ...
    @Ignore
    @Test(timeout = 300000)
    public void testWriteStripedFileWithDNFailure_withUpgrade60() throws IOException {
        for (int fileLength : fileLengths) {
            for (int dataDelNum = 1; dataDelNum <= parityBlocks; dataDelNum++) {
                for (int parityDelNum = 0; (dataDelNum + parityDelNum) <= parityBlocks; parityDelNum++) {
                    try {
