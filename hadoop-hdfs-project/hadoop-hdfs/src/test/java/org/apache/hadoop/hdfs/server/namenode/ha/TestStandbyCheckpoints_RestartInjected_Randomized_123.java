...
public class TestStandbyCheckpoints_RestartInjected_Randomized_123 {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestStandbyCheckpoints_RestartInjected.class);
    ...
    protected Configuration setupCommonConfig() {
        tmpOivImgDir = GenericTestUtils.getTestDir("TestStandbyCheckpoints_RestartInjected");
        tmpOivImgDir.mkdirs();
    ...
    public void testSBNCheckpoints() throws Exception {
        ...
        doEdits(0, 10);
        RestartFramework.at("after_oiv_image_save").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_checkpoint_upload").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_edits").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        HATestUtil.waitForStandbyToCatchUp(nns[0], nns[1]);
        // Once the standby catches up, it should notice that it needs to
        // do a checkpoint and save one to its local directories.
        HATestUtil.waitForCheckpoint(cluster, 1, ImmutableList.of(12));
        RestartFramework.at("after_standby_catchup").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNewDirInitAfterCheckpointing() throws Exception {
        File hdfsDir = new File(PathUtils.getTestDir(TestStandbyCheckpoints_RestartInjected.class), "testNewDirInitAfterCheckpointing");
        File nameDir = new File(hdfsDir, "name1");
        assert nameDir.mkdirs();
        RestartFramework.at("after_trigger_edits").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_namenode_restart_with_new_dir").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBothNodesInStandbyState() throws Exception {
        ...
        RestartFramework.at("after_edits").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<File> dirs = Lists.newArrayList();
        dirs.addAll(FSImageTestUtil.getNameNodeCurrentDirs(cluster, 0));
        RestartFramework.at("after_both_checkpoints").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStandbyAndObserverState() throws Exception {
        ...
        dirs.addAll(FSImageTestUtil.getNameNodeCurrentDirs(cluster, 1));
        RestartFramework.at("after_roll_editlog").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        FSImageTestUtil.assertParallelFilesAreIdentical(dirs, ImmutableSet.of());
        RestartFramework.at("after_transition_to_observer").on(cluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckpointBeforeNameNodeInitializationIsComplete() throws Exception {
        ...
        for (LoggingEvent event : appender.getLog()) {
            ...
            if (message.contains("PutImage failed") && message.contains("FSImage has not been set in the NameNode.")) {
    ...
    public void testCheckpointWhenNoNewTransactionsHappened() throws Exception {
        ...
        Thread.sleep(1000);
        Mockito.verify(spyImage1, Mockito.never()).saveNamespace(any());
        // Roll the primary and wait for the standby to catch up
        HATestUtil.waitForStandbyToCatchUp(nns[0], nns[1]);
        RestartFramework.at("after_namenode_restart").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_standby_catchup").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(2000);
        // We should make exactly one checkpoint at this new txid.
        Mockito.verify(spyImage1, Mockito.times(1)).saveNamespace(any(), Mockito.eq(NameNodeFile.IMAGE), any());
    }
    ...
    public void testCheckpointCancellation() throws Exception {
        ...
        RestartFramework.at("after_nn1_restart").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckpointCancellationDuringUpload() throws Exception {
        ...
        cluster.transitionToStandby(0);
        cluster.transitionToActive(1);
        RestartFramework.at("after_checkpoints").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStandbyExceptionThrownDuringCheckpoint() throws Exception {
        ...
        Mockito.doAnswer(answerer).when(spyImage1).saveNamespace(any(FSNamesystem.class), Mockito.eq(NameNodeFile.IMAGE), any(Canceler.class));
    ...
    public void testReadsAllowedDuringCheckpoint() throws Exception {
        ...
        FSImage spyImage1 = NameNodeAdapter.spyOnFsImage(nns[1]);
        DelayAnswer answerer = new DelayAnswer(LOG);
        Mockito.doAnswer(answerer).when(spyImage1).saveNamespace(any(FSNamesystem.class), any(NameNodeFile.class), any(Canceler.class));
        // Perform some edits and wait for a checkpoint to start on the SBN.
        doEdits(0, 1000);
        nns[0].getRpcServer().rollEditLog();
        answerer.waitForCall();
    ...
    public void testCheckpointSucceedsWithLegacyOIVException() throws Exception {
        ...
        doEdits(0, 10);
        RestartFramework.at("after_edits").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_checkpoint").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        HATestUtil.waitForStandbyToCatchUp(nns[0], nns[1]);
        // Once the standby catches up, it should notice that it needs to
        // do a checkpoint and save one to its local directories.
        HATestUtil.waitForCheckpoint(cluster, 1, ImmutableList.of(12));
