...
public class TestBlockPlacementPolicyRackFaultTolerant_RestartInjected_Randomized_456 {
    ...
    private void doTestChooseTargetNormalCase() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            for (short[] testCase : testSuite) {
                ...
                RestartFramework.at("after_start_file").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_add_block").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_get_additional_datanode").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void doTestChooseTargetSpecialCase() throws Exception {
        RestartFramework.at("after_add_block_special").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String clientMachine = "client.foo.com";
        // Test 5 files
        ...
        RestartFramework.at("after_start_file_special").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        //test chooseTarget for new file
        LocatedBlock locatedBlock = nameNodeRpc.addBlock(src, clientMachine, null, null, fileStatus.getFileId(), null, null);
    ...
    public void testPlacementWithOnlyOneNodeInRackDecommission() throws Exception {
        ...
        cluster = new MiniDFSCluster.Builder(conf).numDataNodes(7).racks(racks).hosts(hosts).build();
        cluster.waitActive();
        RestartFramework.at("after_decommission_complete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_get_located_blocks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nameNodeRpc = cluster.getNameNodeRpc();
        namesystem = cluster.getNamesystem();
        ...
        RestartFramework.at("after_start_decommission").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nameNodeRpc = cluster.getNameNodeRpc();
        namesystem = cluster.getNamesystem();
        ...
        try {
            ...
            //test chooseTarget for new file
            LocatedBlock locatedBlock = nameNodeRpc.addBlock(src, clientMachine, null, null, fileStatus.getFileId(), null, null);
            RestartFramework.at("after_add_block_decommission").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            //test chooseTarget for existing file.
            LocatedBlock additionalLocatedBlock = nameNodeRpc.getAdditionalDatanode(src, fileStatus.getFileId(), locatedBlock.getBlock(), locatedBlock.getLocations(), locatedBlock.getStorageIDs(), DatanodeInfo.EMPTY_ARRAY, additionalReplication, clientMachine);
            RestartFramework.at("after_get_additional_datanode_decommission").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            racksCount.clear();
            ...
            RestartFramework.at("after_stop_decommission").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nameNodeRpc = cluster.getNameNodeRpc();
            namesystem = cluster.getNamesystem();
            bm = cluster.getNamesystem().getBlockManager();
            dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        ...
        try {
            ...
            RestartFramework.at("after_second_decommission").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nameNodeRpc = cluster.getNameNodeRpc();
            namesystem = cluster.getNamesystem();
            bm = cluster.getNamesystem().getBlockManager();
            dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
