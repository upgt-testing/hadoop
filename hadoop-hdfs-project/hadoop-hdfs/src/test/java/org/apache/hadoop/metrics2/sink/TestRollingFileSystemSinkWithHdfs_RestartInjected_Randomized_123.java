 ...
 */
public class TestRollingFileSystemSinkWithHdfs_RestartInjected_Randomized_123 extends RollingFileSystemSinkTestBase {
    ...
    public void testWrite() throws Exception {
        RestartFramework.at("after_metrics_init").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String path = "hdfs://" + cluster.getNameNode().getHostAndPort() + "/tmp";
        MetricsSystem ms = initMetricsSystem(path, false, true);
        RestartFramework.at("after_write_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAppend() throws Exception {
        ...
        RestartFramework.at("after_append_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testSilentAppend() throws Exception {
        ...
        RestartFramework.at("after_silent_append_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testNoAppend() throws Exception {
        RestartFramework.at("after_no_append_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String path = "hdfs://" + cluster.getNameNode().getHostAndPort() + "/tmp";
    ...
    public void testSilentOverwrite() throws Exception {
        ...
        RestartFramework.at("after_silent_overwrite_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testFlushThread() throws Exception {
        ...
        RestartFramework.at("after_second_publish").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        new MyMetrics1().registerWith(ms);
        // Publish the metrics
        ms.publishMetricsNow();
        ...
        // regardless.
        ms.publishMetricsNow();
        RestartFramework.at("after_flush_metrics_init").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_publish").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int count = 0;
        try {
