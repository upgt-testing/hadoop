...
public class TestReadStripedFileWithMissingBlocks {
    ...
    public void setup() throws IOException {
        ...
        cluster.getFileSystem().getClient().setErasureCodingPolicy("/", ecPolicy.getName());
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(ecPolicy.getName());
        dfsClient = new DFSClient(cluster.getNameNode(0).getNameNodeAddress(), conf);
    }
    ...
    private void readFileWithMissingBlocks(Path srcPath, int fileLength, int missingDataNum, int missingParityNum, byte[] expected) throws Exception {
        ...
        for (int i = 0; i < missingParityNum; i++) {
            missingDataNodes[i + missingDataNum] = i + Math.min(ecPolicy.getNumDataUnits(), dataBlocks);
        }
    ...
    @Test
    public void testReadFileWithMissingBlocks_withUpgrade20() throws Exception {
        try {
            setup();
            Path srcPath = new Path("/foo");
    ...
    @Test
    public void testReadFileWithMissingBlocks_withUpgrade40() throws Exception {
        try {
            setup();
            Path srcPath = new Path("/foo");
            final byte[] expected = StripedFileTestUtil.generateBytes(fileLength);
    ...
    @Test
    public void testReadFileWithMissingBlocks_withUpgrade60() throws Exception {
        try {
            setup();
            Path srcPath = new Path("/foo");
            final byte[] expected = StripedFileTestUtil.generateBytes(fileLength);
