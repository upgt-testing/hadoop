...
public class TestPersistentStoragePolicySatisfier {
    ...
    private void clusterSetUp(boolean isHAEnabled, Configuration newConf) throws Exception {
        ...
        MiniDockerDFSCluster.Builder clusterBuilder = new MiniDockerDFSCluster.Builder(conf).storageTypes(storageTypes).storagesPerDatanode(3).numDataNodes(dnNumber);
        if (isHAEnabled) {
    ...
    public void testSPSxAttrWhenSpsCalledForDir() throws Exception {
        try {
            ...
            INodeInterface inode = namesystem.getFSDirectory().getINode("/parent");
            XAttrFeatureInterface f = inode.getXAttrFeature();
    ...
    public void testSPSxAttrWhenSpsCalledForFile() throws Exception {
        try {
            ...
            // Check xAttr for parent directory
            FSNamesystemInterface namesystem = cluster.getNamesystem();
            INodeInterface inode = namesystem.getFSDirectory().getINode("/file");
            XAttrFeatureInterface f = inode.getXAttrFeature();
            assertTrue("SPS xAttr should be exist", f.getXAttr(XATTR_SATISFY_STORAGE_POLICY) != null);
            cluster.restartDataNode(stopDataNode, false);
            // wait and check all the file block moved in ARCHIVE
