 ...
 */
public class TestResolveHdfsSymlink_RestartInjected_Randomized_456 {
    ...
    public void testFcResolveAfs() throws IOException, InterruptedException {
        ...
        RestartFramework.at("before_resolve_symlink").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFcDelegationToken() throws UnsupportedFileSystemException, IOException, InterruptedException {
        RestartFramework.at("after_renew_token").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FileContext fcHdfs = FileContext.getFileContext(cluster.getFileSystem().getUri());
    ...
    public void testLinkTargetNonSymlink() throws UnsupportedFileSystemException, IOException {
        ...
        try {
            ...
            try {
                RestartFramework.at("before_get_link_target").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testLinkTargetNonExistent() throws IOException {
        ...
        DFSClient client = new DFSClient(cluster.getFileSystem().getUri(), cluster.getConfiguration(0));
        try {
            RestartFramework.at("before_get_nonexistent_link").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
