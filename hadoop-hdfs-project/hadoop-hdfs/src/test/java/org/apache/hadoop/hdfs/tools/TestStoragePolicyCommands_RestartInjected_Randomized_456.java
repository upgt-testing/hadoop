 ...
 */
public class TestStoragePolicyCommands_RestartInjected_Randomized_456 {
    ...
    public void testSetAndUnsetStoragePolicy() throws Exception {
        ...
        RestartFramework.at("after_unset_storage_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /*
     ...
     */
        ...
        RestartFramework.at("after_verify_unset_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /*
     ...
     */
        ...
        RestartFramework.at("after_set_storage_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final BlockStoragePolicy hot = suite.getPolicy("HOT");
        RestartFramework.at("after_get_storage_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSetAndGetStoragePolicy() throws Exception {
        final Path foo = new Path("/foo");
        RestartFramework.at("after_get_initial_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_set_storage_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path bar = new Path(foo, "bar");
        DFSTestUtil.createFile(fs, bar, SIZE, REPL, 0);
        final StoragePolicyAdmin admin = new StoragePolicyAdmin(conf);
        DFSTestUtil.toolRun(admin, "-getStoragePolicy -path /foo", 0, "The storage policy of " + foo.toString() + " is unspecified");
        RestartFramework.at("after_verify_set_policies").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.toolRun(admin, "-getStoragePolicy -path /foo/bar", 0, "The storage policy of " + bar.toString() + " is unspecified");
