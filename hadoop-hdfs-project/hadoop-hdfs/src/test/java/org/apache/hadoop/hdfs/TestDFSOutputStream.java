...
public class TestDFSOutputStream {
    ...
    private void runAdjustChunkBoundary(final int configuredWritePacketSize, final int finalWritePacketSize) throws IOException, NoSuchFieldException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException {
        ...
        try {
            ...
            dfsCluster = new MiniDockerDFSCluster.Builder(dfsConf).numDataNodes(1).build();
    ...
    public void testNoLocalWriteFlag() throws IOException {
        ...
        DatanodeManagerInterface dm = bm.getDatanodeManager();
        try (FSDataOutputStream os = fs.create(new Path("/test-no-local"), FsPermission.getDefault(), flags, 512, (short) 2, 512, null)) {
            ...
            // the client.
            DatanodeManagerInterface spyDm = spy(dm);
            DatanodeDescriptorInterface dn1 = dm.getDatanodeListForReport(HdfsConstants.DatanodeReportType.LIVE).get(0);
