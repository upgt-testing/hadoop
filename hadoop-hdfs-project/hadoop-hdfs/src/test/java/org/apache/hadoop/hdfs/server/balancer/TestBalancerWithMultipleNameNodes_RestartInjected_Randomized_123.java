 ...
 */
public class TestBalancerWithMultipleNameNodes_RestartInjected_Randomized_123 {
   ...
   */
    ...
    static void wait(final Suite suite, long expectedUsedSpace, long expectedTotalSpace) throws IOException {
    ...
    private void unevenDistribution(final int nNameNodes, final int nNameNodesToBalance, long[] distributionPerNN, long[] capacities, String[] racks, Configuration conf) throws Exception {
        ...
        final ExtendedBlock[][] blocks;
        {
            ...
            try {
                cluster.waitActive();
                RestartFramework.at("after_first_cluster_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                DFSTestUtil.setFederatedConfiguration(cluster, conf);
                LOG.info("UNEVEN 3");
                final Suite s = new Suite(cluster, nNameNodes, nDataNodes, null, conf);
                blocks = generateBlocks(s, usedSpacePerNN);
                RestartFramework.at("after_generate_blocks").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                LOG.info("UNEVEN 4");
            } finally {
                cluster.shutdown();
            }
        ...
        }
        {
            ...
            try {
                cluster.waitActive();
                RestartFramework.at("after_second_cluster_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                LOG.info("UNEVEN 12");
                ...
                RestartFramework.at("after_inject_blocks").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void runTest(final int nNameNodes, String[] racks, String[] newRack, Configuration conf, int nNameNodestoBalance, BalancerParameters balancerParameters) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_add_datanodes").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
