...
public class HAStressTestHarness {
    ...
    public HAStressTestHarness() {
        ...
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_REPLICATION_STREAMS_HARD_LIMIT_KEY, 16);
    }
    ...
    /**
     * Start and return the MiniDockerDFSCluster.
     */
    public MiniDockerDFSCluster startCluster() throws IOException {
        cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology(nns)).numDataNodes(3).build();
        return cluster;
    }
    ...
    public void addReplicationTriggerThread(final int interval) {
        testCtx.addThread(new RepeatingTestThread(testCtx) {
            ...
            public void doAnAction() throws Exception {
                ...
                for (int i = 0; i < 2; i++) {
                    NameNodeInterface nn = cluster.getNameNode(i);
