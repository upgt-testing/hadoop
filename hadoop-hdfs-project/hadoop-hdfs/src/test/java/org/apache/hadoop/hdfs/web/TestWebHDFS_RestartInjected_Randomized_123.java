 ...
 * Test WebHDFS
 */
public class TestWebHDFS_RestartInjected_Randomized_123 {
    ...
    static void largeFileTest(final long fileLength) throws Exception {
        ...
        RestartFramework.at("after_large_file_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWebHdfsSps() throws Exception {
        ...
        try {
            cluster = new MiniDFSCluster.Builder(conf).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE } }).storagesPerDatanode(2).numDataNodes(1).build();
            cluster.waitActive();
