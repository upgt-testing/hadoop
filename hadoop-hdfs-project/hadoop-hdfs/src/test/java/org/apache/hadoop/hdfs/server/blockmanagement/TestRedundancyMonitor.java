...
public class TestRedundancyMonitor {
    ...
    public void testChooseTargetWhenAllDataNodesStop() throws Throwable {
        ...
        try (MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build()) {
            ...
            Future<Void> stopDatanodesFuture = pool.submit(() -> {
                for (DatanodeDescriptorInterface dn : dns) {
