...
public class TestBlockScanner_RestartInjected_Randomized_123 {

    public static final Logger LOG = LoggerFactory.getLogger(TestBlockScanner_RestartInjected.class);
    ...
    private void testVolumeIteratorImpl(int numFiles, long maxStaleness) throws Exception {
        ...
        RestartFramework.at("after_cluster_creation").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_files_created").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            RestartFramework.at("after_iterator_created").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            while (true) {
    ...
    private void testScanAllBlocksImpl(final boolean rescan) throws Exception {
        ...
        RestartFramework.at("after_scanner_started").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        synchronized (info) {
            info.shouldRun = true;
            info.notify();
        }
    ...
    public void testCorruptBlockHandling() throws Exception {
        RestartFramework.at("after_block_corrupted").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Configuration conf = new Configuration();
        conf.setLong(DFS_DATANODE_SCAN_PERIOD_HOURS_KEY, 100L);
        conf.set(INTERNAL_VOLUME_SCANNER_SCAN_RESULT_HANDLER, TestScanResultHandler.class.getName());
        final TestContext ctx = new TestContext(conf, 1);
    ...
    public void testDatanodeCursor() throws Exception {
        ...
        RestartFramework.at("after_files_created").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_scan_phase").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAppendWhileScanning() throws Exception {
        ...
        RestartFramework.at("after_append_hflush").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_append_close").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_created").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cluster_creation").on(ctx.cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final TestScanResultHandler.Info info = TestScanResultHandler.getInfo(ctx.volumes.get(0));
        String storageID = ctx.volumes.get(0).getStorageID();
        synchronized (info) {
    ...
    public void testSkipRecentAccessFile() throws Exception {
        ...
        assertEquals("Should not scan block accessed in last period", 0, info.blocksScanned);
        ctx.close();
    }
    ...
    public void testSlowDatanodeShutdown() throws Exception {
        ...
        // volume scanner
        testDatanodeShutDown(TimeUnit.MINUTES.toMillis(5), 1000L, false);
    }

    private void testDatanodeShutDown(final long joinTimeOutMS, final long delayMS, boolean isFastShutdown) throws Exception {
        VolumeScannerCBInjector prevVolumeScannerCBInject = VolumeScannerCBInjector.get();
        try {
            DelayVolumeScannerResponseToInterrupt injectDelay = new DelayVolumeScannerResponseToInterrupt(delayMS);
            VolumeScannerCBInjector.set(injectDelay);
            Configuration conf = new Configuration();
            conf.setLong(DFS_DATANODE_SCAN_PERIOD_HOURS_KEY, 100L);
            conf.set(INTERNAL_VOLUME_SCANNER_SCAN_RESULT_HANDLER, TestScanResultHandler.class.getName());
            conf.setLong(INTERNAL_DFS_BLOCK_SCANNER_CURSOR_SAVE_INTERVAL_MS, 0L);
            conf.setLong(DFS_BLOCK_SCANNER_VOLUME_JOIN_TIMEOUT_MSEC_KEY, joinTimeOutMS);
            final TestContext ctx = new TestContext(conf, 1);
            ...
            if (isFastShutdown) {
                assertTrue("total shutdown time of DN must be smaller than " + "VolumeScanner Response time: " + totalTimeShutdown, totalTimeShutdown < delayMS && totalTimeShutdown >= joinTimeOutMS);
                // wait for scanners to terminate before we move to the next test.
                injectDelay.waitForScanners();
                return;
            }
            assertTrue("total shutdown time of DN must be larger than " + "VolumeScanner Response time: " + totalTimeShutdown, totalTimeShutdown >= delayMS && totalTimeShutdown < joinTimeOutMS);
        } finally {
