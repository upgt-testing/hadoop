...
public class TestFileCreation {
    ...
    public void testServerDefaults() throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(DFSConfigKeys.DFS_REPLICATION_DEFAULT + 1).build();
        cluster.waitActive();
    ...
    public void testServerDefaultsWithCaching() throws IOException, InterruptedException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(clusterConf).numDataNodes(0).build();
        cluster.waitActive();
        // Set a spy namesystem inside the namenode and return it
        FSNamesystemInterface spyNamesystem = NameNodeAdapter.spyOnNamesystem(cluster.getNameNode());
     ...
     */
    @Test
    public void testServerDefaultsWithMinimalCaching() throws IOException, InterruptedException {
        // Create cluster with an explicit block size param
        Configuration clusterConf = new HdfsConfiguration();
        long originalBlockSize = DFS_BLOCK_SIZE_DEFAULT * 2;
        clusterConf.setLong(DFS_BLOCK_SIZE_KEY, originalBlockSize);
    ...
    public void checkFileCreation(String netIf, boolean useDnHostname) throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).checkDataNodeHostConfig(true).build();
        FileSystem fs = cluster.getFileSystem();
        try {
            //
            // check that / exists
    ...
    public void testFileCreationNamenodeRestart() throws IOException, NoSuchFieldException, IllegalAccessException {
        ...
        try {
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).nameNodePort(nnport).format(false).build();
            cluster.waitActive();
    ...
    public void testLeaseExpireHardLimit() throws Exception {
        ...
        try {
            ...
            for (DatanodeInfo datanodeinfo : locatedblock.getLocations()) {
                DataNodeInterface datanode = cluster.getDataNode(datanodeinfo.getIpcPort());
        ...
        try {
            ...
            for (String pathStr : NON_CANONICAL_PATHS) {
                ...
                switch(method) {
                    case DIRECT_NN_RPC:
                        try {
                            nnrpc.create(pathStr, new FsPermission((short) 0755), "client", new EnumSetWritable<CreateFlag>(EnumSet.of(CreateFlag.CREATE)), true, (short) 1, 128 * 1024 * 1024L, null, null, null);
    ...
    public void testFileCreationWithOverwrite() throws Exception {
        ...
        try {
            ...
            FSNamesystemInterface fsn = NameNodeAdapter.getNamesystem(nn);
