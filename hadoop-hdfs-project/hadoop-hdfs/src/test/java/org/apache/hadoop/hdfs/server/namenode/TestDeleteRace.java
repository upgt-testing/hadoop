...
public class TestDeleteRace {
    ...
    private class DeleteThread extends Thread {
        ...
        public void run() {
            try {
                ...
                final FSDirectoryInterface fsdir = cluster.getNamesystem().dir;
                INodeInterface fileINode = fsdir.getINode4Write(path.toString());
                INodeMapInterface inodeMap = (INodeMap) Whitebox.getInternalState(fsdir, "inodeMap");
    ...
    private void testDeleteAndCommitBlockSynchronizationRace(boolean hasSnapshot) throws Exception {
        ...
        try {
            ...
            for (AbstractMap.SimpleImmutableEntry<String, Boolean> stest : testList) {
                ...
                DatanodeProtocolClientSideTranslatorPBInterface nnSpy = dnMap.get(primaryDN);
                if (nnSpy == null) {
    ...
    public void testDeleteAndLeaseRecoveryHardLimitSnapshot() throws Exception {
        ...
        FSDataOutputStream stm = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(3).build();
            cluster.waitActive();
    ...
    public void testOpenRenameRace() throws Exception {
        ...
        try {
            dfsCluster = new MiniDockerDFSCluster.Builder(config).build();
