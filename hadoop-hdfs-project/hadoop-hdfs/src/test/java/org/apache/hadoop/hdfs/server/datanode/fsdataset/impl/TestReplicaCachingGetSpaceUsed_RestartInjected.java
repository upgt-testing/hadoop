...
public class TestReplicaCachingGetSpaceUsed_RestartInjected {
  ...
  public void testReplicaCachingGetSpaceUsedByFINALIZEDReplica()
    ...
    os.hsync();
    os.close();
    RestartFramework.at("after_file_finalized")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_block_metadata_collection")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    Thread.sleep(2000);
    RestartFramework.at("after_space_refresh")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
 ...
 // This test cannot pass on Windows
    ...
    IOUtils.copyBytes(is, os, bytes.length);
    os.hsync();
    RestartFramework.at("after_rbw_replica_created")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_rbw_metadata_collection")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    // has been written to disk.
    Thread.sleep(2000);
    RestartFramework.at("after_rbw_space_check")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    os.close();
    RestartFramework.at("after_replica_finalized")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    // recalculated
    Thread.sleep(2000);
    RestartFramework.at("after_finalized_space_check")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testFsDatasetImplDeepCopyReplica() {
    ...
    modifyThread.start();
    RestartFramework.at("after_modify_thread_start")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_deep_copy_operations")
        .on(cluster)
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
