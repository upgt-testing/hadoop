 ...
 */
public class TestINodeFileUnderConstructionWithSnapshot_RestartInjected_Randomized_123 {
    ...
    public void testSnapshotAfterAppending() throws Exception {
        ...
        RestartFramework.at("after_create_snapshot_s2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_snapshot_s1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(hdfs, file, BLOCKSIZE, REPLICATION, seed);
        RestartFramework.at("after_setreplication").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        DFSTestUtil.appendFile(hdfs, file, BLOCKSIZE);
        RestartFramework.at("after_create_snapshot_s0").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSnapshotWhileAppending() throws Exception {
        RestartFramework.at("after_hsync_first").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path file = new Path(dir, "file");
        DFSTestUtil.createFile(hdfs, file, BLOCKSIZE, REPLICATION, seed);
        RestartFramework.at("after_first_append_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_close_first").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // 1. append without closing stream --> create snapshot
        HdfsDataOutputStream out = appendFileWithoutClosing(file, BLOCKSIZE);
        out.hsync(EnumSet.of(SyncFlag.UPDATE_LENGTH));
        SnapshotTestHelper.createSnapshot(hdfs, dir, "s0");
        RestartFramework.at("after_close_third").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_setreplication").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.close();
        RestartFramework.at("after_third_append_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_snapshot_s0").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_hsync_second").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_close_second").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        out.hsync(EnumSet.of(SyncFlag.UPDATE_LENGTH));
        RestartFramework.at("after_create_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        fileNode = (INodeFile) fsdir.getINode(file.toString());
        dirNode = fsdir.getINode(dir.toString()).asDirectory();
        RestartFramework.at("after_second_append_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testGetBlockLocations() throws Exception {
        ...
        RestartFramework.at("after_append_and_hsync").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_close_stream").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testLease() throws Exception {
        try {
            ...
            try {
                ...
                RestartFramework.at("after_lease_checks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            } finally {
                fsn.writeUnlock();
            }
        } finally {
            NameNodeAdapter.setLeasePeriod(fsn, HdfsConstants.LEASE_SOFTLIMIT_PERIOD, conf.getLong(DFSConfigKeys.DFS_LEASE_HARDLIMIT_KEY, DFSConfigKeys.DFS_LEASE_HARDLIMIT_DEFAULT) * 1000);
        }
    }
}
