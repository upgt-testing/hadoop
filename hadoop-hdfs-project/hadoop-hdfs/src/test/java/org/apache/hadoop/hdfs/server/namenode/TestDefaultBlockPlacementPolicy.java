...
public class TestDefaultBlockPlacementPolicy {
    ...
    public void setup() throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(5).racks(racks).hosts(hosts).build();
        cluster.waitActive();
    ...
    public void testPlacementWithDFSNetworkTopology() throws Exception {
        ...
        nameNodeRpc = cluster.getNameNodeRpc();
        namesystem = cluster.getNamesystem();
        DatanodeManagerInterface dm = namesystem.getBlockManager().getDatanodeManager();
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (clientRack != null) {
                if (hasBlockReplicaOnRack) {
                    ...
                    for (DatanodeInfoInterface dni : locatedBlock.getLocations()) {
