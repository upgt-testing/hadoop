...
public class TestStorageRestore {
    ...
    public void setUpNameDirs() throws Exception {
        ...
        hdfsDir = new File(MiniDockerDFSCluster.getBaseDirectory()).getCanonicalFile();
    ...
    public void testStorageRestore() throws Exception {
        int numDatanodes = 0;
        cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(numDatanodes).manageNameDfsDirs(false).build();
        cluster.waitActive();
    ...
    public void testDfsAdminCmd() throws Exception {
        cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(2).manageNameDfsDirs(false).build();
        cluster.waitActive();
        try {
            FSImageInterface fsi = cluster.getNameNode().getFSImage();
    ...
    public void testMultipleSecondaryCheckpoint() throws IOException {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(1).manageNameDfsDirs(false).build();
            cluster.waitActive();
