...
public class TestBalancerWithHANameNodes {
    ...
    public static void waitStoragesNoStale(MiniDockerDFSCluster cluster, ClientProtocol client, int nnIndex) throws Exception {
        ...
        GenericTestUtils.waitFor(() -> {
            BlockManagerInterface bm = cluster.getNamesystem(nnIndex).getBlockManager();
            for (DatanodeInfoInterface dn : dataNodes) {
                DatanodeStorageInfo[] storageInfos = bm.getDatanodeManager().getDatanode(dn.getDatanodeUuid()).getStorageInfos();
                for (DatanodeStorageInfoInterface s : storageInfos) {
