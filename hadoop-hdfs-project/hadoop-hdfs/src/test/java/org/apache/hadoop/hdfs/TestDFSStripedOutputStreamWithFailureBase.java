...
public class TestDFSStripedOutputStreamWithFailureBase {
    ...
    // 8k
    protected final int cellSize = 64 * 1024;
   ...
   */
    ...
    private int[][] getDnIndexSuite() {
        ...
        for (int i = 0; i < maxNumLevel && numIndex > 1; i++) {
            List<List<Integer>> lists = combinations(dataBlocks + parityBlocks, numIndex);
    ...
    void setup(Configuration conf) throws IOException {
        ...
        System.out.println("CELL_SIZE        = " + cellSize + " (=" + StringUtils.TraditionalBinaryPrefix.long2String(cellSize, "B", 2) + ")");
        System.out.println("BLOCK_SIZE       = " + blockSize + " (=" + StringUtils.TraditionalBinaryPrefix.long2String(blockSize, "B", 2) + ")");
        System.out.println("BLOCK_GROUP_SIZE = " + blockGroupSize + " (=" + StringUtils.TraditionalBinaryPrefix.long2String(blockGroupSize, "B", 2) + ")");
        ...
        AddErasureCodingPolicyResponse[] res = dfs.addErasureCodingPolicies(new ErasureCodingPolicy[] { ecPolicy });
     ...
     * @throws Exception
     */
    void runTest(final int length, final int[] killPos, final int[] dnIndex, final boolean tokenExpire) throws Exception {
