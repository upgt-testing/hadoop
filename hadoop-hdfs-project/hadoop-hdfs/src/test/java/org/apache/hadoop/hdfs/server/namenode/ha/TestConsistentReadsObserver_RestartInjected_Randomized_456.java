 ...
 */
public class TestConsistentReadsObserver_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(TestConsistentReadsObserver_RestartInjected.class.getName());
    ...
    public void testRequeueCall() throws Exception {
        ...
        RestartFramework.at("after_getfilestatus").on(dfsCluster).restart("namenode").withIndex(observerIdx).withMode(RestartMode.GRACEFUL).execute();
        String prefix = CommonConfigurationKeys.IPC_NAMESPACE + "." + port + ".";
        RestartFramework.at("after_refresh_callqueue").on(dfsCluster).restart("namenode").withIndex(observerIdx).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMsyncSimple() throws Exception {
        ...
        assertSentTo(0);
        RestartFramework.at("after_mkdir").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_roll_editlog").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testMsync(boolean autoMsync, long autoMsyncPeriodMs) throws Exception {
        RestartFramework.at("after_advance_state").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        // 0 == not completed, 1 == succeeded, -1 == failed
        AtomicInteger readStatus = new AtomicInteger(0);
        ...
        if (autoMsync) {
            conf2.setTimeDuration(ObserverReadProxyProvider.AUTO_MSYNC_PERIOD_KEY_PREFIX + "." + dfs.getUri().getHost(), autoMsyncPeriodMs, TimeUnit.MILLISECONDS);
        }
        ...
        RestartFramework.at("after_roll_editlog_msync").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.rollEditLogAndTail(0);
        dfs.mkdir(testPath, FsPermission.getDefault());
        assertSentTo(0);
        Thread reader = new Thread(() -> {
        ...
        Thread.sleep(100);
        assertEquals(0, readStatus.get());
        RestartFramework.at("after_get_filesystem").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfsCluster.rollEditLogAndTail(0);
    ...
    public void testCallFromNewClient() throws Exception {
        ...
        RestartFramework.at("after_state_transitions").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            RestartFramework.at("after_advance_observer_state").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dfs.mkdir(testPath, FsPermission.getDefault());
            assertSentTo(2);
            RestartFramework.at("after_mkdir_from_new_client").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            Configuration conf2 = new Configuration(conf);
            // Disable FS cache so two different DFS clients will be used.
            conf2.setBoolean("fs.hdfs.impl.disable.cache", true);
            DistributedFileSystem dfs2 = (DistributedFileSystem) FileSystem.get(conf2);
            ...
            dfsCluster.getNameNode(2).getRpcServer().rollEditLog();
            dfsCluster.getNameNode(0).getNamesystem().getEditLogTailer().doTailEdits();
            RestartFramework.at("after_roll_editlog_new_client").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            GenericTestUtils.waitFor(() -> readStatus.get() != 0, 100, 10000);
            assertEquals(1, readStatus.get());
        } finally {
    ...
    public void testUncoordinatedCall() throws Exception {
        RestartFramework.at("after_mkdir_uncoordinated").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRequestFromNonObserverProxyProvider() throws Exception {
        RestartFramework.at("after_get_filesystem_nonproxy").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        dfs.mkdir(testPath, FsPermission.getDefault());
        dfsCluster.rollEditLogAndTail(0);
        RestartFramework.at("after_roll_editlog_nonproxy").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        try {
            ...
            assertTrue("should have thrown StandbyException but got " + e.getClass().getSimpleName(), e instanceof StandbyException);
        }
    }
    ...
    public void testMsyncFileContext() throws Exception {
        ...
        RestartFramework.at("after_get_filecontext").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_roll_editlog_filecontext").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        fc.msync();
        nn0 = dfsCluster.getNameNode(0);
        nn2 = dfsCluster.getNameNode(2);
        dfsCluster.rollEditLogAndTail(0);
        RestartFramework.at("after_msync_filecontext").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nn0 = dfsCluster.getNameNode(0);
        nn2 = dfsCluster.getNameNode(2);
        LOG.info("State id active = {}, Stat id observer = {}", nn0.getNamesystem().getFSImage().getLastAppliedOrWrittenTxId(), nn2.getNamesystem().getFSImage().getLastAppliedOrWrittenTxId());
        try {
    ...
    public void testRpcQueueTimeNumOpsMetrics() throws Exception {
        ...
        RestartFramework.at("after_mkdir_metrics").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        reader.start();
        // the reader is still blocking, not succeeded yet.
        assertEquals(0, readStatus.get());
        dfsCluster.rollEditLogAndTail(0);
        ...
        // the reader should have succeed.
        assertEquals(1, readStatus.get());
        RestartFramework.at("after_roll_editlog_metrics").on(dfsCluster).restart("namenode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        final int observerIdx = 2;
