 ...
 */
public class TestDataNodeVolumeFailure_RestartInjected_Randomized_456 {
   ...
   */
    ...
    public void testVolumeFailure() throws Exception {
        ...
        RestartFramework.at("after_volume_failure").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_new_file_replication").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
   ...
   */
    ...
    public void testDnStartsAfterDiskErrorScanningBlockPool() throws Exception {
        ...
        cluster.close();
        conf.set(DFSConfigKeys.DFS_DATANODE_FSDATASET_FACTORY_KEY, BadDiskFSDataset.Factory.class.getName());
        final MiniDFSCluster localCluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).build();
        try {
            localCluster.waitActive();
            RestartFramework.at("after_local_cluster_start").on(localCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_datanode_fully_started").on(localCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // trigger DN to send heartbeat
            DataNodeTestUtils.triggerHeartbeat(dn);
            final BlockManager bm = localCluster.getNamesystem().getBlockManager();
            // trigger NN handle heartbeat
            BlockManagerTestUtil.checkHeartbeat(bm);
            RestartFramework.at("after_heartbeat").on(localCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRefreshDeadLock() throws Exception {
        ...
        latch.countDown();
        String result = dn.reconfigurePropertyImpl(DFSConfigKeys.DFS_DATANODE_DATA_DIR_KEY, newdir);
    ...
    public void testUnderReplicationAfterVolFailure() throws Exception {
        RestartFramework.at("after_file1_replication").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // The test uses DataNodeTestUtils#injectDataDirFailure() to simulate
        // volume failures which is currently not supported on Windows.
        assumeNotWindows();
        ...
        RestartFramework.at("after_volume_failures").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_new_datanode_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static class BadDiskFSDataset extends SimulatedFSDataset {
        ...
        public void addBlockPool(String bpid, Configuration conf) {
            ...
            Map<FsVolumeSpi, IOException> unhealthyDataDirs = new HashMap<>();
   ...
   */
    ...
    public void testVolumeFailureDuringStartup() throws Exception {
        ...
        data_fail = cluster.getInstanceStorageDir(1, 0);
        failedDir = MiniDFSCluster.getFinalizedDir(data_fail, cluster.getNamesystem().getBlockPoolId());
        RestartFramework.at("after_volume_failure_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_datanode_restart").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        failedDir.setReadOnly();
        // restart the dn
        cluster.restartDataNode(1);
        final DataNode dn = cluster.getDataNodes().get(1);
        RestartFramework.at("after_volume_failure_detected").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
   ...
   */
    ...
    public void testVolumeFailureTwo() throws Exception {
        ...
        failedDir = MiniDFSCluster.getFinalizedDir(data_fail, cluster.getNamesystem().getBlockPoolId());
        failedDir.setReadOnly();
        RestartFramework.at("after_first_volume_failure").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        data_fail = cluster.getInstanceStorageDir(1, 1);
        failedDir = MiniDFSCluster.getFinalizedDir(data_fail, cluster.getNamesystem().getBlockPoolId());
        RestartFramework.at("after_second_volume_failure").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_disk_check").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        failedDir.setReadOnly();
        final DataNode dn = cluster.getDataNodes().get(1);
        dn.checkDiskError();
