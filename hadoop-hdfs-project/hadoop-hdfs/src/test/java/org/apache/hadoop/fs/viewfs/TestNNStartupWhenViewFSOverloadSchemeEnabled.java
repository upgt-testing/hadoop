...
public class TestNNStartupWhenViewFSOverloadSchemeEnabled {
    ...
    public void testNameNodeAndDataNodeStartup() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(CONF).numDataNodes(1).waitSafeMode(false).build();
        cluster.waitActive();
    }
    ...
    @Test(timeout = 30000)
    public void testHANameNodeAndDataNodeStartup_withUpgrade20() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(CONF).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).waitSafeMode(false).build();
        cluster.waitActive();
    ...
    @Test(timeout = 30000)
    public void testHANameNodeAndDataNodeStartup_withUpgrade60() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(CONF).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).waitSafeMode(false).build();
        cluster.waitActive();
        cluster.transitionToActive(0);
        cluster.restartNodeForTesting(0);
    ...
    @Test(timeout = 30000)
    public void testNameNodeAndDataNodeStartup_withUpgrade20() throws Exception {
        cluster = new MiniDFSClusterInJVM.Builder(CONF).numDataNodes(1).waitSafeMode(false).build();
        cluster.waitActive();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
