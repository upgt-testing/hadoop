...
public class TestPipelinesFailover {
    ...
    enum TestScenario {
        ...
        GRACEFUL_FAILOVER {
            ...
            @Override
            void run(MiniDockerDFSCluster cluster, int previousActive, int activeIndex) throws IOException {
        ...
        abstract void run(MiniDockerDFSCluster cluster, int previousActive, int activeIndex) throws IOException;
    }
    ...
    private void doWriteOverFailoverTest(TestScenario scenario, MethodToTestIdempotence methodToTest) throws Exception {
        ...
        MiniDockerDFSCluster cluster = newMiniCluster(conf, 3);
        try {
            int sizeWritten = 0;
            cluster.waitActive();
            ...
            FSNamesystemInterface ns1 = cluster.getNameNode(activeIndex).getNamesystem();
    ...
    private void doTestWriteOverFailoverWithDnFail(TestScenario scenario) throws Exception {
        ...
        conf.setInt(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, BLOCK_SIZE);
        FSDataOutputStream stm = null;
        MiniDockerDFSCluster cluster = newMiniCluster(conf, 5);
        try {
            cluster.waitActive();
            cluster.transitionToActive(0);
            Thread.sleep(500);
    ...
    public void testLeaseRecoveryAfterFailover() throws Exception {
        ...
        conf.setInt(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, BLOCK_SIZE);
        FSDataOutputStream stm = null;
        final MiniDockerDFSCluster cluster = newMiniCluster(conf, 3);
        try {
            cluster.waitActive();
            cluster.transitionToActive(0);
     ...
     */
    private MiniDockerDFSCluster newMiniCluster(Configuration conf, int dnCount) throws IOException {
        return new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology(NN_COUNT)).numDataNodes(dnCount).build();
    }
     ...
     * @throws IOException
     */
    private int failover(MiniDockerDFSCluster cluster, TestScenario scenario) throws IOException {
     ...
     * @return the index of the new active NN
     */
    private int failover(MiniDockerDFSCluster cluster, TestScenario scenario, int activeIndex) throws IOException {
    ...
    private DistributedFileSystem createFsAsOtherUser(final MiniDockerDFSCluster cluster, final Configuration conf) throws IOException, InterruptedException {
