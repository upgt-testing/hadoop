 ...
 */
public class TestDeadDatanode_RestartInjected_Randomized_456 {
    ...
    public void testDeadDatanode() throws Exception {
        ...
        RestartFramework.at("after_datanode_shutdown").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_block_received").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_datanode_live").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_block_report_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_heartbeat_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
        ...
        dnp = cluster.getNameNodeRpc();
        bm = cluster.getNamesystem().getBlockManager();
        RestartFramework.at("after_datanode_dead").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDeadNodeAsBlockTarget() throws Exception {
        RestartFramework.at("after_cluster_start_test2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Configuration conf = new HdfsConfiguration();
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_HEARTBEAT_RECHECK_INTERVAL_KEY, 500);
        conf.setLong(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY, 1L);
        ...
        cluster.waitActive();
        String poolId = cluster.getNamesystem().getBlockPoolId();
        RestartFramework.at("after_block_target_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // wait for datanode to be marked live
        DataNode dn = cluster.getDataNodes().get(0);
        DatanodeRegistration reg = InternalDataNodeTestUtils.getDNRegistrationForBP(cluster.getDataNodes().get(0), poolId);
        ...
        DFSTestUtil.waitForDatanodeState(cluster, reg.getDatanodeUuid(), true, 20000);
        RestartFramework.at("after_datanode_shutdown_test2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bm = cluster.getNamesystem().getBlockManager();
        dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        // Shutdown and wait for datanode to be marked dead
        dn.shutdown();
        RestartFramework.at("after_datanode_dead_test2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_datanode_live_test2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bm = cluster.getNamesystem().getBlockManager();
        dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        DFSTestUtil.waitForDatanodeState(cluster, reg.getDatanodeUuid(), false, 20000);
        bm = cluster.getNamesystem().getBlockManager();
    ...
    public void testNonDFSUsedONDeadNodeReReg() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_cluster_start_test3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            long initialCapacity = cluster.getNamesystem(0).getCapacityTotal();
            assertTrue(initialCapacity > 0);
            RestartFramework.at("after_initial_capacity").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_datanode_dead_test3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_capacity_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_datanode_rereg").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_final_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null) {
                cluster.shutdown();
            }
