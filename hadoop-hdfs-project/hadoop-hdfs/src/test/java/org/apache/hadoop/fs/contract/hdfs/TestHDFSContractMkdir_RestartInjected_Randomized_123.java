...
public class TestHDFSContractMkdir_RestartInjected_Randomized_123 extends AbstractContractMkdirTest {
    ...
    public void testMkdirRootDir() throws Throwable {
        RestartFramework.at("after_mkdir_root").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        describe("test mkdir on root directory with restart injection");
        FileSystem fs = getFileSystem();
        Path root = new Path("/");
        boolean made = fs.mkdirs(root);
        RestartFramework.at("before_mkdir_root").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirNonRecursive() throws Throwable {
        ...
        RestartFramework.at("before_parent_mkdir").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.mkdirs(parent);
        RestartFramework.at("after_nested_mkdir_attempt").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertPathExists("parent directory not created", parent);
        RestartFramework.at("after_parent_mkdir").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
    ...
    public void testMkdirOverFile() throws Throwable {
        ...
        RestartFramework.at("after_mkdir_over_file_attempt").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
