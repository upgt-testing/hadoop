 ...
 */
public class TestBlockReportLease_RestartInjected_Randomized_456 {
    ...
    public void testCheckBlockReportLease() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).build()) {
            ...
            HeartbeatResponse hbResponse = rpcServer.sendHeartbeat(dnRegistration, storages, 0, 0, 0, 0, 0, null, true, SlowPeerReports.EMPTY_REPORT, SlowDiskReports.EMPTY_REPORT);
            RestartFramework.at("after_heartbeat_response").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
            DelayAnswer delayer = new DelayAnswer(BlockManager.LOG);
            doAnswer(delayer).when(spyBlockManager).processReport(any(DatanodeStorageInfo.class), any(BlockListAsLongs.class));
            ...
            delayer.waitForCall();
            RestartFramework.at("after_process_report_started").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Remove full block report lease about dn
            spyBlockManager.getBlockReportLeaseManager().removeLease(datanodeDescriptor);
            RestartFramework.at("after_lease_removal").on(cluster).restart("namenode").withMode(RestartMode.CRASH).execute();
            // Allow blockreport to proceed
            delayer.proceed();
            RestartFramework.at("after_report_proceed").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.CRASH).execute();
