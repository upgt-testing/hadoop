 ...
 */
public class TestViewFileSystemLinkRegex_RestartInjected_Randomized_456 extends ViewFileSystemBaseTest {

    public static final Logger LOGGER = LoggerFactory.getLogger(TestViewFileSystemLinkRegex_RestartInjected.class);
    ...
    private static final File TEST_DIR = GenericTestUtils.getTestDir(TestViewFileSystemLinkRegex_RestartInjected.class.getSimpleName());
    ...
    public static void clusterSetupAtBeginning() throws IOException {
        ...
        cluster = new MiniDFSCluster.Builder(clusterConfig).nnTopology(MiniDFSNNTopology.simpleFederatedTopology(NAME_SPACES_COUNT)).numDataNodes(DATA_NODES_COUNT).build();
    ...
    private void testRegexMountpoint(Configuration config, String regexStr, String dstPathStr, String interceptorSettings, Path dirPathBeforeMountPoint, Path expectedResolveResult, int childrenFilesCnt) throws IOException, URISyntaxException {
        ...
        RestartFramework.at("after_dir_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Asserts
        ...
        try (FileSystem vfs = FileSystem.get(viewFsUri, config)) {
            ...
            RestartFramework.at("after_status_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
