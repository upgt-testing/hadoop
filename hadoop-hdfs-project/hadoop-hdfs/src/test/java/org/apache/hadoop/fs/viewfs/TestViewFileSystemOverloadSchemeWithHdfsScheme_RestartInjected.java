 ...
 */
public class TestViewFileSystemOverloadSchemeWithHdfsScheme_RestartInjected {
  ...
  public void testViewFsOverloadSchemeWhenInnerCacheDisabled()
    ...
    try (FileSystem fs = FileSystem.get(conf)) {
      ...
      RestartFramework.at("after_exists_check")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testMountTableNameShouldIgnorePortFromURI() throws Exception {
    ...
    try (FileSystem fs = FileSystem
      ...
      RestartFramework.at("after_mkdir_without_port")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_delete_without_port")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
    ...
    try (FileSystem fs = FileSystem.get(defaultFSURI, conf)) {
      fs.mkdirs(testDirOnRoot);
      RestartFramework.at("after_mkdir_with_port")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
          .execute();
      fs.delete(testDirOnRoot, true);
      RestartFramework.at("after_delete_with_port")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
