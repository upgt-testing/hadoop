 ...
 *  in the future).
 */
public class TestIncrementalBrVariations_RestartInjected_Randomized_123 {

    public static final Logger LOG = LoggerFactory.getLogger(TestIncrementalBrVariations_RestartInjected.class);
    ...
    static {
        ...
        GenericTestUtils.setLogLevel(TestIncrementalBrVariations_RestartInjected.LOG, Level.TRACE);
    }
    ...
    public void verifyIncrementalBlockReports(boolean splitReports) throws IOException {
        ...
        try (FsDatasetSpi.FsVolumeReferences volumes = dn0.getFSDataset().getFsVolumeReferences()) {
            ...
            if (splitReports) {
                RestartFramework.at("after_send_split_ibr").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            if (!splitReports) {
                // Send a combined report.
                cluster.getNameNodeRpc().blockReceivedAndDeleted(dn0Reg, poolId, reports);
                RestartFramework.at("after_send_combined_ibr").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            ...
            RestartFramework.at("after_flush_blockops").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDataNodeDoesNotSplitReports() throws IOException, InterruptedException {
        RestartFramework.at("after_delete_blocks").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        LocatedBlocks blocks = createFileGetBlocks(GenericTestUtils.getMethodName());
        ...
        RestartFramework.at("after_trigger_report").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(5000);
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNnLearnsNewStorages() throws IOException, InterruptedException {
        ...
        RestartFramework.at("after_flush_blockops").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_generate_report").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Send the report to the NN.
        cluster.getNameNodeRpc().blockReceivedAndDeleted(dn0Reg, poolId, reports);
        RestartFramework.at("after_send_report").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
