...
public class TestCacheAdminCLI_RestartInjected_Randomized_123 extends CLITestHelper {

    public static final Logger LOG = LoggerFactory.getLogger(TestCacheAdminCLI_RestartInjected.class);
    ...
    public void testAll() {
        super.testAll();
        RestartFramework.at("before_cache_operations").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cache_operations").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
}
