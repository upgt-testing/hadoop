...
public class TestWebHDFS {
    ...
    public void testWebHdfsEnabledByDefault() throws Exception {
        ...
        Assert.assertTrue(conf.getBoolean(HdfsClientConfigKeys.DFS_WEBHDFS_ENABLED_KEY, false));
    }
    ...
    public void testGetEZTrashRoot() throws Exception {
        ...
        conf.set(CommonConfigurationKeysPublic.HADOOP_SECURITY_KEY_PROVIDER_PATH, "jceks://file" + new Path(testRootDir.toString(), "test.jks").toUri());
        ...
        dfs.getClient().setKeyProvider(cluster.getNameNode().getNamesystem().getProvider());
        final String testkey = "test_key";
        DFSTestUtil.createKey(testkey, cluster, conf);
        final Path zone1 = new Path("/zone1");
        dfs.mkdirs(zone1, new FsPermission((short) 0700));
        dfsAdmin.createEncryptionZone(zone1, testkey, EnumSet.of(CreateEncryptionZoneFlag.PROVISION_TRASH));
        final Path insideEZ = new Path(zone1, "insideEZ");
        dfs.mkdirs(insideEZ, new FsPermission((short) 0700));
        assertEquals(dfs.getTrashRoot(insideEZ).toUri().getPath(), webhdfs.getTrashRoot(insideEZ).toUri().getPath());
        final Path outsideEZ = new Path("/outsideEZ");
        dfs.mkdirs(outsideEZ, new FsPermission((short) 0755));
        assertEquals(dfs.getTrashRoot(outsideEZ).toUri().getPath(), webhdfs.getTrashRoot(outsideEZ).toUri().getPath());
