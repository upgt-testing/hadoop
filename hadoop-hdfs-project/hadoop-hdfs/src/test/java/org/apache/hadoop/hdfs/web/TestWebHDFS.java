...
public class TestWebHDFS {
    ...
    public void testWebHdfsSnapshotList() throws Exception {
        ...
        try {
            ...
            cluster.upgradeNodeForTesting(0);
            for (int i = 0; i < dfsStatuses.length; i++) {
                Assert.assertEquals(statuses[i].getSnapshotID(), dfsStatuses[i].getSnapshotID());
                Assert.assertEquals(statuses[i].isDeleted(), dfsStatuses[i].isDeleted());
                Assert.assertTrue(Arrays.equals(statuses[i].getParentFullPath(), dfsStatuses[i].getParentFullPath()));
                Assert.assertEquals(dfsStatuses[i].getDirStatus().getChildrenNum(), statuses[i].getDirStatus().getChildrenNum());
                Assert.assertEquals(dfsStatuses[i].getDirStatus().getModificationTime(), statuses[i].getDirStatus().getModificationTime());
                Assert.assertEquals(dfsStatuses[i].getDirStatus().isDir(), statuses[i].getDirStatus().isDir());
    ...
    public void testSetQuota() throws Exception {
        ...
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
        LambdaTestUtils.intercept(IllegalArgumentException.class, () -> webHdfs.setQuotaByStorageType(path, StorageType.NVDIMM, -100));
    }
    ...
    public void testGetSnapshotTrashRoot() throws Exception {
        ...
        Path expectedPathAfter = new Path(ssDir1, new Path(FileSystem.TRASH_PREFIX, currentUser));
        assertEquals(expectedPathAfter.toUri().getPath(), trashPathAfter.toUri().getPath());
        // Cleanup
   ...
   */
    ...
    public void testFsStatus() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals(webHdfsFsStatus.getRemaining(), dfsFsStatus.getRemaining());
            cluster.restartNodeForTesting(0);
            cluster.upgradeNodeForTesting(0);
            Assert.assertEquals(webHdfsFsStatus.getCapacity(), dfsFsStatus.getCapacity());
        } finally {
            cluster.shutdown();
        }
    }
    ...
    public void testGetErasureCodingPolicies() throws Exception {
        ...
        try {
            ...
            final WebHdfsFileSystem webHdfs = WebHdfsTestUtil.getWebHdfsFileSystem(conf, WebHdfsConstants.WEBHDFS_SCHEME);
            final DistributedFileSystem dfs = cluster.getFileSystem();
            Collection<ErasureCodingPolicyInfo> webHdfsEcPolicyInfos = webHdfs.getAllErasureCodingPolicies();
            Collection<ErasureCodingPolicyInfo> dfsEcPolicyInfos = dfs.getAllErasureCodingPolicies();
