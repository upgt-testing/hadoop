...
public class TestWebHDFS {
    ...
    public void testRaceWhileNNStartup() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).build();
        cluster.waitActive();
        final NameNodeInterface namenode = cluster.getNameNode();
    ...
    public void testWebHdfsSps() throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE } }).storagesPerDatanode(2).numDataNodes(1).build();
            cluster.waitActive();
    ...
    public void testWebHdfsAppend() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(dnNumber).build();
        final WebHdfsFileSystem webFS = WebHdfsTestUtil.getWebHdfsFileSystem(conf, WebHdfsConstants.WEBHDFS_SCHEME);
    ...
    public void testFsserverDefaultsBackwardsCompatible() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).build();
        final WebHdfsFileSystem webfs = WebHdfsTestUtil.getWebHdfsFileSystem(conf, WebHdfsConstants.WEBHDFS_SCHEME);
        FSNamesystemInterface fsnSpy = NameNodeAdapter.spyOnNamesystem(cluster.getNameNode());
