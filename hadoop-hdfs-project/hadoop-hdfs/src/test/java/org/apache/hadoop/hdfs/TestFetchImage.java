...
public class TestFetchImage {
    ...
    private void testFetchImageInternal(final int nnIndex, final Path dir1, final Path dir2) throws Exception {
        ...
        try (FileSystem fs = cluster.getFileSystem(nnIndex)) {
            ...
            fs.mkdirs(dir1);
            fs.mkdirs(dir2);
            cluster.getNameNodeRpc(nnIndex).setSafeMode(SafeModeAction.SAFEMODE_ENTER, false);
            cluster.getNameNodeRpc(nnIndex).saveNamespace();
            cluster.getNameNodeRpc(nnIndex).setSafeMode(SafeModeAction.SAFEMODE_LEAVE, false);
            runFetchImage(dfsAdmin, cluster);
        }
    }
     ...
     * correct.
     */
    private static void runFetchImage(DFSAdmin dfsAdmin, MiniDFSClusterInJVM cluster) throws Exception {
    ...
    @Test(timeout = 30000)
    public void testFetchImageHA_withUpgrade20() throws Exception {
        final Path parent = new Path(PathUtils.getTestPath(getClass()), GenericTestUtils.getMethodName());
        int nnIndex = 0;
        /* run on nn0 as active */
        cluster.transitionToActive(nnIndex);
    ...
    @Test(timeout = 30000)
    public void testFetchImageHA_withUpgrade40() throws Exception {
        final Path parent = new Path(PathUtils.getTestPath(getClass()), GenericTestUtils.getMethodName());
        int nnIndex = 0;
        /* run on nn0 as active */
        cluster.transitionToActive(nnIndex);
    ...
    @Test(timeout = 30000)
    public void testFetchImageHA_withUpgrade60() throws Exception {
        final Path parent = new Path(PathUtils.getTestPath(getClass()), GenericTestUtils.getMethodName());
        int nnIndex = 0;
        /* run on nn0 as active */
        cluster.transitionToActive(nnIndex);
    ...
    @Test(timeout = 30000)
    public void testFetchImageHA_withUpgrade80() throws Exception {
        final Path parent = new Path(PathUtils.getTestPath(getClass()), GenericTestUtils.getMethodName());
        int nnIndex = 0;
        /* run on nn0 as active */
        cluster.transitionToActive(nnIndex);
