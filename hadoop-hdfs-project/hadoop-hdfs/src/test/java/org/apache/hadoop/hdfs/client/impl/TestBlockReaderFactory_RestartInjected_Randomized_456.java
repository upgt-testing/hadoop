...
public class TestBlockReaderFactory_RestartInjected_Randomized_456 {
    ...
    public void testMultipleWaitersOnShortCircuitCache() throws Exception {
        ...
        RestartFramework.at("after_cluster_start_multiwaiter").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(500);
        latch.countDown();
        for (int i = 0; i < NUM_THREADS; i++) {
        ...
        cluster.shutdown();
        sockDir.close();
        RestartFramework.at("after_file_creation_multiwaiter").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testShortCircuitCacheTemporaryFailure() throws Exception {
        ...
        final DistributedFileSystem dfs = cluster.getFileSystem();
        final String TEST_FILE = "/test_file";
        RestartFramework.at("after_file_creation_temp_failure").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final int TEST_FILE_LEN = 4000;
        ...
        RestartFramework.at("after_cluster_start_temp_failure").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.shutdown();
        sockDir.close();
        Assert.assertFalse(testFailed.get());
    }
    ...
    public void testShortCircuitCacheUnbufferDefault() throws Exception {
        testShortCircuitCacheUnbufferWithDisableInterval(DFS_DOMAIN_SOCKET_DISABLE_INTERVAL_SECOND_DEFAULT, true);
    }
    ...
    private void testShortCircuitCacheUnbufferWithDisableInterval(final long interval, final boolean disabled) throws Exception {
        ...
        try (TemporarySocketDirectory sockDir = new TemporarySocketDirectory()) {
            ...
            try (MiniDFSCluster cluster = builder.build();
                DistributedFileSystem dfs = (DistributedFileSystem) FileSystem.get(cluster.getURI(0), conf)) {
                cluster.waitActive();
                RestartFramework.at("after_cluster_start_unbuffer").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                final byte[] expected = DFSTestUtil.calculateFileContentsFromSeed(seed, testFileLen);
                try (FSDataInputStream in = dfs.open(testFile)) {
                    Assert.assertEquals(0, dfs.getClient().getClientContext().getShortCircuitCache(0).getReplicaInfoMapSize());
                    ...
                    dfs.getClient().getClientContext().getShortCircuitCache(0).setMaxTotalSize(0);
    ...
    private void validateReadResult(final DistributedFileSystem dfs, final byte[] expected, final byte[] actual, final int expectedScrRepMapSize) {
        Assert.assertThat(expected, CoreMatchers.is(actual));
        Assert.assertEquals(expectedScrRepMapSize, dfs.getClient().getClientContext().getShortCircuitCache(0).getReplicaInfoMapSize());
    }
    ...
    public void testShortCircuitReadFromServerWithoutShm() throws Exception {
        ...
        cluster.waitActive();
        RestartFramework.at("after_cluster_start_no_shm_server").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_creation_no_shm_server").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testShortCircuitReadFromClientWithoutShm() throws Exception {
        ...
        final MiniDFSCluster cluster = new MiniDFSCluster.Builder(serverConf).numDataNodes(1).build();
        cluster.waitActive();
        RestartFramework.at("after_cluster_start_no_shm_client").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        byte[] expected = DFSTestUtil.calculateFileContentsFromSeed(SEED, TEST_FILE_LEN);
        Assert.assertTrue(Arrays.equals(contents, expected));
        RestartFramework.at("after_file_creation_no_shm_client").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final ShortCircuitCache cache = fs.getClient().getClientContext().getShortCircuitCache(0);
    ...
    public void testShortCircuitCacheShutdown() throws Exception {
        ...
        final int TEST_FILE_LEN = 4000;
        final int SEED = 0xFADEC;
        RestartFramework.at("after_file_creation_cache_shutdown").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cluster_start_cache_shutdown").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(fs, new Path(TEST_FILE), TEST_FILE_LEN, (short) 1, SEED);
        byte[] contents = DFSTestUtil.readFileBuffer(fs, new Path(TEST_FILE));
        byte[] expected = DFSTestUtil.calculateFileContentsFromSeed(SEED, TEST_FILE_LEN);
        Assert.assertTrue(Arrays.equals(contents, expected));
    ...
    public void testPurgingClosedReplicas() throws Exception {
        ...
        RestartFramework.at("after_cluster_start_purge_replicas").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_creation_purge_replicas").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        dfs.close();
        cluster.shutdown();
        sockDir.close();
    }
