...
public class TestBlockTokenWithShortCircuitRead_RestartInjected_Randomized_123 {
    ...
    public void testShortCircuitReadWithInvalidToken() throws Exception {
        ...
        try {
            ...
            try (FSDataInputStream in = fs.open(fileToRead)) {
                ...
                RestartFramework.at("after_first_slot_check").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                // check once more. the number of slot objects should not be changed
                checkSlotsAfterSSRWithTokenExpiration(cache, datanode, in, myToken);
                RestartFramework.at("after_second_slot_check").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        } finally {
            if (cluster != null) {
                cluster.shutdown();
