...
public class TestFSImage {
    ...
    private void testPersistHelper(Configuration conf) throws IOException {
        ...
        try {
            ...
            INodeFileInterface file2Node = fsn.dir.getINode4Write(file2.toString()).asFile();
            ...
            // check lease manager
            LeaseInterface lease = fsn.leaseManager.getLease(file2Node);
    ...
    private void testSaveAndLoadStripedINodeFile(FSNamesystem fsn, Configuration conf, boolean isUC) throws IOException {
        ...
        INodeFileInterface fileByLoaded;
        if (isUC) {
    ...
    public void testRemovalStaleFsimageCkpt() throws IOException {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).format(true).build();
            conf.set(DFSConfigKeys.DFS_NAMENODE_SECONDARY_HTTP_ADDRESS_KEY, "0.0.0.0:0");
            secondary = new SecondaryNameNode(conf);
            // Do checkpointing
            secondary.doCheckpoint();
            NNStorageInterface storage = secondary.getFSImage().storage;
    ...
    public void testLoadMtimeAtime() throws Exception {
        ...
        try {
            ...
            cluster.shutdown();
            cluster = new MiniDockerDFSCluster.Builder(conf).format(false).numDataNodes(1).build();
            cluster.waitActive();
    ...
    public void testZeroBlockSize() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).waitSafeMode(false).startupOption(StartupOption.UPGRADE).build();
        try {
    ...
    public void testSupportBlockGroup() throws Exception {
        ...
        MiniDockerDFSCluster cluster = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(GROUP_SIZE).build();
            cluster.waitActive();
            DistributedFileSystem fs = cluster.getFileSystem();
            ...
            FSNamesystemInterface fsn = cluster.getNamesystem();
            INodeFileInterface inode = fsn.dir.getINode(file_10_4.toString()).asFile();
    ...
    public void testHasNonEcBlockUsingStripedIDForLoadFile() throws IOException {
        ...
        MiniDockerDFSCluster cluster = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(9).build();
            cluster.waitActive();
            DistributedFileSystem fs = cluster.getFileSystem();
            FSNamesystemInterface fns = cluster.getNamesystem();
            ...
            INodeFileInterface file = (INodeFile) fns.getFSDirectory().getINode(testFilePath);
    ...
    public void testBlockTypeProtoDefaultsToContiguous() throws Exception {
        ...
        INodeSection.INodeFileInterface inodeFile = builder.build();
    ...
    public void testSaveAndLoadFileUnderReplicationPolicyDir() throws IOException {
        ...
        try {
            ...
            // check internals of file2
            INodeFileInterface file2Node = fsn.dir.getINode4Write(replicaFile2.toString()).asFile();
            assertEquals("hello".length(), file2Node.computeFileSize());
            assertTrue(file2Node.isUnderConstruction());
            BlockInfo[] blks = file2Node.getBlocks();
            assertEquals(1, blks.length);
    ...
    public void testSaveAndLoadErasureCodingPolicies() throws IOException {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(10).build()) {
            cluster.waitActive();
            DistributedFileSystem fs = cluster.getFileSystem();
            DFSTestUtil.enableAllECPolicies(fs);
            // Save namespace and restart NameNode
    ...
    private void testChangeErasureCodingPolicyState(MiniDockerDFSCluster cluster, int blockSize, ErasureCodingPolicy targetPolicy, boolean isDefault) throws IOException {
        DistributedFileSystem fs = cluster.getFileSystem();
    ...
    private boolean isPolicyEnabledInFsImage(ErasureCodingPolicy testPolicy) {
        ...
        for (ErasureCodingPolicyInfoInterface p : persistedPolicies) {
    ...
    private MiniDockerDFSCluster createAndLoadParallelFSImage(Configuration conf) throws IOException {
    ...
    public void testUpdateBlocksMapAndNameCacheAsync() throws IOException {
        ...
        cluster.waitActive();
        DistributedFileSystem fs = cluster.getFileSystem();
        FSDirectoryInterface fsdir = cluster.getNameNode().namesystem.getFSDirectory();
