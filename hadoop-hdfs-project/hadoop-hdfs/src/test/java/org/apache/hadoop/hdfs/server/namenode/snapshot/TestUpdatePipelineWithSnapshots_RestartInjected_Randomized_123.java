...
public class TestUpdatePipelineWithSnapshots_RestartInjected_Randomized_123 {
    ...
    public void testUpdatePipelineAfterDelete() throws Exception {
        ...
        try {
            ...
            try {
                ...
                out.write(1);
                out.hflush();
                RestartFramework.at("after_file_create_hflush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_get_block_info").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_update_pipeline_attempt").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                namenode = cluster.getNameNodeRpc();
