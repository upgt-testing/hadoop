...
public class TestNNWithQJM_RestartInjected_Randomized_123 {
    ...
    public void testLogAndRestart() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_first_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Restart the NN and make sure the edit was persisted
            // and loaded again
            cluster.restartNameNode();
            RestartFramework.at("after_first_namenode_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.restartNameNode();
            RestartFramework.at("after_second_namenode_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(cluster.getFileSystem().exists(TEST_PATH));
    ...
    public void testNewNamenodeTakesOverWriter() throws Exception {
        ...
        try {
            ...
            cluster.getFileSystem().mkdirs(TEST_PATH);
            RestartFramework.at("after_mkdir_first_cluster").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_cluster_start").on(cluster2).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMismatchedNNIsRejected() throws Exception {
        ...
        try {
            ...
            cluster = new MiniDFSCluster.Builder(conf).numDataNodes(0).manageNameDfsDirs(false).format(false).checkExitOnShutdown(false).build();
