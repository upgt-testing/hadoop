...
public class TestMover_RestartInjected_Randomized_123 {
    ...
    public void testScheduleSameBlock() throws IOException {
        ...
        try {
            ...
            mover.init();
            RestartFramework.at("after_mover_init").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWithFederateClusterWithinSameNode() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_storage_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // move to ARCHIVE
            dfs1.setStoragePolicy(dir, "COLD");
            int rc = ToolRunner.run(conf, new Mover.Cli(), new String[] { "-p", nn1 + dir.toString() });
            Assert.assertEquals("Movement to ARCHIVE should be successful", 0, rc);
            RestartFramework.at("after_archive_move").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //move to DISK
    ...
    private void startAdditionalDNs(final Configuration conf, int newNodesRequired, StorageType[][] newTypes, final MiniDFSCluster cluster) throws IOException {
