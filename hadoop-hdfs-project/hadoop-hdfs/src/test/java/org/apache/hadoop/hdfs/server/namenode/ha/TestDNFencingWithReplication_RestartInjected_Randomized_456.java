 ...
 */
public class TestDNFencingWithReplication_RestartInjected_Randomized_456 {
    ...
    public void testFencingStress() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_threads_start").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            togglers.waitFor(RUNTIME);
            togglers.stop();
            harness.stopThreads();
            RestartFramework.at("after_threads_stop").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
