...
public class TestDFSStripedInputStream_RestartInjected_Randomized_123 {

    public static final Logger LOG = LoggerFactory.getLogger(TestDFSStripedInputStream_RestartInjected.class);
    ...
    public void testRefreshBlock() throws Exception {
        RestartFramework.at("after_striped_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPread() throws Exception {
        ...
        DFSTestUtil.createStripedFile(cluster, filePath, null, numBlocks, stripesPerBlock, false, ecPolicy);
        RestartFramework.at("after_file_creation_pread").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        LocatedBlocks lbs = fs.getClient().namenode.getBlockLocations(filePath.toString(), 0, blockGroupSize * numBlocks);
    ...
    public void testPreadWithDNFailure() throws Exception {
        ...
        RestartFramework.at("after_file_creation_with_dn_failure").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertArrayEquals(Arrays.copyOf(expected, done), Arrays.copyOf(readBuffer, done));
        // read the rest
    ...
    private void testStatefulRead(boolean useByteBuffer, boolean cellMisalignPacket) throws Exception {
        ...
        DFSTestUtil.createStripedFile(cluster, filePath, null, numBlocks, stripesPerBlock, false, ecPolicy);
        RestartFramework.at("after_stateful_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStatefulReadWithDNFailure() throws Exception {
        ...
        assertEquals(readSize, done);
        RestartFramework.at("after_file_creation_stateful_dn_failure").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertArrayEquals(expected, readBuffer);
    }
    ...
    public void testIdempotentClose() throws Exception {
        final int numBlocks = 2;
        RestartFramework.at("after_file_creation_idempotent_close").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createStripedFile(cluster, filePath, null, numBlocks, stripesPerBlock, false, ecPolicy);
    ...
    public void testReadFailToGetCurrentBlock() throws Exception {
        ...
        RestartFramework.at("after_file_write_get_current_block").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCloseDoesNotAllocateNewBuffer() throws Exception {
        final int numBlocks = 2;
        DFSTestUtil.createStripedFile(cluster, filePath, null, numBlocks, stripesPerBlock, false, ecPolicy);
        RestartFramework.at("after_file_creation_buffer_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try (DFSInputStream in = fs.getClient().open(filePath.toString())) {
            assertTrue(in instanceof DFSStripedInputStream);
    ...
    public void testReadWhenLastIncompleteCellComeInToDecodeAlignedStripe() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_incomplete_cell_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //2. Stop first DN from stripe.
            LocatedBlocks lbs = cluster.getNameNodeRpc().getBlockLocations(f.toString(), 0, fileLength);
            LocatedStripedBlock bg = (LocatedStripedBlock) (lbs.get(0));
    ...
    public void testUnbuffer() throws Exception {
        ...
        DFSStripedInputStream in = new DFSStripedInputStream(fs.getClient(), filePath.toString(), false, ecPolicy, null);
        RestartFramework.at("after_file_creation_unbuffer").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ByteBuffer readBuffer = ByteBuffer.allocate(fileSize);
        int done = 0;
        while (done < fileSize) {
            int ret = in.read(readBuffer);
    ...
    public void testBlockReader() throws Exception {
        // RS-6-3-1024k
        ErasureCodingPolicy targetPolicy = StripedFileTestUtil.getDefaultECPolicy();
        ...
        RestartFramework.at("after_file_write_block_reader").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try (DFSStripedInputStream in = (DFSStripedInputStream) fs.getClient().open(filePath.toString())) {
            // Verify pread:
            verifyPreadRanges(in, 0, 2 * cellSize, 2 * cellSize, Arrays.asList("0_0_1048576", "1_0_1048576"));
            verifyPreadRanges(in, 0, 5 * cellSize + 9527, 5 * cellSize + 9527, Arrays.asList("0_0_1048576", "1_0_1048576", "2_0_1048576", "3_0_1048576", "4_0_1048576", "5_0_1048576"));
            verifyPreadRanges(in, 100, 5 * cellSize + 9527, 5 * cellSize + 9527, Arrays.asList("0_100_1048476", "1_0_1048576", "2_0_1048576", "3_0_1048576", "4_0_1048576", "5_0_1048576"));
            verifyPreadRanges(in, stripeSize * 3, 2 * cellSize, cellSize + 100, Arrays.asList("0_1048576_1048576", "1_1048576_100"));
            // Verify sread:
            verifySreadRanges(in, 0, Arrays.asList("0_0_2097152", "1_0_2097152", "2_0_2097152", "3_0_2097152", "4_0_2097152", "5_0_2097152"));
            verifySreadRanges(in, stripeSize * 2, Arrays.asList("0_0_2097152", "1_0_1048676", "2_0_1048576", "3_0_1048576", "4_0_1048576", "5_0_1048576"));
        }
    }

    private void verifyPreadRanges(DFSStripedInputStream in, long position, int length, int lengthExpected, List<String> rangesExpected) throws Exception {
        // range format: chunkIndex_offset_len
    ...
    private void verifySreadRanges(DFSStripedInputStream in, long position, List<String> rangesExpected) throws Exception {
        // range format: chunkIndex_offset_len
        List<String> ranges = new ArrayList<>();
        DFSClientFaultInjector.set(new DFSClientFaultInjector() {
