...
public class TestFSImageWithSnapshot {
    ...
    public void testSnapshotOnRoot() throws Exception {
        ...
        fsn = cluster.getNamesystem();
        hdfs = cluster.getFileSystem();
        INodeDirectoryInterface rootNode = fsn.dir.getINode4Write(root.toString()).asDirectory();
        ...
        SnapshotInterface s1 = rootNode.getSnapshot(DFSUtil.string2Bytes("s1"));
    ...
    void checkImage(int s) throws IOException {
        ...
        for (SnapshottableDirectoryStatusInterface sBefore : dirBefore) {
            pathListBefore.add(sBefore.getFullPath().toString());
        }
        for (SnapshottableDirectoryStatusInterface sAfter : dirAfter) {
    ...
    public void testSaveLoadImageAfterSnapshotDeletion() throws Exception {
        ...
        // restart cluster
        cluster.shutdown();
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).format(false).build();
        cluster.waitActive();
        fsn = cluster.getNamesystem();
        hdfs = cluster.getFileSystem();
