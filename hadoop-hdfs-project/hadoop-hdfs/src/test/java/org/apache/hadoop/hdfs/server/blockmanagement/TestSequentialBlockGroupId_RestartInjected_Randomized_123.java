 ...
 * collision handling.
 */
public class TestSequentialBlockGroupId_RestartInjected_Randomized_123 {
    ...
    public void testBlockGroupIdGeneration() throws IOException {
        ...
        RestartFramework.at("after_get_all_blocks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_block_id_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_block_manager_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTriggerBlockGroupIdCollisionWithLegacyBlockId() throws Exception {
        ...
        RestartFramework.at("after_legacy_block_verification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_legacy_block_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Reset back to the initial value to trigger collision
        blockGrpIdGenerator.setCurrentValue(blockGroupIdInitialValue);
        // Trigger collisions by creating a new file.
        Path path2 = new Path(ecDir, "testCollisionWithLegacyBlock_file2.dat");
        RestartFramework.at("after_ec_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(fs, path2, cellSize, fileLen, blockSize, REPLICATION, SEED);
        RestartFramework.at("after_ec_file_blocks_retrieved").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<LocatedBlock> blocks2 = DFSTestUtil.getAllBlocks(fs, path2);
        assertThat("Wrong BlockGrps", blocks2.size(), is(blockGrpCount));
        RestartFramework.at("after_counter_reset").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Make sure that file1 and file2 block IDs are different
