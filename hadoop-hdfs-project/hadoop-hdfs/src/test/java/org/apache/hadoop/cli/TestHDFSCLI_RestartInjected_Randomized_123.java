...
public class TestHDFSCLI_RestartInjected_Randomized_123 extends CLITestHelperDFS {
    ...
    public void testAll() {
        super.testAll();
        RestartFramework.at("before_hdfs_operations").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_hdfs_operations").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
}
