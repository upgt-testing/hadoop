...
public class TestFsckWithMultipleNameNodes {
    ...
    private static class Suite {
        ...
        Suite(MiniDockerDFSCluster cluster, final int nNameNodes, final int nDataNodes) throws IOException {
    ...
    private void runTest(final int nNameNodes, final int nDataNodes, Configuration conf) throws Exception {
        ...
        LOG.info("RUN_TEST -1");
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleFederatedTopology(nNameNodes)).numDataNodes(nDataNodes).build();
        LOG.info("RUN_TEST 0");
