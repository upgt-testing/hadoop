...
public class TestFailureToReadEdits_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestFailureToReadEdits_RestartInjected.class);
    ...
    public TestFailureToReadEdits_RestartInjected_Randomized_123(TestType clusterType, Boolean useAsyncEditLogging) {
    ...
    public void testFailuretoReadEdits() throws Exception {
        RestartFramework.at("after_mkdirs_test_dir3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_mkdirs_test_dir2").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // This is the op the mocking will cause to fail to be read.
        assertTrue(fs.mkdirs(new Path(TEST_DIR3)));
        RestartFramework.at("after_allow_editlog_read").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_setowner_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // Null because it was deleted.
        assertNull(NameNodeAdapter.getFileInfo(nn1, TEST_DIR1, false, false, false));
        RestartFramework.at("after_cause_failure_on_editlog").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // Should have been successfully created.
        assertTrue(NameNodeAdapter.getFileInfo(nn1, TEST_DIR2, false, false, false).isDirectory());
        RestartFramework.at("after_first_mkdirs_and_catchup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckpointStartingMidEditsFile() throws Exception {
        ...
        RestartFramework.at("after_mkdirs_test_dir2_dir3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // It should also upload it back to the active.
    ...
    private LimitedEditLogAnswer causeFailureOnEditLogRead() throws IOException {
        ...
        doAnswer(answer).when(spyEditLog).selectInputStreams(anyLong(), anyLong(), any(), anyBoolean(), anyBoolean());
        return answer;
    }
