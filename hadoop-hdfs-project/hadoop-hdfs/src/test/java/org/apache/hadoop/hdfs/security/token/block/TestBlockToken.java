...
public class TestBlockToken {
    ...
    private static class GetLengthAnswer implements Answer<GetReplicaVisibleLengthResponseProto> {
        ...
        public GetReplicaVisibleLengthResponseProto answer(InvocationOnMock invocation) throws IOException {
            ...
            for (TokenIdentifier tokenId : tokenIds) {
                ...
                sm.checkAccess(id, null, PBHelperClient.convert(req.getBlock()), BlockTokenIdentifier.AccessMode.WRITE);
    ...
    private BlockTokenIdentifier generateTokenId(BlockTokenSecretManager sm, ExtendedBlock block, EnumSet<BlockTokenIdentifier.AccessMode> accessModes) throws IOException {
    ...
    private void tokenGenerationAndVerification(BlockTokenSecretManager master, BlockTokenSecretManager slave) throws Exception {
        ...
        // multi-mode tokens
        Token<BlockTokenIdentifier> mtoken = master.generateToken(block3, EnumSet.allOf(BlockTokenIdentifier.AccessMode.class));
        for (BlockTokenIdentifier.AccessMode mode : BlockTokenIdentifier.AccessMode.values()) {
    ...
    @Test
    public void testBlockTokenInLastLocatedBlock_withUpgrade20() throws IOException, InterruptedException {
        Configuration conf = new HdfsConfiguration();
        conf.setBoolean(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_ENABLE_KEY, true);
        conf.setInt(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, 512);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
    ...
    @Test
    public void testBlockTokenInLastLocatedBlock_withUpgrade40() throws IOException, InterruptedException {
        Configuration conf = new HdfsConfiguration();
        conf.setBoolean(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_ENABLE_KEY, true);
        conf.setInt(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, 512);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
    ...
    @Test
    public void testBlockTokenInLastLocatedBlock_withUpgrade60() throws IOException, InterruptedException {
        Configuration conf = new HdfsConfiguration();
        conf.setBoolean(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_ENABLE_KEY, true);
        conf.setInt(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, 512);
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
