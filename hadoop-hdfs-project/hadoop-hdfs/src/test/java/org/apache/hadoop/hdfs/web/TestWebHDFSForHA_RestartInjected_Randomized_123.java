...
public class TestWebHDFSForHA_RestartInjected_Randomized_123 {
    ...
    public void testHA() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_first_mkdir").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSecureHAToken() throws IOException, InterruptedException {
        ...
        try {
            ...
            RestartFramework.at("after_get_delegation_token").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.shutdownNameNode(0);
            cluster.transitionToActive(1);
            RestartFramework.at("after_failover").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            token.renew(conf);
    ...
    public void testFailoverAfterOpen() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_create").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    synchronized (TestWebHDFSForHA_RestartInjected.this) {
                        resultMap.put("mkdirs", result);
                        TestWebHDFSForHA_RestartInjected.this.notifyAll();
                    }
                }
            }.start();
            Thread.sleep(1000);
