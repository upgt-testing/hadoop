 ...
 */
public class TestNestedSnapshots_RestartInjected_Randomized_456 {
    ...
    public void testNestedSnapshots() throws Exception {
        ...
        RestartFramework.at("after_delete_snapshot_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_mkdirs_subdirs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_file2").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.createSnapshot(foo, s1name);
        RestartFramework.at("after_allow_snapshot_bar").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_allow_snapshot_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_allow_snapshot_foo").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_delete_snapshot_foo").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disallow_snapshot_foo").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path rootSnapshot = hdfs.createSnapshot(rootPath);
        RestartFramework.at("after_create_snapshot_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_disallow_snapshot_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            hdfs.allowSnapshot(rootPath);
            Assert.fail();
    ...
    public void testSnapshotLimit() throws Exception {
        ...
        for (; s < SNAPSHOT_QUOTA_DEFAULT; s++) {
            ...
            if (s % step == 0) {
                ...
                RestartFramework.at("after_create_file_in_loop_" + s).on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                RestartFramework.at("after_create_snapshot_in_loop_" + s).on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
        try {
    ...
    public void testDisallowNestedSnapshottableDir() throws Exception {
        RestartFramework.at("after_allow_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.getNamesystem().getSnapshotManager().setAllowNestedSnapshots(true);
        RestartFramework.at("after_disallow_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
