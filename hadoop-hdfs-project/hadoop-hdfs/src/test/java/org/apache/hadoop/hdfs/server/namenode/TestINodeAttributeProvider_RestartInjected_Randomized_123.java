...
public class TestINodeAttributeProvider_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestINodeAttributeProvider_RestartInjected.class);
    ...
    public static class MyAuthorizationProviderAccessException extends AccessControlException {
    ...
    public static class MyAuthorizationProvider extends INodeAttributeProvider {
        ...
        public static class MyAccessControlEnforcer implements AccessControlEnforcer {
            ...
            public void checkPermissionWithContext(AuthorizationContext authzContext) throws AccessControlException {
                if (authzContext.getAncestorIndex() > 1 && authzContext.getInodes()[1].getLocalName().equals("user") && authzContext.getInodes()[2].getLocalName().equals("acl") || runPermissionCheck) {
    ...
    public void setUp() throws IOException {
        ...
        conf.set(DFSConfigKeys.DFS_NAMENODE_INODE_ATTRIBUTES_PROVIDER_BYPASS_USERS_KEY, " u2,, ,u3, ");
    ...
    public void testDelegationToProvider() throws Exception {
        ...
        RestartFramework.at("after_tmp_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ugi.doAs(new PrivilegedExceptionAction<Void>() {
            ...
            public Void run() throws Exception {
                ...
                CALLED.clear();
                RestartFramework.at("after_foo_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                RestartFramework.at("after_list_status").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testBypassProviderHelper(final String[] users, final short expectedPermission, final boolean bypass) throws Exception {
        ...
        fs.mkdirs(userPath);
        RestartFramework.at("after_directory_structure_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (String user : users) {
            ...
            ugiBypass.doAs(new PrivilegedExceptionAction<Void>() {
                ...
                public Void run() throws Exception {
                    FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
                    Assert.assertEquals(expectedPermission, fs.getFileStatus(authzChild).getPermission().toShort());
                    asserter.doAssert(CALLED.contains("getAttributes"));
                    asserter.doAssert(CALLED.contains("checkPermission|null|null|null"));
                    CALLED.clear();
                    Assert.assertEquals(expectedPermission, fs.listStatus(userPath)[0].getPermission().toShort());
                    asserter.doAssert(CALLED.contains("getAttributes"));
    ...
    public void testAclFeature() throws Exception {
        ...
        ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
            ...
            fs.mkdirs(aclDir);
            RestartFramework.at("after_acl_dir_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path aclChildDir = new Path(aclDir, "subdir");
            fs.mkdirs(aclChildDir);
            RestartFramework.at("after_acl_child_dir_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    @Test
    public // from the Attribute Provider, and not from HDFS.
    void testGetAclStatusReturnsProviderOwnerPerms() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
        final Path authzChild = new Path("/user/authz/child2");
        fs.mkdirs(userPath);
        fs.setPermission(userPath, new FsPermission(HDFS_PERMISSION));
        RestartFramework.at("after_directory_setup").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.mkdirs(authz);
        fs.setPermission(authz, new FsPermission(HDFS_PERMISSION));
        fs.mkdirs(authzChild);
        fs.setPermission(authzChild, new FsPermission(HDFS_PERMISSION));
        ...
        ugi.doAs(new PrivilegedExceptionAction<Void>() {
            ...
            public Void run() throws Exception {
                FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
                Assert.assertEquals(PROVIDER_PERMISSION, fs.getFileStatus(authzChild).getPermission().toShort());
                Assert.assertEquals("foo", fs.getAclStatus(authzChild).getOwner());
                Assert.assertEquals("bar", fs.getAclStatus(authzChild).getGroup());
                Assert.assertEquals(PROVIDER_PERMISSION, fs.getAclStatus(authzChild).getPermission().toShort());
                return null;
            }
        });
    }
    ...
    @Test
    public // and re-thrown as plain ACE exceptions.
    void testSubClassedAccessControlExceptions() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
    ...
    @Test
    public // attribute provider is configured) and not the underlying HDFS permissions.
    void testContentSummary() throws Exception {
        ...
        fs.mkdirs(authzChild);
        RestartFramework.at("after_directory_permissions_setup").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    @Test
    public // revert for future reference.
    void testAttrProviderWorksCorrectlyOnRenamedSnapshotPaths() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        ...
        RestartFramework.at("after_first_rename").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_file1_file2_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path sub1foo = new Path(sub1, "foo");
        hdfs.mkdirs(sub1);
        RestartFramework.at("after_sub1_structure_creation").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.mkdirs(sub1foo);
        Path f = new Path(sub1foo, "file0");
        DFSTestUtil.createFile(hdfs, f, 0, (short) 1, 0);
        hdfs.allowSnapshot(parent);
        RestartFramework.at("after_s1_s2_snapshots").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.createSnapshot(parent, "s0");
        RestartFramework.at("after_s0_snapshot").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_snapshot_enabled").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_second_rename").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.mkdirs(sub3);
        // mv /parent/sub2/foo to /parent/sub3/foo
        hdfs.rename(sub2foo, sub3);
        hdfs.delete(sub3, true);
        UserGroupInformation ugi = UserGroupInformation.createUserForTesting("u1", new String[] { "g1" });
        RestartFramework.at("after_delete").on(miniDFS).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ugi.doAs(new PrivilegedExceptionAction<Void>() {
