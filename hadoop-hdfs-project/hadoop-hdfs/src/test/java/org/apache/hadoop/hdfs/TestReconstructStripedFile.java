...
public class TestReconstructStripedFile {
    ...
    public int getPendingTimeout() {
        return DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_DEFAULT;
    }
    ...
    public void setup() throws IOException {
        ...
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_KEY, getPendingTimeout());
        conf.setBoolean(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_VALIDATION_KEY, isValidationEnabled());
        File basedir = new File(GenericTestUtils.getRandomizedTempPath());
        cluster = new MiniDockerDFSCluster.Builder(conf, basedir).numDataNodes(dnNum).build();
        cluster.waitActive();
    ...
    public void testRecoverOneParityBlock() throws Exception {
        ...
        assertFileBlocksReconstruction("/testRecoverOneParityBlock", fileLen, ReconstructionType.ParityOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverOneParityBlock1() throws Exception {
        int fileLen = cellSize + cellSize / 10;
        assertFileBlocksReconstruction("/testRecoverOneParityBlock1", fileLen, ReconstructionType.ParityOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverOneParityBlock2() throws Exception {
        int fileLen = 1;
        assertFileBlocksReconstruction("/testRecoverOneParityBlock2", fileLen, ReconstructionType.ParityOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverOneParityBlock3() throws Exception {
        int fileLen = (dataBlkNum - 1) * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverOneParityBlock3", fileLen, ReconstructionType.ParityOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverAllParityBlocks() throws Exception {
        int fileLen = dataBlkNum * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverAllParityBlocks", fileLen, ReconstructionType.ParityOnly, parityBlkNum);
    }

    @Test(timeout = 120000)
    public void testRecoverAllDataBlocks() throws Exception {
        int fileLen = (dataBlkNum + parityBlkNum) * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverAllDataBlocks", fileLen, ReconstructionType.DataOnly, parityBlkNum);
    }

    @Test(timeout = 120000)
    public void testRecoverAllDataBlocks1() throws Exception {
        int fileLen = parityBlkNum * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverAllDataBlocks1", fileLen, ReconstructionType.DataOnly, parityBlkNum);
    }

    @Test(timeout = 120000)
    public void testRecoverOneDataBlock() throws Exception {
        int fileLen = (dataBlkNum + 1) * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverOneDataBlock", fileLen, ReconstructionType.DataOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverOneDataBlock1() throws Exception {
        int fileLen = cellSize + cellSize / 10;
        assertFileBlocksReconstruction("/testRecoverOneDataBlock1", fileLen, ReconstructionType.DataOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverOneDataBlock2() throws Exception {
        int fileLen = 1;
        assertFileBlocksReconstruction("/testRecoverOneDataBlock2", fileLen, ReconstructionType.DataOnly, 1);
    }

    @Test(timeout = 120000)
    public void testRecoverAnyBlocks() throws Exception {
        int fileLen = parityBlkNum * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverAnyBlocks", fileLen, ReconstructionType.Any, random.nextInt(parityBlkNum) + 1);
    }

    @Test(timeout = 120000)
    public void testRecoverAnyBlocks1() throws Exception {
        int fileLen = (dataBlkNum + parityBlkNum) * blockSize + blockSize / 10;
        assertFileBlocksReconstruction("/testRecoverAnyBlocks1", fileLen, ReconstructionType.Any, random.nextInt(parityBlkNum) + 1);
    }
     ...
     */
    void assertFileBlocksReconstruction(String fileName, int fileLen, ReconstructionType type, int toRecoverBlockNum) throws Exception {
        ...
        for (DatanodeInfoInterface storageInfo : storageInfos) {
    ...
    private int[] sortTargetsByReplicas(ExtendedBlock[] blocks, int[] targetDNs) {
        ...
        for (int i = 0; i < blocks.length; i++) {
            ...
            if (result[i] == -1) {
                Assert.fail("Failed to reconstruct striped block: " + blocks[i].getBlockId());
            }
        }
        return result;
    }
   ...
   */
    ...
    public void testProcessErasureCodingTasksSubmitionShouldSucceed() throws Exception {
        DataNodeInterface dataNode = cluster.dataNodes.get(0).datanode;
        ...
        DatanodeStorageInfoInterface targetDnInfos_1 = BlockManagerTestUtil.newDatanodeStorageInfo(DFSTestUtil.getLocalDatanodeDescriptor(), new DatanodeStorage("s01"));
    ...
    private void testNNSendsErasureCodingTasks(int deadDN) throws Exception {
        ...
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_KEY, 10);
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_REPLICATION_MAX_STREAMS_KEY, 20);
        conf.setInt(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_THREADS_KEY, 2);
        ...
        for (DataNodeInterface dn : dataNodes.subList(0, deadDN)) {
            shutdownDataNode(dn);
        }
        final FSNamesystemInterface ns = cluster.getNamesystem();
        GenericTestUtils.waitFor(() -> ns.getPendingDeletionBlocks() == 0, 500, 30000);
        ...
        while (ns.getPendingReconstructionBlocks() > 0) {
            ...
            assertTrue(String.format("Found %d timeout pending reconstruction tasks", timeoutPending), timeoutPending == 0);
            Thread.sleep(1000);
        }
        // Verify all DN reaches zero xmitsInProgress.
        GenericTestUtils.waitFor(() -> cluster.getDataNodes().stream().mapToInt(DataNode::getXmitsInProgress).sum() == 0, 500, 30000);
    }

    @Test(timeout = 180000)
    public void testErasureCodingWorkerXmitsWeight() throws Exception {
        testErasureCodingWorkerXmitsWeight(0.5f, (int) (ecPolicy.getNumDataUnits() * 0.5f));
    ...
    private void testErasureCodingWorkerXmitsWeight(float weight, int expectedWeight) throws Exception {
        ...
        conf.setFloat(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_XMITS_WEIGHT_KEY, weight);
        cluster.shutdown();
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(dnNum).build();
        cluster.waitActive();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(ecPolicy.getName());
        fs.getClient().setErasureCodingPolicy("/", ecPolicy.getName());
        ...
        int corruptBlocks = dn.getFSDataset().getFinalizedBlocks(cluster.getNameNode().getNamesystem().getBlockPoolId()).size();
        ...
        try {
            ...
            LambdaTestUtils.await(30 * 1000, 500, () -> {
                int totalXmits = cluster.getDataNodes().stream().mapToInt(DataNode::getXmitsInProgress).sum();
                return totalXmits == expectedXmits;
            });
        } finally {
            barrier.await();
            DataNodeFaultInjector.set(oldInjector);
            for (final DataNodeInterface curDn : cluster.getDataNodes()) {
                GenericTestUtils.waitFor(() -> curDn.getXceiverCount() <= 1, 10, 60000);
                GenericTestUtils.waitFor(() -> curDn.getXmitsInProgress() == 0, 10, 2500);
            }
        }
    }
    ...
    public void testTimeoutReadBlockInReconstruction() throws Exception {
        ...
        int stripedReadTimeoutInMills = conf.getInt(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_STRIPED_READ_TIMEOUT_MILLIS_KEY, DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_STRIPED_READ_TIMEOUT_MILLIS_DEFAULT);
        Assert.assertTrue(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_STRIPED_READ_TIMEOUT_MILLIS_KEY + " must be greater than 2000", stripedReadTimeoutInMills > 2000);
        DataNodeFaultInjector oldInjector = DataNodeFaultInjector.get();
        DataNodeFaultInjector timeoutInjector = new DataNodeFaultInjector() {
            ...
            public void delayBlockReader() {
                ...
                if (index == 1) {
                    try {
                        GenericTestUtils.waitFor(() -> numDelayReader.get() >= ecPolicy.getNumDataUnits() + 1, 50, stripedReadTimeoutInMills * 3);
    ...
    public void testAbnormallyCloseDoesNotWriteBufferAgain() throws Exception {
        ...
        int stripedReadTimeoutInMills = conf.getInt(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_STRIPED_READ_TIMEOUT_MILLIS_KEY, DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_STRIPED_READ_TIMEOUT_MILLIS_DEFAULT);
        Assert.assertTrue(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_STRIPED_READ_TIMEOUT_MILLIS_KEY + " must be greater than 2000", stripedReadTimeoutInMills > 2000);
        ElasticByteBufferPool bufferPool = (ElasticByteBufferPool) ErasureCodingTestHelper.getBufferPool();
        ...
        DataNodeFaultInjector timeoutInjector = new DataNodeFaultInjector() {
                ...
                if (index > ecPolicy.getNumDataUnits() + 1) {
                    try {
                        GenericTestUtils.waitFor(() -> {
                            LOG.info("Close by NPE: {}, continue read: {}", closeByNPE, continueRead);
                            return closeByNPE.get() ? continueRead.get() : index == finishedReadBlock.get() + 1;
                        }, 5, stripedReadTimeoutInMills * 3);
                    } catch (TimeoutException e) {
            ...
            public void interceptFreeBlockReaderBuffer() {
                ...
                if (num >= ecPolicy.getNumDataUnits() + 1) {
                    ...
                    try {
                        GenericTestUtils.waitFor(() -> finishedReadBlock.get() >= 2 * ecPolicy.getNumDataUnits() + 1, 50, stripedReadTimeoutInMills * 3);
                    } catch (TimeoutException e) {
    ...
    private void assertBufferPoolIsEmpty(ElasticByteBufferPool bufferPool, boolean direct) {
    ...
    private void emptyBufferPool(ElasticByteBufferPool bufferPool, boolean direct) {
        while (bufferPool.size(direct) != 0) {
