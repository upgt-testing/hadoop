 ...
 */
public class TestDataNodeVolumeMetrics_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestDataNodeVolumeMetrics_RestartInjected.class);
    ...
    public void testVolumeMetrics() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_file_append").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWriteIoVolumeMetrics() throws IOException {
        Configuration conf = new HdfsConfiguration();
        conf.setInt(DFSConfigKeys.DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, 100);
        MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).storageTypes(new StorageType[] { StorageType.RAM_DISK, StorageType.DISK }).storagesPerDatanode(2).build();
        try {
            FileSystem fs = cluster.getFileSystem();
            final Path fileName = new Path("/test.dat");
            final long fileLen = Integer.MAX_VALUE + 1L;
            ...
            assertTrue(metrics.getWriteIoSampleCount() > metrics.getFlushIoSampleCount());
            lastWriteIoSampleCount = metrics.getWriteIoSampleCount();
            RestartFramework.at("after_first_metrics_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (FSDataOutputStream out = fs.append(fileName)) {
                out.writeBytes("hello world");
                out.hflush();
            }
            RestartFramework.at("after_file_append").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
