...
public class TestNameNodeMetrics {
    ...
    static {
        ...
        CONF.setTimeDuration(MiniDockerDFSCluster.DFS_NAMENODE_DECOMMISSION_INTERVAL_TESTING_KEY, 999, TimeUnit.DAYS);
    ...
    public void testStaleNodes() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            DatanodeDescriptorInterface dnDes = cluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getDatanode(dn.getDatanodeId());
    ...
    public void testTransactionSinceLastCheckpointMetrics() throws Exception {
        ...
        while (retryCount < 5) {
            try {
                ...
                MiniDockerDFSCluster cluster2 = new MiniDockerDFSCluster.Builder(conf2).nnTopology(topology).numDataNodes(1).build();
                cluster2.waitActive();
                DistributedFileSystem fs2 = cluster2.getFileSystem(0);
                NameNodeInterface nn0 = cluster2.getNameNode(0);
                NameNodeInterface nn1 = cluster2.getNameNode(1);
    ...
    public void testGenerateEDEKTime() throws IOException, NoSuchAlgorithmException {
        ...
        try (MiniDockerDFSCluster clusterEDEK = new MiniDockerDFSCluster.Builder(conf, basedir).numDataNodes(1).build()) {
    ...
    public void testEditLogTailing() throws Exception {
        ...
        try {
            dfsCluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(0).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
