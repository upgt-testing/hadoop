...
public class TestMover {
    ...
    private void testWithinSameNode(Configuration conf) throws Exception {
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    public void testWithFederateClusterWithinSameNode() throws Exception {
        final Configuration conf = new HdfsConfiguration();
        initConf(conf);
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(4).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).nnTopology(MiniDFSNNTopology.simpleFederatedTopology(2)).build();
    ...
    public void testWithFederatedCluster() throws Exception {
        final Configuration conf = new HdfsConfiguration();
        initConf(conf);
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).nnTopology(MiniDFSNNTopology.simpleFederatedTopology(2)).numDataNodes(4).build();
        DFSTestUtil.setFederatedConfiguration(cluster, conf);
        try {
            cluster.waitActive();
            final String file = "/test/file";
    ...
    public void testWithFederatedHACluster() throws Exception {
        final Configuration conf = new HdfsConfiguration();
        initConf(conf);
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).nnTopology(MiniDFSNNTopology.simpleHAFederatedTopology(2)).numDataNodes(4).build();
    ...
    public void testMoverCli() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(clusterConf).numDataNodes(0).build();
        try {
    ...
    public void testMoverCliWithHAConf() throws Exception {
        final Configuration conf = new HdfsConfiguration();
        conf.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, StoragePolicySatisfierMode.NONE.toString());
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
    ...
    public void testMoverCliWithFederation() throws Exception {
        final Configuration clusterConf = new HdfsConfiguration();
        clusterConf.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, StoragePolicySatisfierMode.NONE.toString());
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(clusterConf).nnTopology(MiniDFSNNTopology.simpleFederatedTopology(3)).numDataNodes(0).build();
        final Configuration conf = new HdfsConfiguration();
        clusterConf.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, StoragePolicySatisfierMode.NONE.toString());
        DFSTestUtil.setFederatedConfiguration(cluster, conf);
        try {
    ...
    public void testMoverCliWithFederationHA() throws Exception {
        final Configuration clusterConf = new HdfsConfiguration();
        clusterConf.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, StoragePolicySatisfierMode.NONE.toString());
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(clusterConf).nnTopology(MiniDFSNNTopology.simpleHAFederatedTopology(3)).numDataNodes(0).build();
        final Configuration conf = new HdfsConfiguration();
        clusterConf.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, StoragePolicySatisfierMode.NONE.toString());
        DFSTestUtil.setFederatedHAConfiguration(cluster, conf);
        try {
    ...
    public void testTwoReplicaSameStorageTypeShouldNotSelect() throws Exception {
        ...
        final Configuration conf = new HdfsConfiguration();
        initConf(conf);
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE }, { StorageType.DISK, StorageType.DISK }, { StorageType.DISK, StorageType.ARCHIVE } }).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    public void testMoveWhenStoragePolicyNotSatisfying() throws Exception {
        ...
        final Configuration conf = new HdfsConfiguration();
        conf.set(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, StoragePolicySatisfierMode.NONE.toString());
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storageTypes(new StorageType[][] { { StorageType.DISK }, { StorageType.DISK }, { StorageType.DISK } }).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    public void testMoverFailedRetry() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE }, { StorageType.DISK, StorageType.ARCHIVE }, { StorageType.DISK, StorageType.ARCHIVE } }).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    public void testBalancerMaxIterationTimeNotAffectMover() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.DISK }, { StorageType.ARCHIVE, StorageType.ARCHIVE } }).build();
        try {
            cluster.waitActive();
    ...
    public void testMoverWithPinnedBlocks() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    public void testMoverFailedRetryWithPinnedBlocks() throws Exception {
        ...
        initConf(conf);
        conf.set(DFSConfigKeys.DFS_MOVER_RETRY_MAX_ATTEMPTS_KEY, "2");
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE }, { StorageType.DISK, StorageType.ARCHIVE } }).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    public void testMoverWhenStoragePolicyUnset() throws Exception {
        final Configuration conf = new HdfsConfiguration();
        initConf(conf);
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).storageTypes(new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE } }).build();
        try {
            cluster.waitActive();
            final DistributedFileSystem dfs = cluster.getFileSystem();
    ...
    private void createFileWithFavoredDatanodes(final Configuration conf, final MiniDockerDFSCluster cluster, final DistributedFileSystem dfs) throws IOException {
    ...
    private void startAdditionalDNs(final Configuration conf, int newNodesRequired, StorageType[][] newTypes, final MiniDockerDFSCluster cluster) throws IOException {
        cluster.startDataNodes(conf, newNodesRequired, newTypes, true, null, null, null, null, null, false, false, false, null, null, null);
        cluster.triggerHeartbeats();
    }
}
