 ...
 */
public class TestRedundancyMonitor_RestartInjected_Randomized_123 {
    ...
    public void testChooseTargetWhenAllDataNodesStop() throws Throwable {
        ...
        try (MiniDFSCluster miniCluster = new MiniDFSCluster.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build()) {
            miniCluster.waitActive();
            RestartFramework.at("after_cluster_setup").on(miniCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_mock_setup").on(miniCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Future<Void> chooseTargetFuture = pool.submit(() -> {
                replicator.chooseTarget(FILENAME, 2, dns.iterator().next(), new ArrayList<DatanodeStorageInfo>(), false, null, BLOCK_SIZE, TestBlockStoragePolicy.DEFAULT_STORAGE_POLICY, null);
                return null;
            });
            ...
            RestartFramework.at("after_stop_all_datanodes").on(miniCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = miniCluster.getNamesystem();
            blockManager = miniCluster.getNamesystem().getBlockManager();
