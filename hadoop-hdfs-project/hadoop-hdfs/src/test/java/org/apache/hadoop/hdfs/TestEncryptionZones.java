...
public class TestEncryptionZones {
    ...
    public void assertZonePresent(String keyName, String path) throws IOException {
        ...
        boolean match = false;
        while (it.hasNext()) {
            EncryptionZoneInterface zone = it.next();
    ...
    public void testEncryptionZonesDictCp() throws Exception {
        ...
        if (attrs != null) {
            for (Map.EntryInterface<String, byte[]> entry : attrs.entrySet()) {
                String xattrName = entry.getKey();
                try {
                    fs.setXAttr(raw2File, xattrName, entry.getValue());
                    fail("Exception should be thrown while setting: " + xattrName + " on file:" + raw2File);
                } catch (RemoteException e) {
                    Assert.assertEquals(e.getClassName(), IllegalArgumentException.class.getCanonicalName());
                    Assert.assertTrue(e.getMessage().contains("does not belong to the key"));
                }
            }
        }
        assertEquals("File can be created on the root encryption zone " + "with correct length", len, fs.getFileStatus(zone1File).getLen());
        assertTrue("/zone1 dir is encrypted", fs.getFileStatus(zone1).isEncrypted());
        assertTrue("File is encrypted", fs.getFileStatus(zone1File).isEncrypted());
        assertTrue("/zone2 dir is encrypted", fs.getFileStatus(zone2).isEncrypted());
    ...
    public void testEZwithFullyQualifiedPath() throws Exception {
        ...
        EncryptionZoneInterface ezForZone1 = dfsAdmin.getEncryptionZoneForPath(zone1FQP);
        ...
        EncryptionZoneInterface ezForZone2 = dfsAdmin.getEncryptionZoneForPath(zone2FQP);
    ...
    public void testReadWrite() throws Exception {
        ...
        FileEncryptionInfoInterface feInfo1 = getFileEncryptionInfo(encFile1);
        FileEncryptionInfoInterface feInfo2 = getFileEncryptionInfo(encFile2);
    ...
    public void testVersionAndSuiteNegotiation() throws Exception {
        ...
        KeyProviderInterface provider = KeyProviderFactory.get(new URI(conf.getTrimmed(CommonConfigurationKeysPublic.HADOOP_SECURITY_KEY_PROVIDER_PATH)), conf);
        ...
        for (int i = 2; i <= 3; i++) {
            FileEncryptionInfoInterface feInfo = getFileEncryptionInfo(new Path(zone.toString() + "/success" + i));
    ...
    private static void testCipherSuiteNegotiation(DistributedFileSystem fs, Configuration conf) throws Exception {
        ...
        Mockito.when(mcp.complete(anyString(), anyString(), any(), anyLong())).thenReturn(true);
        try {
    ...
    public void testEncryptionZonesWithSnapshots() throws Exception {
        ...
        fsWrapper.mkdir(snapshottable, FsPermission.getDirDefault(), true);
        ...
        SnapshotDiffReportInterface report = fs.getSnapshotDiffReport(snapshottable, "snap1", "");
        Assert.assertEquals(0, report.getDiffList().size());
        report = fs.getSnapshotDiffReport(snapshottable, "snap1", "");
    ...
    public void testDelegationToken() throws Exception {
        ...
        KeyProviderInterface keyProvider = Mockito.mock(KeyProvider.class, withSettings().extraInterfaces(DelegationTokenExtension.class, CryptoExtension.class));
    ...
    public void testSnapshotsOnEncryptionZones() throws Exception {
        ...
        EncryptionZoneInterface ezSnap1 = dfsAdmin.getEncryptionZoneForPath(snap1Zone);
        ...
        for (EncryptionZoneInterface z : listZones) {
            System.out.println(z);
        }
        assertEquals("Did not expect additional encryption zones!", 1, listZones.size());
        EncryptionZoneInterface listZone = listZones.get(0);
    ...
    public void testSnapshotWithFile() throws Exception {
        ...
        FileEncryptionInfoInterface feInfo = getFileEncryptionInfo(snapshottedZoneFile);
    ...
    public void testGetEncryptionZoneOnANonExistentPaths() throws Exception {
        ...
        EncryptionZoneInterface ez = fs.getEZForPath(zoneFile);
    ...
    public void testProviderUriInCredentials() throws Exception {
        ...
        // namenode uri as key
        TextInterface lookUpKey = HdfsKMSUtil.getKeyProviderMapKey(cluster.getFileSystem().getUri());
    ...
    public void testKeyProviderFallBackBehavior() throws IOException {
        ...
        FsServerDefaultsInterface serverDefaultsWithKeyProviderNull = getTestServerDefaults(null);
        ...
        FsServerDefaultsInterface serverDefaultsWithDummyKeyProvider = getTestServerDefaults(dummyKeyProviderUri2);
    ...
    public void testDifferentKMSProviderOnUpgradedNamenode() throws Exception {
        ...
        clusterConf.setBoolean(DFS_CLIENT_IGNORE_NAMENODE_DEFAULT_KMS_URI, true);
        Assert.assertEquals("Expecting Key Provider for client config", "dummy://foo:bar@test_provider1", cluster.getFileSystem().getClient().getKeyProviderUri().toString());
        Assert.assertNotEquals("Key Provider for client and namenode is different", namenodeKeyProviderUri, cluster.getFileSystem().getClient().getKeyProviderUri().toString());
    }
    ...
    public void testDifferentKMSProviderOnUnUpgradedNamenode() throws Exception {
        ...
        FsServerDefaultsInterface spyServerDefaults = getTestServerDefaults(null);
    ...
    public void testListEncryptionZonesWithSnapshots() throws Exception {
        ...
        boolean match = false;
        while (it.hasNext()) {
            EncryptionZoneInterface ez = it.next();
