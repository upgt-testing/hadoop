 ...
 */
public class TestXAttrWithSnapshot_RestartInjected_Randomized_123 {
    ...
    public void testModifyReadsCurrentState() throws Exception {
        ...
        RestartFramework.at("after_set_xattrs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_modify_xattr1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_modify_xattr2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_remove_xattrs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testXattrWithSnapshotAndNNRestart() throws Exception {
        ...
        RestartFramework.at("after_set_xattr").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        report = hdfs.getSnapshotDiffReport(path, snapshotName, "");
        System.out.println(report);
        Assert.assertEquals(0, report.getDiffList().size());
    }
    ...
    public void testRemoveReadsCurrentState() throws Exception {
        // Init
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        RestartFramework.at("after_remove_xattr_name2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_set_xattrs").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        RestartFramework.at("after_remove_xattr_name1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.setXAttr(path, name1, value1);
        hdfs.setXAttr(path, name2, value2);
        // Verify that current path reflects xattrs, snapshot doesn't
        Map<String, byte[]> xattrs = hdfs.getXAttrs(path);
    ...
    public void testXAttrForSnapshotRootAfterChange() throws Exception {
        ...
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        RestartFramework.at("after_create_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_change_xattr").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSuccessiveSnapshotXAttrChanges() throws Exception {
        RestartFramework.at("after_delete_snapshot2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // First snapshot
        FileSystem.mkdirs(hdfs, path, FsPermission.createImmutable((short) 0700));
        hdfs.setXAttr(path, name1, value1);
        RestartFramework.at("after_create_snapshot3").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SnapshotTestHelper.createSnapshot(hdfs, path, snapshotName);
        ...
        Assert.assertEquals(1, xattrs.size());
        Assert.assertArrayEquals(value1, xattrs.get(name1));
        RestartFramework.at("after_create_snapshot2").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        xattrs = hdfs.getXAttrs(snapshotPath3);
        Assert.assertEquals(1, xattrs.size());
        RestartFramework.at("after_create_snapshot1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assert.assertArrayEquals(value1, xattrs.get(name1));
