...
public class TestSnapshotListing_RestartInjected_Randomized_123 {
    ...
    public void testListSnapshots() throws Exception {
        ...
        for (int sNum = 0; sNum < snapshotNum; sNum++) {
            ...
            RestartFramework.at("after_create_snapshot_" + sNum).on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stats = hdfs.listStatus(snapshotsPath);
        ...
        RestartFramework.at("after_delete_last_snapshot").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // list while deleting snapshots
        for (int sNum = snapshotNum - 1; sNum > 0; sNum--) {
            hdfs.deleteSnapshot(dir, "s_" + sNum);
            RestartFramework.at("after_delete_snapshot_" + sNum).on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stats = hdfs.listStatus(snapshotsPath);
