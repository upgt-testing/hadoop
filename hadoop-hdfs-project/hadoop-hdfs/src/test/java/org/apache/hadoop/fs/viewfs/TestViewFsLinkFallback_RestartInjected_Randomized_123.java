 ...
 */
public class TestViewFsLinkFallback_RestartInjected_Randomized_123 {
    ...
    public static void clusterSetupAtBeginning() throws IOException, URISyntaxException {
        ...
        viewFsDefaultClusterUri = new URI(FsConstants.VIEWFS_SCHEME, Constants.CONFIG_VIEWFS_DEFAULT_MOUNT_TABLE, "/", null, null);
    }
    ...
    public void testMkdirOfLinkParentWithFallbackLinkWithSameMountDirectoryTree() throws Exception {
        RestartFramework.at("after_mkdir_in_fallback").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        ConfigUtil.addLink(conf, "/user1/hive/warehouse/partition-0", new Path(targetTestRoot.toString()).toUri());
        Path dir1 = new Path(targetTestRoot, "fallbackDir/user1/hive/warehouse/partition-0");
        ...
        AbstractFileSystem vfs = AbstractFileSystem.get(viewFsDefaultClusterUri, conf);
    ...
    public void testMkdirOfRootWithFallbackLinkAndMountWithSameDirTree() throws Exception {
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        ConfigUtil.addLink(conf, "/user1", new Path(targetTestRoot.toString()).toUri());
    ...
    public void testMkdirOfNewDirWithOutMatchingToMountOrFallbackDirTree() throws Exception {
        ...
        assertFalse(fsTarget.exists(test));
        vfs.mkdir(p, null, true);
        RestartFramework.at("after_mkdir_new_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(fsTarget.exists(test));
    }
    ...
    public void testGetDelegationToken() throws IOException {
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        ConfigUtil.addLink(conf, "/user", new Path(targetTestRoot.toString(), "user").toUri());
        ConfigUtil.addLink(conf, "/data", new Path(targetTestRoot.toString(), "data").toUri());
        ConfigUtil.addLinkFallback(conf, targetTestRoot.toUri());
        FileContext fcView = FileContext.getFileContext(FsConstants.VIEWFS_URI, conf);
        List<Token<?>> tokens = fcView.getDelegationTokens(new Path("/"), "tester");
        RestartFramework.at("after_get_delegation_tokens").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirWithFallbackLinkWithMountPathMatchingDirExist() throws Exception {
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        ConfigUtil.addLink(conf, "/user1/hive", new Path(targetTestRoot.toString()).toUri());
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
        RestartFramework.at("after_mkdir_mount_path_matching").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        AbstractFileSystem vfs = AbstractFileSystem.get(viewFsDefaultClusterUri, conf);
    ...
    public void testMkdirOfDeepTreeWithFallbackLinkAndMountPathMatchingDirExist() throws Exception {
        ...
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
        AbstractFileSystem vfs = AbstractFileSystem.get(viewFsDefaultClusterUri, conf);
        RestartFramework.at("after_mkdir_deep_tree").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        //user1 does not exist in fallback
    ...
    public void testMkdirShouldCreateParentDirInFallbackWhenMountDirExist() throws Exception {
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        ConfigUtil.addLink(conf, "/user1/hive/test", new Path(targetTestRoot.toString()).toUri());
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
        AbstractFileSystem vfs = AbstractFileSystem.get(viewFsDefaultClusterUri, conf);
        ...
        Path test = Path.mergePaths(fallbackTarget, multipleLevelToInternalDir);
        RestartFramework.at("after_mkdir_create_parent").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(fsTarget.exists(test));
    ...
    public void testMkdirShouldFailWhenFallbackFSNotAvailable() throws Exception {
        Configuration conf = new Configuration();
        conf.setBoolean(Constants.CONFIG_VIEWFS_MOUNT_LINKS_AS_SYMLINKS, false);
        ConfigUtil.addLink(conf, "/user1/test", new Path(targetTestRoot.toString()).toUri());
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
        AbstractFileSystem vfs = AbstractFileSystem.get(viewFsDefaultClusterUri, conf);
        ...
        try {
            ...
            vfs.mkdir(nextLevelToInternalDir, FsPermission.getDirDefault(), false);
            RestartFramework.at("after_mkdir_namenode_recovery").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        assertTrue(fsTarget.exists(test));
    }
    ...
    public void testCreateFileOnInternalMountDirWithSameDirTreeExistInFallback() throws Exception {
        Configuration conf = new Configuration();
        RestartFramework.at("after_create_file_in_fallback").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ConfigUtil.addLink(conf, "/user1/hive/warehouse/partition-0", new Path(targetTestRoot.toString()).toUri());
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        ...
        vfs.create(vfsTestFile, EnumSet.of(CREATE), Options.CreateOpts.perms(FsPermission.getDefault())).close();
    ...
    public void testCreateNewFileWithOutMatchingToMountDirOrFallbackDirPath() throws Exception {
        ...
        Path testFileInFallback = Path.mergePaths(fallbackTarget, vfsTestFile);
        RestartFramework.at("after_create_new_file").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(fsTarget.exists(testFileInFallback));
        // user2 does not exist in fallback
        assertFalse(fsTarget.exists(testFileInFallback.getParent()));
        vfs.create(vfsTestFile, EnumSet.of(CREATE), Options.CreateOpts.perms(FsPermission.getDefault()), Options.CreateOpts.createParent()).close();
    ...
    public void testCreateFileOnRootWithFallbackEnabled() throws Exception {
        Configuration conf = new Configuration();
        RestartFramework.at("after_create_file_on_root").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ConfigUtil.addLink(conf, "/user1/hive/", new Path(targetTestRoot.toString()).toUri());
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
    ...
    public void testCreateFileWhereThePathIsSameAsItsMountLinkPath() throws Exception {
        ...
        vfs.create(vfsTestDir, EnumSet.of(CREATE), Options.CreateOpts.perms(FsPermission.getDefault())).close();
    }
    ...
    public void testListFiles() throws Exception {
        RestartFramework.at("before_list_files").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Configuration conf = new Configuration();
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ...
        RemoteIterator<LocatedFileStatus> iterator = fc.util().listFiles(new Path("/"), false);
        assertTrue(iterator.hasNext());
        assertEquals(fileInFallBackRoot.getName(), iterator.next().getPath().getName());
    }
    ...
    public void testRenameOnInternalDirWithFallback() throws Exception {
        ...
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ConfigUtil.addLink(conf, "/user1", new Path(targetTestRoot.toString() + "/user1").toUri());
        ConfigUtil.addLink(conf, "/NewHDFSUser/next", new Path(targetTestRoot.toString() + "/newUser1").toUri());
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
        ...
        final AbstractFileSystem fs = AbstractFileSystem.get(viewFsDefaultClusterUri, conf);
        ...
        RestartFramework.at("after_third_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path dst = new Path("/newFileOnRoot1");
        fs.create(src, EnumSet.of(CREATE), Options.CreateOpts.perms(FsPermission.getDefault())).close();
        ...
        verifyRename(fs, src, dst);
        RestartFramework.at("after_fourth_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        src = new Path("/newUser1/newFileOnRoot");
        dst = new Path("/newUser1/newFileOnRoot1");
        RestartFramework.at("after_fifth_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifyRename(fs, src, dst);
    ...
    public void testRenameWhenDstOnInternalDirWithFallback() throws Exception {
        ...
        Path fallbackTarget = new Path(targetTestRoot, "fallbackDir");
        fsTarget.mkdirs(fallbackTarget);
        ConfigUtil.addLink(conf, "/InternalDirDoesNotExistInFallback/test", new Path(targetTestRoot.toString() + "/user1").toUri());
        ConfigUtil.addLink(conf, "/NewHDFSUser/next/next1", new Path(targetTestRoot.toString() + "/newUser1").toUri());
        ConfigUtil.addLinkFallback(conf, fallbackTarget.toUri());
        try (DistributedFileSystem dfs = new DistributedFileSystem()) {
            dfs.initialize(fsDefault.getUri(), conf);
        ...
        LambdaTestUtils.intercept(IOException.class, () -> fs.rename(src, dst, Options.Rename.OVERWRITE));
        final Path src1 = new Path("/newFileOnRoot1");
        final Path dst1 = new Path("/NewHDFSUser/next/file");
        fs.create(src1, EnumSet.of(CREATE), Options.CreateOpts.perms(FsPermission.getDefault())).close();
        verifyRename(fs, src1, dst1);
        RestartFramework.at("after_rename_to_internal_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final Path src2 = new Path("/newFileOnRoot2");
        final Path dst2 = new Path("/InternalDirDoesNotExistInFallback/file");
        fs.create(src2, EnumSet.of(CREATE), Options.CreateOpts.perms(FsPermission.getDefault())).close();
        // If fallback does not have same structure as internal, rename will fail.
        LambdaTestUtils.intercept(FileNotFoundException.class, () -> fs.rename(src2, dst2, Options.Rename.OVERWRITE));
    }
