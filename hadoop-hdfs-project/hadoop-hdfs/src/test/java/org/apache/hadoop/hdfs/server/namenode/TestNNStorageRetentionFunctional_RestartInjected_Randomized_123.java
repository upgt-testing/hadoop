 ...
 * etc.
 */
public class TestNNStorageRetentionFunctional_RestartInjected_Randomized_123 {
    ...
    public void testPurgingWithNameEditsDirAfterFailure() throws Exception {
        ...
        try {
            ...
            doSaveNamespace(nn);
            RestartFramework.at("after_first_savenamespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            ...
            doSaveNamespace(nn);
            RestartFramework.at("after_second_savenamespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            ...
            RestartFramework.at("after_chmod_fail").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            doSaveNamespace(nn);
            RestartFramework.at("after_third_savenamespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            LOG.info("Restoring accessibility of first storage dir");
            assertEquals(0, FileUtil.chmod(cd0.getAbsolutePath(), "755"));
            RestartFramework.at("after_chmod_restore").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            ...
            doSaveNamespace(nn);
            RestartFramework.at("after_fourth_savenamespace").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
