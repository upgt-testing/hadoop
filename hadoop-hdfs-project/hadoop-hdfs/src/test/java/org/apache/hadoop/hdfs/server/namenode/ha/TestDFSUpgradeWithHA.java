...
public class TestDFSUpgradeWithHA {
    ...
    private static void assertCTimesEqual(MiniDockerDFSCluster cluster) {
    ...
    private static void checkClusterPreviousDirExistence(MiniDockerDFSCluster cluster, boolean shouldExist) {
        for (int i = 0; i < 2; i++) {
            checkNnPreviousDirExistence(cluster, i, shouldExist);
        }
    }

    private static void checkNnPreviousDirExistence(MiniDockerDFSCluster cluster, int index, boolean shouldExist) {
    ...
    private void runFinalizeCommand(MiniDockerDFSCluster cluster) throws IOException {
    ...
    public void testUpgradeWithJournalNodes() throws IOException, URISyntaxException {
        ...
        try {
            ...
            MiniDockerDFSCluster cluster = qjCluster.getDfsCluster();
            // No upgrade is in progress at the moment.
