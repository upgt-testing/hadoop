...
public class TestIncrementalBlockReports {
    ...
    private NameNodeInterface singletonNn;

    private DataNodeInterface singletonDn;

    // BPOS to use for block injection.
    private BPOfferServiceInterface bpos;

    // BPSA to use for block injection.
    private BPServiceActorInterface actor;
    ...
    public void startCluster() throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(DN_COUNT).build();
    ...
    private void injectBlockReceived() {
        ...
        DatanodeStorageInterface s = singletonDn.getFSDataset().getStorage(storageUuid);
    ...
    public void testReportBlockReceived() throws InterruptedException, IOException {
        try {
            DatanodeProtocolClientSideTranslatorPBInterface nnSpy = spyOnDnCallsToNn();
            injectBlockReceived();
