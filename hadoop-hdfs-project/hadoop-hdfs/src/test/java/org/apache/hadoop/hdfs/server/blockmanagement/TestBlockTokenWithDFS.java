...
public class TestBlockTokenWithDFS {
    ...
    public void testAppend() throws Exception {
       ...
       */
            TokenInterface<BlockTokenIdentifier> token = DFSTestUtil.getBlockToken(stm);
    ...
    public void testRead() throws Exception {
        ...
        try {
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).nameNodePort(ServerSocketUtil.getPort(18020, 100)).nameNodeHttpPort(ServerSocketUtil.getPort(19870, 100)).numDataNodes(numDataNodes).build();
            cluster.waitActive();
            assertEquals(numDataNodes, cluster.getDataNodes().size());
    ...
    protected void doTestRead(Configuration conf, MiniDockerDFSCluster cluster, boolean isStriped) throws Exception {
    ...
    protected void doTestRead(Configuration conf, MiniDockerDFSCluster cluster, boolean isStriped) throws Exception {
        final int numDataNodes = cluster.getDataNodes().size();
        final NameNodeInterface nn = cluster.getNameNode();
        //final NamenodeProtocolsInterface nnProto = nn.getRpcServer();
        ClientProtocol nnProto = cluster.getDFSClient().getNamenode();
        final BlockManagerInterface bm = nn.getNamesystem().getBlockManager();
        final BlockTokenSecretManagerInterface sm = bm.getBlockTokenSecretManager();
        // set a short token lifetime (1 second) initially
        SecurityTestUtil.setBlockTokenLifetime(sm, 1000L);
