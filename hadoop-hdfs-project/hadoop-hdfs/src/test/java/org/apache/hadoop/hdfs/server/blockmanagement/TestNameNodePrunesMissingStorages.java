...
public class TestNameNodePrunesMissingStorages {
    ...
    private static void runTest(final String testCaseName, final boolean createFiles, final int numInitialStorages, final int expectedStoragesAfterTest) throws IOException {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).storagesPerDatanode(numInitialStorages).build();
            cluster.waitActive();
            final DataNodeInterface dn0 = cluster.getDataNodes().get(0);
            // Ensure NN knows about the storage.
            final DatanodeIDInterface dnId = dn0.getDatanodeId();
            final DatanodeDescriptorInterface dnDescriptor = cluster.getNamesystem().getBlockManager().getDatanodeManager().getDatanode(dnId);
            assertThat(dnDescriptor.getStorageInfos().length, is(numInitialStorages));
            final String bpid = cluster.getNamesystem().getBlockPoolId();
            final DatanodeRegistrationInterface dnReg = dn0.getDNRegistrationForBP(bpid);
    ...
    public void testRemovingStorageDoesNotProduceZombies() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storagesPerDatanode(NUM_STORAGES_PER_DN).build();
        try {
            cluster.waitActive();
            ...
            final DataNodeInterface datanodeToRemoveStorageFrom;
            ...
            while (true) {
                ...
                DataNodeInterface dn = cluster.getDataNodes().get(datanodeToRemoveStorageFromIdx);
            ...
            StorageLocationInterface volumeLocationToRemove = null;
            ...
            GenericTestUtils.waitFor(new Supplier<Boolean>() {
                ...
                @Override
                public Boolean get() {
                    final DatanodeDescriptorInterface dnDescriptor = cluster.getNamesystem().getBlockManager().getDatanodeManager().getDatanode(datanodeToRemoveStorageFrom.getDatanodeUuid());
                    assertNotNull(dnDescriptor);
                    DatanodeStorageInfo[] infos = dnDescriptor.getStorageInfos();
                    for (DatanodeStorageInfoInterface info : infos) {
    ...
    public void testRenamingStorageIds() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).storagesPerDatanode(1).build();
    ...
    public void testNameNodePrunesUnreportedStorages() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).storagesPerDatanode(2).build();
        try {
            cluster.waitActive();
            ...
            DatanodeDescriptorInterface dnDescriptor = bm.getDatanodeManager().getDatanode(cluster.getDataNodes().get(0).getDatanodeUuid());
            ...
            DatanodeStorageInfoInterface failedStorageInfo = null;
            for (DatanodeStorageInfoInterface dnStorageInfo : dnDescriptor.getStorageInfos()) {
