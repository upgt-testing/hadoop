...
public class TestReplicationPolicy extends BaseReplicationPolicyTest {
    ...
    private void updateHeartbeatForExtraStorage(long capacity, long dfsUsed, long remaining, long blockPoolUsed) {
        DatanodeDescriptorInterface dn = dataNodes[5];
    ...
    public void testChooseTarget5() throws Exception {
        DatanodeDescriptorInterface writerDesc = DFSTestUtil.getDatanodeDescriptor("7.7.7.7", "/d2/r4");
        DatanodeStorageInfo[] targets;
    ...
    public void testChooseTarget6() throws Exception {
        DatanodeStorageInfoInterface storage = DFSTestUtil.createDatanodeStorageInfo("DS-xxxx", "7.7.7.7", "/d2/r3", "host7");
        DatanodeDescriptorInterface newDn = storage.getDatanodeDescriptor();
        Set<Node> excludedNodes;
        List<DatanodeStorageInfo> chosenNodes = new ArrayList<>();
        excludedNodes = new HashSet<>();
    ...
    public void testChooseTargetWithMoreThanHalfStaleNodes() throws Exception {
        ...
        MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(conf).racks(racks).hosts(hosts).numDataNodes(hosts.length).build();
        ...
        try {
            ...
            for (int i = 0; i < 2; i++) {
                DataNodeInterface dn = miniCluster.getDataNodes().get(i);
                DataNodeTestUtils.setHeartbeatsDisabledForTests(dn, true);
                DatanodeDescriptorInterface dnDes = miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getDatanode(dn.getDatanodeId());
            ...
            // Call chooseTarget
            DatanodeDescriptorInterface staleNodeInfo = miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getDatanode(miniCluster.getDataNodes().get(0).getDatanodeId());
            BlockPlacementPolicyInterface replicator = miniCluster.getNameNode().getNamesystem().getBlockManager().getBlockPlacementPolicy();
            ...
            for (int i = 0; i < 4; i++) {
                DataNodeInterface dn = miniCluster.getDataNodes().get(i);
                DataNodeTestUtils.setHeartbeatsDisabledForTests(dn, true);
                DatanodeDescriptorInterface dnDesc = miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getDatanode(dn.getDatanodeId());
    ...
    public void testReplicationWithPriority() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).format(true).build();
        try {
            cluster.waitActive();
            ...
            for (int i = 0; i < 100; i++) {
                ...
                neededReconstruction.add(genBlockInfo(ThreadLocalRandom.current().nextLong(), true), 2, 0, 0, 3);
            }
            // Lets wait for the replication interval, to start process normal
            // priority blocks
            Thread.sleep(DFS_NAMENODE_REPLICATION_INTERVAL);
            // Adding the block directly to high priority list
            neededReconstruction.add(genBlockInfo(ThreadLocalRandom.current().nextLong(), true), 1, 0, 0, 3);
    ...
    public void testChooseReplicaToDelete() throws Exception {
        ...
        DatanodeStorageInfoInterface chosen = ((BlockPlacementPolicyDefault) replicator).chooseReplicaToDelete(first, second, excessTypes, rackMap);
    ...
    public void testStripedChooseReplicaToDelete() throws Exception {
        ...
        List<DatanodeStorageInfo> first = new ArrayList<>();
        List<DatanodeStorageInfo> second = new ArrayList<>();
        BlockPlacementPolicyInterface policy = getStriptedPolicy();
        ...
        List<StorageType> excessTypes = new ArrayList<>();
        excessTypes.add(StorageType.DEFAULT);
        DatanodeStorageInfoInterface chosen = ((BlockPlacementPolicyDefault) policy).chooseReplicaToDelete(first, second, excessTypes, rackMap);
    ...
    private long calculateRemaining(DatanodeDescriptor dataNode) {
        long sum = 0;
        for (DatanodeStorageInfoInterface storageInfo : dataNode.getStorageInfos()) {
    ...
    public void testChooseReplicasToDelete() throws Exception {
        ...
        BlockStoragePolicySuiteInterface POLICY_SUITE = BlockStoragePolicySuite.createDefaultSuite();
        BlockStoragePolicyInterface storagePolicy = POLICY_SUITE.getDefaultPolicy();
        DatanodeStorageInfoInterface excessSSD = DFSTestUtil.createDatanodeStorageInfo("Storage-excess-SSD-ID", "localhost", storages[0].getDatanodeDescriptor().getNetworkLocation(), "foo.com", StorageType.SSD, null);
        ...
        DatanodeDescriptorInterface delHintNode = storages[0].getDatanodeDescriptor();
        ...
        DatanodeStorageInfoInterface excessStorage = DFSTestUtil.createDatanodeStorageInfo("Storage-excess-ID", "localhost", delHintNode.getNetworkLocation(), "foo.com", StorageType.ARCHIVE, null);
    ...
    public void testAddStoredBlockDoesNotCauseSkippedReplication() throws IOException {
        FSNamesystemInterface mockNS = mock(FSNamesystem.class);
        ...
        final INodeFileInterface file = TestINodeFile.createINodeFile(1000L);
    ...
    public void testConvertLastBlockToUnderConstructionDoesNotCauseSkippedReplication() throws IOException {
        ...
        ContentSummaryInterface cs = mock(ContentSummary.class);
        ...
        DatanodeStorageInfoInterface storage = mock(DatanodeStorageInfo.class);
        DatanodeDescriptorInterface dn = mock(DatanodeDescriptor.class);
    ...
    public void testChooseFromFavoredNodesWhenPreferLocalSetToFalse() {
        ...
        try {
            ...
            targets = chooseTarget(2, dataNodes[3], null, favouredNodes);
            assertEquals(targets.length, 2);
            for (int i = 0; i < targets.length; i++) {
                assertTrue("Target should be a part of Expected Targets", expectedTargets.contains(targets[i].getDatanodeDescriptor()));
            }
    ...
    public void testMaxLoad() {
        ...
        DatanodeDescriptorInterface node = mock(DatanodeDescriptor.class);
        ...
        BlockPlacementPolicyInterface bpp = ReflectionUtils.newInstance(replicatorClass, conf);
        ...
        conf.setBoolean(DFS_NAMENODE_REDUNDANCY_CONSIDERLOADBYSTORAGETYPE_KEY, true);
        bppd.initialize(conf, statistics, null, null);
        Map<StorageType, StorageTypeStats> storageStats = new HashMap<>();
        StorageTypeStats diskStorageTypeStats = new StorageTypeStats(StorageType.DISK);
        ...
        StorageTypeStats archiveStorageTypeStats = new StorageTypeStats(StorageType.ARCHIVE);
    ...
    public void testChosenFailureForStorageType() {
        ...
        final Logger logger = Logger.getRootLogger();
        logger.addAppender(appender);
        DatanodeStorageInfo[] targets = replicator.chooseTarget(filename, 1, dataNodes[0], new ArrayList<DatanodeStorageInfo>(), false, null, BLOCK_SIZE, TestBlockStoragePolicy.POLICY_SUITE.getPolicy(HdfsConstants.COLD_STORAGE_POLICY_ID), null);
