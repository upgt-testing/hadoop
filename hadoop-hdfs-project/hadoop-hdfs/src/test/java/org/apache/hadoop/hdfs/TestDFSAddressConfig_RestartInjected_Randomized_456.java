...
public class TestDFSAddressConfig_RestartInjected_Randomized_456 {
     ...
     *------------------------------------------------------------------------*/
        ...
        RestartFramework.at("after_second_datanode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        conf.unset(DFS_DATANODE_ADDRESS_KEY);
        RestartFramework.at("after_first_datanode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
