...
public class TestReconstructStripedBlocks {
    ...
    private void doTestMissingStripedBlock(int numOfMissed, int numOfBusy) throws Exception {
        ...
        initConf(conf);
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(groupSize + 1).build();
        try {
            cluster.waitActive();
            ...
            final INodeFileInterface fileNode = cluster.getNamesystem().getFSDirectory().getINode4Write(filePath.toString()).asFile();
            ...
            for (BlockInfo blk : blocks) {
                ...
                final BlockInfoStripedInterface sb = (BlockInfoStriped) blk;
            ...
            int i = 0;
            for (; i < numOfBusy; i++) {
                DatanodeDescriptorInterface busyNode = storageInfos[i].getDatanodeDescriptor();
            ...
            for (; i < numOfBusy + numOfMissed; i++) {
                DatanodeDescriptorInterface missedNode = storageInfos[i].getDatanodeDescriptor();
            ...
            DataNodeInterface lastDn = cluster.getDataNodes().get(groupSize);
            DatanodeDescriptorInterface last = bm.getDatanodeManager().getDatanode(lastDn.getDatanodeId());
    ...
    public void test2RecoveryTasksForSameBlockGroup() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(groupSize + 2).build();
        try {
            cluster.waitActive();
            ...
            DatanodeInfoInterface dn0 = lbs[0].getLocations()[0];
            ...
            DatanodeInfoInterface dn1 = lbs[1].getLocations()[0];
    ...
    private static int getNumberOfBlocksToBeErasureCoded(MiniDockerDFSCluster cluster) throws Exception {
        ...
        for (DataNodeInterface dn : cluster.getDataNodes()) {
            DatanodeDescriptorInterface dd = dm.getDatanode(dn.getDatanodeId());
    ...
    public void testReconstructionWork() throws Exception {
        ...
        MiniDockerDFSCluster dfsCluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(totalDataNodes).build();
        try {
