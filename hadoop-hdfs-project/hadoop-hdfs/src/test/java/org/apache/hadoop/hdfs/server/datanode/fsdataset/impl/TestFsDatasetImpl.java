...
public class TestFsDatasetImpl {
    ...
    public void setUp() throws IOException {
        ...
        this.conf.set(DFSConfigKeys.DFS_DATANODE_REPLICA_CACHE_ROOT_DIR_KEY, replicaCacheRootDir);
    ...
    public void testAddVolumeWithSameDiskTiering() throws IOException {
        ...
        conf.setDouble(DFSConfigKeys.DFS_DATANODE_RESERVE_FOR_ARCHIVE_DEFAULT_PERCENTAGE, 0.4);
        when(datanode.getConf()).thenReturn(conf);
        final DNConf dnConf = new DNConf(datanode);
        when(datanode.getDnConf()).thenReturn(dnConf);
        final BlockScanner disabledBlockScanner = new BlockScanner(datanode);
        ...
        StorageLocation archive = createStorageWithStorageType("archive1", StorageType.ARCHIVE, conf, storage, datanode);
        ...
        double archiveRatio = dataset.getMountVolumeMap().getCapacityRatioByMountAndStorageType(mount, StorageType.ARCHIVE);
        double diskRatio = dataset.getMountVolumeMap().getCapacityRatioByMountAndStorageType(mount, StorageType.DISK);
        assertEquals(0.4, archiveRatio, 0);
        assertEquals(0.6, diskRatio, 0);
        // Add second ARCHIVAL volume should fail fsDataSetImpl.
        try {
            dataset.addVolume(createStorageWithStorageType("archive2", StorageType.ARCHIVE, conf, storage, datanode), nsInfos);
            fail("Should throw exception for" + " same storage type already exists on same mount.");
        } catch (IOException e) {
            assertTrue(e.getMessage().startsWith("Storage type ARCHIVE already exists on same mount:"));
        }
    }
    ...
    public void testAddVolumeWithCustomizedCapacityRatio() throws IOException {
        ...
        conf.set(DFSConfigKeys.DFS_DATANODE_SAME_DISK_TIERING_CAPACITY_RATIO_PERCENTAGE, configStr);
        when(datanode.getConf()).thenReturn(conf);
        final DNConf dnConf = new DNConf(datanode);
        when(datanode.getDnConf()).thenReturn(dnConf);
        final BlockScanner disabledBlockScanner = new BlockScanner(datanode);
        ...
        dataset = createStorageWithCapacityRatioConfig(configStr, archivedir, diskdir);
        ...
        archiveRatio = dataset.getMountVolumeMap().getCapacityRatioByMountAndStorageType(mount, StorageType.ARCHIVE);
        diskRatio = dataset.getMountVolumeMap().getCapacityRatioByMountAndStorageType(mount, StorageType.DISK);
        assertEquals(0.3, archiveRatio, 0);
        ...
        try {
            createStorageWithCapacityRatioConfig(configStr, archivedir, diskdir);
            fail("Should fail add volume as capacity ratio sum is > 1");
        } catch (IOException e) {
            assertTrue(e.getMessage().contains("Not enough capacity ratio left on mount"));
        }
    }
    ...
    private FsDatasetImpl createStorageWithCapacityRatioConfig(String configStr, String archivedir, String diskdir) throws IOException {
        ...
        StorageLocation archive = createStorageWithStorageType(archivedir, StorageType.ARCHIVE, conf, storage, datanode);
        StorageLocation disk = createStorageWithStorageType(diskdir, StorageType.DISK, conf, storage, datanode);
        dataset.addVolume(archive, nsInfos);
    ...
    public void testAddVolumeWithSameStorageUuid() throws IOException {
        ...
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(config).numDataNodes(1).build();
    ...
    public void testRemoveOneVolume() throws IOException {
        ...
        verify(datanode, times(50)).notifyNamenodeDeletedBlock(any(), any());
        try {
        ...
        assertEquals("The replica infos on this volume has been removed from the " + "volumeMap.", numBlocks / NUM_INIT_VOLUMES, totalNumReplicas);
    }
    ...
    public void testRemoveTwoVolumes() throws IOException {
        ...
        verify(datanode, times(100)).notifyNamenodeDeletedBlock(any(), any());
        for (FsVolumeImpl volume : volumes) {
            try {
                dataset.asyncDiskService.execute(volume, new Runnable() {
        ...
        assertEquals("The replica infos on this volume has been removed from the " + "volumeMap.", 0, totalNumReplicas);
    }
    ...
    public void testConcurrentWriteAndDeleteBlock() throws Exception {
        ...
        GenericTestUtils.waitFor(() -> dataset.asyncDiskService.countPendingDeletions() == 0, 100, 10000);
        for (String bpid : dataset.volumeMap.getBlockPoolList()) {
   ...
   */
    ...
    private void testMoveBlockFailure(Configuration config) {
        MiniDFSClusterInJVM cluster = null;
        try {
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).storagesPerDatanode(2).build();
            FileSystem fs = cluster.getFileSystem();
    ...
    public void testMoveBlockSuccess() {
        MiniDFSClusterInJVM cluster = null;
        try {
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).storageTypes(new StorageType[] { StorageType.DISK, StorageType.DISK }).storagesPerDatanode(2).build();
            FileSystem fs = cluster.getFileSystem();
            DataNodeJVMInterface dataNode = cluster.getDataNodes().get(0);
            Path filePath = new Path(name.getMethodName());
    ...
    public void testDnRestartWithHardLink() throws Exception {
        MiniDFSClusterInJVM cluster = null;
        boolean isReplicaDeletionEnabled = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_DUPLICATE_REPLICA_DELETION, DFSConfigKeys.DFS_DATANODE_DUPLICATE_REPLICA_DELETION_DEFAULT);
        try {
            conf.setBoolean(DFSConfigKeys.DFS_DATANODE_ALLOW_SAME_DISK_TIERING, true);
            conf.setDouble(DFSConfigKeys.DFS_DATANODE_RESERVE_FOR_ARCHIVE_DEFAULT_PERCENTAGE, 0.5);
            ...
            fsDataSetImpl.finalizeNewReplica(createNewReplicaObjWithLink(block, fsDataSetImpl), block);
            ...
            scanner.start();
            scanner.run();
            GenericTestUtils.waitFor(() -> !Files.exists(Paths.get(oldReplicaInfo.getBlockURI())), 100, 10000, "Old replica is not deleted by DirScanner even after " + "10s of waiting has elapsed");
            assertTrue(Files.exists(Paths.get(newReplicaInfo.getBlockURI())));
            validateFileLen(fs, fileLen, filePath);
            // Additional tests to ensure latest replica gets deleted after file
            // deletion.
            fs.delete(filePath, false);
            GenericTestUtils.waitFor(() -> !Files.exists(Paths.get(newReplicaInfo.getBlockURI())), 100, 10000);
        } finally {
            conf.setBoolean(DFSConfigKeys.DFS_DATANODE_DUPLICATE_REPLICA_DELETION, isReplicaDeletionEnabled);
            if (cluster != null && cluster.isClusterUp()) {
    ...
    public void testShouldConsiderSameMountVolume() throws IOException {
        FsVolumeImpl volume = new FsVolumeImplBuilder().setConf(conf).setDataset(dataset).setStorageID("storage-id").setStorageDirectory(new StorageDirectory(StorageLocation.parse(BASE_DIR))).build();
        assertFalse(dataset.shouldConsiderSameMountVolume(volume, StorageType.ARCHIVE, null));
        conf.setBoolean(DFSConfigKeys.DFS_DATANODE_ALLOW_SAME_DISK_TIERING, true);
        conf.setDouble(DFSConfigKeys.DFS_DATANODE_RESERVE_FOR_ARCHIVE_DEFAULT_PERCENTAGE, 0.5);
        volume = new FsVolumeImplBuilder().setConf(conf).setDataset(dataset).setStorageID("storage-id").setStorageDirectory(new StorageDirectory(StorageLocation.parse(BASE_DIR))).build();
    ...
    public void testDataDirWithPercent() throws IOException {
        ...
        dataDir.mkdirs();
        FsVolumeImpl volumeFixed = new FsVolumeImplBuilder().setConf(new HdfsConfiguration()).setDataset(dataset).setStorageID("storage-id").setStorageDirectory(new StorageDirectory(StorageLocation.parse(dataDir.getPath()))).build();
    }
    ...
    public void testReplicaCacheFileToOtherPlace() throws IOException {
        ...
        for (FsVolumeSpi vol : dataset.getFsVolumeReferences()) {
            ...
            File cacheFile = new File(replicaCacheRootDir + Path.SEPARATOR + bpSlice.getDirectory().getCanonicalPath() + Path.SEPARATOR + DataStorage.STORAGE_DIR_CURRENT + Path.SEPARATOR + "replicas");
    ...
    public void testGetMetadataLengthOfFinalizedReplica() throws IOException {
        ...
        long blockID = 1;
        long genStamp = 2;
        File metaFile = new File(blockDir, Block.BLOCK_FILE_PREFIX + blockID + "_" + genStamp + Block.METADATA_EXTENSION);
        ...
        ReplicaInfo replica = new FinalizedReplica(blockID, 2, genStamp, fsv1, blockDir);
    ...
    public void testNotifyNamenodeMissingOrNewBlock() throws Exception {
        ...
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(c).numDataNodes(1).build();
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
        try {
            cluster.waitActive();
            DFSTestUtil.createFile(cluster.getFileSystem(), new Path("/f1"), blockSize, (short) 1, 0);
    ...
    public void testTransferAndNativeCopyMetrics() throws IOException {
        Configuration config = new HdfsConfiguration();
        config.setInt(DFSConfigKeys.DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, 100);
        config.set(DFSConfigKeys.DFS_METRICS_PERCENTILES_INTERVALS_KEY, "60,300,1500");
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(config).numDataNodes(1).storageTypes(new StorageType[] { StorageType.DISK, StorageType.DISK }).storagesPerDatanode(2).build()) {
            FileSystem fs = cluster.getFileSystem();
            DataNodeJVMInterface dataNode = cluster.getDataNodes().get(0);
            ...
            for (FsVolumeSpi fsVolumeReference : fsDataSetImpl.getFsVolumeReferences()) {
                if (!fsVolumeReference.getStorageID().equals(newReplicaInfo.getStorageUuid())) {
     ...
     */
    @Test
    public void testAysncDiskServiceDeleteReplica() throws IOException, InterruptedException, TimeoutException, MalformedObjectNameException, ReflectionException, AttributeNotFoundException, InstanceNotFoundException, MBeanException {
        HdfsConfiguration config = new HdfsConfiguration();
        ...
        try {
            ...
            ObjectName mxbeanName = new ObjectName("Hadoop:service=DataNode,name=FSDatasetState-" + dn.getDatanodeUuid());
            long pendingAsyncDeletions = (long) mbs.getAttribute(mxbeanName, "PendingAsyncDeletions");
            ...
            pendingAsyncDeletions = (long) mbs.getAttribute(mxbeanName, "PendingAsyncDeletions");
            assertEquals(0, pendingAsyncDeletions);
            // Sleep for two heartbeat times.
            Thread.sleep(config.getTimeDuration(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY, DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_DEFAULT, TimeUnit.SECONDS, TimeUnit.MILLISECONDS) * 2);
    ...
    public void tesInvalidateMissingBlock() throws Exception {
        ...
        try {
            cluster.waitActive();
            DFSTestUtil.createFile(cluster.getFileSystem(), new Path("/a"), blockSize, (short) 1, 0);
            String bpid = cluster.getNameNode().getNamesystem().getBlockPoolId();
            DataNodeJVMInterface dn = cluster.getDataNodes().get(0);
            ...
            BlockManagerJVMInterface blockManager = cluster.getNameNode().getNamesystem().getBlockManager();
            GenericTestUtils.waitFor(() -> blockManager.getLowRedundancyBlocksCount() == 1, 100, 5000);
            ...
            fsdataset.checkAndUpdate(bpid, info);
            GenericTestUtils.waitFor(() -> blockManager.getLowRedundancyBlocksCount() == 0, 100, 5000);
        } finally {
            cluster.shutdown();
        }
    }
