   ...
   */
    ...
    public void testNotifyNamenodeMissingOrNewBlock() throws Exception {
        ...
        try {
            ...
            List<ReplicaInfoJVMInterface> replicaInfos = fsdataset.getFinalizedBlocks(bpid);
   ...
   */
    ...
    public void testCheckFilesWhenInvalidateMissingBlock() throws Exception {
        ...
        try {
            ...
            try {
                Future<?> blockReaderFuture = executorService.submit(() -> {
                    try {
                        ...
                        BlockReader blockReader = BlockReaderTestUtil.getBlockReader(cluster.getFileSystem(), blk, 0, 512);
                        blockReader.close();
                    } catch (IOException e) {
                        // Ignore exception.
                    }
            ...
            String expectedMsg = String.format("opReadBlock %s received exception " + "java.io.FileNotFoundException: %s (No such file or directory)", blk.getBlock(), tmpReplicaInfo.getMetadataURI().getPath());
            assertTrue("Expected log message not found in DN log.", logCapturer.getOutput().contains(expectedMsg));
        } finally {
            cluster.shutdown();
