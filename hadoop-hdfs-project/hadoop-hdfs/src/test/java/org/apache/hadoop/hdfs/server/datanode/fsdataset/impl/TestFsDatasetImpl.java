...
public class TestFsDatasetImpl {
    ...
    public void testAddVolumeWithSameStorageUuid() throws IOException {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(1).build();
        try {
            cluster.waitActive();
    ...
    public void testCleanShutdownOfVolume() throws Exception {
        ...
        try {
            ...
            cluster = new MiniDockerDFSCluster.Builder(config, GenericTestUtils.getRandomizedTestDir()).numDataNodes(1).build();
            cluster.waitActive();
    ...
    public void testReportBadBlocks() throws Exception {
        ...
        try {
            Configuration config = new HdfsConfiguration();
            cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(1).build();
            cluster.waitActive();
    ...
    public void testMoveBlockFailure() {
        MiniDockerDFSCluster cluster = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).storageTypes(new StorageType[] { StorageType.DISK, StorageType.DISK }).storagesPerDatanode(2).build();
            FileSystem fs = cluster.getFileSystem();
            DataNodeInterface dataNode = cluster.getDataNodes().get(0);
            Path filePath = new Path("testData");
    ...
    public void testTransferAndNativeCopyMetrics() throws IOException {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(1).storageTypes(new StorageType[] { StorageType.DISK, StorageType.DISK }).storagesPerDatanode(2).build()) {
            FileSystem fs = cluster.getFileSystem();
            DataNodeInterface dataNode = cluster.getDataNodes().get(0);
