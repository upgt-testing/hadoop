...
public class TestHDFSContractUnbuffer_RestartInjected extends AbstractContractUnbufferTest {
  ...
  public void setup() throws Exception {
    ...
    RestartFramework.at("after_file_creation_for_unbuffer")
  ...
  public void testUnbufferAfterRead() throws IOException {
    ...
    try (FSDataInputStream stream = getFileSystem().open(file)) {
      RestartFramework.at("after_open_before_read")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("before_unbuffer_after_read")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testUnbufferBeforeRead() throws IOException {
    ...
    try (FSDataInputStream stream = getFileSystem().open(file)) {
      RestartFramework.at("after_open_before_unbuffer")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_unbuffer_before_read")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testUnbufferEmptyFile() throws IOException {
    ...
    RestartFramework.at("after_empty_file_creation")
        .on(HDFSContract.getCluster())
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    try (FSDataInputStream stream = getFileSystem().open(emptyFile)) {
      RestartFramework.at("before_unbuffer_empty_file")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testUnbufferOnClosedFile() throws IOException {
    ...
    try {
      ...
      RestartFramework.at("after_open_before_validate")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("before_unbuffer_closed_file")
        .on(HDFSContract.getCluster())
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testMultipleUnbuffers() throws IOException {
    ...
    try (FSDataInputStream stream = getFileSystem().open(file)) {
      RestartFramework.at("after_open_before_first_unbuffer")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_unbuffers_before_read")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testUnbufferMultipleReads() throws IOException {
    describe("unbuffer a file multiple times");
    try (FSDataInputStream stream = getFileSystem().open(file)) {
      RestartFramework.at("after_open_for_multiple_reads")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("mid_multiple_reads")
          .on(HDFSContract.getCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
