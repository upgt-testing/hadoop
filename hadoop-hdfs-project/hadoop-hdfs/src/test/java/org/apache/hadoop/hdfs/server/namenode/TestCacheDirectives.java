...
public class TestCacheDirectives {
    ...
    static private MiniDockerDFSCluster cluster;

    static private DistributedFileSystem dfs;

    static private NamenodeProtocols proto;

    static private NameNodeInterface namenode;
    ...
    public void setup() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).build();
        cluster.waitActive();
        dfs = getDFS();
    ...
    public void testBasicPoolOperations() throws Exception {
        ...
        CachePoolInfoInterface info = new CachePoolInfo(poolName).setOwnerName("bob").setGroupName("bobgroup").setMode(new FsPermission((short) 0755)).setLimit(150l);
        // Add a pool
    ...
    public void testCreateAndModifyPools() throws Exception {
        ...
        CachePoolInfoInterface info = iter.next().getInfo();
    ...
    public void testAddRemoveDirectives() throws Exception {
        ...
        CacheDirectiveInfoInterface beta = new CacheDirectiveInfo.Builder().setPath(new Path("/beta")).setPool("pool2").build();
        CacheDirectiveInfoInterface delta = new CacheDirectiveInfo.Builder().setPath(new Path("/delta")).setPool("pool1").build();
        ...
        CacheDirectiveInfoInterface modified = iter.next().getInfo();
        ...
        CacheDirectiveInfoInterface directive = new CacheDirectiveInfo.Builder().setPath(new Path(".")).setPool("pool1").build();
    ...
    public void testCacheManagerRestart() throws Exception {
        ...
        try {
            ...
            dfs.addCachePool(new CachePoolInfo(pool).setGroupName(groupName).setMode(mode).setLimit(limit).setMaxRelativeExpiryMs(maxExpiry));
            RemoteIterator<CachePoolEntry> pit = dfs.listCachePools();
            assertTrue("No cache pools found", pit.hasNext());
            CachePoolInfoInterface info = pit.next().getInfo();
            ...
            for (int i = 0; i < numEntries; i++) {
                ...
                CacheDirectiveInfoInterface cd = dit.next().getInfo();
    ...
    private static void waitForCachedBlocks(NameNode nn, final int expectedCachedBlocks, final int expectedCachedReplicas, final String logString) throws Exception {
        final FSNamesystemInterface namesystem = nn.getNamesystem();
        final CacheManagerInterface cacheManager = namesystem.getCacheManager();
    ...
    private static void waitForCacheDirectiveStats(final DistributedFileSystem dfs, final long targetBytesNeeded, final long targetBytesCached, final long targetFilesNeeded, final long targetFilesCached, final CacheDirectiveInfo filter, final String infoString) throws Exception {
        ...
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            ...
            public Boolean get() {
                ...
                CacheDirectiveEntryInterface entry = null;
                try {
                ...
                CacheDirectiveStatsInterface stats = entry.getStats();
    ...
    private static void waitForCachePoolStats(final DistributedFileSystem dfs, final long targetBytesNeeded, final long targetBytesCached, final long targetFilesNeeded, final long targetFilesCached, final CachePoolInfo pool, final String infoString) throws Exception {
        ...
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            ...
            public Boolean get() {
                ...
                while (true) {
                    CachePoolEntryInterface entry = null;
                    try {
                        if (!iter.hasNext()) {
                            break;
                        }
                    ...
                    CachePoolStatsInterface stats = entry.getStats();
                    if ((targetBytesNeeded == stats.getBytesNeeded()) && (targetBytesCached == stats.getBytesCached()) && (targetFilesNeeded == stats.getFilesNeeded()) && (targetFilesCached == stats.getFilesCached())) {
                        return true;
                    } else {
                        LOG.info(infoString + ": " + "filesNeeded: " + stats.getFilesNeeded() + "/" + targetFilesNeeded + ", filesCached: " + stats.getFilesCached() + "/" + targetFilesCached + ", bytesNeeded: " + stats.getBytesNeeded() + "/" + targetBytesNeeded + ", bytesCached: " + stats.getBytesCached() + "/" + targetBytesCached);
    ...
    public void testWaitForCachedReplicas() throws Exception {
        ...
        DataNodeInterface dn0 = cluster.getDataNodes().get(0);
        ...
        int expected = 0;
        for (int i = 0; i < numFiles; i++) {
            CacheDirectiveInfoInterface directive = new CacheDirectiveInfo.Builder().setPath(new Path(paths.get(i))).setPool(pool).build();
        ...
        long totalUsed = 0;
        for (DatanodeInfoInterface dn : live) {
        ...
        for (int i = 0; i < numFiles; i++) {
            CacheDirectiveEntryInterface entry = entries.next();
    ...
    public void testListCachePoolPermissions() throws Exception {
        ...
        CachePoolInfoInterface info = it.next().getInfo();
    ...
    public void testExpiry() throws Exception {
        ...
        CacheDirectiveEntryInterface ent = it.next();
        assertFalse(it.hasNext());
    ...
    public void testLimit() throws Exception {
        ...
        final CachePoolInfoInterface poolInfo = new CachePoolInfo(destiny).setLimit(2 * BLOCK_SIZE - 1);
        dfs.addCachePool(poolInfo);
        final CacheDirectiveInfoInterface info1 = new CacheDirectiveInfo.Builder().setPool(destiny).setPath(path1).build();
        try {
        ...
        CachePoolStatsInterface stats = it.next().getStats();
        ...
        CachePoolInfoInterface inadequate = new CachePoolInfo("poolofinadequacy").setLimit(BLOCK_SIZE);
    ...
    public void testMaxRelativeExpiry() throws Exception {
        ...
        CachePoolInfoInterface listPool = poolIt.next().getInfo();
        ...
        CacheDirectiveInfoInterface defaultExpiry = new CacheDirectiveInfo.Builder().setPath(new Path("/blah")).setPool(coolPool.getPoolName()).build();
        dfs.addCacheDirective(defaultExpiry);
        RemoteIterator<CacheDirectiveEntry> dirIt = dfs.listCacheDirectives(defaultExpiry);
        CacheDirectiveInfoInterface listInfo = dirIt.next().getInfo();
     ...
     */
    private void checkPendingCachedEmpty(MiniDockerDFSCluster cluster) throws Exception {
        Thread.sleep(1000);
        cluster.getNamesystem().readLock();
        try {
            final DatanodeManagerInterface datanodeManager = cluster.getNamesystem().getBlockManager().getDatanodeManager();
            for (DataNodeInterface dn : cluster.getDataNodes()) {
                DatanodeDescriptorInterface descriptor = datanodeManager.getDatanode(dn.getDatanodeId());
    ...
    public void testAddingCacheDirectiveInfosWhenCachingIsDisabled() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(NUM_DATANODES).build();
        cluster.waitActive();
        dfs = cluster.getFileSystem();
        namenode = cluster.getNameNode();
        CacheManagerInterface cacheManager = namenode.getNamesystem().getCacheManager();
     ...
     */
    DistributedFileSystem getDFS(MiniDockerDFSCluster cluster, int nnIdx) throws IOException {
    ...
    public void testExpiryTimeConsistency() throws Exception {
        ...
        MiniDockerDFSCluster dfsCluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        dfsCluster.transitionToActive(0);
        DistributedFileSystem fs = getDFS(dfsCluster, 0);
        final NameNodeInterface ann = dfsCluster.getNameNode(0);
        ...
        long id = fs.addCacheDirective(new CacheDirectiveInfo.Builder().setPool("pool").setPath(filename).setExpiration(CacheDirectiveInfo.Expiration.newRelative(86400000)).setReplication(replication).build());
        fs.modifyCacheDirective(new CacheDirectiveInfo.Builder().setId(id).setExpiration(CacheDirectiveInfo.Expiration.newRelative(172800000)).build());
        final NameNodeInterface sbn = dfsCluster.getNameNode(1);
        final CacheManagerInterface annCachemanager = ann.getNamesystem().getCacheManager();
        final CacheManagerInterface sbnCachemanager = sbn.getNamesystem().getCacheManager();
        ...
        GenericTestUtils.waitFor(() -> {
            ...
            try {
                ...
                try {
                    Iterator<CacheDirective> annDirectivesIt = annCachemanager.getCacheDirectives().iterator();
                    Iterator<CacheDirective> sbnDirectivesIt = sbnCachemanager.getCacheDirectives().iterator();
                    if (annDirectivesIt.hasNext() && sbnDirectivesIt.hasNext()) {
                        CacheDirectiveInterface annDirective = annDirectivesIt.next();
                        CacheDirectiveInterface sbnDirective = sbnDirectivesIt.next();
