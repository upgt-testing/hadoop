 ...
 */
public class TestRandomOpsWithSnapshots_RestartInjected_Randomized_123 {
   ...
   */
    ...
    public void testRandomOperationsWithSnapshots() throws IOException, InterruptedException, TimeoutException {
        ...
        RestartFramework.at("after_snapshottable_dirs_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
   ...
   */
    public void randomOperationsWithSnapshots(int numberOfIterations, int numberFileSystemOperations, int numberSnapshotOperations) throws IOException, InterruptedException, TimeoutException {
        ...
        for (int i = 0; i < numberOfIterations; i++) {
            ...
            RestartFramework.at("after_filesystem_operations").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // random number of Snapshot operations
            ...
            // Verification
            checkClusterHealth();
            RestartFramework.at("after_health_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
