...
public class TestViewFileSystemOverloadSchemeWithHdfsScheme {
    ...
    public static void init() throws IOException {
        cluster = new MiniDFSClusterInJVM.Builder(new Configuration()).numDataNodes(2).build();
    ...
    public void setUp() throws IOException {
        ...
        config.setInt(CommonConfigurationKeysPublic.IPC_CLIENT_CONNECT_MAX_RETRIES_KEY, 1);
        config.set(String.format(FS_IMPL_PATTERN_KEY, HDFS_SCHEME), ViewFileSystemOverloadScheme.class.getName());
        config.setBoolean(CONFIG_VIEWFS_IGNORE_PORT_IN_MOUNT_TABLE_NAME, CONFIG_VIEWFS_IGNORE_PORT_IN_MOUNT_TABLE_NAME_DEFAULT);
        setConf(config);
        defaultFSURI = URI.create(config.get(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY));
    ...
    public void testMountLinkWithLocalAndHDFS() throws Exception {
        ...
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/testfile
        ...
        try (DistributedFileSystem dfs = new DistributedFileSystem()) {
            ...
            Assert.assertTrue(dfs.exists(new Path(Path.getPathWithoutSchemeAndAuthority(hdfsTargetPath), // should be in hdfs.
            hdfsFile.getName())));
            Assert.assertFalse(dfs.exists(new Path(Path.getPathWithoutSchemeAndAuthority(localTragetPath), // should not be in local fs.
            localDir.getName())));
        }
        try (RawLocalFileSystem lfs = new RawLocalFileSystem()) {
            lfs.initialize(localTragetPath.toUri(), conf);
            Assert.assertFalse(lfs.exists(new Path(Path.getPathWithoutSchemeAndAuthority(hdfsTargetPath), // should not be in hdfs.
            hdfsFile.getName())));
            Assert.assertTrue(lfs.exists(new Path(Path.getPathWithoutSchemeAndAuthority(localTragetPath), // should be in local fs.
            localDir.getName())));
        }
    }
    ...
    public void testMountLinkWithNonExistentLink(boolean expectFsInitFailure) throws Exception {
        ...
        final Path nonExistTargetPath = new Path("nonexistent://NonExistent" + userFolder);
        /**
         * Below addLink will create following mount points
         * hdfs://localhost:xxx/User --> nonexistent://NonExistent/User/
         */
        addMountLinks(defaultFSURI.getAuthority(), new String[] { userFolder }, new String[] { nonExistTargetPath.toUri().toString() }, conf);
    ...
    public void testListStatusOnRootShouldListAllMountLinks() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            ...
            String lsPath1 = Path.getPathWithoutSchemeAndAuthority(ls[0].getPath()).toString();
            String lsPath2 = Path.getPathWithoutSchemeAndAuthority(ls[1].getPath()).toString();
            Assert.assertTrue(HDFS_USER_FOLDER.equals(lsPath1) || LOCAL_FOLDER.equals(lsPath1));
            Assert.assertTrue(HDFS_USER_FOLDER.equals(lsPath2) || LOCAL_FOLDER.equals(lsPath2));
        }
    }
    ...
    public void testAccessViewFsPathWithoutAuthority() throws Exception {
        ...
        try (FileSystem fs = viewFsRootPath.getFileSystem(conf)) {
            ...
            List<String> expectedPaths = Arrays.stream(expectedStatus).map(s -> s.getPath().getName()).sorted().collect(Collectors.toList());
            List<String> paths = Arrays.stream(status).map(s -> s.getPath().getName()).sorted().collect(Collectors.toList());
    ...
    public void testWithLinkFallBack() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        try (FileSystem fs = FileSystem.get(conf)) {
            fs.createNewFile(new Path("/nonMount/myfile"));
            FileStatus[] ls = fs.listStatus(new Path("/nonMount"));
            Assert.assertEquals(1, ls.length);
            Assert.assertEquals(Path.getPathWithoutSchemeAndAuthority(ls[0].getPath()).getName(), "myfile");
        }
    }
    ...
    public void testInvalidOverloadSchemeTargetFS() throws Exception {
        ...
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, defaultFSURI.toString());
        conf.set(String.format(FS_IMPL_PATTERN_KEY, HDFS_SCHEME), ViewFileSystemOverloadScheme.class.getName());
        conf.unset(String.format(FsConstants.FS_VIEWFS_OVERLOAD_SCHEME_TARGET_FS_IMPL_PATTERN, HDFS_SCHEME));
        try (FileSystem fs = FileSystem.get(conf)) {
    ...
    public void testViewFsOverloadSchemeWithInnerCache() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER + 0, HDFS_USER_FOLDER + 1 }, new String[] { hdfsTargetPath.toUri().toString(), hdfsTargetPath.toUri().toString() }, conf);
        ...
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        try (FileSystem vfs = FileSystem.get(conf)) {
            Assert.assertEquals(isFallBackExist(conf) ? 3 : 2, vfs.getChildFileSystems().length);
        }
    }
    ...
    private boolean isFallBackExist(Configuration config) {
        return config.get(ConfigUtil.getConfigViewFsPrefix(defaultFSURI.getAuthority()) + "." + Constants.CONFIG_VIEWFS_LINK_FALLBACK) != null;
    }
    ...
    public void testViewFsOverloadSchemeWithNoInnerCacheAndLocalSchemeTargets() throws Exception {
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { LOCAL_FOLDER + 0, LOCAL_FOLDER + 1 }, new String[] { localTragetPath.toUri().toString(), localTragetPath.toUri().toString() }, conf);
        // Only one local file system should be there if no InnerCache, but fs
        // cache should work.
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        try (FileSystem vfs = FileSystem.get(conf)) {
            Assert.assertEquals(isFallBackExist(conf) ? 2 : 1, vfs.getChildFileSystems().length);
        }
    }
    ...
    public void testNflyRename() throws Exception {
        ...
        final String nflyLinkKey = Constants.CONFIG_VIEWFS_LINK_NFLY + ".minReplication=2." + nflyRoot.toString();
        addMountLinks(defaultFSURI.getAuthority(), new String[] { nflyLinkKey }, new String[] { uri1.toString() + "," + uri2.toString() }, conf);
    ...
    public void testNflyRepair() throws Exception {
        ...
        final URI uri2 = hdfsTargetPath2.toUri();
        final Path nflyRoot = new Path("/nflyroot");
        final String nflyLinkKey = Constants.CONFIG_VIEWFS_LINK_NFLY + ".minReplication=2," + repairKey + "=true." + nflyRoot.toString();
        addMountLinks(defaultFSURI.getAuthority(), new String[] { nflyLinkKey }, new String[] { uri1.toString() + "," + uri2.toString() }, conf);
    ...
    public void testMountTableNameShouldIgnorePortFromURI() throws Exception {
        ...
        try (FileSystem fs = FileSystem.get(uriWithoutPort, conf)) {
    ...
    private void writeString(final FileSystem nfly, final String testString, final Path testFile) throws IOException {
        try (FSDataOutputStream fsDos = nfly.create(testFile)) {
            fsDos.writeUTF(testString);
        }
    }

    private void readString(final FileSystem nfly, final Path testFile, final String testString, final URI testUri) throws IOException {
    ...
    @Test(timeout = 30000)
    public void testMountLinkWithLocalAndHDFS_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/testfile
    ...
    @Test(timeout = 30000)
    public void testMountLinkWithLocalAndHDFS_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/testfile
    ...
    @Test(timeout = 30000)
    public void testMountLinkWithLocalAndHDFS_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/testfile
    ...
    @Test(timeout = 30000)
    public void testMountLinkWithLocalAndHDFS_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/testfile
    ...
    @Test(timeout = 30000)
    public void testMountLinkWithNonExistentLink_withUpgrade20() throws Exception {
        testMountLinkWithNonExistentLink(true);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test(timeout = 30000)
    public void testListStatusOnRootShouldListAllMountLinks_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        try (DistributedFileSystem dfs = new DistributedFileSystem()) {
            dfs.initialize(defaultFSURI, conf);
    ...
    @Test(timeout = 30000)
    public void testListStatusOnRootShouldListAllMountLinks_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        try (DistributedFileSystem dfs = new DistributedFileSystem()) {
            dfs.initialize(defaultFSURI, conf);
    ...
    @Test(timeout = 30000)
    public void testListStatusOnRootShouldListAllMountLinks_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        try (DistributedFileSystem dfs = new DistributedFileSystem()) {
            dfs.initialize(defaultFSURI, conf);
    ...
    @Test(timeout = 30000)
    public void testListStatusOnRootShouldListAllMountLinks_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        try (DistributedFileSystem dfs = new DistributedFileSystem()) {
            dfs.initialize(defaultFSURI, conf);
    ...
    @Test(expected = IOException.class, timeout = 30000)
    public void testListStatusOnNonMountedPath_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        try (FileSystem fs = FileSystem.get(conf)) {
            fs.listStatus(new Path("/nonMount"));
    ...
    @Test(expected = IOException.class, timeout = 30000)
    public void testListStatusOnNonMountedPath_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        try (FileSystem fs = FileSystem.get(conf)) {
            fs.listStatus(new Path("/nonMount"));
    ...
    @Test
    public void testAccessViewFsPathWithoutAuthority_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/test
        Path hdfsDir = new Path(HDFS_USER_FOLDER, "test");
    ...
    @Test
    public void testAccessViewFsPathWithoutAuthority_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/test
        Path hdfsDir = new Path(HDFS_USER_FOLDER, "test");
    ...
    @Test
    public void testAccessViewFsPathWithoutAuthority_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/test
        Path hdfsDir = new Path(HDFS_USER_FOLDER, "test");
    ...
    @Test
    public void testAccessViewFsPathWithoutAuthority_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        // /HDFSUser/test
        Path hdfsDir = new Path(HDFS_USER_FOLDER, "test");
    ...
    @Test(timeout = 30000)
    public void testWithLinkFallBack_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        try (FileSystem fs = FileSystem.get(conf)) {
            fs.createNewFile(new Path("/nonMount/myfile"));
    ...
    @Test(timeout = 30000)
    public void testWithLinkFallBack_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        try (FileSystem fs = FileSystem.get(conf)) {
            fs.createNewFile(new Path("/nonMount/myfile"));
    ...
    @Test(timeout = 30000)
    public void testWithLinkFallBack_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        try (FileSystem fs = FileSystem.get(conf)) {
            fs.createNewFile(new Path("/nonMount/myfile"));
    ...
    @Test(timeout = 30000)
    public void testCreateOnRoot_withUpgrade20() throws Exception {
        testCreateOnRoot(false);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test(expected = IOException.class, timeout = 30000)
    public void testInvalidOverloadSchemeTargetFS_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        String mountTableIfSet = conf.get(Constants.CONFIG_VIEWFS_MOUNTTABLE_PATH);
        conf = new Configuration();
        if (mountTableIfSet != null) {
    ...
    @Test(expected = IOException.class, timeout = 30000)
    public void testInvalidOverloadSchemeTargetFS_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        String mountTableIfSet = conf.get(Constants.CONFIG_VIEWFS_MOUNTTABLE_PATH);
        conf = new Configuration();
        if (mountTableIfSet != null) {
    ...
    @Test(expected = IOException.class, timeout = 30000)
    public void testInvalidOverloadSchemeTargetFS_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        String mountTableIfSet = conf.get(Constants.CONFIG_VIEWFS_MOUNTTABLE_PATH);
        conf = new Configuration();
        if (mountTableIfSet != null) {
    ...
    @Test(expected = IOException.class, timeout = 30000)
    public void testInvalidOverloadSchemeTargetFS_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        String mountTableIfSet = conf.get(Constants.CONFIG_VIEWFS_MOUNTTABLE_PATH);
        conf = new Configuration();
        if (mountTableIfSet != null) {
    ...
    @Test(timeout = 30000)
    public void testViewFsOverloadSchemeWhenInnerCacheDisabled_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        cluster.restartNodeForTesting(0);
    ...
    @Test(timeout = 30000)
    public void testViewFsOverloadSchemeWhenInnerCacheDisabled_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        try (FileSystem fs = FileSystem.get(conf)) {
    ...
    @Test(timeout = 30000)
    public void testViewFsOverloadSchemeWhenInnerCacheDisabled_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString() }, conf);
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        try (FileSystem fs = FileSystem.get(conf)) {
    ...
    @Test(timeout = 30000)
    public void testViewFsOverloadSchemeWithInnerCache_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER + 0, HDFS_USER_FOLDER + 1 }, new String[] { hdfsTargetPath.toUri().toString(), hdfsTargetPath.toUri().toString() }, conf);
        // 1. Only 1 hdfs child file system should be there with cache.
        try (FileSystem vfs = FileSystem.get(conf)) {
    ...
    @Test(timeout = 30000)
    public void testViewFsOverloadSchemeWithInnerCache_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER + 0, HDFS_USER_FOLDER + 1 }, new String[] { hdfsTargetPath.toUri().toString(), hdfsTargetPath.toUri().toString() }, conf);
        // 1. Only 1 hdfs child file system should be there with cache.
        try (FileSystem vfs = FileSystem.get(conf)) {
    ...
    @Test(timeout = 30000)
    public void testViewFsOverloadSchemeWithInnerCache_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER + 0, HDFS_USER_FOLDER + 1 }, new String[] { hdfsTargetPath.toUri().toString(), hdfsTargetPath.toUri().toString() }, conf);
        // 1. Only 1 hdfs child file system should be there with cache.
        try (FileSystem vfs = FileSystem.get(conf)) {
    ...
    @Test(timeout = 3000)
    public void testViewFsOverloadSchemeWithNoInnerCacheAndHdfsTargets_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER + 0, HDFS_USER_FOLDER + 1 }, new String[] { hdfsTargetPath.toUri().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        cluster.restartNodeForTesting(0);
    ...
    @Test(timeout = 3000)
    public void testViewFsOverloadSchemeWithNoInnerCacheAndHdfsTargets_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        addMountLinks(defaultFSURI.getAuthority(), new String[] { HDFS_USER_FOLDER + 0, HDFS_USER_FOLDER + 1 }, new String[] { hdfsTargetPath.toUri().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
        // Two hdfs file systems should be there if no cache.
    ...
    @Test(timeout = 3000)
    public void testViewFsOverloadSchemeWithNoInnerCacheAndLocalSchemeTargets_withUpgrade20() throws Exception {
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { LOCAL_FOLDER + 0, LOCAL_FOLDER + 1 }, new String[] { localTragetPath.toUri().toString(), localTragetPath.toUri().toString() }, conf);
        // cache should work.
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
    ...
    @Test(timeout = 3000)
    public void testViewFsOverloadSchemeWithNoInnerCacheAndLocalSchemeTargets_withUpgrade40() throws Exception {
        final Path localTragetPath = new Path(localTargetDir.toURI());
        addMountLinks(defaultFSURI.getAuthority(), new String[] { LOCAL_FOLDER + 0, LOCAL_FOLDER + 1 }, new String[] { localTragetPath.toUri().toString(), localTragetPath.toUri().toString() }, conf);
        // cache should work.
        conf.setBoolean(Constants.CONFIG_VIEWFS_ENABLE_INNER_CACHE, false);
    ...
    @Test(timeout = 3000)
    public void testNflyRename_withUpgrade20() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyRename_withUpgrade40() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyRename_withUpgrade60() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyWriteRead_withUpgrade20() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyWriteRead_withUpgrade40() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyWriteRead_withUpgrade60() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyWriteRead_withUpgrade80() throws Exception {
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
        final URI uri2 = hdfsTargetPath2.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyRepair_withUpgrade20() throws Exception {
        final NflyFSystem.NflyKey repairKey = NflyFSystem.NflyKey.repairOnRead;
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyRepair_withUpgrade40() throws Exception {
        final NflyFSystem.NflyKey repairKey = NflyFSystem.NflyKey.repairOnRead;
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyRepair_withUpgrade60() throws Exception {
        final NflyFSystem.NflyKey repairKey = NflyFSystem.NflyKey.repairOnRead;
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
    ...
    @Test(timeout = 3000)
    public void testNflyRepair_withUpgrade80() throws Exception {
        final NflyFSystem.NflyKey repairKey = NflyFSystem.NflyKey.repairOnRead;
        final Path hdfsTargetPath1 = new Path(defaultFSURI + HDFS_USER_FOLDER);
        final Path hdfsTargetPath2 = new Path(defaultFSURI + HDFS_USER_FOLDER + 1);
        final URI uri1 = hdfsTargetPath1.toUri();
    ...
    @Test(timeout = 30000)
    public void testMountTableNameShouldIgnorePortFromURI_withUpgrade20() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        conf = new Configuration(getConf());
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, defaultFSURI.toString());
    ...
    @Test(timeout = 30000)
    public void testMountTableNameShouldIgnorePortFromURI_withUpgrade40() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        conf = new Configuration(getConf());
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, defaultFSURI.toString());
    ...
    @Test(timeout = 30000)
    public void testMountTableNameShouldIgnorePortFromURI_withUpgrade60() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        conf = new Configuration(getConf());
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, defaultFSURI.toString());
    ...
    @Test(timeout = 30000)
    public void testMountTableNameShouldIgnorePortFromURI_withUpgrade80() throws Exception {
        final Path hdfsTargetPath = new Path(defaultFSURI + HDFS_USER_FOLDER);
        conf = new Configuration(getConf());
        addMountLinks(defaultFSURI.getHost(), new String[] { HDFS_USER_FOLDER, LOCAL_FOLDER, Constants.CONFIG_VIEWFS_LINK_FALLBACK }, new String[] { hdfsTargetPath.toUri().toString(), localTargetDir.toURI().toString(), hdfsTargetPath.toUri().toString() }, conf);
        conf.set(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, defaultFSURI.toString());
