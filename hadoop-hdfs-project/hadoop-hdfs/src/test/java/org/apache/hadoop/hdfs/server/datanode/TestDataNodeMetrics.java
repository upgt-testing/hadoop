...
public class TestDataNodeMetrics {
    ...
    public void testReceivePacketSlowMetrics() throws Exception {
        ...
        try {
            ...
            Mockito.doAnswer(answer).when(injector).stopSendingPacketDownstream(Mockito.anyString());
            ...
            List<DataNodeJVMInterface> datanodes = cluster.getDataNodes();
            DataNodeJVMInterface headNode = datanodes.stream().filter(d -> d.getDatanodeId().equals(headDatanodeInfo)).findFirst().orElseGet(null);
            ...
            assertTrue("More than 1 packet received", getLongCounter("PacketsReceived", dnMetrics) > 1L);
            assertTrue("More than 1 slow packet to mirror", getLongCounter("PacketsSlowWriteToMirror", dnMetrics) > 1L);
    ...
    public void testTimeoutMetric() throws Exception {
        ...
        try {
            ...
            final ObjectName mxbeanName = new ObjectName("Hadoop:service=DataNodeJVMInterface,name=DataNodeInfo");
    ...
    public void testSlowMetrics() throws Exception {
        ...
        try {
            ...
            final long startFlushOrSyncValue = getLongCounter("SlowFlushOrSyncCount", rb);
            final long startAckToUpstreamValue = getLongCounter("SlowAckToUpstreamCount", rb);
            final AtomicInteger x = new AtomicInteger(0);
            GenericTestUtils.waitFor(new Supplier<Boolean>() {
                ...
                public Boolean get() {
                    x.getAndIncrement();
                    try {
                        DFSTestUtil.createFile(fs, new Path("/time.txt." + x.get()), longFileLen, (short) 3, Time.monotonicNow());
                    } catch (IOException ioe) {
                        LOG.error("Caught IOException while ingesting DN metrics", ioe);
                        return false;
                    }
                    ...
                    return endFlushOrSyncValue > startFlushOrSyncValue && endAckToUpstreamValue > startAckToUpstreamValue;
                }
            }, 30, 30000);
        } finally {
    ...
    public void testNodeLocalMetrics() throws Exception {
        ...
        conf.set(DFSConfigKeys.DFS_DOMAIN_SOCKET_PATH_KEY, new File(sockDir.getDir(), "testNodeLocalMetrics._PORT.sock").getAbsolutePath());
        MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.restartNodeForTesting(0);
        try {
            cluster.waitActive();
    ...
    public void testDataNodeReadWriteXceiversCount() throws Exception {
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(new HdfsConfiguration()).build()) {
            ...
            long writeXceiversCount = MetricsAsserts.getIntGauge("DataNodeWriteActiveXceiversCount", getMetrics(datanode.getMetrics().name()));
            ...
            try (FSDataOutputStream output = fs.create(path)) {
                ...
                GenericTestUtils.waitFor(new Supplier<Boolean>() {
                    ...
                    @Override
                    public Boolean get() {
                        int writeXceiversCount = MetricsAsserts.getIntGauge("DataNodeWriteActiveXceiversCount", getMetrics(datanode.getMetrics().name()));
            ...
            long readXceiversCount = MetricsAsserts.getIntGauge("DataNodeReadActiveXceiversCount", getMetrics(datanode.getMetrics().name()));
            ...
            try (FSDataInputStream input = fs.open(path)) {
                ...
                GenericTestUtils.waitFor(new Supplier<Boolean>() {
                    ...
                    @Override
                    public Boolean get() {
                        int readXceiversCount = MetricsAsserts.getIntGauge("DataNodeReadActiveXceiversCount", getMetrics(datanode.getMetrics().name()));
