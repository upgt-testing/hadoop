...
public class TestDataNodeMetrics {
    ...
    public void testRoundTripAckMetric() throws Exception {
        ...
        Configuration conf = new HdfsConfiguration();
        conf.set(DFSConfigKeys.DFS_METRICS_PERCENTILES_INTERVALS_KEY, "" + interval);
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(datanodeCount).build();
        try {
            cluster.waitActive();
            FileSystem fs = cluster.getFileSystem();
    ...
    public void testNNRpcMetricsWithHA() throws IOException {
        ...
        // only sends heartbeat once after startup
        conf.setTimeDuration(DFS_HEARTBEAT_INTERVAL_KEY, 1, TimeUnit.HOURS);
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        cluster.waitActive();
        DataNodeInterface dn = cluster.getDataNodes().get(0);
