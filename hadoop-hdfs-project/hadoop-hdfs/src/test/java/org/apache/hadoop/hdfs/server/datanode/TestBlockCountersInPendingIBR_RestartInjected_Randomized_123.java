 ...
 */
public class TestBlockCountersInPendingIBR_RestartInjected_Randomized_123 {
     ...
     */
        ...
        RestartFramework.at("after_ibr_sent").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /*
     ...
     */
        ...
        RestartFramework.at("after_counter_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        actor.getIbrManager().addRDBI(rdbi, storage);
        /* block at status of DELETED_BLOCK */
        rdbi = new ReceivedDeletedBlockInfo(new Block(5680, 512, 1000), BlockStatus.DELETED_BLOCK, null);
        actor.getIbrManager().addRDBI(rdbi, storage);
        RestartFramework.at("after_blocks_added").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        datanode = cluster.getDataNodes().get(0);
