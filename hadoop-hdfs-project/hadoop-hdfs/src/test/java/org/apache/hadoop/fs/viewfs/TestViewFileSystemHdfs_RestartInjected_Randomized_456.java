...
public class TestViewFileSystemHdfs_RestartInjected_Randomized_456 extends ViewFileSystemBaseTest {

    private static final Logger LOG = LoggerFactory.getLogger(TestViewFileSystemHdfs_RestartInjected.class);
    ...
    public void testTrashRootsAfterEncryptionZoneDeletion() throws Exception {
        ...
        RestartFramework.at("after_zone_deletion").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_deletion").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDf() throws Exception {
        ...
        RestartFramework.at("at_75_percent").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("at_25_percent").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.FsShellRun("-df viewfs:///user", 0, "/user", newConf);
        DFSTestUtil.FsShellRun("-df /user3", 1, "/user3", newConf);
        RestartFramework.at("at_test_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.FsShellRun("-df /user2/abc", 1, "No such file or directory", newConf);
        DFSTestUtil.FsShellRun("-df /user2/", 0, "/user2", newConf);
        RestartFramework.at("at_50_percent").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFileChecksum() throws IOException {
        ...
        RestartFramework.at("after_first_checksum").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testNflyRepair(NflyFSystem.NflyKey repairKey) throws Exception {
        ...
        RestartFramework.at("after_file_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final URI[] testUris = new URI[] { new URI(uri1.getScheme(), uri1.getAuthority(), "/", null, null), new URI(uri2.getScheme(), uri2.getAuthority(), "/", null, null) };
        ...
        RestartFramework.at("after_file_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final String testString = "Hello Nfly!";
        final Path nflyRoot = new Path("/nflyroot");
        ConfigUtil.addLinkNfly(testConf, clusterName, nflyRoot.toString(), "minReplication=2," + repairKey + "=true", testUris);
        final FileSystem nfly = FileSystem.get(URI.create("viewfs:///"), testConf);
        RestartFramework.at("after_nn_restart_cycle").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_repair_operation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        if (repairKey == NflyFSystem.NflyKey.readMostRecent) {
            ...
            final long expectedMtime = fs2.getFileStatus(testFile).getModificationTime();
            for (final URI testUri : testUris) {
                final FileSystem fs = FileSystem.get(testUri, conf);
                fs.setTimes(testFile, 1L, 1L);
                assertEquals(testUri + "Set mtime failed!", 1L, fs.getFileStatus(testFile).getModificationTime());
                assertEquals("nfly file status wrong", expectedMtime, nfly.getFileStatus(testFile).getModificationTime());
                ...
                try {
                    ...
                    //
                    assertEquals("Repair most recent failed!", expectedMtime, fs.getFileStatus(testFile).getModificationTime());
                } finally {
    ...
    public void testTargetFileSystemLazyInitializationWithUgi() throws Exception {
        ...
        RestartFramework.at("after_final_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Change the permission of /data path so that user1 can create a directory
        fsTarget.setOwner(new Path(targetTestRoot, "data"), "user1", "test2");
        // set permission of target to allow rename to target
        fsTarget.setPermission(new Path(targetTestRoot, "data"), new FsPermission("775"));
        RestartFramework.at("after_permission_change").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        userUgi.doAs(new PrivilegedExceptionAction<Object>() {
        ...
        RestartFramework.at("after_initial_mkdir_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testInternalDirectoryPermissions() throws IOException {
        ...
        ConfigUtil.addLinkFallback(localConf, new Path(targetTestRoot, "fallbackDir").toUri());
        FileSystem fs = FileSystem.get(FsConstants.VIEWFS_URI, localConf);
        RestartFramework.at("after_internal_dir_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_real_dir_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertEquals(fs.getFileStatus(subDirOfInternalDir).getPermission(), fs.getFileStatus(subDirOfRealDir).getPermission());
    }
}
