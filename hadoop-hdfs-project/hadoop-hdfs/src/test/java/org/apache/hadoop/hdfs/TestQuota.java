...
public class TestQuota {
    ...
    public void testSetAndClearSpaceQuotaDirecotryNotExist() throws Exception {
        final Path dir = new Path(PathUtils.getTestPath(getClass()), GenericTestUtils.getMethodName());
        /* set space quota */
        testSetAndClearSpaceQuotaDirecotryNotExistInternal(new String[] { "-setSpaceQuota", "1024", dir.toString() }, dir, -1, "setSpaceQuota");
        /* clear space quota */
        testSetAndClearSpaceQuotaDirecotryNotExistInternal(new String[] { "-clrSpaceQuota", dir.toString() }, dir, -1, "clrSpaceQuota");
    }

    private void testSetAndClearSpaceQuotaDirecotryNotExistInternal(final String[] args, final Path dir, final int cmdRet, final String cmdName) throws Exception {
        resetStream();
        final DFSAdmin dfsAdmin = new DFSAdmin(conf);
        final List<String> outs = Lists.newArrayList();
        final int ret = ToolRunner.run(dfsAdmin, args);
