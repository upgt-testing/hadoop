...
public class TestLazyPersistReplicaRecovery_RestartInjected_Randomized_456 extends LazyPersistTestCase {
    ...
    public void testDnRestartWithSavedReplicas() throws IOException, InterruptedException, TimeoutException {
        ...
        RestartFramework.at("after_lazy_persist_completion").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue("DN did not restart properly", cluster.restartDataNode(0, true));
    ...
    private boolean waitForBlockReport(final DataNode dn, final DatanodeDescriptor dnd) throws IOException, InterruptedException {
        ...
        try {
            GenericTestUtils.waitFor(() -> lastCount != storage.getBlockReportCount(), 10, 10000);
