...
public class TestDiskBalancerCommand {
    ...
    public void setUp() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storagesPerDatanode(2).build();
        cluster.waitActive();
        clusterJson = getClass().getResource("/diskBalancer/data-cluster-64node-3disk.json").toURI();
    }
    ...
    public void testSubmitPlanInNonRegularStatus() throws Exception {
        ...
        MiniDockerDFSCluster miniCluster = null;
    ...
    public void testDiskBalancerExecuteOptionPlanValidityWithException() throws Exception {
        ...
        /* new cluster with imbalanced capacity */
        final MiniDockerDFSCluster miniCluster = DiskBalancerTestUtil.newImbalancedCluster(hdfsConf, numDatanodes, CAPACITIES, DEFAULT_BLOCK_SIZE, FILE_LEN);
        try {
            /* get full path of plan */
            final String planFileFullName = runAndVerifyPlan(miniCluster, hdfsConf);
            /* run execute command */
    ...
    public void testDiskBalancerForceExecute() throws Exception {
        ...
        try {
            ...
            final String planFileFullName = runAndVerifyPlan(miniCluster, hdfsConf);
            /* run execute command */
            final String cmdLine = String.format("hdfs diskbalancer -%s %s -%s", EXECUTE, planFileFullName, SKIPDATECHECK);
    ...
    private String runAndVerifyPlan(final MiniDockerDFSCluster miniCluster, final Configuration hdfsConf) throws Exception {
        String cmdLine = "";
        List<String> outputs = null;
        final DataNodeInterface dn = miniCluster.getDataNodes().get(0);
        /* run plan command */
    ...
    public void testExceptionOnInvalidArguments() throws Exception {
        ...
        thrown.expectMessage("Invalid or extra Arguments: [random1, random2, random3]");
        runCommand(cmdLine);
    }
    ...
    public void testReportSimple() throws Exception {
        ...
        assertThat(outputs.get(1), is(allOf(containsString("No top limit specified"), containsString("using default top value"), containsString("100"))));
    ...
    public void testReportNodeWithoutJson() throws Exception {
        ...
        assertThat(outputs.get(3), is(allOf(containsString("DISK"), containsString(new Path(cluster.getInstanceStorageDir(0, 0).getAbsolutePath()).toString()), containsString("0.00"), containsString("1.00"))));
        assertThat(outputs.get(4), is(allOf(containsString("DISK"), containsString(new Path(cluster.getInstanceStorageDir(0, 1).getAbsolutePath()).toString()), containsString("0.00"), containsString("1.00"))));
    }
    ...
    public void testPlanNode() throws Exception {
        final String planArg = String.format("-%s %s", PLAN, cluster.getDataNodes().get(0).getDatanodeUuid());
        final String cmdLine = String.format("hdfs diskbalancer %s", planArg);
    ...
    public void testPlanJsonNode() throws Exception {
        final String planArg = String.format("-%s %s", PLAN, "a87654a9-54c7-4693-8dd9-c9c7021dc340");
   ...
   */
    ...
    private List<String> runCommandInternal(final String cmdLine, final Configuration clusterConf) throws Exception {
    ...
    private List<String> runCommand(final String cmdLine, MiniDockerDFSCluster miniCluster) throws Exception {
        FileSystem.setDefaultUri(conf, miniCluster.getURI());
        return runCommandInternal(cmdLine);
    }

    private List<String> runCommand(final String cmdLine, Configuration clusterConf, MiniDockerDFSCluster miniCluster) throws Exception {
    ...
    public void testDiskBalancerQueryWithoutSubmit() throws Exception {
        ...
        MiniDockerDFSCluster miniDFSCluster = new MiniDockerDFSCluster.Builder(conf, basedir).numDataNodes(numDatanodes).build();
        try {
            miniDFSCluster.waitActive();
            DataNodeInterface dataNode = miniDFSCluster.getDataNodes().get(0);
    ...
    public void testReportCommandWithReadingHostFile() throws Exception {
        ...
        fw.close();
        final String planArg = String.format("-%s -%s file://%s", REPORT, NODE, filePath);
        final String cmdLine = String.format("hdfs diskbalancer %s", planArg);
        List<String> outputs = runCommand(cmdLine, cluster);
