 ...
 */
public class TestDataNodeErasureCodingMetrics_RestartInjected_Randomized_456 {
    ...
    public void setup() throws IOException {
        ...
        cluster.waitActive();
        RestartFramework.at("after_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void doTest(String fileName, int fileLen, int deadNodeIndex) throws Exception {
        ...
        RestartFramework.at("after_trigger_heartbeats").on(cluster).restart("datanode").withIndex((deadNodeIndex + 1) % numDNs).withMode(RestartMode.GRACEFUL).execute();
        LOG.info("Datanode to be corrupted: " + toCorruptDn);
        RestartFramework.at("after_reconstruction").on(cluster).restart("datanode").withIndex((deadNodeIndex + 1) % numDNs).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_datanode_shutdown").on(cluster).restart("datanode").withIndex((deadNodeIndex + 1) % numDNs).withMode(RestartMode.GRACEFUL).execute();
