...
public class TestDataNodeHotSwapVolumes {
     ...
     */
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(nnTopology).numDataNodes(numDataNodes).storagesPerDatanode(storagePerDataNode).build();
        cluster.waitActive();
    }
    ...
    public void testParseChangedVolumes() throws IOException {
        ...
        DataNode.ChangedVolumesInterface changedVolumes = dn.parseChangedVolumes(newPaths);
    ...
    private void addVolumes(int numNewVolumes, CountDownLatch waitLatch) throws ReconfigurationException, IOException, InterruptedException {
        ...
        for (int i = 0; i < expectDataDirs.length; i++) {
            StorageLocationInterface expectLocation = StorageLocation.parse(expectDataDirs[i]);
            StorageLocationInterface effectiveLocation = StorageLocation.parse(effectiveDataDirs[i]);
    ...
    private List<List<Integer>> getNumBlocksReport(int namesystemIdx) {
        ...
        for (Map<DatanodeStorage, BlockListAsLongs> datanodeReport : blockReports) {
            ...
            for (BlockListAsLongsInterface blocks : datanodeReport.values()) {
    ...
    /**
     * Test adding one volume on a running MiniDockerDFSCluster with only one NameNode.
     */
    @Test(timeout = 60000)
    public void testAddOneNewVolume() throws IOException, ReconfigurationException, InterruptedException, TimeoutException {
        ...
        for (BlockListAsLongsInterface blockList : blockReports.get(0).values()) {
    ...
    /**
     * Test re-adding one volume with some blocks on a running MiniDockerDFSCluster
    ...
    public void testReAddVolumeWithBlocks() throws IOException, ReconfigurationException, InterruptedException, TimeoutException {
        ...
        assertEquals(1, blockReports.size());
        // 2 volumes
        assertEquals(2, blockReports.get(0).size());
        ...
        assertEquals(1, blockReports.size());
        // 1 volume
        assertEquals(1, blockReports.get(0).size());
        for (BlockListAsLongsInterface blockList : blockReports.get(0).values()) {
            assertEquals(15, blockList.getNumberOfBlocks());
        }
        // Now add the original volume back again and ensure 15 blocks are reported
        assertThat("DN did not update its own config", dn.reconfigurePropertyImpl(DFSConfigKeys.DFS_DATANODE_DATA_DIR_KEY, String.join(",", oldDirs)), is(dn.getConf().get(DFS_DATANODE_DATA_DIR_KEY)));
        dn.scheduleAllBlockReport(0);
        blockReports = cluster.getAllBlockReports(bpid);
        // 1 DataNode
        assertEquals(1, blockReports.size());
    ...
    public void testAddVolumesDuringWrite() throws IOException, InterruptedException, TimeoutException, ReconfigurationException {
        ...
        for (BlockListAsLongsInterface blockList : dnReport.values()) {
    ...
    public void testAddVolumesConcurrently() throws IOException, InterruptedException, TimeoutException, ReconfigurationException {
        ...
        List<Thread> addVolumeDelayedThreads = Collections.synchronizedList(new ArrayList<>());
    ...
    public void testRemoveOneVolume() throws ReconfigurationException, InterruptedException, TimeoutException, IOException {
        ...
        BlockListAsLongsInterface blocksForVolume1 = blockReports.get(0).values().iterator().next();
    ...
    public void testAddVolumeFailures() throws IOException {
        ...
        DataStorageInterface storage = dn.getStorage();
    ...
    private void testRemoveVolumeBeingWrittenForDatanode(int dataNodeIdx) throws IOException, ReconfigurationException, TimeoutException, InterruptedException, BrokenBarrierException {
        ...
        try {
            ...
            final DataNodeInterface dataNode = dn;
    ...
    public void testAddBackRemovedVolume() throws IOException, TimeoutException, InterruptedException, ReconfigurationException {
        ...
        for (int i = 0; i < cluster.getNumNameNodes(); i++) {
            ...
            BlockPoolSliceStorageInterface bpsStorage = dn.getStorage().getBPStorage(bpid);
            // Make sure that there is no block pool level storage under removeDataDir.
            for (int j = 0; j < bpsStorage.getNumStorageDirs(); j++) {
                Storage.StorageDirectoryInterface sd = bpsStorage.getStorageDir(j);
    ...
    public void testFullBlockReportAfterRemovingVolumes() throws IOException, ReconfigurationException {
        ...
        DatanodeProtocolClientSideTranslatorPBInterface spy = InternalDataNodeTestUtils.spyOnBposToNN(dn, cluster.getNameNode());
