...
public class TestBlockReaderFactory {
   ...
   */
        ...
        try (TemporarySocketDirectory sockDir = new TemporarySocketDirectory()) {
            ...
            try (MiniDFSClusterInJVM cluster = builder.build();
                ...
                try (FSDataInputStream in = dfs.open(testFile)) {
                    Assert.assertEquals(0, dfs.getClient().getClientContext().getShortCircuitCache().getReplicaInfoMapSize());
                    ...
                    dfs.getClient().getClientContext().getShortCircuitCache().setMaxTotalSize(0);
    ...
    private void validateReadResult(final DistributedFileSystem dfs, final byte[] expected, final byte[] actual, final int expectedScrRepMapSize) {
        ...
        Assert.assertEquals(expectedScrRepMapSize, dfs.getClient().getClientContext().getShortCircuitCache().getReplicaInfoMapSize());
    }
