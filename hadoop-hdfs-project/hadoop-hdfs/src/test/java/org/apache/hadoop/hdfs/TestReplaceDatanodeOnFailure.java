...
public class TestReplaceDatanodeOnFailure {
    ...
    public void testReplaceDatanodeOnFailure() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).racks(racks).numDataNodes(REPLICATION).build();
        try {
            cluster.waitActive();
            ...
            MiniDockerDFSCluster.DataNodeProperties dnprop = cluster.stopDataNode(AppendTestUtil.nextInt(REPLICATION));
