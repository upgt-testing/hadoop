...
public class TestBlockReplacement {
    ...
    public void testBlockReplacement() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(CONF).numDataNodes(REPLICATION_FACTOR).racks(INITIAL_RACKS).build();
        try {
            cluster.waitActive();
    ...
    public void testBlockReplacementWithPinnedBlocks() throws Exception {
        ...
        // types.
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(3).storageTypes(new StorageType[] { StorageType.DISK, StorageType.ARCHIVE }).build();
        try {
            cluster.waitActive();
    ...
    public void testBlockMoveAcrossStorageInSameNode() throws Exception {
        ...
        try {
            ...
            DatanodeInfoInterface source = locations[0];
    ...
    private void checkBlocks(DatanodeInfo[] includeNodes, String fileName, long fileLen, short replFactor, DFSClient client) throws IOException, TimeoutException {
        ...
        do {
            ...
            if (notDone) {
                ...
                for (DatanodeInfoInterface node : includeNodes) {
            ...
            if (Time.monotonicNow() > failtime) {
                ...
                for (DatanodeInfoInterface dn : includeNodes) expectedNodesList += dn + ", ";
                for (DatanodeInfoInterface dn : nodes) currentNodesList += dn + ", ";
