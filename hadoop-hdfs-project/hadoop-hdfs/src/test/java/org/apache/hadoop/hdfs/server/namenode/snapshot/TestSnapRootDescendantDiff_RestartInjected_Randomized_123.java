 ...
 */
public class TestSnapRootDescendantDiff_RestartInjected_Randomized_123 extends TestSnapshotDiffReport {
    ...
    public void testNonSnapRootDiffReport() throws Exception {
        ...
        RestartFramework.at("after_first_snapshot_modification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        modifyAndCreateSnapshot(subsubsub1, new Path[] { getSnapRootDir() });
        RestartFramework.at("after_second_snapshot_modification").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
