...
public class TestDFSInputStreamBlockLocations {
    ...
    public void testRefreshBlockLocations() throws IOException {
        ...
        try (DFSInputStream fin = dfsClient.open(fileName)) {
            ...
            assertFalse("should not have attempted refresh", fin.refreshBlockLocations(null));
            assertEquals("should not have updated lastRefreshedAt", lastRefreshedAt, fin.getLastRefreshedBlocksAtForTesting());
            assertSame("should not have modified locatedBlocks", existing, fin.locatedBlocks);
            ...
            assertTrue("should have attempted refresh", fin.refreshBlockLocations(null));
            ...
            assertTrue("should have attempted refresh", fin.refreshBlockLocations(mockAddressCache));
            verifyChanged(fin, existing, lastRefreshedAt);
        }
    }

    private void verifyChanged(DFSInputStream fin, LocatedBlocks existing, long lastRefreshedAt) {
        assertTrue("lastRefreshedAt should have incremented", fin.getLastRefreshedBlocksAtForTesting() > lastRefreshedAt);
        assertNotSame("located blocks should have changed", existing, fin.locatedBlocks);
        assertTrue("deadNodes should be empty", fin.getLocalDeadNodes().isEmpty());
    }
    ...
    private void testWithRegistrationMethod(ThrowingConsumer registrationMethod) throws IOException {
        ...
        try {
            ...
            assertFalse("should not be tracking input stream on open", dfsClient.getLocatedBlockRefresher().isInputStreamTracked(fin));
            // still not registered because it hasn't been an hour by the time we call this
            registrationMethod.accept(fin);
            assertFalse("should not be tracking input stream after first read", dfsClient.getLocatedBlockRefresher().isInputStreamTracked(fin));
            ...
            registrationMethod.accept(fin);
            assertEquals("SHOULD be tracking input stream on read after interval, only if enabled", enableBlkExpiration, dfsClient.getLocatedBlockRefresher().isInputStreamTracked(fin));
        } finally {
            if (fin != null) {
                fin.close();
    ...
    @Test
    public void testRefreshBlockLocations_withUpgrade20() throws IOException {
        final String fileName = "/test_cache_locations";
        filePath = createFile(fileName);
        try (DFSInputStream fin = dfsClient.open(fileName)) {
            LocatedBlocks existing = fin.locatedBlocks;
    ...
    @Test
    public void testRefreshBlockLocations_withUpgrade40() throws IOException {
        final String fileName = "/test_cache_locations";
        filePath = createFile(fileName);
        try (DFSInputStream fin = dfsClient.open(fileName)) {
            LocatedBlocks existing = fin.locatedBlocks;
    ...
    @Test
    public void testRefreshBlockLocations_withUpgrade60() throws IOException {
        final String fileName = "/test_cache_locations";
        filePath = createFile(fileName);
        try (DFSInputStream fin = dfsClient.open(fileName)) {
            LocatedBlocks existing = fin.locatedBlocks;
    ...
    @Test
    public void testRefreshBlockLocations_withUpgrade80() throws IOException {
        final String fileName = "/test_cache_locations";
        filePath = createFile(fileName);
        try (DFSInputStream fin = dfsClient.open(fileName)) {
            LocatedBlocks existing = fin.locatedBlocks;
