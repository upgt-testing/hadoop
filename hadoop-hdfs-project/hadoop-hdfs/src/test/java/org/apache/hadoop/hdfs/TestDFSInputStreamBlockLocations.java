...
public class TestDFSInputStreamBlockLocations {
    ...
    public void setup() throws IOException {
        ...
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_HEARTBEAT_RECHECK_INTERVAL_KEY, staleInterval / 2);
        ...
        conf.setLong(HdfsClientConfigKeys.Read.PREFETCH_SIZE_KEY, dfsClientPrefetchSize);
        if (enableBlkExpiration) {
            // set the refresh locations for every dfsInputLocationsTimeout
            conf.setLong(HdfsClientConfigKeys.DFS_CLIENT_REFRESH_READ_BLOCK_LOCATIONS_MS_KEY, dfsInputLocationsTimeout);
        }
        // start the cluster and create a DFSClient
        dfsCluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(NUM_DATA_NODES).racks(RACKS).build();
        dfsCluster.waitActive();
        assertEquals(NUM_DATA_NODES, dfsCluster.getDataNodes().size());
        InetSocketAddress addr = new InetSocketAddress("localhost", dfsCluster.getNameNodePort());
    ...
    public void testRead() throws Exception {
        ...
        try {
            ...
            LocatedBlocks referenceLocatedBlocks = dfsClient.getLocatedBlocks(fileName, 0, fileLength);
            ...
            assertEquals(dfsClientPrefetchSize / BLOCK_SIZE, finLocatedBlocks.locatedBlockCount());
            ...
            LocatedBlock firstLocatedBlk = fin.locatedBlocks.getLocatedBlocks().get(0);
            DatanodeInfo[] firstBlkDNInfos = firstLocatedBlk.getLocations();
            while (fin.getPos() < firstBlockMark) {
                bytesRead = fin.read(readBuffer);
                Assert.assertTrue("Unexpected number of read bytes", chunkReadSize >= bytesRead);
                if (currDNInfo == null) {
                ...
                currDNInfo = fin.getCurrentDatanode();
                assertEquals("the DFSInput stream does not read from same node", prevDNInfo, currDNInfo);
            }
            assertEquals("InputStream exceeds expected position", firstBlockMark, fin.getPos());
            // get the second block locations
            LocatedBlock secondLocatedBlk = fin.locatedBlocks.getLocatedBlocks().get(1);
            ...
            DataNodeJVMInterface deadNode = getdataNodeFromHostName(dfsCluster, deadNodeInfo.getHostName());
            // Shutdown and wait for datanode to be marked dead
            DatanodeRegistrationJVMInterface reg = InternalDataNodeTestUtils.getDNRegistrationForBP(dfsCluster.getDataNodes().get(0), poolId);
            DataNodeProperties stoppedDNProps = dfsCluster.stopDataNode(deadNodeInfo.getName());
            ...
            // get the located blocks
            LocatedBlocks afterStoppageLocatedBlocks = dfsClient.getLocatedBlocks(fileName, 0, fileLength);
            // read second block
            ...
            while (fin.getPos() < secondBlockMark) {
                bytesRead = fin.read(readBuffer);
                assertTrue("dead node used to read at position: " + fin.getPos(), fin.deadNodesContain(deadNodeInfo));
                Assert.assertTrue("Unexpected number of read bytes", chunkReadSize >= bytesRead);
                prevDNInfo = currDNInfo;
                currDNInfo = fin.getCurrentDatanode();
                ...
                if (firstIteration) {
                    ...
                    assertFalse("FSInputStream should pick a different DN", firstBlkDNInfos[0].equals(deadNodeInfo) && prevDNInfo.equals(currDNInfo));
                    firstIteration = false;
                }
            }
            assertEquals("InputStream exceeds expected position", secondBlockMark, fin.getPos());
            ...
            while (fin.getPos() < thirdBlockMark) {
                bytesRead = fin.read(readBuffer);
                if (this.enableBlkExpiration) {
                    assertEquals("node is removed from deadNodes after 1st iteration", firstIteration, fin.deadNodesContain(deadNodeInfo));
                } else {
                ...
                if (firstIteration) {
                    ...
                    if (this.enableBlkExpiration) {
                        ...
                        fin.setReadTimeStampsForTesting(Time.monotonicNow() - (dfsInputLocationsTimeout + 1));
                    }
                }
            }
            assertEquals("InputStream exceeds expected position", thirdBlockMark, fin.getPos());
        } finally {
            if (fout != null) {
                fout.close();
            }
    ...
    private DataNodeJVMInterface getdataNodeFromHostName(MiniDFSClusterInJVM cluster, String hostName) {
    ...
    @Test
    public void testRead_withUpgrade20() throws Exception {
        final String fileName = "/test_cache_locations";
        filePath = new Path(fileName);
        DFSInputStream fin = null;
        FSDataOutputStream fout = null;
    ...
    @Test
    public void testRead_withUpgrade40() throws Exception {
        final String fileName = "/test_cache_locations";
        filePath = new Path(fileName);
        DFSInputStream fin = null;
        FSDataOutputStream fout = null;
    ...
    @Test
    public void testRead_withUpgrade60() throws Exception {
        final String fileName = "/test_cache_locations";
        filePath = new Path(fileName);
        DFSInputStream fin = null;
        FSDataOutputStream fout = null;
    ...
    @Test
    public void testRead_withUpgrade80() throws Exception {
        final String fileName = "/test_cache_locations";
        filePath = new Path(fileName);
        DFSInputStream fin = null;
        FSDataOutputStream fout = null;
