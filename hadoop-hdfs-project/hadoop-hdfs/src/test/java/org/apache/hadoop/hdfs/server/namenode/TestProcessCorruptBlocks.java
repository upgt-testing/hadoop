...
public class TestProcessCorruptBlocks {
    ...
    public void testByAddingAnExtraDataNode() throws Exception {
        ...
        conf.setLong(DFSConfigKeys.DFS_BLOCKREPORT_INTERVAL_MSEC_KEY, 1000L);
        conf.set(DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_KEY, Integer.toString(2));
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(4).build();
        FileSystem fs = cluster.getFileSystem();
        final FSNamesystemInterface namesystem = cluster.getNamesystem();
    ...
    private void corruptBlock(MiniDockerDFSCluster cluster, FileSystem fs, final Path fileName, int dnIndex, ExtendedBlock block) throws IOException {
