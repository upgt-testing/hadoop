...
public class TestDelegationToken_RestartInjected_Randomized_456 {
    ...
    public void testDelegationTokenSecretManager() throws Exception {
        ...
        RestartFramework.at("after_token_generation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_token_renewal").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(5000);
        try {
            dtSecretManager.renewToken(token, "JobTracker");
    ...
    public void testCancelDelegationToken() throws Exception {
        ...
        RestartFramework.at("after_token_generation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_token_cancellation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            dtSecretManager.renewToken(token, "JobTracker");
            Assert.fail("should have failed");
        } catch (InvalidToken it) {
