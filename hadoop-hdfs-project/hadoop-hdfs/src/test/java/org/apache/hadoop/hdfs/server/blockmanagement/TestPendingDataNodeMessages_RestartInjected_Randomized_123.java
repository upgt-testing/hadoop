...
public class TestPendingDataNodeMessages_RestartInjected_Randomized_123 {
    ...
    public void testPendingDataNodeMessagesWithEC() throws Exception {
        ...
        final MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(numDn).nnTopology(MiniDFSNNTopology.simpleHATopology()).build();
        try {
            ...
            DFSTestUtil.createFile(fs, new Path(dirPath, "file"), ecPolicy.getCellSize() * ecPolicy.getNumDataUnits(), (short) 1, 0);
            ...
            RestartFramework.at("after_tailer_process").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertEquals("All DN message should processed after tail edits", 0, pendingIBRMsg);
        } finally {
            cluster.shutdown();
        }
    }
