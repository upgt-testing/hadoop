...
public class TestDataNodeMXBean extends SaslDataTransferTestCase {
    ...
    public void testDataNodeMXBeanSecurityEnabled() throws Exception {
        ...
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(simpleConf).build()) {
            List<DataNodeJVMInterface> datanodes = cluster.getDataNodes();
            Assert.assertEquals(datanodes.size(), 1);
            DataNodeJVMInterface datanode = datanodes.get(0);
            MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
        ...
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(secureConf).build()) {
            List<DataNodeJVMInterface> datanodes = cluster.getDataNodes();
            Assert.assertEquals(datanodes.size(), 1);
            DataNodeJVMInterface datanode = datanodes.get(0);
            MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
    ...
    public void testDataNodeMXBeanLastHeartbeats() throws Exception {
        Configuration conf = new Configuration();
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology(2)).build()) {
            cluster.waitActive();
            ...
            GenericTestUtils.waitFor(() -> {
                ...
                long lastHeartbeatResponseTime1 = Long.parseLong(bpServiceActorInfo1.get("LastHeartbeatResponseTime"));
                long lastHeartbeatResponseTime2 = Long.parseLong(bpServiceActorInfo2.get("LastHeartbeatResponseTime"));
                ...
                return (lastHeartbeatResponseTime1 < 5L && lastHeartbeatResponseTime2 > 5L) || (lastHeartbeatResponseTime1 > 5L && lastHeartbeatResponseTime2 < 5L);
            }, 200, 15000, "Last heartbeat response should be higher than 5s for at least one namenode");
            cluster.restartNodeForTesting(0);
            cluster.upgradeNodeForTesting(0);
    ...
    private static void assertLastHeartbeatSentTime(DataNode datanode, String lastHeartbeat) {
        ...
        Map<String, String> bpServiceActorInfo1 = bpServiceActorInfo.get(0);
        Map<String, String> bpServiceActorInfo2 = bpServiceActorInfo.get(1);
        long lastHeartbeatSent1 = Long.parseLong(bpServiceActorInfo1.get(lastHeartbeat));
        long lastHeartbeatSent2 = Long.parseLong(bpServiceActorInfo2.get(lastHeartbeat));
        Assert.assertTrue(lastHeartbeat + " for first bp service actor is higher than 5s", lastHeartbeatSent1 < 5L);
        Assert.assertTrue(lastHeartbeat + " for second bp service actor is higher than 5s", lastHeartbeatSent2 < 5L);
    }
