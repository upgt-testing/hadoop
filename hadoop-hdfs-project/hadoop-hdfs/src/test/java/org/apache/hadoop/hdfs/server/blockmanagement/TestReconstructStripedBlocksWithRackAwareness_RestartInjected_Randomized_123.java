...
public class TestReconstructStripedBlocksWithRackAwareness_RestartInjected_Randomized_123 {

    public static final Logger LOG = LoggerFactory.getLogger(TestReconstructStripedBlocksWithRackAwareness_RestartInjected.class);
    ...
    public void testReconstructForNotEnoughRacks() throws Exception {
        ...
        RestartFramework.at("after_restart_last_datanode").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        LOG.info("Created file {}", file);
        final INodeFile fileNode = fsn.getFSDirectory().getINode4Write(file.toString()).asFile();
        RestartFramework.at("after_stop_last_datanode").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_process_misreplicated_blocks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testChooseExcessReplicasToDelete() throws Exception {
        ...
        cluster.waitActive();
        fs = cluster.getFileSystem();
        RestartFramework.at("test2_after_ec_policy_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
        fs.setErasureCodingPolicy(new Path("/"), StripedFileTestUtil.getDefaultECPolicy().getName());
        MiniDFSCluster.DataNodeProperties lastHost = stopDataNode(hosts[hosts.length - 1]);
        final Path file = new Path("/foo");
        ...
        RestartFramework.at("test2_after_restart_last_host").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // bring last host back
        cluster.restartDataNode(lastHost);
        RestartFramework.at("test2_after_restart_host1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.waitActive();
        RestartFramework.at("test2_after_stop_host1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // wait for reconstruction to finish
        final short blockNum = (short) (dataBlocks + parityBlocks);
        RestartFramework.at("test2_after_reconstruction").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.waitForReplication(fs, file, blockNum, 15 * 1000);
        // restart host1
        cluster.restartDataNode(host1);
        cluster.waitActive();
        RestartFramework.at("test2_after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (DataNode dn : cluster.getDataNodes()) {
        ...
        RestartFramework.at("test2_after_stop_last_host").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReconstructionWithDecommission() throws Exception {
        ...
        RestartFramework.at("test3_after_restart_h10_h11").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        cluster.waitActive();
        RestartFramework.at("test3_after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs = cluster.getFileSystem();
        fs.enableErasureCodingPolicy(StripedFileTestUtil.getDefaultECPolicy().getName());
        RestartFramework.at("test3_after_recovery").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.setErasureCodingPolicy(new Path("/"), StripedFileTestUtil.getDefaultECPolicy().getName());
        RestartFramework.at("test3_after_stop_h11").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        cluster.waitActive();
        RestartFramework.at("test3_after_mark_h9_decommissioning").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("test3_after_restart_h9").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("test3_after_ec_policy_setup").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("test3_after_stop_h9_h10").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
