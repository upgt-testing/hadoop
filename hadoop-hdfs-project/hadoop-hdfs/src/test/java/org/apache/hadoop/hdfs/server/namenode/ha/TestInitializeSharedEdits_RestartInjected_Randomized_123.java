...
public class TestInitializeSharedEdits_RestartInjected_Randomized_123 {
    ...
    public void testInitializeSharedEdits() throws Exception {
        ...
        RestartFramework.at("after_second_init_shared_edits").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_init_shared_edits").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_start_ha_nodes").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_start_ha_nodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        shutdownClusterAndRemoveSharedEditsDir();
        RestartFramework.at("after_shutdown_and_remove").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertCannotStartNameNodes();
