 ...
 */
public class TestDiskError_RestartInjected_Randomized_456 {
    ...
    public void testShutdown() throws Exception {
        ...
        RestartFramework.at("after_additional_datanodes").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        File dir1 = MiniDFSCluster.getRbwDir(storageDir, bpid);
        storageDir = cluster.getInstanceStorageDir(dnIndex, 1);
        RestartFramework.at("after_initial_cluster_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            RestartFramework.at("after_dirs_readonly").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testReplicationError() throws Exception {
        ...
        RestartFramework.at("after_second_datanode_start").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // write the header.
        ...
        out.close();
        RestartFramework.at("after_temp_files_cleanup").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testcheckDiskError() throws Exception {
        ...
        RestartFramework.at("after_datanode_ready").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        //checking for disk error
        final long lastCheckTimestamp = dataNode.getLastDiskErrorCheck();
        dataNode.checkDiskError();
        RestartFramework.at("after_check_disk_error").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDataTransferWhenBytesPerChecksumIsZero() throws IOException {
        ...
        RestartFramework.at("after_block_transfer_initiation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        File metadataFile = cluster.getBlockMetadataFile(0, block);
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RandomAccessFile raFile = new RandomAccessFile(metadataFile, "rw");
        raFile.seek(2);
        RestartFramework.at("after_metadata_corruption").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        raFile.writeByte(0);
        raFile.writeInt(0);
        raFile.close();
