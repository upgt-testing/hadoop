...
public class TestStandbyCheckpoints {
    ...
    public void setupCluster() throws Exception {
        ...
        while (true) {
            try {
                ...
                cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(topology).numDataNodes(1).build();
                cluster.waitActive();
                setNNs();
                ...
                LOG.info("Set up MiniDockerDFSCluster failed due to port conflicts, retry " + retryCount + " times");
            }
        }
    }
    ...
    public void testSBNCheckpoints() throws Exception {
        JournalSetInterface standbyJournalSet = NameNodeAdapter.spyOnJournalSet(nns[1]);
        doEdits(0, 10);
    ...
    public void testCheckpointBeforeNameNodeInitializationIsComplete() throws Exception {
        ...
        // ahead of observer(nn2).
        nns[0].getRpcServer().rollEditLog();
        NameNodeInterface nn2 = nns[2];
        FSImageInterface nnFSImage = NameNodeAdapter.getAndSetFSImageInHttpServer(nn2, null);
        // After standby creating a checkpoint, it will try to push the image to
        // active and all observer, updating it's own txid to the most recent.
        HATestUtil.waitForCheckpoint(cluster, 1, ImmutableList.of(12));
        HATestUtil.waitForCheckpoint(cluster, 0, ImmutableList.of(12));
    ...
    public void testCheckpointWhenNoNewTransactionsHappened() throws Exception {
        ...
        FSImageInterface spyImage1 = NameNodeAdapter.spyOnFsImage(nns[1]);
    ...
    public void testCheckpointCancellation() throws Exception {
        ...
        File tmpDir = new File(MiniDockerDFSCluster.getBaseDirectory(), "testCheckpointCancellation-tmp");
        FSNamesystemInterface fsn = cluster.getNamesystem(0);
