...
public class TestSnapshotDeletion {
    ...
    protected FSNamesystemInterface fsn;

    protected FSDirectoryInterface fsdir;
    ...
    public void setUp() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(REPLICATION).format(true).build();
        cluster.waitActive();
    ...
    private void checkQuotaUsageComputation(final Path dirPath, final long expectedNs, final long expectedDs) throws IOException {
        INodeDirectoryInterface dirNode = getDir(fsdir, dirPath);
        assertTrue(dirNode.isQuotaSet());
        QuotaCountsInterface q = dirNode.getDirectoryWithQuotaFeature().getSpaceConsumed();
        ...
        QuotaCountsInterface counts = dirNode.computeQuotaUsage(fsdir.getBlockStoragePolicySuite(), false);
    ...
    public void testDeleteCurrentFileDirectory() throws Exception {
        ...
        final INodeFileInterface temp = TestSnapshotBlocksMap.assertBlockCollection(tempFile.toString(), 1, fsdir, blockmanager);
        ...
        SnapshotInterface snapshot0 = fsdir.getINode(dir.toString()).asDirectory().getSnapshot(DFSUtil.string2Bytes("s0"));
        SnapshotInterface snapshot1 = fsdir.getINode(dir.toString()).asDirectory().getSnapshot(DFSUtil.string2Bytes("s1"));
        ...
        INodeInterface noChangeFileSCopy = children.get(1);
        ...
        INodeFileInterface metaChangeFile2SCopy = children.get(0).asFile();
        ...
        final INodeFileInterface newFileNode = TestSnapshotBlocksMap.assertBlockCollection(newFile.toString(), 1, fsdir, blockmanager);
        ...
        // sub's snapshot copy
        INodeInterface snapshotNode4Subsub = snapshotNode4Sub.getChildrenList(Snapshot.CURRENT_STATE_ID).get(0);
        ...
        INodeDirectoryInterface snapshotSubsubDir = (INodeDirectory) snapshotNode4Subsub;
    ...
    public void testDeleteEarliestSnapshot2() throws Exception {
        ...
        final INodeFileInterface toDeleteFileNode = TestSnapshotBlocksMap.assertBlockCollection(toDeleteFile.toString(), 1, fsdir, blockmanager);
        ...
        final INodeDirectoryInterface dirNode = fsdir.getINode(dir.toString()).asDirectory();
        SnapshotInterface snapshot0 = dirNode.getSnapshot(DFSUtil.string2Bytes("s0"));
        assertNull(snapshot0);
        SnapshotInterface snapshot1 = dirNode.getSnapshot(DFSUtil.string2Bytes("s1"));
        DirectoryDiffListInterface diffList = dirNode.getDiffs();
        ...
        final INodeDirectoryInterface noChangeDirNode = (INodeDirectory) fsdir.getINode(noChangeDir.toString());
        assertEquals(INodeDirectory.class, noChangeDirNode.getClass());
        final INodeFileInterface noChangeFileNode = (INodeFile) fsdir.getINode(noChangeFile.toString());
    ...
    public void testDeleteSnapshot2() throws Exception {
        ...
        Path file2 = new Path(dir, "file2");
        DFSTestUtil.createFile(hdfs, file2, BLOCKSIZE, REPLICATION, seed);
        INodeFileInterface file2Node = fsdir.getINode(file2.toString()).asFile();
    ...
    public void testDeleteSnapshotWithPermissionsDisabled() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(newConf).numDataNodes(0).build();
        cluster.waitActive();
        hdfs = cluster.getFileSystem();
   ...
   */
    ...
    public void testHANNRestartAfterSnapshotDeletion() throws Exception {
        ...
        cluster.shutdown();
        conf = new Configuration();
        cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).build();
        cluster.transitionToActive(0);
        // stop the standby namenode
        NameNodeInterface snn = cluster.getNameNode(1);
        snn.stop();
        ...
        // roll the edit log
        NameNodeInterface ann = cluster.getNameNode(0);
   ...
   */
    ...
    public void testRenameAndDelete() throws IOException {
        ...
        INodeInterface p = fsdir.getInode(parentId);
        Assert.assertNotNull(p);
        INodeDirectoryInterface pd = p.asDirectory();
    ...
    public void testSnapshotDiffReportWithConcat() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            hdfs.concat(dest, files);
            hdfs.createSnapshot(st, "s" + i);
            SnapshotDiffReportInterface sdr = hdfs.getSnapshotDiffReport(st, "s" + i, "ss");
