...
public class TestBlockTokenWithDFS_RestartInjected_Randomized_456 {
    ...
    public void testAppend() throws Exception {
        ...
        try {
            ...
            stm.close();
            RestartFramework.at("after_initial_file_create").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // open the file again for append
            stm = fs.append(fileToAppend);
            RestartFramework.at("after_append_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
       ...
       */
            ...
            RestartFramework.at("after_token_expiry").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("before_close").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            stm.close();
    ...
    protected void doTestRead(Configuration conf, MiniDFSCluster cluster, boolean isStriped) throws Exception {
       ...
       */
        ...
        RestartFramework.at("after_token_refetch_from_namenode").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
       ...
       */
        ...
        RestartFramework.at("after_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // read should fail
        tryRead(conf, lblock, false);
        RestartFramework.at("after_token_validation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_datanode_restart_new_ports").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nn = cluster.getNameNode();
        RestartFramework.at("after_second_read").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        nnProto = cluster.getNameNode().getRpcServer();
        bm = cluster.getNamesystem().getBlockManager();
        sm = cluster.getNamesystem().getBlockManager().getBlockTokenSecretManager();
        RestartFramework.at("after_namenode_and_datanode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
       ...
       */
        ...
        RestartFramework.at("after_token_refetch_test").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
       ...
       */
        ...
        RestartFramework.at("after_third_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.shutdownNameNode(0);
        // verify blockSeekTo() still works (forced to use cached tokens)
        in1.seek(0);
        assertTrue(checkFile1(in1, expected));
        ...
        assertTrue(checkFile1(in2, expected));
        RestartFramework.at("after_datanode_restart_same_ports").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // verify fetchBlockByteRange() still works (forced to use cached tokens)
        assertTrue(checkFile2(in3, expected));
        RestartFramework.at("after_final_namenode_restart").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        /*
       ...
       */
        ...
        RestartFramework.at("after_namenode_shutdown_test").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
       ...
       */
        ...
        cluster.waitActive();
        assertEquals(numDataNodes, cluster.getDataNodes().size());
        RestartFramework.at("after_token_expiry").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nn = cluster.getNameNode();
        nnProto = cluster.getNameNode().getRpcServer();
        bm = cluster.getNamesystem().getBlockManager();
        sm = cluster.getNamesystem().getBlockManager().getBlockTokenSecretManager();
