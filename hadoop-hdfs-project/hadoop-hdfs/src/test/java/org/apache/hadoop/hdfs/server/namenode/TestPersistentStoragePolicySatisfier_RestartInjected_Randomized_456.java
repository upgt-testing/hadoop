 ...
 */
public class TestPersistentStoragePolicySatisfier_RestartInjected_Randomized_456 {
    ...
    private static StorageType[][] storageTypes = new StorageType[][] { { StorageType.DISK, StorageType.ARCHIVE, StorageType.SSD }, { StorageType.DISK, StorageType.ARCHIVE, StorageType.SSD }, { StorageType.DISK, StorageType.ARCHIVE, StorageType.SSD } };
    ...
    private void clusterSetUp(boolean isHAEnabled, Configuration newConf) throws Exception {
        ...
        conf.setInt(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MAX_RETRY_ATTEMPTS_KEY, 20);
    ...
    public void testWithCheckpoint() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_satisfy_policy_file").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_storage_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.setStoragePolicy(parentDir, COLD);
            fs.satisfyStoragePolicy(parentDir);
            RestartFramework.at("after_satisfy_policy_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWithRestarts() throws Exception {
        try {
            ...
            fs.satisfyStoragePolicy(testFile);
            RestartFramework.at("before_first_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            restartCluster();
            ...
            RestartFramework.at("after_first_verification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // test directory
            fs.setStoragePolicy(parentDir, COLD);
            fs.satisfyStoragePolicy(parentDir);
            RestartFramework.at("before_second_restart").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            restartCluster();
            DFSTestUtil.waitExpectedStorageType(parentFileName, StorageType.ARCHIVE, 3, timeout, fs);
            DFSTestUtil.waitExpectedStorageType(childFileName, StorageType.ARCHIVE, 3, timeout, fs);
        } finally {
    ...
    public void testMultipleSatisfyStoragePolicy() throws Exception {
        try {
            ...
            fs.setStoragePolicy(testFile, ONE_SSD);
            fs.satisfyStoragePolicy(testFile);
            RestartFramework.at("after_first_satisfy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DFSTestUtil.waitExpectedStorageType(testFileName, StorageType.SSD, 1, timeout, fs);
            DFSTestUtil.waitExpectedStorageType(testFileName, StorageType.DISK, 2, timeout, fs);
            ...
            RestartFramework.at("after_xattr_removed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.setStoragePolicy(testFile, COLD);
            fs.satisfyStoragePolicy(testFile);
            RestartFramework.at("after_second_satisfy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDropSPS() throws Exception {
        try {
            ...
            fs.setStoragePolicy(testFile, ONE_SSD);
            fs.satisfyStoragePolicy(testFile);
            RestartFramework.at("after_satisfy_policy").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.getNamesystem().getBlockManager().getSPSManager().changeModeEvent(StoragePolicySatisfierMode.NONE);
            RestartFramework.at("after_drop_sps").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Make sure satisfy xattr has been removed.
            DFSTestUtil.waitForXattrRemoved(testFileName, XATTR_SATISFY_STORAGE_POLICY, cluster.getNamesystem(), 30000);
        } finally {
            clusterShutdown();
    ...
    public void testNameNodeRestartWhenSPSCalledOnChildFileAndParentDir() throws Exception {
        try {
            ...
            RestartFramework.at("after_satisfy_child_file").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSPSOnChildAndParentDirectory() throws Exception {
        try {
            ...
            fs.setStoragePolicy(parentDir, "COLD");
            fs.satisfyStoragePolicy(childDir);
            RestartFramework.at("after_satisfy_child_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DFSTestUtil.waitExpectedStorageType(childFileName, StorageType.ARCHIVE, 3, 30000, cluster.getFileSystem());
            fs.satisfyStoragePolicy(parentDir);
            RestartFramework.at("after_satisfy_parent_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSPSxAttrWhenSpsCalledForFile() throws Exception {
        try {
            ...
            RestartFramework.at("after_file_created").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
