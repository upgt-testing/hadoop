...
public class TestDecommissioningStatus {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestDecommissioningStatus.class);

    final ArrayList<String> decommissionedNodes = new ArrayList<String>(numDatanodes);

    protected MiniDockerDFSCluster getCluster() {
        return cluster;
    }
   ...
   */
    ...
    public void testDecommissionStatus() throws Exception {
        ...
        final DatanodeManagerInterface dm = fsn.getBlockManager().getDatanodeManager();
    ...
    public void testDecommissionDeadDN() throws Exception {
        ...
        FSNamesystemInterface fsn = cluster.getNamesystem();
        final DatanodeManagerInterface dm = fsn.getBlockManager().getDatanodeManager();
        DatanodeDescriptorInterface dnDescriptor = dm.getDatanode(dnID);
        decommissionNode(dnName);
        dm.refreshNodes(conf);
        BlockManagerTestUtil.recheckDecommissionState(dm);
        // Block until the admin's monitor updates the number of tracked nodes.
    ...
    public void testDecommissionLosingData() throws Exception {
        ...
        DatanodeIDInterface dnID = cluster.getDataNodes().get(1).getDatanodeId();
        String dnName = dnID.getXferAddr();
        DatanodeDescriptorInterface dnDescriptor1 = dm.getDatanode(dnID);
        nodes.add(dnName);
        ...
        DatanodeDescriptorInterface dnDescriptor0 = dm.getDatanode(dnID);
        nodes.add(dnName);
