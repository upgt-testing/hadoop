...
public abstract class HATestUtil {
    ...
    public static void waitForStandbyToCatchUp(NameNodeInterface active, NameNodeInterface standby) throws InterruptedException, IOException, CouldNotCatchUpException {
        long activeTxId = active.getNamesystem().getFSImage().getEditLog().getLastWrittenTxId();
        active.getRpcServer().rollEditLog();
        long start = Time.now();
        while (Time.now() - start < TestEditLogTailer.NN_LAG_TIMEOUT) {
     ...
     */
    public static void waitForDNDeletions(final MiniDockerDFSCluster cluster) throws TimeoutException, InterruptedException {
     ...
     */
    public static DistributedFileSystem configureFailoverFs(MiniDockerDFSCluster cluster, Configuration conf) throws IOException, URISyntaxException {
     ...
     * @throws IOException if an error occurs rolling the edit log
     */
    public static DistributedFileSystem configureFailoverFs(MiniDockerDFSCluster cluster, Configuration conf, int nsIndex) throws IOException, URISyntaxException {
    ...
    public static <P extends ObserverReadProxyProvider<?>> DistributedFileSystem configureObserverReadFs(MiniDockerDFSCluster cluster, Configuration conf, Class<P> classFPP, boolean isObserverReadEnabled) throws IOException, URISyntaxException {
        String logicalName = conf.get(DFSConfigKeys.DFS_NAMESERVICES);
        URI nnUri = new URI(HdfsConstants.HDFS_URI_SCHEME + "://" + logicalName);
        conf.set(HdfsClientConfigKeys.Failover.PROXY_PROVIDER_KEY_PREFIX + "." + logicalName, classFPP.getName());
        conf.set("fs.defaultFS", nnUri.toString());
        DistributedFileSystem dfs = (DistributedFileSystem) FileSystem.get(nnUri, conf);
        @SuppressWarnings("unchecked")
        P provider = (P) ((RetryInvocationHandler<?>) Proxy.getInvocationHandler(dfs.getClient().getNamenode())).getProxyProvider();
        provider.setObserverReadEnabled(isObserverReadEnabled);
        return dfs;
    }

    public static boolean isSentToAnyOfNameNodes(DistributedFileSystem dfs, MiniDockerDFSCluster cluster, int... nnIndices) throws IOException {
    ...
    public static MiniQJMHACluster setUpObserverCluster(Configuration conf, int numObservers, int numDataNodes, boolean fastTailing) throws IOException {
        return setUpObserverCluster(conf, numObservers, numDataNodes, fastTailing, null, null);
    }
    ...
    public static MiniQJMHACluster setUpObserverCluster(Configuration conf, int numObservers, int numDataNodes, boolean fastTailing, long[] simulatedCapacities, String[] racks) throws IOException {
        ...
        if (fastTailing) {
            conf.setTimeDuration(DFS_HA_TAILEDITS_PERIOD_KEY, 100, TimeUnit.MILLISECONDS);
        } else {
    ...
    public static <P extends FailoverProxyProvider<?>> void setupHAConfiguration(MiniDockerDFSCluster cluster, Configuration conf, int nsIndex, Class<P> classFPP) {
        MiniDockerDFSCluster.NameNodeInfo[] nns = cluster.getNameNodeInfos(nsIndex);
        List<String> nnAddresses = new ArrayList<String>();
        for (MiniDockerDFSCluster.NameNodeInfo nn : nns) {
            InetSocketAddress addr = nn.nameNode.getNameNodeAddress();
            nnAddresses.add(createUri(HdfsConstants.HDFS_URI_SCHEME, addr).toString());
        }
        setFailoverConfigurations(conf, getLogicalHostname(cluster), nnAddresses, classFPP);
    }

    public static void setFailoverConfigurations(MiniDockerDFSCluster cluster, Configuration conf) {
     ...
     */
    public static void setFailoverConfigurations(MiniDockerDFSCluster cluster, Configuration conf, String logicalName) {
     ...
     */
    public static void setFailoverConfigurations(MiniDockerDFSCluster cluster, Configuration conf, String logicalName, String proxyProvider, int nsIndex) {
        MiniDockerDFSCluster.NameNodeInfo[] nns = cluster.getNameNodeInfos(nsIndex);
     ...
     * Sets the required configurations for performing failover.
     */
    public static <P extends FailoverProxyProvider<?>> void setFailoverConfigurations(MiniDockerDFSCluster cluster, Configuration conf, String logicalName, int nsIndex, Class<P> classFPP) {
        MiniDockerDFSCluster.NameNodeInfo[] nns = cluster.getNameNodeInfos(nsIndex);
        List<InetSocketAddress> nnAddresses = new ArrayList<InetSocketAddress>(3);
        for (MiniDockerDFSCluster.NameNodeInfo nn : nns) {
            nnAddresses.add(nn.nameNode.getNameNodeAddress());
    ...
    public static void setFailoverConfigurations(Configuration conf, String logicalName, String proxyProvider, InetSocketAddress... nnAddresses) {
     ...
     * Sets the required configurations for performing failover.
     */
    public static void setFailoverConfigurations(Configuration conf, String logicalName, String proxyProvider, List<InetSocketAddress> nnAddresses) {
    ...
    public static void setFailoverConfigurations(Configuration conf, String logicalName, String proxyProvider, Iterable<String> nnAddresses) {
        ...
        if (proxyProvider == null) {
            conf.set(Failover.PROXY_PROVIDER_KEY_PREFIX + "." + logicalName, ConfiguredFailoverProxyProvider.class.getName());
        } else {
            conf.set(Failover.PROXY_PROVIDER_KEY_PREFIX + "." + logicalName, proxyProvider);
        }
    ...
    public static String getLogicalHostname(MiniDockerDFSCluster cluster) {
        return String.format(LOGICAL_HOSTNAME, cluster.getInstanceId());
    }

    public static URI getLogicalUri(MiniDockerDFSCluster cluster) throws URISyntaxException {
        return new URI(HdfsConstants.HDFS_URI_SCHEME + "://" + getLogicalHostname(cluster));
    }

    public static void waitForCheckpoint(MiniDockerDFSCluster cluster, int nnIdx, List<Integer> txids) throws InterruptedException {
        long start = Time.now();
        while (true) {
            try {
     ...
     */
    public static long getLastSeenStateId(DistributedFileSystem dfs) throws Exception {
        ObserverReadProxyProvider<?> provider = (ObserverReadProxyProvider<?>) ((RetryInvocationHandler<?>) Proxy.getInvocationHandler(dfs.getClient().getNamenode())).getProxyProvider();
        ClientGSIContext ac = (ClientGSIContext) (provider.getAlignmentContext());
