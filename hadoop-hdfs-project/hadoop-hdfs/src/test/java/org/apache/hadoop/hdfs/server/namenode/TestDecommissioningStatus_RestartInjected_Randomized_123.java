 ...
 */
public class TestDecommissioningStatus_RestartInjected_Randomized_123 {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestDecommissioningStatus_RestartInjected.class);
    ...
    protected Configuration setupConfig() throws Exception {
        ...
        GenericTestUtils.setLogLevel(LoggerFactory.getLogger(DatanodeAdminManager.class), Level.DEBUG);
        return conf;
    }
   ...
   */
    ...
    protected void checkDecommissionStatus(DatanodeDescriptor decommNode, int expectedUnderRep, int expectedDecommissionOnly, int expectedUnderRepInOpenFiles) throws TimeoutException, InterruptedException {
        String errorMsg;
    ...
    public void testDecommissionStatus() throws Exception {
        ...
        RestartFramework.at("before_close_incomplete_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_incomplete_file_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FSDataOutputStream st1 = AdminStatesBaseTest.writeIncompleteFile(fileSys, file2, replicas, (short) (fileSize / blockSize));
        RestartFramework.at("after_verify_initial_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        for (int iteration = 0; iteration < numDatanodes; iteration++) {
            ...
            RestartFramework.at("after_decommission_refresh_nodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fsn = cluster.getNamesystem();
    ...
    public void testDecommissionStatusAfterDNRestart() throws Exception {
        ...
        RestartFramework.at("after_decommission_started").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        dm.refreshNodes(conf);
        RestartFramework.at("after_datanode_stopped").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fsn = cluster.getNamesystem();
    ...
    public void testDecommissionLosingData() throws Exception {
        RestartFramework.at("after_dn1_restarted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        DFSTestUtil.waitForDatanodeState(cluster, dnID.getDatanodeUuid(), false, 30000);
        fsn = cluster.getNamesystem();
        RestartFramework.at("after_dn1_stopped").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bm = cluster.getNamesystem().getBlockManager();
        dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        ...
        RestartFramework.at("after_dn0_restarted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        fsn = cluster.getNamesystem();
        RestartFramework.at("after_dn0_stopped").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bm = cluster.getNamesystem().getBlockManager();
        dm = cluster.getNamesystem().getBlockManager().getDatanodeManager();
        // Bring back dn1
        ...
        // Let replication monitor run
        Thread.sleep(2000);
        assertEquals(underreplicated, bm.getLowRedundancyBlocksCount());
        // Start up a node.
        LOG.info("Starting two more nodes");
        cluster.startDataNodes(conf, 2, true, null, null);
        RestartFramework.at("after_new_datanodes_started").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.waitActive();
