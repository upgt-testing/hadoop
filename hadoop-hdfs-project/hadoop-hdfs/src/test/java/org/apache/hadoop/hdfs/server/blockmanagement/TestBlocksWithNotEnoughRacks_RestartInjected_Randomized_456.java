...
public class TestBlocksWithNotEnoughRacks_RestartInjected_Randomized_456 {
   ...
   */
    ...
    public void testSufficientlyReplBlocksUsesNewRack() throws Exception {
        ...
        try {
            ...
            cluster.waitActive();
            RestartFramework.at("after_new_rack_added").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
   ...
   */
    ...
    public void testSufficientlySingleReplBlockUsesNewRack() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_replication_increase").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
            DFSTestUtil.waitForReplication(cluster, b, 2, REPLICATION_FACTOR, 0);
        } finally {
            cluster.shutdown();
        }
   ...
   */
    ...
    public void testMisReplicatedBlockUsesNewRack() throws Exception {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(racks.length).racks(racks).build()) {
            ...
            RestartFramework.at("after_datanode_stop").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
            // Create a file with one block with a replication factor of 3
            final FileSystem fs = cluster.getFileSystem();
   ...
   */
    ...
    public void testMultipleReplicasScheduledForUpgradeDomain() throws Exception {
        ...
        RestartFramework.at("after_cluster_up").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            // Create a file with one block with a replication factor of 3
            ...
            BlockReconstructionWork work = scheduleReconstruction(cluster.getNamesystem(), storedBlock, 2);
            assertNull(work);
