 ...
 */
public class TestGetContentSummaryWithPermission_RestartInjected_Randomized_123 {
    ...
    public void testGetContentSummarySuperUser() throws Exception {
        ...
        RestartFramework.at("after_third_permission_change").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        summary = cluster.getNameNodeRpc().getContentSummary(foo.toString());
        RestartFramework.at("after_second_permission_change").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_permission_change").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        verifySummary(summary, 2, 1, 10);
    }
    ...
    public void testGetContentSummaryNonSuperUser() throws Exception {
        ...
        dfs.mkdirs(bar);
        RestartFramework.at("after_first_summary").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(dfs, baz, 10, REPLICATION, 0L);
        ...
        RestartFramework.at("after_foo_permission_restore").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals((short) 755, fileStatus.getPermission().toOctal());
        ...
        RestartFramework.at("after_bar_permission_restore").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_permission_change").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
