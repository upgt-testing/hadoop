 ...
 */
public class TestScrLazyPersistFiles_RestartInjected_Randomized_123 extends LazyPersistTestCase {
    ...
    public void testRamDiskShortCircuitRead() throws IOException, InterruptedException, TimeoutException {
        ...
        RestartFramework.at("after_file_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            ...
            RestartFramework.at("after_scr_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void doShortCircuitReadAfterEvictionTest() throws IOException, InterruptedException, TimeoutException {
        final String METHOD_NAME = GenericTestUtils.getMethodName();
        Path path1 = new Path("/" + METHOD_NAME + ".01.dat");
        RestartFramework.at("after_default_storage_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final int SEED = 0xFADED;
        ...
        RestartFramework.at("after_lazy_persist").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_eviction").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        triggerEviction(cluster.getDataNodes().get(0));
        ensureFileReplicasOnStorageType(path1, DEFAULT);
        RestartFramework.at("after_cluster_build").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        metaFile = cluster.getBlockMetadataFile(0, DFSTestUtil.getFirstBlock(fs, path1));
        assertTrue(metaFile.length() > BlockMetadataHeader.getHeaderSize());
        RestartFramework.at("after_ramdisk_read").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(verifyReadRandomFile(path1, BLOCK_SIZE, SEED));
    }
    ...
    public void doShortCircuitReadMetaFileCorruptionTest() throws IOException, InterruptedException, TimeoutException {
        final String METHOD_NAME = GenericTestUtils.getMethodName();
        RestartFramework.at("after_meta_corruption").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path path1 = new Path("/" + METHOD_NAME + ".01.dat");
        makeTestFile(path1, BLOCK_SIZE, true);
        ensureFileReplicasOnStorageType(path1, RAM_DISK);
        waitForMetric("RamDiskBlocksLazyPersisted", 1);
