...
public class TestDatanodeRegistration {
    ...
    public void testDNSLookups() throws Exception {
        ...
        try {
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(8).build();
            cluster.waitActive();
    ...
    public void testForcedRegistration() throws Exception {
        ...
        try {
            ...
            DatanodeDescriptorInterface dnd = NameNodeAdapter.getDatanode(fsn, dn.getDatanodeId());
            DataNodeTestUtils.setHeartbeatsDisabledForTests(dn, true);
            DatanodeStorageInfoInterface storage = dnd.getStorageInfos()[0];
            // registration should not change after heartbeat.
            assertTrue(dnd.isRegistered());
            DatanodeRegistrationInterface lastReg = dn.getDNRegistrationForBP(bpId);
            ...
            waitForHeartbeat(dn, dnd);
            assertTrue(dnd.isRegistered());
            DatanodeRegistrationInterface newReg = dn.getDNRegistrationForBP(bpId);
            ...
            assertTrue(dnd.isRegistered());
            assertSame(lastReg, dn.getDNRegistrationForBP(bpId));
            assertTrue("block report is not processed for DN " + dnd, waitForBlockReport(dn, dnd));
            assertTrue(dnd.isRegistered());
            assertSame(lastReg, dn.getDNRegistrationForBP(bpId));
            // check that block report is not processed and registration didn't
            // change.
            dnd.setForceRegistration(true);
            assertFalse("block report is processed for DN " + dnd, waitForBlockReport(dn, dnd));
            assertFalse(dnd.isRegistered());
            assertSame(lastReg, dn.getDNRegistrationForBP(bpId));
    ...
    private void waitForHeartbeat(final DataNodeInterface dn, final DatanodeDescriptorInterface dnd) throws Exception {
    ...
    private boolean waitForBlockReport(final DataNodeInterface dn, final DatanodeDescriptorInterface dnd) throws Exception {
