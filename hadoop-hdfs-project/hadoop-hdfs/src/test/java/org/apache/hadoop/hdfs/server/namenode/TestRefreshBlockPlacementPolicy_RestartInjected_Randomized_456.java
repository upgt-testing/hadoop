 ...
 */
public class TestRefreshBlockPlacementPolicy_RestartInjected_Randomized_456 {
    ...
    private void verifyRefreshPolicy(DistributedFileSystem dfs, Path file, Refresh func) throws IOException, ReconfigurationException {
        ...
        RestartFramework.at("after_first_file_created").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Refresh to the default policy.
        func.refresh();
        RestartFramework.at("after_second_file_created").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        lastCounter = counter;
        RestartFramework.at("after_policy_refresh").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
