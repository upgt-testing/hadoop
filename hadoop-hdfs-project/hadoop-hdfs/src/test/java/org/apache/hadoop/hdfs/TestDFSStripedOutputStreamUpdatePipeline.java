...
public class TestDFSStripedOutputStreamUpdatePipeline {
    ...
    public void testDFSStripedOutputStreamUpdatePipeline() throws Exception {
        ...
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(5).build()) {
            cluster.waitActive();
    ...
    @Test
    public void testDFSStripedOutputStreamUpdatePipeline_withUpgrade20() throws Exception {
        Configuration conf = new HdfsConfiguration();
        conf.setLong(DFS_BLOCK_SIZE_KEY, 1 * 1024 * 1024);
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(5).build()) {
            cluster.waitActive();
    ...
    @Test
    public void testDFSStripedOutputStreamUpdatePipeline_withUpgrade40() throws Exception {
        Configuration conf = new HdfsConfiguration();
        conf.setLong(DFS_BLOCK_SIZE_KEY, 1 * 1024 * 1024);
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(5).build()) {
            cluster.waitActive();
    ...
    @Test(timeout = 90000)
    public void testECWriteHangWhenAddBlockWithException_withUpgrade20() throws Exception {
        Configuration conf = new HdfsConfiguration();
        conf.setLong(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, 1 * 1024 * 1024);
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build()) {
            cluster.waitActive();
    ...
    @Test(timeout = 90000)
    public void testECWriteHangWhenAddBlockWithException_withUpgrade40() throws Exception {
        Configuration conf = new HdfsConfiguration();
        conf.setLong(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, 1 * 1024 * 1024);
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build()) {
            cluster.waitActive();
    ...
    @Test(timeout = 90000)
    public void testECWriteHangWhenAddBlockWithException_withUpgrade60() throws Exception {
        Configuration conf = new HdfsConfiguration();
        conf.setLong(DFSConfigKeys.DFS_BLOCK_SIZE_KEY, 1 * 1024 * 1024);
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build()) {
            cluster.waitActive();
