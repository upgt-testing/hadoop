...
public class TestCorruptMetadataFile {
    ...
    @Test(timeout = 60000)
    public void testReadBlockFailsWhenMetaIsCorrupt_withUpgrade20() throws Exception {
        cluster = clusterBuilder.build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        DataNodeJVMInterface dn0 = cluster.getDataNodes().get(0);
    ...
    @Test(timeout = 60000)
    public void testReadBlockFailsWhenMetaIsCorrupt_withUpgrade40() throws Exception {
        cluster = clusterBuilder.build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        DataNodeJVMInterface dn0 = cluster.getDataNodes().get(0);
