...
public class TestCorruptMetadataFile {
    ...
    private MiniDockerDFSCluster.Builder clusterBuilder;
    ...
    public void setUp() throws Exception {
        ...
        clusterBuilder = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1);
    }
    ...
    public void testReadBlockFailsWhenMetaIsCorrupt() throws Exception {
        ...
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            ...
            @Override
            public Boolean get() {
                return cluster.getNameNode().getNamesystem().getBlockManager().getCorruptBlocks() == 1;
            }
        }, 100, 5000);
        raFile.close();
    }
    ...
    public void testBlockMetaDataHeaderPReadHandlesCorruptMetaFile() throws Exception {
        ...
        BlockMetadataHeader header = BlockMetadataHeader.preadHeader(raFile.getChannel());
        // Now truncate the meta file to zero and ensure an exception is raised
        raFile.setLength(0);
        LambdaTestUtils.intercept(CorruptMetaHeaderException.class, () -> BlockMetadataHeader.preadHeader(raFile.getChannel()));
