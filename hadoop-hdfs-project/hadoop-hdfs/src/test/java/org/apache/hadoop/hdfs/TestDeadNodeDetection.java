...
public class TestDeadNodeDetection {
    ...
    public void setUp() {
        ...
        conf.setLong(DFS_CLIENT_DEAD_NODE_DETECTION_PROBE_DEAD_NODE_INTERVAL_MS_KEY, 1000);
        conf.setLong(DFS_CLIENT_DEAD_NODE_DETECTION_PROBE_SUSPECT_NODE_INTERVAL_MS_KEY, 100);
        conf.setLong(DFS_CLIENT_DEAD_NODE_DETECTION_PROBE_CONNECTION_TIMEOUT_MS_KEY, 1000);
    ...
    public void testDeadNodeDetectionInBackground() throws Exception {
        ...
        try {
            ...
            assertEquals(3, dfsClient.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
        } finally {
            in.close();
            fs.delete(filePath, true);
            // check the dead node again here, the dead node is expected be removed
            assertEquals(0, dfsClient.getDeadNodes(din).size());
            assertEquals(0, dfsClient.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
        }
    }
    ...
    public void testDeadNodeDetectionInMultipleDFSInputStream() throws IOException {
        ...
        try {
            ...
            assertEquals(1, dfsClient1.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
            assertEquals(1, dfsClient2.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
            // check the dn uuid of dead node to see if its expected dead node
            assertEquals(datanodeUuid, ((DatanodeInfo) dfsClient1.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().toArray()[0]).getDatanodeUuid());
            assertEquals(datanodeUuid, ((DatanodeInfo) dfsClient2.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().toArray()[0]).getDatanodeUuid());
        } finally {
            in1.close();
            in2.close();
            ...
            assertEquals(0, dfsClient1.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
            assertEquals(0, dfsClient2.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
        }
    }
    ...
    public void testDeadNodeDetectionDeadNodeRecovery() throws Exception {
        ...
        try {
            ...
            assertEquals(2, dfsClient.getClientContext().getDeadNodeDetector().clearAndGetDetectedDeadNodes().size());
        } finally {
            in.close();
            deleteFile(fs, filePath);
            assertEquals(0, dfsClient.getDeadNodes(din).size());
    ...
    public void testDeadNodeDetectionDeadNodeProbe() throws Exception {
        ...
        try {
            ...
            DeadNodeDetector.UniqueQueue<DatanodeInfo> queue = deadNodeDetector.getSuspectNodesProbeQueue();
            DeadNodeDetector.UniqueQueue<DatanodeInfo> suspectSpy = Mockito.spy(queue);
            ...
            Thread.sleep(1500);
            Collection<DatanodeInfo> deadNodes = dfsClient.getDeadNodeDetector().clearAndGetDetectedDeadNodes();
    ...
    public void testDeadNodeDetectionSuspectNode() throws Exception {
        ...
        try {
            ...
            cluster.restartDataNode(one, true);
            Assert.assertEquals(1, deadNodeDetector.getSuspectNodesProbeQueue().size());
            Assert.assertEquals(0, deadNodeDetector.clearAndGetDetectedDeadNodes().size());
            deadNodeDetector.startProbeScheduler();
            Thread.sleep(1000);
            Assert.assertEquals(0, deadNodeDetector.getSuspectNodesProbeQueue().size());
            Assert.assertEquals(0, deadNodeDetector.clearAndGetDetectedDeadNodes().size());
        } finally {
            in.close();
            deleteFile(fs, filePath);
    ...
    public void testCloseDeadNodeDetector() throws Exception {
        DistributedFileSystem dfs0 = (DistributedFileSystem) FileSystem.newInstance(new URI("hdfs://127.0.0.1:2001/"), conf);
        DistributedFileSystem dfs1 = (DistributedFileSystem) FileSystem.newInstance(new URI("hdfs://127.0.0.1:2001/"), conf);
        ...
        dfs1 = (DistributedFileSystem) FileSystem.newInstance(new URI("hdfs://127.0.0.1:2001/"), conf);
    ...
    public void testDeadNodeDetectorThreadsShutdown() throws Exception {
        DistributedFileSystem dfs = (DistributedFileSystem) FileSystem.newInstance(new URI("hdfs://127.0.0.1:2001/"), conf);
    ...
    @Test
    public void testDeadNodeDetectionInBackground_withUpgrade20() throws Exception {
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionInBackground");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionInBackground_withUpgrade40() throws Exception {
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionInBackground");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionInBackground_withUpgrade60() throws Exception {
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionInBackground");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionInBackground_withUpgrade80() throws Exception {
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionInBackground");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionInMultipleDFSInputStream_withUpgrade20() throws IOException {
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        Path filePath = new Path("/testDeadNodeMultipleDFSInputStream");
    ...
    @Test
    public void testDeadNodeDetectionInMultipleDFSInputStream_withUpgrade40() throws IOException {
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        Path filePath = new Path("/testDeadNodeMultipleDFSInputStream");
    ...
    @Test
    public void testDeadNodeDetectionInMultipleDFSInputStream_withUpgrade60() throws IOException {
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        Path filePath = new Path("/testDeadNodeMultipleDFSInputStream");
    ...
    @Test
    public void testDeadNodeDetectionInMultipleDFSInputStream_withUpgrade80() throws IOException {
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        Path filePath = new Path("/testDeadNodeMultipleDFSInputStream");
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeRecovery_withUpgrade20() throws Exception {
        // prevent interrupt deadNodeDetectorThr in cluster.waitActive()
        DFSClient.setDisabledStopDeadNodeDetectorThreadForTest(true);
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionDeadNodeRecovery");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeRecovery_withUpgrade40() throws Exception {
        // prevent interrupt deadNodeDetectorThr in cluster.waitActive()
        DFSClient.setDisabledStopDeadNodeDetectorThreadForTest(true);
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionDeadNodeRecovery");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeRecovery_withUpgrade60() throws Exception {
        // prevent interrupt deadNodeDetectorThr in cluster.waitActive()
        DFSClient.setDisabledStopDeadNodeDetectorThreadForTest(true);
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionDeadNodeRecovery");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeRecovery_withUpgrade80() throws Exception {
        // prevent interrupt deadNodeDetectorThr in cluster.waitActive()
        DFSClient.setDisabledStopDeadNodeDetectorThreadForTest(true);
        conf.set(DFS_CLIENT_CONTEXT, "testDeadNodeDetectionDeadNodeRecovery");
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(3).build();
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeProbe_withUpgrade20() throws Exception {
        FileSystem fs = null;
        FSDataInputStream in = null;
        Path filePath = new Path("/" + GenericTestUtils.getMethodName());
        try {
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeProbe_withUpgrade40() throws Exception {
        FileSystem fs = null;
        FSDataInputStream in = null;
        Path filePath = new Path("/" + GenericTestUtils.getMethodName());
        try {
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeProbe_withUpgrade60() throws Exception {
        FileSystem fs = null;
        FSDataInputStream in = null;
        Path filePath = new Path("/" + GenericTestUtils.getMethodName());
        try {
    ...
    @Test
    public void testDeadNodeDetectionDeadNodeProbe_withUpgrade80() throws Exception {
        FileSystem fs = null;
        FSDataInputStream in = null;
        Path filePath = new Path("/" + GenericTestUtils.getMethodName());
        try {
    ...
    @Test
    public void testDeadNodeDetectionSuspectNode_withUpgrade20() throws Exception {
        DeadNodeDetector.setDisabledProbeThreadForTest(true);
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionSuspectNode_withUpgrade40() throws Exception {
        DeadNodeDetector.setDisabledProbeThreadForTest(true);
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionSuspectNode_withUpgrade60() throws Exception {
        DeadNodeDetector.setDisabledProbeThreadForTest(true);
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
    ...
    @Test
    public void testDeadNodeDetectionSuspectNode_withUpgrade80() throws Exception {
        DeadNodeDetector.setDisabledProbeThreadForTest(true);
        cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(1).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
