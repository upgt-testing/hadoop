 ...
 */
public class TestDiskBalancerRPC_RestartInjected_Randomized_123 {
    ...
    public void testSubmitPlanWithInvalidVersion() throws Exception {
        RestartFramework.at("after_version_modification").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RpcTestHelper rpcTestHelper = new RpcTestHelper().invoke();
        DataNode dataNode = rpcTestHelper.getDataNode();
        String planHash = rpcTestHelper.getPlanHash();
        int planVersion = rpcTestHelper.getPlanVersion();
    ...
    public void testGetDiskBalancerVolumeMapping() throws Exception {
        ...
        RestartFramework.at("after_get_volume_mapping").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testgetDiskBalancerBandwidth() throws Exception {
        ...
        NodePlan plan = rpcTestHelper.getPlan();
        dataNode.submitDiskBalancerPlan(planHash, planVersion, PLAN_FILE, plan.toJson(), false);
        RestartFramework.at("after_get_bandwidth").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testQueryPlan() throws Exception {
        ...
        NodePlan plan = rpcTestHelper.getPlan();
        dataNode.submitDiskBalancerPlan(planHash, planVersion, PLAN_FILE, plan.toJson(), false);
        RestartFramework.at("after_query_plan").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMoveBlockAcrossVolume() throws Exception {
        ...
        try {
            ...
            try {
                ...
                RestartFramework.at("after_volume_setup").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                DiskBalancerTestUtil.moveAllDataToDestVolume(dnNode.getFSDataset(), source, dest);
                RestartFramework.at("after_move_blocks").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
