...
public class TestStickyBit_RestartInjected_Randomized_123 {
    ...
    static final Logger LOG = LoggerFactory.getLogger(TestStickyBit_RestartInjected.class);
    ...
    private static void initCluster(boolean format) throws Exception {
        ...
        if (format) {
            RestartFramework.at("after_cluster_init").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testGeneralSBBehavior() throws Exception {
        ...
        RestartFramework.at("after_sticky_bit_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.mkdirs(baseDir);
    ...
    public void testAclGeneralSBBehavior() throws Exception {
        ...
        hdfs.mkdirs(p);
        hdfs.setPermission(p, new FsPermission((short) 01777));
        RestartFramework.at("after_acl_sticky_bit_set").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        applyAcl(p);
        confirmCanAppend(conf, p);
        baseDir = new Path("/eccleston");
        hdfs.mkdirs(baseDir);
    ...
    private void testMovingFiles(boolean useAcl) throws Exception {
        ...
        writeFile(hdfsAsUser1, file);
        RestartFramework.at("after_file_write_before_rename").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStickyBitPersistence() throws Exception {
        ...
        RestartFramework.at("before_shutdown_for_persistence_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_restart_for_persistence_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAclStickyBitPersistence() throws Exception {
        ...
        Path sbNotSpecified = new Path("/INXS");
        Path sbSetOff = new Path("/Easyworld");
        RestartFramework.at("after_restart_for_acl_persistence_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (Path p : new Path[] { sbSet, sbNotSpecified, sbSetOff }) hdfs.mkdirs(p);
        RestartFramework.at("before_shutdown_for_acl_persistence_test").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Two directories had there sticky bits set explicitly...
        hdfs.setPermission(sbSet, new FsPermission((short) 01777));
        applyAcl(sbSet);
        hdfs.setPermission(sbSetOff, new FsPermission((short) 00777));
    ...
    public void testStickyBitRecursiveDeleteFile() throws Exception {
        ...
        // Create a tmp directory with wide-open permissions and sticky bit
        hdfs.mkdirs(tmp);
        RestartFramework.at("after_file_creation_before_recursive_delete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStickyBitRecursiveDeleteDir() throws Exception {
        Path root = new Path("/" + GenericTestUtils.getMethodName());
        RestartFramework.at("after_dir_creation_before_recursive_delete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path tmp = new Path(root, "tmp");
