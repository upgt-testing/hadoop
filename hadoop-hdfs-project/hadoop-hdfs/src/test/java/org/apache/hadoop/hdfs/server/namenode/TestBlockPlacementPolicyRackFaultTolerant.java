...
public class TestBlockPlacementPolicyRackFaultTolerant {
    ...
    private FSNamesystemInterface namesystem = null;

    private PermissionStatusInterface perm = null;
    ...
    public void setup() throws IOException {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(hostList.size()).racks(rackList.toArray(new String[rackList.size()])).hosts(hostList.toArray(new String[hostList.size()])).build();
        cluster.waitActive();
    ...
    public void testPlacementWithOnlyOneNodeInRackDecommission() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(7).racks(racks).hosts(hosts).build();
        cluster.waitActive();
        nameNodeRpc = cluster.getNameNodeRpc();
        namesystem = cluster.getNamesystem();
        ...
        DatanodeDescriptorInterface dnd4 = dnm.getDatanode(cluster.getDataNodes().get(4).getDatanodeId());
        ...
        DatanodeDescriptorInterface dnd3 = dnm.getDatanode(cluster.getDataNodes().get(3).getDatanodeId());
    ...
    private void doTestLocatedBlock(int replication, LocatedBlock locatedBlock) {
        ...
        HashMap<String, Integer> racksCount = new HashMap<String, Integer>();
        for (DatanodeInfoInterface node : locatedBlock.getLocations()) {
