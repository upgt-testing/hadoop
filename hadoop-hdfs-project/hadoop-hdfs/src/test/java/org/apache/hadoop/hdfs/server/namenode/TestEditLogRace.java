...
public class TestEditLogRace {
    ...
    private static final String NAME_DIR = MiniDockerDFSCluster.getBaseDirectory() + "name-0-1";
    ...
    static class Transactions implements Runnable {
        ...
        Transactions(MiniDockerDFSCluster cluster, AtomicReference<Throwable> caught) {
    ...
    private void startTransactionWorkers(MiniDockerDFSCluster cluster, AtomicReference<Throwable> caughtErr) {
    ...
    public void testEditLogRolling() throws Exception {
        ...
        final MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).build();
        ...
        try {
            ...
            for (int i = 0; i < NUM_ROLLS && caughtErr.get() == null; i++) {
                ...
                CheckpointSignatureInterface sig = nn.rollEditLog();
    ...
    public void testSaveImageWhileSyncInProgress() throws Exception {
        ...
        final FSNamesystemInterface namesystem = FSNamesystem.loadFromDisk(conf);
        try {
            FSImageInterface fsimage = namesystem.getFSImage();
            FSEditLogInterface editLog = fsimage.getEditLog();
    ...
    static SetOwnerOp getSetOwnerOp(OpInstanceCache cache, String group) {
        return ((SetOwnerOp) cache.get(OP_SET_OWNER)).setSource("/").setUser("u").setGroup(group);
    }
    ...
    public void testDeadlock() throws Throwable {
        ...
        try {
            final FSEditLogInterface editLog = spy(namesystem.getEditLog());
            ...
            for (int i = 0; i < 8; i++) {
                ...
                Future<Void> blockingEdit = executor.submit(new Callable<Void>() {
                    ...
                    public Void call() throws Exception {
                        ...
                        final FSEditLogOpInterface blockingOp = getSetOwnerOp(cache, "b");
