 ...
 */
public class TestViewFileSystemLinkRegex_RestartInjected extends ViewFileSystemBaseTest {
  public static final Logger LOGGER =
      LoggerFactory.getLogger(TestViewFileSystemLinkRegex_RestartInjected.class);
  ...
  private static final File TEST_DIR = GenericTestUtils
      .getTestDir(TestViewFileSystemLinkRegex_RestartInjected.class.getSimpleName());
  ...
  private void testRegexMountpoint(
    ...
    RestartFramework.at("after_dir_creation")
        .on(cluster)
        .restart("namenode")
        .withIndex(0)
    ...
    try (FileSystem vfs = FileSystem.get(viewFsUri, config)) {
      ...
      RestartFramework.at("after_status_check")
          .on(cluster)
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
