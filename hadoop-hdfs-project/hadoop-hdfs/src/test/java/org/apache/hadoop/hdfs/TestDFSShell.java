...
public class TestDFSShell {
    ...
    private static MiniDockerDFSCluster miniCluster;
    ...
    public static void setup() throws IOException {
        ...
        conf.set(MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR, GenericTestUtils.getTestDir("TestDFSShell").getAbsolutePath());
        conf.setBoolean(DFSConfigKeys.DFS_NAMENODE_XATTRS_ENABLED_KEY, true);
        conf.setBoolean(DFSConfigKeys.DFS_NAMENODE_ACLS_ENABLED_KEY, true);
        conf.setLong(DFSConfigKeys.DFS_NAMENODE_ACCESSTIME_PRECISION_KEY, 1000);
        miniCluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).build();
    ...
    public void testMoveWithTargetPortEmpty() throws Exception {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).format(true).numDataNodes(2).nameNodePort(ServerSocketUtil.waitForPort(HdfsClientConfigKeys.DFS_NAMENODE_RPC_PORT_DEFAULT, 60)).waitSafeMode(true).build();
    ...
    public void testURIPaths() throws Exception {
        ...
        MiniDockerDFSCluster srcCluster = null;
        MiniDockerDFSCluster dstCluster = null;
        File bak = new File(PathUtils.getTestDir(getClass()), "testURIPaths");
        bak.mkdirs();
        try {
            srcCluster = new MiniDockerDFSCluster.Builder(srcConf).numDataNodes(2).build();
            dstConf.set(MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR, bak.getAbsolutePath());
            dstCluster = new MiniDockerDFSCluster.Builder(dstConf).numDataNodes(2).build();
    ...
    private static List<MaterializedReplica> getMaterializedReplicas(MiniDockerDFSCluster cluster) throws IOException {
    ...
    public void testGet() throws IOException {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).format(true).build();
            ...
            cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(2).format(false).build();
            dfs = cluster.getFileSystem();
   ...
   */
    ...
    public void testSetrepLow() throws Exception {
        ...
        MiniDockerDFSCluster cluster = builder.numDataNodes(2).format(true).build();
    ...
    private void deleteFileUsingTrash(boolean serverTrash, boolean clientTrash) throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(serverConf).numDataNodes(1).format(true).build();
