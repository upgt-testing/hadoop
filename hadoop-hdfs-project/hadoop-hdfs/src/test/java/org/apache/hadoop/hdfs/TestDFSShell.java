...
public class TestDFSShell {
    ...
    public void testURIPaths() throws Exception {
        ...
        try {
            ...
            argv[1] = "hdfs:///furi";
            ret = ToolRunner.run(shell, argv);
            dstCluster.restartNodeForTesting(0);
            dstCluster.upgradeNodeForTesting(0);
   ...
   */
    ...
    public void testAppendToFileWithOptionN() throws Exception {
        ...
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(6).build()) {
            ...
            assertTrue("Not a HDFS: " + hdfs.getUri(), hdfs instanceof DistributedFileSystem);
            ...
            FsShell shell = new FsShell();
            shell.setConf(conf);
            String[] argv = new String[] { "-appendToFile", "-n", file1.toString(), remoteFile.toString() };
            int res = ToolRunner.run(shell, argv);
            assertEquals("Run appendToFile command fail", 0, res);
            FileStatus fileStatus = hdfs.getFileStatus(remoteFile);
            assertEquals("File size should be " + inputFileLength, inputFileLength, fileStatus.getLen());
            ...
            assertEquals("File size should be " + inputFileLength * 2, inputFileLength * 2, fileStatus.getLen());
            ...
            ErasureCodingPolicy erasureCodingPolicy = ((DistributedFileSystem) hdfs).getErasureCodingPolicy(new Path(dir));
            ...
            argv = new String[] { "-appendToFile", "-n", file1.toString(), remoteFile.toString() };
            res = ToolRunner.run(shell, argv);
            assertEquals("Run appendToFile command fail", 0, res);
            fileStatus = hdfs.getFileStatus(remoteFile);
            assertEquals("File size should be " + inputFileLength, inputFileLength, fileStatus.getLen());
