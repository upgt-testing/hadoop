...
public class TestDFSShell {
    ...
    public static void setup() throws IOException {
        ...
        conf.set(MiniDFSClusterInJVM.HDFS_MINIDFS_BASEDIR, GenericTestUtils.getTestDir("TestDFSShell").getAbsolutePath());
    ...
    @Rule
    public Timeout // 30s
    globalTimeout = new Timeout(30 * 1000);
    ...
    public void testSetXAttrPermission() throws Exception {
        ...
        try {
            ...
            user.doAs(new PrivilegedExceptionAction<Object>() {
                ...
                @Override
                public Object run() throws Exception {
                    int ret = ToolRunner.run(fshell, new String[] { "-setfattr", "-n", "user.a1", "-v", "1234", p.toString() });
                    assertEquals("Returned should be 1", 1, ret);
                    String str = out.toString();
                    assertTrue("Permission denied printed", str.indexOf("Permission denied") != -1);
                    out.reset();
            ...
            user.doAs(new PrivilegedExceptionAction<Object>() {
                ...
                public Object run() throws Exception {
                    ...
                    out.reset();
                    // Remove
                    ret = ToolRunner.run(fshell, new String[] { "-setfattr", "-x", "user.a1", p.toString() });
                    assertEquals("Returned should be 1", 1, ret);
                    str = out.toString();
                    assertTrue("Permission denied printed", str.indexOf("Permission denied") != -1);
                    out.reset();
    ...
    public void testSetXAttrCaseSensitivity() throws Exception {
        ...
        try {
            ...
            final ByteArrayOutputStream out = new ByteArrayOutputStream();
            System.setOut(new PrintStream(out));
            doSetXattr(out, fshell, new String[] { "-setfattr", "-n", "User.Foo", p.toString() }, new String[] { "-getfattr", "-d", p.toString() }, new String[] { "user.Foo" }, new String[] {});
            doSetXattr(out, fshell, new String[] { "-setfattr", "-n", "user.FOO", p.toString() }, new String[] { "-getfattr", "-d", p.toString() }, new String[] { "user.Foo", "user.FOO" }, new String[] {});
            doSetXattr(out, fshell, new String[] { "-setfattr", "-n", "USER.foo", p.toString() }, new String[] { "-getfattr", "-d", p.toString() }, new String[] { "user.Foo", "user.FOO", "user.foo" }, new String[] {});
            doSetXattr(out, fshell, new String[] { "-setfattr", "-n", "USER.fOo", "-v", "myval", p.toString() }, new String[] { "-getfattr", "-d", p.toString() }, new String[] { "user.Foo", "user.FOO", "user.foo", "user.fOo=\"myval\"" }, new String[] { "user.Foo=", "user.FOO=", "user.foo=" });
            doSetXattr(out, fshell, new String[] { "-setfattr", "-x", "useR.foo", p.toString() }, new String[] { "-getfattr", "-d", p.toString() }, new String[] { "user.Foo", "user.FOO" }, new String[] { "foo" });
            doSetXattr(out, fshell, new String[] { "-setfattr", "-x", "USER.FOO", p.toString() }, new String[] { "-getfattr", "-d", p.toString() }, new String[] { "user.Foo" }, new String[] { "FOO" });
            doSetXattr(out, fshell, new String[] { "-setfattr", "-x", "useR.Foo", p.toString() }, new String[] { "-getfattr", "-n", "User.Foo", p.toString() }, new String[] {}, new String[] { "Foo" });
        } finally {
            if (bak != null) {
                System.setOut(bak);
            }
    ...
    public void testGetFAttrErrors() throws Exception {
        ...
        try {
            ...
            });
            {
                ...
                String str = out.toString();
                assertTrue("xattr value was incorrectly returned", str.indexOf("getfattr: " + XAttrNotFoundException.DEFAULT_EXCEPTION_MSG) >= 0);
                out.reset();
            }
        } finally {
            if (bakErr != null) {
