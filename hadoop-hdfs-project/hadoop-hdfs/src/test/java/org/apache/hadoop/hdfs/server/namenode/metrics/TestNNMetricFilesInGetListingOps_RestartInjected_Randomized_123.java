 ...
 */
public class TestNNMetricFilesInGetListingOps_RestartInjected_Randomized_123 {
    ...
    public void testFilesInGetListingOps() throws Exception {
        ...
        RestartFramework.at("after_first_listing").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_listing").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_tmp1_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        createFile("/tmp2/t1", 3200, (short) 3);
        createFile("/tmp2/t2", 3200, (short) 3);
        RestartFramework.at("after_tmp2_files").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
