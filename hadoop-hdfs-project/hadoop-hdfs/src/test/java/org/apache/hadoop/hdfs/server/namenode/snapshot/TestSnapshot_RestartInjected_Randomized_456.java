 ...
 */
public class TestSnapshot_RestartInjected_Randomized_456 {
    ...
    public void testSnapshot() throws Throwable {
        try {
            ...
            RestartFramework.at("after_snapshot_iterations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } catch (Throwable t) {
    ...
    public void testOfflineImageViewer() throws Exception {
        ...
        RestartFramework.at("after_fsimage_retrieval").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        PBImageXmlWriter v = new PBImageXmlWriter(new Configuration(), o);
        RestartFramework.at("after_snapshot_run").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testUpdateDirectory() throws Exception {
        ...
        RestartFramework.at("after_set_times").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAllowAndDisallowSnapshot() throws Exception {
        ...
        hdfs.disallowSnapshot(dir);
        dirNode = fsdir.getINode4Write(dir.toString()).asDirectory();
        RestartFramework.at("after_disallow_snapshot_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(dirNode.isSnapshottable());
        ...
        // do it again
        hdfs.disallowSnapshot(root);
        RestartFramework.at("after_allow_snapshot_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        rootNode = fsdir.getINode4Write(root.toString()).asDirectory();
        assertTrue(rootNode.isSnapshottable());
        assertEquals(0, rootNode.getDirectorySnapshottableFeature().getSnapshotQuota());
    }
    ...
    public void testRenameSnapshotMtime() throws Exception {
        ...
        RestartFramework.at("after_file_creation").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DFSTestUtil.createFile(hdfs, subFile, BLOCKSIZE, REPLICATION, seed);
        RestartFramework.at("after_snapshot_rename").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        hdfs.allowSnapshot(dir);
        Path snapshotPath = hdfs.createSnapshot(dir, "s1");
        RestartFramework.at("after_snapshot_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FileStatus oldSnapshotStatus = hdfs.getFileStatus(snapshotPath);
