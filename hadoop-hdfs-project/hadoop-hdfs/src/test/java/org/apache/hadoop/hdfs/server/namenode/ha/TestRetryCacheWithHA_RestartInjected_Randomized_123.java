...
public class TestRetryCacheWithHA_RestartInjected_Randomized_123 {
    ...
    public void testRetryCacheOnStandbyNN() throws Exception {
        ...
        RestartFramework.at("after_cache_check_nn1").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_failover").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // 2. Failover the current standby to active.
        cluster.getNameNode(0).getRpcServer().rollEditLog();
        RestartFramework.at("after_run_operations").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.getNameNode(1).getNamesystem().getEditLogTailer().doTailEdits();
        RestartFramework.at("after_cache_check_nn0").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    class CreateOp extends AtMostOnceOp {
        ...
        void invoke() throws Exception {
            ...
            this.status = client.getNamenode().create(fileName, FsPermission.getFileDefault(), client.getClientName(), new EnumSetWritable<CreateFlag>(createFlag), false, DataNodes, BlockSize, new CryptoProtocolVersion[] { CryptoProtocolVersion.ENCRYPTION_ZONES }, null, null);
        }
    ...
    public void testClientRetryWithFailover(final AtMostOnceOp op) throws Exception {
        ...
        RestartFramework.at("after_failover_transition").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // force the failover
        cluster.transitionToStandby(0);
        RestartFramework.at("after_namenode_check").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.transitionToActive(1);
        // disable the block in DummyHandler
        LOG.info("Setting block to false");
        DummyRetryInvocationHandler.block.set(false);
        RestartFramework.at("after_op_prepare").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        GenericTestUtils.waitFor(() -> !cluster.getNamesystem(1).isInStandbyState(), 5, 10000);
        final long[] hitsNN = new long[] { 0, 0 };
        GenericTestUtils.waitFor(() -> {
            hitsNN[0] = cluster.getNamesystem(0).getRetryCache().getMetricsForTests().getCacheHit();
            hitsNN[1] = cluster.getNamesystem(1).getRetryCache().getMetricsForTests().getCacheHit();
            return (hitsNN[0] + hitsNN[1]) > 0;
        }, 5, 10000);
        assertTrue("CacheHit: " + hitsNN[0] + ", " + hitsNN[1], +hitsNN[0] + hitsNN[1] > 0);
        final long[] updatesNN = new long[] { 0, 0 };
        GenericTestUtils.waitFor(() -> {
            updatesNN[0] = cluster.getNamesystem(0).getRetryCache().getMetricsForTests().getCacheUpdated();
            updatesNN[1] = cluster.getNamesystem(1).getRetryCache().getMetricsForTests().getCacheUpdated();
            return updatesNN[0] > 0 && updatesNN[1] > 0;
        }, 5, 10000);
        RestartFramework.at("after_cache_verification").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        if (expectedUpdateCount > 0) {
            assertEquals("CacheUpdated on NN0: " + updatesNN[0], expectedUpdateCount, updatesNN[0]);
            assertEquals("CacheUpdated on NN0: " + updatesNN[1], expectedUpdateCount, updatesNN[1]);
        }
    }
    ...
    public void testListCachePools() throws Exception {
        ...
        RestartFramework.at("after_failover_list_pools").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_add_cache_pools").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_list_pools_nn1").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        cluster.transitionToStandby(0);
        cluster.transitionToActive(1);
        cluster.waitActive(1);
        RestartFramework.at("after_list_pools_nn0").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListCacheDirectives() throws Exception {
        RestartFramework.at("after_list_directives_nn0").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final int poolCount = 7;
        HashSet<String> poolNames = new HashSet<String>(poolCount);
        Path path = new Path("/p");
        RestartFramework.at("after_add_cache_directives").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < poolCount; i++) {
        ...
        RestartFramework.at("after_list_directives_nn1").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        cluster.transitionToStandby(0);
        cluster.transitionToActive(1);
        cluster.waitActive(1);
        RestartFramework.at("after_failover_list_directives").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
