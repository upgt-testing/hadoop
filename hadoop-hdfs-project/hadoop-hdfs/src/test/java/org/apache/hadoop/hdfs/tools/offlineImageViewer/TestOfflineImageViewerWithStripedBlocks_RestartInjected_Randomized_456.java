...
public class TestOfflineImageViewerWithStripedBlocks_RestartInjected_Randomized_456 {
    ...
    private void testFileSize(int numBytes) throws IOException, UnresolvedLinkException, SnapshotAccessControlException {
        ...
        RestartFramework.at("after_striped_close").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String fileStatus = loader.getFileStatus("/eczone/striped");
        RestartFramework.at("after_striped_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        long expectedFileSize = bytes.length;
        // Verify space consumed present in BlockInfoStriped
        FSDirectory fsdir = cluster.getNamesystem().getFSDirectory();
        INodeFile fileNode = fsdir.getINode4Write(file.toString()).asFile();
        RestartFramework.at("after_striped_file_create").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
