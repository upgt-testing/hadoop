...
public class TestFcHdfsSetUMask_RestartInjected_Randomized_456 {

    private static final FileContextTestHelper fileContextTestHelper = new FileContextTestHelper("/tmp/TestFcHdfsSetUMask_RestartInjected");
    ...
    public void testMkdirWithExistingDirClear() throws IOException {
        RestartFramework.at("before_mkdir_existing_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        testMkdirWithExistingDir(BLANK_TEST_UMASK, BLANK_PERMISSIONS);
        RestartFramework.at("after_mkdir_existing_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testMkdirWithExistingDirOpen() throws IOException {
        ...
        RestartFramework.at("before_mkdir_existing_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_existing_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void testMkdirWithExistingDirMiddle() throws IOException {
        RestartFramework.at("before_mkdir_existing_middle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_existing_middle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirRecursiveWithNonExistingDirClear() throws IOException {
        RestartFramework.at("before_mkdir_recursive_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_recursive_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirRecursiveWithNonExistingDirOpen() throws IOException {
        RestartFramework.at("before_mkdir_recursive_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_recursive_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMkdirRecursiveWithNonExistingDirMiddle() throws IOException {
        RestartFramework.at("after_mkdir_recursive_middle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_mkdir_recursive_middle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreateRecursiveWithExistingDirClear() throws IOException {
        RestartFramework.at("after_create_existing_clear").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_create_existing_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreateRecursiveWithExistingDirOpen() throws IOException {
        RestartFramework.at("after_create_existing_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_create_existing_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreateRecursiveWithExistingDirMiddle() throws IOException {
        RestartFramework.at("before_create_existing_middle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_existing_middle").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreateRecursiveWithNonExistingDirClear() throws IOException {
        RestartFramework.at("after_create_nonexisting_clear").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_create_nonexisting_clear").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreateRecursiveWithNonExistingDirOpen() throws IOException {
        RestartFramework.at("before_create_nonexisting_open").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        testCreateRecursiveWithNonExistingDir(WIDE_OPEN_TEST_UMASK, WIDE_OPEN_PERMISSIONS, WIDE_OPEN_FILE_PERMISSIONS);
        RestartFramework.at("after_create_nonexisting_open").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }

    @Test
    public void testCreateRecursiveWithNonExistingDirMiddle() throws IOException {
        RestartFramework.at("after_create_nonexisting_middle").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_create_nonexisting_middle").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCreateRecursiveWithExistingDir(FsPermission umask, FsPermission expectedPerms) throws IOException {
        ...
        Assert.assertEquals("permissions on file are wrong", expectedPerms, fc.getFileStatus(f).getPermission());
    }
