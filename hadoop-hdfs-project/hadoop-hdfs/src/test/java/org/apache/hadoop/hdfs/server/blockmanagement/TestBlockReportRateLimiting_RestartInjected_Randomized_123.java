...
public class TestBlockReportRateLimiting_RestartInjected_Randomized_123 {

    static final Logger LOG = LoggerFactory.getLogger(TestBlockReportRateLimiting_RestartInjected.class);
    ...
    public void testRateLimitingDuringDataNodeStartup() throws Exception {
        ...
        RestartFramework.at("after_all_reports_complete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.CRASH).execute();
        for (int n = 1; n <= NUM_DATANODES; n++) {
            LOG.info("Waiting for " + n + " datanode(s) to report in.");
            fbrSem.release();
            if (n == 1) {
                RestartFramework.at("after_first_lease_grant").on(cluster).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
            }
    ...
    public void testLeaseExpiration() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_cluster_active").on(cluster.get()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assert.assertNotNull(cluster.get().stopDataNode(datanodeToStop.get()));
            RestartFramework.at("after_datanode_stop").on(cluster.get()).restart("namenode").withMode(RestartMode.GRACEFUL).execute();
            gotFbrSem.acquire();
            RestartFramework.at("after_lease_expiration").on(cluster.get()).restart("namenode").withMode(RestartMode.CRASH).execute();
