 ...
 */
public class TestLargeDirectoryDelete_RestartInjected_Randomized_456 {

    private static final Logger LOG = LoggerFactory.getLogger(TestLargeDirectoryDelete_RestartInjected.class);
    ...
    private void runThreads() throws Throwable {
        ...
        BlockManagerTestUtil.waitForMarkedDeleteQueueIsEmpty(mc.getNamesystem(0).getBlockManager());
    ...
    public void largeDelete() throws Throwable {
        ...
        try {
            mc.waitActive();
            Assert.assertNotNull("No Namenode in cluster", mc.getNameNode());
            RestartFramework.at("after_cluster_start").on(mc).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            createFiles();
            RestartFramework.at("after_file_creation").on(mc).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assert.assertEquals(TOTAL_BLOCKS, getBlockCount());
            RestartFramework.at("before_large_delete").on(mc).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            runThreads();
            RestartFramework.at("after_large_delete").on(mc).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            mc.shutdown();
        }
    }
