 ...
 * and messages.
 */
public abstract class BlockReportTestBase_RestartInjected_Randomized_456 {

    public static final Logger LOG = LoggerFactory.getLogger(BlockReportTestBase_RestartInjected.class);
    ...
    public void blockReport_01() throws IOException {
        RestartFramework.at("after_block_report_sent").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_prepare").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void blockReport_07() throws Exception {
        ...
        writeFile(METHOD_NAME, FILE_SIZE, filePath);
        RestartFramework.at("after_first_block_report_sent").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        startDNandWait(filePath, true);
        RestartFramework.at("after_second_block_report_sent").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // all blocks belong to the same file, hence same BP
        DataNode dn = cluster.getDataNodes().get(DN_N1);
        RestartFramework.at("after_file_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String poolId = cluster.getNamesystem().getBlockPoolId();
    ...
    public void testOneReplicaRbwReportArrivesAfterBlockCompleted() throws Exception {
        ...
        final String METHOD_NAME = GenericTestUtils.getMethodName();
        Path filePath = new Path("/" + METHOD_NAME + ".dat");
        RestartFramework.at("after_block_report_processed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testInterleavedBlockReports() throws IOException, ExecutionException, InterruptedException {
        RestartFramework.at("after_get_block_reports").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
