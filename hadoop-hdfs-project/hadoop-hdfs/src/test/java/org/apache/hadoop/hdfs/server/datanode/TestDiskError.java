...
public class TestDiskError {
    ...
    public void testShutdown() throws Exception {
        ...
        File dir1 = MiniDockerDFSCluster.getRbwDir(storageDir, bpid);
        storageDir = cluster.getInstanceStorageDir(dnIndex, 1);
        File dir2 = MiniDockerDFSCluster.getRbwDir(storageDir, bpid);
        try {
    ...
    public void testReplicationError() throws Exception {
        ...
        cluster.waitActive();
        final int sndNode = 1;
        DataNodeInterface datanode = cluster.getDataNodes().get(sndNode);
        ...
        for (DataNodeInterface dn : cluster.getDataNodes()) {
            try (FsDatasetSpi.FsVolumeReferences volumes = dn.getFSDataset().getFsVolumeReferences()) {
                for (FsVolumeSpi vol : volumes) {
                    ...
                    FsPermissionInterface actual = localFS.getFileStatus(dataDir).getPermission();
    ...
    public void testDataTransferWhenBytesPerChecksumIsZero() throws IOException {
        ...
        BlockScannerInterface mockScanner = Mockito.mock(BlockScanner.class);
        ...
        DatanodeDescriptorInterface dnd1 = NameNodeAdapter.getDatanode(cluster.getNamesystem(), dn1.getDatanodeId());
