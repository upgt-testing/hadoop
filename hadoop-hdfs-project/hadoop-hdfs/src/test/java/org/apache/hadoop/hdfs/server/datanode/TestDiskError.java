...
public class TestDiskError {
   ...
   */
    ...
    @Test
    public void testLocalDirs_withUpgrade60() throws Exception {
        Configuration conf = new Configuration();
        final String permStr = conf.get(DFSConfigKeys.DFS_DATANODE_DATA_DIR_PERMISSION_KEY);
        FsPermission expected = new FsPermission(permStr);
        // Check permissions on directories in 'dfs.datanode.data.dir'
    ...
    @Test(timeout = 60000)
    public void testcheckDiskError_withUpgrade20() throws Exception {
        if (cluster.getDataNodes().size() <= 0) {
            cluster.startDataNodes(conf, 1, true, null, null);
            cluster.restartNodeForTesting(0);
            cluster.upgradeNodeForTesting(0);
    ...
    @Test(timeout = 60000)
    public void testcheckDiskError_withUpgrade40() throws Exception {
        if (cluster.getDataNodes().size() <= 0) {
            cluster.startDataNodes(conf, 1, true, null, null);
            cluster.waitActive();
        }
    ...
    @Test(timeout = 60000)
    public void testcheckDiskError_withUpgrade60() throws Exception {
        if (cluster.getDataNodes().size() <= 0) {
            cluster.startDataNodes(conf, 1, true, null, null);
            cluster.waitActive();
        }
