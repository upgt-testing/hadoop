...
public class TestDefaultBlockPlacementPolicy_RestartInjected_Randomized_123 {
    ...
    public void testNonLocalRackPlacement() throws Exception {
        ...
        assertNotEquals("/RACK0", locatedBlock.getLocations()[2].getNetworkLocation());
    }
    ...
    public void testPlacementWithLocalRackNodesDecommissioned() throws Exception {
        ...
        RestartFramework.at("after_decommission_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testPlacement(String clientMachine, String clientRack, boolean hasBlockReplicaOnRack) throws IOException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            // Create the file with client machine
            HdfsFileStatus fileStatus = namesystem.startFile(src, perm, clientMachine, clientMachine, EnumSet.of(CreateFlag.CREATE), true, REPLICATION_FACTOR, DEFAULT_BLOCK_SIZE, null, null, null, false);
            RestartFramework.at("after_start_file_in_loop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LocatedBlock locatedBlock = nameNodeRpc.addBlock(src, clientMachine, null, null, fileStatus.getFileId(), null, null);
            RestartFramework.at("after_add_block_in_loop").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals("Block should be allocated sufficient locations", REPLICATION_FACTOR, locatedBlock.getLocations().length);
            if (clientRack != null) {
