...
public class TestReplicationPolicy_RestartInjected_Randomized_123 extends BaseReplicationPolicyTest {
    ...
    public TestReplicationPolicy_RestartInjected_Randomized_123(String blockPlacementPolicyClassName) {
    ...
    public void testChooseTargetWithMoreThanHalfStaleNodes() throws Exception {
        ...
        RestartFramework.at("after_cluster_start").on(miniCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            ...
            RestartFramework.at("after_first_heartbeat_check").on(miniCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_choose_target").on(miniCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getHeartbeatManager().heartbeatCheck();
            RestartFramework.at("after_second_heartbeat_check").on(miniCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_choose_target").on(miniCluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            // Explicitly call heartbeatCheck
            miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getHeartbeatManager().heartbeatCheck();
            RestartFramework.at("after_third_heartbeat_check").on(miniCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            numStaleNodes = miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getNumStaleNodes();
            assertEquals(numStaleNodes, 2);
            assertTrue(miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().shouldAvoidStaleDataNodesForWrite());
            RestartFramework.at("before_final_choose_target").on(miniCluster).restart("datanode").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            // Call chooseTarget
    ...
    public void testReplicationWithPriority() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_add_normal_priority_blocks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Lets wait for the replication interval, to start process normal
            // priority blocks
            Thread.sleep(DFS_NAMENODE_REPLICATION_INTERVAL);
            RestartFramework.at("after_first_sleep").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Adding the block directly to high priority list
            neededReconstruction.add(genBlockInfo(ThreadLocalRandom.current().nextLong(), true), 1, 0, 0, 3);
            RestartFramework.at("after_add_high_priority_block").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Lets wait for the replication interval
            Thread.sleep(DFS_NAMENODE_REPLICATION_INTERVAL);
            RestartFramework.at("after_second_sleep").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
