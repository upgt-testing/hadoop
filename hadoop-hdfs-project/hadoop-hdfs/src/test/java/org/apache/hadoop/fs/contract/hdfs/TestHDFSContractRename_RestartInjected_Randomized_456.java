...
public class TestHDFSContractRename_RestartInjected_Randomized_456 extends AbstractContractRenameTest {
    ...
    public void testRenameFile() throws Throwable {
        ...
        RestartFramework.at("after_rename").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_rename").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRenameFileOverExistingFile() throws Throwable {
        ...
        RestartFramework.at("before_create_src_dst").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FileSystem fs = getFileSystem();
        Path src = path("testRenameFileOverExistingFile/src");
        Path dst = path("testRenameFileOverExistingFile/dst");
        RestartFramework.at("after_rename_over_existing").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_both_files").on(HDFSContract.getCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        boolean renamed = fs.rename(src, dst);
        assertPathDoesNotExist("source still exists", src);
        assertPathExists("destination does not exist", dst);
    }
    ...
    public void testRenameDirectory() throws Throwable {
        ...
        RestartFramework.at("before_mkdir").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir_before_rename").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.mkdirs(src);
        assertPathExists("source directory not created", src);
        RestartFramework.at("after_rename_directory").on(HDFSContract.getCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        boolean renamed = fs.rename(src, dst);
        assertTrue("rename failed", renamed);
        assertPathDoesNotExist("source still exists", src);
        assertPathExists("destination does not exist", dst);
