...
public class TestDFSAdmin {
    ...
    public void setUp() throws Exception {
        ...
        conf.set(MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR, GenericTestUtils.getRandomizedTempPath());
        restartCluster();
    ...
    public void testGetDatanodeInfo() throws Exception {
        ...
        for (int i = 0; i < cluster.getDataNodes().size(); i++) {
            resetStream();
            final DataNodeInterface dn = cluster.getDataNodes().get(i);
    ...
    public void testPrintTopology() throws Exception {
        ...
        dfsConf.set(MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR, baseDir.getAbsolutePath());
        final int numDn = 4;
        final String[] racks = { "/d1/r1", "/d1/r2", "/d2/r1", "/d2/r2" };
        /* init cluster using topology */
        try (MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(dfsConf).numDataNodes(numDn).racks(racks).build()) {
    ...
    public void testPrintTopologyWithStatus() throws Exception {
        ...
        try (MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(dfsConf).numDataNodes(numDn).racks(racks).build()) {
            miniCluster.waitActive();
            assertEquals(numDn, miniCluster.getDataNodes().size());
            DatanodeManagerInterface dm = miniCluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager();
            DatanodeDescriptorInterface maintenanceNode = dm.getDatanode(miniCluster.getDataNodes().get(1).getDatanodeId());
            maintenanceNode.setInMaintenance();
            DatanodeDescriptorInterface demissionNode = dm.getDatanode(miniCluster.getDataNodes().get(2).getDatanodeId());
    ...
    private void waitForCorruptBlock(MiniDockerDFSCluster miniCluster, DFSClient client, Path file) throws TimeoutException, InterruptedException {
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
    ...
    public void testReportCommand() throws Exception {
        ...
        dfsConf.set(MiniDockerDFSCluster.HDFS_MINIDFS_BASEDIR, baseDir.toString());
        final int numDn = ecPolicy.getNumDataUnits() + ecPolicy.getNumParityUnits();
        try (MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(dfsConf).numDataNodes(numDn).build()) {
            miniCluster.waitActive();
            assertEquals(numDn, miniCluster.getDataNodes().size());
            final DFSAdmin dfsAdmin = new DFSAdmin(dfsConf);
            ...
            DataNodeInterface dataNodeToShutdown = null;
    ...
    public void testListOpenFiles() throws Exception {
        ...
        try (MiniDockerDFSCluster miniCluster = new MiniDockerDFSCluster.Builder(dfsConf).numDataNodes(numDataNodes).build()) {
            final short replFactor = 1;
            final long fileLength = 512L;
    ...
    public void testCheckNumOfBlocksInReportCommand() throws Exception {
        ...
        for (DatanodeInfoInterface d : dn) {
    ...
    public void testAllDatanodesReconfig() throws IOException, InterruptedException, TimeoutException {
        ReconfigurationUtilInterface reconfigurationUtil = mock(ReconfigurationUtil.class);
