...
public class TestDFSStorageStateRecovery {
    ...
    private MiniDockerDFSCluster createCluster(Configuration c) throws IOException {
        return new MiniDockerDFSCluster.Builder(c).numDataNodes(0).startupOption(StartupOption.REGULAR).format(false).manageDataDfsDirs(false).manageNameDfsDirs(false).build();
    }
