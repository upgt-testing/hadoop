...
public class TestQuotasWithHA {
    ...
    public void setupCluster() throws Exception {
        ...
        cluster = new MiniDFSClusterInJVM.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).waitSafeMode(false).build();
        cluster.waitActive();
    ...
    @Test(expected = StandbyException.class)
    public void testGetContentSummaryOnStandby_withUpgrade20() throws Exception {
        Configuration nn1conf = cluster.getConfiguration(1);
        // just reset the standby reads to default i.e False on standby.
        HAUtil.setAllowStandbyReads(nn1conf, false);
    ...
    @Test(expected = StandbyException.class)
    public void testGetContentSummaryOnStandby_withUpgrade40() throws Exception {
        Configuration nn1conf = cluster.getConfiguration(1);
        // just reset the standby reads to default i.e False on standby.
        HAUtil.setAllowStandbyReads(nn1conf, false);
        cluster.restartNameNode(1);
    ...
    @Test(expected = StandbyException.class)
    public void testGetContentSummaryOnStandby_withUpgrade80() throws Exception {
        Configuration nn1conf = cluster.getConfiguration(1);
        // just reset the standby reads to default i.e False on standby.
        HAUtil.setAllowStandbyReads(nn1conf, false);
        cluster.restartNameNode(1);
    ...
    @Test(expected = StandbyException.class)
    public void testGetQuotaUsageOnStandby_withUpgrade20() throws Exception {
        Configuration nn1conf = cluster.getConfiguration(1);
        // just reset the standby reads to default i.e False on standby.
        HAUtil.setAllowStandbyReads(nn1conf, false);
        cluster.restartNodeForTesting(0);
    ...
    @Test(expected = StandbyException.class)
    public void testGetQuotaUsageOnStandby_withUpgrade60() throws Exception {
        Configuration nn1conf = cluster.getConfiguration(1);
        // just reset the standby reads to default i.e False on standby.
        HAUtil.setAllowStandbyReads(nn1conf, false);
        cluster.restartNameNode(1);
