...
/**
 * ProcessBasedMiniDFSCluster version of {@link TestDFSPermission}.
 *
 * Transformed from MiniDFSCluster to ProcessBasedMiniDFSCluster to enable
 * process-based testing and multi-version upgrade scenarios.
 *
 * @see TestDFSPermission Original test using MiniDFSCluster
 */
@RunWith(Parameterized.class)
public class TestDFSPermission_ProcessBased extends ProcessBasedUpgradeTestBase {
  public static final Logger LOG =
      LoggerFactory.getLogger(TestDFSPermission_ProcessBased.class);
  ...
  static {
    try {
      ...
      // Reduce test iterations for parameterized tests (multiple checkpoints)
      Configuration staticConf = new HdfsConfiguration();
      NUM_TEST_PERMISSIONS = staticConf.getInt("test.dfs.permission.num", 10) *
          (MAX_PERMISSION + 1) / 100;
  ...
  private void setupCluster() throws Exception {
    ...
    // Apply fake group mapping to configuration
    Map<String, String[]> u2g_map = new HashMap<String, String[]> (3);
    u2g_map.put(USER1_NAME, new String[] {GROUP1_NAME, GROUP2_NAME });
    u2g_map.put(USER2_NAME, new String[] {GROUP2_NAME, GROUP3_NAME });
    u2g_map.put(USER3_NAME, new String[] {GROUP3_NAME, GROUP4_NAME });
   ...
   */
  @Test
  public void testImmutableFsPermission() throws Exception {
    setupCluster();
    checkpoint(UpgradeCheckpoints.AFTER_CLUSTER_START);
  ...
  public void testTrashPermission() throws Exception {
    ...
    try {
      ...
      DistributedFileSystem localFs = (DistributedFileSystem) FileSystem.get(conf);

      localFs.mkdirs(rootDir);
      localFs.setPermission(rootDir, new FsPermission((short) 0777));

      checkpoint(UpgradeCheckpoints.AFTER_FILE_CREATE);

      localFs = (DistributedFileSystem) DFSTestUtil.login(localFs, conf, USER1);
      localFs.mkdirs(user1Dir);
      localFs.setPermission(user1Dir, new FsPermission((short) 0755));
      localFs.setOwner(user1Dir, USER1.getShortUserName(), GROUP2_NAME);

      create(localFs, conf, OpType.CREATE, user1File);
      localFs.setOwner(user1File, USER1.getShortUserName(), GROUP1_NAME);
      localFs.setPermission(user1File, new FsPermission((short) 0600));

      try {
        // login as user2, attempt to delete /BSS/user1
        // this should fail because user2 has no permission to
        // its sub directory.
        localFs = (DistributedFileSystem) DFSTestUtil.login(localFs, conf, USER2);
        localFs.delete(user1Dir, true);
      ...
      assertTrue(localFs.exists(user1Dir));

      checkpoint(UpgradeCheckpoints.BEFORE_VERIFICATION);

      try {
        localFs = (DistributedFileSystem) DFSTestUtil.login(localFs, conf, SUPERUSER);
        Trash trash = new Trash(localFs, conf);
        ...
        localFs.mkdirs(trashRoot);
        localFs.setPermission(trashRoot, new FsPermission((short) 0777));
        ...
        localFs = (DistributedFileSystem) DFSTestUtil.login(localFs, conf, USER2);
        Trash userTrash = new Trash(localFs, conf);
      ...
      assertEquals(localFs.exists(user1Dir), true);
    } finally {
      DistributedFileSystem cleanupFs = (DistributedFileSystem) DFSTestUtil.login(fs, conf, SUPERUSER);
      cleanupFs.delete(rootDir, true);
   ...
   */
  private void setOwner(FileSystem localFs, Path path, String owner, String group,
      boolean expectDeny) throws IOException {
    try {
      String expectedOwner = (owner == null) ? getOwner(localFs, path) : owner;
      String expectedGroup = (group == null) ? getGroup(localFs, path) : group;
      localFs.setOwner(path, owner, group);
      checkOwnership(localFs, path, expectedOwner, expectedGroup);
  ...
  private void testOwnership(OpType op) throws Exception {
    ...
    DistributedFileSystem localFs = (DistributedFileSystem) FileSystem.get(conf);
    create(localFs, conf, op, FILE_DIR_PATH, DEFAULT_UMASK,
        new FsPermission(DEFAULT_PERMISSION));
    checkOwnership(localFs, FILE_DIR_PATH, SUPERUSER.getShortUserName(),
        getGroup(localFs, FILE_DIR_PATH.getParent()));

    checkpoint(UpgradeCheckpoints.AFTER_FILE_CREATE);

    // case 2: superuser changes FILE_DIR_PATH's owner to be <user1, group3>
    setOwner(localFs, FILE_DIR_PATH, USER1.getShortUserName(), GROUP3_NAME, false);

    // case 3: user1 changes FILE_DIR_PATH's owner to be user2
    localFs = (DistributedFileSystem) DFSTestUtil.login(localFs, conf, USER1);
    setOwner(localFs, FILE_DIR_PATH, USER2.getShortUserName(), null, true);

    // case 4: user1 changes FILE_DIR_PATH's group to be group1 which it belongs
    // to
    setOwner(localFs, FILE_DIR_PATH, null, GROUP1_NAME, false);

    // case 5: user1 changes FILE_DIR_PATH's group to be group3
    // which it does not belong to
    setOwner(localFs, FILE_DIR_PATH, null, GROUP3_NAME, true);
    ...
    localFs = (DistributedFileSystem) DFSTestUtil.login(localFs, conf, SUPERUSER);
    localFs.delete(FILE_DIR_PATH, true);
  }

  /* Return the group owner of the file/directory */
  private String getGroup(FileSystem localFs, Path path) throws IOException {
    return localFs.getFileStatus(path).getGroup();
  }

  /* Return the file owner of the file/directory */
  private String getOwner(FileSystem localFs, Path path) throws IOException {
    return localFs.getFileStatus(path).getOwner();
  }

  /* check if ownership is set correctly */
  private void checkOwnership(FileSystem localFs, Path name, String expectedOwner,
      String expectedGroup) throws IOException {
    // check its owner and group
    FileStatus status = localFs.getFileStatus(name);
  ...
  @Test
  public void testAccessOwner() throws Exception {
    setupCluster();
    checkpoint(UpgradeCheckpoints.AFTER_CLUSTER_START);

    DistributedFileSystem rootFs = (DistributedFileSystem) FileSystem.get(conf);
    Path p1 = new Path("/p1");
    rootFs.mkdirs(p1);
    ...
    DistributedFileSystem userFs = USER1.doAs(new PrivilegedExceptionAction<DistributedFileSystem>() {
      @Override
      public DistributedFileSystem run() throws Exception {
        return (DistributedFileSystem) FileSystem.get(conf);
      }
    });
    userFs.setPermission(p1, new FsPermission((short) 0444));
    userFs.access(p1, FsAction.READ);
    try {
      userFs.access(p1, FsAction.WRITE);
    ...
    try {
      userFs.access(badPath, FsAction.READ);
      fail("The access call should have failed");
  ...
  @Test
  public void testAccessGroupMember() throws Exception {
    setupCluster();
    checkpoint(UpgradeCheckpoints.AFTER_CLUSTER_START);
    ...
    userFs.access(p2, FsAction.READ);
    try {
      userFs.access(p2, FsAction.EXECUTE);
      fail("The access call should have failed.");
    } catch (AccessControlException e) {
      assertTrue("Permission denied messages must carry the username",
              e.getMessage().contains(USER1_NAME));
  ...
  @Test
  public void testAccessOthers() throws Exception {
    setupCluster();
    checkpoint(UpgradeCheckpoints.AFTER_CLUSTER_START);
    ...
    userFs.access(p3, FsAction.READ);
    try {
      userFs.access(p3, FsAction.READ_WRITE);
      fail("The access call should have failed.");
    } catch (AccessControlException e) {
      assertTrue("Permission denied messages must carry the username",
              e.getMessage().contains(USER1_NAME));
  ...
  @Test
  public void testPermissionMessageOnNonDirAncestor() throws Exception {
    setupCluster();
    checkpoint(UpgradeCheckpoints.AFTER_CLUSTER_START);
  ...
  private void testPermissionCheckingPerUser(UserGroupInformation ugi,
    ...
    fs = (DistributedFileSystem) DFSTestUtil.login(fs, conf, SUPERUSER);
    for (int i = 0; i < NUM_TEST_PERMISSIONS; i++) {
      ...
      checkOwnership(fs, dirs[i], USER1_NAME, GROUP2_NAME);
      checkOwnership(fs, files[i], USER1_NAME, GROUP2_NAME);
    ...
    fs = (DistributedFileSystem) DFSTestUtil.login(fs, conf, ugi);
    for (int i = 0; i < NUM_TEST_PERMISSIONS; i++) {
