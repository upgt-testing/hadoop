 ...
 */
public class TestDatanodeRestart_RestartInjected_Randomized_123 {
    ...
    public void testFinalizedReplicas() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_replication_complete").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            util.checkFiles(fs, TopDir);
            cluster.restartDataNodes();
            cluster.waitActive();
            RestartFramework.at("after_datanodes_restarted").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            util.checkFiles(fs, TopDir);
        } finally {
            cluster.shutdown();
        }
