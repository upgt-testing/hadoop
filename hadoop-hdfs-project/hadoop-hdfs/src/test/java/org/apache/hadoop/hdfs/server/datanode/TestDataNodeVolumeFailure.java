...
public class TestDataNodeVolumeFailure {
    ...
    public void setUp() throws Exception {
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(dn_num).build();
        cluster.waitActive();
   ...
   */
    ...
    public void testVolumeFailure() throws Exception {
        ...
        failedDir = MiniDockerDFSCluster.getFinalizedDir(data_fail, cluster.getNamesystem().getBlockPoolId());
   ...
   */
    ...
    public void testDnStartsAfterDiskErrorScanningBlockPool() throws Exception {
        ...
        final MiniDockerDFSCluster localCluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).build();
        try {
            localCluster.waitActive();
            DataNodeInterface dn = localCluster.getDataNodes().get(0);
            try {
            ...
            // trigger DN to send heartbeat
            DataNodeTestUtils.triggerHeartbeat(dn);
            final BlockManagerInterface bm = localCluster.getNamesystem().getBlockManager();
    ...
    public void testFailedVolumeBeingRemovedFromDataNode() throws Exception {
        ...
        BlockPoolSliceStorageInterface bpsStorage = storage.getBPStorage(bpid);
        assertEquals(1, bpsStorage.getNumStorageDirs());
        for (int i = 0; i < bpsStorage.getNumStorageDirs(); i++) {
            Storage.StorageDirectoryInterface sd = bpsStorage.getStorageDir(i);
            assertFalse(sd.getRoot().getAbsolutePath().startsWith(dn0Vol1.getAbsolutePath()));
        }
        ...
        for (ReplicaInfoInterface replica : FsDatasetTestUtil.getReplicas(data, bpid)) {
    ...
    public void testRefreshDeadLock() throws Exception {
        ...
        BPOfferServiceInterface service = allBpOs.get(0);
        BPServiceActorInterface actor = service.getBPServiceActors().get(0);
        DatanodeRegistrationInterface bpRegistration = actor.getBpRegistration();
    ...
    private int countRealBlocks(Map<String, BlockLocs> map) {
        ...
        for (int i = 0; i < dn_num; i++) {
            for (int j = 0; j <= 1; j++) {
                ...
                File dir = MiniDockerDFSCluster.getFinalizedDir(storageDir, bpid);
                if (dir == null) {
                    System.out.println("dir is null for dn=" + i + " and data_dir=" + j);
                    continue;
                }
                List<File> res = MiniDockerDFSCluster.getAllBlockMetadataFiles(dir);
                if (res == null) {
    ...
    private static class BadDiskFSDataset extends SimulatedFSDataset {
        ...
        public synchronized void removeVolumes(Collection<StorageLocation> volumes, boolean clearFailure) {
            ...
            int index = 0;
            while (itr.hasNext()) {
                StorageLocationInterface s = itr.next();
