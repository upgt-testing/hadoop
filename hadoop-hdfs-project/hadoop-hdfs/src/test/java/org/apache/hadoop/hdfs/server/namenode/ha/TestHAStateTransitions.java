...
public class TestHAStateTransitions {
    ...
    public void testTransitionActiveToStandby() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).build();
        try {
            cluster.waitActive();
    ...
    private void addCrmThreads(MiniDockerDFSCluster cluster, LinkedList<Thread> crmThreads) {
     ...
     * @throws Exception
     */
    private void testManualFailoverFailback(MiniDockerDFSCluster cluster, Configuration conf, int nsIndex) throws Exception {
    ...
    public void testLeasesRenewedOnTransition() throws Exception {
        ...
        NameNodeInterface nn1 = cluster.getNameNode(1);
        try {
            cluster.waitActive();
            cluster.transitionToActive(0);
    ...
    public void testManualFailoverFailbackFederationHA() throws Exception {
        Configuration conf = new Configuration();
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHAFederatedTopology(2)).numDataNodes(1).build();
        try {
            cluster.waitActive();
    ...
    public void testSecretManagerState() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(1).waitSafeMode(false).build();
        try {
            cluster.transitionToActive(0);
            ...
            NameNodeInterface nn = cluster.getNameNode(0);
    ...
    public void testIsAtLeastOneActive() throws Exception {
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(new HdfsConfiguration()).nnTopology(MiniDFSNNTopology.simpleHATopology()).numDataNodes(0).build();
        try {
