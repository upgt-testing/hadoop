...
public class TestSlowDiskTracker {
    ...
    public void testDataNodeHeartbeatSlowDiskReport() throws Exception {
        ...
        try {
            ...
            DatanodeManagerInterface datanodeManager = nn.getNamesystem().getBlockManager().getDatanodeManager();
            SlowDiskTrackerInterface slowDiskTracker = datanodeManager.getSlowDiskTracker();
    ...
    public void testRemoveInvalidReport() throws Exception {
        ...
        try {
            ...
            slowDiskTracker.addSlowDiskReport("dn1", generateSlowDiskReport("disk1", Collections.singletonMap(DiskOp.WRITE, 1.3)));
            slowDiskTracker.addSlowDiskReport("dn2", generateSlowDiskReport("disk2", Collections.singletonMap(DiskOp.WRITE, 1.1)));
            // wait for slow disk report
            GenericTestUtils.waitFor(() -> !slowDiskTracker.getSlowDisksReport().isEmpty(), 500, 5000);
            Map<String, DiskLatency> slowDisksReport = getSlowDisksReportForTesting(slowDiskTracker);
