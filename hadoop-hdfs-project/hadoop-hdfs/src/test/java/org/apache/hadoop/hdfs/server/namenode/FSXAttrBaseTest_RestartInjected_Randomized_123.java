 ...
 */
public class FSXAttrBaseTest_RestartInjected_Randomized_123 {
    ...
    private void doTestCreateXAttr(Path usePath, Map<String, byte[]> expectedXAttrs) throws Exception {
        ...
        RestartFramework.at("after_set_multiple_xattrs").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        restart(false);
        RestartFramework.at("after_remove_xattr").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        initFileSystem();
        xattrs = fs.getXAttrs(usePath);
        Assert.assertEquals(xattrs.size(), expectedXAttrs.size());
        for (Map.Entry<String, byte[]> ent : expectedXAttrs.entrySet()) {
        ...
        restart(true);
        initFileSystem();
        RestartFramework.at("after_set_xattr").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        xattrs = fs.getXAttrs(usePath);
        Assert.assertEquals(xattrs.size(), expectedXAttrs.size());
        for (Map.Entry<String, byte[]> ent : expectedXAttrs.entrySet()) {
            final byte[] val = (ent.getValue() == null) ? new byte[0] : ent.getValue();
        ...
        RestartFramework.at("after_create_file").on(dfsCluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRemoveXAttr() throws Exception {
        ...
        fs.setXAttr(path, name1, value1, EnumSet.of(XAttrSetFlag.CREATE));
        fs.setXAttr(path, name2, value2, EnumSet.of(XAttrSetFlag.CREATE));
        RestartFramework.at("after_set_xattrs").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Assert.assertArrayEquals(new byte[0], xattrs.get(name3));
        restart(true);
        RestartFramework.at("after_remove_xattrs").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        initFileSystem();
        xattrs = fs.getXAttrs(path);
        Assert.assertEquals(xattrs.size(), 1);
        Assert.assertArrayEquals(new byte[0], xattrs.get(name3));
    ...
    public void testRenameFileWithXAttr() throws Exception {
        FileSystem.mkdirs(fs, path, FsPermission.createImmutable((short) 0750));
        RestartFramework.at("after_rename").on(dfsCluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        fs.setXAttr(path, name1, value1, EnumSet.of(XAttrSetFlag.CREATE));
        fs.setXAttr(path, name2, value2, EnumSet.of(XAttrSetFlag.CREATE));
