...
public class TestGetBlocks {
    ...
    /**
     * Stop the heartbeat of a datanode in the MiniDockerDFSCluster
     *
     * @param cluster
     *          The MiniDockerDFSCluster
     * @param hostName
     *          The hostName of the datanode to be stopped
     * @return The DataNode whose heartbeat has been stopped
     */
    private DataNodeInterface stopDataNodeHeartbeat(MiniDockerDFSCluster cluster, String hostName) {
    ...
    public void testReadSelectNonStaleDatanode() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(NUM_DATA_NODES).racks(RACKS).build();
        cluster.waitActive();
        InetSocketAddress addr = new InetSocketAddress("localhost", cluster.getNameNodePort());
        DFSClient client = new DFSClient(addr, conf);
        List<DatanodeDescriptorInterface> nodeInfoList = cluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getDatanodeListForReport(DatanodeReportType.LIVE);
        assertEquals("Unexpected number of datanodes", NUM_DATA_NODES, nodeInfoList.size());
        ...
        try {
            ...
            stm = fileSys.create(fileName, true, fileSys.getConf().getInt(CommonConfigurationKeys.IO_FILE_BUFFER_SIZE_KEY, 4096), (short) 3, BLOCK_SIZE);
            stm.write(new byte[(BLOCK_SIZE * 3) / 2]);
            // We do not close the stream so that
            // the writing seems to be still ongoing
            stm.hflush();
            LocatedBlocks blocks = client.getNamenode().getBlockLocations(fileName.toString(), 0, BLOCK_SIZE);
            DatanodeInfo[] nodes = blocks.get(0).getLocations();
            assertEquals(nodes.length, 3);
            DataNodeInterface staleNode = null;
            DatanodeDescriptorInterface staleNodeInfo = null;
            ...
            LocatedBlocks blocksAfterStale = client.getNamenode().getBlockLocations(fileName.toString(), 0, BLOCK_SIZE);
            ...
            DatanodeDescriptorInterface dnDesc = cluster.getNameNode().getNamesystem().getBlockManager().getDatanodeManager().getDatanode(staleNode.getDatanodeId());
    ...
    public void testGetBlocks() throws Exception {
        ...
        config.setLong(DFSConfigKeys.DFS_BALANCER_GETBLOCKS_MIN_BLOCK_SIZE_KEY, blkSize);
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(config).numDataNodes(replicationFactor).storagesPerDatanode(4).build();
        try {
            cluster.waitActive();
            // the third block will not be visible to getBlocks
            testFile = new Path(filePath);
            DFSTestUtil.createFile(cluster.getFileSystem(), testFile, fileLen, replicationFactor, 0L);
            ...
            InetSocketAddress addr = new InetSocketAddress("localhost", cluster.getNameNodePort());
            NamenodeProtocol namenode = NameNodeProxies.createProxy(config, DFSUtilClient.getNNUri(addr), NamenodeProtocol.class).getProxy();
            ...
            locs = namenode.getBlocks(dataNodes[0], blkSize, blkSize).getBlocks();
            ...
            getBlocksWithException(namenode, dataNodes[0], 0, 0, RemoteException.class, "IllegalArgumentException");
            // get blocks of size -1 from dataNodes[0]
            getBlocksWithException(namenode, dataNodes[0], -1, 0, RemoteException.class, "IllegalArgumentException");
            // minBlockSize is -1
            getBlocksWithException(namenode, dataNodes[0], blkSize, -1, RemoteException.class, "IllegalArgumentException");
            // get blocks of size BlockSize from a non-existent datanode
            DatanodeInfo info = DFSTestUtil.getDatanodeInfo("1.2.3.4");
            getBlocksWithException(namenode, info, replicationFactor, 0, RemoteException.class, "HadoopIllegalArgumentException");
            ...
            getBlocksWithException(namenode, info, replicationFactor, 0, RemoteException.class, "Cannot execute getBlocks. Name node is in safe mode.");
    ...
    private void getBlocksWithException(NamenodeProtocol namenode, DatanodeInfo datanode, long size, long minBlkSize, Class exClass, String msg) throws Exception {
        ...
        LambdaTestUtils.intercept(exClass, msg, () -> namenode.getBlocks(datanode, size, minBlkSize));
    }
     ...
     */
    void testBlockIterator(MiniDockerDFSCluster cluster) {
        FSNamesystemInterface ns = cluster.getNamesystem();
        String dId = cluster.getDataNodes().get(0).getDatanodeUuid();
        DatanodeDescriptorInterface dnd = BlockManagerTestUtil.getDatanode(ns, dId);
        DatanodeStorageInfoInterface[] storages = dnd.getStorageInfos();
    ...
    public void testReadSkipStaleStorage() throws Exception {
        ...
        MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).storagesPerDatanode(storageNum).build();
        cluster.waitActive();
        FileSystem fs = cluster.getFileSystem();
        DFSTestUtil.createFile(fs, path, false, 1024, fileLen, BLOCK_SIZE, repFactor, 0, true);
        // get datanode info
        ...
        DatanodeStorageInfoInterface[] storageInfos = bm0.getDatanodeManager().getDatanode(dataNodes[0].getDatanodeUuid()).getStorageInfos();
        InetSocketAddress addr = new InetSocketAddress("localhost", cluster.getNameNodePort());
        ...
        BlockWithLocations[] blocks = namenode.getBlocks(dataNodes[0], fileLen * 2, 0).getBlocks();
        ...
        blocks = namenode.getBlocks(dataNodes[0], fileLen * 2, 0).getBlocks();
