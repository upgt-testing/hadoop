 ...
 */
public class TestNamenodeRetryCache_RestartInjected_Randomized_123 {
    ...
    public void testCreate() throws Exception {
        ...
        newCall();
        HdfsFileStatus status = nnRpc.create(src, perm, "holder", new EnumSetWritable<CreateFlag>(EnumSet.of(CreateFlag.CREATE)), true, (short) 1, BlockSize, null, null, null);
        Assert.assertEquals(status, nnRpc.create(src, perm, "holder", new EnumSetWritable<CreateFlag>(EnumSet.of(CreateFlag.CREATE)), true, (short) 1, BlockSize, null, null, null));
        Assert.assertEquals(status, nnRpc.create(src, perm, "holder", new EnumSetWritable<CreateFlag>(EnumSet.of(CreateFlag.CREATE)), true, (short) 1, BlockSize, null, null, null));
        // A non-retried call fails
        newCall();
        try {
            nnRpc.create(src, perm, "holder", new EnumSetWritable<CreateFlag>(EnumSet.of(CreateFlag.CREATE)), true, (short) 1, BlockSize, null, null, null);
    ...
    public void testUpdatePipelineWithFailOver() throws Exception {
        ...
        newCall();
        RestartFramework.at("after_ha_cluster_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
    ...
    public void testRetryCacheRebuild() throws Exception {
        RestartFramework.at("after_run_operations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
