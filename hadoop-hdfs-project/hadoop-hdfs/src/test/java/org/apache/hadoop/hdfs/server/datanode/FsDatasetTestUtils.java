...
public interface FsDatasetTestUtils {
    ...
    abstract class Factory<D extends FsDatasetTestUtils> {
        ...
        public static Factory<?> getFactory(Configuration conf) {
            String className = conf.get(DFSConfigKeys.DFS_DATANODE_FSDATASET_FACTORY_KEY, FsDatasetFactory.class.getName());
            Preconditions.checkState(className.contains("Factory"));
            className = className.replaceFirst("(\\$)?Factory$", "TestUtilsFactory");
            final Class<? extends Factory> clazz = conf.getClass(className, FsDatasetImplTestUtilsFactory.class, Factory.class);
     ...
     *         dataset.
     */
    MaterializedReplica getMaterializedReplica(ExtendedBlock block) throws ReplicaNotFoundException;
     ...
     * the FsDataset.
     */
    Replica createFinalizedReplica(FsVolumeSpi volume, ExtendedBlock block) throws IOException;
     ...
     * Create a {@link ReplicaInPipeline} and add it into the FsDataset.
     */
    Replica createReplicaInPipeline(FsVolumeSpi volume, ExtendedBlock block) throws IOException;
     ...
     * FsDataset.
     */
    Replica createReplicaWaitingToBeRecovered(ExtendedBlock block) throws IOException;

    /**
     * Create a {@link ReplicaWaitingToBeRecovered} on the particular volume,
     * and add it into the FsDataset.
     */
    Replica createReplicaWaitingToBeRecovered(FsVolumeSpi volume, ExtendedBlock block) throws IOException;

    /**
     * Create a {@link ReplicaUnderRecovery} object and add it into the FsDataset.
     */
    Replica createReplicaUnderRecovery(ExtendedBlock block, long recoveryId) throws IOException;
     ...
     * @throws IOException
     */
    void changeStoredGenerationStamp(ExtendedBlock block, long newGenStamp) throws IOException;
