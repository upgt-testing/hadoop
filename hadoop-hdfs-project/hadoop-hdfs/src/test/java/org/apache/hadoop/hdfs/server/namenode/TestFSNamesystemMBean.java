...
public class TestFSNamesystemMBean {
    ...
    public void testReconstructionQueuesInitProgressMetrics() throws Exception {
        ...
        try (MiniDFSClusterInJVM cluster = new MiniDFSClusterInJVM.Builder(conf).build()) {
            ...
            MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
            ObjectName mxbeanName = new ObjectName("Hadoop:service=NameNode,name=FSNamesystemState");
            float reconstructionQueuesInitProgress = (float) mbs.getAttribute(mxbeanName, "ReconstructionQueuesInitProgress");
            ...
            GenericTestUtils.waitFor(() -> cluster.getNamesystem().getReconstructionQueuesInitProgress() == 1.0, 100, 5 * 1000);
            reconstructionQueuesInitProgress = (float) mbs.getAttribute(mxbeanName, "ReconstructionQueuesInitProgress");
            cluster.restartNodeForTesting(0);
            cluster.upgradeNodeForTesting(0);
