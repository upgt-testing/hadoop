...
public class TestWebHdfsFileSystemContract_RestartInjected_Randomized_123 extends FileSystemContractBaseTest {
    ...
    public void testRootDir() throws IOException {
        ...
        RestartFramework.at("after_get_status").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDatanodeCreateMissingParameter() throws IOException {
        RestartFramework.at("after_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final WebHdfsFileSystem webhdfs = (WebHdfsFileSystem) fs;
        final Path testDir = new Path(MessageFormat.format("/test/{0}/{1}", TestWebHdfsFileSystemContract_RestartInjected.class, GenericTestUtils.getMethodName()));
