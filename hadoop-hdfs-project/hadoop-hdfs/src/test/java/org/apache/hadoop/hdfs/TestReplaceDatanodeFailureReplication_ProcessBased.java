...
/**
 * ProcessBasedMiniDFSCluster version of {@link TestReplaceDatanodeFailureReplication}.
 *
 ...
 *
 * @see TestReplaceDatanodeFailureReplication Original test using MiniDFSCluster
 */
@RunWith(Parameterized.class)
public class TestReplaceDatanodeFailureReplication_ProcessBased extends ProcessBasedUpgradeTestBase {
  static final Logger LOG = LoggerFactory
      .getLogger(TestReplaceDatanodeFailureReplication_ProcessBased.class);

  static final String DIR =
      "/" + TestReplaceDatanodeFailureReplication_ProcessBased.class.getSimpleName() + "/";
  ...
  public static Collection<String> checkpoints() {
    return Arrays.asList(
      ...
      "AFTER_SLOW_WRITER_START",
      "AFTER_INITIAL_WRITES",
      "AFTER_DATANODE_STOP",
      "AFTER_ADDITIONAL_WRITES",
      "BEFORE_VERIFICATION"
    );
  }
  ...
  public void testLessNumberOfLiveDatanodesThanWriteReplaceDatanodeOnFailureRF()
    ...
    try {
      ...
      for (int i = 1; i <= slowwriters.length; i++) {
        ...
        slowwriters[i - 1] = new SlowWriter(dfs, new Path(dir, "file" + i),
            i * 200L);
      }
      ...
      // Verify the file
      verifyFileContent(dfs, slowwriters);
    } finally {
      // Cleanup handled by @After in base class
    }
  }

  private ProcessBasedMiniDFSCluster setupCluster(int failRF) throws Exception {
    ...
    return new ProcessBasedMiniDFSCluster.Builder(conf).racks(racks)
  ...
  private void testWriteFileAndVerifyAfterDNStop(int failRF, int dnindex,
    ...
    try {
      ...
      // stop a datanode
      cluster.shutdownDataNode(dnindex);
      if (failPipeLine) {
        cluster.shutdownDataNode(dnindex);
      }
      ...
      // Everyone should have written something.
      sleepSeconds(5);
      // Wait for block report - replaced cluster.waitFirstBRCompleted with sleep
      Thread.sleep(10000);
  ...
  private void verifyFileContent(DistributedFileSystem dfs,
      SlowWriter[] slowwriters) throws IOException {
    LOG.info("Verify the file");
    for (int i = 0; i < slowwriters.length; i++) {
      LOG.info(slowwriters[i].filepath + ": length=" + dfs
          .getFileStatus(slowwriters[i].filepath).getLen());
      FSDataInputStream in = null;
      try {
        in = dfs.open(slowwriters[i].filepath);
        for (int j = 0, x;; j++) {
          x = in.read();
          if ((x) != -1) {
  ...
  static class SlowWriter extends Thread {
    ...
    SlowWriter(DistributedFileSystem dfs, Path filepath, final long sleepms)
        throws IOException {
      super(SlowWriter.class.getSimpleName() + ":" + filepath);
      this.filepath = filepath;
      this.out = (HdfsDataOutputStream) dfs.create(filepath, REPLICATION);
