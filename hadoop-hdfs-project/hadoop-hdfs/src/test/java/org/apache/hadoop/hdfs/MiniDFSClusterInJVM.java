...
public class MiniDFSClusterInJVM implements AutoCloseable {
  ...
  public static final String PROP_TEST_BUILD_DATA = "test.build.data";
    ...
    /**
     * Used by builder to create and return an instance of MiniDFSClusterInJVM
     */
      ...
      boolean skipFsyncForTesting)
        ...
        try {
            ...
            if (!nnTopology.allHttpPortsSpecified() &&
                    nnTopology.isHA()) {
                LOG.info("MiniDFSClusterInJVM disabling checkpointing in the Standby node " +
            ...
            if (!nnTopology.allIpcPortsSpecified() &&
                    nnTopology.isHA()) {
                LOG.info("MiniDFSClusterInJVM disabling log-roll triggering in the "
  ...
  private void createNameNode(int nnIndex, Configuration hdfsConf,
      ...
      String nnId, NameNodeInstance nnInstance)
      throws IOException {
        ...
        NameNodeJVMInterface nn = nnInstance.createNameNodeForInJVMCluster(args, hdfsConf);
        ...
        DFSUtil.setGenericConf(hdfsConf, nameserviceId, nnId,
                DFS_NAMENODE_HTTP_ADDRESS_KEY);
        nameNodes[nnIndex] = new NameNodeInfo(nn, nnInstance, nameserviceId, nnId,
                operation, hdfsConf);
    }

    /**
     * @return URI of the namenode from a single namenode MiniDFSClusterInJVM
     */
    public URI getURI() {
        checkSingleNameNode();
        return getURI(0);
    ...
    /**
     * @return URI of the given namenode in MiniDFSClusterInJVM
     */
    ...
    public synchronized void startDataNodes(Configuration conf, int numDataNodes,
        ...
        DataNodeJVMInterface[] dns = new DataNodeJVMInterface[numDataNodes];
        for (int i = curDatanodesNum; i < curDatanodesNum+numDataNodes; i++) {
            ...
            DataNodeJVMInterface dn = dnInstancre.createDataNodeForInJVMCluster(dnArgs, dnConf, setSecureResources);
     ...
     */
    public ArrayList<DataNodeJVMInterface> getDataNodes() {
        ArrayList<DataNodeJVMInterface> list = new ArrayList<DataNodeJVMInterface>();
        for (int i = 0; i < dataNodes.size(); i++) {
            DataNodeJVMInterface node = dataNodes.get(i).datanode;
            list.add(node);
        }
        return list;
    }
    ...
    public DataNodeJVMInterface getDataNode(int ipcPort) {
        for(DataNodeJVMInterface dn : getDataNodes()) {
            if (dn.getRpcServer().getListenerAddress().getPort() == ipcPort) {
                return dn;
            }
        }
        return null;
     ...
     * @return a FsDatasetTestUtils for the given DataNode.
     */
    public FsDatasetTestUtils getFsDatasetTestUtils(DataNodeJVMInterface dn) {
    ...
    public void shutdownDataNode(int dnIndex) {
        ...
        DataNodeJVMInterface dn = dataNodes.remove(dnIndex).datanode;
        dn.shutdown();
        numDataNodes--;
    }
    ...
    public synchronized void shutdownNameNode(int nnIndex) {
        NameNodeJVMInterface nn = nameNodes[nnIndex].nameNode;
        if (nn != null) {
            stopAndJoinNameNode(nn);
            Configuration conf = nameNodes[nnIndex].conf;
            nameNodes[nnIndex] = new NameNodeInfo(null, nameNodes[nnIndex].nnInstance, null, null, null, conf);
        }
    }
     ...
     */
    private void stopAndJoinNameNode(NameNodeJVMInterface nn) {
        if (nn == null) {
            return;
        }
    ...
    public synchronized void restartNameNode(int nnIndex, boolean waitActive,
        ...
        String curVersion =  nameNodes[nnIndex].nnInstance.getCurVersion();
        NameNodeInstance nnInstance = new NameNodeInstance(curVersion);
        NameNodeJVMInterface nn = nnInstance.createNameNodeForInJVMCluster(args, nameNodes[nnIndex].conf);
        nameNodes[nnIndex] = new NameNodeInfo(nn, nnInstance, nameserviceId, nnId, startOpt,
                conf);
        if (waitActive) {
            waitClusterUp();
    ...
    private void waitDataNodeFullyStarted(final DataNodeJVMInterface dn)
    ...
    private synchronized void setDataNodeStorageCapacities(
            ...
            final int numDNs,
            final DataNodeJVMInterface[] dns,
    ...
    private synchronized void setDataNodeStorageCapacities(
            final int curDnIdx,
            final DataNodeJVMInterface curDn,
            long[][] storageCapacities) throws IOException {
        ...
        try (FsVolumeReferencesJVMInterface volumes = curDn.getFSDataset()
    ...
    public boolean isNameNodeUp(int nnIndex) {
        NameNodeJVMInterface nameNode = nameNodes[nnIndex].nameNode;
        if (nameNode == null) {
            return false;
        }
        long[] sizes;
    ...
    if (nameNodes.length == 0 || nameNodes[nnIndex] == null
        ...
        final DatanodeManagerJVMInterface dm = ns.getBlockManager().getDatanodeManager();
        GenericTestUtils.waitFor(new Supplier<Boolean>() {
            @Override
            public Boolean get() {
                List<DatanodeDescriptorJVMInterface> nodes = (List<DatanodeDescriptorJVMInterface>) dm.getDatanodeListForReport
                        (DatanodeReportType.LIVE);
                for (DatanodeDescriptorJVMInterface node : nodes) {
     ...
     */
    DataNodeJVMInterface[] listDataNodes() {
        DataNodeJVMInterface[] list = new DataNodeJVMInterface[dataNodes.size()];
        for (int i = 0; i < dataNodes.size(); i++) {
     ...
     */
    public static void shutdownCluster(MiniDFSClusterInJVM cluster) {
        if (cluster != null) {
            cluster.shutdown();
        }
    }
    ...
    /**
     * Throw an exception if the MiniDFSClusterInJVM is not started with a single
     * namenode
     */
    ...
    public void addNameNode(Configuration conf, int namenodePort)
        ...
        createNameNode(nnIndex, conf, numDataNodes, true, null, null,
            nameserviceId, nnId, nnConf.getNnInstance());

        // Refresh datanodes with the newly started namenode
        for (DataNodeProperties dn : dataNodes) {
            DataNodeJVMInterface datanode = dn.datanode;
            // TODO: FIX ME
            throw new UnsupportedOperationException("Not implemented");
