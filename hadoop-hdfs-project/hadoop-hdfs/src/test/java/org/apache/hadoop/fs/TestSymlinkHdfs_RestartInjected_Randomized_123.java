 ...
 */
abstract public class TestSymlinkHdfs_RestartInjected_Randomized_123 extends SymlinkBaseTest {
     ...
     */
    void testLinkAcrossFileSystems() throws IOException {
        ...
        RestartFramework.at("after_create_local_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    void testRenameAcrossFileSystemsViaLink() throws IOException {
        Path localDir = new Path("file://" + wrapper.getAbsoluteTestRootDir() + "/test");
        Path hdfsFile = new Path(testBaseDir1(), "file");
        Path link = new Path(testBaseDir1(), "link");
        Path hdfsFileNew = new Path(testBaseDir1(), "fileNew");
        RestartFramework.at("after_create_hdfs_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_symlink_to_dir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
     ...
     */
    void testCreateLinkToSlash() throws IOException {
        Path dir = new Path(testBaseDir1());
        Path file = new Path(testBaseDir1(), "file");
        Path link = new Path(testBaseDir1(), "linkToSlash");
        RestartFramework.at("after_create_symlink_to_slash").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    void testSetPermissionAffectsTarget() throws IOException {
        Path file = new Path(testBaseDir1(), "file");
        RestartFramework.at("after_create_symlinks").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_file_for_permission").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_set_permissions").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    void testCreateWithPartQualPathFails() throws IOException {
        Path fileWoAuth = new Path("hdfs:///test/file");
        Path linkWoAuth = new Path("hdfs:///test/link");
        RestartFramework.at("before_test_partial_path").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
     ...
     */
    void testSetReplication() throws IOException {
        Path file = new Path(testBaseDir1(), "file");
        Path link = new Path(testBaseDir1(), "linkToFile");
        createAndWriteFile(file);
        wrapper.createSymlink(file, link, false);
        wrapper.setReplication(link, (short) 2);
        RestartFramework.at("after_create_file_for_replication").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(0, wrapper.getFileLinkStatus(link).getReplication());
        assertEquals(2, wrapper.getFileStatus(link).getReplication());
        RestartFramework.at("after_create_symlink_for_replication").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    void testCreateLinkMaxPathLink() throws IOException {
        Path dir = new Path(testBaseDir1());
        Path file = new Path(testBaseDir1(), "file");
        RestartFramework.at("after_create_max_path_symlink").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_create_file_for_max_path").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            wrapper.createSymlink(file, link, false);
     ...
     * Test symlink owner
     */
    void testLinkOwner() throws IOException {
        RestartFramework.at("after_create_file_for_owner").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path file = new Path(testBaseDir1(), "file");
        Path link = new Path(testBaseDir1(), "symlinkToFile");
        createAndWriteFile(file);
        wrapper.createSymlink(file, link, false);
        FileStatus statFile = wrapper.getFileStatus(file);
        RestartFramework.at("after_create_symlink_for_owner").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    void testWebHDFS() throws IOException {
        RestartFramework.at("after_create_webhdfs_symlink").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Path file = new Path(testBaseDir1(), "file");
        Path link = new Path(testBaseDir1(), "linkToFile");
        createAndWriteFile(file);
        RestartFramework.at("after_create_file_for_webhdfs").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
     ...
     */
    void testQuota() throws IOException {
        ...
        RestartFramework.at("after_create_first_symlink").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        createAndWriteFile(file);
        RestartFramework.at("after_create_file_for_quota").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
