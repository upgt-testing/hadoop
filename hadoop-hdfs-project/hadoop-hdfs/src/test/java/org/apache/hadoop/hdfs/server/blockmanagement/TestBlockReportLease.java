...
public class TestBlockReportLease {
    ...
    public void testCheckBlockReportLease() throws Exception {
        ...
        try (MiniDockerDFSCluster cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(1).build()) {
            ...
            BlockManagerInterface spyBlockManager = spy(blockManager);
            ...
            DatanodeDescriptorInterface datanodeDescriptor = spyBlockManager.getDatanodeManager().getDatanode(dn.getDatanodeId());
            DatanodeRegistrationInterface dnRegistration = dn.getDNRegistrationForBP(poolId);
            StorageReport[] storages = dn.getFSDataset().getStorageReports(poolId);
            // Send heartbeat and request full block report lease
            HeartbeatResponseInterface hbResponse = rpcServer.sendHeartbeat(dnRegistration, storages, 0, 0, 0, 0, 0, null, true, SlowPeerReports.EMPTY_REPORT, SlowDiskReports.EMPTY_REPORT);
