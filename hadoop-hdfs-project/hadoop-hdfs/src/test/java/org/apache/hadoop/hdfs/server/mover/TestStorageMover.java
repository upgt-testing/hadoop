...
public class TestStorageMover {
    ...
    class MigrationTest {
        ...
        void setupCluster() throws Exception {
            cluster = new MiniDFSClusterInJVM.Builder(conf).numDataNodes(clusterScheme.numDataNodes).storageTypes(clusterScheme.storageTypes).storageCapacities(clusterScheme.storageCapacities).build();
            cluster.waitActive();
    ...
    public void testMoveSpecificPaths() throws Exception {
        ...
        try {
            ...
            Thread.sleep(5000);
            test.cluster.restartNodeForTesting(0);
            test.cluster.upgradeNodeForTesting(0);
            test.verify(true);
        } finally {
    ...
    private void setVolumeFull(DataNodeJVMInterface dn, StorageType type) {
        try (FsVolumeReferencesJVMInterface refs = dn.getFSDataset().getFsVolumeReferences()) {
            for (FsVolumeSpi fvs : refs) {
                FsVolumeImpl volume = (FsVolumeImpl) fvs;
                if (volume.getStorageType() == type) {
                    LOG.info("setCapacity to 0 for [" + volume.getStorageType() + "]" + volume.getStorageID());
