 ...
 */
public class TestDFSHAAdminMiniCluster_RestartInjected_Randomized_123 {

    static {
        GenericTestUtils.setLogLevel(LoggerFactory.getLogger(HAAdmin.class), Level.TRACE);
    }

    private static final Logger LOG = LoggerFactory.getLogger(TestDFSHAAdminMiniCluster_RestartInjected.class);
    ...
    public void testGetServiceState() throws Exception {
        ...
        RestartFramework.at("after_get_service_state").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_enter_safemode").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(0, runTool("-getServiceState", "nn1"));
    }
    ...
    public void testStateTransition() throws Exception {
        ...
        RestartFramework.at("after_transition_nn1_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        nnode1 = cluster.getNameNode(0);
        assertTrue(nnode1.isStandbyState());
        RestartFramework.at("after_transition_nn1_standby").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        NameNode nnode2 = cluster.getNameNode(1);
        RestartFramework.at("after_transition_nn2_standby").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        nnode1 = cluster.getNameNode(0);
        nnode2 = cluster.getNameNode(1);
        RestartFramework.at("after_transition_nn2_observer").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(nnode2.isStandbyState());
        assertEquals(0, runTool("-transitionToObserver", "nn2"));
        nnode1 = cluster.getNameNode(0);
        RestartFramework.at("after_transition_nn2_active").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        nnode2 = cluster.getNameNode(1);
        assertFalse(nnode2.isStandbyState());
    ...
    public void testObserverTransition() throws Exception {
        ...
        assertEquals(0, runTool("-transitionToObserver", "nn1"));
        RestartFramework.at("after_observer_noop").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nnode1 = cluster.getNameNode(0);
        assertTrue(nnode1.isObserverState());
        // Should also be able to transition back from OBSERVER to STANDBY
        assertEquals(0, runTool("-transitionToStandby", "nn1"));
        RestartFramework.at("after_observer_to_standby").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nnode1 = cluster.getNameNode(0);
        RestartFramework.at("after_transition_to_observer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(nnode1.isStandbyState());
    ...
    public void testObserverIllegalTransition() throws Exception {
        ...
        nnode1 = cluster.getNameNode(0);
        RestartFramework.at("after_transition_to_observer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_failed_active_to_observer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertFalse(nnode1.isStandbyState());
        ...
        RestartFramework.at("after_failed_observer_to_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nnode1 = cluster.getNameNode(0);
        assertTrue(nnode1.isActiveState());
    ...
    public void testTryFailoverToSafeMode() throws Exception {
        ...
        tool.setConf(conf);
        NameNodeAdapter.enterSafeMode(cluster.getNameNode(0), false);
        RestartFramework.at("after_failed_failover").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFencer() throws Exception {
        RestartFramework.at("after_forcefence_first_failover").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // Test failover with no fencer
        assertEquals(-1, runTool("-failover", "nn1", "nn2"));
        RestartFramework.at("after_good_fencer_config").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        if (Shell.WINDOWS) {
            conf.set(DFSConfigKeys.DFS_HA_FENCE_METHODS_KEY, "shell(echo %source_nameserviceid%.%source_namenodeid% " + "%source_port% %dfs_ha_namenode_id% > " + tmpFile.getAbsolutePath() + ")");
        } else {
            conf.set(DFSConfigKeys.DFS_HA_FENCE_METHODS_KEY, "shell(echo -n $source_nameserviceid.$source_namenodeid " + "$source_port $dfs_ha_namenode_id > " + tmpFile.getAbsolutePath() + ")");
        }
        RestartFramework.at("after_failover_with_ns").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Test failover with fencer
        tool.setConf(conf);
        assertEquals(0, runTool("-transitionToActive", "nn1"));
        RestartFramework.at("after_unset_fencer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_forceactive_failover").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Test failover with fencer and forcefence option
        assertEquals(0, runTool("-failover", "nn1", "nn2", "--forcefence"));
        RestartFramework.at("after_failed_failover_no_fencer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        tmpFile.delete();
        RestartFramework.at("after_failed_failover_no_fencer_forcefence").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Test failover with forceactive option
        assertEquals(0, runTool("-failover", "nn2", "nn1", "--forceactive"));
        RestartFramework.at("after_fencer_config").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_failed_failover_bad_fencer").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        tool.setConf(conf);
        RestartFramework.at("after_forcefence_failover").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertEquals(-1, runTool("-failover", "nn1", "nn2", "--forcefence"));
        assertFalse(tmpFile.exists());
        RestartFramework.at("after_bad_fencer_config").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testCheckHealth() throws Exception {
        ...
        RestartFramework.at("after_check_health_nn1").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_check_health_nn2").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testTransitionToActiveWhenOtherNamenodeisActive() throws Exception {
        ...
        RestartFramework.at("after_forceactive_nn2").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_ensure_nn2_standby").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_transition_both_active").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nn1 = cluster.getNameNode(0);
        RestartFramework.at("after_shutdown_nn1").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        nn2 = cluster.getNameNode(1);
        ...
        RestartFramework.at("after_ensure_standby").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        nn1 = cluster.getNameNode(0);
        nn2 = cluster.getNameNode(1);
