 ...
 */
public class TestBalancerService_RestartInjected_Randomized_456 {
    ...
    public void testBalancerServiceBalanceTwice() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_first_datanode_added").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_balance").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.triggerHeartbeats();
            cluster.triggerBlockReports();
            RestartFramework.at("after_trigger_heartbeats").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            totalCapacity = addOneDataNode(conf);
            RestartFramework.at("after_second_datanode_added").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            TestBalancer.waitForBalancer(totalUsedSpace, totalCapacity, client, cluster, BalancerParameters.DEFAULT);
            RestartFramework.at("after_second_balance").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Balancer.stop();
            balancerThread.join();
        } finally {
            if (cluster != null) {
    ...
    public void testBalancerServiceOnError() throws Exception {
        ...
        try {
            ...
            Thread balancerThread = newBalancerService(conf, new String[] { "-asService" });
            balancerThread.start();
            RestartFramework.at("after_balancer_start").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            TestBalancerWithHANameNodes.waitStoragesNoStale(cluster, client, 0);
            long totalCapacity = addOneDataNode(conf);
            RestartFramework.at("after_datanode_added").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            TestBalancer.waitForBalancer(totalUsedSpace, totalCapacity, client, cluster, BalancerParameters.DEFAULT);
            RestartFramework.at("after_final_balance").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Balancer.stop();
            balancerThread.join();
