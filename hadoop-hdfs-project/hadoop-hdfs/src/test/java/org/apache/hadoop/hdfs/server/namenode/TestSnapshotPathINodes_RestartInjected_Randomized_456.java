 ...
 */
public class TestSnapshotPathINodes_RestartInjected_Randomized_456 {
    ...
    public void testSnapshotPathINodes() throws Exception {
        ...
        nodesInPath = INodesInPath.resolve(fsdir.rootDir, components, false);
        assertEquals(nodesInPath.length(), components.length - 1);
        RestartFramework.at("before_snapshot_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertSnapshot(nodesInPath, true, snapshot, 3);
        // Check the INode for file1 (snapshot file)
        ...
        assertNull(nodesInPath.getLastINode());
        RestartFramework.at("after_snapshot_create").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSnapshotPathINodesAfterDeletion() throws Exception {
        ...
        RestartFramework.at("after_snapshot_create").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        INodesInPath nodesInPath = INodesInPath.resolve(fsdir.rootDir, components, false);
        RestartFramework.at("after_file_delete").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
