 ...
 */
public class TestLargeBlockReport_RestartInjected_Randomized_456 {
    ...
    public void testBlockReportExceedsLengthLimit() throws Exception {
        ...
        initCluster();
        RestartFramework.at("after_create_reports").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cluster_init").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try {
            nnProxy.blockReport(bpRegistration, bpId, reports, new BlockReportContext(1, 0, reportId, fullBrLeaseId));
    ...
    public void testBlockReportSucceedsWithLargerLengthLimit() throws Exception {
        RestartFramework.at("after_create_reports").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_block_report").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        conf.setInt(IPC_MAXIMUM_DATA_LENGTH, IPC_MAXIMUM_DATA_LENGTH_DEFAULT * 2);
        initCluster();
        RestartFramework.at("after_cluster_init").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StorageBlockReport[] reports = createReports(6000000);
