...
public class TestDataNodeVolumeFailureReporting {
    ...
    public void testAutoFormatEmptyBlockPoolDirectory() throws Exception {
        ...
        BlockPoolSliceStorageInterface bps = dn.getStorage().getBPStorage(bpid);
        Storage.StorageDirectoryInterface dir = bps.getStorageDir(0);
    ...
    private void checkFailuresAtNameNode(DatanodeManager dm, DataNode dn, boolean expectCapacityKnown, String... expectedFailedVolumes) throws Exception {
        DatanodeDescriptorInterface dd = cluster.getNamesystem().getBlockManager().getDatanodeManager().getDatanode(dn.getDatanodeId());
        assertEquals(expectedFailedVolumes.length, dd.getVolumeFailures());
        VolumeFailureSummaryInterface volumeFailureSummary = dd.getVolumeFailureSummary();
        if (expectedFailedVolumes.length > 0) {
     ...
     */
        ...
        cluster = new MiniDockerDFSCluster.Builder(conf).numDataNodes(numDataNodes).storagesPerDatanode(storagesPerDatanode).build();
        cluster.waitActive();
