 ...
 */
public class TestBlockToken_RestartInjected_Randomized_456 {
    ...
    private BlockTokenIdentifier generateTokenId(BlockTokenSecretManager sm, ExtendedBlock block, EnumSet<BlockTokenIdentifier.AccessMode> accessModes, StorageType[] storageTypes, String[] storageIds) throws IOException {
        Token<BlockTokenIdentifier> token = sm.generateToken(block, accessModes, storageTypes, storageIds);
    ...
    private void tokenGenerationAndVerification(BlockTokenSecretManager master, BlockTokenSecretManager slave, StorageType[] storageTypes, String[] storageIds) throws Exception {
        ...
        for (BlockTokenIdentifier.AccessMode mode : BlockTokenIdentifier.AccessMode.values()) {
            // generated by master
            Token<BlockTokenIdentifier> token1 = master.generateToken(block1, EnumSet.of(mode), storageTypes, storageIds);
            ...
            // generated by slave
            Token<BlockTokenIdentifier> token2 = slave.generateToken(block2, EnumSet.of(mode), storageTypes, storageIds);
        ...
        // multi-mode tokens
        Token<BlockTokenIdentifier> mtoken = master.generateToken(block3, EnumSet.allOf(BlockTokenIdentifier.AccessMode.class), storageTypes, storageIds);
        for (BlockTokenIdentifier.AccessMode mode : BlockTokenIdentifier.AccessMode.values()) {
    ...
    private void testBlockTokenRpc(boolean enableProtobuf) throws Exception {
        ...
        UserGroupInformation.setConfiguration(conf);
        BlockTokenSecretManager sm = new BlockTokenSecretManager(blockKeyUpdateInterval, blockTokenLifetime, 0, 1, "fake-pool", null, enableProtobuf);
        Token<BlockTokenIdentifier> token = sm.generateToken(block3, EnumSet.allOf(BlockTokenIdentifier.AccessMode.class), new StorageType[] { StorageType.DEFAULT }, new String[0]);
    ...
    private void testBlockTokenInLastLocatedBlock(boolean enableProtobuf) throws IOException, InterruptedException {
        ...
        try {
            ...
            out.flush();
            RestartFramework.at("after_flush").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("before_close").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            out.close();
        } finally {
            cluster.shutdown();
        }
    ...
    private void testBadStorageIDCheckAccess(boolean enableProtobuf) throws IOException {
        ...
        BlockTokenIdentifier id = generateTokenId(sm, block3, EnumSet.of(mode), storageTypes, storageIds);
    ...
    public void testLastLocatedBlockTokenExpiry() throws IOException, InterruptedException {
        ...
        try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).build()) {
            ...
            RestartFramework.at("after_open_input").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            out.close();
            RestartFramework.at("after_close_output").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // wait for last block token to expire
            Thread.sleep(2000L);
            RestartFramework.at("after_token_expiry").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
