...
public class TestNameNodeResourceChecker_RestartInjected_Randomized_123 {

    private final static File BASE_DIR = PathUtils.getTestDir(TestNameNodeResourceChecker_RestartInjected.class);
    ...
    public void testCheckThatNameNodeResourceMonitorIsRunning() throws IOException, InterruptedException {
        ...
        try {
            ...
            RestartFramework.at("after_resource_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            mockResourceChecker.setResourcesAvailable(false);
            RestartFramework.at("after_resource_threshold_crossed").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_safe_mode_entered").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster != null)
                cluster.shutdown();
        }
