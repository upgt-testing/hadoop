...
public class TestWebHDFSForHA {
    ...
    public void testHA() throws IOException {
        ...
        FileSystem fs = null;
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(topo).numDataNodes(0).build();
    ...
    public void testClientFailoverWhenStandbyNNHasStaleCredentials() throws IOException {
        ...
        try {
            ...
            cluster.transitionToActive(1);
            final DelegationTokenSecretManagerInterface secretManager = NameNodeAdapter.getDtSecretManager(cluster.getNamesystem(0));
            ...
            RemoteExceptionInterface re = JsonUtilClient.toRemoteException(m);
    ...
    public void testFailoverAfterOpen() throws IOException {
        ...
        try {
            cluster = new MiniDockerDFSCluster.Builder(conf).nnTopology(topo).numDataNodes(1).build();
            HATestUtil.setFailoverConfigurations(cluster, conf, LOGICAL_NAME);
            cluster.waitActive();
            fs = FileSystem.get(WEBHDFS_URI, conf);
            cluster.transitionToActive(1);
    ...
    public void testRetryWhileNNStartup() throws Exception {
        ...
        try {
            ...
            cluster.waitActive();
            cluster.transitionToActive(0);
            final NameNodeInterface namenode = cluster.getNameNode(0);
