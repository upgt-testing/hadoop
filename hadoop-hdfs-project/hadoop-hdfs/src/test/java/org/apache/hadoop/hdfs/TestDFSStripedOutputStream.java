...
public class TestDFSStripedOutputStream {
    ...
    private void testOneFile(String src, int writeBytes) throws Exception {
        ...
        StripedFileTestUtil.checkData(fs, testPath, writeBytes, new ArrayList<DatanodeInfo>(), null, blockSize * dataBlocks);
    }
    ...
    public void testFileBlockSizeSmallerThanCellSize() throws Exception {
        ...
        try {
            ...
            fail("Creating a file with block size smaller than " + "ec policy's cell size should fail");
    ...
    @Test
    public void testFileLessThanFullBlockGroup_withUpgrade20() throws Exception {
        testOneFile("/LessThanFullBlockGroup", cellSize * dataBlocks * (stripesPerBlock - 1) + cellSize);
        cluster.restartNodeForTesting(0);
        cluster.upgradeNodeForTesting(0);
    }
    ...
    @Test
    public void testFileBlockSizeSmallerThanCellSize_withUpgrade20() throws Exception {
        final Path path = new Path("testFileBlockSizeSmallerThanCellSize");
        final byte[] bytes = StripedFileTestUtil.generateBytes(cellSize * 2);
        try {
            DFSTestUtil.writeFile(fs, path, bytes, cellSize / 2);
    ...
    @Test
    public void testFileBlockSizeSmallerThanCellSize_withUpgrade60() throws Exception {
        final Path path = new Path("testFileBlockSizeSmallerThanCellSize");
        final byte[] bytes = StripedFileTestUtil.generateBytes(cellSize * 2);
        try {
            DFSTestUtil.writeFile(fs, path, bytes, cellSize / 2);
    ...
    @Test
    public void testFileBlockSizeSmallerThanCellSize_withUpgrade80() throws Exception {
        final Path path = new Path("testFileBlockSizeSmallerThanCellSize");
        final byte[] bytes = StripedFileTestUtil.generateBytes(cellSize * 2);
        try {
            DFSTestUtil.writeFile(fs, path, bytes, cellSize / 2);
