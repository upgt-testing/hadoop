 ...
 */
public class TestNamenodeCapacityReport_RestartInjected_Randomized_123 {
    ...
    public void testXceiverCountInternal(int minMaintenanceR) throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_initial_health_check").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_admin_operations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            namesystem = cluster.getNamesystem();
