...
public class TestINodeAttributeProvider {
    ...
    void // HDFS-14389 - Ensure getAclStatus returns the owner, group and permissions
    testGetAclStatusReturnsProviderOwnerPerms() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
        final Path authzChild = new Path("/user/authz/child2");
    ...
    void // HDFS-16529 - Ensure enforcer AccessControlException subclass are caught
    testSubClassedAccessControlExceptions() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
    ...
    void // HDFS-15165 - ContentSummary calls should use the provider permissions(if
    testContentSummary() throws Exception {
    ...
    void // See HDFS-16132 where an issue was reported after HDFS-15372. The sequence
    ...
    testAttrProviderWorksCorrectlyOnRenamedSnapshotPaths() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
    ...
    @Test
    public void testAclFeature_withUpgrade20() throws Exception {
        UserGroupInformation ugi = UserGroupInformation.createUserForTesting("testuser", new String[] { "testgroup" });
        ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
            FileSystem fs = miniDFS.getFileSystem();
            miniDFS.restartNodeForTesting(0);
    ...
    @Test
    public void testAclFeature_withUpgrade40() throws Exception {
        UserGroupInformation ugi = UserGroupInformation.createUserForTesting("testuser", new String[] { "testgroup" });
        ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
            FileSystem fs = miniDFS.getFileSystem();
            Path aclDir = new Path("/user/acl");
    ...
    @Test
    public void testAclFeature_withUpgrade60() throws Exception {
        UserGroupInformation ugi = UserGroupInformation.createUserForTesting("testuser", new String[] { "testgroup" });
        ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
            FileSystem fs = miniDFS.getFileSystem();
            Path aclDir = new Path("/user/acl");
    ...
    @Test
    public void testAclFeature_withUpgrade80() throws Exception {
        UserGroupInformation ugi = UserGroupInformation.createUserForTesting("testuser", new String[] { "testgroup" });
        ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
            FileSystem fs = miniDFS.getFileSystem();
            Path aclDir = new Path("/user/acl");
    ...
    @Test
    public // from the Attribute Provider, and not from HDFS.
    void testGetAclStatusReturnsProviderOwnerPerms_withUpgrade20() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
        final Path authzChild = new Path("/user/authz/child2");
    ...
    @Test
    public // from the Attribute Provider, and not from HDFS.
    void testGetAclStatusReturnsProviderOwnerPerms_withUpgrade40() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
        final Path authzChild = new Path("/user/authz/child2");
    ...
    @Test
    public // from the Attribute Provider, and not from HDFS.
    void testGetAclStatusReturnsProviderOwnerPerms_withUpgrade60() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
        final Path authzChild = new Path("/user/authz/child2");
    ...
    @Test
    public // from the Attribute Provider, and not from HDFS.
    void testGetAclStatusReturnsProviderOwnerPerms_withUpgrade80() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
        final Path authzChild = new Path("/user/authz/child2");
    ...
    @Test
    public // and re-thrown as plain ACE exceptions.
    void testSubClassedAccessControlExceptions_withUpgrade20() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        shouldThrowAccessException = true;
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // and re-thrown as plain ACE exceptions.
    void testSubClassedAccessControlExceptions_withUpgrade40() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        shouldThrowAccessException = true;
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // and re-thrown as plain ACE exceptions.
    void testSubClassedAccessControlExceptions_withUpgrade60() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        shouldThrowAccessException = true;
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // and re-thrown as plain ACE exceptions.
    void testSubClassedAccessControlExceptions_withUpgrade80() throws Exception {
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        shouldThrowAccessException = true;
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // attribute provider is configured) and not the underlying HDFS permissions.
    void testContentSummary_withUpgrade20() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // attribute provider is configured) and not the underlying HDFS permissions.
    void testContentSummary_withUpgrade40() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // attribute provider is configured) and not the underlying HDFS permissions.
    void testContentSummary_withUpgrade60() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // attribute provider is configured) and not the underlying HDFS permissions.
    void testContentSummary_withUpgrade80() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        final Path userPath = new Path("/user");
        final Path authz = new Path("/user/authz");
    ...
    @Test
    public // revert for future reference.
    void testAttrProviderWorksCorrectlyOnRenamedSnapshotPaths_withUpgrade20() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        DistributedFileSystem hdfs = miniDFS.getFileSystem();
        final Path parent = new Path("/user");
    ...
    @Test
    public // revert for future reference.
    void testAttrProviderWorksCorrectlyOnRenamedSnapshotPaths_withUpgrade40() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        DistributedFileSystem hdfs = miniDFS.getFileSystem();
        final Path parent = new Path("/user");
    ...
    @Test
    public // revert for future reference.
    void testAttrProviderWorksCorrectlyOnRenamedSnapshotPaths_withUpgrade60() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        DistributedFileSystem hdfs = miniDFS.getFileSystem();
        final Path parent = new Path("/user");
    ...
    @Test
    public // revert for future reference.
    void testAttrProviderWorksCorrectlyOnRenamedSnapshotPaths_withUpgrade80() throws Exception {
        runPermissionCheck = true;
        FileSystem fs = FileSystem.get(miniDFS.getConfiguration(0));
        DistributedFileSystem hdfs = miniDFS.getFileSystem();
        final Path parent = new Path("/user");
