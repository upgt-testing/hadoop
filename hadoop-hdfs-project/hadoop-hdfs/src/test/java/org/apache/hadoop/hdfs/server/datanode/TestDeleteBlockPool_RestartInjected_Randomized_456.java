 ...
 */
public class TestDeleteBlockPool_RestartInjected_Randomized_456 {
    ...
    public void testDeleteBlockPool() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_file_creation_fs1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DFSTestUtil.createFile(fs2, new Path("/beta"), 1024, (short) 2, 54);
            RestartFramework.at("after_file_creation_fs2").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_refresh_namenodes_dn1").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            dn1.deleteBlockPool(bpid1, true);
            RestartFramework.at("after_delete_blockpool_dn1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_shutdown_namenode").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            dn1 = cluster.getDataNodes().get(0);
            dn2 = cluster.getDataNodes().get(1);
            // Although namenode is shutdown, the bp offerservice is still running
            // on dn2
            ...
            RestartFramework.at("after_refresh_namenodes_dn2").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_delete_blockpool_dn2").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            dn1 = cluster.getDataNodes().get(0);
            dn2 = cluster.getDataNodes().get(1);
            ...
            RestartFramework.at("after_create_gamma_file").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dn1 = cluster.getDataNodes().get(0);
            dn2 = cluster.getDataNodes().get(1);
            fs2.setReplication(gammaFile, (short) 2);
            RestartFramework.at("after_set_replication").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDfsAdminDeleteBlockPool() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_file_creation_fs2").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            DataNode dn1 = cluster.getDataNodes().get(0);
            String bpid1 = cluster.getNamesystem(0).getBlockPoolId();
            String bpid2 = cluster.getNamesystem(1).getBlockPoolId();
            ...
            dn1.refreshNamenodes(nn1Conf);
            RestartFramework.at("after_refresh_namenodes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(1, dn1.getAllBpOs().size());
            ...
            RestartFramework.at("after_admin_delete_without_force").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dn1 = cluster.getDataNodes().get(0);
            assertFalse(0 == ret);
            cluster.getFsDatasetTestUtils(0).verifyBlockPoolExists(bpid2);
            String[] forceArgs = { "-deleteBlockPool", dn1Address, bpid2, "force" };
            ret = admin.run(forceArgs);
            RestartFramework.at("after_admin_delete_with_force").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            dn1 = cluster.getDataNodes().get(0);
            assertEquals(0, ret);
