 ...
 */
public abstract class FSAclBaseTest_RestartInjected_Randomized_123 {
    ...
    public void testRemoveAclEntries() throws IOException {
        ...
        RestartFramework.at("after_set_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_remove_acl_entries").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRemoveDefaultAcl() throws Exception {
        ...
        AclStatus s = fs.getAclStatus(path);
        AclEntry[] returned = s.getEntries().toArray(new AclEntry[0]);
        RestartFramework.at("after_remove_default_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDefaultAclNewFile() throws Exception {
        ...
        AclEntry[] returned = s.getEntries().toArray(new AclEntry[0]);
        assertArrayEquals(new AclEntry[] { aclEntry(ACCESS, USER, "foo", ALL), aclEntry(ACCESS, GROUP, READ_EXECUTE) }, returned);
        RestartFramework.at("after_set_default_acl").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDefaultAclNewDir() throws Exception {
        ...
        fs.mkdirs(dirPath);
        RestartFramework.at("after_mkdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_subdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
