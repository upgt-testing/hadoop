...
public class TestDelegationTokenFetcher_RestartInjected_Randomized_456 {
    ...
    public void testDelegationTokenWithoutRenewerViaRPC() throws Exception {
        ...
        try {
            ...
            DelegationTokenFetcher.saveDelegationToken(conf, fs, null, p);
            RestartFramework.at("after_save_token").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Credentials creds = Credentials.readTokenStorageFile(p, conf);
            Iterator<Token<?>> itr = creds.getAllTokens().iterator();
            assertTrue("token not exist error", itr.hasNext());
            final Token token = itr.next();
            assertNotNull("Token should be there without renewer", token);
            RestartFramework.at("after_read_token").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_print_tokens").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
                RestartFramework.at("before_renew_attempt").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
