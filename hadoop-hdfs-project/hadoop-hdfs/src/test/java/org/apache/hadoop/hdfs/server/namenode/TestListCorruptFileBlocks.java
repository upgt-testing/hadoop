...
public class TestListCorruptFileBlocks {
    ...
    public void testListCorruptFilesCorruptedBlock() throws Exception {
        ...
        try {
            ...
            File data_dir = MiniDockerDFSCluster.getFinalizedDir(storageDir, bpid);
            assertTrue("data directory does not exist", data_dir.exists());
            List<File> metaFiles = MiniDockerDFSCluster.getAllBlockFiles(data_dir);
    ...
    public void testListCorruptFileBlocksInSafeMode() throws Exception {
        ...
        try {
            ...
            // Set short retry timeouts so this test runs faster
            conf.setInt(HdfsClientConfigKeys.Retry.WINDOW_BASE_KEY, 10);
            cluster = new MiniDockerDFSCluster.Builder(conf).waitSafeMode(false).build();
            ...
            File data_dir = MiniDockerDFSCluster.getFinalizedDir(storageDir, cluster.getNamesystem().getBlockPoolId());
            assertTrue("data directory does not exist", data_dir.exists());
            List<File> metaFiles = MiniDockerDFSCluster.getAllBlockFiles(data_dir);
            assertTrue("Data directory does not contain any blocks or there was an " + "IO error", metaFiles != null && !metaFiles.isEmpty());
            File metaFile = metaFiles.get(0);
        ...
        try {
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j <= 1; j++) {
                    ...
                    File data_dir = MiniDockerDFSCluster.getFinalizedDir(storageDir, bpid);
                    List<File> metadataFiles = MiniDockerDFSCluster.getAllBlockMetadataFiles(data_dir);
     ...
     */
    private void assertCorruptFilesCount(MiniDockerDFSCluster cluster, int expectedCorrupt) {
        FSNamesystemInterface fs = cluster.getNameNode().getNamesystem();
