 ...
 */
public class TestNameNodeStatusMXBean_RestartInjected_Randomized_123 {
    ...
    public void testNameNodeStatusMXBean() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_initial_attributes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("before_final_attributes").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
    ...
    public void testNameNodeMXBeanSlowDisksEnabled() throws Exception {
        ...
        RestartFramework.at("after_cluster_built").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            ...
            RestartFramework.at("after_slow_disk_added").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            datanodes = cluster.getDataNodes();
            NameNode nn = cluster.getNameNode();
            DatanodeManager datanodeManager = nn.getNamesystem().getBlockManager().getDatanodeManager();
            MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
            ObjectName mxbeanName = new ObjectName("Hadoop:service=NameNode,name=NameNodeStatus");
            RestartFramework.at("before_wait_for_report").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nn = cluster.getNameNode();
            ...
            RestartFramework.at("after_report_ready").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            String slowDisksReport = (String) mbs.getAttribute(mxbeanName, "SlowDisksReport");
