 ...
 */
public class TestOfflineImageViewerForStoragePolicy_RestartInjected_Randomized_123 {

    private static final Logger LOG = LoggerFactory.getLogger(TestOfflineImageViewerForStoragePolicy_RestartInjected.class);
    ...
    public void testPBDelimitedWriterForStoragePolicy() throws Exception {
        RestartFramework.at("before_read_fsimage").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String expected = DFSTestUtil.readResoucePlainFile("testStoragePolicy.csv");
        RestartFramework.at("after_read_fsimage").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private String readStoragePolicyFromFsimageFile() throws Exception {
        ...
        if (OfflineImageViewerPB.run(new String[] { "-p", "Delimited", "-i", originalFsimage.getAbsolutePath(), "-o", delimitedOutput.getAbsolutePath(), "-sp" }) != 0) {
