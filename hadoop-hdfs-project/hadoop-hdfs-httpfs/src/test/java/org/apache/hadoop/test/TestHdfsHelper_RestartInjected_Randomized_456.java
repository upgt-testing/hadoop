...
public class TestHdfsHelper_RestartInjected_Randomized_456 extends TestDirHelper {
    ...
    private static synchronized MiniDFSCluster startMiniHdfs(Configuration conf) throws Exception {
        if (MINI_DFS == null) {
            ...
            RestartFramework.at("after_cluster_build").on(miniHdfs).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_filesystem_setup").on(miniHdfs).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            MINI_DFS = miniHdfs;
        }
        return MINI_DFS;
    }
