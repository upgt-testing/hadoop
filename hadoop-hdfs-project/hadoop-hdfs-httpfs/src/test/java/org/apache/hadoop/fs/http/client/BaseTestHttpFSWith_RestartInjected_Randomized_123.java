...
public abstract class BaseTestHttpFSWith_RestartInjected_Randomized_123 extends HFSTestCase {
    ...
    protected FileSystem getHttpFSFileSystem(Configuration conf) throws Exception {
    ...
    private void testCreate(Path path, boolean override) throws Exception {
        ...
        os.write(1);
        if (!isLocalFS()) {
            RestartFramework.at("after_create_write").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        os.close();
        fs.close();
    ...
    private void testAppend() throws Exception {
        if (!isLocalFS()) {
            ...
            os.write(1);
            os.close();
            RestartFramework.at("after_initial_file_creation").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.close();
            fs = getHttpFSFileSystem();
            os = fs.append(new Path(path.toUri().getPath()));
            os.write(2);
            RestartFramework.at("after_append_write").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            os.close();
            fs.close();
            fs = FileSystem.get(getProxiedFSConf());
            InputStream is = fs.open(path);
    ...
    private void testTruncate() throws Exception {
        if (!isLocalFS()) {
            ...
            RestartFramework.at("after_file_creation").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final int newLength = blockSize;
            boolean isReady = fs.truncate(file, newLength);
            assertTrue("Recovery is not expected.", isReady);
            RestartFramework.at("after_truncate").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testConcat() throws Exception {
        ...
        if (!isLocalFS()) {
            ...
            RestartFramework.at("after_files_creation").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.close();
            fs = getHttpFSFileSystem();
            fs.concat(path1, new Path[] { path2, path3 });
            RestartFramework.at("after_concat").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.close();
    ...
    private void testSetReplication() throws Exception {
        ...
        if (!isLocalFS()) {
            RestartFramework.at("after_set_replication").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        fs.close();
        fs = getHttpFSFileSystem();
        fs.setReplication(path, (short) 1);
        if (!isLocalFS()) {
            RestartFramework.at("after_replication_change").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        fs.close();
        fs = FileSystem.get(getProxiedFSConf());
        FileStatus status1 = fs.getFileStatus(path);
    ...
    private void testQuotaUsage() throws Exception {
        RestartFramework.at("after_file_creations").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_set_quota").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        if (isLocalFS()) {
        ...
        RestartFramework.at("after_storage_quota_set").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testSetXAttr() throws Exception {
        if (!isLocalFS()) {
            ...
            os.write(1);
            os.close();
            RestartFramework.at("after_file_create").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.close();
            ...
            RestartFramework.at("after_set_xattrs").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
    ...
    private static void assertSameAcls(FileSystem expected, FileSystem actual, Path path) throws IOException {
    ...
    private void testFileAcls() throws Exception {
        ...
        RestartFramework.at("after_remove_acl_entries").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        proxyAclStat = proxyFs.getAclStatus(path);
        httpfsAclStat = httpfs.getAclStatus(path);
        RestartFramework.at("after_set_acl").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertSameAcls(httpfsAclStat, proxyAclStat);
        assertSameAcls(httpfs, proxyFs, path);
        RestartFramework.at("after_remove_acl").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_modify_acl").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_file_create_for_acl").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        proxyAclStat = proxyFs.getAclStatus(path);
        httpfsAclStat = httpfs.getAclStatus(path);
        assertSameAcls(httpfsAclStat, proxyAclStat);
        assertSameAcls(httpfs, proxyFs, path);
    ...
    public BaseTestHttpFSWith_RestartInjected_Randomized_123(Operation operation) {
    ...
    private void testCreateSnapshot(String snapshotName) throws Exception {
        if (!this.isLocalFS()) {
            ...
            RestartFramework.at("after_snapshot_preconditions").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_create_snapshot").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void createSnapshotTestsPreconditions(Path snapshottablePath, Boolean allowSnapshot) throws Exception {
    ...
    private void createSnapshotTestsPreconditions(Path snapshottablePath) throws Exception {
        // Allow snapshot by default for snapshot test
        createSnapshotTestsPreconditions(snapshottablePath, true);
    }

    private void cleanSnapshotTests(Path snapshottablePath, String resultingSnapName) throws Exception {
        DistributedFileSystem distributedFs = (DistributedFileSystem) FileSystem.get(snapshottablePath.toUri(), this.getProxiedFSConf());
    ...
    private void testRenameSnapshot() throws Exception {
        if (!this.isLocalFS()) {
            ...
            RestartFramework.at("after_snapshot_creation").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            fs.renameSnapshot(snapshottablePath, "snap-to-rename", "snap-new-name");
            RestartFramework.at("after_rename_snapshot").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path snapshotsDir = new Path("/tmp/tmp-snap-test/.snapshot");
            FileStatus[] snapshotItems = fs.listStatus(snapshotsDir);
            assertTrue("Should have exactly one snapshot.", snapshotItems.length == 1);
            String resultingSnapName = snapshotItems[0].getPath().getName();
    ...
    private void testDeleteSnapshot() throws Exception {
        if (!this.isLocalFS()) {
            ...
            RestartFramework.at("after_snapshot_create_for_delete").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Path snapshotsDir = new Path("/tmp/tmp-snap-test/.snapshot");
            FileStatus[] snapshotItems = fs.listStatus(snapshotsDir);
            assertTrue("Should have exactly one snapshot.", snapshotItems.length == 1);
            fs.deleteSnapshot(snapshottablePath, "snap-to-delete");
            RestartFramework.at("after_delete_snapshot").on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster()).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void testGetSnapshotDiffIllegalParamCase(FileSystem fs, Path path, String oldsnapshotname, String snapshotname) throws IOException {
        try {
            if (fs instanceof HttpFSFileSystem) {
                HttpFSFileSystem httpFS = (HttpFSFileSystem) fs;
    ...
    private void verifyGetSnapshottableDirListing(FileSystem fs, DistributedFileSystem dfs) throws Exception {
    ...
    private void verifyGetServerDefaults(FileSystem fs, DistributedFileSystem dfs) throws Exception {
    ...
    private void verifyAccess(FileSystem fs, DistributedFileSystem dfs) throws Exception {
        Path p1 = new Path("/p1");
        dfs.mkdirs(p1);
