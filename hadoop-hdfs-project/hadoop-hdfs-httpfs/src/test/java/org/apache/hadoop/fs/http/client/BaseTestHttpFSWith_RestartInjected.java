...
public abstract class BaseTestHttpFSWith_RestartInjected extends HFSTestCase {
  ...
  private void testCreate(Path path, boolean override) throws Exception {
    ...
    os.write(1);
    if (!isLocalFS()) {
      RestartFramework.at("after_create_write")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("datanode")
          .withIndex(0)
  ...
  private void testAppend() throws Exception {
    if (!isLocalFS()) {
      ...
      os.write(2);
      RestartFramework.at("after_append_write")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private void testConcat() throws Exception {
    ...
    if (!isLocalFS()) {
      ...
      RestartFramework.at("after_files_creation")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("datanode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_concat")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private void testSetReplication() throws Exception {
    ...
    if (!isLocalFS()) {
      RestartFramework.at("after_set_replication")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private void testQuotaUsage() throws Exception {
    ...
    out.close();
    RestartFramework.at("after_file_creations")
        .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_storage_quota_set")
        .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  private void testSetXAttr() throws Exception {
    if (!isLocalFS()) {
      ...
      RestartFramework.at("after_set_xattrs")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private void testFileAcls() throws Exception {
    ...
    os.write(1);
    os.close();
    RestartFramework.at("after_file_create_for_acl")
        .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
        .restart("datanode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_modify_acl")
        .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_remove_acl_entries")
        .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
        .restart("namenode")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public BaseTestHttpFSWith_RestartInjected(Operation operation) {
  ...
  private void testCreateSnapshot(String snapshotName) throws Exception {
    if (!this.isLocalFS()) {
      ...
      RestartFramework.at("after_snapshot_preconditions")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private void testRenameSnapshot() throws Exception {
    if (!this.isLocalFS()) {
      ...
      fs.renameSnapshot(snapshottablePath, "snap-to-rename",
          "snap-new-name");
      RestartFramework.at("after_rename_snapshot")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  private void testDeleteSnapshot() throws Exception {
    if (!this.isLocalFS()) {
      ...
      RestartFramework.at("after_snapshot_create_for_delete")
          .on(((TestHdfsHelper) hdfsTestHelper).getMiniDFSCluster())
          .restart("namenode")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
