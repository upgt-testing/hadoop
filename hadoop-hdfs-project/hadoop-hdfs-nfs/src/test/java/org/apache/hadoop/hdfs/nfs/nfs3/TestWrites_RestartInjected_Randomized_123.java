...
public class TestWrites_RestartInjected_Randomized_123 {
    ...
    public void testWriteStableHow() throws IOException, InterruptedException {
        ...
        try {
            ...
            RestartFramework.at("after_file1_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Test DATA_SYNC
            byte[] buffer = new byte[10];
            for (int i = 0; i < 10; i++) {
                buffer[i] = (byte) i;
            }
            RestartFramework.at("before_data_sync_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_data_sync_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Readback
            ...
            RestartFramework.at("after_file2_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_file_sync_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_file_sync_write").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Readback
    ...
    public void testOOOWrites() throws IOException, InterruptedException {
        ...
        try {
            ...
            CREATE3Response createRsp = nfsd.create(createXdr.asReadOnlyWrap(), securityHandler, new InetSocketAddress("localhost", 1234));
            FileHandle handle = createRsp.getObjHandle();
            RestartFramework.at("after_ooo_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            byte[][] oooBuf = new byte[numOOO][bufSize];
            for (int i = 0; i < numOOO; i++) {
                Arrays.fill(oooBuf[i], (byte) i);
            }
            RestartFramework.at("before_ooo_writes").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int i = 0; i < numOOO; i++) {
            ...
            waitWrite(nfsd, handle, 60000);
            RestartFramework.at("after_ooo_writes").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testOverlappingWrites() throws IOException, InterruptedException {
        ...
        try {
            ...
            CREATE3Response createRsp = nfsd.create(createXdr.asReadOnlyWrap(), securityHandler, new InetSocketAddress("localhost", 1234));
            FileHandle handle = createRsp.getObjHandle();
            RestartFramework.at("after_overlapping_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            byte[] buffer = new byte[bufSize];
            for (int i = 0; i < bufSize; i++) {
                buffer[i] = (byte) i;
            }
            int[][] ranges = new int[][] { { 0, 10 }, { 5, 7 }, { 5, 5 }, { 10, 6 }, { 18, 6 }, { 20, 6 }, { 28, 4 }, { 16, 2 }, { 25, 4 } };
            RestartFramework.at("before_overlapping_writes").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            waitWrite(nfsd, handle, 60000);
            RestartFramework.at("after_overlapping_writes").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
