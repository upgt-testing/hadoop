 ...
 */
public class TestReaddir_RestartInjected_Randomized_456 {
    ...
    public void testReaddirBasic() throws IOException {
        ...
        RestartFramework.at("after_get_file_info").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Entry3 entry = dirents.get(0);
        assertTrue(entry.getName().equals("f3"));
        RestartFramework.at("after_first_readdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // When the cookie is deleted, list starts over no including dot, dotdot
        hdfs.delete(new Path(testdir + "/f2"), false);
        response = nfsd.readdir(xdr_req.asReadOnlyWrap(), securityHandler, new InetSocketAddress("localhost", 1234));
        dirents = response.getDirList().getEntries();
        RestartFramework.at("after_second_readdir").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // No dot, dotdot
    ...
    void testReaddirPlus() throws IOException {
        ...
        RestartFramework.at("after_get_file_info_plus").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Test start listing from f2
        status = nn.getRpcServer().getFileInfo(testdir + "/f2");
        long f2Id = status.getFileId();
        // Create related part of the XDR request
        ...
        // verifier
        xdr_req.writeLongAsHyper(0);
        RestartFramework.at("after_second_readdirplus").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // dirCount
        xdr_req.writeInt(100);
        // maxCount
        xdr_req.writeInt(1000);
        responsePlus = nfsd.readdirplus(xdr_req.asReadOnlyWrap(), securityHandler, new InetSocketAddress("localhost", 1234));
        RestartFramework.at("after_first_readdirplus").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        // When the cookie is deleted, list starts over no including dot, dotdot
        hdfs.delete(new Path(testdir + "/f2"), false);
        RestartFramework.at("after_file_deletion_plus").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        responsePlus = nfsd.readdirplus(xdr_req.asReadOnlyWrap(), securityHandler, new InetSocketAddress("localhost", 1234));
        direntPlus = responsePlus.getDirListPlus().getEntries();
        // No dot, dotdot
