...
public class TestExportsTable_RestartInjected_Randomized_123 {
    ...
    public void testHdfsExportPoint() throws IOException {
        ...
        try {
            ...
            RestartFramework.at("after_nfs_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testViewFsMultipleExportPoint() throws IOException {
        ...
        try {
            ...
            hdfs1.mkdirs(base1);
            hdfs2.mkdirs(base2);
            RestartFramework.at("after_dir_operations").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            final Nfs3 nfsServer = new Nfs3(config);
            nfsServer.startServiceInternal(false);
            RestartFramework.at("after_nfs_start").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            Mountd mountd = nfsServer.getMountd();
            RpcProgramMountd rpcMount = (RpcProgramMountd) mountd.getRpcProgram();
    ...
    public void testHdfsInternalExportPoint() throws IOException {
        ...
        try {
            ...
            hdfs.mkdirs(base);
            RestartFramework.at("after_export_dir_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Start nfs
            final Nfs3 nfsServer = new Nfs3(config);
            nfsServer.startServiceInternal(false);
            RestartFramework.at("after_nfs_start").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
