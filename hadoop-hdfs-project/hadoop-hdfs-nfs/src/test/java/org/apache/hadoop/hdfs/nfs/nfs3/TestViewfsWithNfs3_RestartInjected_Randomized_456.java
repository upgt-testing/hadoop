 ...
 */
public class TestViewfsWithNfs3_RestartInjected_Randomized_456 {
    ...
    public void testFileStatus() throws Exception {
        RestartFramework.at("before_nn2_getfileinfo").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        HdfsFileStatus status = nn1.getRpcServer().getFileInfo("/user1/file1");
        FileStatus st = viewFs.getFileStatus(new Path("/hdfs1/file1"));
        RestartFramework.at("after_nn1_getfileinfo").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assert.assertEquals(st.isDirectory(), status.isDirectory());
        HdfsFileStatus status2 = nn2.getRpcServer().getFileInfo("/user2/dir2");
        RestartFramework.at("before_nn1_getfileinfo").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_nn2_getfileinfo").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        FileStatus st2 = viewFs.getFileStatus(new Path("/hdfs2/dir2"));
        Assert.assertEquals(st2.isDirectory(), status2.isDirectory());
    }

    // Test for getattr
    private void testNfsGetAttrResponse(long fileId, int namenodeId, int expectedStatus) {
    ...
    public void testNfsAccessNN1() throws Exception {
        RestartFramework.at("after_nn1_access").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_nn1_access").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        HdfsFileStatus status = nn1.getRpcServer().getFileInfo("/user1/file1");
    ...
    public void testNfsAccessNN2() throws Exception {
        ...
        RestartFramework.at("after_nn2_access").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        int namenodeId = Nfs3Utils.getNamenodeId(config, hdfs2.getUri());
        RestartFramework.at("before_nn2_access").on(cluster).restart("namenode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        testNfsGetAttrResponse(status.getFileId(), namenodeId, Nfs3Status.NFS3_OK);
    }
    ...
    public void testWrongNfsAccess() throws Exception {
        ...
        RestartFramework.at("before_file_creation").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    // Test for write
    private void testNfsWriteResponse(long dirId, int namenodeId) throws Exception {
    ...
    public void testNfsWriteNN1() throws Exception {
        ...
        int namenodeId = Nfs3Utils.getNamenodeId(config, hdfs1.getUri());
        RestartFramework.at("after_write_nn1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_write_nn1").on(cluster).restart("datanode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNfsWriteNN2() throws Exception {
        ...
        int namenodeId = Nfs3Utils.getNamenodeId(config, hdfs2.getUri());
        RestartFramework.at("after_write_nn2").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_write_nn2").on(cluster).restart("datanode").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        testNfsWriteResponse(status.getFileId(), namenodeId);
    }

    // Test for rename
    private void testNfsRename(FileHandle fromDirHandle, String fromFileName, FileHandle toDirHandle, String toFileName, int expectedStatus) throws Exception {
        XDR xdrReq = new XDR();
    ...
    public void testNfsRenameMultiNN() throws Exception {
        ...
        RestartFramework.at("after_rename_multi_nn").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_rename_multi_nn").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNfsRenameSingleNN() throws Exception {
        ...
        Assert.assertEquals(statusBeforeRename.isDirectory(), false);
        RestartFramework.at("before_rename_single_nn").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_rename_single_nn").on(cluster).restart("namenode").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
